...
redundant moves. Constant values are tracked between instructions so moves from
a constant register become `const` instructions. Pure function calls with
literal arguments are folded to constants when possible. The optimizer runs
after code generation and before execution to trim unnecessary instructions.
