...
constants when possible. Frequently used integers, strings and booleans are
emitted once per function and reused via `Move` instructions so loops and group
by operations avoid repeated `Const` opcodes. The optimizer runs after code
