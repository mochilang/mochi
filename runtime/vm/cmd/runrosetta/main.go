...
func runCase(root, name string, bench bool) error {
	...
	if bench {
		...
		os.WriteFile(benchPath, js, 0o644)
	} else {
...
func updateChecklist(root string) error {
...
func main() {
	idx := flag.Int("index", 1, "program index")
	name := flag.String("name", "", "program name")
	flag.Parse()
	bench := os.Getenv("MOCHI_BENCHMARK") == "true" || os.Getenv("MOCHI_BENCHMARK") == "1"
	root := repoRoot()
	if *name != "" {
		if err := runCase(root, *name, bench); err != nil {
	...
	*name = strings.TrimSuffix(names[*idx-1], ".mochi")
	...
	if err := updateChecklist(root); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
}
