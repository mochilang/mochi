...
func (m *VM) call(fnIndex int, args []Value, trace []StackFrame) (Value, error) {
	...
	for len(stack) > 0 {
		...
		case OpEqual:
			lb := fr.regs[ins.B]
			rb := fr.regs[ins.C]
			if lb.Tag == ValueNull || rb.Tag == ValueNull {
				fr.regs[ins.A] = Value{Tag: ValueBool, Bool: false}
			} else {
				fr.regs[ins.A] = Value{Tag: ValueBool, Bool: valuesEqual(lb, rb)}
			}
		case OpNotEqual:
			lb := fr.regs[ins.B]
			rb := fr.regs[ins.C]
		...
		case OpEqualInt:
			...
			if lb.Tag == ValueNull || rb.Tag == ValueNull {
				...
				fr.regs[ins.A] = Value{Tag: ValueBool, Bool: toInt(lb) == toInt(rb)}
			}
		case OpEqualFloat:
			lb := fr.regs[ins.B]
			rb := fr.regs[ins.C]
			if lb.Tag == ValueNull || rb.Tag == ValueNull {
				...
				fr.regs[ins.A] = Value{Tag: ValueBool, Bool: toFloat(lb) == toFloat(rb)}
			}
		case OpLess:
			b := fr.regs[ins.B]
			c := fr.regs[ins.C]
