...
func (fc *funcCompiler) compileJoinQuery(q *parser.QueryExpr, dst int) {
	...
	fc.emit(q.Pos, Instr{Op: OpAdd, A: li, B: li, C: one2})
	...
	if joinType == "left" || joinType == "outer" {
		...
		fc.emit(join.Pos, Instr{Op: OpAdd, A: ri2, B: ri2, C: one3})
		...
		fc.emit(q.Pos, Instr{Op: OpAdd, A: li2, B: li2, C: one4})
	...
	if joinType == "right" || joinType == "outer" {
		...
		fc.emit(q.Pos, Instr{Op: OpAdd, A: li3, B: li3, C: one5})
		...
		fc.emit(join.Pos, Instr{Op: OpAdd, A: ri3, B: ri3, C: one6})
...
func (fc *funcCompiler) compileJoinQueryRight(q *parser.QueryExpr, dst int) {
	...
	fc.emit(q.Pos, Instr{Op: OpAdd, A: li, B: li, C: oneL})
	fc.emit(q.Pos, Instr{Op: OpJump, A: lstart})
	lend := len(fc.fn.Code)
	fc.fn.Code[ljmp].B = lend
	...
	fc.emit(join.Pos, Instr{Op: OpAdd, A: ri, B: ri, C: oneR})
	fc.emit(join.Pos, Instr{Op: OpJump, A: rstart})
	end := len(fc.fn.Code)
	fc.fn.Code[rjmp].B = end
}
