	...
	for len(stack) > 0 {
		...
		case OpEqual:
			...
			if b.Tag == ValueNull || c.Tag == ValueNull {
				...
				fr.regs[ins.A] = Value{Tag: ValueBool, Bool: valuesEqual(b, c)}
			}
		case OpNotEqual:
			b := fr.regs[ins.B]
			c := fr.regs[ins.C]
		...
		case OpEqualInt:
			...
			if bVal.Tag == ValueNull || cVal.Tag == ValueNull {
				...
				fr.regs[ins.A] = Value{Tag: ValueBool, Bool: toInt(bVal) == toInt(cVal)}
			}
		case OpEqualFloat:
			b := fr.regs[ins.B]
			c := fr.regs[ins.C]
...
func (fc *funcCompiler) foldCallValue(call *parser.CallExpr) (Value, bool) {
	...
	case "avg":
		...
		if lst, ok := toList(v); ok {
			...
			return Value{Tag: ValueFloat, Float: sum / float64(count)}, true
		}
	case "sum":
		if len(args) != 1 {
			return Value{}, false
