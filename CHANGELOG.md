...
**Mochi AOT v1.0** ships the complete MEP-45 pipeline: compile a single `.mochi` source to a native binary for every tier-1 platform with no external runtime dependency.

### New: `mochi build --target=c-aot`

Compile any Mochi program to a native binary:
...
Cross-compile for any tier-1 triple:

```
mochi build --target=c-aot --triple=aarch64-linux-musl --out=hello-aarch64 hello.mochi
```
...
| Triple | Notes |
|--------|-------|
| `x86_64-linux-gnu` | native on x86_64 Linux |
| `x86_64-linux-musl` | statically linked, zig cc |
| `aarch64-linux-gnu` | cross from any host, zig cc + qemu run-gate |
| `aarch64-linux-musl` | cross from any host, zig cc + qemu run-gate |
| `aarch64-darwin` | native on Apple silicon |
| `x86_64-darwin` | native on Intel Mac or cross from Apple silicon |
| `x86_64-windows-msvc` | clang-cl, GitHub Actions windows-2025 |
| `x86_64-windows-gnu` | zig cc + mingw |
| `wasm32-wasi` | wasmtime, no sched/setjmp |

### Language features compiled to native code

| Phase | Feature |
|-------|---------|
| 1 | Hello world, basic print |
| 2 | `int`, `float`, `bool`; arithmetic, comparison, short-circuit; `let`/`var`; `if`/`else`/`while`/`for`; user functions; type-to-string casts; math builtins |
| 3 | Records; `list<T>`, `map<K,V>`; nested collections; list/map equality; typed empty literals; list slice; index assignment |
| 4 | Sum types; `match` expression and statement; Maranget canonicalisation; exhaustiveness check |
| 5 | Closures (non-capturing + capturing); free-function shim; method shim |
| 6 | String ops; `str(x)` conversion; format-string interpolation; file I/O; UTF-8 validation |
| 7 | `panic`; `try { } catch e { }` exception handling; built-in error codes |
| 8 | Query DSL (`from x in src [where/order/skip/take] select expr`); inner/left/cross join; arena allocation; `loadCSV`/`saveCSV` |
| 9 | M:N work-stealing scheduler; `chan<T>` channels; `stream<T>` broadcast; agents; graceful shutdown |
| 10 | C-direct FFI; boxed `mochi_value_t`; Go/Python/JS FFI via subprocess JSON RPC |

### Build profiles

| Profile | Flags |
|---------|-------|
| `--profile=release` (default) | `-O2 -DNDEBUG` |
| `--profile=debug` | `-O0 -g` + ASan + UBSan |
| `--profile=fast-int` | `-O3 -DMOCHI_FAST_INT` (skips divide-by-zero check) |

### APE (Actually Portable Executables)

`--apex` produces a single binary that runs on Linux, macOS, Windows, FreeBSD, NetBSD, and OpenBSD.

### Caching

Build outputs are cached in `~/.mochi/cache/` (BLAKE3 content-addressed). Rebuild on unchanged source takes under 1 ms.

### Reproducibility

Release-profile binaries are reproducible across hosts. SHA-256 checksums are published with every release.

### Sanitiser matrix

Full fixture corpus passes ASan, UBSan, TSan, and MSan (where supported).

### Performance

Median wall-clock time and binary size within 2x of the Go backend on the benchmark suite (hello_world, sum_loop, fib_iter, fib_rec, list_sum).
