...
> Generated with [Mochi](https://github.com/mochi-lang/mochi) `v0.2.6`
...
```mochi
// for-in.mochi
// Demonstrates iteration over list, map, and range using `for ... in ...`
...
(program
  ...
  (for fruit
    (in (selector fruits))
    (block
      (call print (string "fruit: ") (selector fruit))
    )
  )
  (let scores
    (map
      (entry (string Alice) (int 90))
      (entry (string Bob) (int 82))
      (entry (string Charlie) (int 88))
    )
  )
  (for name
    (in (selector scores))
    (block
      (let score
        (index (selector scores) (selector name))
      )
      (call print (selector name) (string " scored ") (selector score))
    )
  )
  (for i
    (range (int 1) (int 5))
    (block
      (call print (string "number: ") (selector i))
    )
  )
  (let matrix
    (list
  ...
  (for row
    ...
    (block
      (for col
        (in (selector row))
        (block
          (call print (string "cell: ") (selector col))
        )
      )
    )
  )
