...
func write(b *strings.Builder, n *Node, indent string) {
	...
	// Check if all children are leaf nodes (no children of their own)
	allLeaf := true
	...
	if allLeaf {
		// Inline all leaf children
		for _, c := range n.Children {
			b.WriteString(" (" + c.Kind)
			if c.Value != nil {
				b.WriteString(fmt.Sprintf(" %v", c.Value))
			}
			b.WriteString(")")
		}
		b.WriteString(")")
	} else {
		// Multiline for nested children
		b.WriteString("\n")
		for _, c := range n.Children {
			write(b, c, indent+"  ")
			b.WriteString("\n")
...
func fromForStmt(f *parser.ForStmt) *Node {
	return &Node{
		...
		Value: f.Name,
		Children: []*Node{
			&Node{Kind: "range", Children: []*Node{
				FromExpr(f.Start),
				FromExpr(f.End),
			}},
			&Node{Kind: "block", Children: mapStatements(f.Body)},
		},
	}
}
