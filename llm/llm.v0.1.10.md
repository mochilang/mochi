...
// --- Add TypeVar support above FuncType ---
...
func (e typeError) Error() string {
	if e.Pos.Filename == "" {
		if e.Help != "" {
			return fmt.Sprintf("%s: %s\n   → help: %s", e.Pos, e.Msg, e.Help)
		}
		return fmt.Sprintf("%s: %s", e.Pos, e.Msg)
	}
	src, _ := os.ReadFile(e.Pos.Filename)
	lines := strings.Split(string(src), "\n")
	if int(e.Pos.Line) > len(lines) || e.Pos.Line == 0 {
		return fmt.Sprintf("%s: %s", e.Pos, e.Msg)
	}
	line := lines[e.Pos.Line-1]
	marker := strings.Repeat(" ", int(e.Pos.Column)-1) + "^"
	if e.Help != "" {
		return fmt.Sprintf("%s: %s\n   |\n%3d | %s\n   | %s\n   → help: %s",
			e.Pos, e.Msg, e.Pos.Line, line, marker, e.Help)
	}
	return fmt.Sprintf("%s: %s\n   |\n%3d | %s\n   | %s",
		e.Pos, e.Msg, e.Pos.Line, line, marker)
}
...
func checkStmt(s *parser.Statement, env *Env) error {
		...
		if s.Let.Type != nil {
			...
			if s.Let.Value != nil {
				...
				if !typ.Equal(exprType) {
					return typeError{Pos: s.Let.Pos, Msg: fmt.Sprintf("type mismatch: expected %s but got %s", typ, exprType)}
				}
			}
			...
			return typeError{Pos: s.Let.Pos, Msg: "let statement requires a type or value"}
		}
		env.SetVar(name, typ)
		return nil
	...
	case s.Assign != nil:
		...
		if err != nil {
			return typeError{Pos: s.Assign.Pos, Msg: err.Error()}
		}
		if !lhsType.Equal(rhsType) {
			return typeError{Pos: s.Assign.Pos, Msg: fmt.Sprintf("cannot assign %s to %s (expected %s)", rhsType, s.Assign.Name, lhsType)}
		}
		return nil
	...
	case s.Fun != nil:
		...
		for _, p := range s.Fun.Params {
			if p.Type == nil {
				return typeError{Pos: s.Fun.Pos, Msg: fmt.Sprintf("function param %s missing type", p.Name)}
			}
	...
	case s.Expect != nil:
		...
		if !t.Equal(BoolType{}) {
			return typeError{Pos: s.Expect.Pos, Msg: "expect expression must be boolean"}
		}
		return nil
	}
	return nil
...
func checkEquality(e *parser.Equality, env *Env, expected Type) (Type, error) {
	...
	for _, op := range e.Right {
		...
		if !left.Equal(right) {
			return nil, typeError{Pos: op.Pos, Msg: "incompatible types in equality expression"}
		}
		left = BoolType{}
	}
	return left, nil
...
func checkComparison(c *parser.Comparison, env *Env, expected Type) (Type, error) {
	...
	for _, op := range c.Right {
		...
		if !left.Equal(right) {
			return nil, typeError{Pos: op.Pos, Msg: "incompatible types in comparison expression"}
		}
		left = BoolType{}
	}
	return left, nil
...
func checkTerm(t *parser.Term, env *Env, expected Type) (Type, error) {
	...
	for _, op := range t.Right {
		...
		if !left.Equal(right) {
			return nil, typeError{Pos: op.Pos, Msg: "type mismatch in term expression"}
		}
	}
	return left, nil
}
...
func checkFactor(f *parser.Factor, env *Env, expected Type) (Type, error) {
	...
	for _, op := range f.Right {
		...
		if !left.Equal(right) {
			return nil, typeError{Pos: op.Pos, Msg: "type mismatch in factor expression"}
		}
	}
	return left, nil
}
...
func checkPrimary(p *parser.Primary, env *Env, expected Type) (Type, error) {
	...
	case p.Selector != nil:
		...
		if err != nil {
			return nil, typeError{Pos: p.Pos, Msg: err.Error()}
		}
	...
	case p.Call != nil:
		...
		if err != nil {
			return nil, typeError{
				Pos: p.Pos,
				Msg: fmt.Sprintf("unknown function: %s", p.Call.Func),
			}
		}
		...
		case FuncType:
			...
			if argCount > paramCount {
				return nil, typeError{
					Pos: p.Pos,
					Msg: fmt.Sprintf("too many arguments: expected %d, got %d", paramCount, argCount),
				}
			}
			...
			for i := 0; i < argCount; i++ {
				...
				if !argType.Equal(ft.Params[i]) {
					return nil, typeError{
						Pos: p.Pos,
						Msg: fmt.Sprintf("argument %d type mismatch: expected %s, got %s", i+1, ft.Params[i], argType),
					}
				}
			}
			...
			// Partially applied: return curried type
		...
		default:
			return nil, typeError{
				Pos: p.Pos,
				Msg: fmt.Sprintf("%s is not a function", p.Call.Func),
			}
		}
	...
	return nil, typeError{Pos: p.Pos, Msg: "invalid primary expression"}
}

func checkFunExpr(f *parser.FunExpr, env *Env, expected Type, pos lexer.Position) (Type, error) {
	// Try to resolve expected type as FuncType, if possible
	...
	// Step 1: Resolve parameter types
	for i, p := range f.Params {
		if p.Type != nil {
	...
	// Step 3: Create new function scope
	child := NewEnv(env)
	for i, p := range f.Params {
...
func (i *Interpreter) evalCall(c *parser.CallExpr) (any, error) {
	...
	if fn, ok := i.env.GetFunc(c.Func); ok {
		...
		if argCount > paramCount {
			return nil, fmt.Errorf("function %s expects %d arguments, got %d", c.Func, paramCount, argCount)
		}

		// Curry if not enough arguments
		if argCount < paramCount {
			...
			return closure{
				...
				Env:        i.env.Copy(),
				Args:       applied,
				FullParams: fn.Params, // track original full param list
			}, nil
		}
	...
	// Check for closure stored in variable
	...
	if err == nil {
		if cl, ok := val.(closure); ok {
			...
			if totalArgs > fullParamCount {
				return nil, fmt.Errorf("function expects %d arguments, got %d", fullParamCount, totalArgs)
			}

			// Evaluate and combine all arguments
			...
			// Curry again if still incomplete
			if totalArgs < fullParamCount {
				return closure{
					Fn: &parser.FunExpr{
			...
			// Fully apply all arguments
			child := types.NewEnv(cl.Env)
			if len(cl.FullParams) != len(allArgs) {
				return nil, fmt.Errorf("internal error: argument count mismatch")
			}
	...
	return nil, fmt.Errorf("undefined function or closure: %s", c.Func)
}
...
func applyIntBinary(l int, op string, r int) (any, error) {
	...
	return nil, fmt.Errorf("invalid int operator: %s", op)
}
...
func applyFloatBinary(l float64, op string, r float64) (any, error) {
	...
	return nil, fmt.Errorf("invalid float operator: %s", op)
}
...
func applyUnary(op string, val any) (any, error) {
	...
	case "-":
		...
		case float64:
			return -v, nil
		default:
			return nil, fmt.Errorf("invalid use of unary '-' on %T", val)
		}
	case "!":
		if b, ok := val.(bool); ok {
			return !b, nil
