...
func TestTypescryptTranspiler_VMValid_Golden(t *testing.T) {
...
func updateReadme() {
    ...
    outDir := filepath.Join(root, "tests", "transpiler", "x", "typescrypt")
    readmePath := filepath.Join(root, "transpiler", "x", "typescrypt", "README.md")
	...
	var buf bytes.Buffer
        buf.WriteString("# Mochi \u2192 Typescrypt Transpiler\n\n")
        buf.WriteString("This directory contains the experimental Typescrypt transpiler.\n")
        buf.WriteString("Generated sources for the golden tests live under `tests/transpiler/x/typescrypt`.\n\n")
...
func updateTasks() {
	root := repoRoot(&testing.T{})
        taskFile := filepath.Join(root, "transpiler", "x", "typescrypt", "TASKS.md")
	...
	buf.WriteString(fmt.Sprintf("## Progress (%s)\n", ts))
        fmt.Fprintf(&buf, "- Generated Typescrypt for %d/%d programs\n", compiled, total)
