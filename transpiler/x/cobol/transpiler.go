...
func Emit(p *Program) []byte {
	...
	buf.WriteString("*> Generated by Mochi COBOL transpiler\n")
...
func convertPrimary(p *parser.Primary, env *types.Env) (Expr, error) {
	...
	case p.Call != nil && p.Call.Func == "len" && len(p.Call.Args) == 1:
		if lit := literalExpr(p.Call.Args[0]); lit != nil {
			switch v := lit.(type) {
			case *StringLit:
				return &IntLit{Value: len(v.Value)}, nil
			}
		}
		if n, ok := literalLen(p.Call.Args[0]); ok {
...
func literalLen(e *parser.Expr) (int, bool) {
	if e == nil || e.Binary == nil || len(e.Binary.Right) != 0 {
		return 0, false
	}
	u := e.Binary.Left
	...
	if pf.Target.List != nil {
		return len(pf.Target.List.Elems), true
	}
	if pf.Target.Map != nil {
		return len(pf.Target.Map.Items), true
	}
	return 0, false
}
