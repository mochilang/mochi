...
func genStatsLoops() []Stmt {
	statDC := &DataClassDef{Name: "Stat", Fields: []DataClassField{{Name: "city", Type: "str"}, {Name: "count", Type: "int"}, {Name: "avg_age", Type: "float"}}}
	groupDC := &DataClassDef{Name: "StatsGroup", Fields: []DataClassField{{Name: "key", Type: "str"}, {Name: "items", Type: "list"}}}
	groups := &LetStmt{Name: "_stats_groups", Expr: &DictLit{}}
	setdefault := &RawExpr{Code: "g = _stats_groups.setdefault(person.city, StatsGroup(person.city, []))"}
	appendCall := &RawExpr{Code: "g.items.append(person)"}
	loop1 := &ForStmt{Var: "person", Iter: &Name{Name: "people"}, Body: []Stmt{&ExprStmt{Expr: setdefault}, &ExprStmt{Expr: appendCall}}}
	iterVals := &CallExpr{Func: &FieldExpr{Target: &Name{Name: "_stats_groups"}, Name: "values"}, Args: nil}
	appStat := &RawExpr{Code: "stats.append(Stat(city=g.key, count=len(g.items), avg_age=sum(p.age for p in g.items) / len(g.items)))"}
	loop2 := &ForStmt{Var: "g", Iter: iterVals, Body: []Stmt{&ExprStmt{Expr: appStat}}}
	return []Stmt{statDC, groupDC, groups, loop1, &LetStmt{Name: "stats", Expr: &ListLit{}}, loop2}
}
