...
func (u *UnaryExpr) emit(w io.Writer) error {
	if u.Op == "!" {
		// Collapse patterns like !(a in b) into the more idiomatic
		// "a not in b" form for Python.
		if b, ok := u.Expr.(*BinaryExpr); ok && b.Op == "in" {
			expr := &BinaryExpr{Left: b.Left, Op: "not in", Right: b.Right}
			return expr.emit(w)
		}
...
func emitStmtIndent(w io.Writer, s Stmt, indent string) error {
	...
	case *BenchStmt:
		...
		line := fmt.Sprintf("%s    print(json.dumps({\"duration_us\": (_bench_end - _bench_start)//1000, \"memory_bytes\": (_bench_mem_end - _bench_mem_start)*1024, \"name\": %q}, indent=2))\n", indent, st.Name)
...
func Emit(w io.Writer, p *Program, bench bool) error {
	...
	for _, s := range p.Stmts {
		...
		case *BenchStmt:
			...
			line := fmt.Sprintf("    print(json.dumps({\"duration_us\": (_bench_end - _bench_start)//1000, \"memory_bytes\": (_bench_mem_end - _bench_mem_start)*1024, \"name\": %q}, indent=2))\n", st.Name)
			if _, err := io.WriteString(w, line); err != nil {
				return err
			}
		case *BreakStmt:
