...
// IndexAssignStmt represents assignments like `xs[i] = expr`.
	...
	for _, st := range prog.Statements {
		...
		case st.Assign != nil:
			...
			if len(st.Assign.Index) == 1 && len(st.Assign.Field) == 0 {
				...
				p.Stmts = append(p.Stmts, &IndexAssignStmt{Name: st.Assign.Name, Index: idx, Expr: e})
			} else if len(st.Assign.Index) == 0 && len(st.Assign.Field) == 1 {
	...
	for _, s := range list {
		...
		case s.Assign != nil:
			...
			if len(s.Assign.Index) == 1 && len(s.Assign.Field) == 0 {
				...
				out = append(out, &IndexAssignStmt{Name: s.Assign.Name, Index: idx, Expr: e})
			} else if len(s.Assign.Index) == 0 && len(s.Assign.Field) == 1 {
				idx := &StringLit{Value: s.Assign.Field[0].Name}
				out = append(out, &IndexAssignStmt{Name: s.Assign.Name, Index: idx, Expr: e})
...
func stmtNode(s Stmt) *ast.Node {
	...
	case *IndexAssignStmt:
		return &ast.Node{Kind: "index_assign", Value: st.Name, Children: []*ast.Node{exprNode(st.Index), exprNode(st.Expr)}}
	case *ReturnStmt:
