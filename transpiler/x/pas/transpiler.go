...
// IndexAssignStmt assigns to a list element.
...
func (p *Program) Emit() []byte {
	...
	buf.WriteString("function Sum(a: array of integer): integer;\n")
	buf.WriteString("var i, s: integer;\n")
	buf.WriteString("begin\n")
	buf.WriteString("  s := 0;\n")
	buf.WriteString("  for i := Low(a) to High(a) do\n")
	buf.WriteString("    s := s + a[i];\n")
	buf.WriteString("  Sum := s;\n")
	buf.WriteString("end;\n")
	...
	for _, st := range prog.Statements {
			...
			if st.Let.Value != nil {
				...
				if vd.Type == "" {
					...
					case types.ListType:
						...
						case types.ListType:
							if _, ok := elem.Elem.(types.StringType); ok {
								vd.Type = "array of array of string"
							} else {
								vd.Type = "array of array of integer"
							}
						default:
		...
		case st.Assign != nil:
			if len(st.Assign.Index) > 0 {
				target, err := convertAssignTarget(env, st.Assign.Name, st.Assign.Index)
				if err != nil {
					return nil, err
				}
				...
				pr.Stmts = append(pr.Stmts, &IndexAssignStmt{Target: target, Expr: val})
			} else {
	...
	for _, st := range body {
		...
		case st.Assign != nil:
			if len(st.Assign.Index) > 0 {
				...
				out = append(out, &IndexAssignStmt{Target: target, Expr: val})
			} else {
				ex, err := convertExpr(env, st.Assign.Value)
				if err != nil {
					return nil, err
...
func convertAssignTarget(env *types.Env, name string, idx []*parser.IndexOp) (Expr, error) {
	pf := &parser.PostfixExpr{
		Target: &parser.Primary{Selector: &parser.SelectorExpr{Root: name}},
	}
	for _, op := range idx {
		pf.Ops = append(pf.Ops, &parser.PostfixOp{Index: op})
	}
	return convertPostfix(env, pf)
}
