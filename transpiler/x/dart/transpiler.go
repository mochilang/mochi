...
func (s *SumExpr) emit(w io.Writer) error {
	...
	_, err := io.WriteString(w, ".fold(0, (a, b) => a + b)")
	return err
}
...
func (m *MinExpr) emit(w io.Writer) error {
	...
	_, err := io.WriteString(w, ".reduce((a, b) => a < b ? a : b)")
	return err
}
...
func header() string {
	return "// Generated by Mochi Dart transpiler\n"
}
...
func convertPrimary(p *parser.Primary) (Expr, error) {
	...
	case p.Call != nil:
		...
		if p.Call.Func == "values" && len(p.Call.Args) == 1 {
			m, err := convertExpr(p.Call.Args[0])
			if err != nil {
				return nil, err
			}
