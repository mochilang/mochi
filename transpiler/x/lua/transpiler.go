...
type AppendExpr struct{ List, Item Expr }
type AvgExpr struct{ List Expr }
type SumExpr struct{ List Expr }
type ContainsExpr struct{ Collection, Value Expr }
type SubstringExpr struct{ S, I, J Expr }
...
func (a *AppendExpr) emit(w io.Writer) {
	io.WriteString(w, "(function(t,v) table.insert(t,v); return t end)(")
	a.List.emit(w)
	io.WriteString(w, ", ")
	a.Item.emit(w)
	io.WriteString(w, ")")
...
func (a *AvgExpr) emit(w io.Writer) {
	io.WriteString(w, "(function(t) local s=0 for _,v in ipairs(t) do s=s+v end if #t==0 then return 0 end return s/#t end)(")
	a.List.emit(w)
	io.WriteString(w, ")")
}

func (s *SumExpr) emit(w io.Writer) {
	io.WriteString(w, "(function(t) local s=0 for _,v in ipairs(t) do s=s+v end return s end)(")
	s.List.emit(w)
	io.WriteString(w, ")")
}

func (c *ContainsExpr) emit(w io.Writer) {
	io.WriteString(w, "(function(c,v) if type(c)==\"string\" then return string.find(c,v,1,true)~=nil elseif type(c)==\"table\" then for _,x in ipairs(c) do if x==v then return true end end end return false end)(")
	c.Collection.emit(w)
	io.WriteString(w, ", ")
	c.Value.emit(w)
	io.WriteString(w, ")")
...
func convertPrimary(p *parser.Primary) (Expr, error) {
	...
	case p.Call != nil:
		...
		case "str":
			return &CallExpr{Func: "tostring", Args: args}, nil
		case "substring":
			if len(args) == 3 {
				inc := &BinaryExpr{Left: args[1], Op: "+", Right: &IntLit{Value: 1}}
				return &CallExpr{Func: "string.sub", Args: []Expr{args[0], inc, args[2]}}, nil
			}
		case "append":
			if len(args) == 2 {
		...
		case "contains":
			if len(args) == 2 {
				return &ContainsExpr{Collection: args[0], Value: args[1]}, nil
			}
		}
