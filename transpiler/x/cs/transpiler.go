...
func Emit(prog *Program) []byte {
	...
	ts := gitTimestamp()
	hash := gitCommitHash()
	fmt.Fprintf(&buf, "// Generated by Mochi %s (commit %s) on %s\n", strings.TrimSpace(version), hash, ts)
	buf.WriteString("#pragma warning disable 0169, 0649, 0162\n")
	...
	if usesFmt {
		buf.WriteString("using System.Globalization;\n")
	}
