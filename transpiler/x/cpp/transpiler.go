...
var reserved = map[string]bool{
	...
	"extern": true, "false": true, "final": true, "float": true, "for": true, "friend": true,
	"goto": true, "if": true, "inline": true, "int": true, "long": true, "mutable": true,
	"namespace": true, "new": true, "noexcept": true, "not": true, "not_eq": true,
	"nullptr": true, "operator": true, "or": true, "or_eq": true, "override": true,
...
func (p *Program) write(w io.Writer) {
	...
	if usesAny {
		...
		fmt.Fprintln(w, "    if(v.type() == typeid(double)) { std::ostringstream ss; ss << std::setprecision(15) << std::any_cast<double>(v); return ss.str(); }")
	...
	// using a higher precision to preserve numeric accuracy.
...
func (b *BinaryExpr) emit(w io.Writer) {
	...
	if lr, ok := b.Right.(*ListLit); ok && len(lr.Elems) == 0 && strings.HasPrefix(lt0, "std::vector<") {
		lr.ElemType = elementTypeFromListType(lt0)
		rt0 = fmt.Sprintf("std::vector<%s>", lr.ElemType)
	}
...
func Transpile(prog *parser.Program, env *types.Env) (*Program, error) {
	...
	// reset state for each program to ensure deterministic output
	tmpCounter = 0
	useNow = false
	useMem = false
