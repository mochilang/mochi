...
func isLoopCounter(name string) bool {
	switch name {
	case "i", "j", "k", "l", "m", "n", "idx", "term", "chk_map":
...
func (wst *WhileStmt) emit(w io.Writer, indent int) {
	...
	if wst.Simple {
		...
		if len(wst.Vars) > 0 {
			...
			io.WriteString(w, "{:continue, _} -> nil\n")
		}
		for i := 0; i < indent+2; i++ {
			io.WriteString(w, "  ")
		}
...
func compileStmt(st *parser.Statement, env *types.Env) (Stmt, error) {
	...
	case st.Assign != nil:
		...
		if len(st.Assign.Index) == 1 && len(st.Assign.Field) == 1 {
			...
			fieldName := st.Assign.Field[0].Name
			field := &AtomLit{Name: ":" + fieldName}
			t, _ := env.GetVar(st.Assign.Name)
			switch base := t.(type) {
			case types.StructType:
				if ft, ok := base.Fields[fieldName]; ok {
					target := &IndexExpr{Target: &VarRef{Name: st.Assign.Name}, Index: field, UseMapSyntax: true}
					var inner Expr
					switch ft.(type) {
					case types.MapType, types.StructType:
						inner = &CallExpr{Func: "Map.put", Args: []Expr{target, idx, val}}
					default:
						inner = &CallExpr{Func: "List.replace_at", Args: []Expr{target, idx, val}}
					}
					call := &CallExpr{Func: "Map.put", Args: []Expr{&VarRef{Name: st.Assign.Name}, field, inner}}
					return &AssignStmt{Name: st.Assign.Name, Value: call}, nil
				}
			case types.ListType:
				switch base.Elem.(type) {
