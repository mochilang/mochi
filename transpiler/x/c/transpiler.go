...
func (a *AssignStmt) emit(w io.Writer, indent int) {
	...
	} else if lst, ok := a.Value.(*ListLit); ok && len(a.Indexes) == 0 && len(a.Fields) == 0 {
		if len(lst.Elems) == 0 {
			writeIndent(w, indent)
			io.WriteString(w, a.Name+" = NULL;\n")
		}
		writeIndent(w, indent)
		if _, declared := varTypes[a.Name]; declared {
...
func (p *Program) Emit() []byte {
	...
	if needStrConcat || needStrInt || needStrFloat || needStrBool || needStrListInt || needStrListStr || needSubstring || needAtoi || needCharAt || needInput || needNow || needUpper || needLower || needListAppendInt || needListAppendStr || needListAppendPtr || needListAppendDoublePtr || needListAppendDoubleNew || needListAppendStrPtr || needListAppendSizeT || needListAppendStrNew || needListSliceInt {
	...
	if needListSliceInt {
		buf.WriteString("static size_t list_slice_int_len;\n")
		buf.WriteString("static int* list_slice_int(const int *arr, size_t start, size_t end) {\n")
		buf.WriteString("    list_slice_int_len = end > start ? end - start : 0;\n")
		buf.WriteString("    int *out = malloc(list_slice_int_len * sizeof(int));\n")
		buf.WriteString("    for (size_t i = 0; i < list_slice_int_len; i++) out[i] = arr[start + i];\n")
		buf.WriteString("    return out;\n")
		buf.WriteString("}\n\n")
	}
	if needNow {
...
func convertUnary(u *parser.Unary) Expr {
	...
	if len(u.Value.Ops) >= 1 && len(u.Ops) == 0 {
		...
		for _, op := range u.Value.Ops {
			...
			if op.Index != nil && op.Index.Colon != nil && op.Index.Colon2 == nil && op.Index.Step == nil {
				...
				if strings.HasSuffix(typ, "[]") {
					needListSliceInt = true
					funcReturnTypes["list_slice_int"] = strings.TrimSuffix(typ, "[]") + "[]"
					current = &CallExpr{Func: "list_slice_int", Args: []Expr{current, sliceStart, sliceEnd}}
				} else {
					needSubstring = true
	...
	if len(u.Value.Ops) == 1 && u.Value.Ops[0].Index != nil &&
		...
		typ := inferExprType(currentEnv, base)
		if strings.HasSuffix(typ, "[]") {
			needListSliceInt = true
			funcReturnTypes["list_slice_int"] = strings.TrimSuffix(typ, "[]") + "[]"
			if end == nil {
				end = &CallExpr{Func: "len", Args: []Expr{base}}
			}
			return &CallExpr{Func: "list_slice_int", Args: []Expr{base, start, end}}
		}
		if typ == "const char*" {
			if end == nil {
