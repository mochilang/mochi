...
func (i *IndexExpr) emitExpr(w io.Writer) {
	...
	if vr, ok := i.Target.(*VarRef); ok && isMapVar(vr.Name) {
		...
		if keyT == "const char*" && valT == "int" {
			...
			if varTypes[vr.Name] == "MapSL" || varTypes[vr.Name] == "MapSS" || varTypes[vr.Name] == "MapSI" || varTypes[vr.Name] == "MapSD" || varTypes[vr.Name] == "MapIS" || varTypes[vr.Name] == "MapIL" {
	...
	if t := inferExprType(currentEnv, i.Target); t == "MapIS" {
		needMapGetIS = true
		funcReturnTypes["map_get_is"] = "const char*"
		io.WriteString(w, "map_get_is(")
		i.Target.emitExpr(w)
...
func compileStmt(env *types.Env, s *parser.Statement) (Stmt, error) {
	...
	case s.Var != nil:
		...
		if _, ok := valExpr.(*FieldExpr); ok && (declType == "MapSS" || declType == "MapSL" || declType == "MapSD" || declType == "MapIS" || declType == "MapSI" || declType == "MapIL") {
			if declType == "MapIS" {
		...
		if m, isMap := valExpr.(*MapLit); isMap {
			...
			if len(m.Items) == 0 {
				if valT == "const char*" && keyT == "const char*" {
					...
					var buf bytes.Buffer
					fmt.Fprintf(&buf, "MapSI %s = { NULL, NULL, 0, 0 };\n", s.Var.Name)
					return &RawStmt{Code: buf.String()}, nil
				}
			}
			var buf bytes.Buffer
