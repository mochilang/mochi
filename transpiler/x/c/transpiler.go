			...
			if op.Index != nil && op.Index.Colon != nil && op.Index.Colon2 == nil && op.Index.Step == nil {
				...
				if typ == "const char*" {
					...
					if vr, ok := current.(*VarRef); ok {
						...
						current = &CallExpr{Func: "_slice_int", Args: []Expr{current, &VarRef{Name: vr.Name + "_len"}, sliceStart, sliceEnd, &UnaryExpr{Op: "&", Expr: &VarRef{Name: currentVarName + "_len"}}}}
					} else {
						return nil
					}
				} else {
