...
func (p *Program) Emit() []byte {
	...
	for len(emitted) < len(structTypes) {
		progress := false
...
func Transpile(env *types.Env, prog *parser.Program) (*Program, error) {
	...
	for _, st := range prog.Statements {
		if st.Fun != nil {
			...
			p.Functions = append(p.Functions, &Function{Name: st.Fun.Name, Params: params, Body: body, Ret: ret})
			continue
		}
		if st.Let != nil {
...
func compileFunction(env *types.Env, name string, fn *parser.FunExpr) (*Function, error) {
	...
	retType := "int"
	if fn.Return != nil {
		retType = inferExprType(env, convertExpr(fn.ExprBody))
	...
	return &Function{Name: name, Params: params, Body: body, Ret: retType}, nil
}
...
func exprIsString(e Expr) bool {
	...
	case *CallExpr:
		...
		if currentEnv != nil {
			if t, err := currentEnv.GetVar(v.Func); err == nil {
...
func cTypeFromStruct(t types.Type) string {
	switch tt := t.(type) {
	case types.StringType:
		return "const char*"
	case types.BoolType:
