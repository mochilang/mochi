...
var (
	...
	needConcatStruct        map[string]bool
...
func (r *ReturnStmt) emit(w io.Writer, indent int) {
	...
	if lst, ok := r.Expr.(*ListLit); ok && strings.HasSuffix(currentFuncReturn, "[]") {
		dims := strings.Count(currentFuncReturn, "[]")
		base := currentFuncReturn
...
func (d *DeclStmt) emit(w io.Writer, indent int) {
	...
	if strings.HasSuffix(typ, "[][][]") || strings.HasSuffix(typ, "***") {
		...
		if lst, ok := d.Value.(*ListLit); ok && len(lst.Elems) > 0 {
			for i, e := range lst.Elems {
				if ll, ok2 := e.(*ListLit); ok2 {
					...
					fmt.Fprintf(w, "%s *%s_%d[%d] = {", base, d.Name, i, len(ll.Elems))
		...
		if call, ok := d.Value.(*CallExpr); ok {
			...
			if call.Func == "_slice_intptr" {
				fmt.Fprintf(w, "%s **%s = ", base, d.Name)
				d.Value.emitExpr(w)
				io.WriteString(w, ";\n")
				writeIndent(w, indent)
				fmt.Fprintf(w, "size_t %s_len = _slice_intptr_len;\n", d.Name)
				writeIndent(w, indent)
				fmt.Fprintf(w, "size_t *%s_lens = _slice_intptr_lens;\n", d.Name)
				writeIndent(w, indent)
				fmt.Fprintf(w, "size_t %s_lens_len = _slice_intptr_len;\n", d.Name)
				return
			}
			if call.Func == "_slice_doubleptr" {
				fmt.Fprintf(w, "%s **%s = ", base, d.Name)
				d.Value.emitExpr(w)
				io.WriteString(w, ";\n")
				writeIndent(w, indent)
				fmt.Fprintf(w, "size_t %s_len = _slice_doubleptr_len;\n", d.Name)
				writeIndent(w, indent)
			...
			if strings.HasPrefix(call.Func, "_slice_") {
				...
				if needSliceStruct != nil && needSliceStruct[baseName] {
					fmt.Fprintf(w, "%s *%s = ", baseName, d.Name)
					d.Value.emitExpr(w)
					io.WriteString(w, ";\n")
					writeIndent(w, indent)
					fmt.Fprintf(w, "size_t %s_len = _slice_%s_len;\n", d.Name, baseName)
					return
				}
			}
			fmt.Fprintf(w, "%s *%s = ", base, d.Name)
...
func (a *AssignStmt) emit(w io.Writer, indent int) {
	...
	if call, ok := a.Value.(*CallExpr); ok && len(a.Indexes) == 0 && len(a.Fields) == 0 {
		...
		if call.Func == "_slice_intptr" {
			sliceLen := "_slice_intptr_len"
			writeIndent(w, indent)
			if len(a.Fields) == 1 {
				fmt.Fprintf(w, "%s.%s_len = %s;\n", a.Name, a.Fields[0], sliceLen)
				writeIndent(w, indent)
				fmt.Fprintf(w, "%s.%s_lens = _slice_intptr_lens;\n", a.Name, a.Fields[0])
				writeIndent(w, indent)
				fmt.Fprintf(w, "%s.%s_lens_len = %s;\n", a.Name, a.Fields[0], sliceLen)
			} else if _, declared := varTypes[a.Name]; declared {
				fmt.Fprintf(w, "%s_len = %s;\n", a.Name, sliceLen)
				writeIndent(w, indent)
				fmt.Fprintf(w, "%s_lens = _slice_intptr_lens;\n", a.Name)
				writeIndent(w, indent)
		...
		if call.Func == "_slice_doubleptr" {
			sliceLen := "_slice_doubleptr_len"
			writeIndent(w, indent)
			if len(a.Fields) == 1 {
				fmt.Fprintf(w, "%s.%s_len = %s;\n", a.Name, a.Fields[0], sliceLen)
				writeIndent(w, indent)
				fmt.Fprintf(w, "%s.%s_lens = _slice_doubleptr_lens;\n", a.Name, a.Fields[0])
				writeIndent(w, indent)
				fmt.Fprintf(w, "%s.%s_lens_len = %s;\n", a.Name, a.Fields[0], sliceLen)
			} else if _, declared := varTypes[a.Name]; declared {
				fmt.Fprintf(w, "%s_len = %s;\n", a.Name, sliceLen)
				writeIndent(w, indent)
				fmt.Fprintf(w, "%s_lens = _slice_doubleptr_lens;\n", a.Name)
				writeIndent(w, indent)
				fmt.Fprintf(w, "%s_lens_len = %s;\n", a.Name, sliceLen)
			} else {
				fmt.Fprintf(w, "size_t %s_len = %s;\n", a.Name, sliceLen)
		...
		if strings.HasPrefix(call.Func, "_slice_") {
			baseName := strings.TrimPrefix(call.Func, "_slice_")
			if needSliceStruct != nil && needSliceStruct[baseName] {
				sliceLen := "_slice_" + baseName + "_len"
				writeIndent(w, indent)
				if len(a.Fields) == 1 {
					fmt.Fprintf(w, "%s.%s_len = %s;\n", a.Name, a.Fields[0], sliceLen)
				} else if _, declared := varTypes[a.Name]; declared {
...
func (m *MapLit) emitExpr(w io.Writer) {
	...
	if keyT == "const char*" && valT == "MapSD" {
		tmp := fmt.Sprintf("__map%d", tempCounter)
		tempCounter++
		fmt.Fprintf(w, "({const char** %s_keys = malloc(%d * sizeof(char*)); ", tmp, len(m.Items))
		fmt.Fprintf(w, "MapSD *%s_vals = malloc(%d * sizeof(MapSD)); ", tmp, len(m.Items))
		for i, it := range m.Items {
			fmt.Fprintf(w, "%s_keys[%d] = ", tmp, i)
			it.Key.emitExpr(w)
			io.WriteString(w, "; ")
		...
		fmt.Fprintf(w, "MapSMD %s = { %s_keys, %s_vals, %d, %d }; %s;})", tmp, tmp, tmp, len(m.Items), len(m.Items)+16, tmp)
		return
	}
	io.WriteString(w, "(MapIL){0}")
}
...
func (i *IndexExpr) emitExpr(w io.Writer) {
	...
	if vr, ok := i.Target.(*VarRef); ok && isMapVar(vr.Name) {
		...
		if keyT == "const char*" && valT == "MapSD" {
			needMapGetSMD = true
			funcReturnTypes["map_get_smd"] = "MapSD"
			io.WriteString(w, "map_get_smd(")
			if varTypes[vr.Name] == "MapSMD" {
				io.WriteString(w, vr.Name+".keys, ")
				io.WriteString(w, vr.Name+".vals, ")
				io.WriteString(w, vr.Name+".len, ")
			} else {
		...
		if keyT == "const char*" && mapValTypes[vr.Name] != "" && valT != "const char*" && valT != "double" && !strings.HasSuffix(valT, "[]") && valT != "MapSI" && valT != "MapSMI" && valT != "MapSD" {
		...
		if keyT == "int" && strings.HasSuffix(valT, "[]") {
			needMapGetIL = true
	...
	if t := inferExprType(currentEnv, i.Target); t == "MapSI" {
		needMapGetSI = true
		funcReturnTypes["map_get_si"] = "int"
		io.WriteString(w, "map_get_si(")
		i.Target.emitExpr(w)
	...
	if t := inferExprType(currentEnv, i.Target); t == "MapII" {
		needMapGetII = true
		funcReturnTypes["map_get_ii"] = "int"
		io.WriteString(w, "map_get_ii(")
		i.Target.emitExpr(w)
	...
	if t := inferExprType(currentEnv, i.Target); t == "MapIL" {
		needMapGetIL = true
		valT := "int[]"
		if vr, ok := i.Target.(*VarRef); ok {
			if vt, ok2 := mapValTypes[vr.Name]; ok2 {
	...
	i.Target.emitExpr(w)
	io.WriteString(w, "[")
	io.WriteString(w, "(int)({long long _mochi_idx = ")
	i.Index.emitExpr(w)
	io.WriteString(w, "; _mochi_idx < 0 ? ")
	emitLenExpr(w, i.Target)
	io.WriteString(w, " + _mochi_idx : _mochi_idx;})")
	io.WriteString(w, "]")
}
...
func (c *CallExpr) emitExpr(w io.Writer) {
	...
	if c.Func == "concat" && len(c.Args) == 2 {
		...
		if strings.Contains(base, "const char*") {
			...
			default:
				if base != "" {
					if needConcatStruct == nil {
						needConcatStruct = make(map[string]bool)
					}
					needConcatStruct[base] = true
					fmt.Fprintf(w, "(%s*)concat_generic(", base)
					c.Args[0].emitExpr(w)
					io.WriteString(w, ", ")
					emitLenExpr(w, c.Args[0])
					io.WriteString(w, ", ")
	...
	if strings.HasPrefix(c.Func, "_slice_") && len(c.Args) == 5 {
		base := strings.TrimPrefix(c.Func, "_slice_")
		if needSliceStruct != nil && needSliceStruct[base] {
			needSliceStruct[base] = true
			io.WriteString(w, c.Func+"(")
			c.Args[0].emitExpr(w)
			io.WriteString(w, ", ")
			c.Args[1].emitExpr(w)
	...
	if c.Func == "_slice_intptr" && len(c.Args) == 7 {
		needSliceIntPtr = true
		io.WriteString(w, "_slice_intptr(")
		c.Args[0].emitExpr(w)
		io.WriteString(w, ", ")
		c.Args[1].emitExpr(w)
		io.WriteString(w, ", ")
	...
	if c.Func == "_slice_doubleptr" && len(c.Args) == 7 {
		needSliceDoublePtr = true
		io.WriteString(w, "_slice_doubleptr(")
		c.Args[0].emitExpr(w)
		io.WriteString(w, ", ")
		c.Args[1].emitExpr(w)
		io.WriteString(w, ", ")
...
func (b *BinaryExpr) emitExpr(w io.Writer) {
	...
	if b.Op == "in" {
		...
		if fe, ok := b.Right.(*FieldExpr); ok {
			typ := inferExprType(currentEnv, b.Right)
...
func (p *Program) Emit() []byte {
	...
	if needSliceIntPtr {
		buf.WriteString("size_t _slice_intptr_len;\n")
		buf.WriteString("size_t *_slice_intptr_lens;\n")
	}
	...
	if needMapGetSMD || needMapSetSMD {
		buf.WriteString("typedef struct { const char **keys; MapSD *vals; size_t len; size_t cap; } MapSMD;\n\n")
	}
	if needJSONMapSD {
	...
	if needMapSetSMD {
		buf.WriteString("static void map_set_smd(MapSMD *m, const char *key, MapSD val) {\n")
		buf.WriteString("    for (size_t i = 0; i < m->len; i++) {\n")
		buf.WriteString("        if (strcmp(m->keys[i], key) == 0) { m->vals[i] = val; return; }\n")
		buf.WriteString("    }\n")
		buf.WriteString("    if (m->len >= m->cap) {\n")
		buf.WriteString("        m->cap = m->cap ? m->cap * 2 : 16;\n")
		buf.WriteString("        m->keys = realloc((void*)m->keys, m->cap * sizeof(char*));\n")
		buf.WriteString("        m->vals = realloc(m->vals, m->cap * sizeof(MapSD));\n")
		buf.WriteString("    }\n")
		buf.WriteString("    m->keys[m->len] = key; m->vals[m->len] = val; m->len++;\n")
		buf.WriteString("}\n\n")
	}
	if needMapGetSMD {
		buf.WriteString("static MapSD map_get_smd(const char *keys[], const MapSD vals[], size_t len, const char *key) {\n")
		buf.WriteString("    for (size_t i = 0; i < len; i++) {\n")
		buf.WriteString("        if (strcmp(keys[i], key) == 0) return vals[i];\n")
		buf.WriteString("    }\n")
		buf.WriteString("    return (MapSD){0};\n")
		buf.WriteString("}\n\n")
	}
	if needMapGetSD {
	...
	if needMapSetII {
		...
		buf.WriteString("    if (m->len >= m->cap) {\n")
		buf.WriteString("        m->cap = m->cap ? m->cap * 2 : 16;\n")
		buf.WriteString("        int *new_keys = malloc(m->cap * sizeof(int));\n")
		buf.WriteString("        int *new_vals = malloc(m->cap * sizeof(int));\n")
		buf.WriteString("        if (m->keys) memcpy(new_keys, m->keys, m->len * sizeof(int));\n")
		buf.WriteString("        if (m->vals) memcpy(new_vals, m->vals, m->len * sizeof(int));\n")
		buf.WriteString("        m->keys = new_keys;\n")
		buf.WriteString("        m->vals = new_vals;\n")
		buf.WriteString("    }\n")
		buf.WriteString("    m->keys[m->len] = key; m->vals[m->len] = val; m->len++;\n")
		buf.WriteString("}\n\n")
	}
	...
	if needMapGetIL {
		buf.WriteString("static void* map_get_il(const int keys[], void *vals[], const size_t lens[], size_t len, int key, size_t *out_len) {\n")
		buf.WriteString("    for (size_t i = 0; i < len; i++) {\n")
		buf.WriteString("        if (keys[i] == key) { if (out_len) *out_len = lens[i]; return vals[i]; }\n")
		buf.WriteString("    }\n")
		buf.WriteString("    if (out_len) *out_len = 0;\n")
		buf.WriteString("    return NULL;\n")
		buf.WriteString("}\n\n")
	...
	if needConcatIntPtr || needConcatLongLong || needConcatStr || needConcatDouble || len(needConcatStruct) > 0 {
	...
	if len(needConcatStruct) > 0 {
		buf.WriteString("static void* concat_generic(void *a, size_t a_len, const void *b, size_t b_len, size_t elem_size) {\n")
		buf.WriteString("    void *res = realloc(a, (a_len + b_len) * elem_size);\n")
		buf.WriteString("    if (b_len > 0) memcpy((char*)res + a_len * elem_size, b, b_len * elem_size);\n")
		buf.WriteString("    concat_len = a_len + b_len;\n")
		buf.WriteString("    return res;\n")
		buf.WriteString("}\n\n")
	}
	...
	if needSliceDoublePtr {
		buf.WriteString("static double** _slice_doubleptr(double **arr, size_t len, size_t *lens, int start, int end, size_t *out_len, size_t **out_lens) {\n")
		buf.WriteString("    if (start < 0) start = 0;\n")
		buf.WriteString("    if ((size_t)end > len) end = len;\n")
		buf.WriteString("    if (start > end) start = end;\n")
		buf.WriteString("    size_t n = end - start;\n")
		buf.WriteString("    *out_len = n;\n")
		buf.WriteString("    _slice_doubleptr_len = n;\n")
		buf.WriteString("    *out_lens = lens + start;\n")
		buf.WriteString("    _slice_doubleptr_lens = lens + start;\n")
		buf.WriteString("    return arr + start;\n")
		buf.WriteString("}\n\n")
	}
	if needSliceIntPtr {
		buf.WriteString("static long long** _slice_intptr(long long **arr, size_t len, size_t *lens, int start, int end, size_t *out_len, size_t **out_lens) {\n")
		buf.WriteString("    if (start < 0) start = 0;\n")
		buf.WriteString("    if ((size_t)end > len) end = len;\n")
		buf.WriteString("    if (start > end) start = end;\n")
		buf.WriteString("    size_t n = end - start;\n")
		buf.WriteString("    *out_len = n;\n")
		buf.WriteString("    _slice_intptr_len = n;\n")
		buf.WriteString("    *out_lens = lens + start;\n")
		buf.WriteString("    _slice_intptr_lens = lens + start;\n")
		buf.WriteString("    return arr + start;\n")
		buf.WriteString("}\n\n")
	}
	if needSliceStrPtr {
	...
	if len(needSliceStruct) > 0 {
		for typ := range needSliceStruct {
			fmt.Fprintf(&buf, "static struct %s* _slice_%s(struct %s *arr, size_t len, int start, int end, size_t *out_len) {\n", typ, typ, typ)
			buf.WriteString("    if (start < 0) start = 0;\n")
			buf.WriteString("    if ((size_t)end > len) end = len;\n")
			buf.WriteString("    if (start > end) start = end;\n")
			buf.WriteString("    size_t n = end - start;\n")
			buf.WriteString("    *out_len = n;\n")
			fmt.Fprintf(&buf, "    _slice_%s_len = n;\n", typ)
			buf.WriteString("    return arr + start;\n")
			buf.WriteString("}\n\n")
		}
	}
...
func compileStmt(env *types.Env, s *parser.Statement) (Stmt, error) {
	...
	case s.Let != nil:
		...
		if declType == "MapSS" {
			...
			needMapGetSD = true
		} else if declType == "MapSMD" {
			mapKeyTypes[s.Let.Name] = "const char*"
			mapValTypes[s.Let.Name] = "MapSD"
			needMapGetSMD = true
		}
		if m, isMap := valExpr.(*MapLit); isMap {
			...
			if strings.HasSuffix(valT, "[]") {
				...
				needMapGetSD = true
			} else if valT == "MapSD" && keyT == "const char*" {
				varTypes[s.Let.Name] = "MapSMD"
				needMapGetSMD = true
			...
			if strings.HasSuffix(valT, "[]") {
				...
				for i, it := range m.Items {
					if mv, ok := it.Value.(*MapLit); ok {
						...
						buf.WriteString("};\n")
						fmt.Fprintf(&buf, "%sdouble %s_vals_%d_vals[%d] = {", staticStr, s.Let.Name, i, len(mv.Items))
						for j, kv := range mv.Items {
							if j > 0 {
								buf.WriteString(", ")
							}
				...
				fmt.Fprintf(&buf, "%sMapSD %s_vals[%d] = {", staticStr, s.Let.Name, len(m.Items)+16)
				for i, it := range m.Items {
					if i > 0 {
						buf.WriteString(", ")
					}
			...
			if strings.HasSuffix(valT, "[]") {
				if keyT == "const char*" {
					...
					fmt.Fprintf(&buf, "MapIL %s = { %s_keys, %s_vals, %s_lens, %d, %d };\n", s.Let.Name, s.Let.Name, s.Let.Name, s.Let.Name, len(m.Items), len(m.Items)+16)
				}
				...
				fmt.Fprintf(&buf, "MapSMD %s = { %s_keys, %s_vals, %d, %d };\n", s.Let.Name, s.Let.Name, s.Let.Name, len(m.Items), len(m.Items)+16)
			} else if valT == "MapSI" && keyT == "const char*" {
	...
	case s.Var != nil:
		...
		if declType == "MapSS" || declType == "MapSL" || declType == "MapSD" || declType == "MapIS" || declType == "MapSI" || declType == "MapIL" || declType == "MapII" || declType == "MapSMD" {
			if declType == "MapIS" {
		...
		if m, isMap := valExpr.(*MapLit); isMap {
			...
			if strings.HasSuffix(valT, "[]") {
				...
				varTypes[s.Var.Name] = "MapSD"
				needMapGetSD = true
			} else if valT == "MapSD" && keyT == "const char*" {
				varTypes[s.Var.Name] = "MapSMD"
				needMapGetSMD = true
			} else if valT == "int" && keyT == "const char*" {
				varTypes[s.Var.Name] = "MapSI"
				needMapGetSI = true
			} else if valT == "MapSI" && keyT == "const char*" {
				varTypes[s.Var.Name] = "MapSMI"
				needMapGetSMI = true
			} else if valT == "int" && keyT == "int" {
				varTypes[s.Var.Name] = "MapII"
				needMapGetII = true
			} else {
			...
			if len(m.Items) == 0 {
				if valT == "const char*" && keyT == "const char*" {
					...
					var buf bytes.Buffer
					fmt.Fprintf(&buf, "MapSMI %s = { NULL, NULL, 0, 0 };\n", s.Var.Name)
					return &RawStmt{Code: buf.String()}, nil
				} else if valT == "MapSD" && keyT == "const char*" {
					var buf bytes.Buffer
					fmt.Fprintf(&buf, "MapSMD %s = { NULL, NULL, 0, 0 };\n", s.Var.Name)
					return &RawStmt{Code: buf.String()}, nil
				}
			}
			var buf bytes.Buffer
			...
			if strings.HasSuffix(valT, "[]") {
				...
				for i, it := range m.Items {
					if mv, ok := it.Value.(*MapLit); ok {
						fmt.Fprintf(&buf, "%sconst char* %s_vals_%d_keys[%d] = {", staticStr, s.Var.Name, i, len(mv.Items))
						for j, kv := range mv.Items {
							if j > 0 {
								buf.WriteString(", ")
							}
						...
						buf.WriteString("};\n")
						fmt.Fprintf(&buf, "%sint %s_vals_%d_vals[%d] = {", staticStr, s.Var.Name, i, len(mv.Items))
						for j, kv := range mv.Items {
							if j > 0 {
								buf.WriteString(", ")
							}
				...
				fmt.Fprintf(&buf, "%sMapSI %s_vals[%d] = {", staticStr, s.Var.Name, len(m.Items)+16)
				for i, it := range m.Items {
					if i > 0 {
						buf.WriteString(", ")
					}
					if mv, ok := it.Value.(*MapLit); ok {
						fmt.Fprintf(&buf, "{ %s_vals_%d_keys, %s_vals_%d_vals, %d, %d }", s.Var.Name, i, s.Var.Name, i, len(mv.Items), len(mv.Items)+16)
					} else {
						buf.WriteString("{0}")
					}
				}
				...
				for i, it := range m.Items {
					if mv, ok := it.Value.(*MapLit); ok {
						...
						buf.WriteString("};\n")
						fmt.Fprintf(&buf, "%sdouble %s_vals_%d_vals[%d] = {", staticStr, s.Var.Name, i, len(mv.Items))
						for j, kv := range mv.Items {
							if j > 0 {
								buf.WriteString(", ")
							}
				...
				fmt.Fprintf(&buf, "%sMapSD %s_vals[%d] = {", staticStr, s.Var.Name, len(m.Items)+16)
				for i, it := range m.Items {
					if i > 0 {
						buf.WriteString(", ")
					}
	...
	case s.Assign != nil:
		...
		if isMapVar(s.Assign.Name) && len(idxs) == 1 && len(fields) == 0 {
			...
			if keyT == "const char*" && strings.HasSuffix(valT, "[]") {
				...
				var buf bytes.Buffer
				buf.WriteString("map_set_sl(")
				if varTypes[s.Assign.Name] == "MapSL" || varTypes[s.Assign.Name] == "MapSS" || varTypes[s.Assign.Name] == "MapSI" || varTypes[s.Assign.Name] == "MapSD" || varTypes[s.Assign.Name] == "MapIS" || varTypes[s.Assign.Name] == "MapIL" {
...
func convertMatchExpr(n *parser.MatchExpr) Expr {
	...
	if inferExprType(currentEnv, target) == "const char*" {
		return target
	}
	var expr Expr
	...
	for _, p := range fn.Params {
		...
		if mt, ok := mochiT.(types.MapType); ok {
			if _, isStruct := structTypes[typ]; !isStruct {
				...
				} else if _, ok2 := mt.Value.(types.BoolType); ok2 {
					mapValTypes[p.Name] = "int"
				} else {
...
func convertUnary(u *parser.Unary) Expr {
			...
			if op.Index != nil && op.Index.Colon != nil && op.Index.Colon2 == nil && op.Index.Step == nil {
				...
				if typ == "const char*" {
					...
					case *VarRef:
						...
						current = &CallExpr{Func: "_slice_doubleptr", Args: []Expr{current, &VarRef{Name: v.Name + "_len"}, &VarRef{Name: v.Name + "_lens"}, sliceStart, sliceEnd, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_doubleptr_len"}}, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_doubleptr_lens"}}}}
					case *FieldExpr:
						lenField := &FieldExpr{Target: v.Target, Name: v.Name + "_len"}
						...
						needSliceDoublePtr = true
						funcReturnTypes["_slice_doubleptr"] = "double[][]"
						current = &CallExpr{Func: "_slice_doubleptr", Args: []Expr{current, lenField, lensField, sliceStart, sliceEnd, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_doubleptr_len"}}, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_doubleptr_lens"}}}}
					default:
						return nil
					}
				} else if typ == "long long[][]" {
					...
					case *VarRef:
						...
						needSliceIntPtr = true
						funcReturnTypes["_slice_intptr"] = "long long[][]"
						current = &CallExpr{Func: "_slice_intptr", Args: []Expr{current, &VarRef{Name: v.Name + "_len"}, &VarRef{Name: v.Name + "_lens"}, sliceStart, sliceEnd, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_intptr_len"}}, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_intptr_lens"}}}}
					case *FieldExpr:
						lenField := &FieldExpr{Target: v.Target, Name: v.Name + "_len"}
						lensField := &FieldExpr{Target: v.Target, Name: v.Name + "_lens"}
						if sliceEnd == nil {
						...
						needSliceIntPtr = true
						funcReturnTypes["_slice_intptr"] = "long long[][]"
						current = &CallExpr{Func: "_slice_intptr", Args: []Expr{current, lenField, lensField, sliceStart, sliceEnd, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_intptr_len"}}, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_intptr_lens"}}}}
					default:
						return nil
					}
					...
					if _, ok := structTypes[base]; ok {
						...
						case *VarRef:
							...
							if needSliceStruct == nil {
								needSliceStruct = make(map[string]bool)
							}
							needSliceStruct[base] = true
							fn := "_slice_" + base
							funcReturnTypes[fn] = base + "[]"
							current = &CallExpr{Func: fn, Args: []Expr{current, &VarRef{Name: v.Name + "_len"}, sliceStart, sliceEnd, &UnaryExpr{Op: "&", Expr: &VarRef{Name: fn + "_len"}}}}
						case *FieldExpr:
							lenField := &FieldExpr{Target: v.Target, Name: v.Name + "_len"}
							if sliceEnd == nil {
								sliceEnd = lenField
							...
							fn := "_slice_" + base
							funcReturnTypes[fn] = base + "[]"
							current = &CallExpr{Func: fn, Args: []Expr{current, lenField, sliceStart, sliceEnd, &UnaryExpr{Op: "&", Expr: &VarRef{Name: fn + "_len"}}}}
						default:
							return nil
						}
					} else {
		...
		if simple {
			if castType != "" {
				...
				case "int":
					...
					if t := inferExprType(currentEnv, current); t == "const char*" {
						needAtoi = true
						funcReturnTypes["_atoi"] = "long long"
						return &CallExpr{Func: "_atoi", Args: []Expr{current}}
					}
		...
		if call.Func == "slice" && len(call.Args) == 3 {
			...
			case *VarRef:
				...
				if typ == "long long[][]" {
					needSliceIntPtr = true
					funcReturnTypes["_slice_intptr"] = "long long[][]"
					return &CallExpr{Func: "_slice_intptr", Args: []Expr{arg0, lenVar, &VarRef{Name: v.Name + "_lens"}, arg1, arg2, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_intptr_len"}}, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_intptr_lens"}}}}
				}
				if typ == "const char*[]" {
					needSliceStr = true
					funcReturnTypes["_slice_str"] = "const char*[]"
			...
			case *FieldExpr:
				...
				if typ == "long long[][]" {
					needSliceIntPtr = true
					funcReturnTypes["_slice_intptr"] = "long long[][]"
					return &CallExpr{Func: "_slice_intptr", Args: []Expr{arg0, lenField, &FieldExpr{Target: v.Target, Name: v.Name + "_lens"}, arg1, arg2, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_intptr_len"}}, &UnaryExpr{Op: "&", Expr: &VarRef{Name: "_slice_intptr_lens"}}}}
				}
				if typ == "const char*[]" {
					needSliceStr = true
					funcReturnTypes["_slice_str"] = "const char*[]"
	...
	if st := u.Value.Target.Struct; st != nil && len(u.Ops) == 0 {
		if ut, ok := currentEnv.FindUnionByVariant(st.Name); ok && cTypeFromMochiType(ut) == "const char*" {
			if len(st.Fields) == 1 {
				val := convertExpr(st.Fields[0].Value)
				if val == nil {
					return nil
				}
				if variant, ok := ut.Variants[st.Name]; ok {
					if ft, ok := variant.Fields[st.Fields[0].Name]; ok {
						switch ft.(type) {
						case types.StringType:
							return val
						case types.FloatType:
							needStrFloat = true
							return &CallExpr{Func: "str_float", Args: []Expr{val}}
						case types.BoolType:
							needStrBool = true
							return &CallExpr{Func: "str_bool", Args: []Expr{val}}
						default:
							needStrInt = true
							return &CallExpr{Func: "str_int", Args: []Expr{val}}
						}
					}
				}
				return val
			...
			if lst, ok := e.(*ListLit); ok && len(lst.Elems) > 0 {
				if sub, ok2 := lst.Elems[0].(*ListLit); ok2 {
					...
					for _, it := range sub.Elems {
						...
						if inferExprType(env, it) == "double" || exprIsFloat(it) {
							hasFloat = true
							break
						}
					}
		...
		if _, ok := tt.Key.(types.StringType); ok {
			...
			if inner, ok2 := tt.Value.(types.MapType); ok2 {
				if _, ok3 := inner.Key.(types.StringType); ok3 {
					...
					if _, ok4 := inner.Value.(types.FloatType); ok4 {
						return "MapSMD"
					}
				}
			}
