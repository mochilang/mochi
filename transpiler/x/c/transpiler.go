...
func (p *Program) Emit() []byte {
	...
	if needSliceInt {
		buf.WriteString("static size_t slice_int_len;\n")
		buf.WriteString("static int* slice_int(const int *arr, size_t len, int start, int end) {\n")
		buf.WriteString("    if (start < 0) start = 0;\n")
		buf.WriteString("    if (end > (int)len) end = (int)len;\n")
		buf.WriteString("    if (start > end) start = end;\n")
		buf.WriteString("    size_t n = end - start;\n")
		buf.WriteString("    int *res = malloc(n * sizeof(int));\n")
		buf.WriteString("    for(size_t i=0;i<n;i++) res[i] = arr[start+i];\n")
		buf.WriteString("    slice_int_len = n;\n")
		buf.WriteString("    return res;\n")
		buf.WriteString("}\n\n")
	}
	if needNow {
			...
			if op.Index != nil && op.Index.Colon != nil && op.Index.Colon2 == nil && op.Index.Step == nil {
				...
				if tname == "const char*" {
					...
					if vr, ok := current.(*VarRef); ok {
						needSliceInt = true
						funcReturnTypes["slice_int"] = "int[]"
						current = &CallExpr{Func: "slice_int", Args: []Expr{current, &VarRef{Name: vr.Name + "_len"}, sliceStart, sliceEnd}}
					} else {
						return nil
					}
				} else {
