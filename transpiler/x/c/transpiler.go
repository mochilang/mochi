...
func compileStmt(env *types.Env, s *parser.Statement) (Stmt, error) {
	...
	case s.Let != nil:
		...
		if m, isMap := valExpr.(*MapLit); isMap {
			...
			var buf bytes.Buffer
                       fmt.Fprintf(&buf, "%s %s_keys[%d] = {", keyT, s.Let.Name, len(m.Items)+64)
			for i, it := range m.Items {
				if i > 0 {
					buf.WriteString(", ")
				}
			...
			if strings.HasSuffix(valT, "[]") {
				...
				for i, it := range m.Items {
					if lst, ok := it.Value.(*ListLit); ok {
                       ...
                       fmt.Fprintf(&buf, "const %s* %s_vals[%d] = {", base, s.Let.Name, len(m.Items)+64)
				for i := range m.Items {
					if i > 0 {
						buf.WriteString(", ")
					}
				...
				buf.WriteString("};\n")
                       ...
                       fmt.Fprintf(&buf, "const char* %s_vals[%d] = {", s.Let.Name, len(m.Items)+64)
				for i, it := range m.Items {
					if i > 0 {
						buf.WriteString(", ")
					}
				...
				buf.WriteString("};\n")
                       ...
                       fmt.Fprintf(&buf, "%s %s_vals[%d] = {", valT, s.Let.Name, len(m.Items)+64)
				for i, it := range m.Items {
					if i > 0 {
						buf.WriteString(", ")
					}
	...
	case s.Var != nil:
		...
		if m, isMap := valExpr.(*MapLit); isMap {
			...
			var buf bytes.Buffer
                       fmt.Fprintf(&buf, "static %s %s_keys[%d] = {", keyT, s.Var.Name, len(m.Items)+64)
			for i, it := range m.Items {
				if i > 0 {
					buf.WriteString(", ")
				}
			...
			if strings.HasSuffix(valT, "[]") {
				...
				for i, it := range m.Items {
					if lst, ok := it.Value.(*ListLit); ok {
						...
						buf.WriteString("};\n")
                               ...
                               fmt.Fprintf(&buf, "static const %s* %s_vals[%d] = {", base, s.Var.Name, len(m.Items)+64)
				for i := range m.Items {
					if i > 0 {
						buf.WriteString(", ")
					}
				...
				buf.WriteString("};\n")
                               ...
                               fmt.Fprintf(&buf, "static const char* %s_vals[%d] = {", s.Var.Name, len(m.Items)+64)
				for i, it := range m.Items {
					if i > 0 {
						buf.WriteString(", ")
					}
				...
				buf.WriteString("};\n")
                               ...
                               fmt.Fprintf(&buf, "static %s %s_vals[%d] = {", valT, s.Var.Name, len(m.Items)+64)
				for i, it := range m.Items {
					if i > 0 {
						buf.WriteString(", ")
					}
