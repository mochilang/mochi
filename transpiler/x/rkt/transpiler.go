...
// dataDir stores the directory of the source program for resolving auxiliary
// files like those used by read_file.
var dataDir string
...
func sanitizeName(name string) string {
	...
	case "exp":
		...
		return "exp_"
	case "force":
		// avoid shadowing Racket's `force` procedure used for promises
		return "force_"
	case "map":
...
func (l *ListAssignStmt) emit(w io.Writer) {
	...
	fmt.Fprintf(w, "(vector-set! %s (int ", name)
	l.Index.emit(w)
	io.WriteString(w, ") ")
	l.Expr.emit(w)
	io.WriteString(w, ")\n")
...
func header() string {
	...
	if dataDir != "" {
		hdr += fmt.Sprintf("(define _data_dir %q)\n", dataDir)
	}
	...
	hdr += "(define (_fetch url [opts #f]) (let ([p (get-pure-port (string->url url))]) (define txt (port->string p)) (close-input-port p) (string->jsexpr txt)))\n"
...
func Transpile(prog *parser.Program, env *types.Env, bench bool) (*Program, error) {
	...
	if prog != nil {
		dir := filepath.Dir(prog.Pos.Filename)
