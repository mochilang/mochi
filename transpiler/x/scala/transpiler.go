...
var needsFloorDiv bool
var needsFloorMod bool
...
var aliasVars map[string]*FieldExpr
...
func Emit(p *Program) []byte {
	...
	if needsStr {
		...
		buf.WriteString("    case d: Double => { val i = d.toLong; if (d == i.toDouble) java.lang.Long.toString(i) else java.lang.Double.toString(d) }\n")
	...
	if needsSHA256 {
		needsBigInt = true
		buf.WriteString("  private def _sha256(bytes: Array[Byte]): ArrayBuffer[BigInt] = {\n")
		...
		buf.WriteString("    ArrayBuffer(sum.map(b => BigInt(b & 0xff)): _*)\n")
		buf.WriteString("  }\n\n")
	}
	if needsMD5 {
