...
func TestScalaTranspiler_StringConcat(t *testing.T) {
	if _, err := exec.LookPath("scalac"); err != nil {
		t.Skip("scalac not installed")
	}
	if _, err := exec.LookPath("scala"); err != nil {
	...
	code := scalat.Emit(ast)
	scalaFile := filepath.Join(outDir, "string_concat.scala")
	if err := os.WriteFile(scalaFile, code, 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	tmp := t.TempDir()
...
func TestScalaTranspiler_StringCompare(t *testing.T) {
	if _, err := exec.LookPath("scalac"); err != nil {
		t.Skip("scalac not installed")
	}
	if _, err := exec.LookPath("scala"); err != nil {
	...
	code := scalat.Emit(ast)
	scalaFile := filepath.Join(outDir, "string_compare.scala")
	if err := os.WriteFile(scalaFile, code, 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	tmp := t.TempDir()
