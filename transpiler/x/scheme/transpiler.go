...
func convertStmt(st *parser.Statement) (Node, error) {
	...
	case st.Expr != nil:
		...
		if call != nil && call.Func == "print" && len(call.Args) == 1 && len(st.Expr.Expr.Binary.Right) == 0 {
			...
			return &List{Elems: []Node{Symbol("mochi-print"), arg}}, nil
		}
...
func convertCall(target Node, call *parser.CallOp) (Node, error) {
	...
	case "avg":
		...
		return &List{Elems: []Node{
			...
			&List{Elems: []Node{Symbol("inexact"),
	...
	case "min":
		...
		return &List{Elems: []Node{Symbol("apply"), Symbol("min"), args[0]}}, nil
	case "max":
		if len(args) != 1 {
			return nil, fmt.Errorf("max expects 1 arg")
		}
		return &List{Elems: []Node{Symbol("apply"), Symbol("max"), args[0]}}, nil
	case "substring":
		if len(args) != 3 {
