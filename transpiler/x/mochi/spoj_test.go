...
func TestMochiTranspiler_SPOJ_Golden(t *testing.T) {
	...
	src := filepath.Join(root, "tests", "spoj", "x", "mochi", "13.mochi")
	outDir := filepath.Join(root, "tests", "spoj", "x", "mochi")
	outPath := filepath.Join(outDir, "13.out")
	errPath := filepath.Join(outDir, "13.error")
	...
	binPath := filepath.Join(outDir, "mochi_bin")
	build := exec.Command("go", "build", "-o", binPath, "./cmd/mochi")
	build.Dir = root
	if bout, berr := build.CombinedOutput(); berr != nil {
		_ = os.WriteFile(errPath, append([]byte("build: "+berr.Error()+"\n"), bout...), 0o644)
		t.Fatalf("build: %v", berr)
	}

	cmd := exec.Command(binPath, "run", src)
	envv := os.Environ()
	if !bench {
		envv = append(envv, "MOCHI_NOW_SEED=1")
	}
	cmd.Env = envv
	inPath := filepath.Join(outDir, "13.in")
	...
	if bench {
		benchPath := filepath.Join(outDir, "13.bench")
...
func updateSPOJ() {
	...
	dur := ""
	mem := ""
	if _, err := os.Stat(filepath.Join(outDir, "13.error")); err == nil {
		status = "error"
	} else if _, err := os.Stat(filepath.Join(outDir, "13.mochi")); err == nil {
		status = "✓"
	}
	if data, err := os.ReadFile(filepath.Join(outDir, "13.bench")); err == nil {
		var r struct {
	...
	buf.WriteString("## SPOJ Golden Test Checklist (" + fmt.Sprintf("%d/1", compiled) + ")\n")
	buf.WriteString("| Index | Name | Status | Duration | Memory |\n")
	buf.WriteString("|------:|------|:-----:|---------:|-------:|\n")
	buf.WriteString("| 13 | HOTLINE - Hotline | " + status + " | " + dur + " | " + mem + " |\n")
