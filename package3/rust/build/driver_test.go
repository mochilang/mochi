...
func TestNewDriverDefaultCacheDir(t *testing.T) {
	...
	d := NewDriver(Options{})
	if got := d.CacheDir(); got != "/tmp/xdg-test-cache/mochi/rust-deps" {
		t.Errorf("CacheDir() = %q; want /tmp/xdg-test-cache/mochi/rust-deps", got)
	}
}
...
func TestWriteWorkspaceRoot(t *testing.T) {
	...
	if !strings.Contains(string(manifest), `"mochi_user"`) {
		t.Errorf("Cargo.toml missing member mochi_user:\n%s", manifest)
	}
	gitignore, err := os.ReadFile(filepath.Join(root, ".gitignore"))
	if err != nil {
		t.Fatalf("read .gitignore: %v", err)
	}
	if string(gitignore) != "target/\n" {
		t.Errorf(".gitignore = %q; want target/\\n", gitignore)
	}
}
...
func TestWriteWorkspaceRootRejectsBadWorkspace(t *testing.T) {
	...
	bad := &Workspace{Resolver: "1"}
	if _, err := d.WriteWorkspaceRoot(bad); err == nil {
		t.Errorf("WriteWorkspaceRoot accepted invalid resolver; expected error")
	}
}
...
func TestDefaultCacheDirFallbacks(t *testing.T) {
	...
	if got := defaultCacheDir(); got != "/xdg-home/mochi/rust-deps" {
		t.Errorf("defaultCacheDir() with XDG = %q; want /xdg-home/mochi/rust-deps", got)
	}
	// Without XDG, fall back to HOME/.cache/.
	t.Setenv("XDG_CACHE_HOME", "")
	t.Setenv("HOME", "/home/test")
	if got := defaultCacheDir(); got != "/home/test/.cache/mochi/rust-deps" {
		t.Errorf("defaultCacheDir() with HOME = %q; want /home/test/.cache/mochi/rust-deps", got)
	}
}
