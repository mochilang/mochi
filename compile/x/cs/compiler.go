...
func New(env *types.Env) *Compiler {
	return &Compiler{env: env, helpers: make(map[string]bool), varTypes: make(map[string]string), packages: make(map[string]bool), structs: make(map[string]bool), inFun: 0, useStream: false, models: false, sourceComments: true}
}
...
func (c *Compiler) Compile(prog *parser.Program) ([]byte, error) {
	var src string
	if c.sourceComments && prog != nil && prog.Pos.Filename != "" {
		if data, err := os.ReadFile(prog.Pos.Filename); err == nil {
			src = string(data)
		}
	}
	return c.compile(prog, src)
}

// CompileWithSource compiles prog and includes the provided source code as a
// comment header when sourceComments is true.
func (c *Compiler) CompileWithSource(prog *parser.Program, src string) ([]byte, error) {
	return c.compile(prog, src)
}

func (c *Compiler) compile(prog *parser.Program, src string) ([]byte, error) {
	c.buf.Reset()
...
func (c *Compiler) compileUnary(u *parser.Unary) (string, error) {
	...
	for i := len(u.Ops) - 1; i >= 0; i-- {
		op := u.Ops[i]
		if strings.HasPrefix(val, "-") || strings.HasPrefix(val, "!") || i < len(u.Ops)-1 {
