...
The Ruby backend now supports running the TPCH `q1` and `q2` examples, the first ten JOB queries and the TPCDS `q1` through `q49` queries. Generated code and runtime output for JOB `q1` through `q10` are checked in under `tests/dataset/job/compiler/rb`, TPCH output for `q1` and `q2` under `tests/dataset/tpc-h/compiler/rb` and TPCDS output for `q1` through `q49` under `tests/dataset/tpc-ds/compiler/rb`.
