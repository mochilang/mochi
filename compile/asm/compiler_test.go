...
func TestAsmCompiler_SubsetPrograms(t *testing.T) {
	...
	run := func(src string) ([]byte, error) {
		...
		if out, err := exec.Command(cc, sfile, "-o", bin, "-no-pie").CombinedOutput(); err != nil {
...
// TestAsmCompiler_GoldenOutput verifies generated assembly output.
func TestAsmCompiler_GoldenOutput(t *testing.T) {
	...
	dirs := []string{"tests/compiler/asm"}
	for _, dir := range dirs {
		golden.Run(t, dir, ".mochi", ".s.out", compileFn)
	}
}
