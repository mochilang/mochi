...
func (c *Compiler) compileFor(stmt *parser.ForStmt) error {
	...
	if len(mutated) == 0 {
		c.writeln(fmt.Sprintf("Enum.reduce_while(%s, :ok, fn %s, _ ->", srcExpr, name))
		c.indent++
		c.writeln("try do")
		c.indent++
		for _, s := range stmt.Body {
	...
	if len(stmt.Index) > 0 {
		...
		if c.env != nil {
			if t, err2 := c.env.GetVar(stmt.Name); err2 == nil {
				switch t.(type) {
				case types.ListType:
					c.writeln(fmt.Sprintf("%s = List.replace_at(%s, %s, %s)", name, name, idx, value))
					return nil
				case types.MapType:
					if s, ok := simpleAtomKey(stmt.Index[0].Start); ok {
						idx = ":" + s
					}
				}
			}
	...
	for _, op := range p.Ops {
		if op.Index != nil {
			...
			if op.Index.Colon == nil {
				...
				case types.MapType:
					key := idx
					if s, ok := simpleAtomKey(op.Index.Start); ok {
						key = ":" + s
					}
					res = fmt.Sprintf("Map.get(%s, %s)", res, key)
					typ = tt.Value
...
func (c *Compiler) compilePrimary(p *parser.Primary) (string, error) {
	...
	case p.Call != nil:
		...
		switch p.Call.Func {
		case "print":
			return fmt.Sprintf("IO.puts(Enum.join(Enum.map([%s], fn v -> if is_binary(v), do: v, else: String.replace(inspect(v, charlists: :as_lists), \",\", \"\") end), \" \"))", argStr), nil
		case "len":
