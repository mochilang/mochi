...
func main() {
	...
	for _, name := range tasks {
		...
		codeFile := filepath.Join(outDir, name+".java")
		...
		if err := os.WriteFile(srcTmp, code, 0o644); err != nil {
			writeError(outDir, name, fmt.Sprintf("tmp write: %v", err))
			os.Remove(filepath.Join(outDir, name+".out"))
			continue
		}
		if out, err := exec.Command("javac", "-d", tmp, srcTmp).CombinedOutput(); err != nil {
			writeError(outDir, name, fmt.Sprintf("javac: %v\n%s", err, out))
			os.Remove(filepath.Join(outDir, name+".out"))
			continue
		}
