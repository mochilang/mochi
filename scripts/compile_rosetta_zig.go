...
func main() {
	...
	for _, name := range tasks {
		...
		if out, err := exec.Command(zigc, "build-exe", tmp, "-O", "ReleaseSafe", "-femit-bin="+exe).CombinedOutput(); err != nil {
			writeError(outDir, name, fmt.Sprintf("zig build: %v\n%s", err, out))
			os.Remove(filepath.Join(outDir, name+".out"))
			continue
		}
