...
func runRosettaTaskGolden(t *testing.T, name string) {
	root := repoRoot(t)
	script := exec.Command("go", "run", "-tags=archive,slow", "./scripts/compile_rosetta_erlang.go")
	...
	errFile := filepath.Join(root, "tests", "rosetta", "out", "Erlang", name+".error")
	if b, err := os.ReadFile(errFile); err == nil {
		t.Skipf("erlang run failed:\n%s", b)
		return
	}
	...
	codeWant := filepath.Join(root, "tests", "rosetta", "out", "Erlang", name+".erl")
	...
	outWant := filepath.Join(root, "tests", "rosetta", "out", "Erlang", name+".out")
	if shouldUpdateRosetta() {
...
func TestErlangCompiler_Rosetta_Golden(t *testing.T) {
