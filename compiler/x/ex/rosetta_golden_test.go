...
func runRosettaTaskGolden(t *testing.T, name string) {
	root := repoRoot(t)
	script := exec.Command("go", "run", "-tags=archive,slow", "./scripts/compile_rosetta_ex.go")
	...
	errFile := filepath.Join(root, "tests", "rosetta", "out", "Elixir", name+".error")
	if b, err := os.ReadFile(errFile); err == nil {
		t.Skipf("elixir run failed:\n%s", b)
		return
	}
	...
	codeWant := filepath.Join(root, "tests", "rosetta", "out", "Elixir", name+".exs")
	...
	if err := cmd.Run(); err != nil {
		t.Skipf("elixir run error: %v\n%s", err, stderr.Bytes())
		return
	}
	gotOut := bytes.TrimSpace(stdout.Bytes())
	outWant := filepath.Join(root, "tests", "rosetta", "out", "Elixir", name+".out")
	if shouldUpdateRosetta() {
...
func TestExCompiler_Rosetta_Golden(t *testing.T) {
