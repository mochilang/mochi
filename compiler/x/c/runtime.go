...
static void _save_jsonl(list_map_string rows,const char* path){ FILE* f=(!path||path[0]=='\0'||strcmp(path,"-")==0)?stdout:fopen(path,"w"); if(!f){fprintf(stderr,"cannot open %s\n",path); exit(1);} for(int i=0;i<rows.len;i++){ _write_obj(f,rows.data[i]); fputc('\n',f); } if(f!=stdout) fclose(f); }
`
