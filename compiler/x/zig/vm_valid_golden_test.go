...
func writeErrorValid(path string, src []byte, err error) {
...
func TestZigCompiler_VMValid_GoldenRun(t *testing.T) {
	...
	golden.RunWithSummary(t, "tests/vm/valid", ".mochi", ".out", func(src string) ([]byte, error) {
		...
		if err != nil {
			writeErrorValid(filepath.Join(outDir, name+".error"), data, err)
			return nil, err
		}
		env := types.NewEnv(nil)
		if errs := types.Check(prog, env); len(errs) > 0 {
			writeErrorValid(filepath.Join(outDir, name+".error"), data, errs[0])
			return nil, errs[0]
		}
		...
		if out, err := exec.Command(zigc, "build-exe", codePath, "-O", "ReleaseSafe", "-femit-bin="+exe).CombinedOutput(); err != nil {
			writeErrorValid(filepath.Join(outDir, name+".error"), code, fmt.Errorf("zig build error: %w\n%s", err, out))
			return nil, err
		}
		...
		if err != nil {
			writeErrorValid(filepath.Join(outDir, name+".error"), code, fmt.Errorf("run error: %w\n%s", err, out))
			return nil, err
		}
