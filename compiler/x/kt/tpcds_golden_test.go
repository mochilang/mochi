...
func TestKTCompiler_TPCDS_Golden(t *testing.T) {
	...
	for i := 1; i <= 19; i++ {
		...
		t.Run(q, func(t *testing.T) {
			...
			wantPath := filepath.Join(root, "tests", "dataset", "tpc-ds", "compiler", "kt", q+".kt.out")
			...
			outWantPath := filepath.Join(root, "tests", "dataset", "tpc-ds", "compiler", "kt", q+".out")
