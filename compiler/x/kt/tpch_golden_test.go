...
func TestKTCompiler_TPCH_Golden(t *testing.T) {
	...
	for i := 1; i <= 2; i++ {
		...
		t.Run(q, func(t *testing.T) {
			...
			wantPath := filepath.Join(root, "tests", "dataset", "tpc-h", "compiler", "kt", q+".kt.out")
			...
			outWantPath := filepath.Join(root, "tests", "dataset", "tpc-h", "compiler", "kt", q+".out")
