...
func runRosettaTaskGolden(t *testing.T, name string) {
	...
	script := exec.Command("go", "run", "-tags=archive,slow", "./scripts/compile_rosetta_lua.go")
	...
	errFile := filepath.Join(root, "tests", "rosetta", "out", "Lua", name+".error")
	if b, err := os.ReadFile(errFile); err == nil {
		t.Skipf("lua run failed:\n%s", b)
		return
	}
	...
	codeWant := filepath.Join(root, "tests", "rosetta", "out", "Lua", name+".lua")
	...
	outWant := filepath.Join(root, "tests", "rosetta", "out", "Lua", name+".out")
	if shouldUpdateRosetta() {
...
func TestLuaCompiler_Rosetta_Golden(t *testing.T) {
	if err := luacode.EnsureLua(); err != nil {
		t.Skipf("lua not installed: %v", err)
	}
	root := testutil.FindRepoRoot(t)
