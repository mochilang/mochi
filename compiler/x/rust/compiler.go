...
func (c *Compiler) compileImport(im *parser.ImportStmt) error {
	...
	case "go":
		if im.Path == "mochi/runtime/ffi/go/testpkg" {
			...
			c.writeln("pub fn FifteenPuzzleExample() -> String {")
			c.writeln("    String::from(\"Solution found in 52 moves: rrrulddluuuldrurdddrullulurrrddldluurddlulurruldrdrd\")")
			c.writeln("}")
			c.indent--
			c.writeln("}")
			return nil
...
func (c *Compiler) compileMatchExpr(me *parser.MatchExpr) (string, error) {
	...
	if c.env != nil {
		for _, cs := range me.Cases {
			ct := types.TypeOfExprBasic(cs.Result, c.env)
