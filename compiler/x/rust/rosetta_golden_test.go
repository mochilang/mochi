...
func runRosettaTaskGolden(t *testing.T, name string) {
	root := repoRoot()
	script := exec.Command("go", "run", "-tags=archive,slow", "./scripts/compile_rosetta_rust.go")
	...
	errFile := filepath.Join(root, "tests", "rosetta", "out", "Rust", name+".error")
	...
	codeWant := filepath.Join(root, "tests", "rosetta", "out", "Rust", name+".rs")
	...
	outWant := filepath.Join(root, "tests", "rosetta", "out", "Rust", name+".out")
...
func TestRustCompiler_Rosetta_Golden(t *testing.T) {
