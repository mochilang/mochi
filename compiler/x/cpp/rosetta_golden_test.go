...
func runRosettaTaskGolden(t *testing.T, name string) {
	root := repoRoot()
	script := exec.Command("go", "run", "-tags=archive,slow", "./scripts/compile_rosetta_cpp.go")
	...
	errFile := filepath.Join(root, "tests", "rosetta", "out", "cpp", name+".error")
	...
	codeWant := filepath.Join(root, "tests", "rosetta", "out", "cpp", name+".cpp")
	...
	outWant := filepath.Join(root, "tests", "rosetta", "out", "cpp", name+".out")
	if shouldUpdateRosetta() {
...
func TestCPPCompiler_Rosetta_Golden(t *testing.T) {
