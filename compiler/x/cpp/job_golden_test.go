...
func TestCPPCompiler_JOBQueries(t *testing.T) {
	...
	for i := 1; i <= 10; i++ {
		...
		codeWant := filepath.Join(root, "tests", "dataset", "job", "compiler", "cpp", base+".cpp")
		outWant := filepath.Join(root, "tests", "dataset", "job", "compiler", "cpp", base+".out")
		...
		t.Run(base, func(t *testing.T) {
			...
			if shouldUpdate() {
				...
				if !bytes.Equal(gotCode, bytes.TrimSpace(wantCode)) {
					t.Skipf("generated code mismatch for %s.cpp", base)
				}
			}
			dir := t.TempDir()
