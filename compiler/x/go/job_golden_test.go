...
func repoRootJob(t *testing.T) string {
	t.Helper()
	dir, err := os.Getwd()
	if err != nil {
...
func normalizeOutputJob(root string, b []byte) []byte {
	out := string(b)
...
func runJOBQuery(t *testing.T, base string) {
	...
	if !bytes.Equal(normalizeOutputJob(root, gotOut), normalizeOutputJob(root, vmOut)) {
	...
	if !bytes.Equal(normalizeOutputJob(root, gotOut), normalizeOutputJob(root, wantOut)) {
