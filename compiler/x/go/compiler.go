...
func (c *Compiler) compileBinaryOp(left string, leftType types.Type, op string, right string, rightType types.Type) (string, types.Type, error) {
	...
	case "==", "!=", "<", "<=", ">", ">=":
		...
		next = types.BoolType{}
	case "&&", "||":
		if isBoolLike(leftType) && isAnyLike(rightType) {
			right = c.castExpr(right, rightType, types.BoolType{})
		} else if isAnyLike(leftType) && isBoolLike(rightType) {
			left = c.castExpr(left, leftType, types.BoolType{})
		} else if !(isBoolLike(leftType) && isBoolLike(rightType)) {
			return "", types.AnyType{}, fmt.Errorf("operator %q cannot be used on types %s and %s", op, leftType, rightType)
		}
		expr = fmt.Sprintf("(%s %s %s)", left, op, right)
		next = types.BoolType{}
				...
				case types.GroupType:
					if field == "key" {
						...
						if kgo == "" {
							...
							base = fmt.Sprintf("%s.Key.(%s)", base, kgo)
						}
						typ = tt.Key
					} else if field == "items" {
	...
	case p.List != nil:
		...
		for i, e := range p.List.Elems {
			...
			if _, ok := elemT.(types.AnyType); ok && isList(et) && !isAny(et.(types.ListType).Elem) {
				...
				if etl, ok := et.(types.ListType); ok && equalTypes(elt.Elem, etl.Elem) {
					v = c.castExpr(v, et, elt)
				}
			}
			elems[i] = v
		}
			...
			if lt, ok := paramTypes[i].(types.ListType); ok {
				if et, ok := at.(types.ListType); ok {
					if isListOfAny(et) && !isListOfAny(lt) {
						v = c.castExpr(v, at, lt)
					} else if !isListOfAny(et) && equalTypes(lt.Elem, et.Elem) {
						v = c.castExpr(v, at, lt)
					}
				}
			}
