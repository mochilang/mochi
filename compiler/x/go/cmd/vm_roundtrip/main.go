...
func main() {
	...
	report.WriteString("# Go roundtrip VM test failures\n\n")
	...
	if report.Len() == 0 {
		report.WriteString("All Go roundtrip VM tests passed.\n")
	}
	os.WriteFile("compile/go/ERRORS.md", []byte(report.String()), 0644)
}
...
func process(src string) error {
	...
	if err != nil {
		writeErr(base+".go.error", err)
		return fmt.Errorf("parse error: %w", err)
	}
	env := types.NewEnv(nil)
	if errs := types.Check(prog, env); len(errs) > 0 {
		writeErr(base+".go.error", errs[0])
	...
	os.WriteFile(goPath, code, 0644)

	// Convert back to Mochi
	conv, err := go2mochi.Convert(goPath)
	if err != nil {
		writeErr(base+".mochi.error", err)
		return fmt.Errorf("go2mochi error: %w", err)
	}
	...
	// clean up error file if exists
	os.Remove(base + ".go.error")
