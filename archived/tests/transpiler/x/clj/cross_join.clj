...
(def result (for [o orders c customers] {:orderId (:id o) :orderCustomerId (:customerId o) :pairedCustomerName (:name c) :orderTotal (:total o)}))

(defn -main []
  (println "--- Cross Join: All order-customer pairs ---")
  (doseq [entry result] (println "Order" (:orderId entry) "(customerId:" (:orderCustomerId entry) ", total: $" (:orderTotal entry) ") paired with" (:pairedCustomerName entry))))
