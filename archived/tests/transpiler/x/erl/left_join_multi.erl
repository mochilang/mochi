...
main(_) ->
    ...
    {Matched,Acc} = lists:foldl(fun(I, {M,A}) ->
        case (maps:get("id", maps:get("o", O)) == maps:get("orderId", I)) of
    ...
    lists:foreach(fun(R) ->
    io:format("~p ~p ~p~n", [maps:get("orderId", R), maps:get("name", R), maps:get("item", R)])
end, Result).
