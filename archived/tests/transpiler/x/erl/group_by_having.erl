...
main(_) ->
    ...
    Big = lists:map(fun(P) ->
        Key = element(1, P),
        Items = element(2, P),
        #{"city" => Key, "num" => length(Items)} end, lists:filter(fun(P) ->
        Key = element(1, P),
        Items = element(2, P),
        (length(Items) >= 4) end, maps:to_list(lists:foldl(fun(P, Acc) ->
        K = element(1, P),
        V = element(2, P),
        maps:put(K, (maps:get(K, Acc, []) ++ [V]), Acc) end, #{}, [{maps:get("city", P), P} ||
        P <- People])))),
    io:format("~p~n", [Big]).
