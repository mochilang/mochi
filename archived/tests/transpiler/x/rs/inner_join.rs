...
fn main() {
    ...
    let result: Vec<QueryItem> = { let mut _q: Vec<QueryItem> = Vec::new(); for o in &orders { for c in &customers { if (o.customerId == c.id) { _q.push(QueryItem {orderId: o.clone().id, customerName: c.clone().name.clone(), total: o.clone().total}); } } } _q };
