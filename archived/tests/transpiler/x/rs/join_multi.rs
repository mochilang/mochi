...
fn main() {
    ...
    let result: Vec<QueryItem> = { let mut _q: Vec<QueryItem> = Vec::new(); for o in &orders { for c in &customers { if (o.customerId == c.id) { for i in &items { if (o.id == i.orderId) { _q.push(QueryItem {name: c.clone().name.clone(), sku: i.clone().sku.clone()}); } } } } } _q };
    println!("{}", "--- Multi Join ---");
    for r in &result {
