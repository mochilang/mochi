...
result = sortOn (\g -> g.key) [GenType2 {cat = g.key, share = sum [if x.flag then x.val else 0 | x <- g.items] `div` sum [x.val | x <- g.items]} | g <- [MGroup {key = k, items = [i | i <- items, i.cat == k]} | k <- nub ([i.cat | i <- items])]]
