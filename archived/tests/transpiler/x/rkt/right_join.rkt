...
(displayln (string-join (filter (lambda (s) (not (string=? s ""))) (list (format "~a" "Customer") (format "~a" (hash-ref entry "customerName")) (format "~a" "has order") (format "~a" (hash-ref (hash-ref entry "order") "id")) (format "~a" "- $") (format "~a" (hash-ref (hash-ref entry "order") "total")))) " "))
) (begin
(displayln (string-join (filter (lambda (s) (not (string=? s ""))) (list (format "~a" "Customer") (format "~a" (hash-ref entry "customerName")) (format "~a" "has no orders"))) " "))
))
)
