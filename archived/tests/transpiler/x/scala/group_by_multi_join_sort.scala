...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val result: ArrayBuffer[Map[String,Any]] = ArrayBuffer.from(customer.groupBy(c => Map("c_custkey" -> c.c_custkey, "c_name" -> c.c_name, "c_acctbal" -> c.c_acctbal, "c_address" -> c.c_address, "c_phone" -> c.c_phone, "c_comment" -> c.c_comment, "n_name" -> n.n_name)).filter(c => o.o_custkey == c.c_custkey && l.l_orderkey == o.o_orderkey && n.n_nationkey == c.c_nationkey && o.o_orderdate >= start_date && o.o_orderdate < end_date && l.l_returnflag == "R").map{ case (k, g) => Map("key" -> k, "items" -> ArrayBuffer.from(g)) }.toSeq.sortBy(g => 0 - ({ var _res = ArrayBuffer[Any]() ; for (x <- g) { _res.append(x.l.l_extendedprice * 1 - x.l.l_discount) }; _res }).sum).map(_._2).map(g => Map("c_custkey" -> g("key").c_custkey, "c_name" -> g("key").c_name, "revenue" -> ({ var _res = ArrayBuffer[Any]() ; for (x <- g) { _res.append(x.l.l_extendedprice * 1 - x.l.l_discount) }; _res }).sum, "c_acctbal" -> g("key").c_acctbal, "n_name" -> g("key").n_name, "c_address" -> g("key").c_address, "c_phone" -> g("key").c_phone, "c_comment" -> g("key").c_comment)))
    println(result)
  }
}
