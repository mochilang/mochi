...
int main() {
	auto scores = unordered_map<string, int>{{std::string("alice"), 1}};
	scores[std::string("bob")] = 2;
	([&](const auto& __v){ if constexpr(has_size<decay_t<decltype(__v)>>::value){ for(size_t i=0;i<__v.size();++i){ if(i) cout<<' '; cout<<__v[i]; } cout<<endl; } else { cout<<__v<<endl; } })(scores[std::string("bob")]);
	return 0;
}
