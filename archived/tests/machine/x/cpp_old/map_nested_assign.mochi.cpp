...
int main() {
	auto data = unordered_map<string, unordered_map<string, int>>{{std::string("outer"), unordered_map<string, int>{{std::string("inner"), 1}}}};
	data[std::string("outer")][std::string("inner")] = 2;
	([&](const auto& __v){ if constexpr(has_size<decay_t<decltype(__v)>>::value){ for(size_t i=0;i<__v.size();++i){ if(i) cout<<' '; cout<<__v[i]; } cout<<endl; } else { cout<<__v<<endl; } })(data[std::string("outer")][std::string("inner")]);
	return 0;
}
