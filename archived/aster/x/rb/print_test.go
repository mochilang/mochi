...
func TestPrint_Golden(t *testing.T) {
	...
	for _, src := range files {
		...
		t.Run(name, func(t *testing.T) {
			...
			prog, err := rb.Inspect(string(data))
			if err != nil {
			...
			jsonPath := filepath.Join(outDir, name+".rb.json")
			if shouldUpdate() {
			...
			want, err := exec.Command("ruby", src).CombinedOutput()
			if err != nil {
