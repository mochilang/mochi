...
func TestSwiftTranspiler_VMValid_Golden(t *testing.T) {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "swift")
...
func updateReadme() {
	...
	binDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "swift")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "swift", "README.md")
...
func updateTasks() {
	root := repoRoot()
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "swift", "TASKS.md")
	tsRaw, _ := exec.Command("git", "log", "-1", "--format=%cI").Output()
