...
func TestTranspile_ListNestedAssign(t *testing.T) {
	...
	plFile := filepath.Join(outDir, "list_nested_assign.pl")
	if err := os.WriteFile(plFile, buf.Bytes(), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	cmd := exec.Command("swipl", "-q", "-f", plFile)
	...
	got := bytes.TrimSpace(out)
	if err != nil {
		_ = os.WriteFile(filepath.Join(outDir, "list_nested_assign.error"), out, 0o644)
		t.Fatalf("run: %v", err)
	}
	_ = os.Remove(filepath.Join(outDir, "list_nested_assign.error"))
...
func TestTranspile_ListSetOps(t *testing.T) {
	...
	src := filepath.Join(root, "tests", "vm", "valid", "list_set_ops.mochi")
	prog, err := parser.Parse(src)
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	...
	plFile := filepath.Join(outDir, "list_set_ops.pl")
	if err := os.WriteFile(plFile, buf.Bytes(), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	cmd := exec.Command("swipl", "-q", "-f", plFile)
	...
	got := bytes.TrimSpace(out)
	if err != nil {
		_ = os.WriteFile(filepath.Join(outDir, "list_set_ops.error"), out, 0o644)
		t.Fatalf("run: %v", err)
	}
	_ = os.Remove(filepath.Join(outDir, "list_set_ops.error"))
	want, err := os.ReadFile(filepath.Join(outDir, "list_set_ops.out"))
	if err != nil {
		t.Fatalf("read want: %v", err)
	}
	want = bytes.TrimSpace(want)
...
func TestTranspile_RecordAssign(t *testing.T) {
	...
	src := filepath.Join(root, "tests", "vm", "valid", "record_assign.mochi")
	prog, err := parser.Parse(src)
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	...
	plFile := filepath.Join(outDir, "record_assign.pl")
	if err := os.WriteFile(plFile, buf.Bytes(), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	cmd := exec.Command("swipl", "-q", "-f", plFile)
	...
	got := bytes.TrimSpace(out)
	if err != nil {
		_ = os.WriteFile(filepath.Join(outDir, "record_assign.error"), out, 0o644)
		t.Fatalf("run: %v", err)
	}
	_ = os.Remove(filepath.Join(outDir, "record_assign.error"))
	want, err := os.ReadFile(filepath.Join(outDir, "record_assign.out"))
	if err != nil {
		t.Fatalf("read want: %v", err)
	}
	want = bytes.TrimSpace(want)
...
func TestTranspile_BenchBlock(t *testing.T) {
	...
	src := filepath.Join(root, "tests", "vm", "valid", "bench_block.mochi")
	prog, err := parser.Parse(src)
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	...
	plFile := filepath.Join(outDir, "bench_block.pl")
	if err := os.WriteFile(plFile, buf.Bytes(), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
	cmd := exec.Command("swipl", "-q", "-f", plFile)
	...
	got := bytes.TrimSpace(out)
	if err != nil {
		_ = os.WriteFile(filepath.Join(outDir, "bench_block.error"), out, 0o644)
		t.Fatalf("run: %v", err)
	}
	_ = os.Remove(filepath.Join(outDir, "bench_block.error"))
	want, err := os.ReadFile(filepath.Join(outDir, "bench_block.out"))
	if err != nil {
		t.Fatalf("read want: %v", err)
	}
	want = bytes.TrimSpace(want)
...
func updateReadme() {
	...
	srcDir := filepath.Join(root, "tests", "vm", "valid")
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "pl")
	readme := filepath.Join(root, "archived", "transpiler", "x", "pl", "README.md")
	files, _ := filepath.Glob(filepath.Join(srcDir, "*.mochi"))
	sort.Strings(files)
	total := len(files)
	compiled := 0
...
func updateTasks() {
	root := repoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "pl", "TASKS.md")
