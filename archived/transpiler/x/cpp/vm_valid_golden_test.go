...
func TestCPPTranspiler_VMValid_Golden(t *testing.T) {
	ensureGPP(t)
	root := repoRoot(t)
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "cpp")
...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "cpp")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "cpp", "README.md")
...
func updateTasks() {
	root := repoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "cpp", "TASKS.md")
	tsRaw, _ := exec.Command("git", "log", "-1", "--format=%cI").Output()
	...
	files, _ := filepath.Glob(filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "cpp", "*.cpp"))
	compiled := len(files)
