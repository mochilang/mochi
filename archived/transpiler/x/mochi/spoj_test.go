...
func TestMochiTranspiler_SPOJ_Golden(t *testing.T) {
        ...
        src := filepath.Join(root, "tests", "spoj", "x", "mochi", "12.mochi")
        outDir := filepath.Join(root, "tests", "spoj", "x", "mochi")
        codePath := filepath.Join(outDir, "12.mochi")
        outPath := filepath.Join(outDir, "12.out")
        errPath := filepath.Join(outDir, "12.error")
        ...
        cmd := exec.Command("go", "run", filepath.Join(root, "cmd", "mochi"), "run", codePath)
        envv := os.Environ()
        if bench {
        ...
        cmd.Env = envv
        inPath := filepath.Join(outDir, "12.in")
        ...
        if bench {
                benchPath := filepath.Join(outDir, "12.bench")
...
func updateSPOJ() {
        ...
        outDir := filepath.Join(root, "tests", "spoj", "x", "mochi")
        md := filepath.Join(root, "archived", "transpiler", "x", "mochi", "SPOJ.md")
        status := " "
        dur := ""
        mem := ""
        if _, err := os.Stat(filepath.Join(outDir, "12.error")); err == nil {
                status = "error"
        } else if _, err := os.Stat(filepath.Join(outDir, "12.mochi")); err == nil {
                status = "✓"
        }
        if data, err := os.ReadFile(filepath.Join(outDir, "12.bench")); err == nil {
                var r struct {
        ...
        buf.WriteString("|------:|------|:-----:|---------:|-------:|\n")
        buf.WriteString("| 12 | MMIND - The Game of Master-Mind | " + status + " | " + dur + " | " + mem + " |\n")
