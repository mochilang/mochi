...
func TestRSTranspiler_VMValid_Golden(t *testing.T) {
	...
	root := vmRepoRoot(t)
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "rs")
...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "rs")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "rs", "README.md")
	...
	buf.WriteString("Generated sources for the golden tests live under `archived/tests/transpiler/x/rs`.\n")
	if ts != "" {
...
func updateTasks() {
	root := vmRepoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "rs", "TASKS.md")
	out, err := exec.Command("git", "log", "-1", "--format=%cI").Output()
	ts := ""
	if err == nil {
		if t, perr := time.Parse(time.RFC3339, strings.TrimSpace(string(out))); perr == nil {
