...
import (
	...
	"mochi/parser"
	ohp "mochi/archived/transpiler/x/ohp"
	"mochi/types"
)

var updateAlgorithmsFlag = flag.Bool("update-algorithms-ohp", false, "update golden files")
...
func runAlgorithmCase(t *testing.T, file string) {
	...
	outDir := filepath.Join(root, "tests", "algorithms", "x", "OHP", filepath.Dir(file))
	...
	ast, err := ohp.Transpile(prog, env)
	if err != nil {
		_ = os.WriteFile(errPath, []byte("transpile: "+err.Error()), 0o644)
		t.Fatalf("transpile: %v", err)
	}
	var buf bytes.Buffer
	if err := ohp.Emit(&buf, ast); err != nil {
...
func TestOHPTranspiler_Algorithms_Golden(t *testing.T) {
	root := repoRoot(t)
...
func updateAlgorithms() {
	...
	outDir := filepath.Join(root, "tests", "algorithms", "x", "OHP")
	md := filepath.Join(root, "archived", "transpiler", "x", "ohp", "ALGORITHMS.md")
	...
	var buf bytes.Buffer
	buf.WriteString("# OHP Algorithms Transpiler Progress\n\n")
	buf.WriteString("This checklist is auto-generated.\n")
	buf.WriteString("Generated OHP (PHP) code from programs in `tests/github/TheAlgorithms/Mochi` lives in `tests/algorithms/x/OHP`.\n\n")
	buf.WriteString(fmt.Sprintf("Compiled: %d/%d (%.2f%%)\n\n", compiled, total, float64(compiled)/float64(total)*100))
