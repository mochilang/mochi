...
func updateReadme() {
	...
	binDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "go")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "go", "README.md")
...
func updateTasks() {
	root := findRepoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "go", "TASKS.md")
	tsRaw, _ := exec.Command("git", "log", "-1", "--format=%cI").Output()
	...
	for _, f := range files {
		name := strings.TrimSuffix(filepath.Base(f), ".mochi")
		if _, err := os.Stat(filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "go", name+".out")); err == nil {
			compiled++
		}
	}
