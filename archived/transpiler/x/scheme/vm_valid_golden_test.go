...
func TestSchemeTranspiler_VMValid_Golden(t *testing.T) {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "scheme")
...
func updateReadme() {
	...
	binDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "scheme")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "scheme", "README.md")
...
func updateTasks() {
	root := findRepoRoot2(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "scheme", "TASKS.md")
