...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "cobol")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "cobol", "README.md")
...
func updateTasks() {
	root := repoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "cobol", "TASKS.md")
	tsRaw, _ := exec.Command("git", "log", "-1", "--format=%cI").Output()
	...
	files, _ := filepath.Glob(filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "cobol", "*.cob"))
	compiled := len(files)
