...
func TestTranspilePrograms(t *testing.T) {
	...
	root := repoRoot(t)
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "kt")
...
func updateReadme() {
	...
	buf.WriteString("Generated Kotlin sources for golden tests are stored in `archived/tests/transpiler/x/kt`.\n\n")
	if ts != "" {
	...
	buf.WriteString("\n")
	_ = os.WriteFile(filepath.Join(root, "archived", "transpiler", "x", "kt", "README.md"), buf.Bytes(), 0o644)
}

func updateTasks() {
	root := repoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "kt", "TASKS.md")
