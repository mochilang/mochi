...
func runSpojCase(t *testing.T, idx int) {
	...
	root := repoRoot(t)
	outDir := filepath.Join(root, "tests", "spoj", "x", "ocaml")
...
func TestOCamlTranspiler_SPOJ(t *testing.T) {
...
func updateSPOJChecklist() {
	...
	outDir := filepath.Join(root, "tests", "spoj", "x", "ocaml")
	mdPath := filepath.Join(root, "archived", "transpiler", "x", "ocaml", "SPOJ.md")
	...
	for _, f := range files {
		...
		if _, err := os.Stat(filepath.Join(outDir, base+".error")); err == nil {
			status = "error"
		} else if _, err := os.Stat(filepath.Join(outDir, base+".ml")); err == nil {
			status = "✓"
			compiled++
		}
	...
	buf.WriteString("Generated OCaml code from programs in `tests/spoj/x/mochi` lives in `tests/spoj/x/ocaml`.\n\n")
