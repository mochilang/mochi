...
func updateReadme() {
	...
	binDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "fortran")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "fortran", "README.md")
...
func updateTasks() {
	root := repoRootDir(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "fortran", "TASKS.md")
	tsOut, _ := exec.Command("git", "log", "-1", "--format=%cI").Output()
	ts := ""
	if t, err := time.Parse(time.RFC3339, strings.TrimSpace(string(tsOut))); err == nil {
		if loc, lerr := time.LoadLocation("Asia/Bangkok"); lerr == nil {
