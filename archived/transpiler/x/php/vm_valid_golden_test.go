...
func TestPHPTranspiler_VMValid_Golden(t *testing.T) {
	...
	root := vmRepoRoot(t)
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "php")
...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "php")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "php", "README.md")
	...
	buf.WriteString("Generated PHP code from programs in `tests/vm/valid` lives in `archived/tests/transpiler/x/php`.\n\n")
	if ts != "" {
...
func updateTasks() {
	root := vmRepoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "php", "TASKS.md")
	out, err := exec.Command("git", "log", "-1", "--format=%cI").Output()
	ts := ""
	if err == nil {
		if t, perr := time.Parse(time.RFC3339, strings.TrimSpace(string(out))); perr == nil {
