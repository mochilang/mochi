...
func runRosetta(t *testing.T, srcPath, name, outDir string) {
	...
	if bench {
		...
		envVars = append(envVars, "MOCHI_NOW_SEED=1")
	}
	cmd.Env = envVars
	...
	if updateEnabled() {
		...
		if bench {
			...
			if idx := bytes.LastIndexByte(outBytes, '{'); idx >= 0 {
				_ = os.WriteFile(benchFile, outBytes[idx:], 0o644)
			}
		}
		_ = os.Remove(errFile)
		return
	...
	if bench {
		...
		_ = os.WriteFile(benchFile, outBytes, 0o644)
		var js struct {
...
func updateRosettaReadme() {
	...
	srcDir := filepath.Join(root, "tests", "rosetta", "x", "Mochi")
	outDir := filepath.Join(root, "tests", "rosetta", "transpiler", "erl")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "erl", "ROSETTA.md")
	names, err := readIndex(filepath.Join(srcDir, "index.txt"))
	if err != nil {
		return
	}
