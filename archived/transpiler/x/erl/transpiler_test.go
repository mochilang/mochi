...
func runGolden(t *testing.T, name string) {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "erl")
...
func updateReadme() {
	...
	srcDir := filepath.Join(root, "tests", "vm", "valid")
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "erl")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "erl", "README.md")
	files, _ := filepath.Glob(filepath.Join(srcDir, "*.mochi"))
	sort.Strings(files)
	total := len(files)
	compiled := 0
...
func updateTasks() {
	root := repoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "erl", "TASKS.md")
