...
func runSpojCase(t *testing.T, idx int) {
	t.Helper()
	root := repoRoot(t)
       src := filepath.Join(root, "tests", "spoj", "human", "x", "mochi", fmt.Sprintf("%d.mochi", idx))
...
func updateSpojChecklist() {
	root := repoRoot(&testing.T{})
       srcDir := filepath.Join(root, "tests", "spoj", "x", "human", "mochi")
	outDir := filepath.Join(root, "tests", "spoj", "x", "erlang")
	mdPath := filepath.Join(root, "archived", "transpiler", "x", "erl", "SPOJ.md")
	...
	fmt.Fprintf(&buf, "Last updated: %s\n\n", ts)
       buf.WriteString("This directory contains Erlang code generated by the Mochi transpiler for SPOJ problems. Each program in `tests/spoj/human/x/mochi` is transpiled and executed with `escript`.\n\n")
