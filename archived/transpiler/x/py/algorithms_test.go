...
var updateAlgorithmsFlag = flag.Bool("update-algorithms-py", false, "update golden files")
...
func runAlgorithmCase(t *testing.T, file string) {
	...
	outDir := filepath.Join(root, "tests", "algorithms", "x", "Python", filepath.Dir(file))
...
func TestPyTranspiler_Algorithms_Golden(t *testing.T) {
	root := repoRoot(t)
...
func updateAlgorithms() {
	...
	outDir := filepath.Join(root, "tests", "algorithms", "x", "Python")
	md := filepath.Join(root, "archived", "transpiler", "x", "py", "ALGORITHMS.md")
	...
	buf.WriteString("Generated Python code from programs in `tests/github/TheAlgorithms/Mochi` lives in `tests/algorithms/x/Pytho" +
		"n`.\n")
