...
func TestPyTranspiler_VMValid_Golden(t *testing.T) {
	ensurePython(t)
	root := repoRoot(t)
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "py")
...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "py")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "py", "README.md")
	...
	buf.WriteString("Generated Python code from programs in `tests/vm/valid` lives in `archived/tests/transpiler/x/py`.\n")
...
func updateTasks() {
	root := repoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "py", "TASKS.md")
