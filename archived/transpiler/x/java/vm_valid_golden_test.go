...
func TestJavaTranspiler_VMValid_Golden(t *testing.T) {
	...
	root := repoRootDir(t)
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "java")
...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "java")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "java", "README.md")
...
func updateTasks() {
	root := repoRootDir(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "java", "TASKS.md")
