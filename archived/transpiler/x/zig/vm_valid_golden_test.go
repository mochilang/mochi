...
func TestZigTranspiler_VMValid_Golden(t *testing.T) {
	...
	root := vmRepoRoot(t)
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "zig")
...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "zig")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "zig", "README.md")
	...
	buf.WriteString("Generated Zig code for the Mochi VM valid tests lives under `archived/tests/transpiler/x/zig`.\n\n")
	if ts != "" {
...
func updateTasks() {
	root := vmRepoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "zig", "TASKS.md")
