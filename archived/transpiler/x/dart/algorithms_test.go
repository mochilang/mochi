...
var updateAlgorithmsFlag = flag.Bool("update-algorithms-dart", false, "update golden files")
...
func runAlgorithmCase(t *testing.T, file string) {
	...
	outDir := filepath.Join(root, "tests", "algorithms", "x", "Dart", filepath.Dir(file))
...
func updateAlgorithms() {
	...
	md := filepath.Join(root, "archived", "transpiler", "x", "dart", "ALGORITHMS.md")
	...
	for _, e := range entries {
		...
		if _, err := os.Stat(filepath.Join(outDir, name+".error")); err == nil {
			status = "error"
		} else if _, err := os.Stat(filepath.Join(outDir, name+".dart")); err == nil {
			status = "✓"
			compiled++
		}
