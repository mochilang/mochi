...
import (
	...
	"mochi/parser"
	transpiler "mochi/archived/transpiler/x/cs"
	"mochi/types"
)
...
func TestCSTranspiler_Golden(t *testing.T) {
	...
	root := repoRoot(t)
	goldenDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "cs")
...
func updateReadme() {
	root := repoRoot(&testing.T{})
	srcDir := filepath.Join(root, "tests", "vm", "valid")
	binDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "cs")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "cs", "README.md")
	files, _ := filepath.Glob(filepath.Join(srcDir, "*.mochi"))
	total := len(files)
	compiled := 0
	var lines []string
...
func updateTasks() {
	root := repoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "cs", "TASKS.md")
