...
import (
	...
	tstrans "mochi/archived/transpiler/x/ts"
	"mochi/types"
)
...
func TestTSTranspiler_VMValid_Golden(t *testing.T) {
	...
	root := repoRoot(t)
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "ts")
...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "ts")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "ts", "README.md")
	...
	buf.WriteString("Generated sources for the golden tests live under `archived/tests/transpiler/x/ts`.\n\n")
...
func updateTasks() {
	root := repoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "ts", "TASKS.md")
	out, err := exec.Command("git", "log", "-1", "--format=%cI").Output()
	ts := ""
	if err == nil {
		if t, perr := time.Parse(time.RFC3339, strings.TrimSpace(string(out))); perr == nil {
