...
func updateReadme() {
	...
	srcDir := filepath.Join(root, "tests", "vm", "valid")
	binDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "clj")
...
func updateTasks() {
	root := findRepoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "clj", "TASKS.md")
	...
	for _, f := range files {
		name := strings.TrimSuffix(filepath.Base(f), ".mochi")
		if _, err := os.Stat(filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "clj", name+".output")); err == nil {
			compiled++
		}
	}
