...
var updateAlgFlag = flag.Bool("update-algorithms-clj", false, "update golden files")

func algUpdateEnabled() bool { return *updateAlgFlag }
...
func runAlgorithmCase(t *testing.T, file string) {
	...
	outDir := filepath.Join(root, "tests", "algorithms", "x", "Clojure", filepath.Dir(file))
	...
	if want != nil && !bytes.Equal(outBytes, want) {
		t.Errorf("output mismatch for %s\n\n--- Got ---\n%s\n\n--- Want ---\n%s", file, outBytes, want)
	}
}

func TestClojureTranspiler_Algorithms_Golden(t *testing.T) {
	defer updateAlgorithmsReadme()
	root := findRepoRoot(t)
...
func updateAlgorithmsReadme() {
	...
	outDir := filepath.Join(root, "tests", "algorithms", "x", "Clojure")
	md := filepath.Join(root, "archived", "transpiler", "x", "clj", "ALGORITHMS.md")
	...
	for _, e := range entries {
		...
		if _, err := os.Stat(filepath.Join(outDir, name+".error")); err == nil {
			status = "error"
		} else if _, err := os.Stat(filepath.Join(outDir, name+".clj")); err == nil {
			status = "✓"
			compiled++
		}
	...
	buf.WriteString("Generated Clojure code from programs in `tests/github/TheAlgorithms/Mochi` lives in `tests/algorithms/x/Clojure`.\n")
