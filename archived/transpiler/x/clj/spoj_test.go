...
func TestClojureTranspiler_Spoj_Golden(t *testing.T) {
	...
	if err := EnsureClojure(); err != nil {
        ...
        outs, err := filepath.Glob(filepath.Join(outDir, "*.out"))
        if err != nil {
                t.Fatalf("glob: %v", err)
        }
        sort.Strings(outs)
        for _, out := range outs {
                idx := strings.TrimSuffix(filepath.Base(out), ".out")
                if n, err := strconv.Atoi(idx); err == nil && n < 6 {
                        continue
                }
...
func runSpojCase(t *testing.T, idx string) {
        t.Helper()
        root := findRepoRoot(t)
        src := filepath.Join(root, "tests", "spoj", "human", "x", "mochi", idx+".mochi")
        outDir := filepath.Join(root, "tests", "spoj", "x", "clj")
...
func updateSpojReadme() {
        ...
        outDir := filepath.Join(root, "tests", "spoj", "x", "clj")
        md := filepath.Join(root, "archived", "transpiler", "x", "clj", "SPOJ.md")
        files, err := filepath.Glob(filepath.Join(outDir, "*.clj"))
        if err != nil {
                return
        }
        sort.Strings(files)
        ...
        rows = append(rows, "|------:|------|:-----:|---------:|-------:|")
        for _, f := range files {
                idx := strings.TrimSuffix(filepath.Base(f), ".clj")
                if n, err := strconv.Atoi(idx); err == nil && n < 6 {
                        continue
                }
                total++
        ...
        buf.WriteString("Generated Clojure code from programs in `tests/spoj/human/x/mochi` lives in `tests/spoj/x/clj`.\n")
