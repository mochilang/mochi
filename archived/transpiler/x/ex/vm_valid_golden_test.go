...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "ex")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "ex", "README.md")
	...
	buf.WriteString("This directory contains a minimal transpiler that converts a very small subset of Mochi into Elixir source code. The generated files live in `archived/tests/transpiler/x/ex`.\n\n")
...
func updateTasks() {
	root := repoRoot(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "ex", "TASKS.md")
