...
func updateReadme() {
	...
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "st")
	readmePath := filepath.Join(root, "archived", "transpiler", "x", "st", "README.md")
	...
	buf.WriteString("This directory holds an experimental transpiler that converts a small subset of Mochi into Smalltalk. The generated sources for the golden tests live under `archived/tests/transpiler/x/st`.\n")
...
func updateTasks() {
	root := repoRootDir(&testing.T{})
	taskFile := filepath.Join(root, "archived", "transpiler", "x", "st", "TASKS.md")
	srcDir := filepath.Join(root, "tests", "vm", "valid")
	outDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "st")
