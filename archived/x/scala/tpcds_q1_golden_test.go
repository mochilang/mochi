...
func TestScalaCompiler_TPCDSQ1(t *testing.T) {
	...
	wantCode, err := os.ReadFile(filepath.Join(root, "tests", "dataset", "tpc-ds", "compiler", "scala", "q1.scala.out"))
	if err != nil {
	...
	wantOut, err := os.ReadFile(filepath.Join(root, "tests", "dataset", "tpc-ds", "compiler", "scala", "q1.out"))
	if err != nil {
		t.Fatalf("read golden: %v", err)
	}
