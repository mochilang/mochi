...
func TestClojureRoundtripVMValid(t *testing.T) {
	...
	for _, src := range files {
		...
		t.Run(name, func(t *testing.T) {
			...
			cljSrc, err := cljcode.New(env).Compile(prog)
			if err != nil {
			...
			if err := os.WriteFile(cljFile, cljSrc, 0644); err != nil {
				t.Fatalf("write error: %v", err)
			}
			mochiSrc, err := ConvertFile(cljFile)
			if err != nil {
	...
	any2mochi.WriteStatusMarkdown(filepath.Join(root, "tests", "any2mochi", "clj"), status)
}
