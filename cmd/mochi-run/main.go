...
func runProgram(path string, prog *parser.Program) (string, error) {
	...
	if errc != nil {
		...
		return output.String(), fmt.Errorf("compile error: %w", errc)
	}
	m := vm.New(p, output)
...
func generateLLMMarkdown(outPath string) error {
	...
	sources := []sourceFile{
		...
		{"runtime/vm/vm.go", "## runtime/vm/vm.go", "Bytecode virtual machine."},
	...
	sb.WriteString("This document includes core source files used by the Mochi VM and toolchain.\n\n")
