...
// TestPhase2ControlFlow is the MEP-45 Phase 2.1 gate. It walks
// every fixture directory under
// tests/transpiler3/c/fixtures/control-flow, runs the end-to-end
// pipeline (parse, type-check, lower, emit, cc, link, exec), and
// diffs the binary's stdout against the fixture's expect.txt.
// Adding a new fixture directory containing `<name>.mochi` +
// `expect.txt` automatically extends the suite. Mirror of
// TestPhase2Primitives but for the control-flow surface.
func TestPhase2ControlFlow(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Phase 1 ships host-cc discovery only on POSIX; Windows lands in Phase 11")
	}
	root := repoRoot(t)
	base := filepath.Join(root, "tests", "transpiler3", "c", "fixtures", "control-flow")
