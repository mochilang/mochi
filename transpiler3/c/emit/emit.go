...
func walkExprListOfMap(e aotir.Expr, add func(aotir.Type, aotir.Type, aotir.Type)) {
	...
	case *aotir.ListContainsExpr:
		walkExprListOfMap(v.List, add)
		walkExprListOfMap(v.Value, add)
	case *aotir.ListSumExpr:
		walkExprListOfMap(v.Receiver, add)
	case *aotir.MathCallExpr:
		walkExprListOfMap(v.Arg, add)
	case *aotir.VarRef:
		add(v.ElemType, v.MapElemKeyType, v.MapElemValueType)
	case *aotir.CallExpr:
...
func walkExprNodeVisit(e aotir.Expr, visit func(aotir.Expr)) {
	...
	case *aotir.ListMaxExpr:
		walkExprNodeVisit(v.Receiver, visit)
	case *aotir.ListContainsExpr:
		walkExprNodeVisit(v.List, visit)
		walkExprNodeVisit(v.Value, visit)
	case *aotir.ListSumExpr:
		walkExprNodeVisit(v.Receiver, visit)
	case *aotir.MathCallExpr:
		walkExprNodeVisit(v.Arg, visit)
	case *aotir.FieldAccess:
		walkExprNodeVisit(v.Receiver, visit)
	case *aotir.RecordLit:
		for _, f := range v.Fields {
...
func walkExprMapOfList(e aotir.Expr, add func(aotir.Type, aotir.Type, aotir.Type)) {
	...
	case *aotir.ListMaxExpr:
		walkExprMapOfList(v.Receiver, add)
	case *aotir.ListContainsExpr:
		walkExprMapOfList(v.List, add)
		walkExprMapOfList(v.Value, add)
	case *aotir.ListSumExpr:
		walkExprMapOfList(v.Receiver, add)
	case *aotir.MathCallExpr:
		walkExprMapOfList(v.Arg, add)
	case *aotir.FieldAccess:
		walkExprMapOfList(v.Receiver, add)
	case *aotir.RecordLit:
		for _, f := range v.Fields {
...
func walkExprInner(e aotir.Expr, visit func(aotir.Type, aotir.Type)) {
	...
	case *aotir.ListMaxExpr:
		walkExprInner(v.Receiver, visit)
	case *aotir.ListContainsExpr:
		walkExprInner(v.List, visit)
		walkExprInner(v.Value, visit)
	case *aotir.ListSumExpr:
		walkExprInner(v.Receiver, visit)
	case *aotir.MathCallExpr:
		walkExprInner(v.Arg, visit)
	case *aotir.VarRef:
		visit(v.ElemType, v.InnerElemType)
	case *aotir.CallExpr:
...
func emitExpr(e aotir.Expr) (string, error) {
	...
	case *aotir.ListContainsExpr:
		return emitListContainsExpr(v)
	case *aotir.ListSumExpr:
		return emitListSumExpr(v)
	case *aotir.MathCallExpr:
		return emitMathCallExpr(v)
	case *aotir.AppendExpr:
...
// emitListContainsExpr renders `val in list<T>` as
// `mochi_list_<T>_contains(xs, val)`. Returns int (0/1).
func emitListContainsExpr(v *aotir.ListContainsExpr) (string, error) {
	suf, err := scalarListInnerSuffix(v.ElemType)
	if err != nil {
		return "", fmt.Errorf("list contains: %w", err)
	}
	lst, err := emitExpr(v.List)
	if err != nil {
		return "", fmt.Errorf("list contains list: %w", err)
	}
	val, err := emitExpr(v.Value)
	if err != nil {
		return "", fmt.Errorf("list contains value: %w", err)
	}
	return fmt.Sprintf("mochi_list_%s_contains(%s, %s)", suf, lst, val), nil
}

// emitListSumExpr renders `sum(xs)` as `mochi_list_<T>_sum(xs)`.
func emitListSumExpr(v *aotir.ListSumExpr) (string, error) {
	suf, err := scalarListInnerSuffix(v.ElemType)
	if err != nil {
		return "", fmt.Errorf("list sum: %w", err)
	}
	recv, err := emitExpr(v.Receiver)
	if err != nil {
		return "", fmt.Errorf("list sum receiver: %w", err)
	}
	return fmt.Sprintf("mochi_list_%s_sum(%s)", suf, recv), nil
}

// emitMathCallExpr renders abs/floor/ceil as inline C math.h calls.
// abs_i64 -> (int64_t)llabs((long long)(arg))
// abs_f64 -> fabs(arg)
// floor   -> floor(arg)
// ceil    -> ceil(arg)
func emitMathCallExpr(v *aotir.MathCallExpr) (string, error) {
	arg, err := emitExpr(v.Arg)
	if err != nil {
		return "", fmt.Errorf("math call %s arg: %w", v.Func, err)
	}
	switch v.Func {
	case "abs_i64":
		// Emit as a ternary to avoid llabs (requires <stdlib.h> which
		// conflicts with user-defined functions named abs). The ternary
		// is always safe for int64_t (INT64_MIN is not representable in
		// two's-complement negation, but Mochi does not guarantee abs of
		// INT64_MIN to be meaningful, so we follow C semantics here).
		return fmt.Sprintf("((%s) < 0 ? -(%s) : (%s))", arg, arg, arg), nil
	case "abs_f64":
		return fmt.Sprintf("fabs(%s)", arg), nil
	case "floor":
		return fmt.Sprintf("floor(%s)", arg), nil
	case "ceil":
		return fmt.Sprintf("ceil(%s)", arg), nil
	default:
		return "", fmt.Errorf("unknown MathCallExpr func %q", v.Func)
	}
}
