...
- **6.2d.2.d — Inline map ops (`OpMapSetI64I64`, `OpMapGetI64I64`, `OpNewMap`)**: lower the map ops on the same inline pattern. The map table is open-addressed linear probing with `splitmix64`-style hashing (`maps.go:hashI64`); the inline lowering emits the hash mix and the probe loop directly in machine code, deopting on grow or on a probe sequence that exceeds a small cap (e.g. 16 probes). Fourth checkpoint: `maps_fill_sum` inside 2x of Go.
  ...
  - **Step 4 — JIT lowering of `OpMapSetI64I64` / `OpMapGetI64I64` (planned)**: add the map ops to the cell-bank admission whitelist and emit an inline lowering in `lower_arm64.go`. Two paths are viable: (a) full inline (hash mix + linear probe + Cell box/unbox + `nLive++` + deopt-on-grow), mirroring `OpListPushI64`'s shape; or (b) `BL` to a Go helper (`vm3jitMapSet` / `vm3jitMapGet`) that does the actual work after the JIT has spilled caller-saved registers. Path (a) is the bigger lift but eliminates the call ABI per op; path (b) is one PR of plumbing and saves only the big-switch dispatch (~3 ns per op). With pre-size + soft-reuse already in place, the warm cache has no headroom risk on parity-perturbed n, so the deopt-on-grow analog needed for lists (`StatusListGrow` retry) is not on the critical path for the corpus n=128 workload. Target: `maps_fill_sum_n128` inside 2x of Go fair.
...
| `fasta` | 3.80-3.98x | LCG inner loop + cumprob lookup + i64 hash | small (already i64) | **large** (LCG kernel is the `OpAffineModI64K` shape from MEP-39 §6.6; admits as a pure-i64 JIT'd inner loop) | 6.3.4.d, port + admit `fasta.inner` in vm3jit |
...
| `mandelbrot` | 25-27x | f64 mul/add per-pixel | **2x** (no Cell boxing; native f64) | **3-5x** (Phase 6.2b NEON pair-pipelining on the `(z.re² - z.im² + c.re, 2*z.re*z.im + c.im)` recurrence) | 6.3.4.h, port + admit the per-pixel kernel |
...
| `nsieve` | 126-134x | List of bool fill + scan | small (containers are still handle-typed) | **large** (`OpListGetI64` + `OpListSetI64` on the sieve table is already JIT-lowered; the `nsieve.main` outer loop admits as the lists_fill_sum shape) | 6.3.4.k, port + admit `nsieve.main` |
