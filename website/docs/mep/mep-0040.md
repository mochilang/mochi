...
- 6.2d.2.c: `lists_fill_sum_n128` inside 2x of Go. *(Partial: dropped from `15.3x` to `4.05x` on darwin/arm64 in 6.2d.2.c, then to `3.14x` in 6.2d.2.c.1 via the slab-base hoist. The remaining gap is `main`'s interp dispatch (OpNewList + two OpCallMixed sites) plus two interp -> JIT trampoline crossings per iter. Closing to <2x is deferred until 6.2d.2.b proper (JIT-side `OpCallMixed` lowering) lands.)*
