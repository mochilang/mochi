...
Next step (Phase 6.3.4.k.2): admit `OpListSetI64` on the Cell-bank ARM64 backend (mirrors the existing `OpListPushI64` inline lowering, just without the `len++` bookkeeping). Expected post-JIT ratio: 6-15x of Go. Closing the residual to <2x then requires the Phase 3.6 bytes bank so the sieve table can be stored at 1 byte per slot.
