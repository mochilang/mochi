...
- **6.2d.2.a — Cell-bank infrastructure (ARM64 only)**: ship `CallStatusM`, `jitArenaCtx`, the regsCell pinning machinery in `lower_arm64.go`, and a relaxed acceptance check in `compile.go` that admits a function whose only Cell ops are `OpListGetI64` and whose only call op is `OpTailCallMixed` to self with `B=0` (the `sum` shape). Lower exactly two ops: `OpListGetI64` (inline handle decode + cells-slice load + int48 unbox) and `OpTailCallMixed` (self-tail, B=0 → branch to function entry). The first benchmark checkpoint: `lists_fill_sum` runs `sum` through JIT while `main` and `fill` stay in interp. Expected: ratio drops from 21.4x toward the fill-only floor.
- **6.2d.2.b — Mixed call boundary (`OpCallMixed` / general `OpTailCallMixed`)**: extend the interp's `OpCallMixed` (`runtime/vm3/vm.go:647`) to consult `callee.JITCode` and route through `JITCallFn`, paralleling the Phase 6.2c hook on `OpCallI64`. Extend `JITCallFn`'s signature to carry `argsCell []vm3.Cell`. Lower `OpCallMixed` and non-self `OpTailCallMixed` in the JIT so a JIT'd entry can call a JIT'd callee without going back through the interpreter. The second checkpoint: `lists_fill_sum`'s `main` is still interp but the boundary is generic so other corpus shapes inherit the same wiring.
- **6.2d.2.c — Inline list write (`OpListPushI64`, `OpListSetI64`)**: lower the read-write list ops with the inline fast path (`if len < cap: cells[len] = CInt(val); len++; else deopt`). After 6.2d.2.c the `fill` function is JIT'd; the third checkpoint expects `lists_fill_sum` inside 2x of Go on a freshly-warmed slab. `OpNewList` stays a deopt-to-interp call site for now; with `lists_fill_sum`'s single allocation outside the hot loop, that deopt is amortized away.
...
- 6.2d.2.a: `BenchmarkCorpusJITRunner/lists_fill_sum_n128` ratio improves from 21.4x toward the fill-bound floor (estimated 4-6x of Go fair).
- 6.2d.2.b: no kernel regresses; mixed-bank call boundary unit test passes.
- 6.2d.2.c: `lists_fill_sum_n128` inside 2x of Go.
