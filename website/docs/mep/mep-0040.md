...
- 6.2d.2.b: no kernel regresses; mixed-bank call boundary unit test passes. *(Step 1 met 2026-05-19: `TestCrossFnCellBankCallMixed` validates a JIT'd caller BLR-ing into a JIT'd cell-bank callee end-to-end on `n ∈ {0, 1, 2, 8, 32, 128}`; `lists_fill_sum_n128` corpus bench unchanged at ~352 ns/op (no regression). Step 2 landed 2026-05-19: `TestListsFillSumKernelsCompile`/`TestListsFillSumEndToEnd` validate `main` admission via `JITPreAllocList` + cross-fn BLR deopt-passthrough; the corpus bench moved from `360 ns/op` (2.67x) to `~470 ns/op` (3.5x) due to the per-iter list slab truncate/realloc cycle in `truncateToMarks`. Closing under the 2x gate is held until step 2.E adds warm-cache slot recycling.)*
