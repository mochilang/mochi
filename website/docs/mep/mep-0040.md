...
- **6.2d.2.b — Mixed call boundary (`OpCallMixed` / general `OpTailCallMixed`)** *(interp side landed in 6.2d.2.a; landed step 1: cross-fn JIT infrastructure 2026-05-19)*: the interp `OpCallMixed` (`runtime/vm3/vm.go`) already consults `callee.JITCode` and routes through `JITCallFn`, paralleling the Phase 6.2c hook on `OpCallI64`. `JITCallFn` carries `argsCell []vm3.Cell`.
  ...
  - **Step 2 — admit `lists_fill_sum` main (deferred)**: inline `OpNewList` lowering, deopt-passthrough so `OpListPushI64`-bearing callees (`fill`) can be JIT'd-call'd from `main`, and cross-fn `OpTailCallMixed`. Closes the residual interp dispatch of `main` (per the 6.2d.2.c.4 breakdown, removes ~50 ns of `RunWithArgs` setup + interp dispatch and one of the two trampoline crossings ~50 ns, projecting to ~235-265 ns/op vs Go fair ~135 ns/op = 1.74-1.96x; meets the under-2x gate with headroom).
...
- 6.2d.2.b: no kernel regresses; mixed-bank call boundary unit test passes. *(Step 1 met 2026-05-19: `TestCrossFnCellBankCallMixed` validates a JIT'd caller BLR-ing into a JIT'd cell-bank callee end-to-end on `n ∈ {0, 1, 2, 8, 32, 128}`; `lists_fill_sum_n128` corpus bench unchanged at ~352 ns/op (no regression). Closing the under-2x gate on `lists_fill_sum` is held until step 2 admits `main`.)*
