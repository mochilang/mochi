...
`-1` lexes as two tokens. A single-token negative literal would break subtraction in postfix contexts (`xs[len(xs)-1]`); MEP 2's expression grammar handles unary minus at the operand level, with structural symmetry on both sides of every binary operator.
