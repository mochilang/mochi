...
- **Negative numeric literals.** Today `x == -1` and `x == - 1` both fail to parse; only `x == (-1)` works. Fixing this requires the parser to accept a `Unary` after a binary operator, not a `PostfixExpr`. Out of scope for the lexer; tracked here so future maintainers do not move the burden onto the lexer by mistake.
