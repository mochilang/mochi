...
The master correctness gate is byte-equal stdout from the produced `.rb` (or built binary) versus vm3 on the entire fixture corpus, across CRuby 3.2 LTS, CRuby 3.4, CRuby 4.0, JRuby 10, and TruffleRuby 33. vm3 is the recording oracle for `expect.txt`; the transpiler does not link against or depend on vm3.
...
| `fun f(a: T): U { ... }` | `def self.f(a); ...; end` (inside module wrapper) |
| `let f = fun(a: T): U => body` | `f = ->(a) { body }` |
| `match e { Variant(x) => arm }` | `case e; in Variant(x) then arm; end` |
| `type T = A \| B` | `A = Data.define(...); B = Data.define(...)` |
...
| `chan <- v` | `chan.push(v)` |
| `<- chan` | `chan.pop` |
| `agent A { ... } a = AgentLit{...}` | `class A; ...; end; a = A.new(...)` |
...
`mochi-runtime` (Apache-2.0, ~300 LOC of Ruby) exports:
...
- `Mochi::Runtime::Panic < StandardError` with `attr_reader :code`
- `Mochi::Runtime::IO.putln(value)` (float formatting matches vm3)

Stdlib is the fat runtime: `csv`, `json`, `net/http`, `uri`, `set`, `fileutils`, `pathname`, `time`.
...
|-------------|-----------|-----------|-----------|----------|----------------|---------|
| Scalars / arithmetic | required | required | required | required | required | required |
| Lists / maps / sets | required | required | required | required | required | best-effort |
| Records / sums (Data.define) | required | required | required | required | required | n/a (mruby has no Data.define) |
| Closures | required | required | required | required | required | required |
| Queries / Datalog | required | required | required | required | required | best-effort |
| Channels / Threads | required | required | required | required | required | n/a (mruby has no Thread) |
| Streams (Thread::SizedQueue) | required | required | required | required | required | n/a |
| Async (Thread) | required | required | required | required | required | n/a |
| Agents | required | required | required | required | required | n/a |
| File / JSON / CSV / HTTP | required | required | required | required | required | best-effort |
