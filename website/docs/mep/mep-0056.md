...
The **intended** master correctness gate is byte-equal stdout from the produced `.rb` (or built binary) versus vm3 on the entire fixture corpus, across CRuby 3.2 LTS, CRuby 3.4, CRuby 4.0, JRuby 10, and TruffleRuby 33. As of 2026-05-29, only CRuby 4.0 (Homebrew, local) is exercised in the per-phase tests; the remaining runtimes land as N.1, N.2, ... sub-phases per the umbrella-phase coverage rule. See [/docs/implementation/0056/](/docs/implementation/0056/) for the live status. vm3 is the recording oracle for `expect.txt`; the transpiler does not link against or depend on vm3.
...
| Set `{1, 2}` / `add(s, x)` / `has(s, x)` | `Set.new([1, 2])` / `(s \| Set[x])` / `s.include?(x)` |
| OMap `omap{"a": 1}` / `m["a"]` / `has(m, k)` | insertion-ordered `Hash`, `m.fetch("a")`, `m.key?(k)` |
...
| `jsonDecode(s)` | `(require 'json'; JSON.parse(s))` |
| `httpGet(url)` | `(require 'open-uri'; URI.parse(url).open.read)` |
| `(int)f` numeric cast | `f.to_i` (other casts via `Float()`, `String()`, etc.) |
| `!b` boolean negation | `!b` |
| Bareword identifier collisions (`end`, `class`, `module`, ...) | suffix with `_` (e.g., `end_`) to avoid Ruby keyword clash |

### 4. Runtime library

`mochi-runtime` (Apache-2.0, ~110 LOC of Ruby across `runtime.rb`, `runtime/io.rb`, `runtime/panic.rb`, `runtime/stream.rb`, `runtime/version.rb`) exports:
...
See [/docs/implementation/0056/](/docs/implementation/0056/) for the per-phase tracking matrix. Twenty-eight phases cover language constructs (0-21) and packaging targets (22-27).

A phase is LANDED only when its gate is green on every Ruby implementation listed for it in §6 below.

### 6. Target matrix

The matrix below is the **intended** correctness gate, not the current CI matrix. As of 2026-05-29, only CRuby 4.0 (Homebrew, local) is exercised; CRuby 3.2 LTS, 3.4, JRuby 10, TruffleRuby 33, and mruby 4 are reserved as future sub-phases (e.g., phase 0.1 JRuby 10 on phase 0) per the umbrella-phase coverage rule. See [/docs/implementation/0056/](/docs/implementation/0056/) for the live runtime-matrix status.

| Phase scope | CRuby 3.2 | CRuby 3.4 | CRuby 4.0 | JRuby 10 | TruffleRuby 33 | mruby 4 |
|-------------|-----------|-----------|-----------|----------|----------------|---------|
| Scalars / arithmetic | required | required | LANDED (local) | required | required | required |
| Lists / maps / sets | required | required | LANDED (local) | required | required | best-effort |
| Records / sums (Data.define) | required | required | LANDED (local) | required | required | n/a (mruby has no Data.define) |
| Closures | required | required | LANDED (local) | required | required | required |
| Queries / Datalog | required | required | LANDED (local) | required | required | best-effort |
| Channels / Threads | required | required | LANDED (local) | required | required | n/a (mruby has no Thread) |
| Streams (Thread::SizedQueue) | required | required | LANDED (local) | required | required | n/a |
| Async (Thread) | required | required | required (local) | required | required | n/a |
| Agents | required | required | LANDED (local) | required | required | n/a |
| File / JSON / CSV / HTTP | required | required | LANDED (local) | required | required | best-effort |
