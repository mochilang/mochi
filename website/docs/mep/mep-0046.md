...
| 3.4 | `omap[K,V]`: literal, index, len, ordered iteration | LANDED | TBD |
...
| 5.1 | Nested patterns; guard clauses → `c_case` with guards | DEFERRED | — |
| 5.2 | Mochi `option[T]` lowering to `{some, V}` / `none` (matches Erlang idiom) | DEFERRED | — |
| 5.3 | Mochi `Result[T, E]` lowering to `{ok, V}` / `{error, E}` | DEFERRED | — |
