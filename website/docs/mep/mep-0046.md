...
| 3.3 | `set[T]`: literal, add, has, len (over `sets:set/0` v2) | DEFERRED | — |
...
| 14.2 | JSON parse via stdlib `json` (OTP 27); `json_decode(s) -> map<string,string>`; `mochi_json.erl` wraps `json:decode/1` with value coercion | LANDED | TBD |
