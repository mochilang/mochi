...
| 13.2 | Live provider calls (OpenAI, Anthropic) with API key from env | LANDED | TBD |
...
| #    | Scope | Status | Commit |
|------|-------|--------|--------|
| 15.0 | `TargetRelease`: emit rebar3 project + run `rebar3 release` (skips if rebar3 absent) | LANDED | TBD |
...
| 15.3 | `--target=beam-atomvm` emits `.avm` bundle for AtomVM | DEFERRED | — |
...
| Status   | IN PROGRESS |
| Commit   | — |
| Gate     | `mochi build --target=beam-*` ships with all of Phases 1-18 green; user-facing `docs/manual/build-beam.md` page documents the build flow; release notes filed; `mochi` published to Hex.pm |
| Targets  | All Tier 1 |
...
| 19.2 | `mochi` runtime published to Hex.pm under Apache-2.0 | DEFERRED | — |
| 19.3 | MEP-46 status flipped to Final | DEFERRED | — |
