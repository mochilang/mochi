...
| 5.1 | Guard clauses `when <expr>` → `c_case` guard position; `when` added as keyword | LANDED | — |
| 5.2 | `Some(value: T)` / `None` union lowering to `{some, V}` / `none` atom (Erlang idiom) | LANDED | — |
| 5.3 | `Ok(value: T)` / `Err(code: E)` union lowering to `{ok, V}` / `{error, V}` | LANDED | — |
...
| 6.2 | Partial application via Mochi sugar → BEAM fun with captured env | DEFERRED | — |
