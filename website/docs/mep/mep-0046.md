...
| 1.2 | `.mochi/cache/beam/` BLAKE3 content-addressed cache; rebuild on unchanged source is a copyFile no-op | LANDED | TBD |
...
| 6.1 | `lists:map/2`, `lists:filter/2`, `lists:foldl/3` mapping | LANDED | TBD |
...
| 7.2 | `group_by` → map accumulator; `lowerGroupByQueryExpr` in C lowerer; 3 fixtures (`TestPhase7_2GroupBy` green) | LANDED | TBD |
| 7.3 | Hash join via `maps:from_list/1` index; `extractHashJoinKeys` + `buildHashJoin` in C lowerer; `lowerStrConvertExpr` in BEAM lowerer; 2 fixtures (`TestPhase7_3HashJoin` green) | LANDED | TBD |
...
| 8.1 | Stratified negation-as-failure full test coverage; `dl_negation` fixture (`TestPhase8Datalog` green) | LANDED | TBD |
| 8.2 | `query X(a, b)` with multiple free variables; flat list interleaves free-var values in tuple order; `dl_multi_freevar` fixture (`TestPhase8Datalog` green) | LANDED | TBD |
...
| #   | Scope | Status | Commit |
|-----|-------|--------|--------|
| 14.0 | `mochi_fetch` via OTP `httpc`+`inets`+`ssl`; `fetch URL into var`; TLS verify_peer; 3 httptest fixtures | LANDED | TBD |
| 14.1 | TLS via `ssl` (OTP 27 TLS 1.3 default) | LANDED | TBD |
...
| 17.1 | Emit `-opaque` for agent/stream refs; `addOpaqueAttrs` in `spec.go` emits `-opaque <agent>_ref() :: map()` for each declared agent type | LANDED | TBD |
| 17.2 | CI job runs `dialyzer` on BEAM runtime `.erl` sources; `TestDialyzer` (in `dialyzer_test.go`) builds PLT + asserts zero warnings; `transpiler3-beam-dialyzer.yml` runs on OTP 27+28 | LANDED | TBD |
| 17.3 | False-positive allowlist documented in `dialyzer_allowlist.txt` with rationale | LANDED | TBD |
...
| 18.3 | Benchmark harness: `BenchmarkBeamBuild`, `BenchmarkBeamQuery`, `BenchmarkBeamRun` in `bench_test.go`; run with `go test -bench=. ./transpiler3/beam/build/` | LANDED | TBD |
...
| #    | Scope | Status | Commit |
|------|-------|--------|--------|
| 19.0 | `docs/manual/build-beam.mdx` written; CLI help text already documents `beam-escript` target | LANDED | TBD |
| 19.1 | Release notes + changelog entry | LANDED | TBD |
