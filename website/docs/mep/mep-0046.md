...
| 15.1 | `TargetRebar3Project` emits `rebar.config`, `src/`, `ebin/` into output dir | LANDED | TBD |
| 15.2 | `TargetMixProject` emits `mix.exs` and `ebin/` into output dir | LANDED | TBD |
| 15.3 | `--target=beam-atomvm` emits `.avm` bundle for AtomVM | DEFERRED | — |
| 15.4 | `Dockerfile.beam-otp27`: erlang:27-slim + mochi binary | LANDED | TBD |
