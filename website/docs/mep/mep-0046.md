...
| 9.1 | `spawn T(args)` → `mochi_agent_sup:start_child/2`; returns opaque agent ref | DEFERRED | — |
...
| #   | Scope | Status | Commit |
|-----|-------|--------|--------|
| 11.0 | `async expr` → `mochi_async:async/1` (monitored spawn) | DEFERRED | — |
| 11.1 | `await fut` → selective receive on fresh ref (uses recv-marker optimization) | DEFERRED | — |
| 11.2 | `await_all`, `await_any`, `await_timeout` combinators | DEFERRED | — |
...
| 12.1 | FFI: `extern "Erlang" mod <name> { fn(args): T }` declarations | DEFERRED | — |
