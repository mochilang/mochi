...
| 15.3 | `TargetAtomVM`: call `packbeam create` to emit `.avm` bundle (skips if packbeam absent) | LANDED | TBD |
...
| 19.2 | `mochi_runtime` Hex.pm package infrastructure: `{hex,[...]}` metadata in `runtime/rebar.config`; publish via `rebar3 hex publish` | LANDED | TBD |
| 19.3 | MEP-46 status flipped to Final | LANDED | TBD |
