...
| 5.1 | Capturing closures: free-variable capture by value into heap-allocated env struct; fat pointer `(code*, env*)` representation | NOT STARTED | — |
...
| 6.1 | String indexing `s[i]` (one-codepoint string), `contains`, `startsWith`, `endsWith` | NOT STARTED | — |
| 6.2 | `split`, `join`, `toUpper`, `toLower` via utf8proc | NOT STARTED | — |
| 6.3 | Format-string interpolation (`"{name} is {age}"` lowers to printf-style sequence) | NOT STARTED | — |
