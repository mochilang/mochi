...
| Status   | NOT STARTED |
| Commit   | — |
| Gate     | `mochi build tests/transpiler3/c/fixtures/hello/hello.mochi -o /tmp/hello && /tmp/hello \| diff - tests/transpiler3/c/fixtures/hello/expect.txt` exits 0 on host triple |
...
| 1.1 | `--out PATH` and `--emit=c` CLI flags wired through `cmd/mochi/main.go`; `--emit=c` dumps the generated C beside the binary for review | NOT STARTED | — |
| 1.2 | `.mochi/cache/` BLAKE3 content-addressed cache layer; rebuild on unchanged source is a no-op (`stat` only); cache key includes transpiler version, profile, target triple | NOT STARTED | — |
| 1.3 | Fallback to vendored `zig cc` when host `cc` discovery fails; `transpiler3/c/toolchain/zig/install.go` downloads zig 0.16 on first use and pins via SHA-256 manifest | NOT STARTED | — |
...
**Test set.** `go test ./transpiler3/c/build -run TestHello` (5 cases: build, cache-hit, --emit=c, --out=path, host-cc-discovery fallback).
...
| #   | Scope | Status | Commit |
|-----|-------|--------|--------|
| 2.0 | `int` (`int64_t`), `float` (`double`), `bool`; arithmetic ops; comparison ops; short-circuit `&&` / `||` | NOT STARTED | — |
| 2.1 | `if`/`else`, `while`, `return`, `break`, `continue` | NOT STARTED | — |
| 2.2 | `for x in start..end` (int range); user-defined functions (multi-arg, multi-return-via-tuple deferred to Phase 3) | NOT STARTED | — |
| 2.3 | Integer divide-by-zero raises `MOCHI_ERR_DIVZERO` (checked profile); UB under `--fast-int` | NOT STARTED | — |
| 2.4 | Float NaN propagation matches vm3 byte-for-byte (IEEE 754 round-trip on `%.17g` print) | NOT STARTED | — |
...
**Risks.** Swiss-table integration on Windows (cwisstable upstream tests on MSVC pass per their CI; sanity check at 3.2). Determinism of monomorphisation pass output order (3.4 sorts by canonical type printing, so two builds of the same program produce identical generated runtime files).
