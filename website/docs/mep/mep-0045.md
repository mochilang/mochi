...
| 3.4d | `list` / `map` equality (`==`, `!=`) -- DEFERRED: vm3 has no list / map comparison opcodes; fixture expected outputs cannot be recorded from the oracle. Reactivates when vm3 gains `OpCmpEqList` / `OpCmpEqMap` (likely alongside Phase 6 string/slice unification or a dedicated collection-ops MEP) | DEFERRED    | — |
