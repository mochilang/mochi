...
**2. `sum()` return type disagrees between inferrer and checker.** `types/infer.go` returns the element type; `types/check.go` widens to `int` or `float`. The rule in this MEP is the inferrer's. Fix: change the checker to match.
...
**16. Boolean connectives produce `any` when an operand is non-bool.** Should be T030. Fix: replace the loose branch in `inferBinaryType` for `&&` and `||`.
