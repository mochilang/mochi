...
**3. Empty list and map literals self-type.** `[]` infers as `[any]`; `{}` infers as `{any:any}`. Under the new rule both should produce a fresh type variable and require a use site to fix it. **Status: partial.** The annotated case (`let xs: list<int> = []`) is sound today via the `elementContext` carve-out in `types/subtype.go` (`assignableAt`), which admits the literal's any-elements into a concrete slot only at the literal site, not at later uses of a bound name. The unannotated case (`let xs = []; push(xs, 1); push(xs, "s")`) is still unsound because the binding's `[any]` type then accepts any element forever. Closing the unannotated case requires three pieces in concert: (a) the empty literal returning a fresh `TypeVar` instead of `AnyType`, (b) `Subtype`/`Assignable` learning a `TypeVar` rule, and (c) a persistent `Subst` across statements plus `generalise` at the `let` binder. (a) and (c) depend on Algorithm W plumbing tracked in [MEP 12](/docs/mep/mep-0012); P3 stays open until those land.
