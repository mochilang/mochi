...
**17. Closure return type is inferred without parameter types.** A lambda without annotations is typed at `(any, …) → any`. The strict rule requires either annotations or an enclosing expected type. **Status: closed for the call-site case.** `checkFunExpr` now routes missing parameter types through the call-site expected `FuncType` when one is available, so `apply(fun(x) => x + 1, 5)` types `x : int` from the expected `fun(int): int`. The fallback to `any` only fires for top-level unannotated lambdas with no enclosing expected type; that residue is the same as P3's unannotated-let-generalisation problem and stays parked on MEP-12 Algorithm W plumbing.
