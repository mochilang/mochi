...
**19. The `ExprType` / `ExprTypeHint` split duplicates list/map hint logic.** Should be a single bidirectional pass. Fix: collapse the two entry points and route the hint through the recursion. **Status: closed.** Both public entry points in `types/infer.go` now delegate to a single shared `exprTypeWithHint` helper; `ExprType` is `exprTypeWithHint(e, nil, env)` and `ExprTypeHint` is `exprTypeWithHint(e, hint, env)`. The duplicated list/map hint logic is gone. Deeper bidirectional propagation (threading the hint through every recursion node, not just the top-level literal) is the Algorithm W work tracked on [MEP 12](/docs/mep/mep-0012); the synthesis pass here intentionally stops at the outer literal.
