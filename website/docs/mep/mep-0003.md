...
3. **AgentBlock position.** `AgentBlock` does not carry a `lexer.Position`. Diagnostics on an agent body can attach to the body's first child but not to the block itself. The fix is structural (add `Pos lexer.Position` and update the parser tag).
...
7. **FunExpr default return.** A lambda with no annotated return uses `AnyType` at check time. The AST does not record this default; consumers that introspect `Return` must remember that `nil` means "infer", not "void". A canonical `unit` type would remove the special case.
