...
13. **Recursive transparency.** Sub-expressions (the value of a `LetStmt`, the body of a block, the operand of a postfix op, etc.) are themselves validated. A corrupt arm anywhere in the tree fails at the closest enclosing position.
...
1. **ExprStmt observability.** Any `Expr` can be a statement. Many shapes have no observable effect (`1 + 1`, `xs[0]` without binding, `someFun` without call). The language currently accepts these silently. A "useless expression statement" diagnostic would catch them at parse time without changing the grammar.
2. **ImportStmt.Lang shape.** `Lang *string` admits any string. Only a small set of values (`go`, `python`, `typescript`, ...) is meaningful, and unknown values either resolve through a registry or silently no-op. An enum over registered languages would surface the typo `improt "x" lang "pythn"` at parse time.
3. **FunExpr default return.** A lambda with no annotated return uses `AnyType` at check time. The AST does not record this default; consumers that introspect `Return` must remember that `nil` means "infer", not "void". A canonical `unit` type would remove the special case.
