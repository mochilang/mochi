...
12. **Recursive transparency.** Sub-expressions (the value of a `LetStmt`, the body of a block, the operand of a postfix op, etc.) are themselves validated. A corrupt arm anywhere in the tree fails at the closest enclosing position.
...
3. **ImportStmt.Lang shape.** `Lang *string` admits any string. Only a small set of values (`go`, `python`, `typescript`, ...) is meaningful, and unknown values either resolve through a registry or silently no-op. An enum over registered languages would surface the typo `improt "x" lang "pythn"` at parse time.
4. **FunExpr default return.** A lambda with no annotated return uses `AnyType` at check time. The AST does not record this default; consumers that introspect `Return` must remember that `nil` means "infer", not "void". A canonical `unit` type would remove the special case.
