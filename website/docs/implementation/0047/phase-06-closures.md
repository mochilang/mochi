...
| # | Scope | Status | Commit |
|---|-------|--------|--------|
| 6.0 | Lambda `(x: int) -> x + 1` -> `invokedynamic LambdaMetafactory`; functional interface selection | NOT STARTED | — |
| 6.1 | HOF: `list.map`, `list.filter`, `list.reduce` -> `stream().map/filter/reduce` | NOT STARTED | — |
| 6.2 | Partial application via `_` placeholder -> synthesised capturing lambda | NOT STARTED | — |
| 6.3 | Self-recursive tail-call optimisation -> trampoline `while(true)` loop | NOT STARTED | — |
| 6.4 | Mutable capture: `var` binding captured by lambda -> `Cell<T>` box | NOT STARTED | — |
