...
| 7     | Query DSL (Iterator helpers + AsyncIterable)                            | [phase-07-query](/docs/implementation/0052/phase-07-query)                   | LANDED (Node + Deno + Bun); 7.2d merge join, 7.3 group-by, 7.4 top-K heap, 7.5 async sources deferred | (this PR) |
| 8     | Datalog                                                                 | [phase-08-datalog](/docs/implementation/0052/phase-08-datalog)               | LANDED (Node + Deno + Bun); 8.5 runtime engine, 8.6 aggregations, 8.7 incremental maintenance deferred | (this PR) |
| 9     | Agents (synchronous class with mutable fields + method dispatch)        | [phase-09-agents](/docs/implementation/0052/phase-09-agents)                 | LANDED (Node + Deno + Bun); 9.4 AsyncIterableQueue, 9.5 MochiAgent, 9.6 cast/call, 9.7 supervision, 9.8 AggregateError deferred | (this PR) |
| 10    | Streams (chan + stream as inline synchronous TS classes)                | [phase-10-streams](/docs/implementation/0052/phase-10-streams)               | LANDED (Node + Deno + Bun); 10.3 blocking suspend, 10.4 multi-fiber, 10.5 cancellation, 10.6 Web Streams interop deferred | (this PR) |
| 11    | Errors (panic + try-catch)                                              | [phase-11-async](/docs/implementation/0052/phase-11-async)                   | LANDED (Node + Deno + Bun); 11.3 async colour pass, 11.4 MochiResult, 11.5 `?` operator, 11.6 AggregateError deferred | (this PR) |
| 12    | FFI (Node N-API + Deno FFI + Bun FFI dispatch)                          | [phase-12-ffi](/docs/implementation/0052/phase-12-ffi)                       | LANDED (Node + Deno + Bun); 12.1 N-API dispatch, 12.2 Deno dlopen, 12.3 Bun ffi, 12.4 browser rejection, 12.5 pure-TS FFI from npm, 12.6 wasm, 12.7 async FFI, 12.8 struct ABI deferred | (this PR) |
| 13    | LLM (provider dispatch)                                                 | [phase-13-llm](/docs/implementation/0052/phase-13-llm)                       | LANDED (Node + Deno + Bun); 13.1 OpenAI HTTP, 13.2 Anthropic HTTP, 13.3 Gemini HTTP, 13.4 local llama, 13.5 retry / backoff / SSE deferred | (this PR) |
| 14    | fetch (built-in fetch on Node 18+, Deno, Bun, browser)                  | [phase-14-fetch](/docs/implementation/0052/phase-14-fetch)                   | LANDED (Node + Deno + Bun); 14.1 POST + headers, 14.2 streaming, 14.3 typed wrapper, 14.4 status-aware errors, 14.5 Temporal HTTP-date deferred | (this PR) |
| 15    | npm package build via tsc + npm pack                                    | [phase-15-npm-package](/docs/implementation/0052/phase-15-npm-package)       | LANDED (npm pack + Bun install gate); 15.1 tsc + Node install, 15.2 Deno install, 15.3 browser bundle, 15.4 full-corpus scale, 15.5 composite tsconfig deferred | (this PR) |
| 16    | Reproducible build (SOURCE_DATE_EPOCH + sorted tar)                     | [phase-16-repro](/docs/implementation/0052/phase-16-repro)                   | LANDED (same-host SHA invariant + no-host-leak + sorted files); 16.4 two-host CI matrix deferred | (this PR) |
| 17    | Deno JSR publish + Jupyter (Deno kernel) + browser bundle (esbuild)     | [phase-17-jsr-jupyter-browser](/docs/implementation/0052/phase-17-jsr-jupyter-browser) | LANDED (jsr.json + deno publish --dry-run + kernelspec + bun build / esbuild bundle); 17.3 browser stubs, 17.4 Playwright + Jupyter install deferred | (this PR) |
| 18    | npm Trusted Publishing (Sigstore + OIDC + provenance)                   | [phase-18-trusted-publishing](/docs/implementation/0052/phase-18-trusted-publishing) | LANDED (workflow emit + structural gates + provenance dry-run); 18.3 verdaccio round-trip, 18.4 real OIDC publish deferred | (this PR) |
