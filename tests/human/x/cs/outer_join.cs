...
class Program {
    static void Main() {
        ...
        var orders = new List<Order> {
            ...
            new Order{ id=101, customerId=2, total=125},
            new Order{ id=102, customerId=1, total=300},
            new Order{ id=103, customerId=5, total=80}
        };
        var left = from o in orders
                   join c in customers on o.customerId equals c.id into temp
                   from c in temp.DefaultIfEmpty()
                   select new { order = o, customer = c };
        var rightOnly = from c in customers
        ...
        foreach (var row in result) {
            if (row.order != null) {
                if (row.customer != null) {
                    Console.WriteLine($"Order {row.order.id} by {row.customer.name} - ${row.order.total}");
                } else {
