// Generated by Mochi compiler v0.10.24 on 2025-07-13T03:08:46Z
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main() {
  int tmp1[] = {1, 2, 3};
  list_int tmp2 = slice_list_int((list_int){3, tmp1}, 1, 3);
  _print_list_int(tmp2);
  printf("\n");
  int tmp3[] = {1, 2, 3};
  list_int tmp4 = slice_list_int((list_int){3, tmp3}, 0, 2);
  _print_list_int(tmp4);
  printf("\n");
  {
    int first = 1;
    char *tmp5 = slice_string("hello", 1, 4);
    if (!first && strlen(tmp5) > 0)
      printf(" ");
    if (strlen(tmp5) > 0)
      printf("%s", tmp5);
    if (strlen(tmp5) > 0)
