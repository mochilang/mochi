// Generated by Mochi compiler v0.10.26 on 2006-01-02T15:04:05Z
#include <stdio.h>
#include <stdlib.h>

typedef struct {
  int id;
  const char *name;
} customer_t;
typedef struct {
  int len;
  customer_t *data;
} customer_list_t;
customer_list_t create_customer_list(int len) {
  customer_list_t l;
  l.len = len;
  l.data = calloc(len, sizeof(customer_t));
  if (!l.data && len > 0) {
    fprintf(stderr, "alloc failed\n");
    exit(1);
  }
  return l;
}

typedef struct {
  int id;
  int customer_id;
  int total;
} order_t;
typedef struct {
  int len;
  order_t *data;
} order_list_t;
order_list_t create_order_list(int len) {
  order_list_t l;
  l.len = len;
  l.data = calloc(len, sizeof(order_t));
  if (!l.data && len > 0) {
    fprintf(stderr, "alloc failed\n");
    exit(1);
  }
  return l;
}

typedef struct {
  int order_id;
  int order_customer_id;
  const char *paired_customer_name;
  int order_total;
} result_item_t;
typedef struct {
  int len;
  result_item_t *data;
} result_item_list_t;
result_item_list_t create_result_item_list(int len) {
  result_item_list_t l;
  l.len = len;
  l.data = calloc(len, sizeof(result_item_t));
  if (!l.data && len > 0) {
    fprintf(stderr, "alloc failed\n");
    exit(1);
  }
  return l;
}

int _mochi_main() {
  customer_t customers[] = {(customer_t){.id = 1, .name = "Alice"},
                            (customer_t){.id = 2, .name = "Bob"},
                            (customer_t){.id = 3, .name = "Charlie"}};
  int customers_len = sizeof(customers) / sizeof(customers[0]);
  order_t orders[] = {(order_t){.id = 100, .customer_id = 1, .total = 250},
                      (order_t){.id = 101, .customer_id = 2, .total = 125},
                      (order_t){.id = 102, .customer_id = 1, .total = 300}};
  int orders_len = sizeof(orders) / sizeof(orders[0]);
  result_item_list_t tmp1 = create_result_item_list(orders_len * customers_len);
  int tmp2 = 0;
  for (int o_idx = 0; o_idx < orders_len; o_idx++) {
    order_t o = orders[o_idx];
    for (int c_idx = 0; c_idx < customers_len; c_idx++) {
      customer_t c = customers[c_idx];
      tmp1.data[tmp2] = (result_item_t){.order_id = o.id,
                                        .order_customer_id = o.customer_id,
                                        .paired_customer_name = c.name,
                                        .order_total = o.total};
      tmp2++;
    }
  }
  result_item_list_t result = tmp1;
  printf("--- Cross Join: All order-customer pairs ---\n");
  for (int tmp3 = 0; tmp3 < result.len; tmp3++) {
    result_item_t entry = result.data[tmp3];
    printf("Order %d (customerId: %d , total: $ %d ) paired with %s\n",
           entry.order_id, entry.order_customer_id, entry.order_total,
           entry.paired_customer_name);
  }
  free(result.data);
  return 0;
}
int main() { return _mochi_main(); }
