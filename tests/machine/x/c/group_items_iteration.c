...
int _mochi_main() {
  ...
  for (int gi = 0; gi < tmp5.len; gi++) {
    ...
    data_list_t tmp8 = data_list_t_create(_gp.items.len);
  ...
  for (int tmp10 = 0; tmp10 < groups.len; tmp10++) {
    ...
    tmp12.data[0] = (tmp_item_t){.tag = g.key, .total = total};
    list_int tmp13 = concat_list_int(tmp, tmp12);
    tmp = tmp13;
  }
  list_int tmp14 = list_int_create(tmp.len);
  int *tmp17 = (int *)malloc(sizeof(int) * tmp.len);
  int tmp15 = 0;
  for (int tmp16 = 0; tmp16 < tmp.len; tmp16++) {
    int r = tmp.data[tmp16];
    tmp14.data[tmp15] = r;
    tmp17[tmp15] = r.tag;
    tmp15++;
  }
