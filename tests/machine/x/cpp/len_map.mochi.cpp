...
int main() {
	([&](const auto& __v){ if constexpr(has_size<decay_t<decltype(__v)>>::value){ for(size_t i=0;i<__v.size();++i){ if(i) cout<<' '; cout<<__v[i]; } cout<<endl; } else { cout<<__v<<endl; } })(((int)unordered_map<string, int>{{std::string("a"), 1}, {std::string("b"), 2}}.size()));
	return 0;
}
