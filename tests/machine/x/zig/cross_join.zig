...
pub fn main() void {
    ...
    orders = &[_]std.AutoHashMap([]const u8, i32){ blk3: {
        var m = std.AutoHashMap(i32, i32).init(std.heap.page_allocator);
        ...
        for (orders) |o| {
            for (customers) |c| {
                _tmp0.append(blk6: {
                    ...
                    m.put("orderCustomerId", o.customerId) catch unreachable;
                    m.put("pairedCustomerName", c.name) catch unreachable;
                    m.put("orderTotal", o.total) catch unreachable;
                    break :blk6 m;
                }) catch unreachable;
            }
        }
