...
pub fn main() void {
    ...
    items = &[_]std.AutoHashMap([]const u8, i32){blk4: { var m = std.AutoHashMap(i32, i32).init(std.heap.page_allocator); m.put("orderId", 100) catch unreachable; m.put("sku", "a") catch unreachable; break :blk4 m; }, blk5: { var m = std.AutoHashMap(i32, i32).init(std.heap.page_allocator); m.put("orderId", 101) catch unreachable; m.put("sku", "b") catch unreachable; break :blk5 m; }};
    result = blk7: { var _tmp0 = std.ArrayList(std.AutoHashMap([]const u8, i32)).init(std.heap.page_allocator); for (orders) |o| { for (customers) |c| { if (!((o.customerId == c.id))) continue; for (items) |i| { if (!((o.id == i.orderId))) continue; _tmp0.append(blk6: { var m = std.AutoHashMap(i32, i32).init(std.heap.page_allocator); m.put("name", c.name) catch unreachable; m.put("sku", i.sku) catch unreachable; break :blk6 m; }) catch unreachable; } } } const _tmp1 = _tmp0.toOwnedSlice() catch unreachable; break :blk7 _tmp1; };
    std.debug.print("{s}\n", .{"--- Multi Join ---"});
    for (result) |r| {
