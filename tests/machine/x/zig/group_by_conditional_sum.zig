...
pub fn main() void {
    items = &[_]std.AutoHashMap([]const u8, i32){ blk0: {
        ...
        m.put("val", @as(i32, @intCast(10))) catch unreachable;
        m.put("flag", true) catch unreachable;
        break :blk0 m;
    }, blk1: {
        var m = std.AutoHashMap(i32, []const u8).init(std.heap.page_allocator);
        m.put("cat", "a") catch unreachable;
        m.put("val", @as(i32, @intCast(5))) catch unreachable;
        m.put("flag", false) catch unreachable;
        break :blk1 m;
    }, blk2: {
        var m = std.AutoHashMap(i32, []const u8).init(std.heap.page_allocator);
        m.put("cat", "b") catch unreachable;
        m.put("val", @as(i32, @intCast(20))) catch unreachable;
        m.put("flag", true) catch unreachable;
        break :blk2 m;
    } };
    result = blk6: {
        ...
        for (items) |i| {
            ...
            if (_tmp5.get(_tmp6)) |idx| {
                _tmp4.items[idx].Items.append(i) catch unreachable;
            } else {
        ...
        for (_tmp4.items) |g| {
            _tmp7.append(blk3: {
                ...
                m.put("share", (_sum_int(blk4: {
                    var _tmp0 = std.ArrayList(i32).init(std.heap.page_allocator);
                    for (g) |x| {
                        _tmp0.append(if (x.flag) (x.val) else (@as(i32, @intCast(0)))) catch unreachable;
                    }
                    ...
                    for (g) |x| {
                        _tmp2.append(x.val) catch unreachable;
                    }
        ...
        break :blk6 _tmp7.toOwnedSlice() catch unreachable;
    };
    std.debug.print("{any}\n", .{result});
}
