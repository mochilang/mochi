...
func main() {
	...
	result := func() []Result {
		...
		resAny := _query(src, []_joinSpec{
			{items: _toAnySlice(customers), on: func(_a ...any) bool {
				...
				_ = c
				return _equal(_getField(o, "customerId"), _getField(c, "id"))
				...
				_ = o
				return _getField(o, "customerId")
			}, rightKey: func(_v any) any { c := _v.(*Customer); _ = c; return _getField(c, "id") }, left: true, right: true},
	...
	fmt.Println(strings.TrimSpace(fmt.Sprintln("--- Outer Join using syntax ---")))
	for _, row := range result {
		if bool(row.Order) {
			if bool(row.Customer) {
				fmt.Println(strings.TrimSpace(fmt.Sprintln("Order", row.Order.ID, "by", row.Customer.Name, "- $", row.Order.Total)))
			} else {
				fmt.Println(strings.TrimSpace(fmt.Sprintln("Order", row.Order.ID, "by", "Unknown", "- $", row.Order.Total)))
			}
		} else {
			fmt.Println(strings.TrimSpace(fmt.Sprintln("Customer", row.Customer.Name, "has no orders")))
		}
	}
}
