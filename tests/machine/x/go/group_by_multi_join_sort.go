	...
	result := func() []Result {
		...
		for _, g := range items {
			results = append(results, Result{
				(g.Key).(map[string]any)["c_custkey"],
				(g.Key).(map[string]any)["c_name"],
				_sum(func() []any {
					results := []any{}
					for _, x := range g.Items {
						results = append(results, ((((x).(map[string]any)["l"]).(map[string]any)["l_extendedprice"]).(float64) * (float64(1) - (((x).(map[string]any)["l"]).(map[string]any)["l_discount"]).(float64)).(float64)))
				...
				(g.Key).(map[string]any)["c_acctbal"],
				(g.Key).(map[string]any)["n_name"],
				(g.Key).(map[string]any)["c_address"],
				(g.Key).(map[string]any)["c_phone"],
				(g.Key).(map[string]any)["c_comment"],
			})
		}
		return results
	}()
