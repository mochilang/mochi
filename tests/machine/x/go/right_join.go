...
func main() {
	...
	result := func() []Result {
		...
		resAny := _query(src, []_joinSpec{
			{items: _toAnySlice(orders), on: func(_a ...any) bool {
				...
				_ = c
				return _getField(c, "id")
			}, rightKey: func(_v any) any { o := _v.(*Order); _ = o; return _getField(o, "customerId") }, right: true},
			...
			_ = o
			return Result{
				CustomerName: string(_getField(c, "name")),
				Order:        Order(o),
			}
		}, skip: -1, take: -1})
	...
	fmt.Println(strings.TrimSpace(fmt.Sprintln("--- Right Join using syntax ---")))
	for _, entry := range result {
		if bool(entry.Order) {
			fmt.Println(strings.TrimSpace(fmt.Sprintln("Customer", entry.CustomerName, "has order", entry.Order.ID, "- $", entry.Order.Total)))
		} else {
			fmt.Println(strings.TrimSpace(fmt.Sprintln("Customer", entry.CustomerName, "has no orders")))
		}
	}
}
