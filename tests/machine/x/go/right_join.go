...
func main() {
	...
	var result []Result = func() []Result {
		...
		resAny := _query(src, []_joinSpec{
			{items: _toAnySlice(orders), on: func(_a ...any) bool {
				...
				var c CustomersItem
				if _tmp0 != nil {
					c = _cast[CustomersItem](_tmp0)
				}
				_ = c
				_tmp1 := _a[1]
				var o OrdersItem
				if _tmp1 != nil {
					o = _cast[OrdersItem](_tmp1)
				}
				_ = o
				...
				_ = c
				return c.Id
			}, rightKey: func(_v any) any { o := _cast[OrdersItem](_v); _ = o; return o.CustomerId }, right: true},
	...
	fmt.Println(_fmt("--- Right Join using syntax ---"))
	for _, entry := range result {
		if _exists(entry.Order) {
			fmt.Println(strings.TrimRight(strings.Join([]string{_fmt("Customer"), _fmt(entry.CustomerName), _fmt("has order"), _fmt(_cast[map[string]any](entry.Order)["id"]), _fmt("- $"), _fmt(_cast[map[string]any](entry.Order)["total"])}, " "), " "))
		} else {
			fmt.Println(strings.TrimRight(strings.Join([]string{_fmt("Customer"), _fmt(entry.CustomerName), _fmt("has no orders")}, " "), " "))
		}
	}
}
