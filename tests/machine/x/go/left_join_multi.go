...
func main() {
	...
	result := func() []Result {
		...
		resAny := _query(src, []_joinSpec{
			...
			{items: _toAnySlice(items), on: func(_a ...any) bool {
				...
				_ = i
				return _equal(o.ID, _getField(i, "orderId"))
			}, leftKey: func(_a ...any) any {
				tmp0 := _a[0]
				var o Order
				if tmp0 != nil {
				...
				_ = c
				return o.ID
			}, rightKey: func(_v any) any { i := _v.(*Item); _ = i; return _getField(i, "orderId") }, left: true},
	...
	fmt.Println(strings.TrimSpace(fmt.Sprintln("--- Left Join Multi ---")))
	for _, r := range result {
		fmt.Println(strings.TrimSpace(fmt.Sprintln(r.OrderID, r.Name, r.Item)))
	}
}
