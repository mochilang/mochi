...
public class GroupByMultiJoinSort {
	static List<Map<String,Object>> nation = new ArrayList<>(Arrays.asList(mapOfEntries(entry("n_nationkey", 1), entry("n_name", "BRAZIL"))));
	static List<Map<String,Object>> customer = new ArrayList<>(Arrays.asList(mapOfEntries(entry("c_custkey", 1), entry("c_name", "Alice"), entry("c_acctbal", 100.000000), entry("c_nationkey", 1), entry("c_address", "123 St"), entry("c_phone", "123-456"), entry("c_comment", "Loyal"))));
	static List<Map<String,Object>> orders = new ArrayList<>(Arrays.asList(mapOfEntries(entry("o_orderkey", 1000), entry("o_custkey", 1), entry("o_orderdate", "1993-10-15")), mapOfEntries(entry("o_orderkey", 2000), entry("o_custkey", 1), entry("o_orderdate", "1994-01-02"))));
	static List<Map<String,Object>> lineitem = new ArrayList<>(Arrays.asList(mapOfEntries(entry("l_orderkey", 1000), entry("l_returnflag", "R"), entry("l_extendedprice", 1000.000000), entry("l_discount", 0.100000)), mapOfEntries(entry("l_orderkey", 2000), entry("l_returnflag", "N"), entry("l_extendedprice", 500.000000), entry("l_discount", 0.000000))));
	...
	for (var __e : _groups7.entrySet()) {
		...
		_res6.add(mapOfEntries(entry("c_custkey", ((Map)g_key).get("c_custkey")), entry("c_name", ((Map)g_key).get("c_name")), entry("revenue", (new java.util.function.Supplier<List<Integer>>(){public List<Integer> get(){
