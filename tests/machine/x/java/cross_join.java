...
public class Main {
	...
	for (var o : orders) {
		for (var c : customers) {
			_res1.add(mapOfEntries(entry("orderId", ((Integer)((Map)o).get("id"))), entry("orderCustomerId", ((Integer)((Map)o).get("customerId"))), entry("pairedCustomerName", ((Map)c).get("name")), entry("orderTotal", ((Integer)((Map)o).get("total")))));
		}
	}
	return _res1;
}}).get();
