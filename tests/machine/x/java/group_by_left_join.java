...
public class GroupByLeftJoin {
	...
	static List<Map<String,Integer>> orders = new ArrayList<>(Arrays.asList(mapOfEntries(entry("id", 100), entry("customerId", 1)), mapOfEntries(entry("id", 101), entry("customerId", 1)), mapOfEntries(entry("id", 102), entry("customerId", 2))));
	...
	for (var c : customers) {
		List<Map<String,Integer>> _tmp10 = new ArrayList<>();
	...
	for (var __e : _groups9.entrySet()) {
		...
		_res8.add(mapOfEntries(entry("name", g_key), entry("count", (new java.util.function.Supplier<List<CO>>(){public List<CO> get(){
