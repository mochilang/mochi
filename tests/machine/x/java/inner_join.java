...
public class Main {
	...
	static List<Map<Object,Integer>> orders = new ArrayList<>(java.util.Arrays.asList(new LinkedHashMap<>(){{put("id", 100);put("customerId", 1);put("total", 250);}}, new LinkedHashMap<>(){{put("id", 101);put("customerId", 2);put("total", 125);}}, new LinkedHashMap<>(){{put("id", 102);put("customerId", 1);put("total", 300);}}, new LinkedHashMap<>(){{put("id", 103);put("customerId", 4);put("total", 80);}}));
	static List<Object> result = (new java.util.function.Supplier<List<Object>>() {public List<Object> get() {
	List<Object> _res1 = new ArrayList<>();
	for (var o : orders) {
		for (var c : customers) {
			if (!(Objects.equals(((Map)o).get("customerId"), ((Map)c).get("id")))) continue;
			_res1.add(new LinkedHashMap<>(){{put("orderId", ((Map)o).get("id"));put("customerName", ((Map)c).get("name"));put("total", ((Map)o).get("total"));}});
		}
	}
	return _res1;
}}).get();
