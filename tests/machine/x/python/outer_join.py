# Generated by Mochi compiler v0.10.27 on 2025-07-17T06:58:45Z
...
for row in result:
    if row.order:
        if row.customer:
            print(
                f"Order {_fmt(row.order.id)} by {_fmt(row.customer.name)} - $ {_fmt(row.order.total)}"
            )
        else:
            print(
                " ".join(
        ...
        print(" ".join(["Customer", _fmt(row.customer.name), "has no orders"]).rstrip())
