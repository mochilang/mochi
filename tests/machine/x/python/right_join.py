# Generated by Mochi compiler v0.10.27 on 2025-07-17T06:58:48Z
...
result = _query(
    ...
    {"select": lambda c, o: Auto1(customerName=c.name, order=o)},
)
print("--- Right Join using syntax ---")
for entry in result:
    if entry.order:
        print(
            f"Customer {_fmt(entry.customerName)} has order {_fmt(entry.order.id)} - $ {_fmt(entry.order.total)}"
        )
    else:
        print(
            " ".join(["Customer", _fmt(entry.customerName), "has no orders"]).rstrip()
        )
