...
class Program
{
    static void Main()
    {
        var nations = new List<Nation> { new Nation { id = 1, name = "A" }, new Nation { id = 2, name = "B" } };
        List<Dictionary<string, int>> suppliers = new List<Supplier> { new Supplier { id = 1, nation = 1 }, new Supplier { id = 2, nation = 2 } };
        var partsupp = new List<Partsupp> { new Partsupp { part = 100, supplier = 1, cost = 10.000000, qty = 2 }, new Partsupp { part = 100, supplier = 2, cost = 20.000000, qty = 1 }, new Partsupp { part = 200, supplier = 1, cost = 5.000000, qty = 3 } };
        var filtered = (
    from ps in partsupp
    ...
    select new Filtered { part = ps["part"], value = (ps["cost"] * ps["qty"]) }
        ...
        var grouped = _group_by<dynamic, dynamic>(filtered, x => x["part"]).Select(g => new Grouped { part = g.key, total = _sum(g.Items.Select(r => r["value"]).ToList()) }).ToList();
