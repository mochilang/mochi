...
public class Program {
    ...
    items = items.OrderBy(g => (-_sum(g.Select(x => (x["l"]["l_extendedprice"] * ((1 - x["l"]["l_discount"]))))))).ToList();
    var _res = new List<Dictionary<string, dynamic>>();
    foreach (var g in items) {
        _res.Add(new Dictionary<string, dynamic> { { "c_custkey", g["key"].c_custkey }, { "c_name", g["key"].c_name }, { "revenue", _sum(g.Select(x => (x["l"]["l_extendedprice"] * ((1 - x["l"]["l_discount"]))))) }, { "c_acctbal", g["key"].c_acctbal }, { "n_name", g["key"].n_name }, { "c_address", g["key"].c_address }, { "c_phone", g["key"].c_phone }, { "c_comment", g["key"].c_comment } });
    }
    return _res;
})();
