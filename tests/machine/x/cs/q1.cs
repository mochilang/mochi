// Generated by Mochi compiler v0.10.25 on 2025-07-13T05:20:34Z
using System;
...
class Program
{
    ...
    {
        expect(_equal(result, new List<Item> { new Item { returnflag = "N", linestatus = "O", sum_qty = 53, sum_base_price = 3000, sum_disc_price = (950.000000 + 1800.000000), sum_charge = (((950.000000 * 1.070000)) + ((1800.000000 * 1.050000))), avg_qty = 26.500000, avg_price = 1500, avg_disc = 0.075000, count_order = 2 } }));
    }

    static void Main()
    {
        List<Lineitem> lineitem = new List<Lineitem> { new Lineitem { l_quantity = 17, l_extendedprice = 1000.000000, l_discount = 0.050000, l_tax = 0.070000, l_returnflag = "N", l_linestatus = "O", l_shipdate = "1998-08-01" }, new Lineitem { l_quantity = 36, l_extendedprice = 2000.000000, l_discount = 0.100000, l_tax = 0.050000, l_returnflag = "N", l_linestatus = "O", l_shipdate = "1998-09-01" }, new Lineitem { l_quantity = 25, l_extendedprice = 1500.000000, l_discount = 0.000000, l_tax = 0.080000, l_returnflag = "R", l_linestatus = "F", l_shipdate = "1998-09-03" } };
        var result = _group_by<Lineitem, Key>(lineitem.Where(row => string.Compare(row.l_shipdate, "1998-09-02") <= 0), row => new Key { returnflag = row.l_returnflag, linestatus = row.l_linestatus }).Select(g => new Result { returnflag = g.key.returnflag, linestatus = g.key.linestatus, sum_qty = _sum(g.Items.Select(x => x.l_quantity).ToList()), sum_base_price = _sum(g.Items.Select(x => x.l_extendedprice).ToList()), sum_disc_price = _sum(g.Items.Select(x => (x.l_extendedprice * ((1 - x.l_discount)))).ToList()), sum_charge = _sum(g.Items.Select(x => ((x.l_extendedprice * ((1 - x.l_discount))) * ((1 + x.l_tax)))).ToList()), avg_qty = _avg(g.Items.Select(x => x.l_quantity).ToList()), avg_price = _avg(g.Items.Select(x => x.l_extendedprice).ToList()), avg_disc = _avg(g.Items.Select(x => x.l_discount).ToList()), count_order = Enumerable.Count(g) }).ToList();
    ...
    public class Result
    {
        public dynamic returnflag;
        public dynamic linestatus;
