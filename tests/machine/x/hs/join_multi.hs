...
result = [Map.fromList [("name", fromMaybe (error "missing") (Map.lookup "name" c)), ("sku", fromMaybe (error "missing") (Map.lookup "sku" i))] | o <- orders, c <- customers, i <- items, (fromMaybe (error "missing") (Map.lookup "customerId" (o)) == fromMaybe (error "missing") (Map.lookup "id" (c))), (fromMaybe (error "missing") (Map.lookup "id" (o)) == fromMaybe (error "missing") (Map.lookup "orderId" (i)))]

main :: IO ()
main = do
  putStrLn ("--- Multi Join ---")
  mapM_ (\r -> putStrLn (unwords [_showAny (fromMaybe (error "missing") (Map.lookup "name" r)), "bought item", _showAny (fromMaybe (error "missing") (Map.lookup "sku" r))])) result
