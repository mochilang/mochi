...
main = do
  putStrLn ("--- Adults ---")
  mapM_ (\person -> putStrLn (unwords [_showAny (fromMaybe (error "missing") (Map.lookup "name" person)), "is", _showAny (fromMaybe (error "missing") (Map.lookup "age" person)), if fromMaybe (error "missing") (Map.lookup "is_senior" person) then " (senior)" else ""])) adults
