...
result = [Map.fromList [("customerName", VString (fromMaybe (error "missing") (Map.lookup "name" c))), ("order", VString (o))] | o <- orders, c <- let _ms0 = [c | c <- customers, (fromMaybe (error "missing") (Map.lookup "customerId" o) == fromMaybe (error "missing") (Map.lookup "id" c))] in if null _ms0 then [Map.empty] else _ms0]
