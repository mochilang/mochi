...
stats = [Map.fromList [("name", key (g)), ("count", VInt (length [r | r <- filter (\r -> fromMaybe (error "missing") (Map.lookup "o" (r))) g]))] | g <- _group_by [(c, o) | c <- customers, o <- let _ms0 = [o | o <- orders, (fromMaybe (error "missing") (Map.lookup "customerId" (o)) == fromMaybe (error "missing") (Map.lookup "id" (c)))] in if null _ms0 then [Map.empty] else _ms0] (\(c, o) -> fromMaybe (error "missing") (Map.lookup "name" c)), let g = g]

main :: IO ()
main = do
  putStrLn ("--- Group Left Join ---")
  mapM_ (\s -> putStrLn (unwords [_showAny (fromMaybe (error "missing") (Map.lookup "name" s)), "orders:", _showAny (fromMaybe (error "missing") (Map.lookup "count" s))])) stats
