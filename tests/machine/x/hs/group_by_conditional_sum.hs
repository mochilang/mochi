-- Generated by Mochi compiler v0.10.26 on 2025-07-16T15:36:23Z
...
result = map snd (List.sortOn fst [(key (g), Map.fromList [("cat", VString (key (g))), ("share", VDouble ((sum [if _asBool (fromMaybe (error "missing") (Map.lookup "flag" (x))) then fromMaybe (error "missing") (Map.lookup "val" (x)) else 0 | x <- g] / sum [fromMaybe (error "missing") (Map.lookup "val" (x)) | x <- g])))]) | g <- _group_by [(i) | i <- items] (\(i) -> fromMaybe (error "missing") (Map.lookup "cat" i))])
