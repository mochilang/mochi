...
stats = [Map.fromList [("city", VString (key (g))), ("count", VInt (length (items g))), ("avg_age", VDouble ((sum [fromMaybe (error "missing") (Map.lookup "age" (p)) | p <- g] `div` length [fromMaybe (error "missing") (Map.lookup "age" (p)) | p <- g])))] | g <- _group_by people (\person -> fromMaybe (error "missing") (Map.lookup "city" person)), let g = g]

main :: IO ()
main = do
  putStrLn ("--- People grouped by city ---")
  mapM_ (\s -> putStrLn (unwords [_showAny (fromMaybe (error "missing") (Map.lookup "city" s)), ": count =", _showAny (fromMaybe (error "missing") (Map.lookup "count" s)), ", avg_age =", _showAny (fromMaybe (error "missing") (Map.lookup "avg_age" s))])) stats
