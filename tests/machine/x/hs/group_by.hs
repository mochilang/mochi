...
stats = [Map.fromList [("city", key (g)), ("count", VInt (length (items g))), ("avg_age", VDouble ((sum [fromMaybe (error "missing") (Map.lookup "age" (p)) | p <- g] `div` length [fromMaybe (error "missing") (Map.lookup "age" (p)) | p <- g])))] | g <- _group_by people (\person -> fromMaybe (error "missing") (Map.lookup "city" person)), let g = g]
