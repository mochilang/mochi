...
result = [Map.fromList [("orderId", VInt (fromMaybe (error "missing") (Map.lookup "id" o))), ("name", VString (fromMaybe (error "missing") (Map.lookup "name" c))), ("item", VString (i))] | o <- orders, c <- customers, i <- let _ms0 = [i | i <- items, (fromMaybe (error "missing") (Map.lookup "id" (o)) == fromMaybe (error "missing") (Map.lookup "orderId" (i)))] in if null _ms0 then [Map.empty] else _ms0, (fromMaybe (error "missing") (Map.lookup "customerId" (o)) == fromMaybe (error "missing") (Map.lookup "id" (c)))]
