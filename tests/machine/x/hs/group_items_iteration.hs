...
main :: IO ()
main = do
    mapM_ (\g -> fromMaybe () ((let total = 0 in case foldr (\x acc -> case (let total = (total + fromMaybe (error "missing") (Map.lookup "val" (x))) in Nothing) of Just v -> Just v; Nothing -> acc) Nothing items (g) of Just v -> Just v; Nothing -> (let tmp = _append tmp Map.fromList [("tag", VString (key (g))), ("total", VString (total))] in Nothing)))) groups
    print (result)
