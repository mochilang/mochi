:- style_check(-singleton).
get_item(Container, Key, Val) :-
    is_dict(Container), !, (string(Key) -> atom_string(A, Key) ; A = Key), get_dict(A, Container, Val).
get_item(Container, Index, Val) :-
    string(Container), !, string_chars(Container, Chars), nth0(Index, Chars, Val).
get_item(List, Index, Val) :- nth0(Index, List, Val).

:- initialization(main, main).
main :-
    dict_create(_V0, map, [Id-1, Name-"Alice"]),
    dict_create(_V1, map, [Id-2, Name-"Bob"]),
    dict_create(_V2, map, [Id-3, Name-"Charlie"]),
    Customers = [_V0, _V1, _V2],
    dict_create(_V3, map, [Id-100, CustomerId-1]),
    dict_create(_V4, map, [Id-101, CustomerId-1]),
    dict_create(_V5, map, [Id-102, CustomerId-2]),
    Orders = [_V3, _V4, _V5],
    get_item(O, 'customerId', _V6),
    get_item(C, 'id', _V7),
    get_item(G, 'key', _V8),
    get_item(R, 'o', _V9),
    findall(R, (member(R, G), _V9), _V10),
    length(_V10, _V11),
    dict_create(_V12, map, [Name-_V8, Count-_V11]),
    findall(_V12, (member(C, Customers), member(O, Orders), (_V6 == _V7), true), _V13),
    Stats = _V13,
    writeln("--- Group Left Join ---"),
    catch(
        (
            member(S, Stats),
                catch(
                    (
                        get_item(S, 'name', _V14),
                        writeln(_V14),
                        true
                    ), continue, true),
                    fail
                ; true
            ), break, true),
            true.
