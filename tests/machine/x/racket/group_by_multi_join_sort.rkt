...
(define result (let ([groups (make-hash)])
  (for* ([c customer] [o orders] [l lineitem] [n nation] #:when (and (equal? (hash-ref o 'o_custkey) (hash-ref c 'c_custkey)) (equal? (hash-ref l 'l_orderkey) (hash-ref o 'o_orderkey)) (equal? (hash-ref n 'n_nationkey) (hash-ref c 'c_nationkey)) (string=? (and (< (and (>= (hash-ref o 'o_orderdate) start_date) (hash-ref o 'o_orderdate)) end_date) (hash-ref l 'l_returnflag)) "R"))) (let ([key (hash 'c_custkey (hash-ref c 'c_custkey) 'c_name (hash-ref c 'c_name) 'c_acctbal (hash-ref c 'c_acctbal) 'c_address (hash-ref c 'c_address) 'c_phone (hash-ref c 'c_phone) 'c_comment (hash-ref c 'c_comment) 'n_name (hash-ref n 'n_name))] [bucket (hash-ref groups key '())]) (hash-set! groups key (cons (hash 'c c 'o o 'l l 'n n) bucket))))
  (define _groups (for/list ([k (hash-keys groups)]) (hash 'key k 'items (hash-ref groups k))))
  (set! _groups (sort _groups (lambda (a b) (> (let ([g a]) (- (apply + (for*/list ([x (hash-ref g 'items)]) (* (hash-ref (hash-ref x 'l) 'l_extendedprice) (- 1 (hash-ref (hash-ref x 'l) 'l_discount))))))) (let ([g b]) (- (apply + (for*/list ([x (hash-ref g 'items)]) (* (hash-ref (hash-ref x 'l) 'l_extendedprice) (- 1 (hash-ref (hash-ref x 'l) 'l_discount)))))))))))
  (for/list ([g _groups]) (hash 'c_custkey (hash-ref (hash-ref g 'key) 'c_custkey) 'c_name (hash-ref (hash-ref g 'key) 'c_name) 'revenue (apply + (for*/list ([x (hash-ref g 'items)]) (* (hash-ref (hash-ref x 'l) 'l_extendedprice) (- 1 (hash-ref (hash-ref x 'l) 'l_discount))))) 'c_acctbal (hash-ref (hash-ref g 'key) 'c_acctbal) 'n_name (hash-ref (hash-ref g 'key) 'n_name) 'c_address (hash-ref (hash-ref g 'key) 'c_address) 'c_phone (hash-ref (hash-ref g 'key) 'c_phone) 'c_comment (hash-ref (hash-ref g 'key) 'c_comment)))))
(displayln result)
