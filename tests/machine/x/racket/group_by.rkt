...
(define stats (let ([groups (make-hash)])
  ...
  (for/list ([g _groups]) (hash 'city (hash-ref g 'key) 'count (length (hash-ref g 'items)) 'avg_age (let ([xs (for*/list ([p (hash-ref g 'items)]) (hash-ref p 'age))] [n (length (for*/list ([p (hash-ref g 'items)]) (hash-ref p 'age)))]) (if (= n 0) 0 (/ (apply + xs) n)))))))
