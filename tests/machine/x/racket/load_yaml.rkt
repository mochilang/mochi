...
(define people (_simple-yaml-parse (file->string "/workspace/mochi/tests/interpreter/valid/people.yaml")))
(define adults (for*/list ([p people] #:when (and (_ge (hash-ref p 'age) 18))) (hash 'name (hash-ref p 'name) 'email (hash-ref p 'email))))
(for ([a (if (hash? adults) (hash-keys adults) adults)])
(displayln (string-join (map _to_string (list (hash-ref a 'name) (hash-ref a 'email))) " "))
)
