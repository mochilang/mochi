...
fn main() {
    ...
    let big = { let mut tmp1 = std::collections::HashMap::new();for p in &people { let key = p.city; tmp1.entry(key).or_insert_with(Vec::new).push(p.clone()); } let mut tmp2 = Vec::<Group>::new(); for (k,v) in tmp1 { tmp2.push(Group { key: k, items: v }); } let mut tmp3 = Vec::new(); for g in tmp2.into_iter() { if g.clone().items.len() as i32 >= 4 { tmp3.push(g); } } let mut result = Vec::new(); for g in tmp3 { result.push(Result { city: g.key, num: g.clone().items.len() as i32 }); } result };
    println!("{:?}", big);
}
