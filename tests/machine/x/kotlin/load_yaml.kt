// Generated by Mochi compiler v0.10.27 on 2025-07-17T07:14:05Z
...
val adults = run {
    ...
    for (p in people) {
        if (p.age >= 18) {
            __res.add((mutableMapOf("name" to (p.name), "email" to (p.email)) as MutableMap<String, String>))
        }
    }
    __res
}
...
fun main() {
    for (a in adults) {
        println(listOf((a as MutableMap<String, String>)["name"], (a as MutableMap<String, String>)["email"]).joinToString(" "))
    }
}
