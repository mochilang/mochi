// Generated by Mochi compiler v0.10.27 on 2025-07-17T07:13:33Z
fun String.starts_with(prefix: String): Boolean = this.startsWith(prefix)
// Code generated from inner_join.mochi

data class Customer(var id: Int, var name: String)

data class Order(var id: Int, var customerId: Int, var total: Int)

val customers = mutableListOf(Customer(id = 1, name = "Alice"), Customer(id = 2, name = "Bob"), Customer(id = 3, name = "Charlie"))

val orders = mutableListOf(Order(id = 100, customerId = 1, total = 250), Order(id = 101, customerId = 2, total = 125), Order(id = 102, customerId = 1, total = 300), Order(id = 103, customerId = 4, total = 80))

val result = run {
    val __res = mutableListOf<MutableMap<String, Any?>>()
    for (o in orders) {
        for (c in customers) {
            if (o.customerId == c.id) {
                __res.add((mutableMapOf("orderId" to (o.id), "customerName" to (c.name), "total" to (o.total)) as MutableMap<String, Any?>))
            }
        }
    }
    __res
}

fun main() {
    println("--- Orders with customer info ---")
    for (entry in result) {
        println(listOf("Order", (entry as MutableMap<String, Int>)["orderId"], "by", (entry as MutableMap<String, Int>)["customerName"], "- $", (entry as MutableMap<String, Int>)["total"]).joinToString(" "))
    }
}
