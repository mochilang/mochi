; Generated by Mochi compiler v0.10.25 on 2025-07-13T04:58:44Z
(ns main)
...
(defn -main []
  ...
  (->> _groups (sort-by (fn [g] (_sort_key (:key g)))) (map (fn [g] {:cat (:key g) :share (/ (_sum (vec (->> (for [x (:Items g)] (if (:flag x) (:val x) 0))))) (_sum (vec (->> (for [x (:Items g)] (:val x))))))})) vec))) ;; list of
  (println result)
)
