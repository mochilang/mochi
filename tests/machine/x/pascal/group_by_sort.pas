...
begin
  ...
  _tmp4.AddOrSetData('cat', g.key);
  SetLength(_tmp5, 0);
  for x in g do
    begin
      _tmp5 := Concat(_tmp5, [x.val]);
    end;
  _tmp4.AddOrSetData('total', specialize _sumList<Variant>(_tmp5));
  SetLength(_tmp6, 0);
  for x in g do
    begin
  ...
  for i in items do
    begin
      _tmp7 := Concat(_tmp7, [_tmp4]);
      _tmp8 := Concat(_tmp8, [-specialize _sumList<Variant>(_tmp6)]);
    end;
