; Generated by Mochi compiler v0.10.26 on 2025-07-16T16:35:07Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(set! adults (let ((_res '()))
  (for-each (lambda (person)
    (when (_ge (map-get person 'age) 18)
      (set! _res (append _res (list (list (cons 'name (map-get person 'name)) (cons 'age (map-get person 'age)) (cons 'is_senior (_ge (map-get person 'age) 60))))))
    )
