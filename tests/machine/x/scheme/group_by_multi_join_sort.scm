; Generated by Mochi compiler v0.10.26 on 2025-07-16T15:40:01Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
          ...
          (when (equal? (map-get l 'l_orderkey) (map-get o 'o_orderkey))
            (for-each (lambda (n)
              (when (equal? (map-get n 'n_nationkey) (map-get c 'c_nationkey))
                (when (and (and (_ge (map-get o 'o_orderdate) start_date) (_lt (map-get o 'o_orderdate) end_date)) (equal? (map-get l 'l_returnflag) "R"))
