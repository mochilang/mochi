object group_by_join {
  ...
  val stats = ((for { o <- orders; c <- customers; if o("customerId") == (c.id).asInstanceOf[Int] } yield (c("name"), (o, c))).groupBy(_._1).map{ case(k,list) => (k, list.map(_._2)) }.toList).map{ case(gKey,gItems) => { val g = (gKey, gItems); Map("name" -> (g._1), "count" -> ((g._2).size)) } }.toList
