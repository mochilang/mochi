object left_join_multi {
  ...
  val result = for { o <- orders; c <- customers; if o("customerId") == (c.id).asInstanceOf[Int]; i = items.find(i => o("id") == (i.orderId).asInstanceOf[Int]) } yield Map("orderId" -> (o("id")), "name" -> (c("name")), "item" -> (i))
