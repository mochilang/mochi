object group_by_having {
  ...
  val big = (((for { p <- people } yield (p.city, Big(p = p))).groupBy(_._1).map{ case(k,list) => _Group(k, list.map(_._2)) }.toList).filter{ g => { val g = g; (g).size >= 4 } }).map{ g => { val g = g; Map[String, Any]("city" -> (g.key), "num" -> ((g).size)) } }.toList
