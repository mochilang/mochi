object group_by {
  ...
  val stats = ((for { person <- people } yield (person("city"), (person))).groupBy(_._1).map{ case(k,list) => (k, list.map(_._2)) }.toList).map{ case(gKey,gItems) => { val g = (gKey, gItems); Map("city" -> (g._1), "count" -> ((g._2).size), "avg_age" -> ((for { p <- g._2 } yield p.age).sum.toDouble / (for { p <- g._2 } yield p.age).size)) } }.toList
