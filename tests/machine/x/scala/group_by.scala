object group_by {
  ...
  val stats = ((for { person <- people } yield (person.city, Stat(person = person))).groupBy(_._1).map{ case(k,list) => _Group(k, list.map(_._2)) }.toList).map{ g => { val g = g; Stat1(city = g.key, count = (g).size, avg_age = _safe_div((for { p <- g } yield p.age).sum.toDouble, (for { p <- g } yield p.age).size)) } }.toList
