object group_items_iteration {
  val data = List[Map[String, Any]](Map[String, Any]("tag" -> ("a"), "val" -> (1)), Map[String, Any]("tag" -> ("a"), "val" -> (2)), Map[String, Any]("tag" -> ("b"), "val" -> (3)))
  ...
  def main(args: Array[String]): Unit = {
    ...
    for(g <- groups) {
      ...
      tmp = tmp :+ Map[String, Any]("tag" -> (g.key), "total" -> (total))
    }
    println((result))
  }
}
