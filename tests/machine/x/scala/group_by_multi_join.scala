object group_by_multi_join {
  ...
  val grouped = ((for { x <- filtered } yield (x.part, Grouped(x = x))).groupBy(_._1).map{ case(k,list) => _Group(k, list.map(_._2)) }.toList).map{ g => { val g = g; Grouped1(part = g.key, total = (for { r <- g } yield r.value).sum) } }.toList
