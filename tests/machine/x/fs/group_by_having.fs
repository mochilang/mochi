...
let big = [ for gKey, gItems in [ for p in people do yield p ] |> List.groupBy (fun p -> p.city) |> List.filter (fun (gKey, gItems) -> let g = {| key = gKey; items = gItems |} in List.length g.items >= 4) do
