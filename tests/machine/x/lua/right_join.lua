...
result = (function()
    ...
    return __query(_src, {
        { items = orders, on = function(c, o) return (o.customerId == c.id) end, right = true }
    }, { selectFn = function(c, o) return {["customerName"]=c.name, ["order"]=o} end })
end)()
print("--- Right Join using syntax ---")
for _, entry in ipairs(result) do
    if entry.order then
        print(table.concat({tostring("Customer"), tostring(entry.customerName), tostring("has order"), tostring(entry.order.id), tostring("- $"), tostring(entry.order.total)}, ' '))
    else
        print(table.concat({tostring("Customer"), tostring(entry.customerName), tostring("has no orders")}, ' '))
    end
end
