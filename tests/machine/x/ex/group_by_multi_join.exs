...
defmodule Main do
  ...
  def main do
    # filtered :: list(map())
    filtered = for ps <- @partsupp, s <- @suppliers, n <- @nations, ((s.id == ps.supplier) && (n.id == s.nation)) && ((n.name == "A")), do: %{part: ps.part, value: (ps.cost * ps.qty)}
    # grouped :: list(map())
    grouped = Enum.map(_group_by(filtered, fn x -> x.part end), fn g -> %{part: g.key, total: _sum(for r <- g.items, do: r.value)} end)
    IO.inspect(grouped)
  end
  defmodule Group do
