...
defmodule Main do
  ...
  ], %{select: fn c, o, l, n -> [c, o, l, n] end, where: fn c, o, l, n -> (((o.o_orderdate >= @start_date) && (o.o_orderdate < @end_date)) && (l.l_returnflag == "R")) end })
  groups = _group_by(rows, fn [c, o, l, n] -> %{c_custkey: c.c_custkey, c_name: c.c_name, c_acctbal: c.c_acctbal, c_address: c.c_address, c_phone: c.c_phone, c_comment: c.c_comment, n_name: n.n_name} end)
  items = groups
  items = Enum.sort_by(items, fn g -> -_sum(for x <- g, do: (x.l.l_extendedprice * (1 - x.l.l_discount))) end)
  Enum.map(items, fn g -> %{c_custkey: g.key.c_custkey, c_name: g.key.c_name, revenue: _sum(for x <- g.items, do: (x.l.l_extendedprice * (1 - x.l.l_discount))), c_acctbal: g.key.c_acctbal, n_name: g.key.n_name, c_address: g.key.c_address, c_phone: g.key.c_phone, c_comment: g.key.c_comment} end)
end).()
    IO.inspect(result)
  end
  defmodule Group do
