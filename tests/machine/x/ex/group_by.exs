...
defmodule Main do
  ...
  def main do
    # stats :: list(map())
    stats = Enum.map(_group_by(@people, fn person -> person.city end), fn g -> %{city: g.key, count: _count(g), avg_age: _avg(for p <- g.items, do: p.age)} end)
    IO.inspect("--- People grouped by city ---")
    for s <- stats do
      IO.puts(Enum.join(Enum.map([s.city, ": count =", s.count, ", avg_age =", s.avg_age], &inspect(&1)), " "))
    end
  end
  defp _avg(v) do
  list = cond do
