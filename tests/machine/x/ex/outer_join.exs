...
defmodule Main do
  ...
  _query(src, [
    ...
    for row <- result do
      if row.order do
        if row.customer do
          IO.puts(Enum.join(Enum.map(["Order", row.order.id, "by", row.customer.name, "- $", row.order.total], &inspect(&1)), " "))
        else
          IO.puts(Enum.join(Enum.map(["Order", row.order.id, "by", "Unknown", "- $", row.order.total], &inspect(&1)), " "))
        end
      else
        IO.puts(Enum.join(Enum.map(["Customer", row.customer.name, "has no orders"], &inspect(&1)), " "))
      end
    end
  end
