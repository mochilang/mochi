// Generated by Mochi compiler v0.10.26 on 2025-07-16T16:30:19Z
import 'dart:io';
import 'dart:convert';
  ...
  for (var row in result) {
    if (row['order'] != null) {
      if (row['customer'] != null) {
        _print(['Order', row['order']['id'], 'by', row['customer']['name'], '- \$', row['order']['total']].join(' '));
      }
      else {
        _print(['Order', row['order']['id'], 'by', 'Unknown', '- \$', row['order']['total']].join(' '));
      }
    }
    else {
      _print(['Customer', row['customer']['name'], 'has no orders'].join(' '));
    }
  }
}
