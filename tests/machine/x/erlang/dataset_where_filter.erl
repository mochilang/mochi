...
main(_) ->
    ...
    lists:foreach(fun(Person) -> io:format("~p ~p ~p ~p~n", [maps:get(name, Person), "is", maps:get(age, Person), (case maps:get(is_senior, Person) of undefined -> ""; false -> ""; _ -> " (senior)" end)]) end, Adults).
