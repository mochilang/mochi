...
main(_) ->
    ...
    lists:foreach(fun(Entry) -> (case maps:get(order, Entry) of undefined -> io:format("~p ~p ~p~n", ["Customer", maps:get(customerName, Entry), "has no orders"]); false -> io:format("~p ~p ~p~n", ["Customer", maps:get(customerName, Entry), "has no orders"]); _ -> io:format("~p ~p ~p ~p ~p ~p~n", ["Customer", maps:get(customerName, Entry), "has order", maps:get(id, maps:get(order, Entry)), "- $", maps:get(total, maps:get(order, Entry))]) end) end, Result).
