...
main(_) ->
    ...
    Grouped = [#{part => Key0, total => lists:sum([maps:get(value, R) || R <- Val0])} || {Key0, Val0} <- maps:to_list(lists:foldl(fun({Key0, Val0}, Acc0) -> L = maps:get(Key0, Acc0, []), maps:put(Key0, [Val0 | L], Acc0) end, #{}, [{maps:get(part, X), #{X=X}} || X <- Filtered]))],
