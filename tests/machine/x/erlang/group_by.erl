...
main(_) ->
    ...
    Stats = [#{city => Key0, count => length(Val0), avg_age => (lists:sum([maps:get(age, P) || P <- Val0]) / length([maps:get(age, P) || P <- Val0]))} || {Key0, Val0} <- maps:to_list(lists:foldl(fun({Key0, Val0}, Acc0) -> L = maps:get(Key0, Acc0, []), maps:put(Key0, [Val0 | L], Acc0) end, #{}, [{maps:get(city, Person), #{Person=Person}} || Person <- People]))],
