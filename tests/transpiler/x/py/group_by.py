...
for g in _stats_groups.values():
    stats.append({"city": g["key"], "count": len(g["items"]), "avg_age": ((int(sum([p["age"] for p in g["items"]]) / len([p["age"] for p in g["items"]])) if float(sum([p["age"] for p in g["items"]]) / len([p["age"] for p in g["items"]])).is_integer() else sum([p["age"] for p in g["items"]]) / len([p["age"] for p in g["items"]])) if [p["age"] for p in g["items"]] else 0)})
