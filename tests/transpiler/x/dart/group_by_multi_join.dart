...
class Grouped {
  final dynamic part;
  final num total;
  const Grouped({required this.part, required this.total});
}
...
void main() {
  ...
  var res = <dynamic>[];
  for (var g in _list) {
    res.add(Grouped(part: g.key, total: [for (var r in g) r.value].isEmpty ? 0 : ([for (var r in g) r.value].reduce((a, b) => a + b))));
  }
  return res;
})();;
  print("[" + grouped.join(', ') + "]");
