// Generated by Mochi transpiler v0.10.32 on 2025-07-21 13:17 +0700
#[derive(Debug, Clone)]
struct PeopleItem {
    name: String,
    age: i64,
...
fn main() {
    ...
    let adults: Vec<QueryItem> = { let mut _q: Vec<QueryItem> = Vec::new(); for person in &people { if (person.age >= 18) { _q.push(QueryItem {name: person.clone().name.clone(), age: person.clone().age, is_senior: (person.clone().age >= 60)}); } } _q };
    println!("{}", "--- Adults ---");
