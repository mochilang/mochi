...
fn main() {
    ...
    let result: Vec<String> = { let mut _q: Vec<String> = Vec::new(); let mut _tmp: Vec<(i64, String)> = Vec::new(); for i in &items { _tmp.push((i.n, i.clone().v)); } _tmp.sort_by(|a,b| a.0.partial_cmp(&b.0).unwrap()); for (_,v) in _tmp { _q.push(v); } _q };
    println!("{:?}", result);
}
