...
fn main() {
    ...
    let adults: Vec<QueryItem> = { let mut _q: Vec<QueryItem> = Vec::new(); for p in &people { if (p.age >= 18) { _q.push(QueryItem {name: p.clone().name.clone(), email: p.clone().email.clone()}); } } _q };
    for a in &adults {
