...
main(_) ->
    ...
    Result = [#{"orderId" => maps:get("id", O), "customerName" => maps:get("name", C), "total" => maps:get("total", O)} ||
        O <- Orders,
        C <- Customers,
        (maps:get("customerId", O) == maps:get("id", C))],
    io:format("~s~n", ["--- Orders with customer info ---"]),
    lists:foreach(fun(Entry) ->
    io:format("~s ~p ~s ~p ~s ~p~n", ["Order", maps:get("orderId", Entry), "by", maps:get("customerName", Entry), "- $", maps:get("total", Entry)])
end, Result).
