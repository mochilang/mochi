...
main(_) ->
    ...
    Stats = lists:map(fun(P) ->
        Key = element(1, P),
        Items = element(2, P),
        #{"name" => Key, "count" => length(Items)} end, maps:to_list(lists:foldl(fun(P, Acc) ->
        K = element(1, P),
        V = element(2, P),
        maps:put(K, (maps:get(K, Acc, []) ++ [V]), Acc) end, #{}, [{maps:get("name", C), O} || O <- Orders, C <- Customers, (maps:get("customerId", O) == maps:get("id", C))]))),
