...
main(_) ->
    ...
    Expensive = lists:map(fun({_,V}) -> V end, lists:sublist(lists:nthtail(1, lists:sort(fun({K1,_},{K2,_}) -> K1 =< K2 end, [{-maps:get("price", P), P} ||
        P <- Products])), 3)),
    io:format("~s~n", ["--- Top products (excluding most expensive) ---"]),
    lists:foreach(fun(Item) ->
    io:format("~s ~s ~p~n", [maps:get("name", Item), "costs $", maps:get("price", Item)])
end, Expensive).
