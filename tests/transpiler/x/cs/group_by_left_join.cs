...
class Program {
    ...
    static GResult[] stats = (from c in customers join o in orders on c.id equals o.customerId into oTmp from o in oTmp.DefaultIfEmpty() group c by c.name into gTmp let g = new GGroup{ key = gTmp.Key, items = gTmp.ToArray() } select new GResult{name = g.key, count = (from r in g.items where r.o select r).ToArray().Length}).ToArray();
    static void Main() {
