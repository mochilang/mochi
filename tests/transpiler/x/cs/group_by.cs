...
class Program {
    ...
    static GResult[] stats = (from person in people group person by person.city into gTmp let g = new GGroup{ key = gTmp.Key, items = gTmp.ToArray() } select new GResult{city = g.key, count = g.Length, avg_age = ((from p in g select p.age).ToArray().Average())}).ToArray();
    static void Main() {
