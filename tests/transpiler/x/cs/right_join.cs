// Generated by Mochi 0.10.32 on 2025-07-21 14:48 +0700
using System;
...
struct OResult {
    ...
    public Order order;
    public override string ToString() => $"{{\"customerName\": \"{customerName}\", \"order\": {order}}}";
}
class Program {
    ...
    if (entry.order) {
    Console.WriteLine(string.Join(" ", new object[]{"Customer", entry.customerName, "has order", entry.order.id, "- $", entry.order.total}).TrimEnd());
} else {
    Console.WriteLine(string.Join(" ", new string[]{"Customer", entry.customerName, "has no orders"}).TrimEnd());
}
}
    }
}
