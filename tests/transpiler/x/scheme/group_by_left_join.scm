...
(define stats (let ((groups24 (make-hash-table)
        )
       (res27 (list)
        )
      )
     (begin (for-each (lambda (c)
           (for-each (lambda (o)
               (if (= (hash-table-ref o "customerId")
                 ...
                 (let* ((k26 (hash-table-ref c "name")
                      )
                     (g25 (hash-table-ref/default groups24 k26 #f)
                      )
                    )
                   (begin (if (not g25)
                       (begin (set! g25 (alist->hash-table (list (cons "key" k26)
                               (cons "items" (list)
                                )
                              )
                            )
                     ...
                     (hash-table-set! g25 "items" (append (hash-table-ref g25 "items")
         ...
         customers)
       (for-each (lambda (g)
           (set! res27 (append res27 (list (alist->hash-table (list (cons "name" (hash-table-ref g "key")
                      )
                     (cons "count" (length (let ((res23 (list)
                              )
                            )
                           (begin (for-each (lambda (r)
                                 (if (hash-table-ref r "o")
                                   (set! res23 (append res23 (list r)
                                      )
                                    )
                                   (quote nil)
                                  )
