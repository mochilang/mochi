...
console.log(["--- Right Join using syntax ---"].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
for (const entry of result) {
  if (entry["order"]) {
    console.log(["Customer", entry["customerName"], "has order", entry["order"]["id"], "- $", entry["order"]["total"]].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
  } else {
    console.log(["Customer", entry["customerName"], "has no orders"].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
  }
}
