...
export interface Result { orderId: any; customer: any; total: any }
...
console.log(["--- Left Join ---"].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
for (const entry of result) {
  console.log(["Order", entry["orderId"], "customer", entry["customer"], "total", entry["total"]].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
}
