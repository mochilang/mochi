...
export interface Result { orderId: any; customerName: any; total: any }
...
console.log(["--- Orders with customer info ---"].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
for (const entry of result) {
  console.log(["Order", entry["orderId"], "by", entry["customerName"], "- $", entry["total"]].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
}
