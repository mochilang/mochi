// Generated by Mochi v0.10.31 on 2025-07-20 12:04:49 GMT+7
...
console.log(["--- Adults ---"].map(x => String(x)).join(' ').trimEnd());
for (const person of adults) {
  console.log([person["name"], "is", person["age"], (person["is_senior"] ? " (senior)" : "")].map(x => String(x)).join(' ').trimEnd());
}
