// Generated by Mochi v0.10.35 on 2025-07-22 13:27:23 GMT+7
...
console.log(((v => Array.isArray(v) ? '[' + v.map(x => typeof x === 'string' ? '\'' + x + '\'' : String(x)).join(', ') + ']' : String(v))("--- Adults ---")));
for (const person of adults) {
  console.log([String(person["name"]), String("is"), String(person["age"]), String((person["is_senior"] ? " (senior)" : ""))].join(' ').trimEnd());
}
