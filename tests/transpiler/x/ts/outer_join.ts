// Generated by Mochi v0.10.33 on 2025-07-21 18:23:31 GMT+7

export interface Customer { id: number; name: string }
export interface Order { id: number; customerId: number; total: number }
export interface Result { order: any; customer: any }
const customers: Customer[] = [{id: 1, name: "Alice"}, {id: 2, name: "Bob"}, {id: 3, name: "Charlie"}, {id: 4, name: "Diana"}];
const orders: Order[] = [{id: 100, customerId: 1, total: 250}, {id: 101, customerId: 2, total: 125}, {id: 102, customerId: 1, total: 300}, {id: 103, customerId: 5, total: 80}];
const result: Result[] = (() => {
  let _items = orders.map(v => [v])
  { const _joined = []
    const _arr = customers
    const _matched = new Array(_arr.length).fill(false)
    for (const _left of _items) {
      const [o] = _left;
      let _m = false;
      for (let _ri=0; _ri < _arr.length; _ri++) {
        const c = _arr[_ri];
        if (!((o["customerId"] == c["id"]))) continue;
        _m = true; _matched[_ri] = true; _joined.push([..._left, c]) }
      if (!_m) _joined.push([..._left, null])
    }
    for (let _ri=0; _ri < _arr.length; _ri++) { if (!_matched[_ri]) {
      const _undef = Array(_items[0]?.length || 0).fill(null);
      _joined.push([..._undef, _arr[_ri]]) } }
    _items = _joined;
  }
  let _rows = _items
  const result: Result[] = []
  for (const r of _rows) { const [o, c] = r; result.push({order: o, customer: c}) }
  return result
})();
console.log(["--- Outer Join using syntax ---"].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
for (const row of result) {
  if (row["order"]) {
    if (row["customer"]) {
      console.log(["Order", row["order"]["id"], "by", row["customer"]["name"], "- $", row["order"]["total"]].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
    } else {
      console.log(["Order", row["order"]["id"], "by", "Unknown", "- $", row["order"]["total"]].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
    }
  } else {
    console.log(["Customer", row["customer"]["name"], "has no orders"].map(v => v === null ? 'nil' : typeof v === 'object' ? JSON.stringify(v).replace(/:/g, ': ').replace(/,/g, ', ') : v).join(' ').trimEnd());
  }
}
