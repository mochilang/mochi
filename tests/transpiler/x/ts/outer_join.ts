...
console.log(JSON.stringify("--- Outer Join using syntax ---"));
for (const row of result) {
  if (row["order"]) {
    if (row["customer"]) {
      console.log(JSON.stringify("Order"), JSON.stringify(row["order"]["id"]), JSON.stringify("by"), JSON.stringify(row["customer"]["name"]), JSON.stringify("- $"), JSON.stringify(row["order"]["total"]));
    } else {
      console.log(JSON.stringify("Order"), JSON.stringify(row["order"]["id"]), JSON.stringify("by"), JSON.stringify("Unknown"), JSON.stringify("- $"), JSON.stringify(row["order"]["total"]));
    }
  } else {
    console.log(JSON.stringify("Customer"), JSON.stringify(row["customer"]["name"]), JSON.stringify("has no orders"));
  }
}
