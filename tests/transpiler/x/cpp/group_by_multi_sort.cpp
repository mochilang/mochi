// Generated by Mochi transpiler v0.10.35 on 2025-07-22 14:39:35 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <map>
...
std::vector<GroupedItem> grouped = ([]{ std::vector<std::pair<double, GroupedItem>> __tmp;
for (auto g : ([&]{ std::vector<GGroup> __items;
...
return __items; }()); return std::accumulate(__tmp.begin(), __tmp.end(), 0.0); }()), GroupedItem{.a = g.key.a, .b = g.key.b, .total = ([&]{ auto __tmp = ([&]{ std::vector<int> __items;
for (auto x : g) {
        __items.push_back(x.val);
}
