// Generated by Mochi transpiler v0.10.32 on 2025-07-21 11:14:13 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <map>
...
int main() {
    std::cout << std::string("--- Cross Join: All order-customer pairs ---") << std::endl;
    for (auto entry : result) {
        std::cout << std::string("Order") <<  " " << entry.orderId <<  " " << std::string("(customerId:") <<  " " << entry.orderCustomerId <<  " " << std::string(", total: $") <<  " " << entry.orderTotal <<  " " << std::string(") paired with") <<  " " << entry.pairedCustomerName << std::endl;
    }
    return 0;
}
