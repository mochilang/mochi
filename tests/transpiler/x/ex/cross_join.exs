...
Enum.each(result, fn entry ->
  IO.puts("Order #{Kernel.inspect(entry.orderId)} (customerId: #{Kernel.inspect(entry.orderCustomerId)} , total: $ #{Kernel.inspect(entry.orderTotal)} ) paired with #{Kernel.inspect(entry.pairedCustomerName)}")
end)
