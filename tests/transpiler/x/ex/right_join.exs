# Generated by Mochi transpiler on 2025-07-21 16:57 +0700
customers = [%{id: 1, name: "Alice"}, %{id: 2, name: "Bob"}, %{id: 3, name: "Charlie"}, %{id: 4, name: "Diana"}]
orders = [%{id: 100, customerId: 1, total: 250}, %{id: 101, customerId: 2, total: 125}, %{id: 102, customerId: 1, total: 300}]
result = for o <- orders, c <- customers, o.customerId == c.id do
  %{
    customerName: c.name,
    order: o
  }
end
IO.puts("--- Right Join using syntax ---")
Enum.each(result, fn entry ->
  if entry.order do
    IO.puts("Customer #{Kernel.inspect(entry.customerName)} has order #{Kernel.inspect(entry.order.id)} - $ #{Kernel.inspect(entry.order.total)}")
  else
    IO.puts("Customer #{Kernel.inspect(entry.customerName)} has no orders")
  end
end)
