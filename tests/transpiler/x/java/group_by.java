public class Main {
    ...
    static java.util.List<Result4> stats = new java.util.ArrayList<Result4>() {{ java.util.LinkedHashMap<String,Group2> _groups = new java.util.LinkedHashMap<>(); for (var person : people) { var _k = person.city; String _ks = String.valueOf(_k); Group2 g = _groups.get(_ks); if (g == null) { g = new Group2(_k, new java.util.ArrayList<>()); _groups.put(_ks, g); } g.items.add(person); } for (var g : _groups.values()) { add(new Result4(g.key, g.items.size(), (((new java.util.ArrayList<Integer>() {{ for (var p : g.items) { add(p.age); }}}.stream().mapToDouble(v -> ((Number)v).doubleValue()).average().orElse(0)) % 1 == 0) ? (Object)(int)(new java.util.ArrayList<Integer>() {{ for (var p : g.items) { add(p.age); }}}.stream().mapToDouble(v -> ((Number)v).doubleValue()).average().orElse(0)) : (Object)(new java.util.ArrayList<Integer>() {{ for (var p : g.items) { add(p.age); }}}.stream().mapToDouble(v -> ((Number)v).doubleValue()).average().orElse(0))))); }}};
    static class Group2 {
        String key;
