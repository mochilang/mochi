...
(for ([entry result])
(displayln (string-join (filter (lambda (s) (not (string=? s ""))) (list (format "~a" "Order") (format "~a" (hash-ref entry "orderId")) (format "~a" "(customerId:") (format "~a" (hash-ref entry "orderCustomerId")) (format "~a" ", total: $") (format "~a" (hash-ref entry "orderTotal")) (format "~a" ") paired with") (format "~a" (hash-ref entry "pairedCustomerName")))) " "))
)
