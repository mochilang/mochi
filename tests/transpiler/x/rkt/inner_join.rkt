...
(for ([entry result])
(displayln (string-join (filter (lambda (s) (not (string=? s ""))) (list (format "~a" "Order") (format "~a" (hash-ref entry "orderId")) (format "~a" "by") (format "~a" (hash-ref entry "customerName")) (format "~a" "- $") (format "~a" (hash-ref entry "total")))) " "))
)
