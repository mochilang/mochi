...
(define result (for*/list ([o orders] [c customers] [i items]) #:when (and (= (hash-ref o "customerId") (hash-ref c "id")) (= (hash-ref o "id") (hash-ref i "orderId"))) (hash "name" (hash-ref c "name") "sku" (hash-ref i "sku"))))
(displayln "--- Multi Join ---")
(for ([r result])
(displayln (string-join (filter (lambda (s) (not (string=? s ""))) (list (format "~a" (hash-ref r "name")) (format "~a" "bought item") (format "~a" (hash-ref r "sku")))) " "))
)
