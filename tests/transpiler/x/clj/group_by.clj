;; Generated by Mochi v0.10.32 on 2025-07-20 21:29:09 GMT+7
(ns main)
...
(def stats (for [[k rows] (group-by :key (for [person people :let [k (:city person)]] {:key k :item person})) :let [g {:key k :items (map :item rows)}]] {:city (:key g) :count (count g) :avg_age (double (/ (reduce + 0 (for [p (:items g)] (:age p))) (count (for [p (:items g)] (:age p)))))}))
