;; Generated by Mochi v0.10.32 on 2025-07-21 12:07:46 GMT+7
(ns main)
...
(def big (for [g (for [[k rows] (group-by :key (for [p people :let [k (:city p)]] {:key k :item p})) :let [g {:key k :items (map :item rows)}] :when (>= (count g) 4)] g)] {:city (:key g) :num (count g)}))
