;; Generated by Mochi v0.10.32 on 2025-07-21 11:12:33 GMT+7
(ns main)
...
(def result (for [[k rows] (group-by :key (for [c (sort-by (fn [c] (- (reduce + 0 (for [x g] (* (:l_extendedprice (:l x)) (- 1 (:l_discount (:l x)))))))) customer) o orders l lineitem n nation :when (and (= (:o_custkey o) (:c_custkey c)) (= (:l_orderkey l) (:o_orderkey o)) (= (:n_nationkey n) (:c_nationkey c)) (= (and (neg? (compare (and (>= (compare (:o_orderdate o) start_date) 0) (:o_orderdate o)) end_date)) (:l_returnflag l)) "R")) :let [k {:c_custkey (:c_custkey c) :c_name (:c_name c) :c_acctbal (:c_acctbal c) :c_address (:c_address c) :c_phone (:c_phone c) :c_comment (:c_comment c) :n_name (:n_name n)}]] {:key k :item {:c c :o o :l l :n n}})) :let [g {:key k :items (map :item rows)}]] {:c_custkey (:c_custkey (:key g)) :c_name (:c_name (:key g)) :revenue (reduce + 0 (for [x (:items g)] (* (:l_extendedprice (:l x)) (- 1 (:l_discount (:l x)))))) :c_acctbal (:c_acctbal (:key g)) :n_name (:n_name (:key g)) :c_address (:c_address (:key g)) :c_phone (:c_phone (:key g)) :c_comment (:c_comment (:key g))}))
