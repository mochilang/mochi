;; Generated by Mochi v0.10.32 on 2025-07-21 12:29:45 GMT+7
(ns main)
...
(def stats (for [[k rows] (group-by :key (for [o orders c customers :when (= (:customerId o) (:id c)) :let [k (:name c)]] {:key k :item {:o o :c c}})) :let [g {:key k :items (map :item rows)}]] {:name (:key g) :count (count (:items g))}))
