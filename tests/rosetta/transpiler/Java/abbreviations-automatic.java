public class Main {
    ...
    static String[] distinct(String[] xs) {
        java.util.Map<String,Boolean> m = new java.util.LinkedHashMap<String, Boolean>();
        String[] out = new String[]{};
        int i = 0;
        while (i < xs.length) {
            String x = xs[i];
            if (!(Boolean)(m.containsKey(x))) {
m.put(x, true);
    ...
    static int abbrevLen(String[] words) {
        ...
        while (true) {
            ...
            int i = 0;
            while (i < size) {
                abbrs = java.util.stream.Stream.concat(java.util.Arrays.stream(abbrs), java.util.stream.Stream.of(takeRunes(words[i], l))).toArray(String[]::new);
                i = i + 1;
            }
    ...
    static void main() {
        ...
        while (i < lines.length) {
            ...
            System.out.println(pad2(l) + "  " + lines[i]);
            i = i + 1;
        }
    }
