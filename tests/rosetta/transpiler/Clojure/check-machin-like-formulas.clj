...
(defn tanEval [tanEval_coef tanEval_f]
  (try (do (when (= tanEval_coef 1) (throw (ex-info "return" {:v tanEval_f}))) (when (< tanEval_coef 0) (throw (ex-info "return" {:v (- (/ (bigint (tanEval (- tanEval_coef) tanEval_f)) 1))}))) (def tanEval_ca (/ tanEval_coef 2)) (def tanEval_cb (- tanEval_coef tanEval_ca)) (def tanEval_a (tanEval tanEval_ca tanEval_f)) (def tanEval_b (tanEval tanEval_cb tanEval_f)) (throw (ex-info "return" {:v (/ (/ (bigint (+ tanEval_a tanEval_b)) 1) (/ (bigint (- (/ (bigint 1) 1) (* tanEval_a tanEval_b))) 1))}))) (catch clojure.lang.ExceptionInfo e (if (= (ex-message e) "return") (get (ex-data e) :v) (throw e)))))

(defn tans [tans_m]
  (try (do (when (= (count tans_m) 1) (do (def tans_t (nth tans_m 0)) (throw (ex-info "return" {:v (tanEval (get tans_t "a") (br (get tans_t "n") (get tans_t "d")))})))) (def tans_half (/ (count tans_m) 2)) (def tans_a (tans (subvec tans_m 0 tans_half))) (def tans_b (tans (subvec tans_m tans_half (count tans_m)))) (throw (ex-info "return" {:v (/ (/ (bigint (+ tans_a tans_b)) 1) (/ (bigint (- (/ (bigint 1) 1) (* tans_a tans_b))) 1))}))) (catch clojure.lang.ExceptionInfo e (if (= (ex-message e) "return") (get (ex-data e) :v) (throw e)))))

(defn -main []
  (doseq [ts testCases] (println (str (str (str "tan " (format ts)) " = ") (str (tans ts))))))
