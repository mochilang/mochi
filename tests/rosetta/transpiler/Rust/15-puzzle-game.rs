...
unsafe fn isSolved() -> bool {
    let mut i: i64 = 0;
    while (i < 16) {
        if (g_board[i as usize] != g_solved[i as usize]) {
            return false
        }
        i = (i + 1);
    }
...
unsafe fn isValidMove(m: i64) -> Map {
    if (m == 0) {
        return Map {idx: (g_empty - 4), ok: ((g_empty / 4) > 0)}
    }
    if (m == 1) {
        return Map {idx: (g_empty + 4), ok: ((g_empty / 4) < 3)}
    }
    if (m == 2) {
        return Map {idx: (g_empty + 1), ok: ((g_empty % 4) < 3)}
    }
    if (m == 3) {
        return Map {idx: (g_empty - 1), ok: ((g_empty % 4) > 0)}
    }
...
unsafe fn doMove(m: i64) -> bool {
    ...
    let i: i64 = g_empty;
    let j: i64 = int(r.idx);
    let tmp: i64 = g_board[i as usize];
    g_board[i as usize] = g_board[j as usize];
...
unsafe fn printBoard() {
    ...
    let mut i: i64 = 0;
    while (i < 16) {
        let val: i64 = g_board[i as usize];
        if (val == 0) {
            line = format!("{}{}", line, "  .");
        } else {
            let s = val.to_string();
