...
unsafe fn exprEval(mut x: Expr) -> Rational {
    return match x { Expr::Num { value: v } => v, Expr::Bin { op: op, left: l, right: r } => binEval(op, l, r), }
}

unsafe fn exprString(mut x: Expr) -> String {
    return match x { Expr::Num { value: v } => v.num.to_string(), Expr::Bin { op: op, left: l, right: r } => binString(op, l, r), }.to_string()
}
...
unsafe fn solve(mut xs: Vec<Expr>) -> bool {
    ...
    while (i < xs.len() as i64) {
        ...
        while (j < xs.len() as i64) {
            ...
            let mut node: Expr = Expr::Bin { op: g_OP_ADD, left: Box::new(a), right: Box::new(b) };
            for op in vec![g_OP_ADD, g_OP_SUB, g_OP_MUL, g_OP_DIV] {
                node = Expr::Bin { op: op, left: Box::new(a), right: Box::new(b) };
                if true {
                    return true
                }
            }
            node = Expr::Bin { op: g_OP_SUB, left: Box::new(b), right: Box::new(a) };
            if true {
                return true
            }
            node = Expr::Bin { op: g_OP_DIV, left: Box::new(b), right: Box::new(a) };
            if true {
                return true
            }
            j = (j + 1);
