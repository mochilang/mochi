...
fn main() {
    ...
    if ((((x >= 0) && (x < g_width)) && (y >= 0)) && (y < g_height)) {
        ...
        unsafe fn bresenham(mut x0: i64, mut y0: i64, mut x1: i64, mut y1: i64, mut g: Vec<Vec<String>>) {
    ...
    loop {
        ...
        unsafe fn ftree(mut g: Vec<Vec<String>>, mut x: f64, mut y: f64, mut dist: f64, mut dir: f64, mut d: i64) {
    let mut rad: f64 = ((dir * g_PI) / 180.0);
    let mut x2: f64 = (x + (dist * _sin(rad)));
    let mut y2: f64 = (y - (dist * _cos(rad)));
    bresenham((x as i64), (y as i64), (x2 as i64), (y2 as i64), g.clone());
    if (d > 0) {
        ftree(g.clone(), x2, y2, (dist * g_frac), (dir - g_angle), (d - 1));
        ftree(g.clone(), x2, y2, (dist * g_frac), (dir + g_angle), (d - 1));
    }
};
    ...
    ftree(grid.clone(), ((g_width / 2) as f64), ((g_height - 1) as f64), g_length, 0.0, g_depth);
