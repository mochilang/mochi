...
fn main() {
    ...
    while (y <= n) {
        ...
        while (x <= y) {
            ...
            row = { let mut v = row.clone(); v.push(bigAdd(row[((row.len() as i64) - 1) as usize].clone(), val.clone())); v };
            x = (x + 1);
        }
    ...
    while (i < n) {
        ...
        out = { let mut v = out.clone(); v.push(String::from(bigToString(diff.clone()))); v };
        i = (i + 1);
    }
    return out
};
