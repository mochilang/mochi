...
static mut g_vertices: Vec<Vec<i64>> = Vec::new();
...
fn main() {
    unsafe {
        g_width = 60;
        g_height = (((g_width as f64) * 0.86602540378) as i64);
        g_iterations = 5000;
        g_grid = vec![];
        g_y = 0;
        g_seed = 1;
        g_vertices = vec![vec![0, (g_height - 1)], vec![(g_width - 1), (g_height - 1)], vec![(g_width / 2), 0]];
        g_px = (g_width / 2);
        g_py = (g_height / 2);
        g_i = 0;
    ...
    return vec![next, (next % n)]
        ...
        while (g_i < g_iterations) {
            ...
            let mut v: Vec<i64> = g_vertices.clone()[idx as usize].clone();
            g_px = ((g_px + v[0 as usize]) / 2);
            g_py = ((g_py + v[1 as usize]) / 2);
            if ((((g_px >= 0) && (g_px < g_width)) && (g_py >= 0)) && (g_py < g_height)) {
