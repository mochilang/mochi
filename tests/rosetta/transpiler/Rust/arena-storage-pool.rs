...
fn poolGet(mut p: Vec<i64>) -> HashMap<String, Vec<i64>> {
    if ((p.len() as i64) == 0) {
        println!("{}", "pool empty");
...
fn mochi_main() {
    ...
    i = 0;
    j = 0;
    let mut res1: HashMap<String, Vec<i64>> = poolGet(pool.clone());
    pool = res1["pool"].clone();
    i = res1["val"].clone();
    let mut res2: HashMap<String, Vec<i64>> = poolGet(pool.clone());
    pool = res2["pool"].clone();
    j = res2["val"].clone();
    i = 4;
    j = 5;
    println!("{}", (i + j).to_string());
    ...
    let mut res3: HashMap<String, Vec<i64>> = poolGet(pool.clone());
    pool = res3["pool"].clone();
    i = res3["val"].clone();
    let mut res4: HashMap<String, Vec<i64>> = poolGet(pool.clone());
    pool = res4["pool"].clone();
    j = res4["val"].clone();
    i = 7;
    j = 8;
    println!("{}", (i + j).to_string());
