...
fn main() {
    ...
    fn mochi_main() {
    let mut words: Vec<String> = vec![String::from("constitutionalism").clone(), String::from("misconstitutional").clone()];
    ...
    for w in words {
        ...
        if !m.contains_key(&k) {
            m.insert(k.clone(), vec![w.clone()].clone());
            continue
        }
        ...
        m.insert(k.clone(), { let mut v = m[k.as_str()].clone(); v.push(w.clone()); v }.clone());
    }
