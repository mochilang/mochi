...
fn main() {
    ...
    for w in &words {
        if ((w.clone().len() as i64) <= bestLen) {
            continue
        }
        let k: String = sortRunes(w.clone()).clone();
        if !m.contains_key(&k) {
            m.insert(k.clone(), vec![w.clone()]);
            continue
        }
        for c in &m[k.as_str()].clone() {
            if deranged(w.clone(), c.clone()) {
                bestLen = (w.clone().len() as i64);
                w1 = c.clone();
                w2 = w.clone();
                break
            }
        }
        m.insert(k.clone(), { let mut v = m[k.as_str()].clone(); v.push(String::from(w.clone())); v });
    }
