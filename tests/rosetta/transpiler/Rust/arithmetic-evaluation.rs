...
fn main() {
    ...
    while (p.pos < (p.expr.len() as i64)) {
        let ch: String = { let tmp = &p.expr; tmp.chars().skip(p.pos as usize).take(((p.pos + 1) - p.pos) as usize).collect::<String>() }.clone();
    ...
    let token: String = { let tmp = &p.expr; tmp.chars().skip(start as usize).take((p.pos - start) as usize).collect::<String>() }.clone();
    ...
    while true {
        ...
        if (p.pos < (p.expr.len() as i64)) {
            let op: String = { let tmp = &p.expr; tmp.chars().skip(p.pos as usize).take(((p.pos + 1) - p.pos) as usize).collect::<String>() }.clone();
            if (op.as_str() == "*") {
                p.pos = (p.pos + 1);
    ...
    fn evalExpr(mut expr: String) -> i64 {
    let mut p: Parser = Parser {expr: expr.clone(), pos: 0};
    let r: Res = parseExpr(p);
    return r.v
};
    fn mochi_main() {
    let expr = String::from("2*(3-1)+2*5");
