...
fn main() {
    ...
    fn bigCmp(mut a: Vec<i64>, mut b: Vec<i64>) -> i64 {
    ...
    while (i < (a.len() as i64)) {
        ...
        res = { let mut v = res.clone(); v.push((prod % 10)); v };
        carry = (prod / 10);
        i = (i + 1);
    }
    while (carry > 0) {
        res = { let mut v = res.clone(); v.push((carry % 10)); v };
        carry = (carry / 10);
    }
    return bigTrim(res.clone())
};
    ...
    let mut i: i64 = 0;
    while (i <= n) {
        sieve = { let mut v = sieve.clone(); v.push(true); v };
        i = (i + 1);
    }
    let mut p: i64 = 2;
    while ((p * p) <= n) {
    ...
    while (x <= n) {
        if sieve[x as usize] {
            res = { let mut v = res.clone(); v.push(x); v };
        }
        x = (x + 1);
    }
    return res
    ...
    let mut m: HashMap<String, i64> = HashMap::from([]);
    for p in primes {
        if (p > n) {
            break
        }
    ...
    let mut f: HashMap<String, i64> = HashMap::from([]);
    let mut n: i64 = x;
    for p in primes {
        if ((p * p) > n) {
            break
    ...
    exps.insert("2".clone(), ({ exps.get("2".as_str()).cloned().unwrap_or(0) } + 5));
    let t2: i64 = ((((532 * n) * n) + (126 * n)) + 9);
    ...
    exps.insert("3".clone(), ({ exps.get("3".as_str()).cloned().unwrap_or(0) } - 1));
    let mut keys: Vec<i64> = vec![];
    for k in exps.keys() {
        keys = { let mut v = keys.clone(); v.push({ let n: i64 = k.parse().unwrap(); n }); v };
    }
    keys = sortInts(keys.clone());
    let mut res: Vec<i64> = bigFromInt(1);
    for p in keys {
        let mut e: i64 = exps[&p.to_string()];
        let mut i: i64 = 0;
        while (i < e) {
    ...
    if (pw >= (s.len() as i64)) {
        let mut frac: String = format!("{}{}", repeat(String::from("0"), (pw - (s.len() as i64))), s).clone();
        if ((frac.len() as i64) < 33) {
            frac = format!("{}{}", frac, repeat(String::from("0"), (33 - (frac.len() as i64))));
        }
    ...
    let line: String = repeat(String::from("-"), 89).clone();
    println!("{}", line);
    ...
    while true {
        ...
        if (n < 10) {
            let termStr: String = formatTerm(ip.clone(), pw).clone();
        ...
        if ((denomPow >= 70) && (bigCmp(diff.clone(), bigMulPow10(bigFromInt(1), (denomPow - 70))) < 0)) {
            break
        }
        prev = sum;
        n = (n + 1);
    }
    let precision: i64 = 70;
    ...
    while (bigCmp(low.clone(), bigSub(high.clone(), bigFromInt(1))) < 0) {
        ...
        if (bigCmp(prod.clone(), target.clone()) <= 0) {
            low = mid;
        } else {
    ...
    if ((piStr.len() as i64) <= precision) {
        piStr = format!("{}{}", repeat(String::from("0"), ((precision - (piStr.len() as i64)) + 1)), piStr);
    }
