...
const char* repeat(const char* ch, long long n);
long long parseIntStr(const char* str);
const char* toBinary(long long n, long long bits);
long long binToInt(const char* bits);
const char* padRight(const char* s, long long width);
...
long long parseIntStr(const char* str) {
    ...
    long long digits_vals[26] = {0LL, 1LL, 2LL, 3LL, 4LL, 5LL, 6LL, 7LL, 8LL, 9LL};
    size_t digits_len = 10;
    while (i < strlen(str)) {
        n = (n * 10LL) + digits[(int)(_substring(str, i, i + 1LL))];
        i = i + 1LL;
    }
    if (neg) {
        n = -(n);
...
const char* canonicalize(const char* cidr) {
    ...
    long long size = parseIntStr(parts[(int)(1LL)]);
