// Generated by Mochi 0.10.32 on 2025-07-23 00:00 +0700
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
Anon1 isValidMove(int m) {
    if (m == 0) {
        return (Anon1){.idx = empty - 4, .ok = (empty / 4) > 0};
    }
    if (m == 1) {
        return (Anon1){.idx = empty + 4, .ok = (empty / 4) < 3};
    }
    if (m == 2) {
        return (Anon1){.idx = empty + 1, .ok = (empty % 4) < 3};
    }
    if (m == 3) {
        return (Anon1){.idx = empty - 1, .ok = (empty % 4) > 0};
    }
    return (Anon1){.idx = 0, .ok = 0};
}

int doMove(int m) {
    Anon1 r = isValidMove(m);
    if (!(r.ok)) {
        return 0;
    }
    int i = empty;
