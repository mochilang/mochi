...
int showTokens(int tokens) {
    puts(str_concat("Tokens remaining ", str_int(tokens)));
}
...
int user_main() {
    ...
    while (!(done)) {
        ...
        if ((t < 1LL) || (t > 3LL)) {
            ...
            puts(str_concat(str_concat(str_concat(str_concat("  Computer takes ", str_int(ct)), " token"), s), "\n\n"));
            tokens = tokens - 4LL;
        }
        if (tokens == 0LL) {
            showTokens(0LL);
