...
fn qsel(a: []const f64, k: i32) f64 {
    var arr = a; // []const f64
    while (@as(i32, @intCast((arr).len)) > 1) {
        var px = @mod(std.time.nanoTimestamp(), @as(i32, @intCast((arr).len))); // i32
        var pv = arr[px]; // f64
        const last = (@as(i32, @intCast((arr).len)) - 1); // i32
