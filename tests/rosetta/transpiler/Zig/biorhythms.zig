...
fn parseDate(s: []const u8) []const i32 {
    const y = parseIntStr(s[0..4]); // i32
    const m = parseIntStr(s[5..7]); // i32
    const d = parseIntStr(s[8..10]); // i32
    return [_]i32{
    y,
    m,
    d,
...
fn biorhythms(birth: []const u8, target: []const u8) void {
    ...
    while (i < 3) {
        ...
        if (percent > 95.0) {
            ...
            var pct = std.fmt.allocPrint(std.heap.page_allocator, "{d:.1}", .{percent}) catch |err| handleError(err); // []const u8
            if (!contains(pct, ".")) {
...
fn user_main() void {
    ...
    var idx = 0; // i32
    while (idx < @as(i32, @intCast((pairs).len))) {
