// Generated by Mochi transpiler v0.10.40 on 2025-07-25 10:53:28 GMT+7
import Foundation
    ...
    func applyFilter(_ input: [Double], _ a: [Double], _ b: [Double]) -> [Double] {
        ...
        while (i < Int(((input).count))) {
            ...
            while ((j <= i) && (j < Int(((b).count)))) {
                tmp = ((tmp + (Double(b[j]) * Double(input[(i - j)]))) as! Double)
                j = ((j + 1) as! Int)
            }
            j = 0
            while ((j < i) && ((j + 1) < Int(((a).count)))) {
                tmp = ((tmp - (Double(a[(j + 1)]) * Double(out[((i - j) - 1)]))) as! Double)
                j = ((j + 1) as! Int)
            }
