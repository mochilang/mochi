// Generated by Mochi transpiler v0.10.40 on 2025-07-26 00:03:44 GMT+7
import Foundation
...
struct BigInt: Comparable, CustomStringConvertible {
    ...
    static func /(lhs: BigInt, rhs: BigInt) -> BigInt {
        if rhs.digits.isEmpty { return BigInt() }
        let left = lhs.toInt()
        let right = rhs.toInt()
        if right == 0 { return BigInt() }
        return BigInt(left / right)
    }
    static func %(lhs: BigInt, rhs: BigInt) -> BigInt {
        if rhs.digits.isEmpty { return BigInt() }
        let left2 = lhs.toInt()
        let right2 = rhs.toInt()
        if right2 == 0 { return BigInt() }
        return BigInt(left2 % right2)
    }
    func bitLength() -> Int {
...
do {
    ...
    func pow_big(_ base: BigInt, _ exp: Int) -> BigInt {
        ...
        var b: BigInt = BigInt(BigInt(base))
        var e = (exp as! Int)
        while (e > 0) {
            if ((e % 2) == 1) {
    ...
    func bit_len(_ x: BigInt) -> Int {
        var n: BigInt = BigInt(BigInt(x))
        var c = 0
        while (n > BigInt(0)) {
            n = ((n / BigInt(2)) as! BigInt)
            c = ((c + 1) as! Int)
        }
        return (c as! Int)
    }
    ...
    func ackermann2(_ m: BigInt, _ n: BigInt) -> BigInt {
        if (err != "") {
            return BigInt(BigInt(BigInt(0)))
        }
        if (m <= BigInt(3)) {
            let mi = Int(_int(m))
            if (mi == 0) {
                return ((n + BigInt(1)) as! BigInt)
            }
            if (mi == 1) {
                return ((n + BigInt(2)) as! BigInt)
            }
            if (mi == 2) {
                return (((BigInt(2) * n) + BigInt(3)) as! BigInt)
            }
            if (mi == 3) {
                ...
                let r = BigInt(pow_big(BigInt(BigInt(BigInt(2))), Int(_int(n))))
                return (((BigInt(8) * r) - BigInt(3)) as! BigInt)
            }
        }
        if (Int(bit_len((n as! BigInt))) == 0) {
            return BigInt(BigInt(ackermann2(((m - BigInt(BigInt(BigInt(1)))) as! BigInt), BigInt(BigInt(BigInt(1))))))
        }
        return BigInt(BigInt(ackermann2(((m - BigInt(BigInt(BigInt(1)))) as! BigInt), BigInt(BigInt(ackermann2((m as! BigInt), ((n - BigInt(BigInt(BigInt(1)))) as! BigInt)))))))
    }
    func show(_ m: Int, _ n: Int) {
        err = ""
        let res = BigInt(ackermann2(BigInt(BigInt(BigInt(m))), BigInt(BigInt(BigInt(n)))))
        if (err != "") {
