// Generated by Mochi transpiler v0.10.42 on 2025-07-28 10:42:55 GMT+7
import Foundation
...
do {
    ...
    var idx2: Int = 0
    while (idx2 < Int(((faces).count))) {
        let per = faceToPerim((faces[idx2] as! [Edge]))
        if (String(describing: per)
        == String(describing: nil as Any?)
        ) {
            print(_p((("  " + String(describing: names[idx2])) + " => Invalid edge format")))
        } else {
            print(_p(((("  " + String(describing: names[idx2])) + " => ") + String(describing: listStr((per as! [Int]))))))
        }
        idx2 = Int((idx2 + 1))
    }
    let _benchEnd = _now()
