...
(let/ec _break (let loop ()
  (if (< i 51) (let ()
    ...
    (define tmp (if deck (list-ref deck (int i)) #f))
    (set! deck (list-set deck (int i) (if deck (list-ref deck (int j)) #f)))
...
(define i 0)
(let/ec _break (let loop ()
  (if (< i (cond [(string? cards) (string-length cards)] [(hash? cards) (hash-count cards)] [else (length cards)])) (let ()
    (define c (if cards (list-ref cards (int i)) #f))
    (write stdout (string-append (string-append " " (substring nums (quotient c 4) (let ([__l (quotient c 4)] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))) (substring suits (modulo c 4) (let ([__l (modulo c 4)] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))))
