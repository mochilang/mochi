...
(let/ec _break (let loop ()
  (if (< i 16) (let ()
    (if (not (= (list-ref board i) (list-ref solved i))) (let ()
#f
) (void))
    (set! i (+ i 1))
    (loop)) (void))))
...
(if (= m 0) (let ()
(hash "idx" (- empty 4) "ok" (quotient empty (> 4 0)))
) (void))
(if (= m 1) (let ()
(hash "idx" (+ empty 4) "ok" (quotient empty (< 4 3)))
) (void))
(if (= m 2) (let ()
(hash "idx" (+ empty 1) "ok" (< (modulo empty 4) 3))
) (void))
(if (= m 3) (let ()
(hash "idx" (- empty 1) "ok" (> (modulo empty 4) 0))
) (void))
(hash "idx" 0 "ok" #f)
)
(define (doMove m)
(define r (isValidMove m))
(if (not (list-ref r "ok")) (let ()
#f
) (void))
(define i empty)
(define j (int (list-ref r "idx")))
...
(let/ec _break (let loop ()
  (if (and (not quit) (= (isSolved) #f)) (let ()
    (displayln "")
    (printBoard)
    (playOneMove)
    (loop)) (void))))
