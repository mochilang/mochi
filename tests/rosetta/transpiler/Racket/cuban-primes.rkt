...
(let/ec _break (let loop ()
  (if (< i (cond [(string? row) (string-length row)] [(hash? row) (hash-count row)] [else (length row)])) (let ()
    (set! padded (append padded (list (pad (if row (list-ref row (int i)) #f) 9))))
    (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
...
(let/ec _break (let loop ()
  (if (< row 20) (let ()
    ...
    (let/ec _break (let loop ()
  (if (< k 10) (let ()
    (set! slice (append slice (list (if cubans (list-ref cubans (int (let ([__l (* row 10)] [__r k]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))) #f))))
