...
(let/ec _break (let loop ()
  (if (< i (cond [(string? s) (string-length s)] [(hash? s) (hash-count s)] [else (length s)])) (let ()
    ...
    (if (string=? ch "?") (let ()
(set! types (append types (list "Q")))
) (let ()
(if (string=? ch "!") (let ()
(set! types (append types (list "E")))
) (let ()
(if (string=? ch ".") (let ()
(set! types (append types (list "S")))
) (void))
))
))
    (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
(define last (slice s (- (cond [(string? s) (string-length s)] [(hash? s) (hash-count s)] [else (length s)]) 1) (cond [(string? s) (string-length s)] [(hash? s) (hash-count s)] [else (length s)])))
(if (equal? (index-of "?!." last) (- 1)) (let ()
(set! types (append types (list "N")))
) (void))
(_return (join types "|"))
))
)
(define (main)
  (let/ec _return (begin
(define s "hi there, how are you today? I'd like to present to you the washing machine 9001. You have been nominated to win one of these! Just make sure you don't break it")
(define result (sentenceType s))
(displayln result)
))
)
(main)
