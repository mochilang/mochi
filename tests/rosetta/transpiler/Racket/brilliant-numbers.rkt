...
(set! s (append s (list p)))
  ...
  (if (< j (cond [(string? s) (string-length s)] [(hash? s) (hash-count s)] [else (length s)])) (let ()
    (define prod (* (list-ref s i) (list-ref s j)))
    (if (< prod limit) (let ()
(if countOnly (let ()
...
(let/ec _break (let loop ()
  (if (< i (cond [(string? br) (string-length br)] [(hash? br) (hash-count br)] [else (length br)])) (let ()
    (displayln (string-join (map (lambda (x) (format "~a" x)) (list (string-append (pad-start (format "~a" (list-ref br i)) 4 " ") " ") #f)) " "))
