...
(let/ec _break (let loop ()
  (if (< k (cond [(string? a) (string-length a)] [(hash? a) (hash-count a)] [else (length a)])) (let ()
    ...
    (loop)) (void))))
(set! s (let ([__l s] [__r (substring p (- (cond [(string? p) (string-length p)] [(hash? p) (hash-count p)] [else (length p)]) 1) (cond [(string? p) (string-length p)] [(hash? p) (hash-count p)] [else (length p)]))]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
(define w (fields s))
(_return (join w " "))
))
)
