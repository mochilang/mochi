...
(define idx 0)
(let/ec _break (let loop ()
  (if (< idx (cond [(string? strings) (string-length strings)] [(hash? strings) (hash-count strings)] [else (length strings)])) (let ()
    (define s (if strings (list-ref strings (int idx)) #f))
    (define r (collapse s))
    (define cs (if r (list-ref r (int 0)) #f))
    (define olen (if r (list-ref r (int 1)) #f))
    (define clen (if r (list-ref r (int 2)) #f))
