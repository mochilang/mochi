...
(let/ec _break (let loop ()
  (if (<= i k) (let ()
    ...
    (let/ec _break (let loop ()
  (if (< j (cond [(string? list_) (string-length list_)] [(hash? list_) (hash-count list_)] [else (length list_)])) (let ()
    (if (< (list-ref list_ j) (list-ref list_ minIndex)) (let ()
(set! minIndex j)
) (void))
    (set! j (let ([__l j] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
    (define tmp (list-ref list_ i))
    (set! list_ (list-set list_ i (list-ref list_ minIndex)))
    (set! list_ (list-set list_ minIndex tmp))
    (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
(_return (list-ref list_ k))
))
)
(define (median a)
  (let/ec _return (begin
