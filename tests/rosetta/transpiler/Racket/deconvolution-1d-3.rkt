...
(displayln (string-append (string-append "⎡" (fmt1 (if xs (if (hash? xs) (hash-ref xs 0 #f) (list-ref xs (int 0))) #f))) "⎤"))
(define i 1)
(let/ec _break (let loop ()
  (if (< i (- (cond [(string? xs) (string-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)]) 1)) (let ()
    (displayln (string-append (string-append "⎢" (fmt1 (if xs (if (hash? xs) (hash-ref xs i #f) (list-ref xs (int i))) #f))) "⎥"))
    (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
(displayln (string-append (string-append "⎣ " (fmt1 (if xs (if (hash? xs) (hash-ref xs (- (cond [(string? xs) (string-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)]) 1) #f) (list-ref xs (int (- (cond [(string? xs) (string-length xs)] [(hash? xs) (hash-count xs)] [else (length xs)]) 1)))) #f))) "⎦"))
))
)
(define (deconv g f)
  (let/ec _return (begin
...
(set! lower (let ([__l (- n (cond [(string? f) (string-length f)] [(hash? f) (hash-count f)] [else (length f)]))] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
) (void))
    (define i lower)
    (let/ec _break (let loop ()
  (if (< i n) (let ()
...
(displayln "deconv(g, f) =")
(printColumnMatrix (deconv g f))
(displayln "")
(displayln "deconv(g, h) =")
