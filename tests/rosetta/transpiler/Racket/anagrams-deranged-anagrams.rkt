...
(define w1 "")
(define w2 "")
(for ([w words] #:unless (<= (cond [(string? w) (string-length w)] [(hash? w) (hash-count w)] [else (length w)]) bestLen))
(define k (sortRunes w))
(if (not (hash-has-key? m k)) (let ()
(set! m (hash-set m k (list w)))
(loop)
) (void))
(for ([c (hash-ref m k)])
(if (deranged w c) (let ()
...
(set! m (hash-set m k (append (hash-ref m k) (list w))))
)
