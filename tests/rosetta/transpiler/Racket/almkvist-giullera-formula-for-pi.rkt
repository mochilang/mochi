...
(let/ec _break (let loop ()
  (if (equal? (modulo n p) 0) (let ()
    ...
    (loop)) (void))))
(if (> c 0) (let ()
(set! f (hash-set f (format "~a" p) c))
) (void))
)
(if (> n 1) (let ()
(set! f (hash-set f (format "~a" n) (let ([__l (hash-ref f (format "~a" n) 0)] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))))
) (void))
(_return f)
))
)
...
(set! exps (hash-set exps k (- (hash-ref exps k 0) (* 6 (hash-ref fn k)))))
)
(set! exps (hash-set exps "2" (let ([__l (hash-ref exps "2" 0)] [__r 5]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))))
...
(set! exps (hash-set exps k (let ([__l (hash-ref exps k 0)] [__r (hash-ref ft2 k)]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))))
)
(set! exps (hash-set exps "3" (- (hash-ref exps "3" 0) 1)))
(define keys (list))
...
(for ([p keys])
(define e (hash-ref exps (format "~a" p)))
(define i 0)
(let/ec _break (let loop ()
  (if (< i e) (let ()
