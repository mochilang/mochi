...
def createPolybius()
  ...
  while i < 6
    ...
    p = p + [row]
    line = _add(labels[i..._add(i, 1)], " | ")
    j = 0
    while j < 6
      line = _add(_add(line, row[j..._add(j, 1)]), " ")
      j = _add(j, 1)
    end
    puts(((__v = line); __v.is_a?(Array) ? ("[" + __v.map{ |x| if x.is_a?(String) then '\'' + x + '\'' elsif x.respond_to?(:to_h) then '{' + x.to_h.map{ |k,v| "'#{k}': #{v.is_a?(String) ? '\'' + v + '\'' : v.to_s}" }.join(', ') + '}' else x.to_s end }.join(', ') + "]") : __v.to_s))
    i = _add(i, 1)
  end
  return p
end
...
def decrypt(polybius, key, cipherText)
  ...
  idx = 0
  while idx < temp.length
    rIdx = indexOf($adfgvx, temp[idx..._add(idx, 1)])
    cIdx = indexOf($adfgvx, temp[_add(idx, 1)..._add(idx, 2)])
