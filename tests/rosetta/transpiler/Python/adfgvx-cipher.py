...
# Version 0.10.37, generated on 2025-07-23 04:20 +0000
import os
import time
...
def createPolybius():
    ...
    while i < 6:
        ...
        p = p + [row]
        line = "".join(labels[i:i + 1]) + " | "
        j = 0
        while j < 6:
            line = line + "".join(row[j:j + 1]) + " "
            j = j + 1
        print(line)
        i = i + 1
    return p
def createKey(n):
    ...
    while i < n:
        idx = _now() % len(pool)
        key = key + pool[idx]
        pool = "".join(pool[:idx]) + "".join(pool[idx + 1:len(pool)])
        i = i + 1
    print("\nThe key is " + key)
    return key
def orderKey(key):
        ...
        while r < 6:
            ...
            while c < 6:
                if polybius[r][c:c + 1] == plainText[i:i + 1]:
                    temp = temp + "".join(labels[r:r + 1]) + "".join(labels[c:c + 1])
                c = c + 1
            r = r + 1
        i = i + 1
