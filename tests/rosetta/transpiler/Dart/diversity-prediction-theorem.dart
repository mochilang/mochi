...
num averageSquareDiff(num f, List<num> preds) {
  num av = 0.0;
  int i = 0;
  ...
  av = av / ((preds.length).toDouble());
  return av;
}

List<num> diversityTheorem(num truth, List<num> preds) {
  num av = 0.0;
  int i = 0;
  while (i < preds.length) {
    av = av + preds[i];
  ...
  av = av / ((preds.length).toDouble());
  num avErr = averageSquareDiff(truth, preds);
  num crowdErr = (truth - av) * (truth - av);
  num div = averageSquareDiff(av, preds);
  return [avErr, crowdErr, div];
}

void _main() {
  List<List<num>> predsArray = [[48.0, 47.0, 51.0], [48.0, 47.0, 51.0, 42.0]];
  num truth = 49.0;
  int i = 0;
  while (i < predsArray.length) {
    List<num> preds = predsArray[i];
    List<num> res = diversityTheorem(truth, preds);
