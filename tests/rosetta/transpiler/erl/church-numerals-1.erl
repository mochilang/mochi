...
main(_) ->
    ...
    io:format("~ts~n", [("three ^ four -> " ++ lists:flatten(io_lib:format("~p", [toint(math:pow(erlang:get('three'), erlang:get('four')))])))]),
    io:format("~ts~n", [("four ^ three -> " ++ lists:flatten(io_lib:format("~p", [toint(math:pow(erlang:get('four'), erlang:get('three')))])))]),
