...
indexof(Xs, Value) ->
    ...
    case (I < length(Xs)) of
        true ->
            case ((case erlang:is_map(Xs) of true -> maps:get(I, Xs, nil); _ -> lists:nth(I + 1, Xs) end) == Value) of
        true -> throw({return, I});
        _ -> ok
    end,
            I_2 = (I + 1),
...
classifysequence(K) ->
    ...
    case true of
        ...
        true -> Aliquot_5 = (("Sociable[" ++ lists:flatten(io_lib:format("~p", [(N_3 - 1)]))) ++ "]"),
            ...
            Idx_4 = nil;
        _ -> case (Last_2 == (case erlang:is_map(Seq_2) of true -> maps:get((N_3 - 2), Seq_2, nil); _ -> lists:nth((N_3 - 2) + 1, Seq_2) end)) of
...
joinwithcommas(Seq_3) ->
    ...
    case (I_5 < length(Seq_3)) of
        true ->
            S_5 = (S_4 ++ lists:flatten(io_lib:format("~p", [(case erlang:is_map(Seq_3) of true -> maps:get(I_5, Seq_3, nil); _ -> lists:nth(I_5 + 1, Seq_3) end)]))),
...
{K_3} = Fun_8(K_2),
    ...
    case (I_7 < length(S_9)) of
        true ->
            Val = (case erlang:is_map(S_9) of true -> maps:get(I_7, S_9, nil); _ -> lists:nth(I_7 + 1, S_9) end),
