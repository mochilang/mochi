...
fmt4(X) ->
    try
        ...
        Y_5 = (float(mochi_to_int(Y_4)) / 10000),
...
mulpoint(P, M) ->
    try
        #{"x" => (maps:get("x", P, nil) * M), "y" => (maps:get("y", P, nil) * M), "z" => (maps:get("z", P, nil) * M)}
    catch {return, Ret} -> Ret end.
...
sortedges(Edges) ->
    ...
    case (J < length(Tmp)) of
        true ->
            E = lists:nth(J + 1, Tmp),
            case ((lists:nth(0 + 1, E) < lists:nth(0 + 1, Min)) orelse ((lists:nth(0 + 1, E) == lists:nth(0 + 1, Min)) andalso ((lists:nth(1 + 1, E) < lists:nth(1 + 1, Min)) orelse ((lists:nth(1 + 1, E) == lists:nth(1 + 1, Min)) andalso (lists:nth(2 + 1, E) < lists:nth(2 + 1, Min)))))) of
        true -> Min_2 = E,
            Idx_3 = J,
            Idx_4 = Idx_3,
            Min_3 = Min_2;
...
{Edges, Idx_4, J_2, Min_3, Res, Tmp} = Fun_7(Edges, Idx_2, J, Min, Res, Tmp),
    ...
    case (K < length(Tmp)) of
        true ->
            case (K /= Idx_4) of
        true -> Out_2 = lists:append(Out, [lists:nth(K + 1, Tmp)]),
            Out_3 = Out_2;
        _ -> Out_3 = Out
    end,
            K_2 = (K + 1),
...
getedgesfaces(Points_2, Faces_2) ->
    ...
    case (Fnum < length(Faces_2)) of
        true ->
            Face_2 = lists:nth(Fnum + 1, Faces_2),
            NumP = length(Face_2),
            Pi = 0,
            Fun_10 = fun Fun_10_loop(Edges_2, Face_2, Faces_2, Fnum, NumP, Pi, Points_2) ->
    case (Pi < NumP) of
        true ->
            Pn1 = lists:nth(Pi + 1, Face_2),
            Pn2 = 0,
            case (Pi < (NumP - 1)) of
        true -> Pn2_2 = lists:nth((Pi + 1) + 1, Face_2),
            Pn2_4 = Pn2_2;
        _ -> Pn2_3 = lists:nth(0 + 1, Face_2),
            Pn2_4 = Pn2_3
    end,
            case (Pn1 > Pn2_4) of
        true -> Tmpn = Pn1,
...
{Edges_3, Faces_2, Fnum_2, Points_2} = Fun_11(Edges_2, Faces_2, Fnum, Points_2),
    ...
    case (Idx_5 < length(Edges_4)) of
        true ->
            try
                E1 = lists:nth(Idx_5 + 1, Edges_4),
                case (Idx_5 < (length(Edges_4) - 1)) of
        true -> E2 = lists:nth((Idx_5 + 1) + 1, Edges_4),
            case ((lists:nth(0 + 1, E1) == lists:nth(0 + 1, E2)) andalso (lists:nth(1 + 1, E1) == lists:nth(1 + 1, E2))) of
        true -> Merged_2 = lists:append(Merged, [[lists:nth(0 + 1, E1), lists:nth(1 + 1, E1), lists:nth(2 + 1, E1), lists:nth(2 + 1, E2)]]),
            Idx_6 = (Idx_5 + 2),
        ...
        _ -> E2_2 = nil,
            ...
            Merged_4 = Merged
                ...
                Merged_5 = lists:append(Merged_4, [[lists:nth(0 + 1, E1), lists:nth(1 + 1, E1), lists:nth(2 + 1, E1), -1]]),
                Idx_9 = (Idx_8 + 1),
...
{Edges_4, Faces_2, Fnum_2, Idx_9, Merged_5, Points_2} = Fun_12(Edges_4, Faces_2, Fnum_2, Idx_5, Merged, Points_2),
    ...
    case List of
        ...
        [Me|Me_rest] ->
            P1_3 = lists:nth(lists:nth(0 + 1, Me) + 1, Points_2),
            P2_3 = lists:nth(lists:nth(1 + 1, Me) + 1, Points_2),
            Cp = centerpoint(P1_3, P2_3),
            EdgesCenters_2 = lists:append(EdgesCenters, [#{"pn1" => lists:nth(0 + 1, Me), "pn2" => lists:nth(1 + 1, Me), "fn1" => lists:nth(2 + 1, Me), "fn2" => lists:nth(3 + 1, Me), "cp" => Cp}]),
...
getedgepoints(Points_3, EdgesFaces, FacePoints_3) ->
    ...
    case (I_5 < length(EdgesFaces)) of
        true ->
            Edge = lists:nth(I_5 + 1, EdgesFaces),
            Cp_2 = maps:get("cp", Edge, nil),
            Fp1 = lists:nth(maps:get("fn1", Edge, nil) + 1, FacePoints_3),
            Fp2 = Fp1,
            case (maps:get("fn2", Edge, nil) /= (0 - 1)) of
        true -> Fp2_2 = lists:nth(maps:get("fn2", Edge, nil) + 1, FacePoints_3),
            Fp2_3 = Fp2_2;
        _ -> Fp2_3 = Fp2
    end,
...
{FacePoints_4, Faces_3, I_8, NumP_2, Points_4, Temp_2} = Fun_15(FacePoints_4, Faces_3, I_7, NumP_2, Points_4, Temp),
    ...
    case (Fnum_3 < length(Faces_3)) of
        true ->
            Fp_4 = lists:nth(Fnum_3 + 1, FacePoints_4),
...
{FacePoints_4, Faces_3, Fnum_3, Fp_4, I_8, NumP_2, Points_4, Temp_3} = Fun_16(lists:nth(Fnum_3 + 1, Faces_3), FacePoints_4, Faces_3, Fnum_3, Fp_4, I_8, NumP_2, Points_4, Temp_2),
            Fnum_4 = (Fnum_3 + 1),
...
{Faces_4, I_12, Pf_2, Points_6} = Fun_23(Faces_4, I_11, Pf, Points_6),
    ...
    case List of
        ...
        [Pn_3|Pn_3_rest] ->
            Pf_3 = lists:sublist(Pf_2, Pn_3) ++ [(lists:nth(Pn_3 + 1, Pf_2) + 1)] ++ lists:nthtail(Pn_3 + 1, Pf_2),
            Fun_24_loop(Pn_3_rest, Faces_4, Fnum_5, I_12, Pf_3, Points_6)
    end
end,
{Faces_4, Fnum_5, I_12, Pf_3, Points_6} = Fun_24(lists:nth(Fnum_5 + 1, Faces_4), Faces_4, Fnum_5, I_12, Pf_2, Points_6),
            Fnum_6 = (Fnum_5 + 1),
...
getnewpoints(Points_7, Pf_4, Afp, Ame) ->
    ...
    case (I_13 < length(Points_7)) of
        true ->
            N_2 = float(lists:nth(I_13 + 1, Pf_4)),
            M1 = ((N_2 - 3) / N_2),
            M2 = (1 / N_2),
            M3 = (2 / N_2),
            Old = lists:nth(I_13 + 1, Points_7),
            P1_4 = mulpoint(Old, M1),
            P2_4 = mulpoint(lists:nth(I_13 + 1, Afp), M2),
            P3 = mulpoint(lists:nth(I_13 + 1, Ame), M3),
...
{AvgFacePoints, AvgMidEdges, EdgePoints_3, EdgesFaces_3, FacePointNums_2, FacePoints_5, Faces_5, NewPoints_2, NextPoint_2, Points_8, PointsFaces} = Fun_27(FacePoints_5, AvgFacePoints, AvgMidEdges, EdgePoints_3, EdgesFaces_3, FacePointNums, FacePoints_5, Faces_5, NewPoints, NextPoint, Points_8, PointsFaces),
    ...
    case (Idx_10 < length(EdgesFaces_3)) of
        true ->
            E_2 = lists:nth(Idx_10 + 1, EdgesFaces_3),
            NewPoints_3 = lists:append(NewPoints_2, [lists:nth(Idx_10 + 1, EdgePoints_3)]),
...
{AvgFacePoints, AvgMidEdges, EdgePointNums_2, EdgePoints_3, EdgesFaces_3, FacePointNums_2, FacePoints_5, Faces_5, Idx_11, NewPoints_3, NextPoint_3, Points_8, PointsFaces} = Fun_28(AvgFacePoints, AvgMidEdges, EdgePointNums, EdgePoints_3, EdgesFaces_3, FacePointNums_2, FacePoints_5, Faces_5, Idx_10, NewPoints_2, NextPoint_2, Points_8, PointsFaces),
    ...
    case (Fnum_7 < length(Faces_5)) of
        true ->
            OldFace = lists:nth(Fnum_7 + 1, Faces_5),
            case (length(OldFace) == 4) of
        true -> A_2 = lists:nth(0 + 1, OldFace),
            B_2 = lists:nth(1 + 1, OldFace),
            C = lists:nth(2 + 1, OldFace),
            D_2 = lists:nth(3 + 1, OldFace),
            Fpnum = lists:nth(Fnum_7 + 1, FacePointNums_2),
            Ab = lists:nth(key(A_2, B_2) + 1, EdgePointNums_2),
            Da = lists:nth(key(D_2, A_2) + 1, EdgePointNums_2),
            Bc = lists:nth(key(B_2, C) + 1, EdgePointNums_2),
            Cd = lists:nth(key(C, D_2) + 1, EdgePointNums_2),
...
formatface(F) ->
    try
        ...
        S_11 = ("[" ++ fmt2(lists:nth(0 + 1, F))),
        I_15 = 1,
        Fun_30 = fun Fun_30_loop(F, I_15, S_11) ->
    case (I_15 < length(F)) of
        true ->
            S_12 = ((S_11 ++ " ") ++ fmt2(lists:nth(I_15 + 1, F))),
            I_16 = (I_15 + 1),
...
main() ->
    ...
    case (I_17 < 1) of
        true ->
            ...
            OutputPoints_2 = lists:nth(0 + 1, Res_3),
            OutputFaces_2 = lists:nth(1 + 1, Res_3),
            I_18 = (I_17 + 1),
