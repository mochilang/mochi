...
median(S) ->
    try
        N_2 = length(S),
        case ((N_2 rem 2) == 1) of
        true -> throw({return, lists:nth((N_2 / 2) + 1, S)});
        _ -> ok
    end,
        ((lists:nth(((N_2 / 2) - 1) + 1, S) + lists:nth((N_2 / 2) + 1, S)) / 2)
    catch {return, Ret} -> Ret end.
...
fivenum(Xs_2) ->
    try
        S_2 = sortfloat(Xs_2),
        N_3 = length(S_2),
        Q1 = lists:nth(((N_3 - 1) / 4) + 1, S_2),
        Med = median(S_2),
        Q3 = lists:nth(((3 * (N_3 - 1)) / 4) + 1, S_2),
        [lists:nth(0 + 1, S_2), Q1, Med, Q3, lists:nth((N_3 - 1) + 1, S_2)]
    catch {return, Ret} -> Ret end.
