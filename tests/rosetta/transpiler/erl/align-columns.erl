...
rstripempty(Words) ->
    ...
    case ((N > 0) andalso ((case erlang:is_map(Words) of true -> maps:get((N - 1), Words, nil); _ -> lists:nth((N - 1) + 1, Words) end) == "")) of
        true ->
            N_2 = (N - 1),
...
newformatter(Text) ->
    ...
    case (I_7 < length(Lines)) of
        true ->
            try
                case (length(string:substr(Lines, I_7 + 1, 1)) == 0) of
        true -> I_8 = (I_7 + 1),
            throw({continue, FmtLines, I_8, Lines, Text, Width_2}),
            I_9 = I_8;
        _ -> I_9 = I_7
    end,
                Words_2 = rstripempty(string:tokens(string:substr(Lines, I_9 + 1, 1), "$")),
    ...
    case (J < length(Words_2)) of
        true ->
            Wlen = length((case erlang:is_map(Words_2) of true -> maps:get(J, Words_2, nil); _ -> lists:nth(J + 1, Words_2) end)),
            case (J == length(Width_2)) of
        true -> Width_3 = lists:append(Width_2, [Wlen]),
            Width_6 = Width_3;
        _ -> case (Wlen > (case erlang:is_map(Width_2) of true -> maps:get(J, Width_2, nil); _ -> lists:nth(J + 1, Width_2) end)) of
...
printfmt(F, Align_2) ->
    ...
    case (I_11 < length(Lines_2)) of
        true ->
            Words_3 = (case erlang:is_map(Lines_2) of true -> maps:get(I_11, Lines_2, nil); _ -> lists:nth(I_11 + 1, Lines_2) end),
            Line = "",
            J_3 = 0,
            Fun_6 = fun Fun_6_loop(Align_2, F, I_11, J_3, Line, Lines_2, Width_7, Words_3) ->
    case (J_3 < length(Words_3)) of
        true ->
            Line_2 = ((Line ++ pad((case erlang:is_map(Words_3) of true -> maps:get(J_3, Words_3, nil); _ -> lists:nth(J_3 + 1, Words_3) end), (case erlang:is_map(Width_7) of true -> maps:get(J_3, Width_7, nil); _ -> lists:nth(J_3 + 1, Width_7) end), Align_2)) ++ " "),
            J_4 = (J_3 + 1),
