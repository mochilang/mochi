...
% Generated by Mochi transpiler v0.10.52 (1deb4f017c) on 2025-08-01 15:22 +0700
...
{INF, Dist_2, I_2, N, Next_2} = Fun_2(INF, Dist, I, N, Next),
    ...
    case (J_3 < N) of
        true ->
            case ((lists:nth(K + 1, lists:nth(I_3 + 1, Dist_7)) < INF) andalso (lists:nth(J_3 + 1, lists:nth(K + 1, Dist_7)) < INF)) of
        true -> Alt = (lists:nth(K + 1, lists:nth(I_3 + 1, Dist_7)) + lists:nth(J_3 + 1, lists:nth(K + 1, Dist_7))),
            case (Alt < lists:nth(J_3 + 1, lists:nth(I_3 + 1, Dist_7))) of
        true -> Tmp_21 = lists:nth(I_3 + 1, Dist_7),
            ...
            Tmp_23 = lists:nth(I_3 + 1, Next_7),
            Tmp_24 = lists:sublist(Tmp_23, J_3) ++ [lists:nth(K + 1, lists:nth(I_3 + 1, Next_7))] ++ lists:nthtail(J_3 + 1, Tmp_23),
...
{INF, Dist_10, I_4, K_2, N, Next_10, Tmp_26} = Fun_5(INF, Dist_7, I_2, K, N, Next_7, Tmp_20),
    ...
    try
        Ui = (U - 1),
        Vi = (V - 1),
        case (lists:nth(Vi + 1, lists:nth(Ui + 1, Next_10)) == (0 - 1)) of
        true -> throw({return, []});
        _ -> ok
    end,
        P = [U],
    ...
    case (Cur /= Vi) of
        true ->
            Cur_2 = lists:nth(Vi + 1, lists:nth(Cur + 1, Next_10)),
...
{INF, Dist_10, First_2, I_4, Idx_2, K_2, N, Next_10, P_3, Path, PathStr, S_4, Tmp_26} = Fun_7(INF, Dist_10, First, I_4, Idx, K_2, N, Next_10, P_3, Path, PathStr, S, Tmp_26),
    ...
    case (B < N) of
        true ->
            case (A /= B) of
        true -> io:format("~ts~n", [((((((lists:flatten(io_lib:format("~p", [(A + 1)])) ++ " -> ") ++ lists:flatten(io_lib:format("~p", [(B + 1)]))) ++ "\t") ++ lists:flatten(io_lib:format("~p", [lists:nth(B + 1, lists:nth(A + 1, Dist_10))]))) ++ "\t") ++ PathStr(Path((A + 1), (B + 1))))]);
        _ -> ok
    end,
            B_2 = (B + 1),
