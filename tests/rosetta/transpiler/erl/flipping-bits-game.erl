...
newboard(N_2, Seed_2) ->
    ...
    case (J < N_2) of
        true ->
            R = randint(S, 2),
            S_2 = lists:nth(0 + 1, R),
            Row_2 = lists:append(Row, [lists:nth(1 + 1, R)]),
            J_2 = (J + 1),
...
copyboard(B) ->
    ...
    case (J_3 < length(lists:nth(I_3 + 1, B))) of
        true ->
            Row_4 = lists:append(Row_3, [lists:nth(J_3 + 1, lists:nth(I_3 + 1, B))]),
            J_4 = (J_3 + 1),
...
fliprow(B_2, R_2) ->
    ...
    case (J_5 < length(lists:nth(R_2 + 1, B_2))) of
        true ->
            Tmp = lists:nth(R_2 + 1, B_2),
            Tmp_2 = lists:sublist(Tmp, J_5) ++ [(1 - lists:nth(J_5 + 1, lists:nth(R_2 + 1, B_2)))] ++ lists:nthtail(J_5 + 1, Tmp),
...
flipcol(B_4, C) ->
    ...
    case (I_5 < length(B_4)) of
        true ->
            Tmp_3 = lists:nth(I_5 + 1, B_4),
            Tmp_4 = lists:sublist(Tmp_3, C) ++ [(1 - lists:nth(C + 1, lists:nth(I_5 + 1, B_4)))] ++ lists:nthtail(C + 1, Tmp_3),
...
boardsequal(A, B_6) ->
    ...
    case (J_7 < length(lists:nth(I_7 + 1, A))) of
        true ->
            case (lists:nth(J_7 + 1, lists:nth(I_7 + 1, A)) /= lists:nth(J_7 + 1, lists:nth(I_7 + 1, B_6))) of
...
shuffleboard(B_7, Seed_3) ->
    ...
    case (K < (2 * N_3)) of
        true ->
            R_3 = randint(S_3, N_3),
            S_4 = lists:nth(0 + 1, R_3),
            Idx = mochi_to_int(lists:nth(1 + 1, R_3)),
            case ((K rem 2) == 0) of
...
solve(Board_3, Target) ->
    ...
    case (I_9 < N_4) of
        true ->
            Diff = (case (lists:nth(0 + 1, lists:nth(I_9 + 1, Board_3)) /= lists:nth(0 + 1, lists:nth(I_9 + 1, Target))) of
    true -> 1;
    _ -> 0
end),
...
{Board_3, Col, I_10, N_4, Row_6, Target} = Fun_10(Board_3, Col, I_9, N_4, Row_5, Target),
    ...
    case (J_9 < N_4) of
        true ->
            Diff_2 = (case (lists:nth(J_9 + 1, lists:nth(0 + 1, Board_3)) /= lists:nth(J_9 + 1, lists:nth(0 + 1, Target))) of
    true -> 1;
    _ -> 0
end),
            Val = ((Diff_2 + lists:nth(0 + 1, Row_6)) rem 2),
...
applysolution(B_11, Sol) ->
    ...
    case (I_11 < length(maps:get("row", Sol, nil))) of
        true ->
            case (lists:nth(I_11 + 1, maps:get("row", Sol, nil)) == 1) of
...
{B_11, Board_6, I_12, Moves_3, Sol} = Fun_12(B_11, Board_4, I_11, Moves, Sol),
    ...
    case (J_11 < length(maps:get("col", Sol, nil))) of
        true ->
            case (lists:nth(J_11 + 1, maps:get("col", Sol, nil)) == 1) of
...
printboard(B_12) ->
    ...
    case (J_13 < length(lists:nth(I_13 + 1, B_12))) of
        true ->
            Line_2 = (Line ++ lists:flatten(io_lib:format("~p", [lists:nth(J_13 + 1, lists:nth(I_13 + 1, B_12))]))),
            case (J_13 < (length(lists:nth(I_13 + 1, B_12)) - 1)) of
        true -> Line_3 = (Line_2 ++ " "),
            Line_4 = Line_3;
        _ -> Line_4 = Line_2
    end,
...
main() ->
    try
        ...
        Target_2 = lists:nth(0 + 1, Res),
        Seed_5 = mochi_to_int(lists:nth(1 + 1, Res)),
    ...
    case true of
        true ->
            try
                ...
                Board_10 = lists:nth(0 + 1, Sres),
                Seed_6 = mochi_to_int(lists:nth(1 + 1, Sres)),
...
{Board_10, N_5, Res, Seed_6, Target_2} = Fun_16(Board_9, N_5, Res, Seed_5, Target_2),
        ...
        Board_11 = lists:nth(0 + 1, Ares),
        Moves_6 = mochi_to_int(lists:nth(1 + 1, Ares)),
