// Generated by Mochi v0.10.41 on 2025-07-27 11:35:06 GMT+7
...
function spid(pieces: string): number {
  ...
  let r2: number = (((pieces).substring((r1 + 1), (Array.isArray(pieces) || typeof pieces === 'string' ? pieces.length : Object.keys(pieces ?? {}).length)).indexOf("R") + r1) + 1);
  let k: number = pieces.indexOf("K");
  if (((k < r1) || (k > r2))) {
    return -1;
  }
  let b1: number = pieces.indexOf("B");
  let b2: number = (((pieces).substring((b1 + 1), (Array.isArray(pieces) || typeof pieces === 'string' ? pieces.length : Object.keys(pieces ?? {}).length)).indexOf("B") + b1) + 1);
  if ((((b2 - b1) % 2) == 0)) {
    return -1;
  }
  ...
  let n2: number = (((piecesN).substring((n1 + 1), (Array.isArray(piecesN) || typeof piecesN === 'string' ? piecesN.length : Object.keys(piecesN ?? {}).length)).indexOf("N") + n1) + 1);
  let np: string = (String(n1) + String(n2));
  let N = ntable[np];
  let piecesQ: string = strReplace(pieces, "B", "");
  let Q: number = piecesQ.indexOf("Q");
  let D: number = "0246".indexOf(String(b1));
  let L: number = "1357".indexOf(String(b2));
  if ((D == (0 - 1))) {
    D = "0246".indexOf(String(b2));
    L = "1357".indexOf(String(b1));
  }
  return ((((96 * N) + (16 * Q)) + (4 * D)) + L);
}
function main() {
  for (const pieces of ["♕♘♖♗♗♘♔♖", "♖♘♗♕♔♗♘♖", "♖♕♘♗♗♔♖♘", "♖♘♕♗♗♔♖♘"]) {
    console.log(((((pieces + " or ") + g2l(pieces)) + " has SP-ID of ") + String(spid(pieces))));
  }
}
var _nowSeed = 0;
