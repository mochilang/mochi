// Generated by Mochi v0.10.40 on 2025-07-25 21:10:04 GMT+7
...
}
{
  ...
  function cartN(lists): number[][] {
    ...
    let c: number = 1;
    for (const xs of a) {
      c = (c * (Array.isArray(xs) || typeof xs === 'string' ? xs.length : Object.keys(xs ?? {}).length));
    }
    if ((c == 0)) {
      return [];
    }
    ...
    while ((count < c)) {
      ...
      while ((k >= 0)) {
        idx[k] = (idx[k] + 1);
        if ((idx[k] < (Array.isArray(a[k]) || typeof a[k] === 'string' ? a[k].length : Object.keys(a[k] ?? {}).length))) {
          break
        }
        idx[k] = 0;
        k = (k - 1);
