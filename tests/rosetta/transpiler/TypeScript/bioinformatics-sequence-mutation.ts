// Generated by Mochi v0.10.38 on 2025-07-23 17:08:14 GMT+7
...
function makeSeq(s: number, le: number): any[] {
  ...
  while ((i < le)) {
    ...
    s = r[0];
    const idx: number = Math.trunc(r[1]);
...
function mutate(s: number, dna: string, w: number[]): any[] {
  ...
  s = r[0];
  const p: number = Math.trunc(r[1]);
  r = randInt(s, 300);
  s = r[0];
  const x: number = Math.trunc(r[1]);
  let arr: string[] = [];
  let i: number = 0;
  while ((i < le)) {
  ...
  if ((x < w[0])) {
    ...
    if ((x < (w[0] + w[1]))) {
      ...
      r = randInt(s, 4);
      s = r[0];
      const idx2: number = Math.trunc(r[1]);
