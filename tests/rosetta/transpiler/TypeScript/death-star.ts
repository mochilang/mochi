// Generated by Mochi v0.10.39 on 2025-07-25 00:37:46 GMT+7
...
function normalize(v: V3): V3 {
  const len = sqrtApprox((((v.x * v.x) + (v.y * v.y)) + (v.z * v.z)));
  return {"x": Math.trunc(v.x / len), "y": Math.trunc(v.y / len), "z": Math.trunc(v.z / len)};
}
...
function hitSphere(s: Sphere, x: number, y: number): Record<string, any> {
  const dx: number = (x - s.cx);
  const dy: number = (y - s.cy);
  const zsq: number = ((s.r * s.r) - ((dx * dx) + (dy * dy)));
  if ((zsq < 0.0)) {
    return {"hit": false};
  }
  const z: number = sqrtApprox(zsq);
  return {"hit": true, "z1": (s.cz - z), "z2": (s.cz + z)};
}
function main() {
  const shades: string = ".:!*oe&#%@";
  let light: V3 = normalize({"x": -50.0, "y": 30.0, "z": 50.0});
  const pos: Sphere = {"cx": 20.0, "cy": 20.0, "cz": 0.0, "r": 20.0};
  const neg: Sphere = {"cx": 1.0, "cy": 1.0, "cz": -6.0, "r": 20.0};
  let yi: number = 0;
  while ((yi <= 40)) {
    const y: number = (yi + 0.5);
    let line: string = "";
    let xi: number = -20;
    while ((xi <= 60)) {
      const x: number = ((((xi - pos.cx) / 2.0) + 0.5) + pos.cx);
      const hb: Record<string, any> = hitSphere(pos, x, y);
      if (!hb.hit) {
        line = (line + " ");
        xi = (xi + 1);
        continue
      ...
      const zb1 = hb.z1;
      const zb2 = hb.z2;
      const hs: Record<string, any> = hitSphere(neg, x, y);
      let hitRes: number = 1;
      if (!hs.hit) {
        hitRes = 1;
      } else {
