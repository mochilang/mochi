// Generated by Mochi v0.10.40 on 2025-07-25 17:41:14 GMT+7
...
function push(h: Record<string, number>[], it: Record<string, number>): Record<string, number>[] {
  ...
  while (((i > 0) && (h[Math.trunc((i - 1))].s > h[i].s))) {
    const tmp: Record<string, number> = h[Math.trunc((i - 1))];
    h[(i - 1)] = h[i];
    h[i] = tmp;
    i = (i - 1);
  }
...
function step(h: Record<string, number>[], nv: number, dir: number[]): Record<string, any> {
  ...
  for (const p of temp) {
    if ((p[Math.trunc(0)] != p[Math.trunc(1)])) {
      list = [...list, [p[Math.trunc(1)], p[Math.trunc(0)]]];
    }
  }
  temp = list;
  for (const p of temp) {
    if ((p[Math.trunc(1)] != 0)) {
      list = [...list, [p[Math.trunc(0)], -p[Math.trunc(1)]]];
    }
  }
  temp = list;
  for (const p of temp) {
  ...
  let best: number[] = dir;
  for (const p of list) {
    const cross = ((p[Math.trunc(0)] * dir[Math.trunc(1)]) - (p[Math.trunc(1)] * dir[Math.trunc(0)]));
    if ((cross >= 0)) {
      const dot = ((p[Math.trunc(0)] * dir[Math.trunc(0)]) + (p[Math.trunc(1)] * dir[Math.trunc(1)]));
      if ((dot > bestDot)) {
        bestDot = dot;
        best = p;
      }
...
function main() {
  ...
  while ((i < (Array.isArray(pts) || typeof pts === 'string' ? pts.length : Object.keys(pts ?? {}).length))) {
    ...
    const s: string = pad((((("(" + String(p[Math.trunc(0)])) + ", ") + String(p[Math.trunc(1)])) + ")"), 10);
    line = (line + s);
    if ((((i + 1) % 10) == 0)) {
      console.log(line);
      line = "";
