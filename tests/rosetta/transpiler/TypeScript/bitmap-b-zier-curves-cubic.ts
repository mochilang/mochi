// Generated by Mochi v0.10.38 on 2025-07-23 17:13:15 GMT+7

const b3Seg: number = 30;
...
function bezier3(b: Record<string, any>, x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, x4: number, y4: number, p: Pixel) {
  ...
  i = 1;
  while ((i <= b3Seg)) {
    const x: number = px[Math.trunc(i)];
    const y: number = py[Math.trunc(i)];
    line(b, x0, y0, x, y, p);
    x0 = x;
    y0 = y;
    i = (i + 1);
