// Generated by Mochi v0.10.37 on 2025-07-23 12:52:37 GMT+7
...
function createPolybius(): string[] {
  ...
  while ((i < 6)) {
    ...
    p = [...p, row];
    let line: any[] = [...labels.slice(i, (i + 1)), " | "];
    let j: number = 0;
    while ((j < 6)) {
...
function encrypt(polybius: string[], key: string, plainText: string): string {
  ...
  while ((idx < (Array.isArray(temp) || typeof temp === 'string' ? temp.length : Object.keys(temp ?? {}).length))) {
    const row: number = Math.trunc(idx / (Array.isArray(key) || typeof key === 'string' ? key.length : Object.keys(key ?? {}).length));
...
function decrypt(polybius: string[], key: string, cipherText: string): string {
  ...
  let idx: number = 0;
  while ((idx < (Array.isArray(temp) || typeof temp === 'string' ? temp.length : Object.keys(temp ?? {}).length))) {
    const rIdx: number = indexOf(adfgvx, temp.slice(idx, (idx + 1)));
    const cIdx: number = indexOf(adfgvx, temp.slice((idx + 1), (idx + 2)));
