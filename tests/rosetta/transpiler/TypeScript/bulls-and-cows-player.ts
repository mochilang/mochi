// Generated by Mochi v0.10.40 on 2025-07-25 19:31:19 GMT+7
...
}
{
  ...
  function main() {
    ...
    while (true) {
      ...
      const guess: string = patterns[Math.trunc(0)];
      ...
      while (true) {
        ...
        const toks: string[] = fields(line);
        if (((Array.isArray(toks) || typeof toks === 'string' ? toks.length : Object.keys(toks ?? {}).length) == 2)) {
          const c: number = Math.trunc(toks[Math.trunc(0)]);
          const b: number = Math.trunc(toks[Math.trunc(1)]);
      ...
      while ((idx < (Array.isArray(patterns) || typeof patterns === 'string' ? patterns.length : Object.keys(patterns ?? {}).length))) {
        const pat: string = patterns[idx];
        let c: number = 0;
        let b: number = 0;
        let i: number = 0;
        while ((i < 4)) {
          const cg: string = (guess).substring(i, (i + 1));
          const cp: string = (pat).substring(i, (i + 1));
          if ((cg == cp)) {
            b = (b + 1);
          } else {
            if ((pat.indexOf(cg) >= 0)) {
