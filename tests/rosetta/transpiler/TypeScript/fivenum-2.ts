// Generated by Mochi v0.10.41 on 2025-07-27 11:42:13 GMT+7
...
function fivenum(a: number[]): number[] {
  ...
  let m: number = Math.trunc(last / 2);
  let n5: number[] = [];
  ...
  let q3: number = ((last - m) - Math.trunc((Array.isArray(a) || typeof a === 'string' ? a.length : Object.keys(a ?? {}).length) / 4));
...
(() => {
  ...
  const _start = _now()
  console.log(String(fivenum(x1)));
  console.log(String(fivenum(x2)));
  console.log(String(fivenum(x3)));
  const _end = _now()
