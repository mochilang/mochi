// Generated by Mochi v0.10.40 on 2025-07-25 21:13:54 GMT+7
...
}
{
  ...
  function format(ts: Record<string, number>[]): string {
    ...
    while ((i < (Array.isArray(ts) || typeof ts === 'string' ? ts.length : Object.keys(ts ?? {}).length))) {
      const t: Record<string, number> = ts[i];
  ...
  function tanEval(coef: number, f: number): number {
    ...
    const ca: number = Math.trunc(coef / 2);
    const cb: number = (coef - ca);
    const a: number = tanEval(ca, f);
    const b: number = tanEval(cb, f);
    return ((a + b) / (1 - (a * b)));
  }
  function tans(m: Record<string, number>[]): number {
    if (((Array.isArray(m) || typeof m === 'string' ? m.length : Object.keys(m ?? {}).length) == 1)) {
      const t: Record<string, number> = m[Math.trunc(0)];
      return tanEval(t.a, br(t.n, t.d));
    }
    const half: number = Math.trunc((Array.isArray(m) || typeof m === 'string' ? m.length : Object.keys(m ?? {}).length) / 2);
    const a: number = tans(m.slice(0, half));
    const b: number = tans(m.slice(half));
    return ((a + b) / (1 - (a * b)));
  }
