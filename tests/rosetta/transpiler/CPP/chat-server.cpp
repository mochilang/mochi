// Generated by Mochi transpiler v0.10.54 on 2025-08-02 12:07:57 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<std::string> removeName(const std::vector<std::string>& names, std::string name);
int main();
...
std::vector<std::string> removeName(const std::vector<std::string>& names, std::string name) {
    ...
    for (std::string n : names) {
        if ((n != name)) {
            out = ([&]{ auto __tmp = out; __tmp.push_back(n); return __tmp; }());
        }
    }
    return out;
}
...
int main() {
    {
        ...
        std::function<void(std::string)> broadcast = [&](std::string msg) mutable {
            std::cout << msg;
            std::cout << std::endl;
        };
        std::function<void(std::string)> add = [&](std::string name) mutable {
            clients = ([&]{ auto __tmp = clients; __tmp.push_back((std::string)name); return __tmp; }());
            broadcast(((std::string("+++ \"") + name) + std::string("\" connected +++\n")));
        };
        std::function<void(std::string, std::string)> send = [&](std::string name, std::string msg) mutable {
            broadcast((((name + std::string("> ")) + msg) + std::string("\n")));
        };
        std::function<void(std::string)> remove = [&](std::string name) mutable {
            clients = removeName(clients, name);
            broadcast(((std::string("--- \"") + name) + std::string("\" disconnected ---\n")));
        };
