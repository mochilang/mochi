// Generated by Mochi transpiler v0.10.54 on 2025-08-02 12:06:14 GMT+7
#include <iostream>
#include <string>
#include <functional>
#include <vector>
...
std::ostream& operator<<(std::ostream& os, const Box& v) {
    os << '{' << "'Contents': "<< "'" << v.Contents << "'"
 << ", " << "'secret': "<< v.secret
 << '}';
    return os;
}

std::vector<std::function<void()>> newFactory();
int64_t Box_TellSecret(const Box& self);
int main();

std::vector<std::function<void()>> funcs = newFactory();
std::function<void()> New = funcs[int64_t(0)];
std::function<void()> Count = funcs[int64_t(1)];

std::vector<std::function<void()>> newFactory() {
    int64_t sn = int64_t(0);
    std::function<Box()> New = [&]() mutable {
        sn = (sn + int64_t(1));
        Box b = Box{.secret = sn};
        if ((sn == int64_t(1))) {
            b.Contents = std::string("rabbit");
        } else         if ((sn == int64_t(2))) {
            b.Contents = std::string("rock");
        }

        return b;
    };
    std::function<int64_t()> Count = [&]() mutable {
        return sn;
    };
    return std::vector<std::function<std::any()>>{New, Count};
}
