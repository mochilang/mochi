// Generated by Mochi transpiler v0.10.37 on 2025-07-23 10:24:37 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <any>
...
std::map<std::string, std::any> isValidMove(int m) {
    if ((m == 0)) {
        return std::map<std::string, std::any>{{"idx", std::any((empty - 4))}, {"ok", std::any(((empty / 4) > 0))}};
    }
    if ((m == 1)) {
        return std::map<std::string, std::any>{{"idx", std::any((empty + 4))}, {"ok", std::any(((empty / 4) < 3))}};
    }
    if ((m == 2)) {
        return std::map<std::string, std::any>{{"idx", std::any((empty + 1))}, {"ok", std::any(((empty % 4) < 3))}};
    }
    if ((m == 3)) {
        return std::map<std::string, std::any>{{"idx", std::any((empty - 1))}, {"ok", std::any(((empty % 4) > 0))}};
    }
    return std::map<std::string, std::any>{{"idx", std::any(0)}, {"ok", std::any(false)}};
}

bool doMove(int m) {
    std::map<std::string, std::any> r = isValidMove(m);
    if (!std::any_cast<bool>(r[std::string("ok")])) {
        return false;
    }
    int i = empty;
    int j = std::any_cast<int>(r[std::string("idx")]);
    int tmp = board[i];
    board[i] = board[j];
    board[j] = tmp;
    empty = j;
