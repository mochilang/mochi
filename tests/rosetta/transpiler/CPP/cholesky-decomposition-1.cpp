// Generated by Mochi transpiler v0.10.54 on 2025-08-02 12:08:38 GMT+7
#include <iostream>
#include <string>
#include <any>
#include <map>
...
std::vector<std::vector<double>> unpackSym(const std::map<std::string, std::any>& m) {
    ...
    while ((r < any_to_double(n))) {
        ...
        int64_t c = int64_t(0);
        while ((c <= r)) {
            row = ([&]{ auto __tmp = row; __tmp.push_back(std::any_cast<double>(any_index(ele, idx))); return __tmp; }());
            idx = (idx + int64_t(1));
            c = (c + int64_t(1));
        }
