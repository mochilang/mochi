// Generated by Mochi transpiler v0.10.47 on 2025-07-28 11:47:21 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <map>
...
bool damm(std::string s) {
    ...
    int64_t i = int64_t(0);
    while ((i < s.size())) {
        int64_t digit = (int64_t)(digits.at(([&](const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__v)>, std::string>) return __v.substr(i, (i + int64_t(1)) - i); else return std::vector<typename std::decay_t<decltype(__v)>::value_type>(__v.begin()+i, __v.begin()+(i + int64_t(1))); })(s)));
