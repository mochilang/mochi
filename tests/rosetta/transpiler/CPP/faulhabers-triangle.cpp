// Generated by Mochi transpiler v0.10.50 on 2025-07-30 21:15:22 GMT+7
#include <iostream>
#include <string>
...
std::vector<BigRat> faulhaberRow(int64_t p);
std::string ratStr(const BigRat& r);
...
std::vector<BigRat> faulhaberRow(int64_t p) {
    std::vector<BigRat> coeffs = {};
    int64_t i = int64_t(0);
    while ((i <= p)) {
        coeffs = ([&]{ auto __tmp = coeffs; __tmp.push_back(_bigrat(int64_t(0))); return __tmp; }());
        i = (i + int64_t(1));
    }
    int64_t j = int64_t(0);
    ...
    while ((j <= p)) {
        sign = -sign;
        BigRat c = (_bigrat(int64_t(1)) / _bigrat((p + int64_t(1))));
        if ((sign < int64_t(0))) {
            c = -c;
        }
...
std::string ratStr(const BigRat& r) {
    std::string s = ([&]{ std::ostringstream ss; ss << std::boolalpha << r; return ss.str(); }());
    if (endsWith(s, std::string("/1"))) {
        return s.substr(int64_t(0), (s.size() - int64_t(2)) - int64_t(0));
    }
    return s;
}
...
int main() {
    int64_t p = int64_t(0);
    while ((p < int64_t(10))) {
        std::vector<BigRat> row = faulhaberRow(p);
        std::string line = std::string("");
        int64_t idx = int64_t(0);
        while ((idx < row.size())) {
            line = (line + ([&]{ std::string __s = ratStr(row[idx]); while(__s.size() < int64_t(5)) __s = std::string(" ") + __s; return __s; }()));
            if ((idx < (row.size() - int64_t(1)))) {
    ...
    std::cout << std::string("");
    std::cout << std::endl;
    int64_t k = int64_t(17);
    std::vector<BigRat> coeffs = faulhaberRow(k);
    BigRat nn = _bigrat(int64_t(1000));
    BigRat np = _bigrat(int64_t(1));
    BigRat sum = _bigrat(int64_t(0));
    int64_t i = int64_t(0);
    while ((i < coeffs.size())) {
        np = (np * nn);
        sum = (sum + (coeffs[i] * np));
