// Generated by Mochi transpiler v0.10.54 on 2025-08-02 12:08:15 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<std::string> partList = std::vector<std::string>{std::string("A"), std::string("B"), std::string("C"), std::string("D")};
int64_t nAssemblies = int64_t(3);

std::string lower(std::string ch) {
    std::string up = std::string("ABCDEFGHIJKLMNOPQRSTUVWXYZ");
    std::string low = std::string("abcdefghijklmnopqrstuvwxyz");
    int64_t i = int64_t(0);
    while ((i < up.size())) {
        if ((ch == up.substr(i, (i + int64_t(1)) - i))) {
            return low.substr(i, (i + int64_t(1)) - i);
        }
        i = (i + int64_t(1));
    }
    return ch;
...
int main() {
    {
        ...
        for (std::string p : partList) {
            std::cout << (p + std::string(" worker running"));
            std::cout << std::endl;
        }
        for (int cycle = int64_t(1); cycle < (nAssemblies + int64_t(1)); cycle++ ) {
            std::cout << (std::string("begin assembly cycle ") + ([&]{ std::ostringstream ss; any_to_stream(ss, cycle);
 return ss.str(); }()));
            std::cout << std::endl;
            std::string a = std::string("");
            for (std::string p : partList) {
                std::cout << (p + std::string(" worker begins part"));
                std::cout << std::endl;
                std::cout << ((p + std::string(" worker completed ")) + ([&]{ std::string __s = p; for(auto &__c : __s){ __c = std::tolower(static_cast<unsigned char>(__c)); } return __s; }()));
                std::cout << std::endl;
                a = (a + ([&]{ std::string __s = p; for(auto &__c : __s){ __c = std::tolower(static_cast<unsigned char>(__c)); } return __s; }()));
            }
            std::cout << (((a + std::string(" assembled.  cycle ")) + ([&]{ std::ostringstream ss; any_to_stream(ss, cycle);
 return ss.str(); }())) + std::string(" complete"));
            std::cout << std::endl;
        }
        for (std::string p : partList) {
            std::cout << (p + std::string(" worker stopped"));
            std::cout << std::endl;
        }
    }
    return 0;
