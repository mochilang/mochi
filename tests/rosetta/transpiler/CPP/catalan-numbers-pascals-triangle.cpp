// Generated by Mochi transpiler v0.10.54 on 2025-08-02 12:07:17 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
int main() {
    {
        ...
        for (int _ = int64_t(0); _ < (n + int64_t(2)); _++ ) {
            t = ([&]{ auto __tmp = t; __tmp.push_back(int64_t(0)); return __tmp; }());
        }
        t[int64_t(1)] = int64_t(1);
        for (int i = int64_t(1); i < (n + int64_t(1)); i++ ) {
            auto j = i;
            while ((j > int64_t(1))) {
                t[j] = (t[j] + t[(j - int64_t(1))]);
                j = (j - int64_t(1));
            }
            t[(int64_t)((i + int64_t(1)))] = t[i];
            j = (i + int64_t(1));
            while ((j > int64_t(1))) {
                t[j] = (t[j] + t[(j - int64_t(1))]);
                j = (j - int64_t(1));
            }
            int64_t cat = (t[(i + int64_t(1))] - t[i]);
            if ((i < int64_t(10))) {
                std::cout << (((std::string(" ") + ([&]{ std::ostringstream ss; any_to_stream(ss, i);
 return ss.str(); }())) + std::string(" : ")) + ([&]{ std::ostringstream ss; ss << std::boolalpha << cat; return ss.str(); }()));
                std::cout << std::endl;
            } else {
                std::cout << ((([&]{ std::ostringstream ss; any_to_stream(ss, i);
 return ss.str(); }()) + std::string(" : ")) + ([&]{ std::ostringstream ss; ss << std::boolalpha << cat; return ss.str(); }()));
                std::cout << std::endl;
            }
        }
