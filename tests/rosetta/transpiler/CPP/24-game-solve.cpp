// Generated by Mochi transpiler v0.10.40 on 2025-07-25 11:56:52 GMT+7
#include <iostream>
#include <string>
#include <memory>
#include <sstream>
...
struct Bin : Expr {
    ...
    Bin(int op_, std::unique_ptr<Expr> left_, std::unique_ptr<Expr> right_) : op(op_), left(std::move(left_)), right(std::move(right_)) {}
...
std::unique_ptr<Expr> newNum(int n);
Rational exprEval(Expr* x);
std::string exprString(Expr* x);
bool solve(const std::vector<std::unique_ptr<Expr>>& xs);
int main();
...
std::unique_ptr<Expr> newNum(int n) {
    return std::make_unique<Num>(Rational{n, 1});
}
...
bool solve(const std::vector<std::unique_ptr<Expr>>& xs) {
    ...
    while ((i < xs.size())) {
        int j = (i + 1);
        while ((j < xs.size())) {
            std::vector<std::unique_ptr<Expr>> rest = {};
            int k = 0;
            while ((k < xs.size())) {
                if (((k != i) && (k != j))) {
            ...
            Expr* a = xs[i].get();
            Expr* b = xs[j].get();
            Bin node = Bin{OP_ADD, a, b};
            for (int op : std::vector<int>{OP_ADD, OP_SUB, OP_MUL, OP_DIV}) {
                node = Bin{op, a, b};
                if (solve((std::vector<std::unique_ptr<Expr>>)(([&]{ auto __tmp = rest; __tmp.push_back((std::unique_ptr<Expr>)node); return __tmp; }())))) {
                    return true;
                }
            }
            node = Bin{OP_SUB, b, a};
...
int main() {
    int iter = 0;
    while ((iter < 10)) {
        std::vector<std::unique_ptr<Expr>> cards = {};
        int i = 0;
        while ((i < n_cards)) {
