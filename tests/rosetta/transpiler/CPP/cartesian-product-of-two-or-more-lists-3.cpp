// Generated by Mochi transpiler v0.10.54 on 2025-08-02 12:06:53 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <vector>
...
std::vector<std::vector<int64_t>> cartN(std::any lists) {
    if ((any_to_string(lists) == any_to_string(nullptr))) {
    ...
    for (int64_t x : a[int64_t(0)]) {
        for (std::vector<int64_t> p : rest) {
            out = ([&]{ auto __tmp = out; __tmp.push_back(concat(std::vector<int64_t>{x}, p)); return __tmp; }());
        }
    }
    return out;
}
