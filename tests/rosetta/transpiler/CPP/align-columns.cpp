// Generated by Mochi transpiler v0.10.39 on 2025-07-24 21:22:22 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
std::vector<std::string> rstripEmpty(std::vector<std::string>& words);
...
std::vector<std::string> rstripEmpty(const std::vector<std::string>& words) {
    ...
    return ([&](const auto& c){ if constexpr(std::is_same_v<std::decay_t<decltype(c)>, std::string>) return c.substr(0, n - 0); else return std::vector<typename std::decay_t<decltype(c)>::value_type>(c.begin()+0, c.begin()+n); })(words);
}
