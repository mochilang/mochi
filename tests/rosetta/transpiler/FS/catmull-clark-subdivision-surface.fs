...
and fmt4 (x: float) =
    ...
    try
        ...
        if y >= (unbox<float> 0) then
            y <- y + 0.5
        else
            y <- y - 0.5
        y <- (unbox<float> (unbox<int> y)) / 10000.0
...
and sortEdges (edges: int array array) =
    ...
    try
        ...
        while (unbox<int> (Array.length tmp)) > 0 do
            ...
            let mutable k: int = 0
            while k < (unbox<int> (Array.length tmp)) do
                if k <> idx then
...
and getEdgesFaces (points: Point array) (faces: int array array) =
    ...
    try
        ...
        while fnum < (unbox<int> (Array.length faces)) do
            ...
            while pi < numP do
                ...
                if pi < (numP - 1) then
                    pn2 <- unbox<int> (face.[pi + 1])
                else
                    pn2 <- unbox<int> (face.[0])
                if pn1 > pn2 then
        ...
        let mutable idx: int = 0
        try
            while idx < (unbox<int> (Array.length edges)) do
                let e1: int array = edges.[idx]
                if idx < (unbox<int> ((unbox<int> (Array.length edges)) - 1)) then
...
and getEdgePoints (points: Point array) (edgesFaces: Edge array) (facePoints: Point array) =
    ...
    try
        ...
        let mutable i: int = 0
        while i < (unbox<int> (Array.length edgesFaces)) do
...
and getAvgFacePoints (points: Point array) (faces: int array array) (facePoints: Point array) =
    ...
    try
        ...
        while j < numP do
            ...
            avg <- unbox<Point array> (Array.append avg [|divPoint (tp.p) (unbox<float> (tp.n))|])
            j <- j + 1
...
and getPointsFaces (points: Point array) (faces: int array array) =
    ...
    try
        ...
        let mutable i: int = 0
        while i < (unbox<int> (Array.length points)) do
            pf <- unbox<int array> (Array.append pf [|0|])
            i <- i + 1
        let mutable fnum: int = 0
        while fnum < (unbox<int> (Array.length faces)) do
            for pn in faces.[fnum] do
                pf.[pn] <- (unbox<int> (pf.[pn])) + 1
            fnum <- fnum + 1
...
and getNewPoints (points: Point array) (pf: int array) (afp: Point array) (ame: Point array) =
    ...
    try
        ...
        let mutable i: int = 0
        while i < (unbox<int> (Array.length points)) do
            let mutable n: float = unbox<float> (pf.[i])
...
and cmcSubdiv (points: Point array) (faces: int array array) =
    ...
    try
        ...
        let mutable idx: int = 0
        while idx < (unbox<int> (Array.length edgesFaces)) do
        ...
        while fnum < (unbox<int> (Array.length faces)) do
            ...
            if (unbox<int> (Array.length oldFace)) = 4 then
                let a: int = oldFace.[0]
                let b: int = oldFace.[1]
                let c: int = oldFace.[2]
                let d: int = oldFace.[3]
...
and formatPoint (p: Point) =
    ...
    let mutable p = p
    try
        __ret <- ((((("[" + (fmt4 (p.x))) + " ") + (fmt4 (p.y))) + " ") + (fmt4 (p.z))) + "]"
        raise Return
        __ret
    with
        | Return -> __ret
and formatFace (f: int array) =
    ...
    let mutable f = f
    try
        if (unbox<int> (Array.length f)) = 0 then
            __ret <- "[]"
            raise Return
        let mutable s: string = "[" + (fmt2 (unbox<int> (f.[0])))
        let mutable i: int = 1
        while i < (unbox<int> (Array.length f)) do
            s <- (s + " ") + (fmt2 (unbox<int> (f.[i])))
            i <- i + 1
        s <- s + "]"
        __ret <- s
        raise Return
