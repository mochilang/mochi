...
let rec removeKey (m: Map<string, int>) (k: string) =
    ...
    try
        ...
        for KeyValue(key, _) in m do
            if key <> k then
                out <- Map.add key ((defaultArg (Map.tryFind key m) Unchecked.defaultof<obj>)) out
        __ret <- out
        raise Return
        __ret
    with
...
and main () =
    ...
    try
        ...
        x <- Map.ofList []
        x <- Map.add "foo" 3 x
        let y1 = (defaultArg (Map.tryFind "bar" x) Unchecked.defaultof<obj>)
        let ok: bool = Map.containsKey "bar" x
        printfn "%A" y1
        printfn "%d" (if ok then 1 else 0)
        x <- removeKey x "foo"
        x <- Map.ofList [("foo", 2); ("bar", 42); ("baz", -1)]
        printfn "%s" (String.concat " " [|sprintf "%A" ((defaultArg (Map.tryFind "foo" x) Unchecked.defaultof<obj>)); sprintf "%A" ((defaultArg (Map.tryFind "bar" x) Unchecked.defaultof<obj>)); sprintf "%A" ((defaultArg (Map.tryFind "baz" x) Unchecked.defaultof<obj>))|])
        __ret
    with
        | Return -> __ret
main()
