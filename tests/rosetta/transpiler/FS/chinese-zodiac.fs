...
let rec cz (yr: int) (animal: string array) (yinYang: string array) (element: string array) (sc: string array) (bc: string array) =
    ...
    try
        ...
        __ret <- { animal = unbox<string> (animal.[branch]); yinYang = unbox<string> (yinYang.[((stem % 2 + 2) % 2)]); element = unbox<string> (element.[unbox<int> (stem / 2)]); stemBranch = sb; cycle = (((y % 60 + 60) % 60)) + 1 }
        raise Return
        __ret
    with
        | Return -> __ret
for yr in [|1935; 1938; 1968; 1972; 1976|] do
    let r: Info = cz (unbox<int> yr) animal yinYang element stemChArr branchChArr
