...
let rec quibble (items: string array) =
    ...
    try
        let n: int = Array.length items
        if n = 0 then
            __ret <- "{}"
            raise Return
        else
            if n = 1 then
                __ret <- ("{" + (unbox<string> (items.[0]))) + "}"
                raise Return
            else
                if n = 2 then
                    __ret <- ((("{" + (unbox<string> (items.[0]))) + " and ") + (unbox<string> (items.[1]))) + "}"
                    raise Return
                else
                    ...
                    for i in 0 .. ((n - 1) - 1) do
                        try
                            ...
                            if i > 0 then
                                prefix <- prefix + ", "
                            prefix <- prefix + (unbox<string> (items.[i]))
                        with
                        | Break -> ()
                        | Continue -> ()
                    __ret <- ((("{" + prefix) + " and ") + (unbox<string> (items.[n - 1]))) + "}"
                    raise Return
        __ret
    with
        | Return -> __ret
