...
let rec primeFactors (n: int) =
    ...
    try
        ...
        let mutable x: int = n
        while (((x % 2 + 2) % 2)) = 0 do
            factors <- unbox<int array> (Array.append factors [|2|])
            x <- unbox<int> (x / 2)
        let mutable p: int = 3
        while (p * p) <= x do
            while (((x % p + p) % p)) = 0 do
                factors <- unbox<int array> (Array.append factors [|p|])
                x <- unbox<int> (x / p)
            p <- p + 2
        if x > 1 then
            factors <- unbox<int array> (Array.append factors [|x|])
        __ret <- unbox<int array> factors
        raise Return
        __ret
    with
        | Return -> __ret
    ...
    try
        ...
        try
            while count < 20 do
                ...
                if (unbox<int> (Array.length factors)) > 1 then
                    ...
                    for f in factors do
                        ...
                        | Continue -> ()
                    if includesAll then
                        res <- unbox<int array> (Array.append res [|k|])
                        count <- count + 1
                k <- k + 2
        with
        | Break -> ()
        ...
        for e in Array.sub res 0 (10 - 0) do
            line <- (line + (pad10 (commatize (unbox<int> e)))) + " "
