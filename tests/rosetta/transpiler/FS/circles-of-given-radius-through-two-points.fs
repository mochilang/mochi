...
for tc in td do
    ...
    let res: obj array = circles p1 p2 r
    let c1: obj = box (res.[0])
    let c2: obj = box (res.[1])
    let caseStr: obj = box (res.[2])
    ...
    if ((unbox<string> caseStr) = "Points form a diameter and describe only a single circle.") || ((unbox<string> caseStr) = "Coincident points with r==0.0 describe a degenerate circle.") then
        printfn "%s" (((("   Center:  {" + (string (((c1 :?> Map<string, obj>).["x"])))) + " ") + (string (((c1 :?> Map<string, obj>).["y"])))) + "}")
    else
        if (unbox<string> caseStr) = "Two circles." then
            printfn "%s" (((("   Center 1:  {" + (string (((c1 :?> Map<string, obj>).["x"])))) + " ") + (string (((c1 :?> Map<string, obj>).["y"])))) + "}")
            printfn "%s" (((("   Center 2:  {" + (string (((c2 :?> Map<string, obj>).["x"])))) + " ") + (string (((c2 :?> Map<string, obj>).["y"])))) + "}")
    printfn "%s" ""
