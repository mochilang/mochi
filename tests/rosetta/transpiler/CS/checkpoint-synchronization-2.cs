...
class Program {
    ...
    static string lower(string ch_0) {
        ...
        long i_3 = 0;
        while ((Convert.ToDouble(i_3) < Convert.ToDouble(upper_1.Length))) {
    ...
    static void Main() {
        {
            ...
            for (var cycle_6 = 1; cycle_6 < (nAssemblies_5 + 1); cycle_6++) {
                Console.WriteLine(_fmtTop(("begin assembly cycle " + _fmt(cycle_6))));
                string a_7 = "";
                foreach (var p_8 in partList_4) {
                    Console.WriteLine(_fmtTop((p_8 + " worker begins part")));
                    Console.WriteLine(_fmtTop(((p_8 + " worker completed ") + lower(p_8))));
                    a_7 = (a_7 + lower(p_8));
                }
                Console.WriteLine(_fmtTop((((a_7 + " assembled.  cycle ") + _fmt(cycle_6)) + " complete")));
            }
            var __end = _now();
            var __memEnd = _mem();
