...
(define (modPow base exp mod)
 (call/cc (lambda (ret4)
 (let ((result (modulo 1 mod)
)
)
 (begin (let ((b (modulo base mod)
)
)
 (begin (let ((e exp)
)
...
(define (main)
 ...
 (letrec ((loop8 (lambda ()
 (if (<= k 10)
 (begin (_display (to-str (string-append (string-append "The first 50 Curzon numbers using a base of " (to-str k)
)
 " :")
)
)
 ...
 (begin (let ((curzon50 (_list)
)
)
 (begin (call/cc (lambda (break11)
 (letrec ((loop10 (lambda ()
 (if #t (begin (let ((d (_add (* k n)
 1)
)
)
 (begin (if (equal? (fmod (_add (modPow k n d)
 1)
 d)
 0)
 (begin (if (< count 50)
 (begin (set! curzon50 (append curzon50 (_list n)
)
)
)
 (quote ()
 ...
 (if (< idx (cond ((string? curzon50)
 (string-length curzon50)
)
 ((hash-table? curzon50)
 (hash-table-size curzon50)
)
 (else (length curzon50)
)
)
)
 (begin (let ((line "")
 ...
 (if (< j 10)
 (begin (set! line (string-append (string-append line (padLeft (list-ref curzon50 idx)
 4)
)
 " ")
)
