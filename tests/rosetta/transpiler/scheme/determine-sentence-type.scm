;; Generated on 2025-07-28 11:38 +0700
...
(let ((start11 (now)
 ...
 (begin (if (string=? ch "?")
 (begin (set! types (append types (_list "Q")
)
)
)
 (if (string=? ch "!")
 (begin (set! types (append types (_list "E")
)
)
)
 (if (string=? ch ".")
 (begin (set! types (append types (_list "S")
)
)
)
 (quote ()
 ...
 (let ((last (substring s (- (cond ((string? s)
 (string-length s)
)
 ((hash-table? s)
 (hash-table-size s)
 ...
 (begin (if (equal? (indexOf "?!." last)
 (- 1)
)
 (begin (set! types (append types (_list "N")
)
)
)
 (quote ()
 ...
 (begin (let ((result (sentenceType s)
)
)
 (begin (_display (to-str result)
)
 (newline)
)
)
