...
(define (ftree g x y dist dir d)
 ...
 (begin (let ((x2 (_add x (* dist (_sin rad)
)
)
)
)
 (begin (let ((y2 (- y (* dist (_cos rad)
)
)
)
)
 (begin (bresenham (let ((v19 x)
)
 (cond ((string? v19)
 ...
 g)
 (if (> d 0)
 (begin (ftree g x2 y2 (* dist frac)
 (- dir angle)
 (- d 1)
)
 (ftree g x2 y2 (* dist frac)
 (+ dir angle)
 (- d 1)
)
)
...
(define (main)
 ...
 (begin (ftree grid (+ 0.0 (quotient width 2)
)
 (+ 0.0 (- height 1)
)
 length 0.0 depth)
