...
(define (newFps fn)
 (call/cc (lambda (ret1)
 (ret1 (alist->hash-table (_list (cons "coeffs" (_list)
)
 (cons "compute" fn)
)
)
...
(define (extract f n)
 ...
 (letrec ((loop3 (lambda ()
 (if (<= (cond ((string? (hash-table-ref f "coeffs")
)
 (string-length (hash-table-ref f "coeffs")
)
)
 ((hash-table? (hash-table-ref f "coeffs")
)
 (hash-table-size (hash-table-ref f "coeffs")
)
)
 (else (length (hash-table-ref f "coeffs")
)
)
)
 n)
 (begin (let ((idx (cond ((string? (hash-table-ref f "coeffs")
)
 (string-length (hash-table-ref f "coeffs")
)
)
 ...
 (begin (let ((v ((hash-table-ref f "compute")
 idx)
)
)
 (begin (hash-table-set! f "coeffs" (append (hash-table-ref f "coeffs")
 (_list v)
)
)
)
 ...
 (ret2 (list-ref (hash-table-ref f "coeffs")
 n)
)
)
)
...
(define (add a b)
 (call/cc (lambda (ret7)
 (ret7 (newFps (lambda (n)
 (call/cc (lambda (ret8)
 (ret8 (_add (extract a n)
 (extract b n)
)
)
)
...
(define (mul a b)
 ...
 (if (_le k n)
 (begin (set! s (_add s (* (extract a k)
 (extract b (- n k)
)
)
)
...
(define (div a b)
 ...
 (begin (hash-table-set! q "compute" (lambda (n)
 (call/cc (lambda (ret17)
 (let ((b0 (extract b 0)
)
)
 (begin (if (equal? b0 0.0)
 (begin (ret17 (/ 0.0 0.0)
)
)
 (quote ()
...
(define (sinCos)
 ...
 (begin (let ((cos (sub (one)
 (integrate sin)
)
)
)
 (begin (hash-table-set! sin "compute" (lambda (n)
 ...
 (ret24 (alist->hash-table (_list (cons "sin" sin)
 (cons "cos" cos)
)
)
)
...
(define (fmtF5 x)
 ...
 (begin (set! s (string-append s ".00000")
)
)
 ...
 (begin (if (> decs 5)
 (begin (set! s (substring s 0 (+ dot 6)
)
)
)
...
(define (main)
 ...
 (begin (_display (to-str (string-append "sin:" (partialSeries (hash-table-ref p "sin")
)
)
)
)
