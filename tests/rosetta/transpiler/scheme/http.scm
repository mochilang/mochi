...
(import (chibi json))
(define (_fetch url) (let ((d (string->json (process->string (string-append "curl -s " url))))) (if (and (list? d) (pair? d) (pair? (car d))) (alist->hash-table (map (lambda (p) (cons (symbol->string (car p)) (cdr p))) d)) d)))
(define (to-str x)
  (cond ((pair? x)
