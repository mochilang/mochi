...
(define (randOrder seed n)
 ...
 (begin (ret1 (list next (modulo next n)
)
)
)
)
...
(define (randChaos seed n)
 ...
 (begin (ret2 (list next (modulo next n)
)
)
)
)
...
(define (main)
 ...
 (begin (let ((buckets (list)
)
)
 (begin (call/cc (lambda (break5)
 (letrec ((loop4 (lambda (i)
 (if (< i nBuckets)
 (begin (begin (set! buckets (append buckets (list 0)
)
)
)
 (loop4 (+ i 1)
 ...
 (begin (set! seedOrder (list-ref r 0)
)
 (let ((b1 (list-ref r 1)
)
)
 ...
 (let ((amt (list-ref r 1)
)
)
 (begin (if (> amt (list-ref buckets b1)
)
