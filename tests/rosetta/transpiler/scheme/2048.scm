;; Generated on 2025-07-23 12:28 +0700
(import (only (scheme base) call/cc list-ref list-set! list))
(import (scheme time))
(import (srfi 69))
(import (chibi io))
(define (to-str x)
  (cond ((pair? x)
         (string-append "[" (string-join (map to-str x) ", ") "]"))
        ((string? x) x)
        ((boolean? x) (if x "1" "0"))
        (else (number->string x))))
(define (_input)
  (let ((l (read-line)))
    (if (eof-object? l) "" l)))
(define SIZE 4)
(define (newBoard)
 (call/cc (lambda (ret1)
 (let ((b (list)
)
 (y 0)
)
 (begin (call/cc (lambda (break3)
 (letrec ((loop2 (lambda ()
 (if (< y SIZE)
 (begin (let ((row (list)
)
 (x 0)
)
 (begin (call/cc (lambda (break5)
 (letrec ((loop4 (lambda ()
 (if (< x SIZE)
 (begin (set! row (append row (list 0)
)
)
 (set! x (+ x 1)
)
 (loop4)
)
 (quote nil)
)
)
)
)
 (loop4)
)
)
)
 (set! b (append b (list row)
)
)
 (set! y (+ y 1)
)
)
)
 (loop2)
)
 (quote nil)
)
)
)
)
 (loop2)
)
)
)
 (ret1 b)
)
)
)
)
)
(define (spawnTile b)
 (call/cc (lambda (ret6)
 (let ((empty (list)
)
 (y 0)
)
 (begin (call/cc (lambda (break8)
 (letrec ((loop7 (lambda ()
 (if (< y SIZE)
 (begin (let ((x 0)
)
 (begin (call/cc (lambda (break10)
 (letrec ((loop9 (lambda ()
 (if (< x SIZE)
 (begin (if (= (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
 0)
 (begin (set! empty (append empty (list (list x y)
)
)
)
)
 (quote nil)
)
 (set! x (+ x 1)
)
 (loop9)
)
 (quote nil)
)
)
)
)
 (loop9)
)
)
)
 (set! y (+ y 1)
)
)
)
 (loop7)
)
 (quote nil)
)
)
)
)
 (loop7)
)
)
)
 (if (= (length empty)
 0)
 (begin (ret6 (alist->hash-table (list (cons "board" b)
 (cons "full" #t)
)
)
)
)
 (quote nil)
)
 (define idx (modulo (current-jiffy)
 (length empty)
)
)
 (define cell (list-ref empty idx)
)
 (define val 4)
 (if (< (modulo (current-jiffy)
 10)
 9)
 (begin (set! val 2)
)
 (quote nil)
)
 (list-set! (list-ref b (list-ref cell 1)
)
 (list-ref cell 0)
 val)
 (ret6 (alist->hash-table (list (cons "board" b)
 (cons "full" (= (length empty)
 1)
)
)
)
)
)
)
)
)
)
(define (pad n)
 (call/cc (lambda (ret11)
 (let ((s (number->string n)
)
 (pad (- 4 (string-length s)
)
)
 (i 0)
 (out "")
)
 (begin (call/cc (lambda (break13)
 (letrec ((loop12 (lambda ()
 (if (< i pad)
 (begin (set! out (string-append out " ")
)
 (set! i (+ i 1)
)
 (loop12)
)
 (quote nil)
)
)
)
)
 (loop12)
)
)
)
 (ret11 (string-append out s)
)
)
)
)
)
)
(define (draw b score)
 (call/cc (lambda (ret14)
 (begin (display (to-str (string-append "Score: " (number->string score)
)
)
)
 (newline)
 (define y 0)
 (call/cc (lambda (break16)
 (letrec ((loop15 (lambda ()
 (if (< y SIZE)
 (begin (display (to-str "+----+----+----+----+")
)
 (newline)
 (define line "|")
 (define x 0)
 (call/cc (lambda (break18)
 (letrec ((loop17 (lambda ()
 (if (< x SIZE)
 (begin (let ((v (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
)
)
 (begin (if (= v 0)
 (begin (set! line (string-append line "    |")
)
)
 (begin (set! line (string-append (string-append line (pad v)
)
 "|")
)
)
)
 (set! x (+ x 1)
)
)
)
 (loop17)
)
 (quote nil)
)
)
)
)
 (loop17)
)
)
)
 (display (to-str line)
)
 (newline)
 (set! y (+ y 1)
)
 (loop15)
)
 (quote nil)
)
)
)
)
 (loop15)
)
)
)
 (display (to-str "+----+----+----+----+")
)
 (newline)
 (display (to-str "W=Up S=Down A=Left D=Right Q=Quit")
)
 (newline)
)
)
)
)
(define (reverseRow r)
 (call/cc (lambda (ret19)
 (let ((out (list)
)
 (i (- (length r)
 1)
)
)
 (begin (call/cc (lambda (break21)
 (letrec ((loop20 (lambda ()
 (if (>= i 0)
 (begin (set! out (append out (list (list-ref r i)
)
)
)
 (set! i (- i 1)
)
 (loop20)
)
 (quote nil)
)
)
)
)
 (loop20)
)
)
)
 (ret19 out)
)
)
)
)
)
(define (slideLeft row)
 (call/cc (lambda (ret22)
 (let ((xs (list)
)
 (i 0)
)
 (begin (call/cc (lambda (break24)
 (letrec ((loop23 (lambda ()
 (if (< i (length row)
)
 (begin (if (not (= (list-ref row i)
 0)
)
 (begin (set! xs (append xs (list (list-ref row i)
)
)
)
)
 (quote nil)
)
 (set! i (+ i 1)
)
 (loop23)
)
 (quote nil)
)
)
)
)
 (loop23)
)
)
)
 (define res (list)
)
 (define gain 0)
 (set! i 0)
 (call/cc (lambda (break26)
 (letrec ((loop25 (lambda ()
 (if (< i (length xs)
)
 (begin (if (and (< (+ i 1)
 (length xs)
)
 (= (list-ref xs i)
 (list-ref xs (+ i 1)
)
)
)
 (begin (let ((v (* (list-ref xs i)
 2)
)
)
 (begin (set! gain (+ gain v)
)
 (set! res (append res (list v)
)
)
 (set! i (+ i 2)
)
)
)
)
 (begin (set! res (append res (list (list-ref xs i)
)
)
)
 (set! i (+ i 1)
)
)
)
 (loop25)
)
 (quote nil)
)
)
)
)
 (loop25)
)
)
)
 (call/cc (lambda (break28)
 (letrec ((loop27 (lambda ()
 (if (< (length res)
 SIZE)
 (begin (set! res (append res (list 0)
)
)
 (loop27)
)
 (quote nil)
)
)
)
)
 (loop27)
)
)
)
 (ret22 (alist->hash-table (list (cons "row" res)
 (cons "gain" gain)
)
)
)
)
)
)
)
)
(define (moveLeft b score)
 (call/cc (lambda (ret29)
 (let ((moved #f)
 (y 0)
)
 (begin (call/cc (lambda (break31)
 (letrec ((loop30 (lambda ()
 (if (< y SIZE)
 (begin (let ((r (slideLeft (list-ref b y)
)
)
 (new (hash-table-ref r "row")
)
)
 (begin (set! score (+ score (hash-table-ref r "gain")
)
)
 (define x 0)
 (call/cc (lambda (break33)
 (letrec ((loop32 (lambda ()
 (if (< x SIZE)
 (begin (if (not (= (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
 (cond ((string? new)
 (string-ref new x)
)
 ((hash-table? new)
 (hash-table-ref new x)
)
 (else (list-ref new x)
)
)
)
)
 (begin (set! moved #t)
)
 (quote nil)
)
 (list-set! (list-ref b y)
 x (cond ((string? new)
 (string-ref new x)
)
 ((hash-table? new)
 (hash-table-ref new x)
)
 (else (list-ref new x)
)
)
)
 (set! x (+ x 1)
)
 (loop32)
)
 (quote nil)
)
)
)
)
 (loop32)
)
)
)
 (set! y (+ y 1)
)
)
)
 (loop30)
)
 (quote nil)
)
)
)
)
 (loop30)
)
)
)
 (ret29 (alist->hash-table (list (cons "board" b)
 (cons "score" score)
 (cons "moved" moved)
)
)
)
)
)
)
)
)
(define (moveRight b score)
 (call/cc (lambda (ret34)
 (let ((moved #f)
 (y 0)
)
 (begin (call/cc (lambda (break36)
 (letrec ((loop35 (lambda ()
 (if (< y SIZE)
 (begin (let ((rev (reverseRow (list-ref b y)
)
)
 (r (slideLeft rev)
)
)
 (begin (set! rev (hash-table-ref r "row")
)
 (set! score (+ score (hash-table-ref r "gain")
)
)
 (set! rev (reverseRow rev)
)
 (define x 0)
 (call/cc (lambda (break38)
 (letrec ((loop37 (lambda ()
 (if (< x SIZE)
 (begin (if (not (= (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
 (list-ref rev x)
)
)
 (begin (set! moved #t)
)
 (quote nil)
)
 (list-set! (list-ref b y)
 x (list-ref rev x)
)
 (set! x (+ x 1)
)
 (loop37)
)
 (quote nil)
)
)
)
)
 (loop37)
)
)
)
 (set! y (+ y 1)
)
)
)
 (loop35)
)
 (quote nil)
)
)
)
)
 (loop35)
)
)
)
 (ret34 (alist->hash-table (list (cons "board" b)
 (cons "score" score)
 (cons "moved" moved)
)
)
)
)
)
)
)
)
(define (getCol b x)
 (call/cc (lambda (ret39)
 (let ((col (list)
)
 (y 0)
)
 (begin (call/cc (lambda (break41)
 (letrec ((loop40 (lambda ()
 (if (< y SIZE)
 (begin (set! col (append col (list (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
)
)
)
 (set! y (+ y 1)
)
 (loop40)
)
 (quote nil)
)
)
)
)
 (loop40)
)
)
)
 (ret39 col)
)
)
)
)
)
(define (setCol b x col)
 (call/cc (lambda (ret42)
 (let ((y 0)
)
 (begin (call/cc (lambda (break44)
 (letrec ((loop43 (lambda ()
 (if (< y SIZE)
 (begin (list-set! (list-ref b y)
 x (list-ref col y)
)
 (set! y (+ y 1)
)
 (loop43)
)
 (quote nil)
)
)
)
)
 (loop43)
)
)
)
)
)
)
)
)
(define (moveUp b score)
 (call/cc (lambda (ret45)
 (let ((moved #f)
 (x 0)
)
 (begin (call/cc (lambda (break47)
 (letrec ((loop46 (lambda ()
 (if (< x SIZE)
 (begin (let ((col (getCol b x)
)
 (r (slideLeft col)
)
 (new (hash-table-ref r "row")
)
)
 (begin (set! score (+ score (hash-table-ref r "gain")
)
)
 (define y 0)
 (call/cc (lambda (break49)
 (letrec ((loop48 (lambda ()
 (if (< y SIZE)
 (begin (if (not (= (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
 (cond ((string? new)
 (string-ref new y)
)
 ((hash-table? new)
 (hash-table-ref new y)
)
 (else (list-ref new y)
)
)
)
)
 (begin (set! moved #t)
)
 (quote nil)
)
 (list-set! (list-ref b y)
 x (cond ((string? new)
 (string-ref new y)
)
 ((hash-table? new)
 (hash-table-ref new y)
)
 (else (list-ref new y)
)
)
)
 (set! y (+ y 1)
)
 (loop48)
)
 (quote nil)
)
)
)
)
 (loop48)
)
)
)
 (set! x (+ x 1)
)
)
)
 (loop46)
)
 (quote nil)
)
)
)
)
 (loop46)
)
)
)
 (ret45 (alist->hash-table (list (cons "board" b)
 (cons "score" score)
 (cons "moved" moved)
)
)
)
)
)
)
)
)
(define (moveDown b score)
 (call/cc (lambda (ret50)
 (let ((moved #f)
 (x 0)
)
 (begin (call/cc (lambda (break52)
 (letrec ((loop51 (lambda ()
 (if (< x SIZE)
 (begin (let ((col (reverseRow (getCol b x)
)
)
 (r (slideLeft col)
)
)
 (begin (set! col (hash-table-ref r "row")
)
 (set! score (+ score (hash-table-ref r "gain")
)
)
 (set! col (reverseRow col)
)
 (define y 0)
 (call/cc (lambda (break54)
 (letrec ((loop53 (lambda ()
 (if (< y SIZE)
 (begin (if (not (= (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
 (list-ref col y)
)
)
 (begin (set! moved #t)
)
 (quote nil)
)
 (list-set! (list-ref b y)
 x (list-ref col y)
)
 (set! y (+ y 1)
)
 (loop53)
)
 (quote nil)
)
)
)
)
 (loop53)
)
)
)
 (set! x (+ x 1)
)
)
)
 (loop51)
)
 (quote nil)
)
)
)
)
 (loop51)
)
)
)
 (ret50 (alist->hash-table (list (cons "board" b)
 (cons "score" score)
 (cons "moved" moved)
)
)
)
)
)
)
)
)
(define (hasMoves b)
 (call/cc (lambda (ret55)
 (let ((y 0)
)
 (begin (call/cc (lambda (break57)
 (letrec ((loop56 (lambda ()
 (if (< y SIZE)
 (begin (let ((x 0)
)
 (begin (call/cc (lambda (break59)
 (letrec ((loop58 (lambda ()
 (if (< x SIZE)
 (begin (if (= (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
 0)
 (begin (ret55 #t)
)
 (quote nil)
)
 (if (and (< (+ x 1)
 SIZE)
 (= (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
 (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 (+ x 1)
)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 (+ x 1)
)
)
 (else (list-ref (list-ref b y)
 (+ x 1)
)
)
)
)
)
 (begin (ret55 #t)
)
 (quote nil)
)
 (if (and (< (+ y 1)
 SIZE)
 (= (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
 (cond ((string? (list-ref b (+ y 1)
)
)
 (string-ref (list-ref b (+ y 1)
)
 x)
)
 ((hash-table? (list-ref b (+ y 1)
)
)
 (hash-table-ref (list-ref b (+ y 1)
)
 x)
)
 (else (list-ref (list-ref b (+ y 1)
)
 x)
)
)
)
)
 (begin (ret55 #t)
)
 (quote nil)
)
 (set! x (+ x 1)
)
 (loop58)
)
 (quote nil)
)
)
)
)
 (loop58)
)
)
)
 (set! y (+ y 1)
)
)
)
 (loop56)
)
 (quote nil)
)
)
)
)
 (loop56)
)
)
)
 (ret55 #f)
)
)
)
)
)
(define (has2048 b)
 (call/cc (lambda (ret60)
 (let ((y 0)
)
 (begin (call/cc (lambda (break62)
 (letrec ((loop61 (lambda ()
 (if (< y SIZE)
 (begin (let ((x 0)
)
 (begin (call/cc (lambda (break64)
 (letrec ((loop63 (lambda ()
 (if (< x SIZE)
 (begin (if (>= (cond ((string? (list-ref b y)
)
 (string-ref (list-ref b y)
 x)
)
 ((hash-table? (list-ref b y)
)
 (hash-table-ref (list-ref b y)
 x)
)
 (else (list-ref (list-ref b y)
 x)
)
)
 2048)
 (begin (ret60 #t)
)
 (quote nil)
)
 (set! x (+ x 1)
)
 (loop63)
)
 (quote nil)
)
)
)
)
 (loop63)
)
)
)
 (set! y (+ y 1)
)
)
)
 (loop61)
)
 (quote nil)
)
)
)
)
 (loop61)
)
)
)
 (ret60 #f)
)
)
)
)
)
(define board (newBoard)
)
(define r (spawnTile board)
)
(set! board (hash-table-ref r "board")
)
(define full (hash-table-ref r "full")
)
(set! r (spawnTile board)
)
(set! board (hash-table-ref r "board")
)
(set! full (hash-table-ref r "full")
)
(define score 0)
(draw board score)
(call/cc (lambda (break66)
 (letrec ((loop65 (lambda ()
 (if #t (begin (display (to-str "Move: ")
)
 (newline)
 (define cmd (_input)
)
 (define moved #f)
 (if (or (string=? cmd "a")
 (string=? cmd "A")
)
 (begin (let ((m (moveLeft board score)
)
)
 (begin (set! board (hash-table-ref m "board")
)
 (set! score (hash-table-ref m "score")
)
 (set! moved (hash-table-ref m "moved")
)
)
)
)
 (quote nil)
)
 (if (or (string=? cmd "d")
 (string=? cmd "D")
)
 (begin (let ((m (moveRight board score)
)
)
 (begin (set! board (hash-table-ref m "board")
)
 (set! score (hash-table-ref m "score")
)
 (set! moved (hash-table-ref m "moved")
)
)
)
)
 (quote nil)
)
 (if (or (string=? cmd "w")
 (string=? cmd "W")
)
 (begin (let ((m (moveUp board score)
)
)
 (begin (set! board (hash-table-ref m "board")
)
 (set! score (hash-table-ref m "score")
)
 (set! moved (hash-table-ref m "moved")
)
)
)
)
 (quote nil)
)
 (if (or (string=? cmd "s")
 (string=? cmd "S")
)
 (begin (let ((m (moveDown board score)
)
)
 (begin (set! board (hash-table-ref m "board")
)
 (set! score (hash-table-ref m "score")
)
 (set! moved (hash-table-ref m "moved")
)
)
)
)
 (quote nil)
)
 (if (or (string=? cmd "q")
 (string=? cmd "Q")
)
 (begin (break66 (quote nil)
)
)
 (quote nil)
)
 (if moved (begin (let ((r2 (spawnTile board)
)
)
 (begin (set! board (hash-table-ref r2 "board")
)
 (set! full (hash-table-ref r2 "full")
)
 (if (and full (not (hasMoves board)
)
)
 (begin (draw board score)
 (display (to-str "Game Over")
)
 (newline)
 (break66 (quote nil)
)
)
 (quote nil)
)
)
)
)
 (quote nil)
)
 (draw board score)
 (if (has2048 board)
 (begin (display (to-str "You win!")
)
 (newline)
 (break66 (quote nil)
)
)
 (quote nil)
)
 (if (not (hasMoves board)
)
 (begin (display (to-str "Game Over")
)
 (newline)
 (break66 (quote nil)
)
)
 (quote nil)
)
 (loop65)
)
 (quote nil)
)
)
)
)
 (loop65)
)
)
)
