...
let rec encrypt polybius key plainText =
  ...
  temp := ((!temp + List.of_seq (Seq.take ((!r + 1) - !r) (Seq.drop !r (List.to_seq !labels)))) + List.of_seq (Seq.take ((!c + 1) - !c) (Seq.drop !c (List.to_seq !labels))));
  );
  c := (!c + 1);
    with Continue -> ()
  done with Break -> ());
  ...
  let colLen = ref ((List.length (!temp) / String.length (key))) in
  if ((List.length (!temp) mod String.length (key)) > 0) then (
  colLen := (!colLen + 1);
  );
  let table = ref ([]) in
  let rIdx = ref (0) in
  ...
  let idx = ref (0) in
  (try while (!idx < List.length (!temp)) do
    try
  let row = (!idx / String.length (key)) in
  let col = (!idx mod String.length (key)) in
  table := (List.mapi (fun __i __x -> if __i = row then (List.mapi (fun __i __x -> if __i = col then List.of_seq (Seq.take ((!idx + 1) - !idx) (Seq.drop !idx (List.to_seq !temp))) else __x) (List.nth (!table) (row))) else __x) (!table));
  idx := (!idx + 1);
    with Continue -> ()
  done with Break -> ());
