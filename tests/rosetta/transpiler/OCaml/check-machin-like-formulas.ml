...
let rec tanEval coef f =
  ...
  if (coef < 0) then (
  __ret := (Obj.magic ((Q.neg (tanEval (-(coef)) (f)))) : Q.t); raise Return
  );
  let ca = (coef / 2) in
  let cb = (coef - ca) in
  let a = tanEval (ca) (f) in
  let b = tanEval (cb) (f) in
...
let rec tans m =
  ...
  __ret := (Obj.magic (tanEval ((try List.assoc ("a") !t with Not_found -> (Obj.magic 0))) (br ((try List.assoc ("n") !t with Not_found -> (Obj.magic 0))) ((try List.assoc ("d") !t with Not_found -> (Obj.magic 0))))) : Q.t); raise Return
  );
