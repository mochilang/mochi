...
let rec rstripEmpty words =
  ...
  __ret := List.of_seq (Seq.take (!n - 0) (Seq.drop 0 (List.to_seq words))); raise Return
  with Return -> !__ret)
...
let rec pad word width align =
  ...
  if (align = 0) then (
  __ret := (word ^ spaces (diff)); raise Return
  );
  if (align = 2) then (
  __ret := (spaces (diff) ^ word); raise Return
  );
  let left = ref ((diff / 2)) in
  let right = ref ((diff - !left)) in
  __ret := ((spaces (!left) ^ word) ^ spaces (!right)); raise Return
  with Return -> !__ret)
...
let rec newFormatter text =
  ...
  __ret := [("text", Obj.repr !fmtLines); ("width", Obj.repr !width)]; raise Return
  with Return -> !__ret)
