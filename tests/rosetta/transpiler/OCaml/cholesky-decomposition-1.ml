...
let rec unpackSym m =
  ...
  let ele = (Obj.obj (List.assoc ("ele") m) : int) in
...
let rec printMat m =
  ...
  (try while (!j < List.length (List.nth (m) (!i))) do
    try
  line := (!line ^ (string_of_float (List.nth (List.nth (m) (!i)) (!j))));
...
let rec choleskyLower a =
  ...
  let ae = (Obj.obj (List.assoc ("ele") a) : int) in
  ...
  if (!i < !dr) then (
  let d = ref (((float_of_int (e) -. List.nth (!le) (!i)) /. List.nth (!le) (!dc))) in
  ...
  le := (List.mapi (fun __i __x -> if __i = !i then sqrtApprox ((float_of_int (e) -. List.nth (!le) (!i))) else __x) (!le));
  row := (!row + 1);
  dr := (!dr + !row);
  col := 1;
  dc := 0;
