(* Generated by Mochi transpiler v0.10.40 on 2025-07-25 05:42 UTC *)


let rec __show v =
  let open Obj in
  let rec list_aux o =
    if is_int o && (magic (obj o) : int) = 0 then "" else
      let hd = field o 0 in
      let tl = field o 1 in
      let rest = list_aux tl in
      if rest = "" then __show (obj hd) else __show (obj hd) ^ "; " ^ rest
  in
  let r = repr v in
  if is_int r then string_of_int (magic v) else
  match tag r with
  | 0 -> if size r = 0 then "[]" else "[" ^ list_aux r ^ "]"
  | 252 -> (magic v : string)
  | 253 -> string_of_float (magic v)
  | _ -> "<value>"


let _now_seed = ref 0
let _now_seeded = ref false

let _now () =
  if not !_now_seeded then (
    match Sys.getenv_opt "MOCHI_NOW_SEED" with
    | Some s -> (try _now_seed := int_of_string s; _now_seeded := true with _ -> ())
    | None -> ()
  );
  if !_now_seeded then (
    _now_seed := (!(_now_seed) * 1664525 + 1013904223) mod 2147483647;
    !_now_seed
  ) else int_of_float (Sys.time () *. 1000000000.)


let _mem () =
  int_of_float (Gc.allocated_bytes ())

exception Break
exception Continue

exception Return

let rec isPrime n =
  let __ret = ref false in
  (try
  if (n < 2) then (
  __ret := false; raise Return
  );
  if ((n mod 2) = 0) then (
  __ret := (n = 2); raise Return
  );
  if ((n mod 3) = 0) then (
  __ret := (n = 3); raise Return
  );
  let d = ref (5) in
  (try while ((!d * !d) <= n) do
    try
  if ((n mod !d) = 0) then (
  __ret := false; raise Return
  );
  d := (!d + 2);
  if ((n mod !d) = 0) then (
  __ret := false; raise Return
  );
  d := (!d + 4);
    with Continue -> ()
  done with Break -> ());
  __ret := true; raise Return
  with Return -> !__ret)

let rec sumDigits n =
  let __ret = ref 0 in
  (try
  let s = ref (0) in
  let x = ref (n) in
  (try while (!x > 0) do
    try
  s := (!s + (!x mod 10));
  x := (!x / 10);
    with Continue -> ()
  done with Break -> ());
  __ret := !s; raise Return
  with Return -> !__ret)

let rec pad n =
  let __ret = ref "" in
  (try
  if (n < 10) then (
  __ret := ("  " ^ (string_of_int (n))); raise Return
  );
  if (n < 100) then (
  __ret := (" " ^ (string_of_int (n))); raise Return
  );
  __ret := (string_of_int (n)); raise Return
  with Return -> !__ret)

let rec main () =
  let __ret = ref (Obj.magic 0) in
  (try
  print_endline ("Additive primes less than 500:");
  let count = ref (0) in
  let line = ref ("") in
  let lineCount = ref (0) in
  let i = ref (2) in
  (try while (!i < 500) do
    try
  if (isPrime (!i) && isPrime (sumDigits (!i))) then (
  count := (!count + 1);
  line := ((!line ^ pad (!i)) ^ "  ");
  lineCount := (!lineCount + 1);
  if (!lineCount = 10) then (
  print_endline (String.sub (!line) 0 ((String.length (!line) - 2) - 0));
  line := "";
  lineCount := 0;
  );
  );
  if (!i > 2) then (
  i := (!i + 2);
  ) else (
  i := (!i + 1);
  );
    with Continue -> ()
  done with Break -> ());
  if (!lineCount > 0) then (
  print_endline (String.sub (!line) 0 ((String.length (!line) - 2) - 0));
  );
  print_endline (((string_of_int (!count)) ^ " additive primes found."));
    !__ret
  with Return -> !__ret)

let () =
  let mem_start = _mem () in
  let start = _now () in
  ignore (main ());
  let finish = _now () in
  let mem_end = _mem () in
  let dur = (finish - start) / 1000 in
  let mem_bytes = max 0 (mem_end - mem_start) in
  Printf.printf "{\n  \"duration_us\": %d,\n  \"memory_bytes\": %d,\n  \"name\": \"%s\"\n}\n" dur mem_bytes "main";
