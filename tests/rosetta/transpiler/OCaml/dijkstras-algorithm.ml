...
let rec addEdge u v w =
  ...
  graph := ((u, []) :: List.remove_assoc (u) !graph);
  );
  graph := ((u, Obj.repr (((v, Obj.repr (w)) :: List.remove_assoc (v) (Obj.magic ((try List.assoc (u) !graph with Not_found -> [])) : (string * Obj.t) list)))) :: List.remove_assoc (u) (Obj.magic (!graph) : (string * Obj.t) list));
  if not ((List.mem_assoc v !graph)) then (
  graph := ((v, []) :: List.remove_assoc (v) !graph);
  );
    !__ret
  with Return -> !__ret)
...
and dijkstra source =
  ...
  (try List.iter (fun v ->
    try
  let alt = ((try List.assoc (!u) !dist with Not_found -> 0) + List.nth ((try List.assoc (!u) !graph with Not_found -> 0)) (v)) in
  if (alt < (try List.assoc (v) !dist with Not_found -> 0)) then (
  dist := ((v, alt) :: List.remove_assoc (v) !dist);
  prev := ((v, !u) :: List.remove_assoc (v) !prev);
  );
    with Continue -> ()) ((try List.assoc (!u) !graph with Not_found -> 0)) with Break -> ());
    with Continue -> ()
  done with Break -> ());
