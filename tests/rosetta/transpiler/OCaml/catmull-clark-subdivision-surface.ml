...
let rec divPoint p d =
  let __ret = ref ([] : (string * Obj.t) list) in
  (try
  let d = (Obj.magic d : float) in
  __ret := (Obj.magic (mulPoint (p) ((1.0 /. d))) : (string * Obj.t) list); raise Return
  with Return -> !__ret)

let rec centerPoint p1 p2 =
  let __ret = ref ([] : (string * Obj.t) list) in
  (try
  __ret := (Obj.magic (divPoint (sumPoint (p1) (p2)) (2.0)) : (string * Obj.t) list); raise Return
  with Return -> !__ret)

let rec getFacePoints points faces =
  let __ret = ref ([] : (string * Obj.t) list list) in
  (try
  let facePoints = ref (([] : Obj.t list)) in
  let i = ref (0) in
  ...
  fp := divPoint (!fp) (float_of_int (List.length (!face)));
  facePoints := (List.append !facePoints [(Obj.magic (!fp) : Obj.t)]);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
let rec getEdgesFaces points faces =
  let __ret = ref ([] : (string * Obj.t) list list) in
  (try
  ...
  let edgesCenters = ref (([] : Obj.t list)) in
  ...
  edgesCenters := (List.append !edgesCenters [(Obj.magic ([("pn1", Obj.repr List.nth (me) (0)); ("pn2", Obj.repr List.nth (me) (1)); ("fn1", Obj.repr List.nth (me) (2)); ("fn2", Obj.repr List.nth (me) (3)); ("cp", Obj.repr cp)]) : Obj.t)]);
    with Continue -> ()) !merged with Break -> ());
  __ret := (Obj.magic (!edgesCenters) : (string * Obj.t) list list); raise Return
  with Return -> !__ret)

let rec getEdgePoints points edgesFaces facePoints =
  let __ret = ref ([] : (string * Obj.t) list list) in
  (try
  let edgePoints = ref (([] : Obj.t list)) in
  let i = ref (0) in
  ...
  edgePoints := (List.append !edgePoints [(Obj.magic (centerPoint (!cp) (cfp)) : Obj.t)]);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
let rec getAvgFacePoints points faces facePoints =
  let __ret = ref ([] : (string * Obj.t) list list) in
  (try
  let numP = ref (List.length (points)) in
  let temp = ref (([] : Obj.t list)) in
  let i = ref (0) in
  (try while (!i < !numP) do
    try
  temp := (List.append !temp [(Obj.magic ([("p", Obj.repr [("x", Obj.repr 0.0); ("y", Obj.repr 0.0); ("z", Obj.repr 0.0)]); ("n", Obj.repr 0)]) : Obj.t)]);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  let fnum = ref (0) in
  ...
  let avg = ref (([] : Obj.t list)) in
  let j = ref (0) in
  (try while (!j < !numP) do
    try
  let tp = List.nth (!temp) (!j) in
  avg := (List.append !avg [(Obj.magic (divPoint ((Obj.obj (List.assoc ("p") tp) : (string * Obj.t) list)) (float_of_int ((Obj.obj (List.assoc ("n") tp) : int)))) : Obj.t)]);
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
...
let rec getAvgMidEdges points edgesFaces =
  let __ret = ref ([] : (string * Obj.t) list list) in
  (try
  let numP = ref (List.length (points)) in
  let temp = ref (([] : Obj.t list)) in
...
let rec getPointsFaces points faces =
...
let rec getNewPoints points pf afp ame =
  let __ret = ref ([] : (string * Obj.t) list list) in
  (try
  let newPts = ref (([] : Obj.t list)) in
  let i = ref (0) in
  (try while (!i < List.length (points)) do
    try
  ...
  let p1 = mulPoint (!old) (!m1) in
  let p2 = mulPoint (List.nth (afp) (!i)) (!m2) in
  let p3 = mulPoint (List.nth (ame) (!i)) (!m3) in
  newPts := (List.append !newPts [(Obj.magic (sumPoint (sumPoint (p1) (p2)) (p3)) : Obj.t)]);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
let rec cmcSubdiv points faces =
  let __ret = ref [] in
  (try
  ...
  (try List.iter (fun fp ->
    try
  newPoints := (List.append !newPoints [(Obj.magic (fp) : Obj.t)]);
  ...
  newPoints := (List.append !newPoints [(Obj.magic (List.nth (!edgePoints) (!idx)) : Obj.t)]);
  edgePointNums := ((key ((Obj.obj (List.assoc ("pn1") e) : int)) ((Obj.obj (List.assoc ("pn2") e) : int)), !nextPoint) :: List.remove_assoc key ((Obj.obj (List.assoc ("pn1") e) : int)) ((Obj.obj (List.assoc ("pn2") e) : int)) !edgePointNums);
  nextPoint := (!nextPoint + 1);
  idx := (!idx + 1);
    with Continue -> ()
  done with Break -> ());
  ...
  let ab = (try List.assoc (key (a) (b)) !edgePointNums with Not_found -> (Obj.magic 0)) in
  let da = (try List.assoc (key (d) (a)) !edgePointNums with Not_found -> (Obj.magic 0)) in
  let bc = (try List.assoc (key (b) (c)) !edgePointNums with Not_found -> (Obj.magic 0)) in
  let cd = (try List.assoc (key (c) (d)) !edgePointNums with Not_found -> (Obj.magic 0)) in
...
let rec formatPoint p =
  let __ret = ref "" in
  (try
  __ret := (Obj.magic ((((((("[" ^ fmt4 ((Obj.obj (List.assoc ("x") p) : float))) ^ " ") ^ fmt4 ((Obj.obj (List.assoc ("y") p) : float))) ^ " ") ^ fmt4 ((Obj.obj (List.assoc ("z") p) : float))) ^ "]")) : string); raise Return
  with Return -> !__ret)

let rec formatFace f =
  let __ret = ref "" in
  (try
  if (List.length (f) = 0) then (
  __ret := (Obj.magic ("[]") : string); raise Return
  );
  let s = ref (("[" ^ fmt2 (List.nth (f) (0)))) in
  let i = ref (1) in
  (try while (!i < List.length (f)) do
    try
  s := ((!s ^ " ") ^ fmt2 (List.nth (f) (!i)));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  s := (!s ^ "]");
