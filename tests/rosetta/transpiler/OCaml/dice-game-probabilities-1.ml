...
let rec throwDie nSides nDice s counts =
  ...
  (try while (!i <= nSides) do
    try
  ignore (throwDie (nSides) ((nDice - 1)) ((s + !i)) (counts));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
    !__ret
...
let rec beatingProbability nSides1 nDice1 nSides2 nDice2 =
  ...
  ignore (throwDie (nSides1) (nDice1) (0) (c1));
  ...
  ignore (throwDie (nSides2) (nDice2) (0) (c2));
  let p12 = (float_of_int (powInt (nSides1) (nDice1)) *. float_of_int (powInt (nSides2) (nDice2))) in
  let tot = ref (0.0) in
  i := 0;
  (try while (!i < len1) do
    try
  j := 0;
  let m = minInt (!i) (len2) in
  (try while (!j < m) do
    try
...
let () =
  ...
  let start = _now () in
  print_endline ((Printf.sprintf "%.16g" (beatingProbability (4) (9) (6) (6))));
  print_endline ((Printf.sprintf "%.16g" (beatingProbability (10) (5) (7) (6))));
  let finish = _now () in
  let mem_end = _mem () in
