...
let rec cartN lists =
  ...
  let rest = ref (cartN (List.map (fun v -> Obj.repr v) (List.of_seq (Seq.take (List.length (!a) - 1) (Seq.drop 1 (List.to_seq !a)))))) in
  (try List.iter (fun x ->
    try
  (try List.iter (fun p ->
    try
  out := (List.append !out [(Obj.magic (concat ([x]) (p)) : int list)]);
    with Continue -> ()) !rest with Break -> ());
    with Continue -> ()) List.nth (!a) (0) with Break -> ());
