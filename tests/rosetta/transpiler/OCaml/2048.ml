(* Generated by Mochi transpiler v0.10.37 on 2025-07-23 09:29 +0700 *)
...
let rec moveLeft b score =
  ...
  let new_ = (Obj.obj (List.assoc "row" r) : int) in
...
let rec moveRight b score =
  ...
  rev := (Obj.obj (List.assoc "row" r) : int);
  score := (!score + (Obj.obj (List.assoc "gain" r) : int));
  rev := reverseRow (!rev);
  let x = ref (0) in
  (try while (!x < size) do
...
let rec moveDown b score =
  ...
  let r = slideLeft (!col) in
  col := (Obj.obj (List.assoc "row" r) : int);
  score := (!score + (Obj.obj (List.assoc "gain" r) : int));
  col := reverseRow (!col);
  let y = ref (0) in
  (try while (!y < size) do
...
let full = ref ((Obj.obj (List.assoc "full" !r) : int))
let score = ref (0)
let () =
  board := (Obj.obj (List.assoc "board" !r) : int);
  r := spawnTile (!board);
  board := (Obj.obj (List.assoc "board" !r) : int);
  full := (Obj.obj (List.assoc "full" !r) : int);
  draw (!board) (!score);
  (try while true do
    try
  print_endline "Move: ";
  ...
  board := (Obj.obj (List.assoc "board" m) : int);
  score := (Obj.obj (List.assoc "score" m) : int);
  moved := (Obj.obj (List.assoc "moved" m) : int);
  );
  if ((cmd = "d") || (cmd = "D")) then (
  ...
  board := (Obj.obj (List.assoc "board" r2) : int);
  full := (Obj.obj (List.assoc "full" r2) : int);
