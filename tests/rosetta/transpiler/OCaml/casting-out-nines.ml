...
let rec subset base begin_ end_ =
  let __ret = ref ([] : string list) in
  (try
  let base = (Obj.magic base : int) in
  let b = ref (parseIntBase (begin_) (base)) in
  let e = ref (parseIntBase (end_) (base)) in
  let out = ref (([] : string list)) in
  let k = ref (!b) in
  (try while (!k <= !e) do
    try
  let ks = intToBase (!k) (base) in
  let mod_ = (base - 1) in
  let r1 = (parseIntBase (ks) (base) mod mod_) in
  let r2 = ((parseIntBase (ks) (base) * parseIntBase (ks) (base)) mod mod_) in
  if (r1 = r2) then (
...
let () =
  ...
  let s = ref (subset ((Obj.obj (List.assoc ("base") !tc) : int)) ((Obj.obj (List.assoc ("begin") !tc) : string)) ((Obj.obj (List.assoc ("end") !tc) : string))) in
