...
let rec multDigitalRoot n base =
...
let rec main () =
  ...
  let base = 10 in
  let size = 5 in
  print_endline (((((pad ("Number") (20) ^ " ") ^ pad ("MDR") (3)) ^ " ") ^ pad ("MP") (3)));
  ...
  let r = multDigitalRoot (n) (base) in
  print_endline (((((pad ((Z.to_string (n))) (20) ^ " ") ^ pad ((string_of_int ((Obj.obj (List.assoc ("mdr") r) : int)))) (3)) ^ " ") ^ pad ((string_of_int ((Obj.obj (List.assoc ("mp") r) : int)))) (3)));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  print_endline ("");
  ...
  (try while (!j < base) do
    try
  print_endline (((pad ((string_of_int (!j))) (3) ^ ": ") ^ __show (List.nth (!list) (!j))));
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
    !__ret
