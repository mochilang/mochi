// Generated by Mochi v0.10.39 on 2025-07-25 00:24:23 GMT+7
...
object Main {
  def bigTrim(_a: ArrayBuffer[Int]): ArrayBuffer[Int] = {
    ...
    var n: Int = (a).size
    while (n > 1 && a(n - 1) == 0) {
      a = (a.slice(0, n - 1)).asInstanceOf[ArrayBuffer[Int]]
  ...
  def bigMulBig(a: ArrayBuffer[Int], b: ArrayBuffer[Int]): ArrayBuffer[Int] = {
    ...
    while (i < (a).size) {
      ...
      while (j < (b).size) {
        ...
        res(idx) = prod.asInstanceOf[Int] % 10
  ...
  def factorSmall(x: Int, primes: ArrayBuffer[Int]): Map[String,Int] = {
    ...
    _br24.breakable {
      for (p <- primes) {
        if (p.toString.toDouble * p.toString.toDouble > n) {
          _br24.break()
        }
        var c: Int = 0
        while (n % p == 0) {
  ...
  def computeIP(n: Int, primes: ArrayBuffer[Int]): ArrayBuffer[Int] = {
    ...
    for (k <- fn.keys) {
      exps(k) = (exps.getOrElse(k, 0) - 6 * fn(k)).asInstanceOf[Int]
    }
    ...
    for (k <- ft2.keys) {
      exps(k) = (exps.getOrElse(k, 0) + ft2(k)).asInstanceOf[Int]
    }
    ...
    for (p <- keys) {
      var e: Int = exps(String.valueOf(p))
      var i: Int = 0
      while (i < e) {
