// Generated by Mochi v0.10.50 on 2025-07-30 21:40:27 GMT+7
...
def main(args: Array[String]): Unit = {
  {
    ...
    def randPerm(n: BigInt): ArrayBuffer[BigInt] = {
      ...
      var idx = n - BigInt(1)
      while (idx > BigInt(0)) {
        val j = _now() % (idx + BigInt(1))
        val tmp: BigInt = arr((idx).toInt)
        arr((idx).toInt) = arr((j).toInt)
        arr((j).toInt) = tmp
    ...
    def reduceToEven(xs: ArrayBuffer[BigInt]): ArrayBuffer[BigInt] = {
      ...
      var i: BigInt = BigInt(0)
      while (i < (arr).size) {
        val e: BigInt = arr((i).toInt)
        if (e % BigInt(2) == BigInt(0)) {
          arr((last).toInt) = e
          last = (last + BigInt(1)).asInstanceOf[BigInt]
        }
        i = (i + BigInt(1)).asInstanceOf[BigInt]
      }
