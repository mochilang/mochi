// Generated by Mochi v0.10.50 on 2025-07-30 21:22:45 GMT+7
...
def main(args: Array[String]): Unit = {
  {
    ...
    def randInt(seed: BigInt, n: BigInt): ArrayBuffer[BigInt] = {
      val next = (seed * BigInt(1664525) + BigInt(1013904223)) % BigInt(2147483647)
    ...
    def solve(board: ArrayBuffer[ArrayBuffer[BigInt]], target: ArrayBuffer[ArrayBuffer[BigInt]]): scala.collection.mutable.Map[String,ArrayBuffer[BigInt]] = {
      ...
      while (j < n) {
        ...
        val `val` = (diff + row((BigInt(0)).toInt)) % BigInt(2)
        col = col :+ `val`.asInstanceOf[BigInt]
        j = (j + BigInt(1)).asInstanceOf[BigInt]
      }
    ...
    def applySolution(b: ArrayBuffer[ArrayBuffer[BigInt]], sol: scala.collection.mutable.Map[String,ArrayBuffer[BigInt]]): ArrayBuffer[Any] = {
      ...
      var i: BigInt = BigInt(0)
      while (i < (sol.getOrElse("row", null.asInstanceOf[ArrayBuffer[BigInt]])).size) {
        if (sol.getOrElse("row", null.asInstanceOf[ArrayBuffer[BigInt]])((i).toInt) == BigInt(1)) {
          board = flipRow(board, i)
          moves = (moves + BigInt(1)).asInstanceOf[BigInt]
        }
        i = (i + BigInt(1)).asInstanceOf[BigInt]
      }
      var j: BigInt = BigInt(0)
      while (j < (sol.getOrElse("col", null.asInstanceOf[ArrayBuffer[BigInt]])).size) {
        if (sol.getOrElse("col", null.asInstanceOf[ArrayBuffer[BigInt]])((j).toInt) == BigInt(1)) {
    ...
    def printBoard(b: ArrayBuffer[ArrayBuffer[BigInt]]): Any = {
      ...
      while (i < (b).size) {
        ...
        var j: BigInt = BigInt(0)
        while (j < (b((i).toInt)).size) {
          line = (line + String.valueOf(b((i).toInt)((j).toInt))).toString
    ...
    def main(): Any = {
      ...
      seed = (res((BigInt(1)).toInt)).asInstanceOf[BigInt]
      ...
      _br15.breakable {
        while (true) {
          ...
          seed = (sres((BigInt(1)).toInt)).asInstanceOf[BigInt]
      ...
      val moves: BigInt = (ares((BigInt(1)).toInt)).asInstanceOf[BigInt]
      println("Solved:")
      printBoard(board)
