// Generated by Mochi v0.10.55 on 2025-08-02 17:56:52 GMT+7
...
def sortPoints(ps: ArrayBuffer[Point]): ArrayBuffer[Point] = {
  ...
  while (i < n) {
    ...
    while (j < n - BigInt(1).toInt) {
      ...
      val q: Point = arr((j + BigInt(1).toInt).toInt)
      if ((p.x > q.x || (p.x == q.x && p.y > q.y).asInstanceOf[Boolean]).asInstanceOf[Boolean]) {
        arr((j).toInt) = q
        arr((BigInt(j.toString) + BigInt(1)).toInt) = p
      }
      j = j + BigInt(1)
    }
    i = i + BigInt(1)
...
def convexHull(_ps: ArrayBuffer[Point]): ArrayBuffer[Point] = {
  ...
  for (pt <- ps) {
    while ((h).size >= BigInt(2) && ccw(h(((h).size - BigInt(2).toInt.toInt).toInt), h(((h).size - BigInt(1).toInt.toInt).toInt), pt) == false) {
      h = h.slice((BigInt(0)).toInt, ((h).size - BigInt(1).toInt).toInt)
    }
    h = h :+ pt
  }
  var i: BigInt = (ps).size - BigInt(2).toInt
  val t: BigInt = (h).size + BigInt(1).toInt
  while (i >= BigInt(0)) {
    val pt: Point = ps((i.toInt).toInt)
    while ((h).size >= t && ccw(h(((h).size - BigInt(2).toInt.toInt).toInt), h(((h).size - BigInt(1).toInt.toInt).toInt), pt) == false) {
      h = h.slice((BigInt(0)).toInt, ((h).size - BigInt(1).toInt).toInt)
    }
    h = h :+ pt
    i = i - BigInt(1)
  }
  return h.slice((BigInt(0)).toInt, ((h).size - BigInt(1).toInt).toInt)
}
...
def hullStr(h: ArrayBuffer[Point]): String = {
  ...
  var i: BigInt = BigInt(0)
  while (i < (h).size) {
    s = (s + pointStr(h((i.toInt).toInt))).toString
    if (i < (h).size - BigInt(1).toInt) {
      s = s + " "
    }
    i = i + BigInt(1)
  }
