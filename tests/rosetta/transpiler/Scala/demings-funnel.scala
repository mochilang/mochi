// Generated by Mochi v0.10.52 on 2025-08-01 19:19:48 GMT+7
...
def mean(fa: ArrayBuffer[Double]): Double = {
  ...
  var i: BigInt = BigInt(0)
  while (i < (fa).size) {
    sum = (sum + fa((i).toInt)).toString.toDouble
    i = i + BigInt(1)
  }
...
def experiment(label: String, r: (Double, Double) => Double): Any = {
  val rxs: ArrayBuffer[Double] = funnel(dxs.asInstanceOf[ArrayBuffer[Double]], r)
  val rys: ArrayBuffer[Double] = funnel(dys.asInstanceOf[ArrayBuffer[Double]], r)
  println(label + "  :      x        y")
