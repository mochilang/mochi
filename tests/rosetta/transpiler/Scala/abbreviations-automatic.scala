// Generated by Mochi v0.10.39 on 2025-07-25 09:41:59 GMT+7
...
object Main {
  ...
  def distinct(xs: ArrayBuffer[String]): ArrayBuffer[String] = {
    var m: Map[String,Boolean] = Map()
    var out: ArrayBuffer[String] = ArrayBuffer()
    var i: Int = 0
    while (i < (xs).size) {
      val x: String = xs(i)
      if ((!(m.contains(x)).asInstanceOf[Boolean]).asInstanceOf[Boolean]) {
        m(x) = true
        out = out :+ x
      }
      i = i + 1
  ...
  def abbrevLen(words: ArrayBuffer[String]): Int = {
    val size: Int = (words).size
    var l: Int = 1
    while (true) {
      var abbrs: ArrayBuffer[String] = ArrayBuffer()
      var i: Int = 0
      while (i < size) {
        abbrs = abbrs :+ takeRunes(words(i), l)
        i = i + 1
      }
  ...
  def main(): Unit = {
    ...
    var i: Int = 0
    while (i < (lines).size) {
      val words: ArrayBuffer[String] = fields(lines(i))
