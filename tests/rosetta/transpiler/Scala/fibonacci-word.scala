// Generated by Mochi v0.10.50 on 2025-07-30 21:34:35 GMT+7
...
def main(args: Array[String]): Unit = {
  {
    ...
    def entropy(s: String): Double = {
      ...
      while (i < (s).size) {
        ...
        if ((counts.contains(ch)).asInstanceOf[Boolean]) {
          counts.update(ch, counts.getOrElse(ch, BigInt(0)) + BigInt(1))
        } else {
      ...
      for (k <- counts.keys.toSeq.sorted) {
        val c: Double = (counts.getOrElse(k, BigInt(0))).toString.toDouble
        hm = hm + c * ((math.log(c)).toString.toDouble / (math.log(2.0)).toString.toDouble)
      }
      val l: Double = ((s).size).toString.toDouble
      return (math.log(l)).toString.toDouble / (math.log(2.0)).toString.toDouble - hm / l
    }
    ...
    def main(): Any = {
      ...
      while (n < BigInt(10)) {
        ...
        println(pad((String.valueOf(n)).toString, BigInt(3)) + pad((String.valueOf((s).size)).toString, BigInt(9)) + "  " + fmt(entropy(s)) + "  " + s)
        n = (n + BigInt(1)).asInstanceOf[BigInt]
      }
      while (n <= BigInt(37)) {
        val s: String = fibonacciWord(n)
        println(pad((String.valueOf(n)).toString, BigInt(3)) + pad((String.valueOf((s).size)).toString, BigInt(9)) + "  " + fmt(entropy(s)))
        n = (n + BigInt(1)).asInstanceOf[BigInt]
      }
    }
