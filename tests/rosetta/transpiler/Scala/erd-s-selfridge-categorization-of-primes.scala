// Generated by Mochi v0.10.52 on 2025-08-02 01:17:48 GMT+7
...
def primeFactors(n: BigInt, primes: ArrayBuffer[BigInt]): ArrayBuffer[BigInt] = {
  ...
  var num: BigInt = n
  var i: BigInt = BigInt(0)
  while (i < (primes).size && primes((i).toInt) * primes((i).toInt) <= num) {
    val p: BigInt = primes((i).toInt)
...
def main(): Any = {
  var primes: ArrayBuffer[BigInt] = generatePrimes(BigInt(1000))
  ...
  while (idx < BigInt(200)) {
    val p: BigInt = primes((idx).toInt)
    var c: BigInt = cat(p, primes)
    es((c - BigInt(1)).toInt) = es((c - BigInt(1)).toInt) :+ p
    idx = idx + BigInt(1)
  }
  var c: BigInt = BigInt(1)
  while (c <= BigInt(6)) {
    if ((es((c - BigInt(1)).toInt)).size > BigInt(0)) {
      println("Category " + String.valueOf(c) + ":")
      println(String.valueOf(es((c - BigInt(1)).toInt)))
      println("")
    }
    c = c + BigInt(1)
  }
  ...
  while (idx < BigInt(1000)) {
    ...
    es((cv - BigInt(1)).toInt) = es((cv - BigInt(1)).toInt) :+ p
    idx = idx + BigInt(1)
  }
  c = BigInt(1)
  while (c <= BigInt(12)) {
    val e: ArrayBuffer[BigInt] = es((c - BigInt(1)).toInt)
    if ((e).size > BigInt(0)) {
      val line: String = "Category " + padLeft(c, BigInt(2)) + ": First = " + padLeft(e((BigInt(0)).toInt), BigInt(7)) + "  Last = " + padLeft(e(((e).size - BigInt(1).toInt).toInt), BigInt(8)) + "  Count = " + padLeft(BigInt(((e).size).toInt), BigInt(6))
      println(line)
    }
    c = c + BigInt(1)
  }
