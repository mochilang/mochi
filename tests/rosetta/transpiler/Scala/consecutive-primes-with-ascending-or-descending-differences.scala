// Generated by Mochi v0.10.55 on 2025-08-02 17:50:27 GMT+7
...
def longestSeq(dir: String): Any = {
  ...
  while (i < (primes).size) {
    val d: BigInt = primes((i.toInt).toInt) - primes((i - BigInt(1).toInt).toInt)
    if (((dir == "ascending" && d <= pd).asInstanceOf[Boolean] || (dir == "descending" && d >= pd).asInstanceOf[Boolean]).asInstanceOf[Boolean]) {
      if ((currSeq).size > (longSeqs((BigInt(0).toInt).toInt)).size) {
      ...
      currSeq = ArrayBuffer(primes((i - BigInt(1).toInt).toInt), primes((i.toInt).toInt))
    } else {
      currSeq = currSeq :+ primes((i.toInt).toInt)
    }
    pd = d
    i = i + BigInt(1)
  }
  ...
  println("Longest run(s) of primes with " + dir + " differences is " + String.valueOf((longSeqs((BigInt(0).toInt).toInt)).size) + " :")
  for (ls <- longSeqs) {
    var diffs: ArrayBuffer[BigInt] = ArrayBuffer()
    var j: BigInt = BigInt(1)
    while (j < (ls).size) {
      diffs = diffs :+ ls((j.toInt).toInt) - ls((j - BigInt(1).toInt).toInt)
      j = j + BigInt(1)
    }
    var k: BigInt = BigInt(0)
    while (k < (ls).size - BigInt(1).toInt) {
      println(List(String.valueOf(ls((k.toInt).toInt)) + " (" + String.valueOf(diffs((k.toInt).toInt)) + ") ", false).mkString(" "))
      k = k + BigInt(1)
    }
    println(String.valueOf(ls(((ls).size - BigInt(1).toInt.toInt).toInt)))
  }
  println("")
}
