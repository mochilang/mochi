// Generated by Mochi v0.10.55 on 2025-08-02 17:54:07 GMT+7
...
val span: BigInt = rMax + BigInt(1) + rMax
...
def main(args: Array[String]): Unit = {
  {
    ...
    val _start = _now()
    while (y <= rMax) {
      var x: BigInt = BigInt(0) - rMax
      while (x <= rMax) {
        val r2: BigInt = x * x + y * y
    ...
    while (n < nPts) {
      var i: BigInt = _now() % (poss).size
      var x: BigInt = poss((i.toInt).toInt)((BigInt(0).toInt).toInt)
      val yy: BigInt = poss((i.toInt).toInt)((BigInt(1).toInt).toInt)
      var row: BigInt = yy + rMax
      val col: BigInt = (x + rMax) * BigInt(2)
    ...
    while (i2 < span) {
      ...
      while (j < span * BigInt(2)) {
        line = (line + rows((i2.toInt).toInt)((j.toInt).toInt)).toString
        j = j + BigInt(1)
      }
      println(line)
      i2 = i2 + BigInt(1)
