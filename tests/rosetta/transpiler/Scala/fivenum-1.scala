// Generated by Mochi v0.10.50 on 2025-07-30 21:21:47 GMT+7
...
def main(args: Array[String]): Unit = {
  {
    ...
    def median(s: ArrayBuffer[Double]): Double = {
      ...
      if (n % BigInt(2) == BigInt(1)) {
        return s((n / BigInt(2)).toInt)
      }
      return (s((n / BigInt(2) - BigInt(1)).toInt) + s((n / BigInt(2)).toInt)) / 2.0
    }
    def fivenum(xs: ArrayBuffer[Double]): ArrayBuffer[Double] = {
      var s: ArrayBuffer[Double] = sortFloat(xs)
      val n: Int = (s).size
      val q1: Double = s(((n - BigInt(1)) / BigInt(4)).toInt)
      val med: Double = median(s)
      val q3: Double = s((BigInt(3) * (n - BigInt(1)) / BigInt(4)).toInt)
      return (ArrayBuffer(s((BigInt(0)).toInt), q1, med, q3, s((n - BigInt(1)).toInt))).asInstanceOf[ArrayBuffer[Double]]
    }
