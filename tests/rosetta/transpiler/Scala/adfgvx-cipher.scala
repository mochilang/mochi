// Generated by Mochi v0.10.39 on 2025-07-24 21:22:59 GMT+7
...
def createPolybius(): ArrayBuffer[String] = {
  ...
  var i: Int = 0
  while (i < 6) {
    var row = shuffled.slice(i * 6, (i + 1) * 6)
    p = p :+ row
...
def createKey(n: Int): String = {
  ...
  var i: Int = 0
  while (i < n) {
    val idx: Int = _now() % (pool).size
    key = (key + pool.slice(idx, idx + 1)).asInstanceOf[String]
    pool = (pool.slice(0, idx)).toString + (pool.slice(idx + 1, (pool).size)).toString
    i = (i + 1).asInstanceOf[Int]
  }
  println("\nThe key is " + key)
  return key
...
def orderKey(key: String): ArrayBuffer[Int] = {
  ...
  while (m < n) {
    var j: Int = 0
    while (j < n - 1) {
      if ((pairs(j).asInstanceOf[ArrayBuffer[Any]](0)).toString > (pairs(j + 1).asInstanceOf[ArrayBuffer[Any]](0)).toString) {
        val tmp = pairs(j)
        pairs(j) = pairs(j + 1)
        pairs(j + 1) = tmp
      }
...
def encrypt(polybius: ArrayBuffer[String], key: String, plainText: String): String = {
    ...
    while (r < 6) {
      ...
      while (c < 6) {
        if (polybius(r).slice(c, c + 1) == plainText.slice(i, i + 1)) {
          temp = (temp + labels.slice(r, r + 1)).toString + (labels.slice(c, c + 1)).toString
        }
  ...
  while (idx < (temp).size) {
    ...
    val col = idx % (key).size
    table(row)(col) = (temp.slice(idx, idx + 1)).asInstanceOf[String]
...
def decrypt(polybius: ArrayBuffer[String], key: String, cipherText: String): String = {
  ...
  while (i <= (cipherText).size) {
    if ((i == (cipherText).size || cipherText.slice(i, i + 1) == " ").asInstanceOf[Boolean]) {
  ...
  while (idx < (temp).size) {
    ...
    plainText = (plainText + polybius(rIdx).slice(cIdx, cIdx + 1)).asInstanceOf[String]
