// Generated by Mochi v0.10.50 on 2025-07-31 01:07:56 GMT+7
...
def main(args: Array[String]): Unit = {
  {
    ...
    def floydWarshall(graph: ArrayBuffer[ArrayBuffer[BigInt]]): FWResult = {
      ...
      while (i < n) {
        ...
        while (j < n) {
          drow = drow :+ graph((i).toInt)((j).toInt)
          if ((graph((i).toInt)((j).toInt) < INF && i != j).asInstanceOf[Boolean]) {
            nrow = nrow :+ j
          } else {
    ...
    def pathStr(p: ArrayBuffer[BigInt]): String = {
      ...
      var i: BigInt = BigInt(0)
      while (i < (p).size) {
        s = s + String.valueOf(p((i).toInt) + BigInt(1))
        if (i < (p).size - BigInt(1)) {
          s = s + " -> "
        }
        i = i + BigInt(1)
      }
    ...
    while (i < n) {
      ...
      while (j < n) {
        if (i != j) {
          ...
          println(String.valueOf(i + BigInt(1)) + " -> " + String.valueOf(j + BigInt(1)) + "\t" + String.valueOf(res.dist((i).toInt)((j).toInt)) + "\t" + pathStr(p))
        }
        j = j + BigInt(1)
      }
      i = i + BigInt(1)
