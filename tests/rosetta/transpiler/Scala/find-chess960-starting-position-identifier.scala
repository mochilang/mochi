// Generated by Mochi v0.10.50 on 2025-07-30 21:20:21 GMT+7
...
def main(args: Array[String]): Unit = {
  {
    ...
    def strReplace(s: String, old: String, `new`: String): String = {
      ...
      var i: BigInt = BigInt(0)
      while (i < (s).size) {
        if ((((old).size > BigInt(0) && (i + (old).size).asInstanceOf[Int] <= (s).size).asInstanceOf[Boolean] && s.slice((i).toInt, (i + (old).size).toInt) == old).asInstanceOf[Boolean]) {
          res = (res + `new`).toString
          i = (i + (old).size).asInstanceOf[BigInt]
        } else {
    ...
    def g2l(pieces: String): String = {
      ...
      while (i < (pieces).size) {
        ...
        lets = (lets + g2lMap.getOrElse(ch, null.asInstanceOf[String])).toString
        i = (i + BigInt(1)).asInstanceOf[BigInt]
      }
      return lets
    }
    def spid(_pieces: String): BigInt = {
      ...
      val np: Double = (String.valueOf(n1)).toString.toDouble + (String.valueOf(n2)).toString.toDouble
      val N: BigInt = ntable.getOrElse(np, BigInt(0))
