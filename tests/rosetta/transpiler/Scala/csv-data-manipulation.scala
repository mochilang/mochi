// Generated by Mochi v0.10.55 on 2025-08-02 17:59:29 GMT+7
...
def join(xs: ArrayBuffer[String], sep: String): String = {
  ...
  while (i < (xs).size) {
    ...
    res = (res + xs((i.toInt).toInt)).toString
    i = i + BigInt(1)
  }
  return res
}
...
def main(args: Array[String]): Unit = {
  {
    ...
    val _start = _now()
    rows((BigInt(0)).toInt) = rows((BigInt(0).toInt).toInt) :+ "SUM"
    while (i < (rows).size) {
      var sum: BigInt = BigInt(0)
      for (s <- rows((i.toInt).toInt)) {
        sum = sum + _parseIntStr(s, BigInt(10))
      }
