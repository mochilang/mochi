// Generated by Mochi v0.10.55 on 2025-08-02 17:54:14 GMT+7
...
def cfPi(nTerms: BigInt): ArrayBuffer[scala.collection.mutable.Map[String,BigInt]] = {
  ...
  var n: BigInt = BigInt(0)
  while (n < nTerms) {
    val g: BigInt = BigInt(2) * n - BigInt(1)
    f = f :+ newTerm(BigInt(6), g * g)
    n = n + BigInt(1)
  }
  if (nTerms > BigInt(0)) {
...
def real(f: ArrayBuffer[scala.collection.mutable.Map[String,BigInt]]): Double = {
  var r: Double = 0.0
  var i: BigInt = (f).size - BigInt(1).toInt
  while (i > BigInt(0)) {
    r = (f((i.toInt).toInt).getOrElse("b", null).asInstanceOf[BigInt]).toString.toDouble / ((f((i.toInt).toInt).getOrElse("a", null).asInstanceOf[BigInt]).toString.toDouble + r)
    i = i - BigInt(1)
  }
  if ((f).size > BigInt(0)) {
    r = r + (f((BigInt(0).toInt).toInt).getOrElse("a", null).asInstanceOf[BigInt]).toString.toDouble
  }
  return r
}
