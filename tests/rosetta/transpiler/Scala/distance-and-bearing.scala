// Generated by Mochi v0.10.52 on 2025-08-01 19:21:28 GMT+7
...
def atan2Approx(y: Double, x: Double): Double = {
  if (x > 0.0) {
    val r: Double = atanApprox((y / x).toString.toDouble)
    return r
  }
  if (x < 0.0) {
    if (y >= 0.0) {
...
def distance(lat1: Double, lon1: Double, lat2: Double, lon2: Double): Double = {
  ...
  val dphi: Double = rad((lat2 - lat1).toString.toDouble)
  val dlambda: Double = rad((lon2 - lon1).toString.toDouble)
  val sdphi: Double = sinApprox(dphi / BigInt(2).toString.toDouble)
  val sdlambda: Double = sinApprox(dlambda / BigInt(2).toString.toDouble)
  val a: Double = (sdphi * sdphi).toString.toDouble + (cosApprox(phi1) * cosApprox(phi2) * sdlambda * sdlambda).toString.toDouble
  val c: Double = BigInt(2).toString.toDouble * atan2Approx(sqrtApprox(a), sqrtApprox(BigInt(1).toString.toDouble - a))
...
def bearing(lat1: Double, lon1: Double, lat2: Double, lon2: Double): Double = {
  val phi1: Double = rad(lat1)
  val phi2: Double = rad(lat2)
  val dl: Double = rad((lon2 - lon1).toString.toDouble)
  val y = sinApprox(dl) * cosApprox(phi2)
  val x: Double = (cosApprox(phi1) * sinApprox(phi2)).toString.toDouble - (sinApprox(phi1) * cosApprox(phi2) * cosApprox(dl)).toString.toDouble
  val br: Double = deg(atan2Approx(y, x))
  if (br < BigInt(0).toString.toDouble) {
    br = br + BigInt(360).toString.toDouble
  }
  return br
}
...
def round(x: Double, n: BigInt): Double = {
  ...
  return (floor(x * m + 0.5) / m).toString.toDouble
}
...
def sortByDistance(xs: ArrayBuffer[ArrayBuffer[Any]]): ArrayBuffer[ArrayBuffer[Any]] = {
  ...
  while (i < (arr).size) {
    var j: BigInt = i
    while (j > BigInt(0) && (arr((j - BigInt(1)).toInt)((BigInt(0)).toInt)).toString > (arr((j).toInt)((BigInt(0)).toInt)).toString) {
      val tmp: ArrayBuffer[Any] = arr((j - BigInt(1)).toInt)
      arr((j - BigInt(1)).toInt) = arr((j).toInt)
      arr((j).toInt) = tmp
      j = j - BigInt(1)
    }
    i = i + BigInt(1)
...
def main(): Any = {
  ...
  var results: ArrayBuffer[ArrayBuffer[Any]] = ArrayBuffer()
  for (ap <- airports) {
    val d: Double = distance(planeLat, planeLon, (ap.lat).toString.toDouble, (ap.lon).toString.toDouble)
    val b: Double = bearing(planeLat, planeLon, (ap.lat).toString.toDouble, (ap.lon).toString.toDouble)
    results = results :+ ArrayBuffer(d, b, ap)
  }
  ...
  while (i < (results).size) {
    val r: ArrayBuffer[Any] = results((i).toInt)
    val ap = r((BigInt(2)).toInt)
    val dist = r((BigInt(0)).toInt)
    val bear = r((BigInt(1)).toInt)
    val line: String = String.valueOf(round(dist.toString.toDouble, BigInt(1))) + "\t" + String.valueOf(round(bear.toString.toDouble, BigInt(0))) + "\t" + ap.icao + "\t" + ap.country + " " + ap.name
    println(line)
    i = i + BigInt(1)
  }
}
