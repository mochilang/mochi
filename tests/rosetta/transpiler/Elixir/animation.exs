# Code generated by Mochi transpiler 2025-07-26 17:24 +0700
defmodule Main do
  defp _now() do
    seeded = Process.get(:_now_seeded, false)
    seed = Process.get(:_now_seed, 0)
    if !seeded do
      case System.get_env("MOCHI_NOW_SEED") do
        nil -> :ok
        s ->
          case Integer.parse(s) do
            {v, ""} ->
              Process.put(:_now_seed, v)
              Process.put(:_now_seeded, true)
              seed = v
              seeded = true
            _ -> :ok
          end
      end
    end
    if seeded do
      seed = rem(seed * 1664525 + 1013904223, 2147483647)
      Process.put(:_now_seed, seed)
      seed
    else
      System.os_time(:nanosecond)
    end
  end
  defp _mem() do
    :erlang.memory(:total)
  end
  defp _lookup_host(host) do
    case :inet.gethostbyname(String.to_charlist(host)) do
      {:ok, {:hostent, _, _, _, _, addrs}} ->
        ips = Enum.map(addrs, &:inet.ntoa/1)
        [ips, nil]
      {:error, reason} ->
        [nil, reason]
    end
  end
  defp _slice(base, start, len) do
    cond do
      is_binary(base) -> String.slice(base, start, len)
      len == 1 -> Enum.at(base, start)
      true -> Enum.slice(base, start, len)
    end
  end
  def main() do
    msg = "Hello World! "
    shift = 0
    inc = 1
    clicks = 0
    frames = 0
    while_fun = fn while_fun, clicks, frames, inc, shift ->
      if clicks < 5 do
        line = ""
        i = 0
        while_fun_2 = fn while_fun_2, i, line ->
          if i < String.length(msg) do
            idx = :math.fmod((shift + i), String.length(msg))
            line = (line <> String.slice(msg, idx, idx + 1 - idx))
            i = i + 1
            while_fun_2.(while_fun_2, i, line)
          else
            {i, line}
          end
        end
        {i, line} = try do
            while_fun_2.(while_fun_2, i, line)
          catch
            :break -> {i, line}
          end

        IO.puts(line)
        shift = :math.fmod((shift + inc), String.length(msg))
        frames = frames + 1
        if :math.fmod(frames, String.length(msg)) == 0 do
          inc = String.length(msg) - inc
          clicks = clicks + 1
        end
        while_fun.(while_fun, clicks, frames, inc, shift)
      else
        {clicks, frames, inc, shift}
      end
    end
    {clicks, frames, inc, shift} = try do
        while_fun.(while_fun, clicks, frames, inc, shift)
      catch
        :break -> {clicks, frames, inc, shift}
      end

  end
end
Main.main()
