    ...
    try do
      ...
      while_fun_7 = fn while_fun_7, idx ->
        if idx < _len(primeSums) do
          ...
          j = Main.search(compSums, s)
          if j < _len(compSums) && Enum.at(compSums, j) == s do
            sumStr = String.pad_leading(Kernel.to_string(Main.commatize(s)), 10, " ")
            piStr = String.pad_leading(Kernel.to_string(Main.commatize(idx + 1)), 11, " ")
            ciStr = String.pad_leading(Kernel.to_string(Main.commatize(j + 1)), 15, " ")
