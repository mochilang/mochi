...
defmodule Main do
  ...
  def main() do
    try do
      ...
      mins = []
      i = 0
      while_fun_6 = fn while_fun_6, count, i, mins ->
        if i < (if is_binary(commands), do: String.length(commands), else: (if is_map(commands), do: map_size(commands), else: length(commands))) do
          count = 0
          j = 0
          cmd = Enum.at(commands, i)
          while_fun_7 = fn while_fun_7, count, j ->
            if j < (if is_binary(cmd), do: String.length(cmd), else: (if is_map(cmd), do: map_size(cmd), else: length(cmd))) do
          ...
          mins = (mins ++ [count])
          i = i + 1
          while_fun_6.(while_fun_6, count, i, mins)
        else
          {count, i, mins}
        end
      end
      ...
      while_fun_8 = fn while_fun_8, k, out1 ->
        if k < (if is_binary(words), do: String.length(words), else: (if is_map(words), do: map_size(words), else: length(words))) do
          out1 = ((out1 <> padRight(Enum.at(words, k), (if is_binary(Enum.at(results, k)), do: String.length(Enum.at(results, k)), else: (if is_map(Enum.at(results, k)), do: map_size(Enum.at(results, k)), else: length(Enum.at(results, k)))))) <> " ")
          k = k + 1
