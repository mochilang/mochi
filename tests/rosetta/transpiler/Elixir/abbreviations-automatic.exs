...
defmodule Main do
    ...
    try do
      ...
      while_fun_4 = fn while_fun_4, l ->
        if true do
          ...
          if (if is_binary(distinct(abbrs)), do: String.length(distinct(abbrs)), else: (if is_map(distinct(abbrs)), do: map_size(distinct(abbrs)), else: length(distinct(abbrs)))) == size do
            throw {:return, l}
          end
          l = l + 1
  ...
  def pad2(n) do
    try do
      s = to_string(n)
      if (if is_binary(s), do: String.length(s), else: (if is_map(s), do: map_size(s), else: length(s))) < 2 do
        throw {:return, (" " <> s)}
      end
      throw {:return, s}
    catch
