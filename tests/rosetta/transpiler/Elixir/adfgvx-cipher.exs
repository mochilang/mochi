  ...
  def createPolybius() do
    try do
      ...
      while_fun_5 = fn while_fun_5, i, p ->
        if i < 6 do
          row = Enum.slice(shuffled, i * 6, (i + 1) * 6 - i * 6)
          p = (p ++ [row])
          line = (Enum.at(labels, i) <> " | ")
          j = 0
          while_fun_6 = fn while_fun_6, j, line ->
            if j < 6 do
              line = ((line <> Enum.slice(row, j, j + 1 - j)) <> " ")
              j = j + 1
  ...
  def orderKey(key) do
    try do
      ...
      while_fun_11 = fn while_fun_11, i, res ->
        if i < n do
          res = (res ++ [Enum.at(Enum.at(pairs, i), 1)])
          i = i + 1
              ...
              while_fun_15 = fn while_fun_15, c, temp ->
                if c < 6 do
                  if String.slice(Enum.at(polybius, r), c, c + 1 - c) == String.slice(plainText, i, i + 1 - i) do
                    temp = ((temp <> Enum.slice(labels, r, r + 1 - r)) <> Enum.slice(labels, c, c + 1 - c))
                  end
                  c = c + 1
