...
defmodule Main do
  ...
  def copyInts(xs) do
    try do
      out = []
      {out} = Enum.reduce(xs, {out}, fn x, {out} ->
        out = (out ++ [x])
        {out}
      end)
      throw {:return, out}
      ...
      while_fun_9 = fn while_fun_9, done, edges, last, le, perim ->
        if le > 0 && (!done) do
          ...
          while_fun_10 = fn while_fun_10, done, edges, found, idx, last, le, perim ->
            if idx < le do
              e = Enum.at(edges, idx)
              {found, last, perim} = if e.a == last do
                perim = (perim ++ [e.b])
                last = e.b
                found = true
                {found, last, perim}
              else
                {found, last, perim} = if e.b == last do
                  perim = (perim ++ [e.a])
                  last = e.a
                  found = true
                  {found, last, perim}
  ...
  def main() do
    try do
      ...
      names = ["E", "F", "G", "H"]
      idx = 0
      while_fun_12 = fn while_fun_12, idx ->
        if idx < _len(faces) do
          per = Main.faceToPerim(Enum.at(faces, idx))
          if per == nil do
            IO.puts((("  " <> Enum.at(names, idx)) <> " => Invalid edge format"))
          else
            IO.puts(((("  " <> Enum.at(names, idx)) <> " => ") <> Main.listStr(per)))
          end
          idx = idx + 1
          while_fun_12.(while_fun_12, idx)
        else
          idx
        end
      end
