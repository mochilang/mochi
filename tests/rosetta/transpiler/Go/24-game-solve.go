...
// Generated by Mochi v0.10.37 on 2025-07-23 09:02:23 GMT+7
package main
...
func newNum(n int) map[string]any {
	return map[string]any{"op": OP_NUM, "value": map[string]any{"num": n, "denom": 1}}
}
...
func exprEval(x map[string]any) map[string]int {
	...
	var l map[string]int = exprEval(x["left"])
	var r map[string]int = exprEval(x["right"])
	if x["op"] == OP_ADD {
...
func exprString(x map[string]any) string {
	if x["op"] == OP_NUM {
		return fmt.Sprint(x["value"]["num"])
	}
	var ls string = exprString(x["left"])
	var rs string = exprString(x["right"])
	var opstr string = ""
	if x["op"] == OP_ADD {
		opstr = " + "
	} else {
