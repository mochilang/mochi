...
// Generated by Mochi v0.10.38 on 2025-07-23 23:46:30 GMT+7
package main
...
func orderKey(key string) []int {
	...
	i = 0
	for i < n {
		res = append(res, int(pairs[i].([]any)[1]))
		i = (i + 1)
	}
	return res
}
		...
		for r < 6 {
			...
			for c < 6 {
				if string([]rune(polybius[r])[c:(c+1)]) == string([]rune(plainText)[i:(i+1)]) {
					temp = func() []string {
						res := make([]string, len((temp + labels[r:(r+1)])))
						copy(res, (temp + labels[r:(r+1)]))
