//go:build ignore

// Generated by Mochi v0.10.40 on 2025-07-25 21:14:41 GMT+7
package main

import (
	"encoding/json"
	"fmt"
	"os"
	"runtime"
	"strconv"
	"time"
)

var seededNow bool
var nowSeed int64

func init() {
	if s := os.Getenv("MOCHI_NOW_SEED"); s != "" {
		if v, err := strconv.ParseInt(s, 10, 64); err == nil {
			nowSeed = v
			seededNow = true
		}
	}
}
func _now() int {
	if seededNow {
		nowSeed = (nowSeed*1664525 + 1013904223) % 2147483647
		return int(nowSeed)
	}
	return int(time.Now().UnixNano())
}

var b3Seg int

type Pixel struct {
	R int `json:"r"`
	G int `json:"g"`
	B int `json:"b"`
}

func pixelFromRgb(rgb int) Pixel {
	var r int = int(((rgb / 65536) % 256))
	_ = r
	var g int = int(((rgb / 256) % 256))
	_ = g
	var b int = int((rgb % 256))
	_ = b
	return Pixel{
		R: r,
		G: g,
		B: b,
	}
}

func newBitmap(cols int, rows int) map[string]any {
	var d [][]Pixel = [][]Pixel{}
	_ = d
	var y int = 0
	_ = y
	for y < rows {
		var row []Pixel = []Pixel{}
		_ = row
		var x int = 0
		_ = x
		for x < cols {
			row = append(row, Pixel{
				R: 0,
				G: 0,
				B: 0,
			})
			x = (x + 1)
		}
		d = append(d, row)
		y = (y + 1)
	}
	return map[string]any{"cols": cols, "rows": rows, "data": d}
}

func setPx(b map[string]any, x int, y int, p Pixel) {
	var cols int = b["cols"].(int)
	_ = cols
	var rows int = b["rows"].(int)
	_ = rows
	if (((x >= 0) && (x < cols)) && (y >= 0)) && (y < rows) {
		b["data"].([][]Pixel)[y][x] = p
	}
}

func fill(b map[string]any, p Pixel) {
	var cols int = b["cols"].(int)
	_ = cols
	var rows int = b["rows"].(int)
	_ = rows
	var y int = 0
	_ = y
	for y < rows {
		var x int = 0
		_ = x
		for x < cols {
			b["data"].([][]Pixel)[y][x] = p
			x = (x + 1)
		}
		y = (y + 1)
	}
}

func fillRgb(b map[string]any, rgb int) {
	fill(b, pixelFromRgb(rgb))
}

func line(b map[string]any, x0 int, y0 int, x1 int, y1 int, p Pixel) {
	var dx int = (x1 - x0)
	_ = dx
	if dx < 0 {
		dx = (0 - dx)
	}
	var dy int = (y1 - y0)
	_ = dy
	if dy < 0 {
		dy = (0 - dy)
	}
	var sx int = (0 - 1)
	_ = sx
	if x0 < x1 {
		sx = 1
	}
	var sy int = (0 - 1)
	_ = sy
	if y0 < y1 {
		sy = 1
	}
	var err int = (dx - dy)
	_ = err
	for {
		setPx(b, x0, y0, p)
		if (x0 == x1) && (y0 == y1) {
			break
		}
		var e2 int = (2 * err)
		_ = e2
		if e2 > (0 - dy) {
			err = (err - dy)
			x0 = (x0 + sx)
		}
		if e2 < dx {
			err = (err + dx)
			y0 = (y0 + sy)
		}
	}
}

func bezier3(b map[string]any, x1 int, y1 int, x2 int, y2 int, x3 int, y3 int, x4 int, y4 int, p Pixel) {
	var px []int = []int{}
	_ = px
	var py []int = []int{}
	_ = py
	var i int = 0
	_ = i
	for i <= b3Seg {
		px = append(px, 0)
		py = append(py, 0)
		i = (i + 1)
	}
	var fx1 float64 = float64(x1)
	_ = fx1
	var fy1 float64 = float64(y1)
	_ = fy1
	var fx2 float64 = float64(x2)
	_ = fx2
	var fy2 float64 = float64(y2)
	_ = fy2
	var fx3 float64 = float64(x3)
	_ = fx3
	var fy3 float64 = float64(y3)
	_ = fy3
	var fx4 float64 = float64(x4)
	_ = fx4
	var fy4 float64 = float64(y4)
	_ = fy4
	i = 0
	for i <= b3Seg {
		var d float64 = (float64(i) / float64(b3Seg))
		_ = d
		var a float64 = (1.0 - d)
		_ = a
		var bcoef float64 = (a * a)
		_ = bcoef
		var ccoef float64 = (d * d)
		_ = ccoef
		var a2 float64 = (a * bcoef)
		_ = a2
		var b2 float64 = ((3.0 * bcoef) * d)
		_ = b2
		var c2 float64 = ((3.0 * a) * ccoef)
		_ = c2
		var d2 float64 = (ccoef * d)
		_ = d2
		px[i] = int(((((a2 * fx1) + (b2 * fx2)) + (c2 * fx3)) + (d2 * fx4)))
		py[i] = int(((((a2 * fy1) + (b2 * fy2)) + (c2 * fy3)) + (d2 * fy4)))
		i = (i + 1)
	}
	var x0 int = px[0]
	_ = x0
	var y0 int = py[0]
	_ = y0
	i = 1
	for i <= b3Seg {
		var x int = px[i]
		_ = x
		var y int = py[i]
		_ = y
		line(b, x0, y0, x, y, p)
		x0 = x
		y0 = y
		i = (i + 1)
	}
}

var b map[string]any

func main() {
	func() {
		var ms runtime.MemStats
		runtime.ReadMemStats(&ms)
		startMem := ms.Alloc
		start := _now()
		b3Seg = 30
		b = newBitmap(400, 300)
		fillRgb(b, 16773055)
		bezier3(b, 20, 200, 700, 50, (0 - 300), 50, 380, 150, pixelFromRgb(4165615))
		runtime.ReadMemStats(&ms)
		endMem := ms.Alloc
		end := _now()
		data := map[string]any{"name": "main", "duration_us": (end - start) / 1000, "memory_bytes": endMem - startMem}
		out, _ := json.MarshalIndent(data, "", "  ")
		fmt.Println(string(out))
	}()
}
