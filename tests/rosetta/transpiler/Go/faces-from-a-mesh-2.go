...
// Generated by Mochi v0.10.52 on 2025-08-01 19:12:11 GMT+7
package main
...
func mochiMain() {
	...
	var faces [][]Edge = [][]Edge{e, f, g, h}
	_ = faces
	var names []string = []string{"E", "F", "G", "H"}
	_ = names
	var idx int = 0
	_ = idx
	for idx < len(faces) {
		...
		if _toFloat(per) == _toFloat(nil) {
			fmt.Println((("  " + names[idx]) + " => Invalid edge format"))
		} else {
			fmt.Println(((("  " + names[idx]) + " => ") + listStr(func(v any) []int {
				if v == nil {
					return nil
				}
				if vv, ok := v.([]int); ok {
