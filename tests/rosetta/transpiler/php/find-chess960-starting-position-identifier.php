...
$__start = _now();
  ...
  global $glyphs, $g2lMap, $names, $ntable;
  $i = 0;
  ...
  $i = 0;
  while ($i < strlen($s)) {
  if (strlen($old) > 0 && $i + strlen($old) <= strlen($s) && substr($s, $i, $i + strlen($old) - $i) == $old) {
  $res = $res . $new;
  $i = $i + strlen($old);
} else {
