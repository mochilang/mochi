...
bool amb(List<List<String>> wordsets, List<String> res, int idx) {
  ...
  var i = 0;
  while (i.toString().compareTo(wordsets[idx].length.toString()) < 0) {
...
void main() {
  ...
  List<String> res = [];
  var i = 0;
  while (i.toString().compareTo(wordset.length.toString()) < 0) {
  ...
  if (amb(wordset, res, 0)) {
    var out = '[' + res[0];
    var j = 1;
    while (j.toString().compareTo(res.length.toString()) < 0) {
      out = out + ' ' + res[j];
      j = (j as num) + 1;
    }
    out = out + ']';
