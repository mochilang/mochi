...
List<int> primeFactors(int n) {
  ...
  var p = 3;
  while ((p as num) * (p as num).toString().compareTo(x.toString()) <= 0) {
  ...
  if (x.toString().compareTo(1.toString()) > 0) {
...
void main() {
  List<int> vals = [];
  var n = -99;
  while (n.toString().compareTo(101.toString()) < 0) {
    vals = List.from(vals)..add(int.parse((D(double.parse(n)))));
    n = (n as num) + 1;
  }
  var i = 0;
  while (i.toString().compareTo(vals.length.toString()) < 0) {
    var line = '';
    var j = 0;
  ...
  var pow = 1;
  var m = 1;
  while (m.toString().compareTo(21.toString()) < 0) {
    pow = (pow as num) * 10;
    var exp = m.toString();
    if (exp.length < 2) {
      exp = exp + ' ';
