// Generated by Mochi compiler v0.10.26 on 2025-07-16T11:59:15Z
import 'dart:io';
import 'dart:convert';
...
List<any> shuffleChars(String s, int seed) {
  ...
  var i = 0;
  while (i < s.length) {
    chars = List.from(chars)..add(s.toString().substring(i, i + 1));
    i = i + 1;
  }
  var sd = seed;
...
List<any> bestShuffle(String s, int seed) {
  ...
  var i = 0;
  while (i < t.length) {
    arr = List.from(arr)..add(t.toString().substring(i, i + 1));
    i = i + 1;
  }
  i = 0;
  while (i < arr.length) {
    var j = 0;
    while (j < arr.length) {
      if (i != j && arr[i] != s.toString().substring(j, j + 1) && arr[j] != s.toString().substring(i, i + 1)) {
        var tmp = arr[i];
        arr[i] = arr[j];
        arr[j] = tmp;
        break;
  ...
  i = 0;
  while (i < arr.length) {
    if (arr[i] == s.toString().substring(i, i + 1)) {
