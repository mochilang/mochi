...
bool isUnique(int a, int b, int c, int d, int e, int f, int g) {
  ...
  while (i.toString().compareTo(nums.length.toString()) < 0) {
    var j = (i as num) + 1;
    while (j.toString().compareTo(nums.length.toString()) < 0) {
      if (nums[i] == nums[j]) {
        return false;
      }
      j = (j as num) + 1;
      ...
      for (var d = low; d < (high + 1); d++) {
        ...
        for (var e = low; e < (high + 1); e++) {
          for (var f = low; f < (high + 1); f++) {
            var a = (s as num) - b;
            var g = (s as num) - f;
            if (a.toString().compareTo(low.toString()) < 0 || a.toString().compareTo(high.toString()) > 0) {
              continue;
            }
            if (g.toString().compareTo(low.toString()) < 0 || g.toString().compareTo(high.toString()) > 0) {
              continue;
            }
            if (d + e + f != s) {
              continue;
