// Generated by Mochi compiler v0.10.28 on 2006-01-02T15:04:05Z
fn main() {
    let SIZE = 4;
    let newBoard = move || -> Vec<Vec<i32>> {
        let mut b: Vec<Vec<i32>> = vec![];
        let mut y = 0;
        while y < SIZE {
            let mut row: Vec<i32> = vec![];
            let mut x = 0;
            while x < SIZE {
                row = { let mut tmp = row.clone(); tmp.push(0); tmp };
                x += 1;
            }
            b = { let mut tmp = b.clone(); tmp.push(row); tmp };
            y += 1;
        }
        return b;
    };
    let spawnTile = move |b: &mut Vec<Vec<i32>>| -> std::collections::HashMap<&'static str, i32> {
        let mut empty: Vec<Vec<i32>> = vec![];
        let mut y = 0;
        while y < SIZE {
            let mut x = 0;
            while x < SIZE {
                if b[y as usize][x as usize] == 0 {
                    empty = { let mut tmp = empty.clone(); tmp.push(vec![x, y]); tmp };
                }
                x += 1;
            }
            y += 1;
        }
        if empty.len() as i32 == 0 {
            return { let mut m = std::collections::BTreeMap::new(); m.insert("board", b); m.insert("full", true); m };
        }
        let mut idx = now() % empty.len() as i32;
        let cell = empty[idx as usize];
        let mut val = 4;
        if now() % 10 < 9 {
            val = 2;
        }
        b[cell[1] as usize][cell[0] as usize] = val;
        return { let mut m = std::collections::BTreeMap::new(); m.insert("board", b); m.insert("full", empty.len() as i32 == 1); m };
    };
    fn pad(n: i32) -> &'static str {
        let mut s = n.to_string();
        let mut pad = 4 - s.len() as i32;
        let mut i = 0;
        let mut out = String::new();
        while i < pad {
            out += " ";
            i += 1;
        }
        return format!("{}{}", out, s);
    }
    let draw = move |b: Vec<Vec<i32>>, score: i32| -> () {
        println!("{}", vec![format!("{}", format!("{}{}", "Score: ", score.to_string()))].into_iter().filter(|s| !s.is_empty()).collect::<Vec<_>>().join(" ") );
        let mut y = 0;
        while y < SIZE {
            println!("+----+----+----+----+");
            let mut line = String::from("|");
            let mut x = 0;
            while x < SIZE {
                let mut v = b[y as usize][x as usize];
                if v == 0 {
                    line += "    |";
                } else {
                    line = format!("{}{}", format!("{}{}", line, pad(v)), "|");
                }
                x += 1;
            }
            println!("{}", vec![format!("{}", line)].into_iter().filter(|s| !s.is_empty()).collect::<Vec<_>>().join(" ") );
            y += 1;
        }
        println!("+----+----+----+----+");
        println!("W=Up S=Down A=Left D=Right Q=Quit");
    };
    fn reverseRow(r: Vec<i32>) -> Vec<i32> {
        let mut out: Vec<i32> = vec![];
        let mut i = r.len() as i32 - 1;
        while i >= 0 {
            out = { let mut tmp = out.clone(); tmp.push(r[i as usize]); tmp };
            i -= 1;
        }
        return out;
    }
    let slideLeft = move |row: Vec<i32>| -> std::collections::HashMap<&'static str, i32> {
        let mut xs: Vec<i32> = vec![];
        let mut i = 0;
        while i < row.len() as i32 {
            if row[i as usize] != 0 {
                xs = { let mut tmp = xs.clone(); tmp.push(row[i as usize]); tmp };
            }
            i += 1;
        }
        let mut res: Vec<i32> = vec![];
        let mut gain = 0;
        i = 0;
        while i < xs.len() as i32 {
            if i + 1 < xs.len() as i32 && xs[i as usize] == xs[i + 1 as usize] {
                let v = xs[i as usize] * 2;
                gain += v;
                res = { let mut tmp = res.clone(); tmp.push(v); tmp };
                i += 2;
            } else {
                res = { let mut tmp = res.clone(); tmp.push(xs[i as usize]); tmp };
                i += 1;
            }
        }
        while res.len() as i32 < SIZE {
            res = { let mut tmp = res.clone(); tmp.push(0); tmp };
        }
        return { let mut m = std::collections::BTreeMap::new(); m.insert("row", res); m.insert("gain", gain); m };
    };
    let moveLeft = move |b: &mut Vec<Vec<i32>>, score: &mut i32| -> std::collections::HashMap<&'static str, i32> {
        let mut moved = false;
        let mut y = 0;
        while y < SIZE {
            let r = slideLeft(b[y as usize]);
            let new = *r.get(&"row").unwrap();
            score += *r.get(&"gain").unwrap();
            let mut x = 0;
            while x < SIZE {
                if b[y as usize][x as usize] != new[x as usize] {
                    moved = true;
                }
                b[y as usize][x as usize] = new[x as usize];
                x += 1;
            }
            y += 1;
        }
        return { let mut m = std::collections::BTreeMap::new(); m.insert("board", b); m.insert("score", score); m.insert("moved", moved); m };
    };
    let moveRight = move |b: &mut Vec<Vec<i32>>, score: &mut i32| -> std::collections::HashMap<&'static str, i32> {
        let mut moved = false;
        let mut y = 0;
        while y < SIZE {
            let mut rev = reverseRow(b[y as usize]);
            let r = slideLeft(rev);
            rev = *r.get(&"row").unwrap();
            score += *r.get(&"gain").unwrap();
            rev = reverseRow(rev);
            let mut x = 0;
            while x < SIZE {
                if b[y as usize][x as usize] != rev[x as usize] {
                    moved = true;
                }
                b[y as usize][x as usize] = rev[x as usize];
                x += 1;
            }
            y += 1;
        }
        return { let mut m = std::collections::BTreeMap::new(); m.insert("board", b); m.insert("score", score); m.insert("moved", moved); m };
    };
    let getCol = move |b: Vec<Vec<i32>>, x: i32| -> Vec<i32> {
        let mut col: Vec<i32> = vec![];
        let mut y = 0;
        while y < SIZE {
            col = { let mut tmp = col.clone(); tmp.push(b[y as usize][x as usize]); tmp };
            y += 1;
        }
        return col;
    };
    let setCol = move |b: &mut Vec<Vec<i32>>, x: i32, col: Vec<i32>| -> () {
        let mut y = 0;
        while y < SIZE {
            b[y as usize][x as usize] = col[y as usize];
            y += 1;
        }
    };
    let moveUp = move |b: &mut Vec<Vec<i32>>, score: &mut i32| -> std::collections::HashMap<&'static str, i32> {
        let mut moved = false;
        let mut x = 0;
        while x < SIZE {
            let mut col = getCol(b, x);
            let r = slideLeft(col);
            let new = *r.get(&"row").unwrap();
            score += *r.get(&"gain").unwrap();
            let mut y = 0;
            while y < SIZE {
                if b[y as usize][x as usize] != new[y as usize] {
                    moved = true;
                }
                b[y as usize][x as usize] = new[y as usize];
                y += 1;
            }
            x += 1;
        }
        return { let mut m = std::collections::BTreeMap::new(); m.insert("board", b); m.insert("score", score); m.insert("moved", moved); m };
    };
    let moveDown = move |b: &mut Vec<Vec<i32>>, score: &mut i32| -> std::collections::HashMap<&'static str, i32> {
        let mut moved = false;
        let mut x = 0;
        while x < SIZE {
            let mut col = reverseRow(getCol(b, x));
            let r = slideLeft(col);
            col = *r.get(&"row").unwrap();
            score += *r.get(&"gain").unwrap();
            col = reverseRow(col);
            let mut y = 0;
            while y < SIZE {
                if b[y as usize][x as usize] != col[y as usize] {
                    moved = true;
                }
                b[y as usize][x as usize] = col[y as usize];
                y += 1;
            }
            x += 1;
        }
        return { let mut m = std::collections::BTreeMap::new(); m.insert("board", b); m.insert("score", score); m.insert("moved", moved); m };
    };
    let hasMoves = move |b: Vec<Vec<i32>>| -> bool {
        let mut y = 0;
        while y < SIZE {
            let mut x = 0;
            while x < SIZE {
                if b[y as usize][x as usize] == 0 {
                    return true;
                }
                if x + 1 < SIZE && b[y as usize][x as usize] == b[y as usize][x + 1 as usize] {
                    return true;
                }
                if y + 1 < SIZE && b[y as usize][x as usize] == b[y + 1 as usize][x as usize] {
                    return true;
                }
                x += 1;
            }
            y += 1;
        }
        return false;
    };
    let has2048 = move |b: Vec<Vec<i32>>| -> bool {
        let mut y = 0;
        while y < SIZE {
            let mut x = 0;
            while x < SIZE {
                if b[y as usize][x as usize] >= 2048 {
                    return true;
                }
                x += 1;
            }
            y += 1;
        }
        return false;
    };
    let mut board = newBoard();
    let mut r = spawnTile(&mut board);
    board = *r.get(&"board").unwrap();
    let mut full = *r.get(&"full").unwrap();
    r = spawnTile(&mut board);
    board = *r.get(&"board").unwrap();
    full = *r.get(&"full").unwrap();
    let mut score = 0;
    draw(board, score);
    while true {
        println!("Move: ");
        let cmd = input();
        let mut moved = false;
        if cmd == "a" || cmd == "A" {
            let m = moveLeft(&mut board, &mut score);
            board = *m.get(&"board").unwrap();
            score = *m.get(&"score").unwrap();
            moved = *m.get(&"moved").unwrap();
        }
        if cmd == "d" || cmd == "D" {
            let m = moveRight(&mut board, &mut score);
            board = *m.get(&"board").unwrap();
            score = *m.get(&"score").unwrap();
            moved = *m.get(&"moved").unwrap();
        }
        if cmd == "w" || cmd == "W" {
            let m = moveUp(&mut board, &mut score);
            board = *m.get(&"board").unwrap();
            score = *m.get(&"score").unwrap();
            moved = *m.get(&"moved").unwrap();
        }
        if cmd == "s" || cmd == "S" {
            let m = moveDown(&mut board, &mut score);
            board = *m.get(&"board").unwrap();
            score = *m.get(&"score").unwrap();
            moved = *m.get(&"moved").unwrap();
        }
        if cmd == "q" || cmd == "Q" {
            break;
        }
        if moved {
            let r2 = spawnTile(&mut board);
            board = *r2.get(&"board").unwrap();
            full = *r2.get(&"full").unwrap();
            if full && (!hasMoves(board)) != Default::default() {
                draw(board, score);
                println!("Game Over");
                break;
            }
        }
        draw(board, score);
        if has2048(board) {
            println!("You win!");
            break;
        }
        if !hasMoves(board) {
            println!("Game Over");
            break;
        }
    }
}
