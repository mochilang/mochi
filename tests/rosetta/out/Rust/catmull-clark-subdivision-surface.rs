...
fn main() {
    ...
    fn getFacePoints(points: Vec<Point>, faces: Vec<Vec<i32>>) -> Vec<Point> {
        ...
        while i < faces.len() as i32 {
            ...
            facePoints = append(facePoints, fp.clone());
            i += 1;
        }
        return facePoints;
    }
    ...
    fn getEdgesFaces(points: Vec<Point>, faces: Vec<Vec<i32>>) -> Vec<Edge> {
        ...
        while fnum < faces.len() as i32 {
            ...
            while pi < numP {
                ...
                edges = append(edges, vec![pn1, pn2, fnum]);
                pi += 1;
            }
            fnum += 1;
        }
        ...
        while idx < edges.len() as i32 {
            ...
            if idx < edges.len() as i32 - 1 {
                ...
                if e1[0] == e2[0] && e1[1] == e2[1] {
                    merged = append(merged, vec![e1[0], e1[1], e1[2], e2[2]]);
                    idx += 2;
                    continue;
                }
            }
            merged = append(merged, vec![e1[0], e1[1], e1[2], -1]);
            idx += 1;
        }
        ...
        for me in merged {
            ...
            edgesCenters = append(edgesCenters, Edge { pn1: me[0], pn2: me[1], fn1: me[2], fn2: me[3], cp: cp.clone() });
        }
        return edgesCenters;
    }
    fn getEdgePoints(points: Vec<Point>, edgesFaces: Vec<Edge>, facePoints: Vec<Point>) -> Vec<Point> {
        ...
        while i < edgesFaces.len() as i32 {
            ...
            edgePoints = append(edgePoints, centerPoint(&cp.clone(), &cfp.clone()));
            i += 1;
        }
        return edgePoints;
    }
    ...
    fn getNewPoints(points: Vec<Point>, pf: Vec<i32>, afp: Vec<Point>, ame: Vec<Point>) -> Vec<Point> {
        ...
        while i < points.len() as i32 {
            ...
            newPts = append(newPts, sumPoint(&sumPoint(&p1.clone(), &p2.clone()), &p3.clone()));
            i += 1;
        }
        return newPts;
    }
    ...
    fn cmcSubdiv(points: Vec<Point>, faces: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        for fp in facePoints {
            newPoints = append(newPoints, fp.clone());
        ...
        while idx < edgesFaces.len() as i32 {
            ...
            newPoints = append(newPoints, edgePoints[idx as usize]);
        ...
        while fnum < faces.len() as i32 {
            ...
            if oldFace.len() as i32 == 4 {
                ...
                newFaces = append(newFaces, vec![a, ab, fpnum, da]);
                newFaces = append(newFaces, vec![b, bc, fpnum, ab]);
                newFaces = append(newFaces, vec![c, cd, fpnum, bc]);
                newFaces = append(newFaces, vec![d, da, fpnum, cd]);
            }
            fnum += 1;
        }
