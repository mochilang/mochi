...
getFacePoints(Points, Faces) ->
    FacePoints0 = [],
    I2 = 0,
    (fun Loop2(Fp, FacePoints, I) -> case (I < length(Faces)) of true -> Face = lists:nth((I)+1, Faces), Fp0 = #{"__name" => "Point", x => 0, y => 0, z => 0}, {Fp1} = lists:foldl(fun(Idx, {Fp}) -> Fp1 = sumPoint(Fp, lists:nth((Idx)+1, Points)), {Fp1} end, {Fp}, Face), Fp2 = divPoint(Fp1, (length(Face))), FacePoints1 = FacePoints ++ [Fp2], I3 = (I + 1), Loop2(Fp2, FacePoints1, I3); _ -> ok end end(Fp, FacePoints0, I2)),
    FacePoints1.
...
getPointsFaces(Points, Faces) ->
    Pf0 = [],
    I10 = 0,
    (fun Loop15(Pf, I) -> case (I < length(Points)) of true -> Pf1 = Pf ++ [0], I11 = (I + 1), Loop15(I11, Pf1); _ -> ok end end(Pf0, I10)),
    Fnum4 = 0,
...
cmcSubdiv(Points, Faces) ->
    ...
    {NewPoints1, FacePointNums1, NextPoint1} = lists:foldl(fun(Fp, {NewPoints, FacePointNums, NextPoint}) -> NewPoints1 = NewPoints ++ [Fp], FacePointNums1 = FacePointNums ++ [NextPoint], NextPoint1 = (NextPoint + 1), {NewPoints1, FacePointNums1, NextPoint1} end, {NewPoints0, FacePointNums0, NextPoint0}, FacePoints),
    EdgePointNums0 = #{},
    Idx5 = 0,
    (fun Loop18(NewPoints, EdgePointNums, NextPoint, Idx) -> case (Idx < length(EdgesFaces)) of true -> E = lists:nth((Idx)+1, EdgesFaces), NewPoints2 = NewPoints ++ [lists:nth((Idx)+1, EdgePoints)], EdgePointNums1 = maps:put(key(mochi_get(pn1, E), mochi_get(pn2, E)), NextPoint, EdgePointNums), NextPoint2 = (NextPoint + 1), Idx6 = (Idx + 1), Loop18(EdgePointNums1, NextPoint2, Idx6, NewPoints2); _ -> ok end end(NewPoints1, EdgePointNums0, NextPoint1, Idx5)),
    NewFaces0 = [],
    Fnum6 = 0,
