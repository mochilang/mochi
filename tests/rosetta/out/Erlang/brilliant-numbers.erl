...
primesUpTo(N) ->
    Sieve0 = [],
    I0 = 0,
    (fun Loop0(I, Sieve) -> case (I =< N) of true -> Sieve1 = Sieve ++ [true], I1 = (I + 1), Loop0(Sieve1, I1); _ -> ok end end(I0, Sieve0)),
    P0 = 2,
...
sortInts(Xs) ->
    Res2 = [],
    Tmp0 = Xs,
    (fun Loop6(Tmp, Res) -> case (length(Tmp) > 0) of true -> Min0 = lists:nth((0)+1, Tmp), Idx0 = 0, I2 = 1, (fun Loop4(I) -> case (I < length(Tmp)) of true -> (case (mochi_get(I, Tmp) < Min0) of true -> Min1 = mochi_get(I, Tmp), Idx1 = I; _ -> ok end), I3 = (I + 1), Loop4(I3); _ -> ok end end(I2)), Res3 = Res ++ [Min1], Out0 = [], J0 = 0, (fun Loop5(J) -> case (J < length(Tmp)) of true -> (case (J /= Idx1) of true -> Out1 = Out0 ++ [mochi_get(J, Tmp)]; _ -> ok end), J1 = (J + 1), Loop5(J1); _ -> ok end end(J0)), Tmp1 = Out1, Loop6(Res3, Tmp1); _ -> ok end end(Tmp0, Res2)),
    Res3.

commatize(N) ->
    S0 = lists:flatten(io_lib:format("~p", [N])),
    I4 = (length(S0) - 3),
    (fun Loop7(S, I) -> case (I >= 1) of true -> S1 = lists:sublist(S, (0)+1, (I)-(0)) ++ "," ++ lists:sublist(S, (I)+1, (length(S))-(I)), I5 = (I - 3), Loop7(I5, S1); _ -> ok end end(S0, I4)),
    S1.
