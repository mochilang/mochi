...
sieve(Limit) ->
    Spf0 = [],
    I0 = 0,
    (fun Loop0(Spf, I) -> case (I =< Limit) of true -> Spf1 = Spf ++ [0], I1 = (I + 1), Loop0(I1, Spf1); _ -> ok end end(Spf0, I0)),
    I2 = 2,
...
arithmeticNumbers(Limit, Spf) ->
    Arr0 = [1],
    N0 = 3,
    (fun Loop9(N) -> case (length(Arr0) < Limit) of true -> (case (lists:nth((N)+1, Spf3) == N) of true -> Arr1 = Arr0 ++ [N]; _ -> X0 = N, Sigma0 = 1, Tau0 = 1, (fun Loop8(Sigma, Tau) -> case (X0 > 1) of true -> P0 = lists:nth((X0)+1, Spf3), (case (P0 == 0) of true -> P1 = X0; _ -> ok end), Cnt0 = 0, Power0 = P1, Sum0 = 1, (fun Loop7(Cnt, Sum, Power, X) -> case (rem(X, P1) == 0) of true -> X1 = (X / P1), Cnt1 = (Cnt + 1), Sum1 = (Sum + Power), Power1 = (Power * P1), Loop7(X1, Cnt1, Sum1, Power1); _ -> ok end end(Cnt0, Sum0, Power0, X0)), Sigma1 = (Sigma * Sum1), Tau1 = (Tau * ((Cnt1 + 1))), Loop8(Tau1, Sigma1); _ -> ok end end(Sigma0, Tau0)), (case (rem(Sigma1, Tau1) == 0) of true -> Arr2 = Arr1 ++ [N]; _ -> ok end) end), N1 = (N + 1), Loop9(N1); _ -> ok end end(N0)),
    Arr2.
