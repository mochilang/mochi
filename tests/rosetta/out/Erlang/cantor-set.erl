...
main(_) ->
    ...
    (fun Loop3(Stack) -> case (length(Stack) > 0) of true -> Frame0 = lists:nth(((length(Stack) - 1))+1, Stack), Stack1 = lists:sublist(Stack, (0)+1, ((length(Stack) - 1))-(0)), Start = mochi_get("start", Frame0), LenSeg = mochi_get("len", Frame0), Index = mochi_get("index", Frame0), Seg = ((LenSeg / 3)), (case (Seg == 0) of true -> throw(continue); _ -> ok end), I0 = Index, (fun Loop2(I) -> case (I < 5) of true -> J2 = (Start + Seg), (fun Loop1(J, Lines) -> case (J < (Start + (2 * Seg))) of true -> Lines2 = lists:sublist(Lines, I) ++ [setChar(lists:nth((I)+1, Lines), J, " ")] ++ lists:nthtail((I)+1, Lines), J3 = (J + 1), Loop1(Lines2, J3); _ -> ok end end(J2, Lines1)), I1 = (I + 1), Loop2(I1); _ -> ok end end(I0)), Stack2 = Stack ++ [#{"start" => Start, "len" => Seg, "index" => (Index + 1)}], Stack3 = Stack ++ [#{"start" => (Start + (Seg * 2)), "len" => Seg, "index" => (Index + 1)}], Loop3(Stack3); _ -> ok end end(Stack0)),
