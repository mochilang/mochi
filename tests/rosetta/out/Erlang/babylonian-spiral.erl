...
step(H, Nv, Dir) ->
    (fun Loop1(H, Nv) -> case ((length(H) == 0) orelse ((Nv * Nv) =< mochi_get("s", lists:nth((0)+1, H)))) of true -> H3 = push(H, #{"s" => (Nv * Nv), "a" => Nv, "b" => 0}), Nv0 = (Nv + 1), Loop1(H3, Nv0); _ -> ok end end(H2, Nv)),
...
positions(N) ->
    ...
    Nv1 = 1,
    I2 = 0,
    (fun Loop3(Pos, Dir, Heap, Nv, X, Y, I) -> case (I < N) of true -> Pos1 = Pos ++ [[X, Y]], St = step(Heap, Nv, Dir), Dir1 = mochi_get("d", St), Heap1 = mochi_get("heap", St), Nv2 = mochi_get("n", St), X1 = (X + lists:nth((0)+1, Dir)), Y1 = (Y + lists:nth((1)+1, Dir)), I3 = (I + 1), Loop3(Pos1, Dir1, Heap1, Nv2, X1, Y1, I3); _ -> ok end end(Pos0, Dir0, Heap0, Nv1, X0, Y0, I2)),
    Pos1.
...
main() ->
    ...
    Line0 = "",
    I4 = 0,
    (fun Loop5(Line, I) -> case (I < length(Pts)) of true -> P = mochi_get(I, Pts), S = pad("(" ++ lists:flatten(io_lib:format("~p", [lists:nth((0)+1, P)])) ++ ", " ++ lists:flatten(io_lib:format("~p", [lists:nth((1)+1, P)])) ++ ")", 10), Line1 = (Line + S), (case (rem(((I + 1)), 10) == 0) of true -> io:format("~p~n", [Line]), Line2 = ""; _ -> ok end), I5 = (I + 1), Loop5(Line2, I5); _ -> ok end end(Line0, I4)).
