...
main() ->
    ...
    (case (length(Groups) > 0) of true -> Out0 = "Groups: [", I0 = 0, (fun Loop0(I, Out) -> case (I < length(Groups)) of true -> Out1 = Out ++ "\"" ++ mochi_get(I, Groups) ++ "\"", (case (I < (length(Groups) - 1)) of true -> Out2 = Out ++ ", "; _ -> ok end), I1 = (I + 1), Loop0(Out2, I1); _ -> ok end end(I0, Out0)), Out3 = Out2 ++ "]", io:format("~p~n", [Out3]); _ -> io:format("~p~n", ["User not found"]) end).
