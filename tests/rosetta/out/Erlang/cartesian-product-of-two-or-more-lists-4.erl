...
llStr(Lst) ->
    S4 = "[",
    I2 = 0,
    (fun Loop1(I, S) -> case (I < length(Lst)) of true -> S5 = (S + listStr(lists:nth((I)+1, Lst))), (case (I < (length(Lst) - 1)) of true -> S6 = S ++ " "; _ -> ok end), I3 = (I + 1), Loop1(S6, I3); _ -> ok end end(I2, S4)),
    S7 = S6 ++ "]",
    S7.
...
cartN(Lists) ->
    ...
    lists:foreach(fun(P) -> {Out3, Row1} = lists:foldl(fun(X, {Row, Out}) -> Row0 = copy(P), Row1 = Row ++ [X], Out3 = Out ++ [Row], {Out3, Row1} end, {Row, Out2}, mochi_get(Last, A)) end, Left),
    Out3.
