...
poly(P) ->
    ...
    (fun Loop0(S, Coef, I) -> case (I > 0) of true -> S2 = S ++ "x", (case (I /= 1) of true -> S3 = S ++ "^" ++ lists:flatten(io_lib:format("~p", [I])); _ -> ok end), Coef1 = (((Coef * I) / (((P - I) + 1)))), D0 = Coef, (case (rem(((P - ((I - 1)))), 2) == 1) of true -> D1 = -D0; _ -> ok end), (case (D1 < 0) of true -> S4 = S ++ " - " ++ lists:flatten(io_lib:format("~p", [-D1])); _ -> S5 = S ++ " + " ++ lists:flatten(io_lib:format("~p", [D1])) end), I1 = (I - 1), Loop0(S5, Coef1, I1); _ -> ok end end(S1, Coef0, I0)),
...
aks(N) ->
    ...
    C0 = N,
    I2 = 1,
    (fun Loop1(C, I) -> case (I < N) of true -> (case (rem(C, N) /= 0) of true -> false; _ -> ok end), C1 = (((C * ((N - I))) / ((I + 1)))), I3 = (I + 1), Loop1(C1, I3); _ -> ok end end(C0, I2)),
    true.
