...
pileString(P) ->
    S0 = "",
    R0 = 0,
    (fun Loop3(R, S) -> case (R < 3) of true -> C0 = 0, (fun Loop2(S, C) -> case (C < 3) of true -> S1 = (S + lists:flatten(io_lib:format("~p", [lists:nth((((3 * R) + C))+1, P1)]))) ++ " ", C1 = (C + 1), Loop2(S1, C1); _ -> ok end end(S, C0)), S2 = S1 ++ "\n", R1 = (R + 1), Loop3(R1, S2); _ -> ok end end(R0, S0)),
    S2.
