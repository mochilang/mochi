...
parseIntBase(S, Base) ->
    N0 = 0,
    I0 = 0,
    (fun Loop1(N, I) -> case (I < length(S)) of true -> J0 = 0, V0 = 0, (fun Loop0(J) -> case (J < length(Digits)) of true -> (case (string:substr(Digits, (J)+1, ((J + 1))-(J)) == string:substr(S, (I)+1, ((I + 1))-(I))) of true -> V1 = J, throw(break); _ -> ok end), J1 = (J + 1), Loop0(J1); _ -> ok end end(J0)), N1 = ((N * Base) + V1), I1 = (I + 1), Loop1(N1, I1); _ -> ok end end(N0, I0)),
    N1.
...
intToBase(N, Base) ->
    ...
    Out0 = "",
    V2 = N1,
    (fun Loop2(Out, V) -> case (V > 0) of true -> D = rem(V, Base), Out1 = (lists:sublist(Digits, (D)+1, ((D + 1))-(D)) + Out), V3 = (V / Base), Loop2(Out1, V3); _ -> ok end end(Out0, V2)),
    Out1.
