...
bernoulli(N) ->
    A0 = [],
    M0 = 0,
    (fun Loop1(A, M) -> case (M =< N) of true -> A1 = A ++ [(1 / (((M + 1))))], J0 = M, (fun Loop0(A, J) -> case (J >= 1) of true -> A2 = lists:sublist(A, (J - 1)) ++ [((J) * ((lists:nth(((J - 1))+1, A) - lists:nth((J)+1, A))))] ++ lists:nthtail(((J - 1))+1, A), J1 = (J - 1), Loop0(A2, J1); _ -> ok end end(A, J0)), M1 = (M + 1), Loop1(A2, M1); _ -> ok end end(A0, M0)),
    lists:nth((0)+1, A2).
