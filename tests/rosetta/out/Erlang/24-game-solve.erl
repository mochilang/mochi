...
solve(Xs) ->
    ...
    I0 = 0,
    (fun Loop2(I) -> case (I < length(Xs)) of true -> J0 = (I + 1), (fun Loop1(Node, J) -> case (J < length(Xs)) of true -> Rest0 = [], K0 = 0, (fun Loop0(K) -> case (K < length(Xs)) of true -> (case ((K /= I) andalso (K /= J)) of true -> Rest1 = Rest0 ++ [lists:nth((K)+1, Xs)]; _ -> ok end), K1 = (K + 1), Loop0(K1); _ -> ok end end)(K0), A = lists:nth((I)+1, Xs), B = lists:nth((J)+1, Xs), lists:foreach(fun(Op) -> Node0 = #{"op" => Op, "left" => A, "right" => B}, (case solve(Rest1 ++ [Node]) of undefined -> ok; false -> ok; _ -> true end) end, [1, 2, 3, 4]), Node1 = #{"op" => 2, "left" => B, "right" => A}, (case solve(Rest1 ++ [Node]) of undefined -> ok; false -> ok; _ -> true end), Node2 = #{"op" => 4, "left" => B, "right" => A}, (case solve(Rest1 ++ [Node2]) of undefined -> ok; false -> ok; _ -> true end), J1 = (J + 1), Loop1(Node2, J1); _ -> ok end end)(Node, J0), I1 = (I + 1), Loop2(I1); _ -> ok end end)(I0),
    false.

main() ->
    Iter0 = 0,
    (fun Loop4(Iter) -> case (Iter < 10) of true -> Cards0 = [], I2 = 0, (fun Loop3(Cards, I) -> case (I < 4) of true -> N = ((rem(now(), ((9 - 1)))) + 1), Cards1 = Cards ++ [newNum(N)], io:format("~p~n", [" " ++ lists:flatten(io_lib:format("~p", [N]))]), I3 = (I + 1), Loop3(I3, Cards1); _ -> ok end end)(Cards0, I2), io:format("~p~n", [":  "]), (case not solve(Cards1) of true -> io:format("~p~n", ["No solution"]); _ -> ok end), Iter1 = (Iter + 1), Loop4(Iter1); _ -> ok end end)(Iter0).
