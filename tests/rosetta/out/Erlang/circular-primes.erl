...
main(_) ->
    ...
    Fq0 = [1, 2, 3, 5, 7, 9],
    Count0 = 0,
    (fun Loop4(Q, Fq) -> case true of true -> F = lists:nth((0)+1, Q), Fd = lists:nth((0)+1, Fq), (case (isPrime(F) andalso isCircular(F)) of true -> Circs1 = Circs0 ++ [F], Count1 = (Count0 + 1), (case (Count1 == 19) of true -> throw(break); _ -> ok end); _ -> ok end), Q1 = lists:sublist(Q, (1)+1, (length(Q))-(1)), Fq1 = lists:sublist(Fq, (1)+1, (length(Fq))-(1)), (case ((F /= 2) andalso (F /= 5)) of true -> {Q2, Fq2} = lists:foldl(fun(D, {Q, Fq}) -> Q2 = Q ++ [((F * 10) + D2)], Fq2 = Fq ++ [Fd], {Q2, Fq2} end, {Q, Fq}, Digits0); _ -> ok end), Loop4(Fq2, Q2); _ -> ok end end(Q0, Fq0)),
