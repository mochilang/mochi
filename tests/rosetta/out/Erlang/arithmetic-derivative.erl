...
repeat(Ch, N) ->
    S0 = "",
    I0 = 0,
    (fun Loop3(S, I) -> case (I < N) of true -> S1 = (S + Ch), I1 = (I + 1), Loop3(S1, I1); _ -> ok end end(S0, I0)),
    S1.
...
main() ->
    Vals0 = [],
    N0 = -99,
    (fun Loop5(Vals, N) -> case (N < 101) of true -> Vals1 = Vals ++ [(D(N))], N1 = (N + 1), Loop5(Vals1, N1); _ -> ok end end(Vals0, N0)),
    I2 = 0,
    (fun Loop7(I) -> case (I < length(Vals1)) of true -> Line0 = "", J0 = 0, (fun Loop6(Line, J) -> case (J < 10) of true -> Line1 = (Line + pad(lists:nth(((I + J))+1, Vals1))), (case (J < 9) of true -> Line2 = Line ++ " "; _ -> ok end), J1 = (J + 1), Loop6(J1, Line2); _ -> ok end end(Line0, J0)), io:format("~p~n", [Line2]), I3 = (I + 10), Loop7(I3); _ -> ok end end(I2)),
    Pow0 = 1,
    M0 = 1,
    (fun Loop8(Pow, M) -> case (M < 21) of true -> Pow1 = (Pow * 10), Exp0 = lists:flatten(io_lib:format("~p", [M])), (case (length(Exp0) < 2) of true -> Exp1 = Exp0 ++ " "; _ -> ok end), Res0 = (lists:flatten(io_lib:format("~p", [M])) + repeat("0", (M - 1))), io:format("~p~n", ["D(10^" ++ Exp1 ++ ") / 7 = " ++ Res0]), M1 = (M + 1), Loop8(Pow1, M1); _ -> ok end end(Pow0, M0)).
