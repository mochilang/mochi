...
cartN(Lists) ->
    ...
    N0 = length(A),
    Count0 = 0,
    (fun Loop4(Res, Count) -> case (Count < C1) of true -> Row0 = [], J0 = 0, (fun Loop2(Row, J) -> case (J < N0) of true -> Row1 = Row ++ [mochi_get(lists:nth((J)+1, Idx1), mochi_get(J, A))], J1 = (J + 1), Loop2(Row1, J1); _ -> ok end end(Row0, J0)), Res1 = Res ++ [Row1], K0 = (N0 - 1), (fun Loop3(K, Idx) -> case (K >= 0) of true -> Idx2 = lists:sublist(Idx, K) ++ [(lists:nth((K)+1, Idx) + 1)] ++ lists:nthtail((K)+1, Idx), (case (lists:nth((K)+1, Idx) < length(mochi_get(K, A))) of true -> throw(break); _ -> ok end), Idx3 = lists:sublist(Idx, K) ++ [0] ++ lists:nthtail((K)+1, Idx), K1 = (K - 1), Loop3(Idx3, K1); _ -> ok end end(K0, Idx1)), Count1 = (Count + 1), Loop4(Res1, Count1); _ -> ok end end(Res0, Count0)),
    Res1.
