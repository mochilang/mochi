...
FillRgb(B, C) ->
    Y0 = 0,
    P = pixelFromRgb(C),
    (fun Loop3(Y) -> case (Y < mochi_get(rows, B)) of true -> X0 = 0, (fun Loop2(Row, Px, B, X) -> case (X < mochi_get(cols, B)) of true -> Px0 = mochi_get(px, B), Row2 = mochi_get(Y, Px), Row3 = maps:put(X, P, Row), Px1 = maps:put(Y, Row, Px), B0 = B#{px => Px}, X1 = (X + 1), Loop2(X1, Row3, Px1, B0); _ -> ok end end(Row1, Px, B, X0)), Y1 = (Y + 1), Loop3(Y1); _ -> ok end end(Y0)).
