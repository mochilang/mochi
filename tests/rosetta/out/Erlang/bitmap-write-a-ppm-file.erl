...
newBitmap(W, H, C) ->
    Rows0 = [],
    Y0 = 0,
    (fun Loop1(Rows, Y) -> case (Y < H) of true -> Row0 = [], X0 = 0, (fun Loop0(X, Row) -> case (X < W) of true -> Row1 = Row ++ [C], X1 = (X + 1), Loop0(Row1, X1); _ -> ok end end(X0, Row0)), Rows1 = Rows ++ [Row1], Y1 = (Y + 1), Loop1(Y1, Rows1); _ -> ok end end(Rows0, Y0)),
...
writePPMP3(B) ->
    ...
    (fun Loop8(Out, Y) -> case (Y >= 0) of true -> Line0 = "", X4 = 0, (fun Loop7(Line, X) -> case (X < mochi_get(width, B0)) of true -> P = mochi_get(X, mochi_get(Y, mochi_get(pixels, B0))), Line1 = Line ++ "   " ++ pad(mochi_get(R, P), Numsize0) ++ " " ++ pad(mochi_get(G, P), Numsize0) ++ " " ++ pad(mochi_get(B, P), Numsize0), X5 = (X + 1), Loop7(Line1, X5); _ -> ok end end(Line0, X4)), Out1 = (Out + Line1), (case (Y > 0) of true -> Out2 = Out ++ "\n"; _ -> Out3 = Out ++ "\n" end), Y5 = (Y - 1), Loop8(Out3, Y5); _ -> ok end end(Out0, Y4)),
    Out3.
