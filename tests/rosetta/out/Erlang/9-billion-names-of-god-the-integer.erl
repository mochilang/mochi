#!/usr/bin/env escript
% Generated by Mochi compiler v0.10.26 on 2025-07-16T09:34:06Z
...
bigSub(A, B) ->
    ...
    Borrow0 = 0,
    I2 = 0,
    (fun Loop3(Res, I) -> case (I < (case A0 of #{items := It} -> length(It); _ -> length(A0) end)) of true -> Av2 = lists:nth((I)+1, A0), Bv2 = 0, (case (I < (case B of #{items := It} -> length(It); _ -> length(B) end)) of true -> Bv3 = lists:nth((I)+1, B); _ -> ok end), Diff0 = ((Av2 - Bv3) - Borrow0), (case (Diff0 < 0) of true -> Diff1 = (Diff0 + 10), Borrow1 = 1; _ -> Borrow2 = 0 end), Res3 = Res ++ [Diff1], I3 = (I + 1), Loop3(Res3, I3); _ -> ok end end(Res2, I2)),
    bigTrim(Res3).

bigToString(A) ->
    S1 = "",
    I4 = ((case A0 of #{items := It} -> length(It); _ -> length(A0) end) - 1),
    (fun Loop4(I, S) -> case (I >= 0) of true -> S2 = (S + lists:flatten(io_lib:format("~p", [lists:nth((I)+1, A0)]))), I5 = (I - 1), Loop4(S2, I5); _ -> ok end end(I4, S1)),
    S2.
...
cumu(N) ->
    Cache0 = [[bigFromInt(1)]],
    Y0 = 1,
    (fun Loop6(Cache, Y) -> case (Y =< N3) of true -> Row0 = [bigFromInt(0)], X0 = 1, (fun Loop5(X, Row) -> case (X =< Y) of true -> Val = mochi_get(minInt(X, (Y - X)), lists:nth(((Y - X))+1, Cache)), Row1 = Row ++ [bigAdd(lists:nth((((case Row of #{items := It} -> length(It); _ -> length(Row) end) - 1))+1, Row), Val)], X1 = (X + 1), Loop5(Row1, X1); _ -> ok end end(X0, Row0)), Cache1 = Cache ++ [Row1], Y1 = (Y + 1), Loop6(Cache1, Y1); _ -> ok end end(Cache0, Y0)),
...
main(_) ->
    io:format("~p~n", ["rows:"]),
    X2 = 1,
    (fun Loop9(X) -> case (X < 11) of true -> R = Row1(X), Line0 = "", I8 = 0, (fun Loop8(Line, I) -> case (I < (case R of #{items := It} -> length(It); _ -> length(R) end)) of true -> Line1 = Line ++ " " ++ mochi_get(I, R) ++ " ", I9 = (I + 1), Loop8(Line1, I9); _ -> ok end end(Line0, I8)), io:format("~p~n", [Line1]), X3 = (X + 1), Loop9(X3); _ -> ok end end(X2)),
    io:format("~p~n", [""]),
    io:format("~p~n", ["sums:"]),
