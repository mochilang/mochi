...
main :: IO ()
main = do
  mapM_ (\i -> fromMaybe () ((let doors = (doors ++ [False]) in Nothing))) [0 .. 100 - 1]
  mapM_ (\pass -> fromMaybe () ((let idx = (pass - 1) in whileLoop (\() -> (_asInt (idx) < 100)) (\() -> (let doors = _updateAt idx (const not (doors !! idx)) doors in (let idx = (_asInt (idx) + pass) in Nothing)))))) [1 .. 101 - 1]
