...
var
  ...
  fmtLines: specialize TArray<specialize TArray<string>>;
  i: integer;
  j: integer;
  lines: Variant;
  width: specialize TArray<integer>;
  wlen: Variant;
  words: Variant;
begin
...
function printFmt(f: specialize TFPGMap<string, any>; align: integer): integer;
var
  i: integer;
  j: integer;
  line: string;
...
begin
  ...
  begin
    ...
    while (j < Length(words)) do
    begin
      line := line + pad(words[j], specialize _indexList<integer>(width, j), align) + ' ';
      j := j + 1;
    end;
    writeln(line);
    i := i + 1;
