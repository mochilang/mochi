...
let distinct (xs) =
    ...
    while i < List.length xs do
        let x: obj = xs.[i]
        if not (m.ContainsKey x) then
            m.[x] <- true
            out <- out @ [x]
        i <- i + 1
    out
let abbrevLen (words) =
    let size: obj = List.length words
    let mutable l: int = 1
    while true do
        let mutable abbrs: obj = [||]
        let mutable i: int = 0
        while i < size do
...
let main () =
    ...
    while i < List.length lines do
        let words: obj = fields lines.[i]
        let l: obj = abbrevLen words
        printfn "%A" (pad2 l + "  " + lines.[i])
        i <- i + 1
printfn "%A" (main())
