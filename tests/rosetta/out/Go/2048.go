...
func spawnTile(b *[][]int) map[string]any {
	...
	if _count(_toAnySlice(empty)) == 0 {
		return map[string]any{"board": (*b), "full": true}
	}
	idx := (int64(_now()) % int64(_count(_toAnySlice(empty))))
	cell := empty[idx]
	val := 4
	...
	(*b)[cell[1]][cell[0]] = val
	return map[string]any{"board": (*b), "full": (_count(_toAnySlice(empty)) == 1)}
}
...
func slideLeft(row []int) map[string]any {
	...
	for i < _count(_toAnySlice(xs)) {
		if ((i + 1) < _count(_toAnySlice(xs))) && (xs[i] == xs[(i+1)]) {
			v := (xs[i] * 2)
			gain = (gain + v)
			res = append(res, v)
			i = (i + 2)
	...
	for _count(_toAnySlice(res)) < SIZE {
		res = append(res, 0)
	}
