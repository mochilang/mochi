...
func main() {
	...
	_ = Far
	td := [][]any{
		[]any{Point{X: 0.1234, Y: 0.9876}, Point{X: 0.8765, Y: 0.2345}, 2.0},
		[]any{Point{X: 0.0, Y: 2.0}, Point{X: 0.0, Y: 0.0}, 1.0},
		[]any{Point{X: 0.1234, Y: 0.9876}, Point{X: 0.1234, Y: 0.9876}, 2.0},
		[]any{Point{X: 0.1234, Y: 0.9876}, Point{X: 0.8765, Y: 0.2345}, 0.5},
		[]any{Point{X: 0.1234, Y: 0.9876}, Point{X: 0.1234, Y: 0.9876}, 0.0},
	}
	for _, tc := range td {
		p1 := tc[0]
		p2 := tc[1]
		r := tc[2]
		fmt.Println("p1:  {" + fmt.Sprint(_getField(p1, "x")) + " " + fmt.Sprint(_getField(p1, "y")) + "}")
		fmt.Println("p2:  {" + fmt.Sprint(_getField(p2, "x")) + " " + fmt.Sprint(_getField(p2, "y")) + "}")
		...
		if _equal(caseStr, "Points form a diameter and describe only a single circle.") || _equal(caseStr, "Coincident points with r==0.0 describe a degenerate circle.") {
			fmt.Println("   Center:  {" + fmt.Sprint(_getField(c1, "x")) + " " + fmt.Sprint(_getField(c1, "y")) + "}")
		} else {
			if _equal(caseStr, "Two circles.") {
				fmt.Println("   Center 1:  {" + fmt.Sprint(_getField(c1, "x")) + " " + fmt.Sprint(_getField(c1, "y")) + "}")
				fmt.Println("   Center 2:  {" + fmt.Sprint(_getField(c2, "x")) + " " + fmt.Sprint(_getField(c2, "y")) + "}")
			}
		}
		fmt.Println("")
	}
