...
quad_spline_list_t subdivideQuadSpline(quad_spline_t *q, double t) {
  double s = 1.0 - t;
  quad_spline_t u = (quad_spline_t){.c0 = q->c0, .c1 = 0.0, .c2 = 0.0};
  quad_spline_t v = (quad_spline_t){.c0 = 0.0, .c1 = 0.0, .c2 = q->c2};
  u.c1 = s * q->c0 + t * q->c1;
  v.c1 = s * q->c1 + t * q->c2;
  u.c2 = s * u.c1 + t * v.c1;
  v.c0 = u.c2;
...
quad_curve_list_t subdivideQuadCurve(quad_curve_t *q, double t) {
  ...
  quad_curve_t u = (quad_curve_t){.x = xs.data[0], .y = ys.data[0]};
  quad_curve_t v = (quad_curve_t){.x = xs.data[1], .y = ys.data[1]};
...
int testIntersect(quad_curve_t *p, quad_curve_t *q, double tol) {
  ...
  int exclude = 1;
  int accept = 0;
  point_t inter = (point_t){.x = 0.0, .y = 0.0};
...
point_list_t findIntersects(quad_curve_t *p, quad_curve_t *q, double tol,
  ...
  while (workload_len > 0) {
    ...
    int inter = (point_t)(res.data["intersect"]);
    if ((acc)) {
