...
const char *padInt(int n, int width) {
  char *tmp4 = _str(n);
  const char *s = tmp4;
...
const char *padFloat(double x, int width) {
  const char *s = fmtF(x);
  while (strlen(s) < width) {
...
double avgLen(int n) {
  ...
  int t = 0;
  while (t < tests) {
    list_int tmp7 = {0, NULL};
    list_int visited = tmp7;
    int i = 0;
    while (i < n) {
      int tmp8_data[1];
      list_int tmp8 = {1, tmp8_data};
      tmp8.data[0] = 0;
      list_int tmp9 = concat_list_int(visited, tmp8);
      visited = tmp9;
      i = i + 1;
    }
    int x = 0;
...
int main() {
  ...
  while (n <= nmax) {
    ...
    const char *line = padInt(n, 3) + "  " + padFloat(a, 9) + "  " +
                       padFloat(b, 12) + "  (" + padFloat(err, 6) + "%)";
    printf("%s\n", line);
    n = n + 1;
  }
}
