...
list_int parseDate(char *s) {
  ...
  int y = parseIntStr(tmp4);
  char *tmp5 = slice_string(s, 5, 7);
  int m = parseIntStr(tmp5);
  char *tmp6 = slice_string(s, 8, 10);
  int d = parseIntStr(tmp6);
...
int biorhythms(char *birth, char *target) {
  ...
  char *tmp14 = concat_string("Born ", birth);
  char *tmp15 = concat_string(tmp14, ", Target ");
  char *tmp16 = concat_string(tmp15, target);
  printf("%s\n", tmp16);
  char *tmp17 = _str(diff);
  char *tmp18 = concat_string("Day ", tmp17);
  printf("%s\n", tmp18);
  ...
  lengths.data[1] = 28;
  lengths.data[2] = 33;
  list_string tmp19 = list_string_create(2);
  tmp19.data[0] = "up and rising";
  tmp19.data[1] = "peak";
  list_string tmp20 = list_string_create(2);
  tmp20.data[0] = "up but falling";
  tmp20.data[1] = "transition";
  list_string tmp21 = list_string_create(2);
  tmp21.data[0] = "down and falling";
  tmp21.data[1] = "valley";
  list_string tmp22 = list_string_create(2);
  tmp22.data[0] = "down but rising";
  tmp22.data[1] = "transition";
  list_int quadrants_data[] = {tmp19, tmp20, tmp21, tmp22};
  list_list_int quadrants = {4, quadrants_data};
  int i = 0;
  while (i < 3) {
    int length = lengths.data[i];
    int cycle = cycles.data[i];
    ...
    if (percent > 95.0) {
      ...
      int nm = res.data[1];
      int nd = res.data[2];
      char *transition = dateString(ny, nm, nd);
      char *trend = quadrants.data[quadrant].data[0];
      char *next = quadrants.data[quadrant].data[1];
      char *tmp23 = _str(percent);
      char *pct = tmp23;
      if ((!contains(pct, "."))) {
        char *tmp24 = concat_string(pct, ".0");
        pct = tmp24;
      }
      char *tmp25 = concat_string(" ", pct);
      char *tmp26 = concat_string(tmp25, "% (");
      char *tmp27 = concat_string(tmp26, trend);
      char *tmp28 = concat_string(tmp27, ", next ");
      char *tmp29 = concat_string(tmp28, next);
      char *tmp30 = concat_string(tmp29, " ");
      char *tmp31 = concat_string(tmp30, transition);
      char *tmp32 = concat_string(tmp31, ")");
      description = tmp32;
    }
    char *tmp33 = _str(position);
    char *posStr = tmp33;
    if ((position < 10)) {
      char *tmp34 = concat_string(" ", posStr);
      posStr = tmp34;
    }
    printf("%d\n", cycle + posStr + " : " + description);
    i = i + 1;
  }
  printf("\n");
}
...
int mochi_main() {
  list_string tmp35 = list_string_create(2);
  tmp35.data[0] = "1943-03-09";
  tmp35.data[1] = "1972-07-11";
  list_string tmp36 = list_string_create(2);
  tmp36.data[0] = "1809-01-12";
  tmp36.data[1] = "1863-11-19";
  list_string tmp37 = list_string_create(2);
  tmp37.data[0] = "1809-02-12";
  tmp37.data[1] = "1863-11-19";
  list_int pairs_data[] = {tmp35, tmp36, tmp37};
  list_list_int pairs = {3, pairs_data};
  int idx = 0;
  while (idx < pairs.len) {
    list_string p = pairs.data[idx];
