...
list_list_int cartN(int lists) {
  ...
  if (a.len == 0) {
    ...
    list_int tmp9_data[] = {{0, tmp10}};
    list_list_int tmp9 = {1, tmp9_data};
    return tmp9;
  }
  list_list_int tmp11 = {0, NULL};
  list_list_int out = tmp11;
  int last = a.len - 1;
  list_int tmp12 = slice_list_int(a, 0, last);
  list_list_int left = cartN(tmp12);
  for (int tmp13 = 0; tmp13 < left.len; tmp13++) {
    list_int p = left.data[tmp13];
    for (int tmp14 = 0; tmp14 < a.data[last].len; tmp14++) {
      int x = a.data[last].data[tmp14];
      list_int row = copy(p);
      int tmp15_data[1];
