...
char *numberName(int n) {
  ...
  if (n < 100) {
    int t = tens.data[(int)((((double)n) / ((double)10)))];
    int s = n % 10;
    if (s > 0) {
      t = t + " " + small.data[s];
    }
...
char *pluralizeFirst(char *s, int n) {
  ...
  list_string w = fields(s);
  if (w.len > 0) {
    char *tmp8 = concat_string(w.data[0], "s");
    char *tmp9 = concat_string(w.data[0], "s");
    w.data[0] = tmp9;
  }
  return join(w, " ");
}
...
char *slur(char *p, int d) {
  ...
  while (i < strlen(p) - 1) {
    ...
    tmp10.data[0] = slice_string(p, i, i + 1);
    list_string tmp11 = concat_list_string(a, tmp10);
    a = tmp11;
    i = i + 1;
  }
  int idx = a.len - 1;
  ...
  char *s = slice_string(p, 0, 1);
  int k = 0;
  while (k < a.len) {
    char *tmp12 = concat_string(s, a.data[k]);
    s = tmp12;
    k = k + 1;
  }
  char *tmp13 = concat_string(s, slice_string(p, strlen(p) - 1, strlen(p)));
  s = tmp13;
  list_string w = fields(s);
  return join(w, " ");
}
...
int mochi_main() {
  int i = 99;
  while (i > 0) {
    char *tmp14 = concat_string(slur(numberName(i), i), " ");
    char *tmp15 = concat_string(tmp14, pluralizeFirst(slur("bottle of", i), i));
    char *tmp16 = concat_string(tmp15, " ");
    char *tmp17 = concat_string(tmp16, slur("beer on the wall", i));
    printf("%s\n", tmp17);
    char *tmp18 = concat_string(slur(numberName(i), i), " ");
    char *tmp19 = concat_string(tmp18, pluralizeFirst(slur("bottle of", i), i));
    char *tmp20 = concat_string(tmp19, " ");
    char *tmp21 = concat_string(tmp20, slur("beer", i));
    printf("%s\n", tmp21);
    char *tmp22 = concat_string(slur("take one", i), " ");
    char *tmp23 = concat_string(tmp22, slur("down", i));
    char *tmp24 = concat_string(tmp23, " ");
    char *tmp25 = concat_string(tmp24, slur("pass it around", i));
    printf("%s\n", tmp25);
    char *tmp26 = concat_string(slur(numberName(i - 1), i), " ");
    char *tmp27 =
        concat_string(tmp26, pluralizeFirst(slur("bottle of", i), i - 1));
    char *tmp28 = concat_string(tmp27, " ");
    char *tmp29 = concat_string(tmp28, slur("beer on the wall", i));
    printf("%s\n", tmp29);
    i = i - 1;
  }
}
