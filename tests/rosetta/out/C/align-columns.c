...
int rstripEmpty(list_string words) {
  int n = words.len;
...
const char *pad(char *word, int width, int align) {
...
int newFormatter(char *text) {
  int lines = split(text, "\n");
  list_int tmp8 = list_int_create(0);
  list_int fmtLines = tmp8;
  ...
  while (i < lines.len) {
    ...
    int words = rstripEmpty(split(lines.data[i], "$"));
    fmtLines = 0;
    int j = 0;
    while (j < words.len) {
  ...
  return (tmp_item_t){.text = fmtLines, .width = tmp10};
}

int printFmt(map_string_int f, int align) {
  list_int lines = (int)(f.data["text"]);
  list_int width = (list_int)(f.data["width"]);
  int i = 0;
  while (i < lines.len) {
    int words = lines.data[i];
    const char *line = "";
    int j = 0;
    while (j < words.len) {
...
int main() {
  ...
  char *tmp13 =
      concat_string(tmp12, "column$are$separated$by$at$least$one$space.\n");
  char *tmp14 = concat_string(
      tmp13, "Further,$allow$for$each$word$in$a$column$to$be$either$left\n");
  char *tmp15 = concat_string(
      tmp14,
      "justified,$right$justified,$or$center$justified$within$its$column.");
  const char *text = tmp15;
