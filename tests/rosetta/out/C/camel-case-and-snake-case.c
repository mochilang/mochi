...
const char *snakeToCamel(char *s) {
  s = trimSpace(s);
  const char *out = "";
  int up = 0;
  int i = 0;
...
const char *camelToSnake(char *s) {
  s = trimSpace(s);
  const char *out = "";
  int prevUnd = 0;
  int i = 0;
...
int main() {
  ...
  samples.data[8] = "ɛrgo rE tHis";
  samples.data[9] = "hurry-up-joe!";
  samples.data[10] = "c://my-docs/happy_Flag-Day/12.doc";
  samples.data[11] = " spaces ";
  int samples = samples;
  printf("=== To snake_case ===\n");
  for (int tmp18 = 0; tmp18 < samples.len; tmp18++) {
    char *s = samples.data[tmp18];
    printf("%s\n", padLeft(s, 34) + " => " + camelToSnake(s));
  }
  printf("\n");
  printf("=== To camelCase ===\n");
  for (int tmp19 = 0; tmp19 < samples.len; tmp19++) {
    char *s = samples.data[tmp19];
