// Generated by Mochi compiler v0.10.26 on 2025-07-16T11:37:23Z
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

typedef struct {
  int len;
  int *data;
} list_int;
static list_int list_int_create(int len) {
  list_int l;
  l.len = len;
  l.data = calloc(len, sizeof(int));
  if (!l.data && len > 0) {
    fprintf(stderr, "alloc failed\n");
    exit(1);
  }
  return l;
}
typedef struct {
  int len;
  list_int *data;
} list_list_int;
static list_list_int list_list_int_create(int len) {
  list_list_int l;
  l.len = len;
  l.data = calloc(len, sizeof(list_int));
  if (!l.data && len > 0) {
    fprintf(stderr, "alloc failed\n");
    exit(1);
  }
  return l;
}
static list_list_int concat_list_list_int(list_list_int a, list_list_int b) {
  list_list_int r = list_list_int_create(a.len + b.len);
  for (int i = 0; i < a.len; i++)
    r.data[i] = a.data[i];
  for (int i = 0; i < b.len; i++)
    r.data[a.len + i] = b.data[i];
  return r;
}
static char *concat_string(char *a, char *b) {
  size_t len1 = strlen(a);
  size_t len2 = strlen(b);
  char *buf = (char *)malloc(len1 + len2 + 1);
  memcpy(buf, a, len1);
  memcpy(buf + len1, b, len2);
  buf[len1 + len2] = '\0';
  return buf;
}
static char *_str(int v) {
  char *buf = (char *)malloc(32);
  sprintf(buf, "%d", v);
  return buf;
}
static list_int slice_list_int(list_int v, int start, int end) {
  if (start < 0)
    start += v.len;
  if (end < 0)
    end += v.len;
  if (start < 0)
    start = 0;
  if (end > v.len)
    end = v.len;
  if (start > end)
    start = end;
  list_int r = list_int_create(end - start);
  for (int i = 0; i < r.len; i++)
    r.data[i] = v.data[start + i];
  return r;
}
const char *listStr(list_int xs) {
  const char *s = "[";
  int i = 0;
  while (i < xs.len) {
    char *tmp1 = _str(xs.data[i]);
    char *tmp2 = concat_string(s, tmp1);
    s = tmp2;
    if (i < xs.len - 1) {
      char *tmp3 = concat_string(s, " ");
      s = tmp3;
    }
    i = i + 1;
  }
  char *tmp4 = concat_string(s, "]");
  s = tmp4;
  return s;
}

const char *llStr(list_list_int lst) {
  const char *s = "[";
  int i = 0;
  while (i < lst.len) {
    s = s + listStr(lst.data[i]);
    if (i < lst.len - 1) {
      char *tmp5 = concat_string(s, " ");
      s = tmp5;
    }
    i = i + 1;
  }
  char *tmp6 = concat_string(s, "]");
  s = tmp6;
  return s;
}

list_int concat(list_int a, list_int b) {
  list_int out = list_int_create(0);
  for (int tmp7 = 0; tmp7 < a.len; tmp7++) {
    int v = a.data[tmp7];
    list_int tmp8 = list_int_create(0);
    out = 0;
  }
  for (int tmp9 = 0; tmp9 < b.len; tmp9++) {
    int v = b.data[tmp9];
    list_int tmp10 = list_int_create(0);
    out = 0;
  }
  list_int tmp11 = list_int_create(0);
  return tmp11;
}

list_list_int cartN(int lists) {
  if ((lists == 0)) {
    list_int tmp12 = list_int_create(0);
    return tmp12;
  }
  list_list_int a = (list_list_int)(lists);
  if (a.len == 0) {
    list_int tmp14 = list_int_create(0);
    list_int tmp13_data[] = {{0, tmp14}};
    list_list_int tmp13 = {1, tmp13_data};
    return tmp13;
  }
  list_list_int tmp15 = {0, NULL};
  list_list_int out = tmp15;
  list_int tmp16 = slice_list_int(a, 1, a.len);
  list_list_int rest = cartN(tmp16);
  for (int tmp17 = 0; tmp17 < a.data[0].len; tmp17++) {
    int x = a.data[0].data[tmp17];
    for (int tmp18 = 0; tmp18 < rest.len; tmp18++) {
      list_int p = rest.data[tmp18];
      list_int tmp19 = list_int_create(0);
      list_int tmp20 = list_int_create(1);
      tmp20.data[0] = x;
      list_list_int tmp21 = list_list_int_create(1);
      tmp21.data[0] = concat(tmp20, p);
      list_list_int tmp22 = concat_list_list_int(tmp19, tmp21);
      out = tmp22;
    }
  }
  list_int tmp23 = list_int_create(0);
  return tmp23;
}

int main() {
  list_int tmp25 = list_int_create(2);
  tmp25.data[0] = 1;
  tmp25.data[1] = 2;
  list_int tmp26 = list_int_create(2);
  tmp26.data[0] = 3;
  tmp26.data[1] = 4;
  list_int tmp24_data[] = {{2, tmp25}, {2, tmp26}};
  list_list_int tmp24 = {2, tmp24_data};
  printf("%s\n", llStr(cartN(tmp24)));
  list_int tmp28 = list_int_create(2);
  tmp28.data[0] = 3;
  tmp28.data[1] = 4;
  list_int tmp29 = list_int_create(2);
  tmp29.data[0] = 1;
  tmp29.data[1] = 2;
  list_int tmp27_data[] = {{2, tmp28}, {2, tmp29}};
  list_list_int tmp27 = {2, tmp27_data};
  printf("%s\n", llStr(cartN(tmp27)));
  list_int tmp31 = list_int_create(2);
  tmp31.data[0] = 1;
  tmp31.data[1] = 2;
  list_int tmp32 = list_int_create(0);
  list_int tmp30_data[] = {{2, tmp31}, {0, tmp32}};
  list_list_int tmp30 = {2, tmp30_data};
  printf("%s\n", llStr(cartN(tmp30)));
  list_int tmp34 = list_int_create(0);
  list_int tmp35 = list_int_create(2);
  tmp35.data[0] = 1;
  tmp35.data[1] = 2;
  list_int tmp33_data[] = {{0, tmp34}, {2, tmp35}};
  list_list_int tmp33 = {2, tmp33_data};
  printf("%s\n", llStr(cartN(tmp33)));
  printf("\n");
  printf("[\n");
  list_int tmp37 = list_int_create(2);
  tmp37.data[0] = 1776;
  tmp37.data[1] = 1789;
  list_int tmp38 = list_int_create(2);
  tmp38.data[0] = 7;
  tmp38.data[1] = 12;
  list_int tmp39 = list_int_create(3);
  tmp39.data[0] = 4;
  tmp39.data[1] = 14;
  tmp39.data[2] = 23;
  list_int tmp40 = list_int_create(2);
  tmp40.data[0] = 0;
  tmp40.data[1] = 1;
  list_int tmp36_data[] = {{2, tmp37}, {2, tmp38}, {3, tmp39}, {2, tmp40}};
  list_list_int tmp36 = {4, tmp36_data};
  for (int tmp41 = 0; tmp41 < cartN(tmp36).len; tmp41++) {
    list_int p = cartN(tmp36).data[tmp41];
    printf("%s\n", " " + listStr(p));
  }
  printf("]\n");
  list_int tmp43 = list_int_create(3);
  tmp43.data[0] = 1;
  tmp43.data[1] = 2;
  tmp43.data[2] = 3;
  list_int tmp44 = list_int_create(1);
  tmp44.data[0] = 30;
  list_int tmp45 = list_int_create(2);
  tmp45.data[0] = 500;
  tmp45.data[1] = 100;
  list_int tmp42_data[] = {{3, tmp43}, {1, tmp44}, {2, tmp45}};
  list_list_int tmp42 = {3, tmp42_data};
  printf("%s\n", llStr(cartN(tmp42)));
  list_int tmp47 = list_int_create(3);
  tmp47.data[0] = 1;
  tmp47.data[1] = 2;
  tmp47.data[2] = 3;
  list_int tmp48 = list_int_create(0);
  list_int tmp49 = list_int_create(2);
  tmp49.data[0] = 500;
  tmp49.data[1] = 100;
  list_int tmp46_data[] = {{3, tmp47}, {0, tmp48}, {2, tmp49}};
  list_list_int tmp46 = {3, tmp46_data};
  printf("%s\n", llStr(cartN(tmp46)));
  printf("\n");
  printf("%s\n", llStr(cartN(0)));
  list_int tmp50 = list_int_create(0);
  printf("%s\n", llStr(cartN(tmp50)));
}

int main() {
  main();
  return 0;
}
