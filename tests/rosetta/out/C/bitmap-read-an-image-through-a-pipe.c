...
int parsePpm(char *data) {
  ...
  return (tmp_item1_t){.magic = magic, .w = w, .h = h, .max = maxv, .px = tmp9};
}

int main() {
  int img = parsePpm(ppmData);
  char *tmp10 = _str(img.w);
  char *tmp11 = concat_string("width=", tmp10);
  char *tmp12 = concat_string(tmp11, " height=");
  char *tmp13 = _str(img.h);
