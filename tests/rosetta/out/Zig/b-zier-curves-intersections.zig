    ...
    while ((workload).len > 0) {
        ...
        if (acc) {
            if (!seemsToBeDuplicate(inters, &inter, spacing)) {
                inters = blk1: { var _tmp1 = std.ArrayList(Point).init(std.heap.page_allocator); defer _tmp1.deinit(); _tmp1.appendSlice(inters) catch |err| handleError(err); _tmp1.append(inter) catch |err| handleError(err); break :blk1 _tmp1.items; };
            }
        } else         if (!excl) {
...
fn user_main() void {
    ...
    while (i < (inters).len) {
        ...
        std.debug.print("{s}\n", .{_concat_string(_concat_string(_concat_string(_concat_string("(", std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{pt.x}) catch |err| handleError(err)), ", "), std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{pt.y}) catch |err| handleError(err)), ")")});
        i = (i + 1);
    }
}
