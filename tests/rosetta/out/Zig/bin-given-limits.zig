...
fn getBins(limits: []const i32, data: []const i32) []const i32 {
    var n = (limits).len; // i32
    var bins = std.ArrayList(i32).init(std.heap.page_allocator);
    var i = 0; // i32
    while (i < (n + 1)) {
        bins = blk0: { var _tmp0 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(bins) catch |err| handleError(err); _tmp0.append(0) catch |err| handleError(err); break :blk0 _tmp0.items; };
        i = (i + 1);
    }
    var j = 0; // i32
    while (j < (data).len) {
