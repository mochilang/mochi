...
fn deranged(a: []const u8, b: []const u8) bool {
    if (@as(i32, @intCast((a).len)) != @as(i32, @intCast((b).len))) {
        return false;
    }
    var i = 0; // i32
...
fn user_main() void {
    ...
    var w2 = ""; // []const u8
    for (words) |w| {
        if (@as(i32, @intCast((w).len)) <= bestLen) {
            continue;
        }
        ...
        for (m[k]) |c| {
            if (deranged(w, c)) {
                bestLen = @as(i32, @intCast((w).len));
                w1 = c;
                w2 = w;
                break;
            }
        }
        _ = m.put(k, blk1: { var _tmp1 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp1.deinit(); _tmp1.appendSlice(m[k]) catch |err| handleError(err); _tmp1.append(w) catch |err| handleError(err); const res = _tmp1.toOwnedSlice() catch |err| handleError(err); break :blk1 res; }) catch |err| handleError(err);
    }
