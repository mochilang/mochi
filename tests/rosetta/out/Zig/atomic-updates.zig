...
fn main() void {
    ...
    for (0 .. nBuckets) |i| {
        buckets = blk0: { var _tmp0 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(buckets) catch |err| handleError(err); _tmp0.append(0) catch |err| handleError(err); break :blk0 _tmp0.items; };
    }
    var i = nBuckets; // i32
    ...
    while (t < 5) {
        ...
        total = ((total + tc0) + tc1);
        nTicks = (nTicks + 1);
        std.debug.print("{s}\n", .{_concat_string(_concat_string(_concat_string(_concat_string(_concat_string(_concat_string(_concat_string(_concat_string(std.fmt.allocPrint(std.heap.page_allocator, "{any}", .{sum}) catch |err| handleError(err), " "), std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{tc0}) catch |err| handleError(err)), " "), std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{tc1}) catch |err| handleError(err)), " "), std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{(total / nTicks)}) catch |err| handleError(err)), "  "), std.fmt.allocPrint(std.heap.page_allocator, "{any}", .{buckets}) catch |err| handleError(err))});
        tc0 = 0;
        tc1 = 0;
        t = (t + 1);
    }
