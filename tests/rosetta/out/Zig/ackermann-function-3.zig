...
fn show(m: i32, n: i32) void {
    ...
    if (bit_len(res) <= 256) {
        ...
        const pre = s[0..20]; // []const u8
        const suf = _slice_string(s, (@as(i32, @intCast((s).len)) - 20), @as(i32, @intCast((s).len)), 1); // []const u8
        std.debug.print("{s}\n", .{_concat_string(_concat_string(_concat_string(_concat_string(_concat_string(_concat_string(_concat_string(_concat_string(_concat_string("A(", std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{m}) catch |err| handleError(err)), ", "), std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{n}) catch |err| handleError(err)), ") = "), std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{@as(i32, @intCast((s).len))}) catch |err| handleError(err)), " digits starting/ending with: "), pre), "..."), suf)});
    }
}
