...
fn ppmSize(b: *Bitmap) i32 {
    ...
    return ((header).len + ((3 * b.cols) * b.rows));
}
...
fn user_main() void {
    ...
    std.debug.print("{s}\n", .{_concat_string(_concat_string(_concat_string("Image size: ", std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{bm.cols}) catch |err| handleError(err)), " × "), std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{bm.rows}) catch |err| handleError(err))});
