...
fn parseIntStr(str: []const u8) i32 {
    var i = 0; // i32
    var neg = false; // bool
    if (((str).len > 0) and std.mem.eql(u8, str[0], "-")) {
        neg = true;
        i = 1;
    }
    var n = 0; // i32
...
fn base64Encode(text: []const u8) []const u8 {
    ...
    const pad = @mod(((3 - (@mod((text).len, 3)))), 3); // i32
    if (pad == 1) {
        out = _concat_string(_slice_string(out, 0, ((out).len - 1), 1), "=");
    }
    if (pad == 2) {
        out = _concat_string(_slice_string(out, 0, ((out).len - 2), 1), "==");
    }
    return out;
}
