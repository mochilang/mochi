...
fn validate(commands: []const []const u8, words: []const []const u8, mins: []const i32) []const []const u8 {
    ...
    while (wi < @as(i32, @intCast((words).len))) {
        ...
        while (ci < @as(i32, @intCast((commands).len))) {
            ...
            if (((mins[ci] != 0) and (wlen >= mins[ci])) and (wlen <= @as(i32, @intCast((cmd).len)))) {
                ...
                if (std.mem.eql(u8, c[0..@as(usize, @intCast(wlen))], ww)) {
                    results = blk2: { var _tmp2 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(results) catch |err| handleError(err); _tmp2.append(c) catch |err| handleError(err); const res = _tmp2.toOwnedSlice() catch |err| handleError(err); break :blk2 res; };
                    found = true;
                    break;
                }
            }
        ...
        if (!found) {
            results = blk3: { var _tmp3 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp3.deinit(); _tmp3.appendSlice(results) catch |err| handleError(err); _tmp3.append("*error*") catch |err| handleError(err); const res = _tmp3.toOwnedSlice() catch |err| handleError(err); break :blk3 res; };
        }
        wi = (wi + 1);
    }
    return results.items;
...
fn user_main() void {
    ...
    while (i < @as(i32, @intCast((commands).len))) {
        ...
        mins = blk4: { var _tmp4 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp4.deinit(); _tmp4.appendSlice(mins) catch |err| handleError(err); _tmp4.append(count) catch |err| handleError(err); const res = _tmp4.toOwnedSlice() catch |err| handleError(err); break :blk4 res; };
        i = (i + 1);
    }
