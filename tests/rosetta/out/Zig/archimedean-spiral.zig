...
pub fn main() void {
    while (theta < stop) {
        ...
        if (@mod(count, 100) == 0) {
            std.debug.print("{s}\n", .{_concat_string(_concat_string(std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{(centre + x)}) catch |err| handleError(err), ","), std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{(centre - y)}) catch |err| handleError(err))});
        }
