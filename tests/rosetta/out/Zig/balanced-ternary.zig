...
fn trimLeftZeros(s: []const u8) []const u8 {
    var i = 0; // i32
    while ((i < (s).len) and std.mem.eql(u8, substring(s, i, (i + 1)), "0")) {
        i = (i + 1);
    }
    return substring(s, i, (s).len);
}
    ...
    while (i >= 0) {
        ...
        if (std.mem.eql(u8, ch, "+")) {
            ...
            if (std.mem.eql(u8, ch, "0")) {
                b = blk1: { var _tmp1 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp1.deinit(); _tmp1.appendSlice(b) catch |err| handleError(err); _tmp1.append(0) catch |err| handleError(err); break :blk1 _tmp1.items; };
            } else {
                if (std.mem.eql(u8, ch, "-")) {
                    b = blk2: { var _tmp2 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(b) catch |err| handleError(err); _tmp2.append((0 - 1)) catch |err| handleError(err); break :blk2 _tmp2.items; };
                } else {
                    return struct {
    bt: []const i32,
    ok: bool,
...
fn btInt(i: i32) []const i32 {
    ...
    while (n != 0) {
        ...
        b = blk3: { var _tmp3 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp3.deinit(); _tmp3.appendSlice(b) catch |err| handleError(err); _tmp3.append(m) catch |err| handleError(err); break :blk3 _tmp3.items; };
    }
    return b.items;
}
...
fn btNeg(b: []const i32) []const i32 {
    ...
    var i = 0; // i32
    while (i < (b).len) {
        r = blk4: { var _tmp4 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp4.deinit(); _tmp4.appendSlice(r) catch |err| handleError(err); _tmp4.append(-b[i]) catch |err| handleError(err); break :blk4 _tmp4.items; };
        i = (i + 1);
    }
    return r.items;
}
