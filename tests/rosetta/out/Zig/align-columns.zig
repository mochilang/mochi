...
fn split(s: []const u8, sep: []const u8) []const []const u8 {
    ...
    while (i < (s).len) {
        if ((((sep).len > 0) and ((i + (sep).len) <= (s).len)) and std.mem.eql(u8, substring(s, i, (i + (sep).len)), sep)) {
            ...
            cur = _concat_string(cur, substring(s, i, (i + 1)));
            i = (i + 1);
        }
    }
...
fn newFormatter(text: []const u8) std.AutoHashMap([]const u8, i32) {
    ...
    while (i < (lines).len) {
        ...
        fmtLines = blk2: { var _tmp2 = std.ArrayList([]const []const u8).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(fmtLines) catch |err| handleError(err); _tmp2.append(words) catch |err| handleError(err); break :blk2 _tmp2.items; };
        var j = 0; // i32
        while (j < (words).len) {
            const wlen = (words[j]).len; // i32
            if (j == (width).len) {
                width = blk3: { var _tmp3 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp3.deinit(); _tmp3.appendSlice(width) catch |err| handleError(err); _tmp3.append(wlen) catch |err| handleError(err); break :blk3 _tmp3.items; };
