...
fn main() void {
    ...
    std.debug.print("{s}\n", .{_concat_string(_concat_string(_concat_string(_concat_string(_concat_string("A: ", std.fmt.allocPrint(std.heap.page_allocator, "{any}", .{a}) catch |err| handleError(err)), " G: "), std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{g}) catch |err| handleError(err)), " H: "), std.fmt.allocPrint(std.heap.page_allocator, "{any}", .{h}) catch |err| handleError(err))});
