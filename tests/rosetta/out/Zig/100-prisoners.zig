// Generated by Mochi compiler v0.10.30 on 2025-07-19T00:25:45Z
...
fn doTrials(trials: i32, np: i32, strategy: []const u8) void {
    ...
    var t: i32 = 0; // i32
    while (t < trials) {
        var drawers: []i32 = &[_]i32{};
        var i: i32 = 0; // i32
        while (i < 100) {
            drawers = blk0: {
                var _tmp0 = std.ArrayList(i32).init(std.heap.page_allocator);
                defer _tmp0.deinit();
                _tmp0.appendSlice(drawers.items) catch |err| handleError(err);
                _tmp0.append(i) catch |err| handleError(err);
        ...
        while (p < np) {
            ...
            if (std.mem.eql(u8, strategy, "optimal")) {
                ...
                while (k < 100) {
                    opened = blk1: {
                        var _tmp1 = std.ArrayList(bool).init(std.heap.page_allocator);
                        defer _tmp1.deinit();
                        _tmp1.appendSlice(opened.items) catch |err| handleError(err);
                        _tmp1.append(false) catch |err| handleError(err);
