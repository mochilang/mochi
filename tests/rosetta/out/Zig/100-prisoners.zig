// Generated by Mochi Zig transpiler on 2025-07-22 23:51 +0700
...
fn shuffle(xs: []i64) []i64 {
    ...
    var i: i64 = 99;
    while (i > 0) {
        const j = @mod(_now(), i + 1);
        const tmp = arr[@as(usize, @intCast(i))];
...
fn doTrials(trials: i64, np: i64, strategy: []const u8) void {
    ...
    while (t < trials) {
        ...
        while (p < np) {
            ...
            if (std.mem.eql(u8, strategy, "optimal")) {
                ...
                var d: i64 = 0;
                while (d < 50) {
                    const this = drawers[@as(usize, @intCast(prev))];
                    if (this == p) {
                        found = true;
                        break;
                    }
    ...
    const rf = @as(f64, @floatFromInt(pardoned)) / @as(f64, @floatFromInt(trials)) * 100.0;
...
fn mochi_main() void {
    ...
    for ([2]i64{10, 100}) |np| {
        ...
        for ([2][]const u8{"random", "optimal"}) |strat| {
