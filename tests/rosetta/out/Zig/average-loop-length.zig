...
fn avgLen(n: i32) f64 {
    ...
    while (t < tests) {
        ...
        var i = 0; // i32
        while (i < n) {
            visited = blk0: { var _tmp0 = std.ArrayList(bool).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(visited) catch |err| handleError(err); _tmp0.append(false) catch |err| handleError(err); const res = _tmp0.toOwnedSlice() catch |err| handleError(err); break :blk0 res; };
            i = (i + 1);
        }
        var x = 0; // i32
        while (!visited[x]) {
