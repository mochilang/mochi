...
fn sieve(limit: i32) []const i32 {
    ...
    var i = 0; // i32
    while (i <= limit) {
        spf = blk0: { var _tmp0 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(spf) catch |err| handleError(err); _tmp0.append(0) catch |err| handleError(err); break :blk0 _tmp0.items; };
        i = (i + 1);
    }
    i = 2;
    while (i <= limit) {
...
fn primesFrom(spf: []const i32, limit: i32) []const i32 {
    ...
    while (i <= limit) {
        if (spf[i] == i) {
            primes = blk1: { var _tmp1 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp1.deinit(); _tmp1.appendSlice(primes) catch |err| handleError(err); _tmp1.append(i) catch |err| handleError(err); break :blk1 _tmp1.items; };
        }
        i = (i + 1);
    }
    return primes.items;
...
fn commatize(n: i32) []const u8 {
    ...
    var c = 0; // i32
    while (i >= 0) {
        out = _concat_string(substring(s, i, (i + 1)), out);
        c = (c + 1);
...
fn arithmeticNumbers(limit: i32, spf: []const i32) []const i32 {
    ...
    while ((arr).len < limit) {
        if (spf[n] == n) {
            arr = blk2: { var _tmp2 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(arr) catch |err| handleError(err); _tmp2.append(n) catch |err| handleError(err); break :blk2 _tmp2.items; };
        } else {
            var x = n; // i32
            var sigma = 1; // i32
            var tau = 1; // i32
            ...
            if (@mod(sigma, tau) == 0) {
                arr = blk3: { var _tmp3 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp3.deinit(); _tmp3.appendSlice(arr) catch |err| handleError(err); _tmp3.append(n) catch |err| handleError(err); break :blk3 _tmp3.items; };
            }
        }
        n = (n + 1);
    }
