...
fn numberName(n: i32) []const u8 {
    ...
    if (n < 100) {
        var t: []const u8 = tens[@as(i32, ((n / 10)))]; // []const u8
        var s = @mod(n, 10); // i32
        if (s > 0) {
...
fn slur(p: []const u8, d: i32) []const u8 {
    ...
    while (i < (@as(i32, @intCast((p).len)) - 1)) {
        a = blk2: {
            ...
            defer _tmp2.deinit();
            _tmp2.appendSlice(a.items) catch |err| handleError(err);
            _tmp2.append(_slice_string(p, i, (i + 1), 1)) catch |err| handleError(err);
