...
fn slur(p: []const u8, d: i32) []const u8 {
    if (@as(i32, @intCast((p).len)) <= 2) {
        return p;
    }
    var a = std.ArrayList(u8).init(std.heap.page_allocator);
    var i = 1; // i32
    while (i < (@as(i32, @intCast((p).len)) - 1)) {
        a = blk2: { var _tmp2 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(a) catch |err| handleError(err); _tmp2.append(_slice_string(p, i, (i + 1), 1)) catch |err| handleError(err); const res = _tmp2.toOwnedSlice() catch |err| handleError(err); break :blk2 res; };
        i = (i + 1);
    }
    var idx = (@as(i32, @intCast((a).len)) - 1); // i32
    var seed = d; // i32
    while (idx >= 1) {
    ...
    var s = p[0..1]; // []const u8
    var k = 0; // i32
    while (k < @as(i32, @intCast((a).len))) {
        s = _concat_string(s, a[k]);
        k = (k + 1);
    }
    s = _concat_string(s, _slice_string(p, (@as(i32, @intCast((p).len)) - 1), @as(i32, @intCast((p).len)), 1));
    const w = fields(s); // []const []const u8
    return _join_strings(w, " ");
}
