...
fn push(h: []const std.AutoHashMap([]const u8, i32), it: std.AutoHashMap([]const u8, i32)) []const std.AutoHashMap([]const u8, i32) {
    h = blk0: { var _tmp0 = std.ArrayList(std.StringHashMap(i32)).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(h) catch |err| handleError(err); _tmp0.append(it) catch |err| handleError(err); break :blk0 _tmp0.items; };
    var i = ((h).len - 1); // i32
...
fn step(h: []const std.AutoHashMap([]const u8, i32), nv: i32, dir: []const i32) std.AutoHashMap([]const u8, i32) {
    ...
    while (((h).len > 0) and (h[0]["s"] == s)) {
        const it = h[0]; // std.StringHashMap(i32)
        h = _slice_list(std.StringHashMap(i32), h, 1, h.len, 1);
    ...
    it["a"],
    it["b"],
}) catch |err| handleError(err); break :blk1 _tmp1.items; };
        if (it["a"] > it["b"]) {
            h = push(h, struct {
    s: i32,
    a: i32,
    ...
    for (v) |p| {
        list = blk2: { var _tmp2 = std.ArrayList([]const i32).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(list) catch |err| handleError(err); _tmp2.append(p) catch |err| handleError(err); break :blk2 _tmp2.items; };
    }
