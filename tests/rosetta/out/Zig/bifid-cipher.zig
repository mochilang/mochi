...
fn encrypt(text: []const u8, emap: std.AutoHashMap([]const u8, []const i32), dmap: std.AutoHashMap([]const u8, []const u8)) []const u8 {
    ...
    while (i < (text).len) {
        ...
        row0 = blk0: { var _tmp0 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(row0) catch |err| handleError(err); _tmp0.append(xy[0]) catch |err| handleError(err); break :blk0 _tmp0.items; };
        row1 = blk1: { var _tmp1 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp1.deinit(); _tmp1.appendSlice(row1) catch |err| handleError(err); _tmp1.append(xy[1]) catch |err| handleError(err); break :blk1 _tmp1.items; };
        i = (i + 1);
    }
    for (row1) |v| {
        row0 = blk2: { var _tmp2 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(row0) catch |err| handleError(err); _tmp2.append(v) catch |err| handleError(err); break :blk2 _tmp2.items; };
    }
    var res = ""; // []const u8
    var j = 0; // i32
    while (j < (row0).len) {
...
fn decrypt(text: []const u8, emap: std.AutoHashMap([]const u8, []const i32), dmap: std.AutoHashMap([]const u8, []const u8)) []const u8 {
    ...
    while (i < (text).len) {
        const ch = _slice_string(text, i, (i + 1), 1); // []const u8
        const xy = emap[ch]; // []const i32
        coords = blk3: { var _tmp3 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp3.deinit(); _tmp3.appendSlice(coords) catch |err| handleError(err); _tmp3.append(xy[0]) catch |err| handleError(err); break :blk3 _tmp3.items; };
        coords = blk4: { var _tmp4 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp4.deinit(); _tmp4.appendSlice(coords) catch |err| handleError(err); _tmp4.append(xy[1]) catch |err| handleError(err); break :blk4 _tmp4.items; };
        i = (i + 1);
    }
    var half = ((coords).len / 2); // f64
    var k1 = std.ArrayList(i32).init(std.heap.page_allocator);
    var k2 = std.ArrayList(i32).init(std.heap.page_allocator);
    var idx = 0; // i32
    while (idx < half) {
        k1 = blk5: { var _tmp5 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp5.deinit(); _tmp5.appendSlice(k1) catch |err| handleError(err); _tmp5.append(coords[idx]) catch |err| handleError(err); break :blk5 _tmp5.items; };
        idx = (idx + 1);
    }
    while (idx < (coords).len) {
        k2 = blk6: { var _tmp6 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp6.deinit(); _tmp6.appendSlice(k2) catch |err| handleError(err); _tmp6.append(coords[idx]) catch |err| handleError(err); break :blk6 _tmp6.items; };
        idx = (idx + 1);
    }
    var res = ""; // []const u8
    var j = 0; // i32
