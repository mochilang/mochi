...
fn user_main() void {
    ...
    var i = 0; // i32
    while (i < 20000) {
        sums = blk0: { var _tmp0 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(sums) catch |err| handleError(err); _tmp0.append(0) catch |err| handleError(err); break :blk0 _tmp0.items; };
        i = (i + 1);
    }
    i = 1;
    while (i < 20000) {
