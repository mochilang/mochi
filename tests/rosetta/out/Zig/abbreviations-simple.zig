...
fn fields(s: []const u8) []const []const u8 {
    ...
    while (i < @as(i32, @intCast((s).len))) {
        ...
        if ((std.mem.eql(u8, ch, " ") or std.mem.eql(u8, ch, "\n")) or std.mem.eql(u8, ch, "\t")) {
            if (@as(i32, @intCast((cur).len)) > 0) {
                words = blk0: { var _tmp0 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(words) catch |err| handleError(err); _tmp0.append(cur) catch |err| handleError(err); const res = _tmp0.toOwnedSlice() catch |err| handleError(err); break :blk0 res; };
                cur = "";
            }
        } else {
    ...
    if (@as(i32, @intCast((cur).len)) > 0) {
        words = blk1: { var _tmp1 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp1.deinit(); _tmp1.appendSlice(words) catch |err| handleError(err); _tmp1.append(cur) catch |err| handleError(err); const res = _tmp1.toOwnedSlice() catch |err| handleError(err); break :blk1 res; };
    }
    return words.items;
}
...
fn readTable(table: []const u8) std.AutoHashMap([]const u8, i32) {
    ...
    while (i < @as(i32, @intCast((toks).len))) {
        ...
        var minlen = @as(i32, @intCast((cmd).len)); // i32
        i = (i + 1);
        ...
        cmds = blk2: { var _tmp2 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(cmds) catch |err| handleError(err); _tmp2.append(cmd) catch |err| handleError(err); const res = _tmp2.toOwnedSlice() catch |err| handleError(err); break :blk2 res; };
        mins = blk3: { var _tmp3 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp3.deinit(); _tmp3.appendSlice(mins) catch |err| handleError(err); _tmp3.append(minlen) catch |err| handleError(err); const res = _tmp3.toOwnedSlice() catch |err| handleError(err); break :blk3 res; };
    }
    return struct {
...
fn validate(commands: []const []const u8, mins: []const i32, words: []const []const u8) []const []const u8 {
    ...
    while (wi < @as(i32, @intCast((words).len))) {
        ...
        while (ci < @as(i32, @intCast((commands).len))) {
            ...
            if (((mins[ci] != 0) and (wlen >= mins[ci])) and (wlen <= @as(i32, @intCast((cmd).len)))) {
                ...
                if (std.mem.eql(u8, c[0..@as(usize, @intCast(wlen))], ww)) {
                    results = blk4: { var _tmp4 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp4.deinit(); _tmp4.appendSlice(results) catch |err| handleError(err); _tmp4.append(c) catch |err| handleError(err); const res = _tmp4.toOwnedSlice() catch |err| handleError(err); break :blk4 res; };
                    found = true;
                    break;
                }
            }
        ...
        if (!found) {
            results = blk5: { var _tmp5 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp5.deinit(); _tmp5.appendSlice(results) catch |err| handleError(err); _tmp5.append("*error*") catch |err| handleError(err); const res = _tmp5.toOwnedSlice() catch |err| handleError(err); break :blk5 res; };
        }
        wi = (wi + 1);
    }
