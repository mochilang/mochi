...
fn copy(xs: []const i32) []const i32 {
...
fn cartN(lists: i32) []const []const i32 {
    ...
    var out = std.ArrayList(i32).init(std.heap.page_allocator);
    const last = ((a).len - 1); // i32
    const left = cartN(_slice_list([]const i32, a, 0, last, 1)); // []const []const i32
    for (left) |p| {
        for (a[last]) |x| {
            var row = copy(p); // []const i32
            row = blk1: { var _tmp1 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp1.deinit(); _tmp1.appendSlice(row) catch |err| handleError(err); _tmp1.append(x) catch |err| handleError(err); break :blk1 _tmp1.items; };
            out = blk2: { var _tmp2 = std.ArrayList([]const i32).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(out) catch |err| handleError(err); _tmp2.append(row) catch |err| handleError(err); break :blk2 _tmp2.items; };
        }
    }
    return out.items;
}
