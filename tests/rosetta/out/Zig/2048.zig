...
fn newBoard() []const []const i32 {
    ...
    while (y < SIZE) {
        ...
        var x: i32 = 0; // i32
        while (x < SIZE) {
            row = blk0: { var _tmp0 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(row.items) catch |err| handleError(err); _tmp0.append(0) catch |err| handleError(err); const res = _tmp0.toOwnedSlice() catch |err| handleError(err); break :blk0 res; };
            x = (x + 1);
        }
        b = blk1: { var _tmp1 = std.ArrayList([]const i32).init(std.heap.page_allocator); defer _tmp1.deinit(); _tmp1.appendSlice(b.items) catch |err| handleError(err); _tmp1.append(row) catch |err| handleError(err); const res = _tmp1.toOwnedSlice() catch |err| handleError(err); break :blk1 res; };
        y = (y + 1);
    }
    return b.items;
}
...
fn spawnTile(b: []const []const i32) std.AutoHashMap([]const u8, i32) {
    ...
    while (y < SIZE) {
        ...
        while (x < SIZE) {
            if (b[y][x] == 0) {
                empty = blk2: { var _tmp2 = std.ArrayList([]const i32).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(empty.items) catch |err| handleError(err); _tmp2.append(&[_]i32{
    x,
    y,
...
fn pad(n: i32) []const u8 {
    var s: []const u8 = std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{n}) catch |err| handleError(err); // []const u8
    var pad = (4 - @as(i32, @intCast((s).len))); // i32
    var i: i32 = 0; // i32
    var out: []const u8 = ""; // []const u8
    while (i < pad) {
...
fn reverseRow(r: []const i32) []const i32 {
    ...
    while (i >= 0) {
        out = blk3: { var _tmp3 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp3.deinit(); _tmp3.appendSlice(out.items) catch |err| handleError(err); _tmp3.append(r[i]) catch |err| handleError(err); const res = _tmp3.toOwnedSlice() catch |err| handleError(err); break :blk3 res; };
        i = (i - 1);
    }
    return out.items;
}
...
fn slideLeft(row: []const i32) std.AutoHashMap([]const u8, i32) {
    ...
    while (i < @as(i32, @intCast((row).len))) {
        if (row[i] != 0) {
            xs = blk4: { var _tmp4 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp4.deinit(); _tmp4.appendSlice(xs.items) catch |err| handleError(err); _tmp4.append(row[i]) catch |err| handleError(err); const res = _tmp4.toOwnedSlice() catch |err| handleError(err); break :blk4 res; };
        }
        i = (i + 1);
    }
    var res: []i32 = &[_]i32{};
    ...
    while (i < @as(i32, @intCast((xs).len))) {
        if (((i + 1) < @as(i32, @intCast((xs).len))) and (xs[i] == xs[(i + 1)])) {
            const v = (xs[i] * 2); // i32
            gain = (gain + v);
            res = blk5: { var _tmp5 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp5.deinit(); _tmp5.appendSlice(res.items) catch |err| handleError(err); _tmp5.append(v) catch |err| handleError(err); const res = _tmp5.toOwnedSlice() catch |err| handleError(err); break :blk5 res; };
            i = (i + 2);
        } else {
            res = blk6: { var _tmp6 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp6.deinit(); _tmp6.appendSlice(res.items) catch |err| handleError(err); _tmp6.append(xs[i]) catch |err| handleError(err); const res = _tmp6.toOwnedSlice() catch |err| handleError(err); break :blk6 res; };
            i = (i + 1);
        }
    }
    while (@as(i32, @intCast((res).len)) < SIZE) {
        res = blk7: { var _tmp7 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp7.deinit(); _tmp7.appendSlice(res.items) catch |err| handleError(err); _tmp7.append(0) catch |err| handleError(err); const res = _tmp7.toOwnedSlice() catch |err| handleError(err); break :blk7 res; };
    }
    return struct {
    row: []const i32,
    gain: i32,
...
fn getCol(b: []const []const i32, x: i32) []const i32 {
    ...
    var y: i32 = 0; // i32
    while (y < SIZE) {
        col = blk8: { var _tmp8 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp8.deinit(); _tmp8.appendSlice(col.items) catch |err| handleError(err); _tmp8.append(b[y][x]) catch |err| handleError(err); const res = _tmp8.toOwnedSlice() catch |err| handleError(err); break :blk8 res; };
        y = (y + 1);
    }
    return col.items;
}
