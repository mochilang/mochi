...
fn gen(k: i32, count: i32) []const i32 {
    ...
    var n = 2; // i32
    while (@as(i32, @intCast((r).len)) < count) {
        if (kPrime(n, k)) {
            r = blk0: { var _tmp0 = std.ArrayList(i32).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(r) catch |err| handleError(err); _tmp0.append(n) catch |err| handleError(err); const res = _tmp0.toOwnedSlice() catch |err| handleError(err); break :blk0 res; };
        }
        n = (n + 1);
    }
    return r.items;
