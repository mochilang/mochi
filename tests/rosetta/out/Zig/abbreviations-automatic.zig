...
fn distinct(xs: []const []const u8) []const []const u8 {
    ...
    while (i < (xs).len) {
        ...
        if (!(m.contains(x))) {
            ...
            out = blk2: { var _tmp2 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(out) catch |err| handleError(err); _tmp2.append(x) catch |err| handleError(err); break :blk2 _tmp2.items; };
        }
        i = (i + 1);
    }
    return out.items;
...
fn abbrevLen(words: []const []const u8) i32 {
    const size = (words).len; // i32
    var l = 1; // i32
    while (true) {
        var abbrs = std.ArrayList(u8).init(std.heap.page_allocator);
        var i = 0; // i32
        while (i < size) {
            abbrs = blk3: { var _tmp3 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp3.deinit(); _tmp3.appendSlice(abbrs) catch |err| handleError(err); _tmp3.append(takeRunes(words[i], l)) catch |err| handleError(err); break :blk3 _tmp3.items; };
            i = (i + 1);
        }
