...
const OP_NUM: i32 = 0; // i32
const OP_ADD: i32 = 1; // i32
const OP_SUB: i32 = 2; // i32
const OP_MUL: i32 = 3; // i32
const OP_DIV: i32 = 4; // i32
const n_cards: i32 = 4; // i32
const goal: i32 = 24; // i32
const digit_range: i32 = 9; // i32
...
fn solve(xs: []const std.AutoHashMap([]const u8, i32)) bool {
    ...
    while (i < @as(i32, @intCast((xs).len))) {
        ...
        while (j < @as(i32, @intCast((xs).len))) {
            var rest: []std.StringHashMap(i32) = &[_]std.StringHashMap(i32){};
            var k: i32 = 0; // i32
            while (k < @as(i32, @intCast((xs).len))) {
                if ((k != i) and (k != j)) {
                    rest = blk0: {
                        ...
                        defer _tmp0.deinit();
                        _tmp0.appendSlice(rest.items) catch |err| handleError(err);
            ...
            if (solve(blk4: {
                ...
                defer _tmp4.deinit();
                _tmp4.appendSlice(rest.items) catch |err| handleError(err);
            ...
            if (solve(blk5: {
                ...
                defer _tmp5.deinit();
                _tmp5.appendSlice(rest.items) catch |err| handleError(err);
...
fn user_main() void {
    var iter: i32 = 0; // i32
    while (iter < 10) {
        var cards: []std.StringHashMap(i32) = &[_]std.StringHashMap(i32){};
        var i: i32 = 0; // i32
        while (i < n_cards) {
            ...
            cards = blk6: {
                ...
                defer _tmp6.deinit();
                _tmp6.appendSlice(cards.items) catch |err| handleError(err);
