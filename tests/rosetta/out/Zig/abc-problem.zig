...
fn fields(s: []const u8) []const []const u8 {
    ...
    while (i < @as(i32, @intCast((s).len))) {
        ...
        if (std.mem.eql(u8, c, " ")) {
            if (@as(i32, @intCast((cur).len)) > 0) {
                res = blk0: { var _tmp0 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp0.deinit(); _tmp0.appendSlice(res) catch |err| handleError(err); _tmp0.append(cur) catch |err| handleError(err); const res = _tmp0.toOwnedSlice() catch |err| handleError(err); break :blk0 res; };
                cur = "";
            }
        } else {
    ...
    if (@as(i32, @intCast((cur).len)) > 0) {
        res = blk1: { var _tmp1 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp1.deinit(); _tmp1.appendSlice(res) catch |err| handleError(err); _tmp1.append(cur) catch |err| handleError(err); const res = _tmp1.toOwnedSlice() catch |err| handleError(err); break :blk1 res; };
    }
    return res.items;
}
    ...
    while (i < @as(i32, @intCast((blks).len))) {
        ...
        if (std.mem.eql(u8, c, std.ascii.lowerString(b[0..1])) or std.mem.eql(u8, c, std.ascii.lowerString(b[1..2]))) {
            ...
            while (j < @as(i32, @intCast((blks).len))) {
                if (j != i) {
                    rest = blk2: { var _tmp2 = std.ArrayList([]const u8).init(std.heap.page_allocator); defer _tmp2.deinit(); _tmp2.appendSlice(rest) catch |err| handleError(err); _tmp2.append(blks[j]) catch |err| handleError(err); const res = _tmp2.toOwnedSlice() catch |err| handleError(err); break :blk2 res; };
                }
                j = (j + 1);
            }
...
fn newSpeller(blocks: []const u8) fn([]const u8) bool {
    ...
    return (blk: { const closure = struct { bl: []const []const u8, }{ .bl = bl }; break :blk struct { fn inner(w: []const u8) bool {
