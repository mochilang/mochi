...
// Source: /workspace/mochi/tests/rosetta/x/Mochi/abbreviations-automatic.mochi
...
function distinct(xs: string[]): string[] {
  var m: { [key: string]: boolean } = {};
  var out: string[] = [];
  var i = 0;
  while ((i < xs.length)) {
    let x = xs[i];
    if ((!(Object.prototype.hasOwnProperty.call(m, String(x))))) {
      m[x] = true;
      out = [...out, x];
    }
    i = i + 1;
