// Generated by Mochi compiler v0.10.30 on 2025-07-18T12:38:42Z
...
function solve(xs: Record<string, any>[]): boolean {
  if ((xs.length == 1)) {
    ...
    if (((f["denom"] != 0) && (f["num"] == (f["denom"] * goal)))) {
      console.log(
        Array.isArray(exprString(xs[0]))
          ? _fmtList(exprString(xs[0]))
          : (typeof exprString(xs[0]) === "object"
            ? _fmt(exprString(xs[0]))
            : String(exprString(xs[0]))),
      );
      return true;
    }
    return false;
...
function main(): void {
  ...
  while ((iter < 10)) {
    ...
    while ((i < n_cards)) {
      ...
      console.log(
        Array.isArray(` ${String(n)}`)
          ? _fmtList(` ${String(n)}`)
          : (typeof ` ${String(n)}` === "object"
            ? _fmt(` ${String(n)}`)
            : String(` ${String(n)}`)),
      );
      i = i + 1;
    ...
    console.log(
      Array.isArray(":  ")
        ? _fmtList(":  ")
        : (typeof ":  " === "object" ? _fmt(":  ") : String(":  ")),
    );
    if ((!solve(cards))) {
      console.log(
        Array.isArray("No solution")
          ? _fmtList("No solution")
          : (typeof "No solution" === "object"
            ? _fmt("No solution")
            : String("No solution")),
      );
    }
