...
// Source: /workspace/mochi/tests/rosetta/x/Mochi/atomic-updates.mochi
...
function main(): void {
  ...
  let initialSum = 1000;
  var buckets: number[] = [];
  for (let i: number = 0; i < nBuckets; i++) {
    buckets = [...buckets, 0];
  }
  var i = nBuckets;
  var dist = initialSum;
  while ((i > 0)) {
    let v = Math.trunc(dist / i);
    i = i - 1;
    buckets[i] = v;
    dist = dist - v;
  }
  ...
  while ((t < 5)) {
    ...
    nTicks = nTicks + 1;
    console.log(
      `${String(sum)} ${String(tc0)} ${String(tc1)} ${
        String(Math.trunc(total / nTicks))
      }  ${String(buckets)}`,
    );
    tc0 = 0;
    tc1 = 0;
