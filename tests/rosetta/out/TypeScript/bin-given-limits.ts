...
// Source: /workspace/mochi/tests/rosetta/x/Mochi/bin-given-limits.mochi
...
function printBins(limits: number[], bins: number[]): void {
  var n = limits.length;
  console.log(`           < ${padLeft(limits[0], 3)} = ${padLeft(bins[0], 2)}`);
  var i = 1;
  while ((i < n)) {
    console.log(
      `>= ${padLeft(limits[i - 1], 3)} and < ${padLeft(limits[i], 3)} = ${
        padLeft(bins[i], 2)
      }`,
    );
    i = i + 1;
  ...
  console.log(
    `>= ${padLeft(limits[n - 1], 3)}           = ${padLeft(bins[n], 2)}`,
  );
  return console.log("");
}
