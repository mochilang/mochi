...
class Program
{
    ...
    static List<int> plus(List<int> a, List<int> b)
    {
        ...
        while (i < a.Length)
        {
            res = new List<int>(res) { (_indexList(a, i) + _indexList(b, i)) };
            i = (i + 1);
        }
        return res;
    }
    ...
    static int topple(List<int> p)
    {
        ...
        while (i < p.Length)
        {
            if (_indexList(p, i) > 3)
            {
                p[i] = (_indexList(p, i) - 4);
                List<int> nbs = _indexList(neighbors, i);
                foreach (var j in nbs)
                {
                    p[j] = (_indexList(p, j) + 1);
                }
                return 0;
            }
            i = (i + 1);
    ...
    static string pileString(List<int> p)
    {
        ...
        while (r < 3)
        {
            ...
            while (c < 3)
            {
                s = ((s + Convert.ToString(_indexList(p, ((3 * r) + c)))) + " ");
                c = (c + 1);
            }
            s = string.Concat(s, "\n");
            r = (r + 1);
