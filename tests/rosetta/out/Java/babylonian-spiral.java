// Generated by Mochi compiler v0.10.26 on 2025-07-16T11:32:21Z
...
public class BabylonianSpiral {
    ...
    static Map<String,any> step(List<Map<String,Integer>> h, int nv, List<Integer> dir) {
        while (Objects.equals(h.size(), 0) || String.valueOf(nv * nv).compareTo(String.valueOf(((List)h.get(0)).get("s"))) <= 0) {
            h = push(h, new SAB(nv * nv, nv, 0));
            nv = (int)(nv + 1);
        }
        ...
        while (h.size() > 0 && Objects.equals(((List)h.get(0)).get("s"), s)) {
            ...
            if (String.valueOf(it.get("a")).compareTo(String.valueOf(it.get("b"))) > 0) {
                h = push(h, new SAB(((Number)((Number)it.get("a")).doubleValue() * ((Number)it.get("a")).doubleValue()).doubleValue() + ((Number)((Number)(((Number)it.get("b")).doubleValue() + 1)).doubleValue() * ((Number)(((Number)it.get("b")).doubleValue() + 1)).doubleValue()).doubleValue(), it.get("a"), ((Number)it.get("b")).doubleValue() + 1));
            }
        }
    ...
    static List<List<Integer>> positions(int n) {
        ...
        while (i < n) {
            ...
            Map<String,any> st = step(heap, nv, dir);
            dir = (List<Integer>)((any)st.get("d"));
            heap = (List<Map<String,Integer>>)((any)st.get("heap"));
            nv = (int)(Integer.parseInt(((any)st.get("n"))));
