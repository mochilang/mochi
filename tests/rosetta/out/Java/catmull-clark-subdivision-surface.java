// Generated by Mochi compiler v0.10.26 on 2025-07-16T12:11:19Z
...
class Edge {
    ...
    int fn2;
    Object cp;
    Edge(int pn1, int pn2, int fn1, int fn2, Object cp) {
        this.pn1 = pn1;
        this.pn2 = pn2;
        this.fn1 = fn1;
        this.fn2 = fn2;
...
public class CatmullClarkSubdivisionSurface {
    ...
    static Object sumPoint(Object p1, Object p2) {
        return new Point(((Number)((Map<?,?>)p1).get("x")).doubleValue() + ((Number)((Map<?,?>)p2).get("x")).doubleValue(), ((Number)((Map<?,?>)p1).get("y")).doubleValue() + ((Number)((Map<?,?>)p2).get("y")).doubleValue(), ((Number)((Map<?,?>)p1).get("z")).doubleValue() + ((Number)((Map<?,?>)p2).get("z")).doubleValue());
    }
    static Object mulPoint(Object p, double m) {
        return new Point(((Number)((Map<?,?>)p).get("x")).doubleValue() * m, ((Number)((Map<?,?>)p).get("y")).doubleValue() * m, ((Number)((Map<?,?>)p).get("z")).doubleValue() * m);
    }
    static Object divPoint(Object p, double d) {
        return mulPoint(p, 1.000000 / d);
    }
    static Object centerPoint(Object p1, Object p2) {
        return divPoint(sumPoint(p1, p2), 2.000000);
    }
    static List<Object> getFacePoints(List<Object> points, List<List<Integer>> faces) {
        List<Object> facePoints = Arrays.asList();
        int i = 0;
        while (i < faces.size()) {
    ...
    static List<Object> getEdgesFaces(List<Object> points, List<List<Integer>> faces) {
        ...
        List<Object> edgesCenters = Arrays.asList();
        for (List<Integer> me : merged) {
            List<Object> p1 = points.get(me.get(0));
            List<Object> p2 = points.get(me.get(1));
            Object cp = centerPoint(p1, p2);
            edgesCenters.add(new Edge(me.get(0), me.get(1), me.get(2), me.get(3), cp));
        }
        return edgesCenters;
    }
    static List<Object> getEdgePoints(List<Object> points, List<Object> edgesFaces, List<Object> facePoints) {
        List<Object> edgePoints = Arrays.asList();
        int i = 0;
        while (i < edgesFaces.size()) {
            List<Object> edge = edgesFaces.get(i);
            Object cp = edge.cp;
            List<Object> fp1 = facePoints.get(edge.fn1);
            List<Object> fp2 = fp1;
            if (!Objects.equals(edge.fn2, 0 - 1)) {
                fp2 = facePoints.get(edge.fn2);
            }
            Object cfp = centerPoint(fp1, fp2);
            edgePoints.add(centerPoint(cp, cfp));
            i = (int)(i + 1);
        }
        return edgePoints;
    }
    static List<Object> getAvgFacePoints(List<Object> points, List<List<Integer>> faces, List<Object> facePoints) {
        int numP = points.size();
        List<Object> temp = Arrays.asList();
        int i = 0;
        while (i < numP) {
        ...
        int fnum = 0;
        while (fnum < faces.size()) {
            List<Object> fp = facePoints.get(fnum);
            for (List<Integer> pn : (List<List<Integer>>)faces.get(fnum)) {
                List<Object> tp = temp.get(pn);
                temp.set(pn, new PointEx(sumPoint(tp.p, fp), tp.n + 1));
            }
            fnum = (int)(fnum + 1);
        }
        List<Object> avg = Arrays.asList();
        int j = 0;
        while (j < numP) {
            List<Object> tp = temp.get(j);
            avg.add(divPoint(tp.p, Double.parseDouble(String.valueOf(tp.n))));
            j = (int)(j + 1);
        }
        return avg;
    }
    static List<Object> getAvgMidEdges(List<Object> points, List<Object> edgesFaces) {
        int numP = points.size();
        List<Object> temp = Arrays.asList();
        int i = 0;
        while (i < numP) {
        ...
        for (Object edge : edgesFaces) {
            Object cp = ((Map<?,?>)edge).get("cp");
            List<Object> arr = new ArrayList<>(Arrays.asList(((Map<?,?>)edge).get("pn1"), ((Map<?,?>)edge).get("pn2")));
            for (Object pn : arr) {
                List<Object> tp = temp.get(pn);
    ...
    static List<Integer> getPointsFaces(List<Object> points, List<List<Integer>> faces) {
    ...
    static List<Object> getNewPoints(List<Object> points, List<Integer> pf, List<Object> afp, List<Object> ame) {
        List<Object> newPts = Arrays.asList();
        int i = 0;
        while (i < points.size()) {
            ...
            List<Object> old = points.get(i);
            Object p1 = mulPoint(old, m1);
            Object p2 = mulPoint(afp.get(i), m2);
            Object p3 = mulPoint(ame.get(i), m3);
    ...
    static List<Object> cmcSubdiv(List<Object> points, List<List<Integer>> faces) {
        List<Object> facePoints = getFacePoints(points, faces);
        List<Object> edgesFaces = getEdgesFaces(points, faces);
        List<Object> edgePoints = getEdgePoints(points, edgesFaces, facePoints);
        List<Object> avgFacePoints = getAvgFacePoints(points, faces, facePoints);
        List<Object> avgMidEdges = getAvgMidEdges(points, edgesFaces);
        List<Integer> pointsFaces = getPointsFaces(points, faces);
        List<Object> newPoints = getNewPoints(points, pointsFaces, avgFacePoints, avgMidEdges);
        List<Integer> facePointNums = Arrays.asList();
        int nextPoint = newPoints.size();
        for (Object fp : facePoints) {
            newPoints.add(fp);
        ...
        while (idx < edgesFaces.size()) {
            List<Object> e = edgesFaces.get(idx);
    ...
    static String formatPoint(Object p) {
        return ((Number)((Number)((Number)((Number)((Number)"[" + ((Number)fmt4(((Map<?,?>)p).get("x"))).doubleValue()).doubleValue() + " ").doubleValue() + ((Number)fmt4(((Map<?,?>)p).get("y"))).doubleValue()).doubleValue() + " ").doubleValue() + ((Number)fmt4(((Map<?,?>)p).get("z"))).doubleValue()).doubleValue() + "]";
    }
