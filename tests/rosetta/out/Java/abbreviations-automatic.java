...
public class AbbreviationsAutomatic {
    ...
    static int abbrevLen(List<String> words) {
        int size = words.size();
        int l = 1;
        while (true) {
            List<String> abbrs = Arrays.asList();
            int i = 0;
            while (i < size) {
                abbrs.add(takeRunes(words.get(i), l));
                i = (int)(i + 1);
            }
            if (Objects.equals(distinct(abbrs).size(), size)) {
                return l;
            }
            l = (int)(l + 1);
        }
