object abbreviations_automatic {
  ...
  def takeRunes(s: String, n: Int): String = {
    var idx = 0
    var count = 0
    while (idx < s.length) {
      if (count == n) {
  ...
  def distinct(xs: List[String]): List[String] = {
    var m: Map[String, Boolean] = scala.collection.mutable.Map()
  ...
  def abbrevLen(words: List[String]): Int = {
    val size = words.length
    var l = 1
    while (true) {
      var abbrs: List[String] = scala.collection.mutable.ArrayBuffer[Any]()
      var i = 0
      while (i < size) {
        abbrs = abbrs :+ takeRunes((words).apply(i), l)
        i += 1
      }
