...
function validComb(a, b, c, d, e, f, g)
    ...
    return ((__eq(square1, square2) and __eq(square2, square3)) and __eq(square3, square4))
end
...
function isUnique(a, b, c, d, e, f, g)
    ...
    while (i < #nums) do
        local j = __add(i, 1)
        while (j < #nums) do
            if __eq(__index(nums, i), __index(nums, j)) then
                return false
            end
            j = __add(j, 1)
        end
            ...
            for d = low, ((__add(high, 1)))-1 do
                ...
                for e = low, ((__add(high, 1)))-1 do
                    for f = low, ((__add(high, 1)))-1 do
                        ...
                        if not __eq(__add(__add(d, e), f), s) then
                            goto __continue4
                        end
                        if not __eq(__add(f, g), s) then
                            goto __continue4
                        end
