...
func TestASTPrinterGolden(t *testing.T) {
	...
	skip := map[string]bool{"bench_block": true, "tree_sum": true, "binary_precedence": true, "dataset_where_filter": true, "go_auto": true}
	...
	for _, path := range files {
		...
		wantSrc, _ := os.ReadFile(path)
		want, err := runMochi(string(wantSrc))
		if err != nil {
			t.Fatalf("run original %s: %v", name, err)
		}
