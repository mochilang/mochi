...
function test_Q2_finds_earliest_title_for_German_companies_with_character_keyword()
    if not (__eq(result, "Der Film")) then error('expect failed') end
end
...
titles = (function()
    ...
    return __query(_src, {
        ...
        { items = title, on = function(cn, mc, t) return __eq(mc.movie_id, t.id) end },
        { items = movie_keyword, on = function(cn, mc, t, mk) return __eq(mk.movie_id, t.id) end },
        { items = keyword, on = function(cn, mc, t, mk, k) return __eq(mk.keyword_id, k.id) end }
    }, { selectFn = function(cn, mc, t, mk, k) return t.title end, where = function(cn, mc, t, mk, k) return (((__eq(cn.country_code, "[de]") and __eq(k.keyword, "character-name-in-title")) and __eq(mc.movie_id, mk.movie_id))) end })
end)()
result = __min(titles)
__json(result)
local __tests = {
    {name="Q2 finds earliest title for German companies with character keyword", fn=test_Q2_finds_earliest_title_for_German_companies_with_character_keyword},
}
