...
function test_Q4_returns_minimum_rating_and_title_for_sequels()
    if not (__eq(result, {{["rating"]="6.2", ["movie_title"]="Alpha Movie"}})) then error('expect failed') end
end
...
rows = (function()
    local _src = info_type
    return __query(_src, {
        { items = movie_info_idx, on = function(it, mi) return __eq(it.id, mi.info_type_id) end },
        { items = title, on = function(it, mi, t) return __eq(t.id, mi.movie_id) end },
        { items = movie_keyword, on = function(it, mi, t, mk) return __eq(mk.movie_id, t.id) end },
        { items = keyword, on = function(it, mi, t, mk, k) return __eq(k.id, mk.keyword_id) end }
    }, { selectFn = function(it, mi, t, mk, k) return {["rating"]=mi.info, ["title"]=t.title} end, where = function(it, mi, t, mk, k) return (((((__eq(it.info, "rating") and __contains(k.keyword, "sequel")) and (mi.info > "5.0")) and (t.production_year > 2005)) and __eq(mk.movie_id, mi.movie_id))) end })
end)()
...
__json(result)
local __tests = {
    {name="Q4 returns minimum rating and title for sequels", fn=test_Q4_returns_minimum_rating_and_title_for_sequels},
}
