-- Generated by Mochi compiler v0.10.25 on 2025-07-13T10:54:09Z
...
function test_Q5_finds_the_lexicographically_first_qualifying_title()
    if not (__eq(result, {{["typical_european_movie"]="A Film"}})) then error('expect failed') end
end
...
candidate_titles = (function()
    ...
    return __query(_src, {
        { items = movie_companies, on = function(ct, mc) return __eq(mc.company_type_id, ct.ct_id) end },
        { items = movie_info, on = function(ct, mc, mi) return __eq(mi.movie_id, mc.movie_id) end },
        { items = info_type, on = function(ct, mc, mi, it) return __eq(it.it_id, mi.info_type_id) end },
        { items = title, on = function(ct, mc, mi, it, t) return __eq(t.t_id, mc.movie_id) end }
    }, { selectFn = function(ct, mc, mi, it, t) return t.title end, where = function(ct, mc, mi, it, t) return (((((__eq(ct.kind, "production companies") and __contains(mc.note, "(theatrical)")) and __contains(mc.note, "(France)")) and (t.production_year > 2005)) and (__contains({"Sweden", "Norway", "Germany", "Denmark", "Swedish", "Denish", "Norwegian", "German"}, mi.info)))) end })
end)()
result = {{["typical_european_movie"]=__min(candidate_titles)}}
__json(result)
local __tests = {
    {name="Q5 finds the lexicographically first qualifying title", fn=test_Q5_finds_the_lexicographically_first_qualifying_title},
}
