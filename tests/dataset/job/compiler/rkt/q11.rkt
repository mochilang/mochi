...
(define matches (for*/list ([cn company_name] [mc movie_companies] [ct company_type] [t title] [mk movie_keyword] [k keyword] [ml movie_link] [lt link_type] #:when (and (equal? (hash-ref mc 'company_id) (hash-ref cn 'id)) (equal? (hash-ref ct 'id) (hash-ref mc 'company_type_id)) (equal? (hash-ref t 'id) (hash-ref mc 'movie_id)) (equal? (hash-ref mk 'movie_id) (hash-ref t 'id)) (equal? (hash-ref k 'id) (hash-ref mk 'keyword_id)) (equal? (hash-ref ml 'movie_id) (hash-ref t 'id)) (equal? (hash-ref lt 'id) (hash-ref ml 'link_type_id)) (and (and (and (and (and (and (and (and (and (and (not (string=? (hash-ref cn 'country_code) "[pl]")) (or (regexp-match? (regexp "Film") (hash-ref cn 'name)) (regexp-match? (regexp "Warner") (hash-ref cn 'name)))) (string=? (hash-ref ct 'kind) "production companies")) (string=? (hash-ref k 'keyword) "sequel")) (regexp-match? (regexp "follow") (hash-ref lt 'link))) (equal? (hash-ref mc 'note) '())) (>= (hash-ref t 'production_year) 1950)) (<= (hash-ref t 'production_year) 2000)) (equal? (hash-ref ml 'movie_id) (hash-ref mk 'movie_id))) (equal? (hash-ref ml 'movie_id) (hash-ref mc 'movie_id))) (equal? (hash-ref mk 'movie_id) (hash-ref mc 'movie_id))))) (hash 'company (hash-ref cn 'name) 'link (hash-ref lt 'link) 'title (hash-ref t 'title))))
