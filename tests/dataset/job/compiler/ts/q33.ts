// Generated by Mochi compiler v0.10.25 on 2025-07-15T03:09:12Z
  ...
  rows = (() => {
    ...
    for (const cn1 of _src) {
      for (const mc1 of movie_companies) {
        if (!(_equal(cn1.id, mc1.company_id))) continue;
        for (const t1 of title) {
          if (!(_equal(t1.id, mc1.movie_id))) continue;
          for (const mi_idx1 of movie_info_idx) {
            if (!(_equal(mi_idx1.movie_id, t1.id))) continue;
            for (const it1 of info_type) {
              if (!(_equal(it1.id, mi_idx1.info_type_id))) continue;
              for (const kt1 of kind_type) {
                if (!(_equal(kt1.id, t1.kind_id))) continue;
                for (const ml of movie_link) {
                  ...
                  for (const t2 of title) {
                    ...
                    for (const mi_idx2 of movie_info_idx) {
                      if (!(_equal(mi_idx2.movie_id, t2.id))) continue;
                      for (const it2 of info_type) {
                        if (!(_equal(it2.id, mi_idx2.info_type_id))) continue;
                        for (const kt2 of kind_type) {
                          if (!(_equal(kt2.id, t2.kind_id))) continue;
                          for (const mc2 of movie_companies) {
                            if (!(_equal(mc2.movie_id, t2.id))) continue;
                            for (const cn2 of company_name) {
                              if (!(_equal(cn2.id, mc2.company_id))) continue;
                              for (const lt of link_type) {
                                if (!(_equal(lt.id, ml.link_type_id))) continue;
                                if (
                                  !((((((((_equal(cn1.country_code, "[us]") &&
                                    _equal(it1.info, "rating")) &&
                                    _equal(it2.info, "rating")) &&
                                    _equal(kt1.kind, "tv series")) &&
                                    _equal(kt2.kind, "tv series")) &&
                                    ((_equal(lt.link, "sequel") ||
                                      _equal(lt.link, "follows")) ||
                                      _equal(lt.link, "followed by"))) &&
                                    (mi_idx2.info < "3.0")) &&
