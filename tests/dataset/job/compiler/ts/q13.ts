            ...
            for (const kt of kind_type) {
              ...
              for (const mi of movie_info) {
                ...
                for (const it2 of info_type) {
                  if (!(_equal(it2.id, mi.info_type_id))) continue;
                  for (const miidx of movie_info_idx) {
                    if (!(_equal(miidx.movie_id, t.id))) continue;
                    for (const it of info_type) {
                      if (!(_equal(it.id, miidx.info_type_id))) continue;
                      if (
                        !((((_equal(cn.country_code, "[de]") &&
                          _equal(ct.kind, "production companies")) &&
                          _equal(it.info, "rating")) &&
                          _equal(it2.info, "release dates")) &&
