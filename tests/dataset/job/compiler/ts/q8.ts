// Generated by Mochi compiler v0.10.25 on 2025-07-13T11:58:06Z
  ...
  eligible = (() => {
    ...
    for (const an1 of _src) {
      for (const n1 of name) {
        if (!(_equal(n1.id, an1.person_id))) continue;
        for (const ci of cast_info) {
          if (!(_equal(ci.person_id, an1.person_id))) continue;
          for (const t of title) {
            if (!(_equal(t.id, ci.movie_id))) continue;
            for (const mc of movie_companies) {
              if (!(_equal(mc.movie_id, ci.movie_id))) continue;
