  ...
  candidate_titles = (() => {
    ...
    for (const ct of _src) {
      for (const mc of movie_companies) {
        if (!(_equal(mc.company_type_id, ct.ct_id))) continue;
        for (const mi of movie_info) {
          if (!(_equal(mi.movie_id, mc.movie_id))) continue;
          for (const it of info_type) {
            if (!(_equal(it.it_id, mi.info_type_id))) continue;
            for (const t of title) {
              if (!(_equal(t.t_id, mc.movie_id))) continue;
              if (
