; Generated by Mochi compiler v0.10.25 on 2025-07-13T11:54:05Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define candidate_titles (let ((_res '()))
          ...
          (when (equal? (map-get mi 'movie_id) (map-get mk 'movie_id))
            (for-each (lambda (t)
              (when (equal? (map-get t 'id) (map-get mi 'movie_id))
                (when (and (and (and (if (string-contains (map-get k 'keyword) "sequel") #t #f) (if (member (map-get mi 'info) allowed_infos) #t #f)) (_gt (map-get t 'production_year) 2005)) (equal? (map-get mk 'movie_id) (map-get mi 'movie_id)))
  ...
  _res))
(define result (list (list (cons 'movie_title (_min candidate_titles)))))
(_json result)
