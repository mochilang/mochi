; Generated by Mochi compiler v0.10.25 on 2025-07-13T11:54:03Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define filtered (let ((_res '()))
              ...
              (when (equal? (map-get mi 'movie_id) (map-get t 'id))
                (for-each (lambda (it)
                  (when (equal? (map-get it 'id) (map-get mi 'info_type_id))
                    (when (and (and (and (equal? (map-get ct 'kind) "production companies") (equal? (map-get it 'info) "top 250 rank")) (not (if (string-contains (map-get mc 'note) "(as Metro-Goldwyn-Mayer Pictures)") #t #f))) (or (if (string-contains (map-get mc 'note) "(co-production)") #t #f) (if (string-contains (map-get mc 'note) "(presents)") #t #f)))
  ...
  _res))
(define result (list (cons 'production_note (_min (let ((_res '()))
  (for-each (lambda (r)
  ...
  _res))) (cons 'movie_year (_min (let ((_res '()))
  (for-each (lambda (r)
