; Generated by Mochi compiler v0.10.25 on 2025-07-13T11:54:08Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define candidate_titles (let ((_res '()))
              ...
              (when (equal? (map-get it 'it_id) (map-get mi 'info_type_id))
                (for-each (lambda (t)
                  (when (equal? (map-get t 't_id) (map-get mc 'movie_id))
                    (when (and (and (and (and (equal? (map-get ct 'kind) "production companies") (if (member "(theatrical)" (map-get mc 'note)) #t #f)) (if (member "(France)" (map-get mc 'note)) #t #f)) (_gt (map-get t 'production_year) 2005)) (if (member (map-get mi 'info) (list "Sweden" "Norway" "Germany" "Denmark" "Swedish" "Denish" "Norwegian" "German")) #t #f))
  ...
  _res))
(define result (list (list (cons 'typical_european_movie (_min candidate_titles)))))
(_json result)
