; Generated by Mochi compiler v0.10.25 on 2025-07-13T11:54:14Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define eligible (let ((_res '()))
                      ...
                      (when (equal? (map-get cn 'id) (map-get mc 'company_id))
                        (for-each (lambda (rt)
                          (when (equal? (map-get rt 'id) (map-get ci 'role_id))
                            (when (and (and (and (and (and (and (equal? (map-get ci 'note) "(voice: English version)") (equal? (map-get cn 'country_code) "[jp]")) (if (string-contains (map-get mc 'note) "(Japan)") #t #f)) (not (if (string-contains (map-get mc 'note) "(USA)") #t #f))) (if (string-contains (map-get n1 'name) "Yo") #t #f)) (not (if (string-contains (map-get n1 'name) "Yu") #t #f))) (equal? (map-get rt 'role) "actress"))
  ...
  _res))
(define result (list (list (cons 'actress_pseudonym (_min (let ((_res '()))
  (for-each (lambda (x)
    (set! _res (append _res (list (map-get x 'pseudonym))))
  ) (if (string? eligible) (string->list eligible) eligible))
  _res))) (cons 'japanese_movie_dubbed (_min (let ((_res '()))
  (for-each (lambda (x)
