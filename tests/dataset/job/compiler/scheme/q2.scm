; Generated by Mochi compiler v0.10.25 on 2025-07-13T11:54:04Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
              ...
              (when (equal? (map-get mk 'movie_id) (map-get t 'id))
                (for-each (lambda (k)
                  (when (equal? (map-get mk 'keyword_id) (map-get k 'id))
                    (when (and (and (equal? (map-get cn 'country_code) "[de]") (equal? (map-get k 'keyword) "character-name-in-title")) (equal? (map-get mc 'movie_id) (map-get mk 'movie_id)))
