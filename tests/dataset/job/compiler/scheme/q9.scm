; Generated by Mochi compiler v0.10.25 on 2025-07-13T11:58:03Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
                          ...
                          (when (equal? (map-get cn 'id) (map-get mc 'company_id))
                            (for-each (lambda (rt)
                              (when (equal? (map-get rt 'id) (map-get ci 'role_id))
                                (when (and (and (and (and (and (and (and (if (member (map-get ci 'note) (list "(voice)" "(voice: Japanese version)" "(voice) (uncredited)" "(voice: English version)")) #t #f) (equal? (map-get cn 'country_code) "[us]")) (or (if (string-contains (map-get mc 'note) "(USA)") #t #f) (if (string-contains (map-get mc 'note) "(worldwide)") #t #f))) (equal? (map-get n 'gender) "f")) (if (string-contains (map-get n 'name) "Ang") #t #f)) (equal? (map-get rt 'role) "actress")) (_ge (map-get t 'production_year) 2005)) (_le (map-get t 'production_year) 2015))
