...
function main(): void {
  ...
  ss98 = _sum(
    store_sales.filter((ss) => (ss.ss_sold_date_sk == 1998)).map((ss) =>
      ss.ss_ext_list_price
    ),
  );
  ss99 = _sum(
    store_sales.filter((ss) => (ss.ss_sold_date_sk == 1999)).map((ss) =>
      ss.ss_ext_list_price
    ),
  );
  ws98 = _sum(
    web_sales.filter((ws) => (ws.ws_sold_date_sk == 1998)).map((ws) =>
      ws.ws_ext_list_price
    ),
  );
  ws99 = _sum(
    web_sales.filter((ws) => (ws.ws_sold_date_sk == 1999)).map((ws) =>
      ws.ws_ext_list_price
    ),
  );
