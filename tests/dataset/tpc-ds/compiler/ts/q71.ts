  ...
  union_sales = [].concat(
    (() => {
      ...
      for (const ws of _src) {
        for (const d of date_dim) {
          if (!(_equal(d.d_date_sk, ws.ws_sold_date_sk))) continue;
    ...
    (() => {
      ...
      for (const cs of _src) {
        for (const d of date_dim) {
          if (!(_equal(d.d_date_sk, cs.cs_sold_date_sk))) continue;
          if (!((d.d_moy == month) && (d.d_year == year))) continue;
          _res.push({
    ...
    (() => {
      ...
      for (const ss of _src) {
        for (const d of date_dim) {
          if (!(_equal(d.d_date_sk, ss.ss_sold_date_sk))) continue;
          if (!((d.d_moy == month) && (d.d_year == year))) continue;
          _res.push({
  ...
  result = (() => {
    ...
    for (const i of _src) {
      for (const s of union_sales) {
        if (!(_equal(s.item_sk, i.i_item_sk))) continue;
        for (const t of time_dim) {
          if (!(_equal(t.t_time_sk, s.time_sk))) continue;
          if (
