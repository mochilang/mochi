    ...
    return _query(_src, [
      {
        ...
        on: (
          cs,
          cr,
        ) => (_equal(cs.order, cr.order) && _equal(cs.item_sk, cr.item_sk)),
        left: true,
      },
      {
        items: warehouse,
        on: (cs, cr, w) => (_equal(cs.warehouse_sk, w.warehouse_sk)),
      },
      { items: item, on: (cs, cr, w, i) => (_equal(cs.item_sk, i.item_sk)) },
      {
        items: date_dim,
        on: (cs, cr, w, i, d) => (_equal(cs.date_sk, d.date_sk)),
      },
    ], {
      select: (cs, cr, w, i, d) => ({
        "w_state": w.state,
