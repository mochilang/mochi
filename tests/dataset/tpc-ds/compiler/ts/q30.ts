// Generated by Mochi compiler v0.10.25 on 2025-07-15T03:49:40Z
  ...
  customer_total_return = (() => {
    ...
    for (const wr of _src) {
      for (const d of date_dim) {
        if (!(_equal(wr.wr_returned_date_sk, d.d_date_sk))) continue;
        for (const ca of customer_address) {
          if (!(_equal(wr.wr_returning_addr_sk, ca.ca_address_sk))) continue;
          if (!((d.d_year == 2000) && _equal(ca.ca_state, "CA"))) continue;
          const _key = {
  ...
  result = (() => {
    ...
    for (const ctr of _src) {
      for (const avg of avg_by_state) {
        if (!(_equal(ctr.ctr_state, avg.state))) continue;
        for (const c of customer) {
          if (!(_equal(ctr.ctr_customer_sk, c.c_customer_sk))) continue;
