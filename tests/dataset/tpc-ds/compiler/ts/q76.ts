...
function main(): void {
  ...
  store_part = (() => {
    ...
    for (const ss of _src) {
      for (const i of item) {
        if (!(_equal(i.i_item_sk, ss.ss_item_sk))) continue;
        for (const d of date_dim) {
          if (!(_equal(d.d_date_sk, ss.ss_sold_date_sk))) continue;
          if (!(_equal(ss.ss_customer_sk, null))) continue;
          _res.push({
            "channel": "store",
  ...
  web_part = (() => {
    ...
    for (const ws of _src) {
      for (const i of item) {
        if (!(_equal(i.i_item_sk, ws.ws_item_sk))) continue;
        for (const d of date_dim) {
          if (!(_equal(d.d_date_sk, ws.ws_sold_date_sk))) continue;
          if (!(_equal(ws.ws_bill_customer_sk, null))) continue;
          _res.push({
            "channel": "web",
    ...
    for (const cs of _src) {
      for (const i of item) {
        ...
        for (const d of date_dim) {
          ...
          if (!(_equal(cs.cs_bill_customer_sk, null))) continue;
          _res.push({
            "channel": "catalog",
