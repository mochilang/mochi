// Generated by Mochi compiler v0.10.25 on 2025-07-15T03:24:23Z
  ...
  customer_total_return = (() => {
    ...
    for (const sr of _src) {
      for (const d of date_dim) {
        if (
          !(_equal(sr.sr_returned_date_sk, d.d_date_sk) && (d.d_year == 1998))
        ) continue;
        const _key = {
  ...
  result = (() => {
    ...
    for (const ctr1 of _src) {
      for (const s of store) {
        if (!(_equal(ctr1.ctr_store_sk, s.s_store_sk))) continue;
        for (const c of customer) {
          if (!(_equal(ctr1.ctr_customer_sk, c.c_customer_sk))) continue;
          if (
