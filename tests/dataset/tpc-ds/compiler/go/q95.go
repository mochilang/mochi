...
func main() {
	...
	web_returns = []Web_return{Web_return{Wr_order_number: 1}}
		...
		for _, ws1 := range web_sales {
			for _, ws2 := range web_sales {
				if (ws1.Ws_order_number == ws2.Ws_order_number) && (ws1.Ws_warehouse_sk != ws2.Ws_warehouse_sk) {
					if (ws1.Ws_order_number == ws2.Ws_order_number) && (ws1.Ws_warehouse_sk != ws2.Ws_warehouse_sk) {
						results = append(results, Ws_wh{Ws_order_number: ws1.Ws_order_number})
					}
				}
			}
		}
			...
			for _, d := range date_dim {
				...
				for _, ca := range customer_address {
					...
					for _, w := range web_site {
						...
						if (((ca.Ca_state == "CA") && (w.Web_company_name == "pri")) && slices.Contains((func() []int {
							results := []int{}
							for _, x := range ws_wh {
								results = append(results, x.Ws_order_number)
							}
							return results
						}()), ws.Ws_order_number)) && slices.Contains((func() []int {
							results := []int{}
							for _, wr := range web_returns {
								results = append(results, wr.Wr_order_number)
							}
							return results
						}()), ws.Ws_order_number) {
							if (((ca.Ca_state == "CA") && (w.Web_company_name == "pri")) && slices.Contains((func() []int {
