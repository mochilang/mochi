...
func main() {
	...
	groups = _convSlice[v, Group](func() []Group {
			...
			for _, d := range date_dim {
				...
				for _, s := range store {
					...
					for _, hd := range household_demographics {
						...
						if ((((((d.D_dom >= 1) && (d.D_dom <= 2)) && ((hd.Hd_buy_potential == "1001-5000") || (hd.Hd_buy_potential == "0-500"))) && (hd.Hd_vehicle_count > 0)) && ((float64(hd.Hd_dep_count) / float64(hd.Hd_vehicle_count)) > float64(1))) && (((d.D_year == 1998) || (d.D_year == 1999)) || (d.D_year == 2000))) && (s.S_county == "A") {
							key := GKey{
	...
	result = func() []Result {
		...
		resAny := _query(src, []_joinSpec{
			{items: _toAnySlice(customer), on: func(_a ...any) bool {
				...
				_ = c
				return _equal(c.C_customer_sk, _getField(g.Key, "cust"))
			...
			return Result{
				...
				Ss_ticket_number:      _getField(g.Key, "ticket"),
				Cnt:                   g.Cnt,
			}
			...
			_ = c
			return []any{-g.Cnt, c.C_last_name}
		}, skip: -1, take: -1})
