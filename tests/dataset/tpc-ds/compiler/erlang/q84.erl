#!/usr/bin/env escript
% Generated by Mochi compiler v0.10.25 on 2025-07-15T04:58:53Z
...
main(_) ->
    ...
    Result = lists:sum([maps:get(amt, Sr) || C <- Customer, Ca <- Customer_address, Cd <- Customer_demographics, Sr <- Store_returns, Hd <- Household_demographics, Ib <- Income_band, ((maps:get(current_addr, C) == maps:get(ca_address_sk, Ca)) andalso (maps:get(ca_city, Ca) == "Springfield")), (maps:get(cdemo, C) == maps:get(cd_demo_sk, Cd)), (maps:get(cd_demo_sk, Cd) == maps:get(sr_cdemo_sk, Sr)), (maps:get(hdemo, C) == maps:get(hd_demo_sk, Hd)), (maps:get(income_band_sk, Hd) == maps:get(ib_income_band_sk, Ib))]),
    mochi_json(Result),
