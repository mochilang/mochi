#!/usr/bin/env escript
% Generated by Mochi compiler v0.10.25 on 2025-07-15T04:58:30Z
...
main(_) ->
    ...
    Store_ids = distinct([maps:get(ss_customer_sk, S) || S <- Store_sales, ((maps:get(d_month_seq, S) >= 1200) andalso (maps:get(d_month_seq, S) =< 1211))]),
    Catalog_ids = distinct([maps:get(cs_bill_customer_sk, C) || C <- Catalog_sales, ((maps:get(d_month_seq, C) >= 1200) andalso (maps:get(d_month_seq, C) =< 1211))]),
    Web_ids = distinct([maps:get(ws_bill_customer_sk, W) || W <- Web_sales, ((maps:get(d_month_seq, W) >= 1200) andalso (maps:get(d_month_seq, W) =< 1211))]),
