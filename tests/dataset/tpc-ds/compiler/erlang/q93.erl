#!/usr/bin/env escript
% Generated by Mochi compiler v0.10.25 on 2025-07-15T04:58:58Z
...
main(_) ->
    ...
    Result = [V || {_, V} <- lists:keysort(1, [{[lists:sum([maps:get(act_sales, Y) || Y <- Val0]), Key0], #{ss_customer_sk => Key0, sumsales => lists:sum([maps:get(act_sales, Y) || Y <- Val0])}} || {Key0, Val0} <- maps:to_list(lists:foldl(fun({Key0, Val0}, Acc0) -> L = maps:get(Key0, Acc0, []), maps:put(Key0, [Val0 | L], Acc0) end, #{}, [{maps:get(ss_customer_sk, X), X} || X <- T]))])],
    mochi_json(Result),
