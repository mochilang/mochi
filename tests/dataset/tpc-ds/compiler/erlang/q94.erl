#!/usr/bin/env escript
% Generated by Mochi compiler v0.10.26 on 2025-07-15T06:32:11Z
% q94.erl - generated from q94.mochi
...
main(_) ->
    ...
    Filtered = [Ws || Ws <- Web_sales, D <- Date_dim, Ca <- Customer_address, W <- Web_site, (maps:get(ws_ship_date_sk, Ws) == maps:get(d_date_sk, D)), (maps:get(ws_ship_addr_sk, Ws) == maps:get(ca_address_sk, Ca)), (maps:get(ws_web_site_sk, Ws) == maps:get(web_site_sk, W)), ((((maps:get(ca_state, Ca) == "CA") andalso (maps:get(web_company_name, W) == "pri")) andalso lists:any(fun(Ws2) -> ((maps:get(ws_order_number, Ws) == maps:get(ws_order_number, Ws2)) andalso (maps:get(ws_warehouse_sk, Ws) /= maps:get(ws_warehouse_sk, Ws2))) end, Web_sales)) andalso (lists:any(fun(Wr) -> (maps:get(wr_order_number, Wr) == maps:get(ws_order_number, Ws)) end, Web_returns) == false))],
