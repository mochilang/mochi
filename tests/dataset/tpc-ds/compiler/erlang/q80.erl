#!/usr/bin/env escript
% Generated by Mochi compiler v0.10.26 on 2025-07-15T06:32:00Z
...
main(_) ->
    ...
    Total_profit = ((lists:sum([(maps:get(price, S) - maps:get(ret, S)) || S <- Store_sales]) + lists:sum([(maps:get(price, C) - maps:get(ret, C)) || C <- Catalog_sales])) + lists:sum([(maps:get(price, W) - maps:get(ret, W)) || W <- Web_sales])),
