#!/usr/bin/env escript
% Generated by Mochi compiler v0.10.26 on 2025-07-15T06:31:06Z
...
main(_) ->
    ...
    Joined = [#{qty => maps:get(ss_quantity, Ss), ret => maps:get(sr_return_quantity, Sr), csq => maps:get(cs_quantity, Cs), i_item_id => maps:get(i_item_id, I), i_item_desc => maps:get(i_item_desc, I), s_state => maps:get(s_state, S)} || Ss <- Store_sales, Sr <- Store_returns, Cs <- Catalog_sales, D1 <- Date_dim, D2 <- Date_dim, D3 <- Date_dim, S <- Store, I <- Item, (((maps:get(ss_customer_sk, Ss) == maps:get(sr_customer_sk, Sr)) andalso (maps:get(ss_item_sk, Ss) == maps:get(sr_item_sk, Sr))) andalso (maps:get(ss_ticket_number, Ss) == maps:get(sr_ticket_number, Sr))), ((maps:get(sr_customer_sk, Sr) == maps:get(cs_bill_customer_sk, Cs)) andalso (maps:get(sr_item_sk, Sr) == maps:get(cs_item_sk, Cs))), ((maps:get(ss_sold_date_sk, Ss) == maps:get(d_date_sk, D1)) andalso (maps:get(d_quarter_name, D1) == "1998Q1")), ((maps:get(sr_returned_date_sk, Sr) == maps:get(d_date_sk, D2)) andalso lists:member(maps:get(d_quarter_name, D2), ["1998Q1", "1998Q2", "1998Q3"])), ((maps:get(cs_sold_date_sk, Cs) == maps:get(d_date_sk, D3)) andalso lists:member(maps:get(d_quarter_name, D3), ["1998Q1", "1998Q2", "1998Q3"])), (maps:get(ss_store_sk, Ss) == maps:get(s_store_sk, S)), (maps:get(ss_item_sk, Ss) == maps:get(i_item_sk, I))],
