...
function test_TPCDS_Q53_simplified()
    if not (__eq(result, {{["i_manufact_id"]=1, ["sum_sales"]=20.0}, {["i_manufact_id"]=2, ["sum_sales"]=53.0}})) then error('expect failed') end
end

item = {{["i_item_sk"]=1, ["i_manufact_id"]=1}, {["i_item_sk"]=2, ["i_manufact_id"]=2}}
store_sales = {{["item"]=1, ["date"]=1, ["price"]=10.0}, {["item"]=1, ["date"]=2, ["price"]=10.0}, {["item"]=2, ["date"]=1, ["price"]=30.0}, {["item"]=2, ["date"]=2, ["price"]=23.0}}
date_dim = {{["d_date_sk"]=1, ["d_month_seq"]=1}, {["d_date_sk"]=2, ["d_month_seq"]=2}}
grouped = (function()
    local _src = store_sales
    local _rows = __query(_src, {
        { items = item, on = function(ss, i) return __eq(ss.item, i.i_item_sk) end },
        { items = date_dim, on = function(ss, i, d) return __eq(ss.date, d.d_date_sk) end }
    }, { selectFn = function(ss, i, d) return {ss, i, d} end })
    local _groups = __group_by_rows(_rows, function(ss, i, d) return i.i_manufact_id end, function(ss, i, d) local _row = __merge(ss, i, d); _row.ss = ss; _row.i = i; _row.d = d; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["manu"]=g.key, ["sum_sales"]=__sum((function()
    local _res = {}
    for _, x in ipairs(g.items) do
        _res[#_res+1] = x.price
    end
    return _res
end)()), ["avg_sales"]=__avg((function()
    local _res = {}
    for _, x in ipairs(g.items) do
        _res[#_res+1] = x.price
    end
...
result = (function()
    ...
    for _, g in ipairs(grouped) do
        if ((g.avg_sales > 0.0) and (__div(math.abs((g.sum_sales - g.avg_sales)), g.avg_sales) > 0.1)) then
            _res[#_res+1] = {["i_manufact_id"]=g.manu, ["sum_sales"]=g.sum_sales}
        end
    end
    return _res
end)()
__json(result)
local __tests = {
    {name="TPCDS Q53 simplified", fn=test_TPCDS_Q53_simplified},
}
