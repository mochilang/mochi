...
function test_TPCDS_Q73_simplified()
    if not (__eq(result, {{["c_last_name"]="Smith", ["c_first_name"]="Alice", ["c_salutation"]="Ms.", ["c_preferred_cust_flag"]="Y", ["ss_ticket_number"]=1, ["cnt"]=1}})) then error('expect failed') end
end
...
groups = (function()
    ...
    }, { selectFn = function(ss, d, s, hd) return {ss, d, s, hd} end, where = function(ss, d, s, hd) return ((((((((d.d_dom >= 1) and (d.d_dom <= 2)) and ((__eq(hd.hd_buy_potential, "1001-5000") or __eq(hd.hd_buy_potential, "0-500")))) and (hd.hd_vehicle_count > 0)) and (__div(hd.hd_dep_count, hd.hd_vehicle_count) > 1)) and ((((d.d_year == 1998) or (d.d_year == 1999)) or (d.d_year == 2000)))) and __eq(s.s_county, "A"))) end })
    ...
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["key"]=g.key, ["cnt"]=__count(g)}
    end
    return _res
end)()
result = (function()
    local _src = groups
    return __query(_src, {
        { items = customer, on = function(g, c) return __eq(c.c_customer_sk, g.key.cust) end }
    }, { selectFn = function(g, c) return {["c_last_name"]=c.c_last_name, ["c_first_name"]=c.c_first_name, ["c_salutation"]=c.c_salutation, ["c_preferred_cust_flag"]=c.c_preferred_cust_flag, ["ss_ticket_number"]=g.key.ticket, ["cnt"]=g.cnt} end, where = function(g, c) return (((g.cnt >= 1) and (g.cnt <= 5))) end, sortKey = function(g, c) return ({-g.cnt, c.c_last_name}) end })
end)()
__json(result)
local __tests = {
    {name="TPCDS Q73 simplified", fn=test_TPCDS_Q73_simplified},
}
