...
function test_TPCDS_Q94_shipping()
...
filtered = (function()
    ...
    }, { selectFn = function(ws, d, ca, w) return ws end, where = function(ws, d, ca, w) return ((((__eq(ca.ca_state, "CA") and __eq(w.web_company_name, "pri")) and __exists((function()
    local _res = {}
    for _, ws2 in ipairs(web_sales) do
        if (__eq(ws.ws_order_number, ws2.ws_order_number) and not __eq(ws.ws_warehouse_sk, ws2.ws_warehouse_sk)) then
            _res[#_res+1] = ws2
        end
    end
    return _res
    ...
    for _, wr in ipairs(web_returns) do
        if __eq(wr.wr_order_number, ws.ws_order_number) then
            _res[#_res+1] = wr
        end
    end
    return _res
...
__json(result)
local __tests = {
    {name="TPCDS Q94 shipping", fn=test_TPCDS_Q94_shipping},
}
