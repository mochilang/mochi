...
function test_TPCDS_Q70_simplified()
    if not (__eq(result, {{["s_state"]="CA", ["s_county"]="Orange", ["total_sum"]=15.0}, {["s_state"]="TX", ["s_county"]="Travis", ["total_sum"]=20.0}})) then error('expect failed') end
end
...
store_sales = {{["ss_sold_date_sk"]=1, ["ss_store_sk"]=1, ["ss_net_profit"]=10.0}, {["ss_sold_date_sk"]=1, ["ss_store_sk"]=2, ["ss_net_profit"]=5.0}, {["ss_sold_date_sk"]=2, ["ss_store_sk"]=3, ["ss_net_profit"]=20.0}}
dms = 1200
result = (function()
    ...
    }, { selectFn = function(ss, d, s) return {ss, d, s} end, where = function(ss, d, s) return (((d.d_month_seq >= dms) and (d.d_month_seq <= (dms + 11)))) end })
    local _groups = __group_by_rows(_rows, function(ss, d, s) return {["state"]=s.s_state, ["county"]=s.s_county} end, function(ss, d, s) local _row = __merge(ss, d, s); _row.ss = ss; _row.d = d; _row.s = s; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["s_state"]=g.key.state, ["s_county"]=g.key.county, ["total_sum"]=__sum((function()
    local _res = {}
...
__json(result)
local __tests = {
    {name="TPCDS Q70 simplified", fn=test_TPCDS_Q70_simplified},
}
