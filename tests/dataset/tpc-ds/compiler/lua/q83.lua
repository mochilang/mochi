...
function test_TPCDS_Q83_sample()
    if not ((result == 83)) then error('expect failed') end
end
...
result = ((__sum((function()
    local _res = {}
    for _, x in ipairs(sr_items) do
        _res[#_res+1] = x.qty
    end
    return _res
end)()) + __sum((function()
    local _res = {}
    for _, x in ipairs(cr_items) do
        _res[#_res+1] = x.qty
    end
    return _res
end)())) + __sum((function()
    local _res = {}
    for _, x in ipairs(wr_items) do
        _res[#_res+1] = x.qty
    end
    return _res
end)()))
__json(result)
local __tests = {
    {name="TPCDS Q83 sample", fn=test_TPCDS_Q83_sample},
}
