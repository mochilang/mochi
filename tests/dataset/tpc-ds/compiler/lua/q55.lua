...
function test_TPCDS_Q55_simplified()
    if not (__eq(result, {{["brand_id"]=10, ["ext_price"]=35.0}, {["brand_id"]=20, ["ext_price"]=20.0}})) then error('expect failed') end
end

store_sales = {{["item"]=1, ["sold_date"]=1, ["price"]=10.0}, {["item"]=2, ["sold_date"]=1, ["price"]=20.0}, {["item"]=3, ["sold_date"]=1, ["price"]=25.0}}
...
grouped = (function()
    ...
    local _rows = __query(_src, {
        ...
        { items = date_dim, on = function(ss, i, d) return __eq(ss.sold_date, d.d_date_sk) end }
    }, { selectFn = function(ss, i, d) return {ss, i, d} end })
    local _groups = __group_by_rows(_rows, function(ss, i, d) return {["brand_id"]=i.i_brand_id} end, function(ss, i, d) local _row = __merge(ss, i, d); _row.ss = ss; _row.i = i; _row.d = d; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["brand_id"]=g.key.brand_id, ["ext_price"]=__sum((function()
    local _res = {}
...
result = (function()
    ...
    for _, g in ipairs(grouped) do
        _res[#_res+1] = {__key = {-g.ext_price, g.brand_id}, __val = g}
    end
    local items = _res
...
__json(result)
local __tests = {
    {name="TPCDS Q55 simplified", fn=test_TPCDS_Q55_simplified},
}
