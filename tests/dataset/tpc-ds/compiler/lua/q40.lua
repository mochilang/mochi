...
function test_TPCDS_Q40_simplified()
    if not (__eq(result, {{["w_state"]="CA", ["i_item_id"]="I1", ["sales_before"]=100.0, ["sales_after"]=0.0}})) then error('expect failed') end
end

catalog_sales = {{["order"]=1, ["item_sk"]=1, ["warehouse_sk"]=1, ["date_sk"]=1, ["price"]=100.0}, {["order"]=2, ["item_sk"]=1, ["warehouse_sk"]=1, ["date_sk"]=2, ["price"]=150.0}}
catalog_returns = {{["order"]=2, ["item_sk"]=1, ["refunded"]=150.0}}
...
records = (function()
    ...
    return __query(_src, {
        { items = catalog_returns, on = function(cs, cr) return (__eq(cs.order, cr.order) and __eq(cs.item_sk, cr.item_sk)) end, left = true },
        { items = warehouse, on = function(cs, cr, w) return __eq(cs.warehouse_sk, w.warehouse_sk) end },
        { items = item, on = function(cs, cr, w, i) return __eq(cs.item_sk, i.item_sk) end },
        { items = date_dim, on = function(cs, cr, w, i, d) return __eq(cs.date_sk, d.date_sk) end }
    }, { selectFn = function(cs, cr, w, i, d) return {["w_state"]=w.state, ["i_item_id"]=i.item_id, ["sold_date"]=d.date, ["net"]=(cs.price - ((function()
    if __eq(cr, nil) then
        return 0.0
    else
        return cr.refunded
    end
end)()))} end, where = function(cs, cr, w, i, d) return (((i.current_price >= 0.99) and (i.current_price <= 1.49))) end })
end)()
result = (function()
    local _groups = __group_by(records, function(r) return {["w_state"]=r.w_state, ["i_item_id"]=r.i_item_id} end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["w_state"]=g.key.w_state, ["i_item_id"]=g.key.i_item_id, ["sales_before"]=__sum((function()
    local _res = {}
    ...
    return _res
end)()), ["sales_after"]=__sum((function()
    local _res = {}
    for _, x in ipairs(g.items) do
        _res[#_res+1] = (function()
    if (x.sold_date >= sales_date) then
...
__json(result)
local __tests = {
    {name="TPCDS Q40 simplified", fn=test_TPCDS_Q40_simplified},
}
