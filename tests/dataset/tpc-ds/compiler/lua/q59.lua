...
function test_TPCDS_Q59_simplified()
    if not (__eq(result, {{["s_store_id1"]=1, ["ratio"]=1.5}})) then error('expect failed') end
end

sales_year1 = {{["week"]=1, ["store"]=1, ["amount"]=100.0}}
sales_year2 = {{["week"]=53, ["store"]=1, ["amount"]=150.0}}
join = (function()
    local _src = sales_year1
    return __query(_src, {
        { items = sales_year2, on = function(y1, y2) return (__eq(y1.store, y2.store) and __eq(y1.week, (y2.week - 52))) end }
    }, { selectFn = function(y1, y2) return {["s_store_id1"]=y1.store, ["ratio"]=__div(y2.amount, y1.amount)} end })
end)()
result = join
__json(result)
local __tests = {
    {name="TPCDS Q59 simplified", fn=test_TPCDS_Q59_simplified},
}
