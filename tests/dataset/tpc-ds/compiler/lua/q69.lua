...
function test_TPCDS_Q69_simplified()
    if not ((result == 69)) then error('expect failed') end
end
...
result = (__sum((function()
    ...
    for _, w in ipairs(web_sales) do
        _res[#_res+1] = w.amount
    end
    return _res
end)()) + __sum((function()
    local _res = {}
    for _, s in ipairs(store_sales) do
        _res[#_res+1] = s.amount
    end
    return _res
end)()))
__json(result)
local __tests = {
    {name="TPCDS Q69 simplified", fn=test_TPCDS_Q69_simplified},
}
