...
function test_TPCDS_Q8_result()
    if not (__eq(result, {{["s_store_name"]="Store1", ["net_profit"]=10.0}})) then error('expect failed') end
end

store_sales = {{["ss_store_sk"]=1, ["ss_sold_date_sk"]=1, ["ss_net_profit"]=10.0}}
...
result = (function()
    ...
    local _rows = __query(_src, {
        { items = date_dim, on = function(ss, d) return ((__eq(ss.ss_sold_date_sk, d.d_date_sk) and __eq(d.d_qoy, 1)) and __eq(d.d_year, 1998)) end },
        { items = store, on = function(ss, d, s) return __eq(ss.ss_store_sk, s.s_store_sk) end },
        { items = customer_address, on = function(ss, d, s, ca) return (__slice(s.s_zip, 0, 2) == __slice(ca.ca_zip, 0, 2)) end },
        { items = customer, on = function(ss, d, s, ca, c) return (__eq(ca.ca_address_sk, c.c_current_addr_sk) and __eq(c.c_preferred_cust_flag, "Y")) end }
    }, { selectFn = function(ss, d, s, ca, c) return {ss, d, s, ca, c} end, where = function(ss, d, s, ca, c) return (__contains(zip_list, __slice(ca.ca_zip, 0, 5))) end })
    local _groups = __group_by_rows(_rows, function(ss, d, s, ca, c) return s.s_store_name end, function(ss, d, s, ca, c) local _row = __merge(ss, d, s, ca, c); _row.ss = ss; _row.d = d; _row.s = s; _row.ca = ca; _row.c = c; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["s_store_name"]=g.key, ["net_profit"]=__sum((function()
    local _res = {}
...
__json(result)
local __tests = {
    {name="TPCDS Q8 result", fn=test_TPCDS_Q8_result},
}
