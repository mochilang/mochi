...
function test_TPCDS_Q92_threshold()
    if not ((result == 4.0)) then error('expect failed') end
end

web_sales = {{["ws_item_sk"]=1, ["ws_sold_date_sk"]=1, ["ws_ext_discount_amt"]=1.0}, {["ws_item_sk"]=1, ["ws_sold_date_sk"]=1, ["ws_ext_discount_amt"]=1.0}, {["ws_item_sk"]=1, ["ws_sold_date_sk"]=1, ["ws_ext_discount_amt"]=2.0}}
item = {{["i_item_sk"]=1, ["i_manufact_id"]=1}}
date_dim = {{["d_date_sk"]=1, ["d_date"]="2000-01-02"}}
sum_amt = __sum((function()
    local _res = {}
    for _, ws in ipairs(web_sales) do
        _res[#_res+1] = ws.ws_ext_discount_amt
    end
    return _res
end)())
avg_amt = __avg((function()
    local _res = {}
    for _, ws in ipairs(web_sales) do
        _res[#_res+1] = ws.ws_ext_discount_amt
    end
...
__json(result)
local __tests = {
    {name="TPCDS Q92 threshold", fn=test_TPCDS_Q92_threshold},
}
