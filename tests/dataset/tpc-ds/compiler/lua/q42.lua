...
records = (function()
    ...
    }, { selectFn = function(dt, ss, it) return {["d_year"]=dt.d_year, ["i_category_id"]=it.i_category_id, ["i_category"]=it.i_category, ["price"]=ss.ext_sales_price} end, where = function(dt, ss, it) return (((__eq(it.i_manager_id, 1) and (dt.d_moy == month)) and (dt.d_year == year))) end })
end)()
grouped = (function()
    local _groups = __group_by(records, function(r) return {["d_year"]=r.d_year, ["i_category_id"]=r.i_category_id, ["i_category"]=r.i_category} end)
    local _res = {}
...
base = (function()
    ...
    for _, g in ipairs(grouped) do
        _res[#_res+1] = {__key = -g.sum_ss_ext_sales_price, __val = g}
    end
    local items = _res
