...
function test_TPCDS_Q6_result()
    if not (__eq(result, {{["state"]="CA", ["cnt"]=10}})) then error('expect failed') end
end
...
item = {{["i_item_sk"]=1, ["i_category"]="A", ["i_current_price"]=100.0}, {["i_item_sk"]=2, ["i_category"]="A", ["i_current_price"]=50.0}}
target_month_seq = __max((function()
    local _res = {}
    for _, d in ipairs(date_dim) do
        if ((d.d_year == 1999) and (d.d_moy == 5)) then
...
result = (function()
    ...
    }, { selectFn = function(a, c, s, d, i) return {a, c, s, d, i} end, where = function(a, c, s, d, i) return ((__eq(d.d_month_seq, target_month_seq) and (i.i_current_price > (1.2 * __avg((function()
    local _res = {}
    ...
    local _groups = __group_by_rows(_rows, function(a, c, s, d, i) return a.ca_state end, function(a, c, s, d, i) local _row = __merge(a, c, s, d, i); _row.a = a; _row.c = c; _row.s = s; _row.d = d; _row.i = i; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        if (__count(g) >= 10) then
...
__json(result)
local __tests = {
    {name="TPCDS Q6 result", fn=test_TPCDS_Q6_result},
}
