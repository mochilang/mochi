...
function test_TPCDS_Q98_revenue()
    if not (__eq(result, {{["i_item_id"]="I1", ["i_item_desc"]="desc1", ["i_category"]="CatA", ["i_class"]="Class1", ["i_current_price"]=100.0, ["itemrevenue"]=50.0, ["revenueratio"]=33.333333333333336}, {["i_item_id"]="I2", ["i_item_desc"]="desc2", ["i_category"]="CatB", ["i_class"]="Class1", ["i_current_price"]=200.0, ["itemrevenue"]=100.0, ["revenueratio"]=66.66666666666667}})) then error('expect failed') end
end

store_sales = {{["ss_item_sk"]=1, ["ss_sold_date_sk"]=1, ["ss_ext_sales_price"]=50.0}, {["ss_item_sk"]=2, ["ss_sold_date_sk"]=1, ["ss_ext_sales_price"]=100.0}}
item = {{["i_item_sk"]=1, ["i_item_id"]="I1", ["i_item_desc"]="desc1", ["i_category"]="CatA", ["i_class"]="Class1", ["i_current_price"]=100.0}, {["i_item_sk"]=2, ["i_item_id"]="I2", ["i_item_desc"]="desc2", ["i_category"]="CatB", ["i_class"]="Class1", ["i_current_price"]=200.0}}
date_dim = {{["d_date_sk"]=1, ["d_date"]="2000-02-01"}}
grouped = (function()
    local _src = store_sales
    local _rows = __query(_src, {
        { items = item, on = function(ss, i) return __eq(ss.ss_item_sk, i.i_item_sk) end },
        { items = date_dim, on = function(ss, i, d) return __eq(ss.ss_sold_date_sk, d.d_date_sk) end }
    }, { selectFn = function(ss, i, d) return {ss, i, d} end })
    local _groups = __group_by_rows(_rows, function(ss, i, d) return {["item_id"]=i.i_item_id, ["item_desc"]=i.i_item_desc, ["category"]=i.i_category, ["class"]=i.i_class, ["price"]=i.i_current_price} end, function(ss, i, d) local _row = __merge(ss, i, d); _row.ss = ss; _row.i = i; _row.d = d; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["i_item_id"]=g.key.item_id, ["i_item_desc"]=g.key.item_desc, ["i_category"]=g.key.category, ["i_class"]=g.key.class, ["i_current_price"]=g.key.price, ["itemrevenue"]=__sum((function()
    local _res = {}
...
totals = (function()
    local _groups = __group_by(grouped, function(g) return g.i_class end)
    local _res = {}
    for _, cg in ipairs(_groups) do
        _res[#_res+1] = {["class"]=cg.key, ["total"]=__sum((function()
    local _res = {}
    for _, x in ipairs(cg.items) do
...
result = (function()
    local _src = grouped
    return __query(_src, {
        { items = totals, on = function(g, t) return __eq(g.i_class, t.class) end }
    }, { selectFn = function(g, t) return {["i_item_id"]=g.i_item_id, ["i_item_desc"]=g.i_item_desc, ["i_category"]=g.i_category, ["i_class"]=g.i_class, ["i_current_price"]=g.i_current_price, ["itemrevenue"]=g.itemrevenue, ["revenueratio"]=__div((g.itemrevenue * 100), t.total)} end, sortKey = function(g, t) return ({g.i_category, g.i_class, g.i_item_id}) end })
end)()
__json(result)
local __tests = {
    {name="TPCDS Q98 revenue", fn=test_TPCDS_Q98_revenue},
}
