...
function test_TPCDS_Q68_simplified()
    if not ((result == 68)) then error('expect failed') end
end
...
result = ((__sum((function()
    ...
    for _, c in ipairs(catalog_sales) do
        _res[#_res+1] = c.profit
    end
    return _res
end)()) - __sum((function()
    local _res = {}
    for _, s in ipairs(store_sales) do
        _res[#_res+1] = s.profit
    end
    return _res
end)())) + 30)
__json(result)
local __tests = {
    {name="TPCDS Q68 simplified", fn=test_TPCDS_Q68_simplified},
}
