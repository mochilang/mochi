...
function test_TPCDS_Q56_simplified()
    if not (__eq(result, {{["i_item_id"]=1, ["total_sales"]=60.0}})) then error('expect failed') end
end

store_sales = {{["item"]=1, ["price"]=10.0}}
catalog_sales = {{["item"]=1, ["price"]=20.0}}
web_sales = {{["item"]=1, ["price"]=30.0}}
ss = (function()
    local _groups = __group_by(store_sales, function(s) return s.item end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["item"]=g.key, ["total"]=__sum((function()
    local _res = {}
...
cs = (function()
    local _groups = __group_by(catalog_sales, function(s) return s.item end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["item"]=g.key, ["total"]=__sum((function()
    local _res = {}
...
ws = (function()
    local _groups = __group_by(web_sales, function(s) return s.item end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["item"]=g.key, ["total"]=__sum((function()
    local _res = {}
...
union = __concat(__concat(ss, cs), ws)
result = (function()
    local _groups = __group_by(union, function(u) return u.item end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["i_item_id"]=g.key, ["total_sales"]=__sum((function()
    local _res = {}
    for _, x in ipairs(g.items) do
        _res[#_res+1] = x.total
    end
...
__json(result)
local __tests = {
    {name="TPCDS Q56 simplified", fn=test_TPCDS_Q56_simplified},
}
