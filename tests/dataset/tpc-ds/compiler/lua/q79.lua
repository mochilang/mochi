...
function test_TPCDS_Q79_simplified()
    if not (__eq(result, {{["c_last_name"]="Smith", ["c_first_name"]="Alice", ["s_city"]="CityA", ["ss_ticket_number"]=1, ["amt"]=5.0, ["profit"]=10.0}})) then error('expect failed') end
end
...
store_sales = {{["ss_sold_date_sk"]=1, ["ss_store_sk"]=1, ["ss_ticket_number"]=1, ["ss_customer_sk"]=1, ["ss_hdemo_sk"]=1, ["ss_coupon_amt"]=5.0, ["ss_net_profit"]=10.0}}
...
agg = (function()
    ...
    local _rows = __query(_src, {
        ...
        { items = store, on = function(ss, d, s) return __eq(s.s_store_sk, ss.ss_store_sk) end },
        { items = household_demographics, on = function(ss, d, s, hd) return __eq(hd.hd_demo_sk, ss.ss_hdemo_sk) end }
    }, { selectFn = function(ss, d, s, hd) return {ss, d, s, hd} end, where = function(ss, d, s, hd) return ((((((((hd.hd_dep_count == 2) or (hd.hd_vehicle_count > 1))) and (d.d_dow == 1)) and ((((d.d_year == 1998) or (d.d_year == 1999)) or (d.d_year == 2000)))) and (s.s_number_employees >= 200)) and (s.s_number_employees <= 295))) end })
    local _groups = __group_by_rows(_rows, function(ss, d, s, hd) return {["ticket"]=ss.ss_ticket_number, ["customer_sk"]=ss.ss_customer_sk, ["city"]=s.s_city} end, function(ss, d, s, hd) local _row = __merge(ss, d, s, hd); _row.ss = ss; _row.d = d; _row.s = s; _row.hd = hd; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["key"]=g.key, ["amt"]=__sum((function()
    local _res = {}
...
result = (function()
    local _src = agg
    return __query(_src, {
        { items = customer, on = function(a, c) return __eq(c.c_customer_sk, a.key.customer_sk) end }
    }, { selectFn = function(a, c) return {["c_last_name"]=c.c_last_name, ["c_first_name"]=c.c_first_name, ["s_city"]=a.key.city, ["ss_ticket_number"]=a.key.ticket, ["amt"]=a.amt, ["profit"]=a.profit} end, sortKey = function(a, c) return ({c.c_last_name, c.c_first_name, a.key.city, a.profit}) end })
end)()
__json(result)
local __tests = {
    {name="TPCDS Q79 simplified", fn=test_TPCDS_Q79_simplified},
}
