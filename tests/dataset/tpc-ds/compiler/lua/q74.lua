...
function test_TPCDS_Q74_simplified()
    if not (__eq(result, {{["customer_id"]=1, ["customer_first_name"]="Alice", ["customer_last_name"]="Smith"}})) then error('expect failed') end
end
...
store_sales = {{["ss_customer_sk"]=1, ["ss_sold_date_sk"]=1, ["ss_net_paid"]=100.0}, {["ss_customer_sk"]=1, ["ss_sold_date_sk"]=2, ["ss_net_paid"]=110.0}}
web_sales = {{["ws_bill_customer_sk"]=1, ["ws_sold_date_sk"]=1, ["ws_net_paid"]=40.0}, {["ws_bill_customer_sk"]=1, ["ws_sold_date_sk"]=2, ["ws_net_paid"]=80.0}}
year_total = __concat((function()
    local _src = customer
    local _rows = __query(_src, {
        { items = store_sales, on = function(c, ss) return __eq(c.c_customer_sk, ss.ss_customer_sk) end },
        { items = date_dim, on = function(c, ss, d) return __eq(d.d_date_sk, ss.ss_sold_date_sk) end }
    }, { selectFn = function(c, ss, d) return {c, ss, d} end, where = function(c, ss, d) return (((d.d_year == 1998) or (d.d_year == 1999))) end })
    local _groups = __group_by_rows(_rows, function(c, ss, d) return {["id"]=c.c_customer_id, ["first"]=c.c_first_name, ["last"]=c.c_last_name, ["year"]=d.d_year} end, function(c, ss, d) local _row = __merge(c, ss, d); _row.c = c; _row.ss = ss; _row.d = d; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["customer_id"]=g.key.id, ["customer_first_name"]=g.key.first, ["customer_last_name"]=g.key.last, ["year"]=g.key.year, ["year_total"]=__sum((function()
    local _res = {}
    for _, x in ipairs(g.items) do
        _res[#_res+1] = x.ss.ss_net_paid
    end
    return _res
end)()), ["sale_type"]="s"}
    end
    ...
    local _rows = __query(_src, {
        ...
        { items = date_dim, on = function(c, ws, d) return __eq(d.d_date_sk, ws.ws_sold_date_sk) end }
    }, { selectFn = function(c, ws, d) return {c, ws, d} end, where = function(c, ws, d) return (((d.d_year == 1998) or (d.d_year == 1999))) end })
    local _groups = __group_by_rows(_rows, function(c, ws, d) return {["id"]=c.c_customer_id, ["first"]=c.c_first_name, ["last"]=c.c_last_name, ["year"]=d.d_year} end, function(c, ws, d) local _row = __merge(c, ws, d); _row.c = c; _row.ws = ws; _row.d = d; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["customer_id"]=g.key.id, ["customer_first_name"]=g.key.first, ["customer_last_name"]=g.key.last, ["year"]=g.key.year, ["year_total"]=__sum((function()
    local _res = {}
    for _, x in ipairs(g.items) do
        _res[#_res+1] = x.ws.ws_net_paid
    end
    return _res
end)()), ["sale_type"]="w"}
    end
...
s_firstyear = __first((function()
    local _res = {}
    for _, y in ipairs(year_total) do
        if (__eq(y.sale_type, "s") and __eq(y.year, 1998)) then
            _res[#_res+1] = y
        end
    end
    return _res
end)())
s_secyear = __first((function()
    local _res = {}
    for _, y in ipairs(year_total) do
        if (__eq(y.sale_type, "s") and __eq(y.year, 1999)) then
            _res[#_res+1] = y
        end
    end
    return _res
end)())
w_firstyear = __first((function()
    local _res = {}
    for _, y in ipairs(year_total) do
        if (__eq(y.sale_type, "w") and __eq(y.year, 1998)) then
            _res[#_res+1] = y
        end
    end
    return _res
end)())
w_secyear = __first((function()
    local _res = {}
    for _, y in ipairs(year_total) do
        if (__eq(y.sale_type, "w") and __eq(y.year, 1999)) then
            _res[#_res+1] = y
        end
    end
    return _res
...
result = (function()
    if (((s_firstyear.year_total > 0) and (w_firstyear.year_total > 0)) and ((__div(w_secyear.year_total, w_firstyear.year_total)) > (__div(s_secyear.year_total, s_firstyear.year_total)))) then
        return {{["customer_id"]=s_secyear.customer_id, ["customer_first_name"]=s_secyear.customer_first_name, ["customer_last_name"]=s_secyear.customer_last_name}}
    else
        return {}
    end
end)()
__json(result)
local __tests = {
    {name="TPCDS Q74 simplified", fn=test_TPCDS_Q74_simplified},
}
