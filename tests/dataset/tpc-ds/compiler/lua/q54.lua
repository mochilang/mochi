...
function test_TPCDS_Q54_simplified()
    if not (__eq(result, {{["segment"]=1, ["num_customers"]=1, ["segment_base"]=50}, {["segment"]=0, ["num_customers"]=1, ["segment_base"]=0}})) then error('expect failed') end
end

store_sales = {{["customer"]=1, ["sold_date"]=2, ["price"]=60.0}, {["customer"]=2, ["sold_date"]=2, ["price"]=40.0}}
...
revenue = (function()
    ...
    return __query(_src, {
        { items = date_dim, on = function(ss, d) return __eq(ss.sold_date, d.d_date_sk) end },
        { items = customer, on = function(ss, d, c) return __eq(ss.customer, c.c_customer_sk) end },
        { items = customer_address, on = function(ss, d, c, ca) return ((__eq(c.c_current_addr_sk, ca.ca_address_sk) and __eq(ca.ca_county, "X")) and __eq(ca.ca_state, "Y")) end },
        { items = store, on = function(ss, d, c, ca, s) return ((__eq(1, s.s_store_sk) and __eq(ca.ca_county, s.s_county)) and __eq(ca.ca_state, s.s_state)) end }
    }, { selectFn = function(ss, d, c, ca, s) return {["customer"]=c.c_customer_sk, ["amt"]=ss.price} end })
end)()
by_customer = (function()
    local _groups = __group_by(revenue, function(r) return r.customer end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["customer"]=g.key, ["revenue"]=__sum((function()
    local _res = {}
...
segments = (function()
    local _groups = __group_by(by_customer, function(r) return {["seg"]=int(__div(r.revenue, 50))} end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["segment"]=g.key.seg, ["num_customers"]=__count(g), ["segment_base"]=(g.key.seg * 50)}
    end
    return _res
end)()
result = segments
__json(result)
local __tests = {
    {name="TPCDS Q54 simplified", fn=test_TPCDS_Q54_simplified},
}
