...
function test_TPCDS_Q80_sample()
    if not ((total_profit == 80.0)) then error('expect failed') end
end

store_sales = {{["price"]=20.0, ["ret"]=5.0}, {["price"]=10.0, ["ret"]=2.0}, {["price"]=5.0, ["ret"]=0.0}}
catalog_sales = {{["price"]=15.0, ["ret"]=3.0}, {["price"]=8.0, ["ret"]=1.0}}
web_sales = {{["price"]=25.0, ["ret"]=5.0}, {["price"]=15.0, ["ret"]=8.0}, {["price"]=8.0, ["ret"]=2.0}}
total_profit = ((__sum((function()
    local _res = {}
    for _, s in ipairs(store_sales) do
        _res[#_res+1] = (s.price - s.ret)
    end
    return _res
end)()) + __sum((function()
    local _res = {}
    for _, c in ipairs(catalog_sales) do
        _res[#_res+1] = (c.price - c.ret)
    end
    return _res
end)())) + __sum((function()
    local _res = {}
    for _, w in ipairs(web_sales) do
        _res[#_res+1] = (w.price - w.ret)
    end
    return _res
end)()))
__json(total_profit)
local __tests = {
    {name="TPCDS Q80 sample", fn=test_TPCDS_Q80_sample},
}
