...
function test_TPCDS_Q39_simplified()
    if not (__eq(summary, {{["w_warehouse_sk"]=1, ["i_item_sk"]=1, ["cov"]=1.539600717839002}})) then error('expect failed') end
end

local math = { sqrt = math.sqrt, pow = math.pow, sin = math.sin, log = math.log, pi = math.pi, e = math.exp(1) }
inventory = {{["inv_item_sk"]=1, ["inv_warehouse_sk"]=1, ["inv_date_sk"]=1, ["inv_quantity_on_hand"]=10}, {["inv_item_sk"]=1, ["inv_warehouse_sk"]=1, ["inv_date_sk"]=2, ["inv_quantity_on_hand"]=10}, {["inv_item_sk"]=1, ["inv_warehouse_sk"]=1, ["inv_date_sk"]=3, ["inv_quantity_on_hand"]=250}}
item = {{["i_item_sk"]=1}}
warehouse = {{["w_warehouse_sk"]=1, ["w_warehouse_name"]="W1"}}
date_dim = {{["d_date_sk"]=1, ["d_year"]=2000, ["d_moy"]=1}, {["d_date_sk"]=2, ["d_year"]=2000, ["d_moy"]=2}, {["d_date_sk"]=3, ["d_year"]=2000, ["d_moy"]=3}}
monthly = (function()
    ...
    local _rows = __query(_src, {
        ...
        { items = warehouse, on = function(inv, d, i, w) return __eq(inv.inv_warehouse_sk, w.w_warehouse_sk) end }
    }, { selectFn = function(inv, d, i, w) return {inv, d, i, w} end, where = function(inv, d, i, w) return ((d.d_year == 2000)) end })
    local _groups = __group_by_rows(_rows, function(inv, d, i, w) return {["w"]=w.w_warehouse_sk, ["i"]=i.i_item_sk, ["month"]=d.d_moy} end, function(inv, d, i, w) local _row = __merge(inv, d, i, w); _row.inv = inv; _row.d = d; _row.i = i; _row.w = w; return _row end)
    local _res = {}
...
for _, m in ipairs(monthly) do
    local key = tostring({["w"]=m.w, ["i"]=m.i})
    if (grouped[key] ~= nil) then
        local g = grouped[key]
        grouped[key] = {["w"]=g.w, ["i"]=g.i, ["qtys"]=__append(g.qtys, m.qty)}
    else
        grouped[key] = {["w"]=m.w, ["i"]=m.i, ["qtys"]={m.qty}}
    end
end
summary = {}
for _, g in ipairs(__values(grouped)) do
    local mean = __avg(g.qtys)
    local sumsq = 0.0
    for _, q in __iter(g.qtys) do
        sumsq = __add(sumsq, (((q - mean)) * ((q - mean))))
    end
    local variance = __div(sumsq, ((#g.qtys - 1)))
    local cov = __div(math.sqrt(variance), mean)
    if (cov > 1.5) then
        summary = __append(summary, {["w_warehouse_sk"]=g.w, ["i_item_sk"]=g.i, ["cov"]=cov})
    end
end
__json(summary)
local __tests = {
    {name="TPCDS Q39 simplified", fn=test_TPCDS_Q39_simplified},
}
