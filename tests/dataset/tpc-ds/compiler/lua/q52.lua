-- Generated by Mochi compiler v0.10.26 on 2025-07-15T07:21:06Z
_Group = {}
...
function test_TPCDS_Q52_simplified()
    if not (__eq(result, {{["d_year"]=2001, ["brand_id"]=1, ["ext_price"]=30.0}, {["d_year"]=2001, ["brand_id"]=2, ["ext_price"]=22.0}})) then error('expect failed') end
end

store_sales = {{["item"]=1, ["sold_date"]=1, ["price"]=10.0}, {["item"]=2, ["sold_date"]=1, ["price"]=22.0}, {["item"]=1, ["sold_date"]=1, ["price"]=20.0}}
...
filtered = (function()
    ...
    local _rows = __query(_src, {
        { items = item, on = function(ss, i) return (__eq(ss.item, i.i_item_sk) and __eq(i.i_manager_id, 1)) end },
        { items = date_dim, on = function(ss, i, d) return ((__eq(ss.sold_date, d.d_date_sk) and __eq(d.d_year, 2001)) and __eq(d.d_moy, 11)) end }
    }, { selectFn = function(ss, i, d) return {ss, i, d} end })
    local _groups = __group_by_rows(_rows, function(ss, i, d) return {["year"]=d.d_year, ["brand_id"]=i.i_brand_id, ["brand"]=i.i_brand} end, function(ss, i, d) local _row = __merge(ss, i, d); _row.ss = ss; _row.i = i; _row.d = d; return _row end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["d_year"]=g.key.year, ["brand_id"]=g.key.brand_id, ["ext_price"]=__sum((function()
    local _res = {}
...
result = (function()
    ...
    for _, r in ipairs(filtered) do
        _res[#_res+1] = {__key = {r.d_year, -r.ext_price, r.brand_id}, __val = r}
    end
    local items = _res
...
__json(result)
local __tests = {
    {name="TPCDS Q52 simplified", fn=test_TPCDS_Q52_simplified},
}
