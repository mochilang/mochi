...
function test_TPCDS_Q67_simplified()
    if not ((result == 67)) then error('expect failed') end
end
...
result = __sum((function()
    ...
    return __query(_src, {
        { items = reason, on = function(ss, r) return __eq(ss.reason, r.id) end }
    }, { selectFn = function(ss, r) return ss.price end, where = function(ss, r) return (__contains({"PROMO", "RETURN"}, r.name)) end })
end)())
__json(result)
local __tests = {
    {name="TPCDS Q67 simplified", fn=test_TPCDS_Q67_simplified},
}
