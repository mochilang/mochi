...
main(_) ->
    ...
    Filtered = [maps:get(l_extendedprice, L) || L <- Lineitem, P <- Part, (maps:get(p_partkey, P) == maps:get(l_partkey, L)), (case (((((maps:get(p_brand, P) == "Brand#23")) andalso ((maps:get(p_container, P) == "MED BOX"))) andalso ((maps:get(l_quantity, L) < ((0.2 * (lists:sum([maps:get(l_quantity, X) || X <- Lineitem, (maps:get(l_partkey, X) == maps:get(p_partkey, P))]) / length([maps:get(l_quantity, X) || X <- Lineitem, (maps:get(l_partkey, X) == maps:get(p_partkey, P))])))))))) of undefined -> false; false -> false; _ -> true end)],
