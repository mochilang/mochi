...
main(_) ->
    ...
    Result = lists:sum([(maps:get(l_extendedprice, L) * maps:get(l_discount, L)) || L <- Lineitem, ((((((maps:get(l_shipdate, L) >= "1994-01-01")) andalso ((maps:get(l_shipdate, L) < "1995-01-01"))) andalso ((maps:get(l_discount, L) >= 0.05))) andalso ((maps:get(l_discount, L) =< 0.07))) andalso ((maps:get(l_quantity, L) < 24)))]),
    mochi_json(Result),
