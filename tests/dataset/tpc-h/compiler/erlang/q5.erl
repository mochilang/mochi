...
main(_) ->
    ...
    Asia_nations = [N || R <- Region, N <- Nation, (maps:get(n_regionkey, N) == maps:get(r_regionkey, R)), (maps:get(r_name, R) == "ASIA")],
    Local_customer_supplier_orders = [#{nation => maps:get(n_name, N), revenue => (maps:get(l_extendedprice, L) * ((1 - maps:get(l_discount, L))))} || C <- Customer, N <- Asia_nations, O <- Orders, L <- Lineitem, S <- Supplier, (maps:get(c_nationkey, C) == maps:get(n_nationkey, N)), (maps:get(o_custkey, O) == maps:get(c_custkey, C)), (maps:get(l_orderkey, L) == maps:get(o_orderkey, O)), (maps:get(s_suppkey, S) == maps:get(l_suppkey, L)), (((maps:get(o_orderdate, O) >= "1994-01-01") andalso (maps:get(o_orderdate, O) < "1995-01-01")) andalso (maps:get(s_nationkey, S) == maps:get(c_nationkey, C)))],
    Result = [V || {_, V} <- lists:keysort(1, [{-lists:sum([maps:get(revenue, X) || X <- Val0]), #{n_name => Key0, revenue => lists:sum([maps:get(revenue, X) || X <- Val0])}} || {Key0, Val0} <- maps:to_list(lists:foldl(fun({Key0, Val0}, Acc0) -> L = maps:get(Key0, Acc0, []), maps:put(Key0, [Val0 | L], Acc0) end, #{}, [{maps:get(nation, R), R} || R <- Local_customer_supplier_orders]))])],
    mochi_json(Result),
