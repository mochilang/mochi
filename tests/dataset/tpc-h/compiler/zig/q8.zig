...
pub fn main() void {
    ...
    o_year: []const u8,
    mkt_share: i32,
}).init(std.heap.page_allocator);for (_tmp15.items) |year| { _tmp16.append(ResultItem{
    .o_year = year.key,
    .mkt_share = (_sum_int(blk2: { var _tmp5 = std.ArrayList(i32).init(std.heap.page_allocator); for (year.Items.items) |x| { _tmp5.append(switch (std.mem.eql(u8, x.n.n_name, target_nation)) {true => (x.l.l_extendedprice * ((1 - x.l.l_discount))), else => 0, }) catch |err| handleError(err); } const _tmp6 = _tmp5.toOwnedSlice() catch |err| handleError(err); break :blk2 _tmp6; }) / _sum_int(blk3: { var _tmp7 = std.ArrayList(i32).init(std.heap.page_allocator); for (year.Items.items) |x| { _tmp7.append((x.l.l_extendedprice * ((1 - x.l.l_discount)))) catch |err| handleError(err); } const _tmp8 = _tmp7.toOwnedSlice() catch |err| handleError(err); break :blk3 _tmp8; })),
