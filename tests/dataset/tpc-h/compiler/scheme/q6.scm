; Generated by Mochi compiler v0.10.25 on 2025-07-13T17:53:37Z
(define (map-get m k)
    (let ((p (assoc k m)))
        (if p (cdr p) '()))
)
...
(define (test_Q6_calculates_revenue_from_qualified_lineitems)
  (when (not (equal? result (+ (* 1000.0 0.06) (* 500.0 0.07)))) (error "expect failed"))
)

(define lineitem (list (list (cons 'l_extendedprice 1000.0) (cons 'l_discount 0.06) (cons 'l_shipdate "1994-02-15") (cons 'l_quantity 10)) (list (cons 'l_extendedprice 500.0) (cons 'l_discount 0.07) (cons 'l_shipdate "1994-03-10") (cons 'l_quantity 23)) (list (cons 'l_extendedprice 400.0) (cons 'l_discount 0.04) (cons 'l_shipdate "1994-04-10") (cons 'l_quantity 15)) (list (cons 'l_extendedprice 200.0) (cons 'l_discount 0.06) (cons 'l_shipdate "1995-01-01") (cons 'l_quantity 5))))
(define result (_sum (let ((_res '()))
  (for-each (lambda (l)
    (when (and (and (and (and (_ge (map-get l 'l_shipdate) "1994-01-01") (_lt (map-get l 'l_shipdate) "1995-01-01")) (_ge (map-get l 'l_discount) 0.05)) (_le (map-get l 'l_discount) 0.07)) (_lt (map-get l 'l_quantity) 24))
      (set! _res (append _res (list (_sum (* (map-get l 'l_extendedprice) (map-get l 'l_discount))))))
    )
  ) (if (string? lineitem) (string->list lineitem) lineitem))
  _res)))
(_json result)
(run-test "Q6 calculates revenue from qualified lineitems" test_Q6_calculates_revenue_from_qualified_lineitems)
