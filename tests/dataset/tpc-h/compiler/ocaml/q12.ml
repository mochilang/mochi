(* Generated by Mochi compiler v0.10.25 on 2025-07-13T16:29:30Z *)
...
let result : record4 list = (let (__groups0 : (string * record2 list) list ref) = ref [] in
  ...
  List.iter (fun ((gKey : string), gItems) ->
    ...
    __res0 := { l_shipmode = g.key; high_line_count = (sum (let __res1 = ref [] in
  List.iter (fun x ->
      __res1 := (if (List.mem x.o.o_orderpriority ["1-URGENT";"2-HIGH"]) then 1 else 0) :: !__res1;
  ) g.items;
List.rev !__res1)
); low_line_count = (sum (let __res2 = ref [] in
  List.iter (fun x ->
