...
type record7 = { mutable c_custkey : int; mutable c_name : string; mutable revenue : float; mutable c_acctbal : float; mutable n_name : string; mutable c_address : string; mutable c_phone : string; mutable c_comment : string }
...
let result : record7 list = (let (__groups0 : (record5 * record2 list) list ref) = ref [] in
  ...
  List.iter (fun ((gKey : record5), gItems) ->
    ...
    __res0 := { c_custkey = g.key.c_custkey; c_name = g.key.c_name; revenue = (sum_float (let __res1 = ref [] in
  List.iter (fun x ->
...
let () =
  json result;
  assert ((result = [{ c_custkey = 1; c_name = "Alice"; revenue = (1000. *. 0.9); c_acctbal = 100.; n_name = "BRAZIL"; c_address = "123 St"; c_phone = "123-456"; c_comment = "Loyal" }]))
