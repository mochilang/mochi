...
let result : record4 list = (let (__groups0 : (record2 * record1 list) list ref) = ref [] in
  ...
  List.iter (fun ((gKey : record2), gItems) ->
    ...
    __res0 := { returnflag = g.key.returnflag; linestatus = g.key.linestatus; sum_qty = (sum_float (let __res1 = ref [] in
  List.iter (fun x ->
      __res1 := x.l_quantity :: !__res1;
  ) g.items;
List.rev !__res1)
); sum_base_price = (sum_float (let __res2 = ref [] in
  List.iter (fun x ->
      __res2 := x.l_extendedprice :: !__res2;
  ) g.items;
List.rev !__res2)
); sum_disc_price = (sum_float (let __res3 = ref [] in
  List.iter (fun x ->
      __res3 := (x.l_extendedprice *. ((1 -. x.l_discount))) :: !__res3;
  ) g.items;
List.rev !__res3)
); sum_charge = (sum_float (let __res4 = ref [] in
  List.iter (fun x ->
