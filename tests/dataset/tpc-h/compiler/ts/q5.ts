...
// Source: tests/dataset/tpc-h/q5.mochi
...
function main(): void {
  ...
  asia_nations = (() => {
    ...
    for (const r of _src) {
      for (const n of nation) {
        ...
        if (!(_equal(r.r_name, "ASIA"))) continue;
        _res.push(n);
      }
    }
    return _res;
  ...
  local_customer_supplier_orders = (() => {
    ...
    for (const c of _src) {
      for (const n of asia_nations) {
        if (!(_equal(c.c_nationkey, n.n_nationkey))) continue;
  ...
  result = (() => {
    ...
    let _pairs = _items.map((it) => {
      const g = it;
      return { item: it, key: (-_sum(g.items.map((x) => x.revenue))) };
    });
    _pairs.sort((a, b) => {
      const ak = a.key;
      const bk = b.key;
    ...
    for (const g of _items) {
      _res.push({
        "n_name": g.key,
        "revenue": _sum(g.items.map((x) => x.revenue)),
      });
    }
    return _res;
  })();
