        ...
        for (const c of customer) {
          ...
          for (const s of supplier) {
            ...
            for (const n1 of nation) {
              if (!(_equal(n1.n_nationkey, s.s_nationkey))) continue;
              for (const n2 of nation) {
                if (!(_equal(n2.n_nationkey, c.c_nationkey))) continue;
                if (
                  !((((l.l_shipdate >= start_date) &&
                    (l.l_shipdate <= end_date)) &&
                    (_equal(n1.n_name, nation1) &&
                      _equal(n2.n_name, nation2))) ||
                    (_equal(n1.n_name, nation2) && _equal(n2.n_name, nation1)))
                ) continue;
                const _key = {
