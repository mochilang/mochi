// Generated by Mochi compiler v0.10.25 on 2025-07-13T16:55:01Z
object q5 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val asia_nations = for { r <- region; n <- nation; if (n.n_regionkey).asInstanceOf[Int] == r.r_regionkey; if r.r_name == "ASIA" } yield n
    val local_customer_supplier_orders = for { c <- customer; n <- asia_nations; if c.c_nationkey == (n.n_nationkey).asInstanceOf[Int]; o <- orders; if (o.o_custkey).asInstanceOf[Int] == c.c_custkey; l <- lineitem; if (l.l_orderkey).asInstanceOf[Int] == o.o_orderkey; s <- supplier; if (s.s_suppkey).asInstanceOf[Int] == l.l_suppkey; if o.o_orderdate >= "1994-01-01" && o.o_orderdate < "1995-01-01" && s.s_nationkey == c.c_nationkey } yield LocalCustomerSupplierOrder(nation = n.n_name, revenue = l.l_extendedprice * (1 - l.l_discount))
    val result = (((for { r <- local_customer_supplier_orders } yield (r.nation, r)).groupBy(_._1).map{ case(k,list) => _Group(k, list.map(_._2)) }.toList).sortBy(g => -(for { x <- g } yield x.revenue).sum)).map{ g => Map("n_name" -> g.key, "revenue" -> (for { x <- g } yield x.revenue).sum) }.toList
