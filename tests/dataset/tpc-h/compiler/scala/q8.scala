...
object q8 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val result = (((for { l <- lineitem; p <- part; if (p.p_partkey).asInstanceOf[Int] == l.l_partkey; s <- supplier; if (s.s_suppkey).asInstanceOf[Int] == l.l_suppkey; o <- orders; if (o.o_orderkey).asInstanceOf[Int] == l.l_orderkey; c <- customer; if (c.c_custkey).asInstanceOf[Int] == o.o_custkey; n <- nation; if (n.n_nationkey).asInstanceOf[Int] == c.c_nationkey; r <- region; if (r.r_regionkey).asInstanceOf[Int] == n.n_regionkey; if (p.p_type == target_type && o.o_orderdate >= start_date && o.o_orderdate <= end_date && r.r_name == "AMERICA") } yield (o.o_orderdate.substring(0, 4), Result(l = l, p = p, s = s, o = o, c = c, n = n, r = r))).groupBy(_._1).map{ case(k,list) => _Group(k, list.map(_._2)) }.toList).sortBy(year => year.key)).map{ g => { val year = g; Map("o_year" -> year.key, "mkt_share" -> (for { x <- year } yield x.n.n_name == target_nation match {
      case true => x.l.l_extendedprice * (1 - x.l.l_discount)
      case _ => 0
    }).sum / (for { x <- year } yield x.l.l_extendedprice * (1 - x.l.l_discount)).sum) } }.toList
