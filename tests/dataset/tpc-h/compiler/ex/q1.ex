# Generated by Mochi compiler v0.10.28 on 2025-07-18T03:24:43Z
defmodule Main do
  def main do
    # lineitem :: list(map())
    lineitem = [
      ...
      (fn ->
         ...
         Enum.map(items, fn g ->
           %{
             ...
             sum_qty: Enum.sum(for x <- g.items, do: x.l_quantity),
             sum_base_price: Enum.sum(for x <- g.items, do: x.l_extendedprice),
             sum_disc_price:
               Enum.sum(for x <- g.items, do: x.l_extendedprice * (1 - x.l_discount)),
             sum_charge:
               Enum.sum(
                 for x <- g.items, do: x.l_extendedprice * (1 - x.l_discount) * (1 + x.l_tax)
               ),
             avg_qty:
               Enum.sum(for x <- g.items, do: x.l_quantity) /
                 Enum.count(for x <- g.items, do: x.l_quantity),
             avg_price:
               Enum.sum(for x <- g.items, do: x.l_extendedprice) /
                 Enum.count(for x <- g.items, do: x.l_extendedprice),
             avg_disc:
               Enum.sum(for x <- g.items, do: x.l_discount) /
                 Enum.count(for x <- g.items, do: x.l_discount),
             count_order: length(g.items)
           }
         end)
       end).()
