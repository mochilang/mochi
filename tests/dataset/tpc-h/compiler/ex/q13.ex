# Generated by Mochi compiler v0.10.28 on 2025-07-18T03:25:17Z
defmodule Main do
  def main do
    # customer :: list(map())
              ...
              length(
                for o <- orders,
                    o.o_custkey == c.c_custkey &&
                      !if(is_map(o.o_comment),
                        do: Map.has_key?(o.o_comment, "special"),
                        else: Enum.member?(o.o_comment, "special")
                      ) &&
                      !if(is_map(o.o_comment),
                        do: Map.has_key?(o.o_comment, "requests"),
                        else: Enum.member?(o.o_comment, "requests")
                      ),
                    do: o
              )
          }
    ...
    grouped =
      (fn ->
         ...
         groups = _group_by(rows, fn [x] -> x.c_count end)
         items = groups
         items = Enum.sort_by(items, fn g -> -g.key end)
         Enum.map(items, fn g -> %{c_count: g.key, custdist: length(g.items)} end)
       end).()
