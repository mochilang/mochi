...
(defn test_Q21_returns_Saudi_suppliers_who_caused_unique_delivery_delays []
  (assert (_equal result [{:s_name "Desert Trade" :numwait 1}]) "expect failed")
)
...
(defn -main []
  ...
  (def result (let [_src supplier
      ...
      ] { :select (fn [s l1 o n] [s l1 o n]) :where (fn [s l1 o n] (and (and (and (_equal (:o_orderstatus o) "F") (> (compare (:l_receiptdate l1) (:l_commitdate l1)) 0)) (_equal (:n_name n) "SAUDI ARABIA")) (not (boolean (seq (vec (->> (for [x lineitem :when (and (and (_equal (:l_orderkey x) (:l_orderkey l1)) (not (_equal (:l_suppkey x) (:l_suppkey l1)))) (> (compare (:l_receiptdate x) (:l_commitdate x)) 0))] x)))))))) })
      _groups (_group_by _rows (fn [s l1 o n] (:s_name s)))
      ]
  (vec (map (fn [g] {:s_name (:key g) :numwait (count (:Items g))}) _groups)))) ;; list of
  (_json result)
