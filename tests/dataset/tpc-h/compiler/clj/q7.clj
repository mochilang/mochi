; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:34:56Z
(ns main)
...
(defn test_Q7_computes_revenue_between_FRANCE_and_GERMANY_by_year []
  (assert (_equal result [{:supp_nation "FRANCE" :cust_nation "GERMANY" :l_year "1995" :revenue 900.0}]) "expect failed")
)
...
(defn -main []
  ...
  (def result (let [_src lineitem
      ...
      ] { :select (fn [l o c s n1 n2] [l o c s n1 n2]) :where (fn [l o c s n1 n2] (or (and (and (>= (compare (:l_shipdate l) start_date) 0) (<= (compare (:l_shipdate l) end_date) 0)) (and (_equal (:n_name n1) nation1) (_equal (:n_name n2) nation2))) (and (_equal (:n_name n1) nation2) (_equal (:n_name n2) nation1)))) })
      _groups (_group_by _rows (fn [l o c s n1 n2] {:supp_nation (:n_name n1) :cust_nation (:n_name n2) :l_year (.substring (:l_shipdate l) 0 4)}))
      ]
  (vec (map (fn [g] {:supp_nation (:supp_nation (:key g)) :cust_nation (:cust_nation (:key g)) :l_year (:l_year (:key g)) :revenue (_sum (vec (->> (for [x (:Items g)] (* (:l_extendedprice (:l x)) (- 1 (:l_discount (:l x))))))))}) _groups)))) ;; list of
  (_json result)
