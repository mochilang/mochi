...
(defn test_Q19_returns_total_revenue_from_qualifying_branded_parts []
  (assert (_equal result 2800.0) "expect failed")
)
...
(defn -main []
  ...
  (def revenues (vec (->> (for [l lineitem p part :when (_equal (:p_partkey p) (:l_partkey l)) :when (and (and (or (or (and (and (and (_equal (:p_brand p) "Brand#12") (some #(= (:p_container p) %) ["SM CASE" "SM BOX" "SM PACK" "SM PKG"])) (and (>= (:l_quantity l) 1) (<= (:l_quantity l) 11))) (and (>= (:p_size p) 1) (<= (:p_size p) 5))) (and (and (and (_equal (:p_brand p) "Brand#23") (some #(= (:p_container p) %) ["MED BAG" "MED BOX" "MED PKG" "MED PACK"])) (and (>= (:l_quantity l) 10) (<= (:l_quantity l) 20))) (and (>= (:p_size p) 1) (<= (:p_size p) 10)))) (and (and (and (_equal (:p_brand p) "Brand#34") (some #(= (:p_container p) %) ["LG CASE" "LG BOX" "LG PACK" "LG PKG"])) (and (>= (:l_quantity l) 20) (<= (:l_quantity l) 30))) (and (>= (:p_size p) 1) (<= (:p_size p) 15)))) (some #(= (:l_shipmode l) %) ["AIR" "AIR REG"])) (_equal (:l_shipinstruct l) "DELIVER IN PERSON"))] (* (:l_extendedprice l) (- 1 (:l_discount l))))))) ;; list of float
