...
(defn test_Q15_returns_top_revenue_supplier_s__for_Q1_1996 []
  ...
  (assert (_equal result [{:s_suppkey 100 :s_name "Best Supplier" :s_address "123 Market St" :s_phone "123-456" :total_revenue rev}]) "expect failed")
)
...
(defn -main []
  ...
  (->> _groups (map (fn [g] {:supplier_no (:key g) :total_revenue (_sum (vec (->> (for [x (:Items g)] (* (:l_extendedprice x) (- 1 (:l_discount x)))))))})) vec))) ;; list of
  (def revenues (vec (->> (for [x revenue0] (:total_revenue x))))) ;; list of float
  (def max_revenue (apply max revenues)) ;; float
  (def result (vec (->> (for [s supplier r revenue0 :when (_equal (:s_suppkey s) (:supplier_no r)) :when (_equal (:total_revenue r) max_revenue)] {:s_suppkey (:s_suppkey s) :s_name (:s_name s) :s_address (:s_address s) :s_phone (:s_phone s) :total_revenue (:total_revenue r)}) (sort-by (fn [s] (_sort_key (:s_suppkey s))))))) ;; list of
  (_json result)
