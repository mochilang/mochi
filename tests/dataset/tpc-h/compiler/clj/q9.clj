; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:34:57Z
(ns main)
...
(defn test_Q9_computes_profit_for_green_parts_by_nation_and_year []
  ...
  (assert (_equal result [{:nation "BRAZIL" :o_year "1995" :profit (- revenue cost)}]) "expect failed")
)
...
(defn -main []
  ...
  (def result (let [_src lineitem
      _rows (_query _src [
        ...
        {:items partsupp :on (fn [l p s ps] (and (_equal (:ps_partkey ps) (:l_partkey l)) (_equal (:ps_suppkey ps) (:l_suppkey l))))}
      ...
      ] { :select (fn [l p s ps o n] [l p s ps o n]) :where (fn [l p s ps o n] (and (and (_equal (.substring (:p_name p) 0 (count prefix)) prefix) (>= (compare (:o_orderdate o) start_date) 0)) (<= (compare (:o_orderdate o) end_date) 0))) })
      _groups (_group_by _rows (fn [l p s ps o n] {:nation (:n_name n) :o_year (Integer/parseInt (.substring (:o_orderdate o) 0 4))}))
      ]
  (vec (map (fn [g] {:nation (:nation (:key g)) :o_year (str (:o_year (:key g))) :profit (_sum (vec (->> (for [x (:Items g)] (- (* (:l_extendedprice (:l x)) (- 1 (:l_discount (:l x)))) (* (:ps_supplycost (:ps x)) (:l_quantity (:l x))))))))}) _groups)))) ;; list of
  (_json result)
