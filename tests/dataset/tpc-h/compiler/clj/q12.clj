...
(defn test_Q12_counts_lineitems_by_ship_mode_and_priority []
  (assert (_equal result [{:l_shipmode "MAIL" :high_line_count 1 :low_line_count 0}]) "expect failed")
)
...
(defn -main []
  ...
  (vec (map (fn [g] {:l_shipmode (:key g) :high_line_count (reduce + 0 (vec (->> (for [x (:Items g)] (if (some #(= (:o_orderpriority (:o x)) %) ["1-URGENT" "2-HIGH"]) 1 0))))) :low_line_count (reduce + 0 (vec (->> (for [x (:Items g)] (if (not (some #(= (:o_orderpriority (:o x)) %) ["1-URGENT" "2-HIGH"])) 1 0)))))}) _groups)))) ;; list of
  (_json result)
