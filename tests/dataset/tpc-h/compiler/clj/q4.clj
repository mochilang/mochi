; Generated by Mochi compiler v0.10.25 on 2025-07-13T18:17:36Z
(ns main)
...
(defn test_Q4_returns_count_of_orders_with_late_lineitems_in_range []
  (assert (_equal result [{:o_orderpriority "1-URGENT" :order_count 1} {:o_orderpriority "2-HIGH" :order_count 1}]) "expect failed")
)
...
(defn -main []
  ...
  (def late_orders (vec (->> (for [o date_filtered_orders :when (boolean (seq (vec (->> (for [l lineitem :when (and (_equal (:l_orderkey l) (:o_orderkey o)) (< (compare (:l_commitdate l) (:l_receiptdate l)) 0))] l)))))] o)))) ;; list of
  (def result (let [_src late_orders
      _groups (_group_by _src (fn [o] (:o_orderpriority o)))
      ]
  (->> _groups (sort-by (fn [g] (_sort_key (:key g)))) (map (fn [g] {:o_orderpriority (:key g) :order_count (count (:Items g))})) vec))) ;; list of
  (_json result)
