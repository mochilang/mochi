// Generated by Mochi compiler v0.10.25 on 2025-07-13T16:56:03Z
import 'dart:io';
import 'dart:convert';
...
var result = (() {
  ...
  for (var entry in _g1.entries) {
    ...
    _q0.add({'returnflag': _k12['returnflag'], 'linestatus': _k12['linestatus'], 'sum_qty': _sum(g.map((x) => x['l_quantity'])), 'sum_base_price': _sum(g.map((x) => x['l_extendedprice'])), 'sum_disc_price': _sum(g.map((x) => (x['l_extendedprice'] as num) * ((1 - (x['l_discount'] as num)) as num))), 'sum_charge': _sum(g.map((x) => ((x['l_extendedprice'] as num) * ((1 - (x['l_discount'] as num)) as num) as num) * ((1 + (x['l_tax'] as num)) as num))), 'avg_qty': (() { var _t18 = (() {
  var _q17 = <dynamic>[];
  for (var x in g) {
