# Generated by Mochi compiler v0.10.25 on 2025-07-13T06:12:07Z
...
def _q0():
    ...
    _items1 = _groups
    return [
        {"ps_partkey": _get(g, "key"), "value": _sum([r["value"] for r in g])}
        for g in _items1
    ]


grouped = _q0()
total = _sum([x["value"] for x in filtered])
threshold = total * 0.0001
result = [
    x
    for x in sorted(
        [x for x in grouped if x["value"] > threshold],
        key=lambda x: _sort_key(-x["value"]),
    )
]
