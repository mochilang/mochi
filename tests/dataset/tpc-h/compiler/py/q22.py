# Generated by Mochi compiler v0.10.25 on 2025-07-13T06:12:16Z
...
avg_balance = _avg(
    [
        c["c_acctbal"]
        for c in customer
        if c["c_acctbal"] > 0.0 and _slice(c["c_phone"], 0, 2) in valid_codes
    ]
)
eligible_customers = [
    {"cntrycode": _slice(c["c_phone"], 0, 2), "c_acctbal": c["c_acctbal"]}
    for c in customer
    if (_slice(c["c_phone"], 0, 2) in valid_codes and c["c_acctbal"] > avg_balance)
...
tmp = []
for g in groups:
    total = _sum([x["c_acctbal"] for x in _get(g, "items")])
    row = {"cntrycode": _get(g, "key"), "numcust": len(g.Items), "totacctbal": total}
    tmp = tmp + [row]
result = [
