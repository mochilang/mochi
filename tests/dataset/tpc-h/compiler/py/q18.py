# Generated by Mochi compiler v0.10.25 on 2025-07-14T07:14:25Z
...
nation: list[Nation] = [Nation(n_nationkey=1, n_name="GERMANY")]
...
def _q0():
    ...
    _items1 = _groups
    _items1 = [
        g for g in _items1 if _sum([_get(x[2], "l_quantity") for x in g]) > threshold
    ]
    _items1 = sorted(
        _items1,
        key=lambda g: _sort_key(
            -_sum(
                [
                    _get(x[2], "l_extendedprice") * (1 - _get(x[2], "l_discount"))
                    for x in g
                ]
            )
        ),
