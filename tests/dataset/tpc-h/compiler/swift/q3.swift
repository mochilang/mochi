...
var order_line_join = { () -> [Any] in
	...
	_tmp.sort { [-_sum($0.items.map { r in ((r["l"] as! Auto3)["l_extendedprice"] as! Double) * (1 - ((r["l"] as! Auto3)["l_discount"] as! Double)) }), ($0.key["o_orderdate"] as! String)] < [-_sum($1.items.map { r in ((r["l"] as! Auto3)["l_extendedprice"] as! Double) * (1 - ((r["l"] as! Auto3)["l_discount"] as! Double)) }), ($1.key["o_orderdate"] as! String)] }
	return _tmp.map { g in ["l_orderkey": (g.key["o_orderkey"] as! Int), "revenue": _sum(g.items.map { r in ((r["l"] as! Auto3)["l_extendedprice"] as! Double) * (1 - ((r["l"] as! Auto3)["l_discount"] as! Double)) }), "o_orderdate": (g.key["o_orderdate"] as! String), "o_shippriority": (g.key["o_shippriority"] as! Int)] }
}())
