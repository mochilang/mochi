// Generated by Mochi compiler v0.10.25 on 2025-07-13T13:59:48Z
import Foundation
	...
	for ps in partsupp {
		for p in target_parts {
			...
			for s in europe_suppliers {
				...
				_res.append(["s_acctbal": s["s"]!.s_acctbal, "s_name": s["s"]!.s_name, "n_name": s["n"]!.n_name, "p_partkey": p.p_partkey, "p_mfgr": p.p_mfgr, "s_address": s["s"]!.s_address, "s_phone": s["s"]!.s_phone, "s_comment": s["s"]!.s_comment, "ps_supplycost": ps.ps_supplycost])
			}
		}
	}
	return _res
}())
var costs = target_partsupp.map { x in x["ps_supplycost"]! }
let min_cost = _min(costs)
var result = target_partsupp.filter { x in x["ps_supplycost"]! == min_cost }.sorted { $0.s_acctbal > $1.s_acctbal }
_json(result)
expect(_equal(result, [Auto6(n_name: "FRANCE", p_mfgr: "M1", p_partkey: 1000, ps_supplycost: 10.0, s_acctbal: 1000.0, s_address: "123 Rue", s_comment: "Fast and reliable", s_name: "BestSupplier", s_phone: "123")]))
