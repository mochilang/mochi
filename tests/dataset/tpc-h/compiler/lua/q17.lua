...
function test_Q17_returns_average_yearly_revenue_for_small_quantity_orders()
    ...
    if not (__eq(result, expected)) then error('expect failed') end
end
...
filtered = (function()
    ...
    }, { selectFn = function(l, p) return l.l_extendedprice end, where = function(l, p) return (((((__eq(p.p_brand, brand)) and (__eq(p.p_container, container))) and ((l.l_quantity < ((0.2 * __avg((function()
    local _res = {}
    for _, x in ipairs(lineitem) do
        if __eq(x.l_partkey, p.p_partkey) then
