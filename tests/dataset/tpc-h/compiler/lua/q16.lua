...
excluded_suppliers = (function()
    local _src = partsupp
    return __query(_src, {
        { items = part, on = function(ps, p) return __eq(p.p_partkey, ps.ps_partkey) end }
    }, { selectFn = function(ps, p) return ps.ps_suppkey end, where = function(ps, p) return (((__eq(p.p_brand, "Brand#12") and __contains(p.p_type, "SMALL")) and __eq(p.p_size, 5))) end })
end)()
result = (function()
    local _res = {}
    for _, s in ipairs(supplier) do
        if ((not (__contains(excluded_suppliers, s.s_suppkey)) and (not __contains(s.s_comment, "Customer"))) and (not __contains(s.s_comment, "Complaints"))) then
