-- Generated by Mochi compiler v0.10.28 on 2025-07-18T03:25:07Z
_Group = {}
...
function test_Q11_returns_high_value_partkeys_from_GERMANY()
    if not (__eq(result, {{["ps_partkey"]=1000, ["value"]=2000.0}, {["ps_partkey"]=2000, ["value"]=50.0}})) then error('expect failed') end
end
...
partsupp = {{["ps_partkey"]=1000, ["ps_suppkey"]=100, ["ps_supplycost"]=10.0, ["ps_availqty"]=100}, {["ps_partkey"]=1000, ["ps_suppkey"]=200, ["ps_supplycost"]=20.0, ["ps_availqty"]=50}, {["ps_partkey"]=2000, ["ps_suppkey"]=100, ["ps_supplycost"]=5.0, ["ps_availqty"]=10}, {["ps_partkey"]=3000, ["ps_suppkey"]=300, ["ps_supplycost"]=8.0, ["ps_availqty"]=500}}
target_nation = "GERMANY"
filtered = (function()
    local _src = partsupp
    return __query(_src, {
        { items = supplier, on = function(ps, s) return __eq(s.s_suppkey, ps.ps_suppkey) end },
        { items = nation, on = function(ps, s, n) return __eq(n.n_nationkey, s.s_nationkey) end }
    }, { selectFn = function(ps, s, n) return {["ps_partkey"]=ps.ps_partkey, ["value"]=(ps.ps_supplycost * ps.ps_availqty)} end, where = function(ps, s, n) return (__eq(n.n_name, target_nation)) end })
end)()
grouped = (function()
    local _groups = __group_by(filtered, function(x) return x.ps_partkey end)
    local _res = {}
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["ps_partkey"]=g.key, ["value"]=__sum((function()
    local _res = {}
...
result = (function()
    ...
    for _, x in ipairs(grouped) do
        if (x.value > threshold) then
            _res[#_res+1] = {__key = -x.value, __val = x}
        end
    end
    local items = _res
...
__json(result)
local __tests = {
    {name="Q11 returns high-value partkeys from GERMANY", fn=test_Q11_returns_high_value_partkeys_from_GERMANY},
}
