...
function test_Q21_returns_Saudi_suppliers_who_caused_unique_delivery_delays()
    if not (__eq(result, {{["s_name"]="Desert Trade", ["numwait"]=1}})) then error('expect failed') end
end
...
result = (function()
    ...
    local _rows = __query(_src, {
        { items = lineitem, on = function(s, l1) return __eq(s.s_suppkey, l1.l_suppkey) end },
        { items = orders, on = function(s, l1, o) return __eq(o.o_orderkey, l1.l_orderkey) end },
        { items = nation, on = function(s, l1, o, n) return __eq(n.n_nationkey, s.s_nationkey) end }
    }, { selectFn = function(s, l1, o, n) return {s, l1, o, n} end, where = function(s, l1, o, n) return ((((__eq(o.o_orderstatus, "F") and (l1.l_receiptdate > l1.l_commitdate)) and __eq(n.n_name, "SAUDI ARABIA")) and (not __exists((function()
    local _res = {}
    for _, x in ipairs(lineitem) do
        if ((__eq(x.l_orderkey, l1.l_orderkey) and not __eq(x.l_suppkey, l1.l_suppkey)) and (x.l_receiptdate > x.l_commitdate)) then
            _res[#_res+1] = x
        end
    end
    return _res
    ...
    for _, g in ipairs(_groups) do
        _res[#_res+1] = {["s_name"]=g.key, ["numwait"]=__count(g)}
    end
    return _res
end)()
__json(result)
