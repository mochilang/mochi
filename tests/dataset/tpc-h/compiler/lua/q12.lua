...
function test_Q12_counts_lineitems_by_ship_mode_and_priority()
    if not (__eq(result, {{["l_shipmode"]="MAIL", ["high_line_count"]=1, ["low_line_count"]=0}})) then error('expect failed') end
end
...
result = (function()
    ...
    }, { selectFn = function(l, o) return {l, o} end, where = function(l, o) return ((((((__contains({"MAIL", "SHIP"}, l.l_shipmode)) and ((l.l_commitdate < l.l_receiptdate))) and ((l.l_shipdate < l.l_commitdate))) and ((l.l_receiptdate >= "1994-01-01"))) and ((l.l_receiptdate < "1995-01-01")))) end })
    ...
    if __contains({"1-URGENT", "2-HIGH"}, x.o.o_orderpriority) then
        return 1
    else
        return 0
    end
    ...
    for _, x in ipairs(g.items) do
        _res[#_res+1] = (function()
    if not (__contains({"1-URGENT", "2-HIGH"}, x.o.o_orderpriority)) then
        return 1
    else
        return 0
    end
