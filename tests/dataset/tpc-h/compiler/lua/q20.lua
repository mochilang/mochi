...
function test_Q20_returns_suppliers_from_CANADA_with_forest_part_stock___50__of_1994_shipments()
    if not (__eq(result, {{["s_name"]="Maple Supply", ["s_address"]="123 Forest Lane"}})) then error('expect failed') end
end
...
target_partkeys = (function()
    local _src = partsupp
    return __query(_src, {
        { items = part, on = function(ps, p) return __eq(ps.ps_partkey, p.p_partkey) end },
        { items = shipped_94, on = function(ps, p, s) return (__eq(ps.ps_partkey, s.partkey) and __eq(ps.ps_suppkey, s.suppkey)) end }
    }, { selectFn = function(ps, p, s) return ps.ps_suppkey end, where = function(ps, p, s) return ((((__slice(p.p_name, 0, #prefix) == prefix) and 1 or 0) and (ps.ps_availqty > ((0.5 * s.qty))))) end })
end)()
result = (function()
    local _src = supplier
    return __query(_src, {
        { items = nation, on = function(s, n) return __eq(n.n_nationkey, s.s_nationkey) end }
    }, { selectFn = function(s, n) return {["s_name"]=s.s_name, ["s_address"]=s.s_address} end, where = function(s, n) return (((function(_l,_v) for _,x in ipairs(_l) do if x == _v then return true end end return false end)(target_partkeys,s.s_suppkey) and __eq(n.n_name, "CANADA"))) end, sortKey = function(s, n) return (s.s_name) end })
end)()
__json(result)
local __tests = {
