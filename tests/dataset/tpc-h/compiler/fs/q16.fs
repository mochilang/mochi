...
let result: Anon4 list = [ for s in supplier do if not (List.contains s.s_suppkey excluded_suppliers) && (not s.s_comment.contains("Customer")) && (not s.s_comment.contains("Complaints")) then yield { s_name = s.s_name; s_address = s.s_address } ] |> List.sortBy (fun s -> s.s_name)
