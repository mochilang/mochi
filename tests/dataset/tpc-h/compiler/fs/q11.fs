...
let grouped: Anon5 list = [ for g in _group_by [ for x in filtered do yield x ] (fun x -> x.ps_partkey) do
...
let result: Anon5 list = [ for x in grouped do if x.value > threshold then yield x ] |> List.sortByDescending (fun x -> x.value)
