...
struct Part {
  ...
  decltype(std::string("Brand#23")) p_brand;
  decltype(std::string("MED BOX")) p_container;
};
struct Lineitem {
  decltype(1) l_partkey;
  decltype(1) l_quantity;
  decltype(100) l_extendedprice;
};
...
int main() {
  ...
  auto filtered = ([&]() {
    std::vector<decltype(l.l_extendedprice)> __items;
    for (auto l : lineitem) {
      for (auto p : part) {
        ...
        if (!((((((p.p_brand == brand)) && ((p.p_container == container))) &&
                ((l.l_quantity <
                  ((0.2 * __avg(([&]() {
                      std::vector<decltype(x.l_quantity)> __items;
                      for (auto x : lineitem) {
