...
struct Customer {
  ...
  decltype(std::string("13-123-4567")) c_phone;
  decltype(600) c_acctbal;
};
struct Order {
  decltype(10) o_orderkey;
  decltype(2) o_custkey;
};
...
struct EligibleCustomer {
  decltype(std::string(c.c_phone).substr(0, (2) - (0))) cntrycode;
  decltype(c.c_acctbal) c_acctbal;
};
struct __struct4 {
  decltype(std::declval<EligibleCustomer>().cntrycode) key;
  std::vector<EligibleCustomer> items;
};
struct Row {
  decltype(std::declval<EligibleCustomer>().key) cntrycode;
  int numcust;
  decltype(total) totacctbal;
};
...
int main() {
  ...
  auto avg_balance = __avg(([&]() {
    std::vector<decltype(c.c_acctbal)> __items;
    for (auto c : customer) {
      if (!(((c.c_acctbal > 0) &&
  ...
  std::vector<EligibleCustomer> groups = ([&]() {
    std::map<decltype(std::declval<EligibleCustomer>().cntrycode),
             std::vector<EligibleCustomer>>
        __groups;
    for (auto c : eligible_customers) {
      __groups[c.cntrycode].push_back(EligibleCustomer{c});
    }
  ...
  for (auto g : groups) {
    auto total = ([&](auto v) {
      ...
      std::vector<decltype(std::declval<EligibleCustomer>().c_acctbal)> __items;
      for (auto x : g.items) {
  ...
  auto result = ([&]() {
    std::vector<std::pair<decltype(r.cntrycode), decltype(r)>> __items;
    for (auto r : tmp) {
