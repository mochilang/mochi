...
struct Part {
  ...
  decltype(std::string("SM BOX")) p_container;
  decltype(3) p_size;
};
struct Lineitem {
  ...
  decltype(std::string("AIR")) l_shipmode;
  decltype(std::string("DELIVER IN PERSON")) l_shipinstruct;
};
...
int main() {
  ...
  std::vector<Lineitem> lineitem = {
      ...
      Lineitem{3, 35, 1500, 0, std::string("AIR"),
               std::string("DELIVER IN PERSON")}};
  auto revenues = ([&]() {
    std::vector<decltype((l.l_extendedprice * ((1 - l.l_discount))))> __items;
    for (auto l : lineitem) {
      for (auto p : part) {
