...
struct __struct4 {
  decltype(std::declval<Result>().o_orderpriority) key;
  std::vector<Result> items;
};
struct __struct5 {
  decltype(std::declval<__struct4>().key) o_orderpriority;
  int order_count;
};
  ...
  std::vector<Result> result = ([&]() {
    ...
    for (auto o : late_orders) {
      ...
      for (auto &__g : __groups) {
        if (__g.key == __key) {
          __g.items.push_back(Result{o});
          __found = true;
          break;
        }
      }
      if (!__found) {
        __groups.push_back(__struct4{__key, std::vector<Result>{Result{o}}});
      }
    }
    std::vector<std::pair<decltype(std::declval<__struct4>().key), __struct5>>
        __items;
    for (auto &g : __groups) {
      __items.push_back({g.key, __struct5{g.key, ((int)g.items.size())}});
    }
