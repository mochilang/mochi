	...
	result = func() []Result {
		...
		for _, g := range items {
			results = append(results, Result{
				C_custkey: g.Key.(GKey).C_custkey,
				C_name:    g.Key.(GKey).C_name,
				...
				C_acctbal: g.Key.(GKey).C_acctbal,
				N_name:    g.Key.(GKey).N_name,
				C_address: g.Key.(GKey).C_address,
				C_phone:   g.Key.(GKey).C_phone,
				C_comment: g.Key.(GKey).C_comment,
			})
		}
		return results
	}()
