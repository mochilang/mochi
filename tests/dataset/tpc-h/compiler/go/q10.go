	...
	result = func() []Result {
		...
		for _, g := range items {
			results = append(results, Result{
				C_custkey: g.Key.(v).C_custkey,
				C_name:    g.Key.(v).C_name,
				...
				C_acctbal: g.Key.(v).C_acctbal,
				N_name:    g.Key.(v).N_name,
				C_address: g.Key.(v).C_address,
				C_phone:   g.Key.(v).C_phone,
				C_comment: g.Key.(v).C_comment,
			})
		}
		return results
	}()
