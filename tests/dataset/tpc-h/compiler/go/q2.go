...
func main() {
	...
	costs = func() []float64 {
		results := []float64{}
		for _, xRaw := range target_partsupp {
			x := xRaw.(Target_partsupp)
			results = append(results, x.Ps_supplycost)
		}
		return results
	}()
	min_cost = _minOrdered[float64](costs)
