...
// Generated by Mochi compiler v0.10.26 on 2025-07-15T08:32:09Z
	...
	order_line_join = func() []Order_line_join {
		...
		for idx, it := range items {
			...
			pairs[idx] = pair{item: it, key: _toAnySlice([]any{-_sum(func() []any {
				...
				for _, r := range g.Items {
					results = append(results, ((_getField(r.L, "l_extendedprice")).(float64) * float64((float64(1) - (_getField(r.L, "l_discount")).(float64)))))
				}
				return results
		...
		for _, g := range items {
			results = append(results, Order_line_join{
				L_orderkey: g.Key.(GKey).O_orderkey,
				...
				O_orderdate:    g.Key.(GKey).O_orderdate,
				O_shippriority: g.Key.(GKey).O_shippriority,
			})
		}
		return results
	}()
