...
// Generated by Mochi compiler v0.10.25 on 2025-07-14T12:47:21Z
...
func main() {
	...
	result = func() []Result {
		...
		for idx, it := range items {
			g := it
			pairs[idx] = pair{item: it, key: _toAnySlice([]any{_getField(g.Key, "nation"), -(_getField(g.Key, "o_year")).(float64)})}
		}
		...
		for _, g := range items {
			results = append(results, Result{
				Nation: _getField(g.Key, "nation"),
				O_year: fmt.Sprint(_getField(g.Key, "o_year")),
				Profit: _sum(func() []any {
					results := []any{}
					for _, x := range g.Items {
						results = append(results, (float64(((_getField(_getField(x, "l"), "l_extendedprice")).(float64) * float64((float64(1) - (_getField(_getField(x, "l"), "l_discount")).(float64))))) - float64(((_getField(_getField(x, "ps"), "ps_supplycost")).(float64) * (_getField(_getField(x, "l"), "l_quantity")).(float64)))))
					}
					return results
				}()),
			})
