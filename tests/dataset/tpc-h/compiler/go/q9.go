			...
			results = append(results, Result{
				...
				Profit: _sum(func() []any {
					...
					for _, x := range g.Items {
						results = append(results, (((_getField(_getField(x, "l"), "l_extendedprice")).(float64) * (float64(1) - (_getField(_getField(x, "l"), "l_discount")).(float64)).(float64)).(float64) - ((_getField(_getField(x, "ps"), "ps_supplycost")).(float64) * (_getField(_getField(x, "l"), "l_quantity")).(float64)).(float64)))
					}
					return results
				}()),
			})
