...
func main() {
	...
	supplier = _convSlice[v, Supplier]([]Supplier{Supplier{S_suppkey: 1000}, Supplier{S_suppkey: 2000}})
				...
				Mkt_share: (_sum(func() []any {
					...
					for _, x := range year.Items {
						results = append(results, func() any {
							...
							if _equal(_t, true) {
								return ((_getField(_getField(x, "l"), "l_extendedprice")).(float64) * (float64(1) - (_getField(_getField(x, "l"), "l_discount")).(float64)).(float64))
							}
							return 0.0
						}())
					}
