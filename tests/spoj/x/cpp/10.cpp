// Generated by Mochi transpiler v0.10.74 on 2025-08-27 07:21:04 GMT+7
#include <iostream>
#include <string>
#include <map>
#include <vector>
...
std::any parse(std::string s) {
    ...
    while ((i < s.size())) {
        ...
        if (((ch >= std::string("a")) && (ch <= std::string("z")))) {
            vals = ([&]{ auto __tmp = vals; __tmp.push_back((std::any)std::map<std::string, std::string>{{"kind", std::string("var")}, {"val", ch}}); return __tmp; }());
...
bool needParen(std::string parent, bool isRight, std::any child) {
    if ((([&]{ std::ostringstream ss; any_to_stream(ss, std::any_cast<const std::map<std::string, std::any>&>(child).at("kind"));
 return ss.str(); }()) != std::string("op"))) {
        return false;
    }
    int64_t p = precedence(parent);
    int64_t c = precedence(any_to_string(std::any_cast<const std::map<std::string, std::any>&>(child).at("op")));
    if ((c < p)) {
        return true;
    }
    if ((c > p)) {
    ...
    if ((((parent == std::string("-")) && isRight) && ((([&]{ std::ostringstream ss; any_to_stream(ss, std::any_cast<const std::map<std::string, std::any>&>(child).at("op"));
 return ss.str(); }()) == std::string("+")) || (([&]{ std::ostringstream ss; any_to_stream(ss, std::any_cast<const std::map<std::string, std::any>&>(child).at("op"));
...
std::string formatRec(std::any node, std::string parent, bool isRight) {
    if ((([&]{ std::ostringstream ss; any_to_stream(ss, std::any_cast<const std::map<std::string, std::any>&>(node).at("kind"));
 return ss.str(); }()) != std::string("op"))) {
        return any_to_string(std::any_cast<const std::map<std::string, std::any>&>(node).at("val"));
    }
    std::string left = formatRec(std::any_cast<const std::map<std::string, std::any>&>(node).at("left"), any_to_string(std::any_cast<const std::map<std::string, std::any>&>(node).at("op")), false);
    std::string right = formatRec(std::any_cast<const std::map<std::string, std::any>&>(node).at("right"), any_to_string(std::any_cast<const std::map<std::string, std::any>&>(node).at("op")), true);
    std::string res = ((left + ([&]{ std::ostringstream ss; any_to_stream(ss, std::any_cast<const std::map<std::string, std::any>&>(node).at("op"));
 return ss.str(); }())) + right);
