// Generated by Mochi transpiler v0.10.74 on 2025-08-27 07:11:05 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
std::vector<int64_t> parse_ints(std::string line) {
    ...
    for (std::string p : pieces) {
        if ((p.size() > int64_t(0))) {
            nums = ([&]{ auto __tmp = nums; __tmp.push_back((int64_t)std::stoll(p)); return __tmp; }());
        }
    }
    return nums;
}
...
int main() {
    {
        ...
        int64_t caseIdx = int64_t(0);
        while ((caseIdx < t)) {
            std::vector<int64_t> header = parse_ints(([&]{ std::string __line; std::getline(std::cin, __line); return __line; }()));
            int64_t s = _index(header, int64_t(0));
            int64_t c = _index(header, int64_t(1));
            std::vector<int64_t> seq = parse_ints(([&]{ std::string __line; std::getline(std::cin, __line); return __line; }()));
            std::vector<std::vector<int64_t>> levels = {};
            levels = ([&]{ auto __tmp = levels; __tmp.push_back(seq); return __tmp; }());
            std::vector<int64_t> current = seq;
            while ((current.size() > int64_t(1))) {
                std::vector<int64_t> next = {};
                int64_t i = int64_t(0);
                while (((i + int64_t(1)) < current.size())) {
                    next = ([&]{ auto __tmp = next; __tmp.push_back((_index(current, (i + int64_t(1))) - _index(current, i))); return __tmp; }());
                    i = (i + int64_t(1));
                }
                levels = ([&]{ auto __tmp = levels; __tmp.push_back(next); return __tmp; }());
                current = next;
            }
            int64_t depth = (levels.size() - int64_t(1));
            int64_t step = int64_t(0);
            std::vector<int64_t> res = {};
            while ((step < c)) {
                std::vector<int64_t> bottom = _index(levels, depth);
                bottom = ([&]{ auto __tmp = bottom; __tmp.push_back(_index(bottom, (bottom.size() - int64_t(1)))); return __tmp; }());
                _index(levels, depth) = bottom;
                int64_t level = (depth - int64_t(1));
                while ((level >= int64_t(0))) {
                    std::vector<int64_t> arr = _index(levels, level);
                    std::vector<int64_t> arrBelow = _index(levels, (level + int64_t(1)));
                    int64_t nextVal = (_index(arr, (arr.size() - int64_t(1))) + _index(arrBelow, (arrBelow.size() - int64_t(1))));
                    arr = ([&]{ auto __tmp = arr; __tmp.push_back(nextVal); return __tmp; }());
                    _index(levels, level) = arr;
                    level = (level - int64_t(1));
                }
                res = ([&]{ auto __tmp = res; __tmp.push_back(_index(_index(levels, int64_t(0)), (_index(levels, int64_t(0)).size() - int64_t(1)))); return __tmp; }());
                step = (step + int64_t(1));
            }
            ...
            while ((i2 < res.size())) {
                ...
                out = (out + ([&]{ std::ostringstream ss; ss << std::boolalpha << _index(res, i2); return ss.str(); }()));
                i2 = (i2 + int64_t(1));
            }
            std::cout << out;
            std::cout << std::endl;
            caseIdx = (caseIdx + int64_t(1));
        }
    }
    return 0;
}
