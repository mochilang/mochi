...
class Program {
    ...
    static GResult[] stats = (from person in people personperson.city gTmp gnew GGroup{key = gTmp.Key, items = gTmp.ToArray()} select new GResult{city = g.key, count = g.items.Length, avg_age = ((from p in g.items select p.age).ToArray().Average())}).ToArray();
    static void Main() {
