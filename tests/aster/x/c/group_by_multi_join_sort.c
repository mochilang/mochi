...
int main(void) {
                        ...
                        for (size_t m = 0; m < 1; m++) {
                            ...
                            if ((n.n_nationkey == c.c_nationkey)) {
                                if ((strcmp(o.o_orderdate, start_date) >= 0 && strcmp(o.o_orderdate, end_date) < 0 && strcmp(l.l_returnflag, "R") == 0)) {
                                    ...
                                    int found = 0;
                                    if ((found)) {
                                        result[idx].revenue += rev;
                                    } else {
                                        result[result_len++] = (ResultItem){c.c_custkey,c.c_name,rev,c.c_acctbal,n.n_name,c.c_address,c.c_phone,c.c_comment};
                                    }
                                }
                            }
                        }
    ...
    for (size_t a = 0; a < result_len; a++) {
        for (size_t b = a + 1; b < result_len; b++) {
            if ((result[a].revenue < result[b].revenue)) {
                ResultItem tmp = result[a];
                result[a] = result[b];
                result[b] = tmp;
            }
        }
    ...
    for (size_t i = 0; i < result_len; i++) {
        ResultItem r = result[i];
        printf("{\"c_custkey\": %d, \"c_name\": %s, \"revenue\": %g, \"c_acctbal\": %g, \"n_name\": %s, \"c_address\": %s, \"c_phone\": %s, \"c_comment\": %s}%s", r.c_custkey, r.c_name, r.revenue, r.c_acctbal, r.n_name, r.c_address, r.c_phone, r.c_comment, i + 1 < result_len ? " " : "");
    }
    puts("");
    return 0;
}
