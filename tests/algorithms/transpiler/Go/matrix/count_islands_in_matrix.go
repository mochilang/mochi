...
// Generated by Mochi v0.10.64 on 2025-08-12 16:24:15 GMT+7
package main
...
func is_safe(grid [][]int, visited [][]bool, row int, col int) bool {
	...
	_ = not_visited
	_ = not_visited
	return (not_visited && (int(_index(_index(grid, row), col)) == 1))
}
...
func dfs(grid [][]int, visited [][]bool, row int, col int) {
	...
	_ = k
	for k < 8 {
		var new_row int = (row + int(_index(row_nbr, k)))
		_ = new_row
		var new_col int = (col + int(_index(col_nbr, k)))
		_ = new_col
...
func count_islands(grid [][]int) int {
	...
	for i < rows {
		...
		_ = j
		for j < cols {
			if !_index(_index(visited, i), j) && (int(_index(_index(grid, i), j)) == 1) {
				dfs(func(v any) [][]int {
					if v == nil {
						return nil
					}
