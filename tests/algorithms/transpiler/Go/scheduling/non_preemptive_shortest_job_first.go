...
// Generated by Mochi v0.10.63 on 2025-08-11 16:26:36 GMT+7
package main
...
func calculate_waitingtime(arrival_time []int, burst_time []int, no_of_processes int) []int {
	...
	for completed != no_of_processes {
		...
		for j < no_of_processes {
			if (int(_index(arrival_time, j)) <= total_time) && (int(_index(remaining_time, j)) > 0) {
		...
		if len(ready_process) > 0 {
			...
			for k < len(ready_process) {
				...
				_ = idx
				if int(_index(remaining_time, idx)) < int(_index(remaining_time, target_process)) {
					target_process = idx
				}
				k = (k + 1)
			}
			total_time = (total_time + int(_index(burst_time, target_process)))
			completed = (completed + 1)
			_setIndex(remaining_time, target_process, 0)
			_setIndex(waiting_time, target_process, ((total_time - int(_index(arrival_time, target_process))) - int(_index(burst_time, target_process))))
		} else {
...
func main() {
	func() {
		...
		arrival_time = []int{func(v any) int {
			if vv, ok := v.(int); ok {
				return vv
			}
			return 0
