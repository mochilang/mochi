...
// Generated by Mochi v0.10.66 on 2025-08-15 11:21:05 GMT+7
package main
...
func nth_root(x float64, n int) float64 {
	...
	_ = i
	for i < 10 {
		var denom float64 = math.Pow(guess, float64((n - 1)))
		_ = denom
...
func compute_geometric_mean(nums []float64) float64 {
	...
	var mean float64 = math.Pow(math.Abs(product), 1.0/float64(len(nums)))
	_ = mean
	if product < 0.0 {
		mean = (0 - mean)
	}
	...
	_ = possible
	if math.Pow(possible, float64(len(nums))) == product {
		mean = possible
	}
	return mean
}
