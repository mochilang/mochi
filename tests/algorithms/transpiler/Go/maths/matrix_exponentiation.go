...
// Generated by Mochi v0.10.63 on 2025-08-12 08:25:54 GMT+7
package main
...
func matrix_mul(a [][]int, b [][]int) [][]int {
	...
	for i < n {
		...
		for j < n {
			...
			_ = k
			for k < n {
				cell = (cell + (int(_index(_index(a, i), k)) * int(_index(_index(b, k), j))))
				k = (k + 1)
			}
			row = append(row, cell)
			j = (j + 1)
...
func fibonacci_with_matrix_exponentiation(n int, f1 int, f2 int) int {
	...
	var base [][]int = [][]int{[]int{func(v any) int {
		if vv, ok := v.(int); ok {
			return vv
		}
		return 0
	...
	_ = m
	_ = m
	return ((f2 * int(_index(_index(m, 0), 0))) + (f1 * int(_index(_index(m, 0), 1))))
}
