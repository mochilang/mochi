...
// Generated by Mochi v0.10.64 on 2025-08-12 16:24:04 GMT+7
package main
...
func three_sum(nums []int) [][]int {
	...
	_ = i
	for i < (n - 2) {
		if (i == 0) || (int(_index(sorted, i)) != int(_index(sorted, (i-1)))) {
			var low int = (i + 1)
			_ = low
			var high int = (n - 1)
			_ = high
			var c int = (0 - int(_index(sorted, i)))
			_ = c
			for low < high {
				var s int = (int(_index(sorted, low)) + int(_index(sorted, high)))
				_ = s
				if s == c {
					...
					for (low < high) && (int(_index(sorted, low)) == int(_index(sorted, (low+1)))) {
						low = (low + 1)
					}
					for (low < high) && (int(_index(sorted, high)) == int(_index(sorted, (high-1)))) {
						high = (high - 1)
					}
					low = (low + 1)
					high = (high - 1)
