...
// Generated by Mochi v0.10.60 on 2025-08-07 19:13:17 GMT+7
package main
...
func parse_float(token string) float64 {
	...
	for (idx < len(token)) && (_substr(token, idx, (idx+1)) != ".") {
		int_part = ((int_part * 10) + func() int { n, _ := strconv.Atoi(_substr(token, idx, (idx + 1))); return n }())
		idx = (idx + 1)
	}
	...
	if (idx < len(token)) && (_substr(token, idx, (idx+1)) == ".") {
		...
		_ = place
		for idx < len(token) {
			var digit int = func() int { n, _ := strconv.Atoi(_substr(token, idx, (idx + 1))); return n }()
			_ = digit
	...
	for _, token := range tokens {
		if ((((token == "+") || (token == "-")) || (token == "*")) || (token == "/")) || (token == "^") {
			if ((token == "+") || (token == "-")) && (len(stack) < 2) {
				var b float64 = stack[(len(stack) - 1)]
				_ = b
				...
				var a float64 = stack[(len(stack) - 1)]
				_ = a
				stack = slice_without_last(func(v any) []float64 {
					if v == nil {
						return nil
