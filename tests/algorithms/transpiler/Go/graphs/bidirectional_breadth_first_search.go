...
// Generated by Mochi v0.10.73 on 2025-08-25 16:50:14 GMT+7
package main
...
func neighbors(pos string) []string {
	...
	_ = i
	for i < len(delta) {
		var ny int = (y + _index(_index(delta, i), 0))
		_ = ny
		var nx int = (x + _index(_index(delta, i), 1))
		_ = nx
		if (((ny >= 0) && (ny < len(grid))) && (nx >= 0)) && (nx < len(_index(grid, 0))) {
			if _index(_index(grid, ny), nx) == 0 {
