...
// Generated by Mochi v0.10.73 on 2025-08-25 16:50:05 GMT+7
package main
...
func search(grid [][]int, init []int, goal []int, cost int, heuristic [][]int) SearchResult {
	...
	var g int = 0
	_ = g
	var f int = (g + _index(_index(heuristic, x), y))
	_ = f
	...
	for !found && !resign {
		if len(cell) == 0 {
			...
			_ = i
			for i < len(cell) {
				if _index(_index(cell, i), 0) < best_f {
			...
			if (x == _index(goal, 0)) && (y == _index(goal, 1)) {
				found = true
			} else {
				var d int = 0
				_ = d
				for d < len(DIRECTIONS) {
					var x2 int = (x + _index(_index(DIRECTIONS, d), 0))
					_ = x2
					var y2 int = (y + _index(_index(DIRECTIONS, d), 1))
					_ = y2
					if (((((x2 >= 0) && (x2 < len(grid))) && (y2 >= 0)) && (y2 < len(_index(grid, 0)))) && (_index(_index(closed, x2), y2) == 0)) && (_index(_index(grid, x2), y2) == 0) {
						var g2 int = (g + cost)
						_ = g2
						var f2 int = (g2 + _index(_index(heuristic, x2), y2))
						_ = f2
	...
	for (x != _index(init, 0)) || (y != _index(init, 1)) {
		var dir int = _index(_index(action, x), y)
		_ = dir
		var x2 int = (x - _index(_index(DIRECTIONS, dir), 0))
		_ = x2
		var y2 int = (y - _index(_index(DIRECTIONS, dir), 1))
		_ = y2
		x = x2
		y = y2
		invpath = append(invpath, []int{func(v any) int {
...
func mochiMain() {
	...
	for i < len(grid) {
		...
		for j < len(_index(grid, 0)) {
			var h int = (iabs((i - _index(goal, 0))) + iabs((j - _index(goal, 1))))
			_ = h
			if _index(_index(grid, i), j) == 1 {
				row = append(row, 99)
			} else {
				row = append(row, h)
			}
	...
	for rr < len(result.Action) {
		fmt.Println(strings.ReplaceAll(fmt.Sprint(_index(result.Action, rr)), " ", ", "))
		rr = (rr + 1)
	}
	var p int = 0
	_ = p
	for p < len(result.Path) {
		fmt.Println(strings.ReplaceAll(fmt.Sprint(_index(result.Path, p)), " ", ", "))
		p = (p + 1)
	}
}
