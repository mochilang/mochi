...
// Generated by Mochi v0.10.61 on 2025-08-09 10:24:37 GMT+7
package main
...
func mochiMain() {
	var grid [][]int = [][]int{[]int{func(v any) int {
		if vv, ok := v.(int); ok {
			return vv
		}
		return 0
	...
	_ = grid
	var init []int = []int{func(v any) int {
		if vv, ok := v.(int); ok {
			return vv
		}
		return 0
	...
	for i < len(grid) {
		...
		for j < len(_index(grid, 0)) {
			h := (iabs((i - int(_index(goal, 0)))) + iabs((j - int(_index(goal, 1)))))
			_ = h
	...
	var rr int = 0
	_ = rr
	for rr < _len(SearchResult(result).Action) {
		fmt.Println(_index(SearchResult(result).Action, rr))
		rr = (rr + 1)
	}
	var p int = 0
	_ = p
	for p < _len(SearchResult(result).Path) {
		fmt.Println(_index(SearchResult(result).Path, p))
		p = (p + 1)
	}
}
