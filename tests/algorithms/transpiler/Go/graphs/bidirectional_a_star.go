...
// Generated by Mochi v0.10.61 on 2025-08-09 10:24:48 GMT+7
package main
...
func heuristic(a Pos, b Pos) float64 {
	...
	_ = dxf
	_ = dxf
	return sqrtApprox(float64(((_toFloat(dyf) * _toFloat(dyf)) + (_toFloat(dxf) * _toFloat(dxf)))))
}
...
func astar(start Pos, goal Pos) []Pos {
	...
	_ = closed
	for len(open) > 0 {
		idx := find_lowest_f(func(v any) []Node {
			if v == nil {
				return nil
			}
			if vv, ok := v.([]Node); ok {
		...
		for i < _len(succ) {
			...
			_ = tentative_g
			idx_open := open_index_of_pos(func(v any) []Node {
				if v == nil {
					return nil
				}
				if vv, ok := v.([]Node); ok {
			...
			if (idx_open == (0 - 1)) || (tentative_g < _index(open, idx_open).GCost) {
				new_path := append_pos_list(func(v any) []Pos {
					if v == nil {
						return nil
					}
					if vv, ok := v.([]Pos); ok {
					...
					return v.([]Pos)
				}(current.Path), Pos(pos))
				_ = new_path
				h := heuristic(Pos(pos), goal)
				_ = h
				f := (float64(tentative_g) + _toFloat(h))
				_ = f
				if idx_open != (0 - 1) {
					open = remove_node_at(func(v any) []Node {
						if v == nil {
...
func bidirectional_astar(start Pos, goal Pos) []Pos {
	...
	for (len(open_f) > 0) && (len(open_b) > 0) {
		idx_f := find_lowest_f(func(v any) []Node {
			if v == nil {
				return nil
			}
			if vv, ok := v.([]Node); ok {
		...
		idx_b := find_lowest_f(func(v any) []Node {
			if v == nil {
				return nil
			}
			if vv, ok := v.([]Node); ok {
		...
		for i < _len(succ_f) {
			...
			_ = tentative_g
			h := heuristic(Pos(pos), current_b.Pos)
			_ = h
			f := (float64(tentative_g) + _toFloat(h))
			_ = f
			idx_open := open_index_of_pos(func(v any) []Node {
		...
		for i < _len(succ_b) {
			...
			_ = tentative_g
			h := heuristic(Pos(pos), current_f.Pos)
			_ = h
			f := (float64(tentative_g) + _toFloat(h))
			_ = f
			idx_open := open_index_of_pos(func(v any) []Node {
