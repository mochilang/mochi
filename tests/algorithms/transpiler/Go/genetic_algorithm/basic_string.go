...
// Generated by Mochi v0.10.61 on 2025-08-09 10:21:16 GMT+7
package main
...
func crossover(parent1 string, parent2 string) PairString {
	cut := (len(parent1) / 2)
	_ = cut
	child1 := (_substr(parent1, 0, cut) + _substr(parent2, cut, len(parent2)))
	_ = child1
	_ = child1
	child2 := (_substr(parent2, 0, cut) + _substr(parent1, cut, len(parent1)))
	_ = child2
	_ = child2
	return PairString{
		First:  child1,
...
func mochiMain() {
	...
	_ = pair
	fmt.Println(PairString(pair).First)
	fmt.Println(PairString(pair).Second)
	mut := mutate("123456", func(v any) []string {
		if v == nil {
			return nil
		}
