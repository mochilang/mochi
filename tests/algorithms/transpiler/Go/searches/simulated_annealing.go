...
// Generated by Mochi v0.10.63 on 2025-08-11 16:29:26 GMT+7
package main
	...
	for !search_end {
		...
		for !found_next && (len(neighbors) > 0) {
			...
			if change > 0.0 {
				...
				probability := expApprox((float64(change) / float64(current_temp)))
				_ = probability
...
func mochiMain() {
	...
	_ = prob1
	min_state := simulated_annealing(SearchProblem(prob1), test_f1, false, 100.0, 5.0, 50.0, (0 - 5.0), 100.0, 0.01, 1.0)
	_ = min_state
	_ = min_state
	fmt.Println("min1", func() string {
		f := float64(test_f1(SearchProblem(min_state).X.(float64), SearchProblem(min_state).Y.(float64)))
	...
	_ = prob2
	max_state := simulated_annealing(SearchProblem(prob2), test_f1, true, 100.0, 5.0, 50.0, (0 - 5.0), 100.0, 0.01, 1.0)
	_ = max_state
	_ = max_state
	fmt.Println("max1", func() string {
		f := float64(test_f1(SearchProblem(max_state).X.(float64), SearchProblem(max_state).Y.(float64)))
		if f == 0 && math.Signbit(f) {
			return "-0"
		}
		if f == float64(int(f)) {
	...
	_ = prob3
	min_state2 := simulated_annealing(SearchProblem(prob3), test_f2, false, 1000.0, (0 - 1000.0), 1000.0, (0 - 1000.0), 100.0, 0.01, 1.0)
	_ = min_state2
	_ = min_state2
	fmt.Println("min2", func() string {
		f := float64(test_f2(SearchProblem(min_state2).X.(float64), SearchProblem(min_state2).Y.(float64)))
		if f == 0 && math.Signbit(f) {
			return "-0"
		}
		if f == float64(int(f)) {
	...
	_ = prob4
	max_state2 := simulated_annealing(SearchProblem(prob4), test_f2, true, 1000.0, (0 - 1000.0), 1000.0, (0 - 1000.0), 100.0, 0.01, 1.0)
	_ = max_state2
	_ = max_state2
	fmt.Println("max2", func() string {
		f := float64(test_f2(SearchProblem(max_state2).X.(float64), SearchProblem(max_state2).Y.(float64)))
		if f == 0 && math.Signbit(f) {
			return "-0"
		}
		if f == float64(int(f)) {
