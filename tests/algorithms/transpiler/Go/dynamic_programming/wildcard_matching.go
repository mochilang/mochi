...
// Generated by Mochi v0.10.61 on 2025-08-08 18:15:21 GMT+7
package main
...
func is_match(s string, p string) bool {
	...
	m := len(p)
	_ = m
	var dp [][]bool = make_bool_matrix(int((n + 1)), int((m + 1)))
	_ = dp
	...
	for i <= n {
		...
		_ = j2
		for j2 <= m {
			pc := string([]rune(p)[(j2 - 1):j2])
			_ = pc
			sc := string([]rune(s)[(i - 1):i])
			_ = sc
			if (_toFloat(pc) == _toFloat(sc)) || (fmt.Sprint(pc) == "?") {
