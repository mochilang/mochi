...
// Generated by Mochi v0.10.61 on 2025-08-08 18:15:13 GMT+7
package main
...
func is_sum_subset(arr []int, required_sum int) bool {
	...
	for i <= arr_len {
		j = 1
		for j <= required_sum {
			if int(_index(arr, (i-1))) > j {
				_setIndex(_index(subset, i), j, _index(_index(subset, (i-1)), j))
			}
			if int(_index(arr, (i-1))) <= j {
				_setIndex(_index(subset, i), j, (_index(_index(subset, (i-1)), j) || _index(_index(subset, (i-1)), (j-int(_index(arr, (i-1)))))))
			}
			j = (j + 1)
		}
		i = (i + 1)
