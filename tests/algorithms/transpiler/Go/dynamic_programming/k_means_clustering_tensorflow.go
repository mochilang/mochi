...
// Generated by Mochi v0.10.61 on 2025-08-08 17:57:56 GMT+7
package main
...
func k_means(vectors [][]float64, k int, iterations int) KMeansResult {
	...
	for it < iterations {
		...
		for cIdx < k {
			...
			_ = v2
			for v2 < n {
				if int(_index(assignments, v2)) == cIdx {
...
func mochiMain() {
	...
	_ = vectors
	result := k_means(func(v any) [][]float64 {
		if v == nil {
			return nil
		}
		if vv, ok := v.([][]float64); ok {
	...
	_ = result
	fmt.Println(fmt.Sprint(KMeansResult(result).Centroids))
	fmt.Println(fmt.Sprint(KMeansResult(result).Assignments))
}
