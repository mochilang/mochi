...
// Generated by Mochi v0.10.61 on 2025-08-08 17:57:58 GMT+7
package main
...
func largest_divisible_subset(items []int) []int {
	...
	for i < n {
		...
		_ = j
		for j < i {
			if ((int(_index(nums, j)) == 0) || (_mod(int(_index(nums, i)), int(_index(nums, j))) == 0)) && ((int(_index(memo, j)) + 1) > int(_index(memo, i))) {
				_setIndex(memo, i, (int(_index(memo, j)) + 1))
				_setIndex(prev, i, j)
			}
			j = (j + 1)
		}
	...
	_ = result
	for int(_index(prev, last_index)) != last_index {
...
func mochiMain() {
	var items []int = []int{func(v any) int {
		if vv, ok := v.(int); ok {
			return vv
		}
		return 0
	...
	_ = items
	subset := largest_divisible_subset(func(v any) []int {
		if v == nil {
			return nil
		}
		if vv, ok := v.([]int); ok {
