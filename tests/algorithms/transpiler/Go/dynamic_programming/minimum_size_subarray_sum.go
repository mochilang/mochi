...
// Generated by Mochi v0.10.61 on 2025-08-08 18:14:51 GMT+7
package main
...
func minimum_subarray_sum(target int, numbers []int) int {
	...
	_ = min_len
	for right < n {
		curr_sum = (curr_sum + int(_index(numbers, right)))
		for (curr_sum >= target) && (left <= right) {
			...
			curr_sum = (curr_sum - int(_index(numbers, left)))
			left = (left + 1)
		}
		right = (right + 1)
	}
