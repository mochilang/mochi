...
// Generated by Mochi v0.10.61 on 2025-08-08 17:57:19 GMT+7
package main
...
func get_rotation(img [][]int, pt1 [][]float64, pt2 [][]float64, rows int, cols int) [][]int {
	...
	_ = src
	inv := mat_inverse3(func(v any) [][]float64 {
		if v == nil {
			return nil
		}
		if vv, ok := v.([][]float64); ok {
	...
	_ = vecy
	avec := mat_vec_mul(func(v any) [][]float64 {
		if v == nil {
			return nil
		}
		if vv, ok := v.([][]float64); ok {
	...
	_ = avec
	bvec := mat_vec_mul(func(v any) [][]float64 {
		if v == nil {
			return nil
		}
		if vv, ok := v.([][]float64); ok {
	...
	for y < rows {
		...
		_ = x
		for x < cols {
			xf := (((_toFloat(a0) * (1.0 * float64(x))) + (_toFloat(a1) * (1.0 * float64(y)))) + _toFloat(a2))
			_ = xf
			yf := (((_toFloat(b0) * (1.0 * float64(x))) + (_toFloat(b1) * (1.0 * float64(y)))) + _toFloat(b2))
			_ = yf
			sx := round_to_int(xf)
			_ = sx
			sy := round_to_int(yf)
