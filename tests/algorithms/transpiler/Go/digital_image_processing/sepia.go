...
// Generated by Mochi v0.10.61 on 2025-08-08 17:57:20 GMT+7
package main
...
func make_sepia(img [][][]int, factor int) [][][]int {
	...
	for i < pixel_h {
		...
		for j < pixel_v {
			...
			_ = pixel
			grey := to_grayscale(_index(pixel, 0), _index(pixel, 1), _index(pixel, 2))
			_ = grey
...
func main() {
	func() {
		...
		image = [][][]int{[][]int{[]int{func(v any) int {
			if vv, ok := v.(int); ok {
				return vv
			}
			return 0
