...
// Generated by Mochi v0.10.63 on 2025-08-11 15:35:39 GMT+7
package main
...
func damerau_levenshtein_distance(first_string string, second_string string) int {
	...
	for i := 1; i < (len1 + 1); i++ {
		var row []int = _index(dp_matrix, i)
		_ = row
		first_char := _substr(first_string, (i - 1), i)
		_ = first_char
		_ = first_char
		for j := 1; j < (len2 + 1); j++ {
			second_char := _substr(second_string, (j - 1), j)
			_ = second_char
			_ = second_char
			cost := func() int {
				if _toFloat(first_char) == _toFloat(second_char) {
					return 0
				} else {
					return 1
				}
