// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:01:48 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func matInv(_ mat: [[Double]]) -> [[Double]] {
        ...
        while (col < n) {
            ...
            var j: Int = 0
            while (j < (2 &* n)) {
                _idx(_idx(aug, col), j) = Double((Double(_idx(_idx(aug, col), j)) / pivot))
                j = _int((j &+ 1))
            }
            var r: Int = 0
            while (r < n) {
                if (r != col) {
                    ...
                    j = 0
                    while (j < (2 &* n)) {
                        _idx(_idx(aug, r), j) = Double((Double(_idx(_idx(aug, r), j)) - (factor * Double(_idx(_idx(aug, col), j)))))
                        j = _int((j &+ 1))
                    }
                }
                r = _int((r &+ 1))
    ...
    func weight_matrix(_ point: [Double], _ x_train: [[Double]], _ tau: Double) -> [[Double]] {
        ...
        while (j < m) {
            ...
            _idx(_idx(weights, j), j) = Double(expApprox(Double((-diff_sq / Double(((2.0 * tau) * tau))))))
            j = _int((j &+ 1))
        }
        return weights
    }
    func local_weight(_ point: [Double], _ x_train: [[Double]], _ y_train: [Double], _ tau: Double) -> [[Double]] {
        ...
        while (i < _int(((y_train).count))) {
            y_col = (_append(y_col, ([Double(_idx(y_train, i))] as! [Double])) as! [[Double]])
            i = _int((i &+ 1))
        }
    ...
    let x_train: [[Double]] = ([([16.99, 10.34] as! [Double]), ([21.01, 23.68] as! [Double]), ([24.59, 25.69] as! [Double])] as! [[Double]])
    let y_train: [Double] = ([1.01, 1.66, 3.5] as! [Double])
