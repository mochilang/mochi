// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:01:24 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func load_data() -> [[String]] {
        return ([(["milk"] as! [String]), (["milk", "butter"] as! [String]), (["milk", "bread"] as! [String]), (["milk", "bread", "chips"] as! [String])] as! [[String]])
    }
    ...
    func combinations_lists(_ xs: [[String]], _ k: Int) -> [[[String]]] {
        ...
        if (k == 0) {
            result = ((_append(result, []) as! [Any?]) as! [[[String]]])
            return result
        }
        var i: Int = 0
        while (i < _int(((xs).count))) {
    ...
    func apriori(_ data: [[String]], _ min_support: Int) -> [Itemset] {
        ...
        while (_int(((itemset).count)) > 0) {
            ...
            while (m < _int(((itemset).count))) {
                var sorted_item: [String] = (sort_strings((_idx(itemset, m) as! [String])) as! [String])
                frequent = (_append(frequent, Itemset(items: sorted_item, support: (_idx(counts, m) as? Int ?? 0))) as! [Itemset])
                m = _int((m &+ 1))
            }
    ...
    var frequent_itemsets: [Itemset] = (apriori((load_data() as! [[String]]), 2) as! [Itemset])
