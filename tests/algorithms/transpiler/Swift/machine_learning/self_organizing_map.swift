// Generated by Mochi transpiler v0.10.62 on 2025-08-09 16:38:46 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func update(_ weights: [[Double]], _ sample: [Int], _ j: Int, _ alpha: Double) -> [[Double]] {
        var weights = weights
        for i in 0..<_int(((weights).count)) {
            var _tmp2 = (_idx(weights, j) as! [Double])
            _tmp2 = _set(_tmp2, i, Double((Double(_idx(_idx(weights, j)!, i)) + (alpha * Double((Double((_idx(sample, i) as? Int ?? 0)) - Double(_idx(_idx(weights, j)!, i))))))))
            weights = _set(weights, j, (_tmp2 as! [Double]))
        }
        return weights
    }
    ...
    func main() {
        ...
        for _ in 0..<epochs {
            for j in 0..<_int(((training_samples).count)) {
                ...
                weights = (update((weights as! [[Double]]), (sample as! [Int]), winner, Double(alpha)) as! [[Double]])
            }
        }
        let sample: [Int] = ({
            var _arr: [Int] = []
        ...
        let winner = _int(get_winner((weights as! [[Double]]), (sample as! [Int])))
        print(_p(("Clusters that the test sample belongs to : " + _p(winner))))
        print(_p(("Weights that have been trained : " + String(describing: matrix_to_string((weights as! [[Double]]))))))
    }
    _ = main()
    let _benchEnd = _now()
