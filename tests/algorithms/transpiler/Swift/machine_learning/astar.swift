// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:01:26 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func get_neighbours(_ p: Point, _ x_limit: Int, _ y_limit: Int) -> [Point] {
        let deltas: [Point] = ([Point(x: _int((0 &- 1)), y: _int((0 &- 1))), Point(x: _int((0 &- 1)), y: 0), Point(x: _int((0 &- 1)), y: 1), Point(x: 0, y: _int((0 &- 1))), Point(x: 0, y: 1), Point(x: 1, y: _int((0 &- 1))), Point(x: 1, y: 0), Point(x: 1, y: 1)] as! [Point])
        var neighbours: [Point] = ([] as! [Point])
        for d in deltas {
            let nx = (p.x &+ d.x)
            let ny = (p.y &+ d.y)
            if ((((0 <= nx) && (nx < x_limit)) && (0 <= ny)) && (ny < y_limit)) {
                neighbours = (_append(neighbours, Point(x: nx, y: ny)) as! [Point])
            }
        }
        return neighbours
    }
    ...
    func get_node(_ nodes: [Node], _ p: Point) -> Node {
        ...
        return Node(pos: p, parent: Point(x: _int((0 &- 1)), y: _int((0 &- 1))), g: 0, h: 0, f: 0)
    }
    func astar(_ x_limit: Int, _ y_limit: Int, _ start: Point, _ goal: Point) -> [Point] {
        var open: [Node] = ([] as! [Node])
        var closed: [Node] = ([] as! [Node])
        open = (_append(open, Node(pos: start, parent: Point(x: _int((0 &- 1)), y: _int((0 &- 1))), g: 0, h: 0, f: 0)) as! [Node])
        var current: Node = (_idx(open, 0) as! Node)
        while (_int(((open).count)) > 0) {
            ...
            for np in neighbours {
                if ((closed as! [Node]).contains((np as! Point))) {
                    continue
                }
                let g = (current.g &+ 1)
                let dx = (goal.x &- np.x)
                ...
                open = (_append(open, Node(pos: np, parent: current.pos, g: g, h: h, f: f)) as! [Node])
            }
        }
