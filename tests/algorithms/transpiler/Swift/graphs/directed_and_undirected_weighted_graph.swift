// Generated by Mochi transpiler v0.10.60 on 2025-08-08 10:43:59 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func edge_exists(_ edges: [[Int]], _ w: Int, _ v: Int) -> Bool {
        ...
        while (i < _int(((edges).count))) {
            if (((edges[i][0] as? Int ?? 0) == w) && ((edges[i][1] as? Int ?? 0) == v)) {
                return true
            }
            i = _int((i &+ 1))
        }
    ...
    func dg_add_pair(_ g: inout DirectedGraph, _ u: Int, _ v: Int, _ w: Int) {
        if (g.graph[u] != nil) {
            ...
            if (!edge_exists((edges as! [[Int]]), w, v)) {
                edges = (_append(edges, ([w, v] as! [Int])) as! [[Int]])
                var m: [Int: [[Int]]] = g.graph
                m = _set(m, u, (edges as! [[Int]]))
                g.graph = (m as! [Int: [[Int]]])
            }
        } else {
            var m0: [Int: [[Int]]] = g.graph
            m0 = _set(m0, u, ([([w, v] as! [Int])] as! [[Int]]))
            g.graph = (m0 as! [Int: [[Int]]])
        }
        if (!(g.graph[v] != nil)) {
            var m1: [Int: [[Int]]] = g.graph
            m1 = _set(m1, v, ([] as! [[Int]]))
    ...
    func dg_remove_pair(_ g: inout DirectedGraph, _ u: Int, _ v: Int) {
        if (g.graph[u] != nil) {
            ...
            var i: Int = 0
            while (i < _int(((edges).count))) {
                if ((edges[i][1] as? Int ?? 0) != v) {
                    new_edges = (_append(new_edges, (edges[i] as! [Int])) as! [[Int]])
                }
                i = _int((i &+ 1))
            }
            var m: [Int: [[Int]]] = g.graph
    ...
    func dg_dfs_util(_ g: DirectedGraph, _ node: Int, _ visited: inout [Int: Bool], _ order: inout [Int], _ d: Int) -> [Int] {
        ...
        var edges: [[Int]] = (g.graph[node] as! [[Int]])
        var i: Int = 0
        while (i < _int(((edges).count))) {
            let neigh = (edges[i][1] as? Int ?? 0)
            if (!(visited[neigh] != nil)) {
                order = (dg_dfs_util((g as! DirectedGraph), neigh, &visited, &order, d) as! [Int])
                if ((d != _int(-1)) && ((order[(_int(((order).count)) &- 1)] as? Int ?? 0) == d)) {
                    return order
                }
            }
            i = _int((i &+ 1))
    ...
    func dg_bfs(_ g: DirectedGraph, _ s: Int) -> [Int] {
        ...
        while (_int(((queue).count)) > 0) {
            let node = (queue[0] as? Int ?? 0)
            queue = (Array(queue[1..<_int(((queue).count))]) as! [Int])
            order = (_append(order, node) as! [Int])
            var edges: [[Int]] = (g.graph[node] as! [[Int]])
            var i: Int = 0
            while (i < _int(((edges).count))) {
    ...
    func dg_in_degree(_ g: DirectedGraph, _ u: Int) -> Int {
        var count: Int = 0
        for k in g.graph.keys.sorted() {
            var edges: [[Int]] = (g.graph[k] as! [[Int]])
            var i: Int = 0
            while (i < _int(((edges).count))) {
                if ((edges[i][1] as? Int ?? 0) == u) {
                    count = _int((count &+ 1))
                }
                i = _int((i &+ 1))
            }
    ...
    func dg_topological_sort(_ g: DirectedGraph) -> [Int] {
        ...
        while (i >= 0) {
            res = (_append(res, (stack[i] as? Int ?? 0)) as! [Int])
            i = _int((i &- 1))
        }
        return res
    }
    ...
    func dg_fill_graph_randomly(_ g: inout DirectedGraph, _ c: Int) {
        ...
        var i: Int = 0
        while (i < count) {
            var edge_count: Int = _int(rand_range(1, 103))
            var j: Int = 0
            while (j < edge_count) {
    ...
    func g_add_pair(_ g: inout Graph, _ u: Int, _ v: Int, _ w: Int) {
        ...
        if (g.graph[v] != nil) {
            ...
            if (!edge_exists((edges2 as! [[Int]]), w, u)) {
                edges2 = (_append(edges2, ([w, u] as! [Int])) as! [[Int]])
                var m2: [Int: [[Int]]] = g.graph
                m2 = _set(m2, v, (edges2 as! [[Int]]))
                g.graph = (m2 as! [Int: [[Int]]])
            }
        } else {
            var m3: [Int: [[Int]]] = g.graph
            m3 = _set(m3, v, ([([w, u] as! [Int])] as! [[Int]]))
    ...
    func g_remove_pair(_ g: inout Graph, _ u: Int, _ v: Int) {
        ...
        if (g.graph[v] != nil) {
            ...
            while (j < _int(((edges2).count))) {
                if ((edges2[j][1] as? Int ?? 0) != u) {
                    new_edges2 = (_append(new_edges2, (edges2[j] as! [Int])) as! [[Int]])
                }
                j = _int((j &+ 1))
            }
            var m2: [Int: [[Int]]] = g.graph
