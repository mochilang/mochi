// Generated by Mochi transpiler v0.10.60 on 2025-08-08 10:43:57 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func add_edge(_ g: inout Dinic, _ a: Int, _ b: Int, _ c: Int, _ rcap: Int) -> Void {
        var adj: [[[Int]]] = g.adj
        var list_a: [[Int]] = (adj[a] as! [[Int]])
        var list_b: [[Int]] = (adj[b] as! [[Int]])
        let e1: [Int] = ([b, _int(((list_b).count)), c, 0] as! [Int])
        let e2: [Int] = ([a, _int(((list_a).count)), rcap, 0] as! [Int])
    ...
    func dfs(_ g: inout Dinic, _ v: Int, _ sink: Int, _ flow: Int) -> Int {
        ...
        var ptr: [Int] = g.ptr
        var i: Int = (ptr[v] as? Int ?? 0)
        var adj_all: [[[Int]]] = g.adj
        var adj_v: [[Int]] = (adj_all[v] as! [[Int]])
        while (i < _int(((adj_v).count))) {
            var e: [Int] = (adj_v[i] as! [Int])
            let to = (e[0] as? Int ?? 0)
            if ((g.lvl[to] as? Int ?? 0) == ((g.lvl[v] as? Int ?? 0) &+ 1)) {
                let avail = ((e[2] as? Int ?? 0) &- (e[3] as? Int ?? 0))
                let pushed = _int(dfs(&g, to, sink, _int(min2(flow, avail))))
                if (pushed > 0) {
                    e = _set(e, 3, _int(((e[3] as? Int ?? 0) &+ pushed)))
                    adj_v = _set(adj_v, i, (e as! [Int]))
                    var adj_to: [[Int]] = (adj_all[to] as! [[Int]])
                    var back: [Int] = (adj_to[(e[1] as? Int ?? 0)] as! [Int])
                    back = _set(back, 3, _int(((back[3] as? Int ?? 0) &- pushed)))
                    adj_to = _set(adj_to, (e[1] as? Int ?? 0), (back as! [Int]))
    ...
    func max_flow(_ g: inout Dinic, _ source: Int, _ sink: Int) -> Int {
        ...
        while (l < 31) {
            ...
            while true {
                ...
                while ((qi < qe) && ((g.lvl[sink] as? Int ?? 0) == 0)) {
                    let v = (q[qi] as? Int ?? 0)
                    qi = _int((qi &+ 1))
                    var edges: [[Int]] = (g.adj[v] as! [[Int]])
                    var j: Int = 0
                    while (j < _int(((edges).count))) {
                        let e: [Int] = (edges[j] as! [Int])
                        let to = (e[0] as? Int ?? 0)
                        let residual = ((e[2] as? Int ?? 0) &- (e[3] as? Int ?? 0))
                        var lvl_inner: [Int] = g.lvl
                        if (((lvl_inner[to] as? Int ?? 0) == 0) && (residual >= threshold)) {
                            q = _set(q, qe, Int(to))
                            qe = _int((qe &+ 1))
                            lvl_inner = _set(lvl_inner, to, _int(((lvl_inner[v] as? Int ?? 0) &+ 1)))
                            g.lvl = (lvl_inner as! [Int])
                        }
                        j = _int((j &+ 1))
                    }
                }
                var p: Int = _int(dfs(&g, source, sink, INF))
                while (p > 0) {
                    flow = _int((flow &+ p))
                    p = _int(dfs(&g, source, sink, INF))
                }
                if ((g.lvl[sink] as? Int ?? 0) == 0) {
                    break
                }
            }
            l = _int((l &+ 1))
