// Generated by Mochi transpiler v0.10.66 on 2025-08-16 13:06:43 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func distance(_ a: Point, _ b: Point) -> Double {
        ...
        let dy = (b.y - a.y)
        let dz = (b.z - a.z)
        return Double(sqrt_approx(Double(absf(Double((((dx * dx) + (dy * dy)) + (dz * dz)))))))
    }
    func point_to_string(_ p: Point) -> String {
        return (_p((_p((_p((_p((_p((_p("Point(") + _p(_p(p.x)))) + _p(", "))) + _p(_p(p.y)))) + _p(", "))) + _p(_p(p.z)))) + _p(")"))
    }
    func test_distance() {
        let p1: Point = Point(x: 2.0, y: Double(-1.0), z: 7.0)
        let p2: Point = Point(x: 1.0, y: Double(-3.0), z: 5.0)
        let d = Double(distance((p1 as! Point), (p2 as! Point)))
        if (Double(absf(Double((d - 3.0)))) > 0.0001) {
