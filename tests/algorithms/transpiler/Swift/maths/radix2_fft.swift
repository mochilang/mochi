// Generated by Mochi transpiler v0.10.66 on 2025-08-16 13:07:04 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func c_mul(_ a: Complex, _ b: Complex) -> Complex {
        return Complex(re: Double(((a.re * b.re) - (a.im * b.im))), im: Double(((a.re * b.im) + (a.im * b.re))))
    }
    ...
    func exp_i(_ theta: Double) -> Complex {
        return Complex(re: Double(cos_taylor(Double(theta))), im: Double(sin_taylor(Double(theta))))
    }
    ...
    func fft(_ a: [Complex], _ invert: Bool) -> [Complex] {
        ...
        let angle = (((2.0 * PI) / Double(n)) * Double((invert ? -1.0 : 1.0)))
        var w = Complex(re: 1.0, im: 0.0)
        let wn = exp_i(Double(angle))
    ...
    func list_to_string(_ l: [Double]) -> String {
        ...
        while (i < _int(((l).count))) {
            s = ((_p(s) + _p(_p(Double(_idx(l, i))))) as! String)
            if ((i &+ 1) < _int(((l).count))) {
                s = ((_p(s) + _p(", ")) as! String)
            }
            i = _int((i &+ 1))
    ...
    func multiply_poly(_ a: [Double], _ b: [Double]) -> [Double] {
        ...
        while (i < _int(((a).count))) {
            fa = _set(fa, i, { let _tmp2 = (Complex(re: Double(_idx(a, i)), im: 0.0) as! Complex); return _tmp2 }())
            i = _int((i &+ 1))
        }
        i = 0
        while (i < _int(((b).count))) {
            fb = _set(fb, i, { let _tmp3 = (Complex(re: Double(_idx(b, i)), im: 0.0) as! Complex); return _tmp3 }())
            i = _int((i &+ 1))
        }
        ...
        while (i < ((_int(((a).count)) &+ _int(((b).count))) &- 1)) {
            ...
            res = (_append(res, Double(round_to(Double(val.re), 8))) as! [Double])
            i = _int((i &+ 1))
        }
        while ((_int(((res).count)) > 0) && (Double(_idx(res, (_int(((res).count)) &- 1))) == 0.0)) {
