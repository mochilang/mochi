// Generated by Mochi transpiler v0.10.66 on 2025-08-16 13:05:35 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func add(_ p: Polynomial, _ q: Polynomial) -> Polynomial {
        if (p.degree > q.degree) {
            ...
            var i = 0
            while (i <= q.degree) {
                coeffs = _set(coeffs, i, { let _tmp0 = Double((Double(_idx(coeffs, i)) + Double(_idx(q.coefficients, i)))); return _tmp0 }())
                i = _int((i &+ 1))
            }
            ...
            var i = 0
            while (i <= p.degree) {
                coeffs = _set(coeffs, i, { let _tmp1 = Double((Double(_idx(coeffs, i)) + Double(_idx(p.coefficients, i)))); return _tmp1 }())
                i = _int((i &+ 1))
            }
    ...
    func neg(_ p: Polynomial) -> Polynomial {
        ...
        var i = 0
        while (i <= p.degree) {
            coeffs = (_append(coeffs, -Double(_idx(p.coefficients, i))) as! [Double])
            i = _int((i &+ 1))
        }
        return Polynomial(degree: (p.degree as? Int ?? 0), coefficients: (coeffs as! [Double]))
    }
    ...
    func mul(_ p: Polynomial, _ q: Polynomial) -> Polynomial {
        ...
        while (i <= p.degree) {
            var j = 0
            while (j <= q.degree) {
                coeffs = _set(coeffs, (i &+ j), { let _tmp2 = Double((Double(_idx(coeffs, (i &+ j))) + (Double(_idx(p.coefficients, i)) * Double(_idx(q.coefficients, j))))); return _tmp2 }())
                j = _int((j &+ 1))
            }
            i = _int((i &+ 1))
        }
    ...
    func evaluate(_ p: Polynomial, _ x: Double) -> Double {
        ...
        var i = 0
        while (i <= p.degree) {
            result = Double((result + (Double(_idx(p.coefficients, i)) * Double(power(Double(x), i)))))
            i = _int((i &+ 1))
        }
        return result
    }
    func poly_to_string(_ p: Polynomial) -> String {
        ...
        var i = p.degree
        while (i >= 0) {
            let coeff = Double(_idx(p.coefficients, i))
            if (coeff != 0.0) {
                if (_int(((s).count)) > 0) {
                    if (coeff > 0.0) {
                        s = ((_p(s) + _p(" + ")) as! String)
                ...
                let abs_coeff = Double(((coeff < 0.0) ? -coeff : coeff))
                if (i == 0) {
    ...
    func derivative(_ p: Polynomial) -> Polynomial {
        ...
        var i = 0
        while (i < p.degree) {
            coeffs = (((_append(coeffs, (Double(_idx(p.coefficients, (i &+ 1))) * Double(Double((i &+ 1))))) as! [Double]) as! [Any?]) as! [Double])
            i = _int((i &+ 1))
        }
    ...
    func integral(_ p: Polynomial, _ constant: Double) -> Polynomial {
        ...
        var i = 0
        while (i <= p.degree) {
            coeffs = (((_append(coeffs, (Double(_idx(p.coefficients, i)) / Double(Double((i &+ 1))))) as! [Double]) as! [Any?]) as! [Double])
            i = _int((i &+ 1))
        }
        return Polynomial(degree: _int((p.degree &+ 1)), coefficients: (coeffs as! [Double]))
    }
    func equals(_ p: Polynomial, _ q: Polynomial) -> Bool {
        ...
        var i = 0
        while (i <= p.degree) {
            if (Double(_idx(p.coefficients, i)) != Double(_idx(q.coefficients, i))) {
                return false
            }
            i = _int((i &+ 1))
        }
    ...
    func test_polynomial() {
        ...
        if (Double(evaluate((p as! Polynomial), 2.0)) != 17.0) {
