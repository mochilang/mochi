// Generated by Mochi transpiler v0.10.63 on 2025-08-12 08:03:25 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func main() {
        ...
        while (i < _int(((x_values).count))) {
            ...
            while (j < _int(((y_values).count))) {
                let k = String(describing: key((_idx(x_values, i) as? Int ?? 0), (_idx(y_values, j) as? Int ?? 0)))
                let prob = Double(jpd[String(String(describing: k))])
                print(_p(((k + "=") + _p(prob))))
                j = _int((j &+ 1))
            }
            i = _int((i &+ 1))
        ...
        print(_p(("Vy=" + _p(vy))))
        print(_p(("Cov=" + _p(cov))))
        print(_p(("Sx=" + _p(Double(standard_deviation(Double(vx)))))))
        print(_p(("Sy=" + _p(Double(standard_deviation(Double(vy)))))))
    }
    _ = main()
    let _benchEnd = _now()
