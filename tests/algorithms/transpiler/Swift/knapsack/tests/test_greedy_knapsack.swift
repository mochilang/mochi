// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:00:54 GMT+7
import Foundation
import Dispatch
#if canImport(FoundationNetworking)
import FoundationNetworking
#endif

let stdout = FileHandle.standardOutput
extension FileHandle {
    func write(_ string: String) {
        if let data = string.data(using: .utf8) {
            self.write(data)
        }
    }
}

func _p(_ v: Any?) -> String {
    if let val = v {
        if let d = val as? Double {
            if d.rounded(.towardZero) == d {
                return String(Int64(d))
            }
        }
        return String(describing: val)
    }
    return "<nil>"
}

extension Double { init(_ v: Any) { if let d = v as? Double { self = d } else if let i = v as? Int { self = Double(i) } else if let i = v as? Int64 { self = Double(i) } else if let s = v as? String { self = Double(s) ?? 0 } else { self = 0 } } }
var _nowSeed = 0
var _nowSeeded = false
func _now() -> Int {
    if !_nowSeeded {
        if let s = ProcessInfo.processInfo.environment["MOCHI_NOW_SEED"], let v = Int(s) {
            _nowSeed = v
            _nowSeeded = true
        }
    }
    if _nowSeeded {
        _nowSeed = (_nowSeed * 1664525 + 1013904223) % 2147483647
        return _nowSeed
    }
    return Int(DispatchTime.now().uptimeNanoseconds)
}
func _int(_ v: Any) -> Int {
    if let s = v as? String { return Int(s) ?? 0 }
    if let i = v as? Int { return i }
    if let i = v as? Int64 { return Int(i) }
    if let d = v as? Double { return Int(d) }
    return 0
}
func _mem() -> Int {
    if let status = try? String(contentsOfFile: "/proc/self/status") {
        for line in status.split(separator: "\n") {
            if line.hasPrefix("VmRSS:") {
                let parts = line.split(whereSeparator: { $0 == " " || $0 == "\t" })
                if parts.count >= 2, let kb = Int(parts[1]) {
                    return kb * 1024
                }
            }
        }
    }
    return 0
}
func _idx<T>(_ xs: [T], _ i: Int) -> T? {
    var idx = i
    if idx < 0 { idx += xs.count }
    if idx >= 0 && idx < xs.count { return xs[idx] }
    return nil
}
func _append<T>(_ xs: [T], _ v: T) -> [T] {
    var out = xs
    out.append(v)
    return out
}
func _set<T>(_ xs: [T], _ idx: Int, _ v: T) -> [T] {
    var out = xs
    if idx < out.count {
        out[idx] = v
    } else {
        out.append(contentsOf: Array(repeating: v, count: idx - out.count + 1))
        out[idx] = v
    }
    return out
}
func _set<K: Hashable, V>(_ xs: [K: V], _ key: K, _ v: V) -> [K: V] {
    var out = xs
    out[key] = v
    return out
}
struct CalcResult: Codable {
    var ok: Bool
    var value: Double
    var error: String
    init() {
        self.ok = false
        self.value = 0
        self.error = ""
    }
    init(ok: Bool, value: Double, error: String) {
        self.ok = ok
        self.value = value
        self.error = error
    }
}
do {
    let _benchMemStart = _mem()
    let _benchStart = _now()
    func calc_profit(_ profit: [Int], _ weight: [Int], _ max_weight: Int) -> CalcResult {
        if (_int(((profit).count)) != _int(((weight).count))) {
            return CalcResult(ok: false, value: 0.0, error: "The length of profit and weight must be same.")
        }
        if (max_weight <= 0) {
            return CalcResult(ok: false, value: 0.0, error: "max_weight must greater than zero.")
        }
        var i: Int = 0
        while (i < _int(((profit).count))) {
            if ((_idx(profit, i) as? Int ?? 0) < 0) {
                return CalcResult(ok: false, value: 0.0, error: "Profit can not be negative.")
            }
            if ((_idx(weight, i) as? Int ?? 0) < 0) {
                return CalcResult(ok: false, value: 0.0, error: "Weight can not be negative.")
            }
            i = _int((i &+ 1))
        }
        var used: [Bool] = ([] as! [Bool])
        var j: Int = 0
        while (j < _int(((profit).count))) {
            used = (_append(used, false) as! [Bool])
            j = _int((j &+ 1))
        }
        var limit: Int = 0
        var gain: Double = 0.0
        while (limit < max_weight) {
            var max_ratio: Double = -1.0
            var idx: Int = (0 &- 1)
            var k: Int = 0
            while (k < _int(((profit).count))) {
                if !(_idx(used, k) ?? false) {
                    let ratio = (Double(_idx(profit, k)) / Double(_idx(weight, k)))
                    if (ratio > max_ratio) {
                        max_ratio = Double(ratio)
                        idx = Int(k)
                    }
                }
                k = _int((k &+ 1))
            }
            if (idx == (0 &- 1)) {
                break
            }
            used = _set(used, idx, true)
            if ((max_weight &- limit) >= (_idx(weight, idx) as? Int ?? 0)) {
                limit = _int((limit &+ (_idx(weight, idx) as? Int ?? 0)))
                gain = Double((gain + Double(_idx(profit, idx))))
            } else {
                gain = Double((gain + (Double((Double((max_weight &- limit)) / Double(_idx(weight, idx)))) * Double(_idx(profit, idx)))))
                break
            }
        }
        return CalcResult(ok: true, value: gain, error: "")
    }
    func test_sorted() -> Bool {
        let profit: [Int] = ([10, 20, 30, 40, 50, 60] as! [Int])
        let weight: [Int] = ([2, 4, 6, 8, 10, 12] as! [Int])
        let res = calc_profit((profit as! [Int]), (weight as! [Int]), 100)
        return (res.ok && (res.value == 210.0))
    }
    func test_negative_max_weight() -> Bool {
        let profit: [Int] = ([10, 20, 30, 40, 50, 60] as! [Int])
        let weight: [Int] = ([2, 4, 6, 8, 10, 12] as! [Int])
        let res = calc_profit((profit as! [Int]), (weight as! [Int]), -15)
        return ((!res.ok) && (res.error == "max_weight must greater than zero."))
    }
    func test_negative_profit_value() -> Bool {
        let profit: [Int] = ([10, -20, 30, 40, 50, 60] as! [Int])
        let weight: [Int] = ([2, 4, 6, 8, 10, 12] as! [Int])
        let res = calc_profit((profit as! [Int]), (weight as! [Int]), 15)
        return ((!res.ok) && (res.error == "Profit can not be negative."))
    }
    func test_negative_weight_value() -> Bool {
        let profit: [Int] = ([10, 20, 30, 40, 50, 60] as! [Int])
        let weight: [Int] = ([2, -4, 6, -8, 10, 12] as! [Int])
        let res = calc_profit((profit as! [Int]), (weight as! [Int]), 15)
        return ((!res.ok) && (res.error == "Weight can not be negative."))
    }
    func test_null_max_weight() -> Bool {
        let profit: [Int] = ([10, 20, 30, 40, 50, 60] as! [Int])
        let weight: [Int] = ([2, 4, 6, 8, 10, 12] as! [Int])
        let res = calc_profit((profit as! [Int]), (weight as! [Int]), 0)
        return ((!res.ok) && (res.error == "max_weight must greater than zero."))
    }
    func test_unequal_list_length() -> Bool {
        let profit: [Int] = ([10, 20, 30, 40, 50] as! [Int])
        let weight: [Int] = ([2, 4, 6, 8, 10, 12] as! [Int])
        let res = calc_profit((profit as! [Int]), (weight as! [Int]), 100)
        return ((!res.ok) && (res.error == "The length of profit and weight must be same."))
    }
    print(_p(test_sorted()))
    print(_p(test_negative_max_weight()))
    print(_p(test_negative_profit_value()))
    print(_p(test_negative_weight_value()))
    print(_p(test_null_max_weight()))
    print(_p(test_unequal_list_length()))
    print(_p(true))
    let _benchEnd = _now()
    let _benchMemEnd = _mem()
    print("{\n  \"duration_us\": \((_benchEnd - _benchStart) / 1000),\n  \"memory_bytes\": \(_benchMemEnd - _benchMemStart),\n  \"name\": \"main\"\n}")
}
