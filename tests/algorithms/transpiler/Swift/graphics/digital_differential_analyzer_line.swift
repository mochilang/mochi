// Generated by Mochi transpiler v0.10.59 on 2025-08-07 08:55:32 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func digital_differential_analyzer_line(_ p1: Point, _ p2: Point) -> [Point] {
        ...
        let steps = Int(((abs_dx > abs_dy) ? abs_dx : abs_dy))
        let x_increment = (Double(dx) / Double(steps))
        let y_increment = (Double(dy) / Double(steps))
        var coordinates: [Point] = ([] as! [Point])
        var x: Double = Double(p1.x)
        var y: Double = Double(p1.y)
        var i: Int = 0
        while (i < steps) {
            x = Double((x + x_increment))
            y = Double((y + y_increment))
            let point = Point(x: Int(round_int(Double(x))), y: Int(round_int(Double(y))))
