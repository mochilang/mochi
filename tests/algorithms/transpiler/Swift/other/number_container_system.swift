// Generated by Mochi transpiler v0.10.64 on 2025-08-12 16:34:30 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func change(_ cont: NumberContainer, _ idx: Int, _ num: Int) -> NumberContainer {
        ...
        if (indexmap[idx] != nil) {
            let old = (indexmap[idx] as? Int ?? 0)
            let indexes: [Int] = (numbermap[old] as! [Int])
            if (_int(((indexes).count)) == 1) {
                numbermap = _set(numbermap, old, ([] as! [Int]))
            } else {
                numbermap = _set(numbermap, old, (binary_search_delete((indexes as! [Int]), idx) as! [Int]))
            }
        }
        indexmap = _set(indexmap, idx, Int(num))
        if (numbermap[num] != nil) {
            numbermap = _set(numbermap, num, (binary_search_insert((numbermap[num] as! [Int]), idx) as! [Int]))
        } else {
            numbermap = _set(numbermap, num, ({
                var _arr: [Int] = []
