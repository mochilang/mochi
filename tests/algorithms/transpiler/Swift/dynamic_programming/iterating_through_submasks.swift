// Generated by Mochi transpiler v0.10.62 on 2025-08-09 16:05:35 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func list_of_submasks(_ mask: Int) -> [Int] {
        if (mask <= 0) {
            _ = fatalError(("mask needs to be positive integer, your input " + _p(mask)))
        }
        var all_submasks: [Int] = ([] as! [Int])
        var submask = mask
        while (submask != 0) {
            all_submasks = (_append(all_submasks, submask) as! [Int])
            submask = _int(bitwise_and((submask &- 1), mask))
        }
        return all_submasks
    }
