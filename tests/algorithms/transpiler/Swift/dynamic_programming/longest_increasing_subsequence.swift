// Generated by Mochi transpiler v0.10.62 on 2025-08-09 16:06:08 GMT+7
import Foundation
import Dispatch
#if canImport(FoundationNetworking)
import FoundationNetworking
#endif

let stdout = FileHandle.standardOutput
extension FileHandle {
    func write(_ string: String) {
        if let data = string.data(using: .utf8) {
            self.write(data)
        }
    }
}

func _p(_ v: Any?) -> String {
    if let val = v {
        if let d = val as? Double {
            if d.rounded(.towardZero) == d {
                return String(Int64(d))
            }
        }
        return String(describing: val)
    }
    return "<nil>"
}

extension Double { init(_ v: Any) { if let d = v as? Double { self = d } else if let i = v as? Int { self = Double(i) } else if let i = v as? Int64 { self = Double(i) } else if let s = v as? String { self = Double(s) ?? 0 } else { self = 0 } } }
var _nowSeed = 0
var _nowSeeded = false
func _now() -> Int {
    if !_nowSeeded {
        if let s = ProcessInfo.processInfo.environment["MOCHI_NOW_SEED"], let v = Int(s) {
            _nowSeed = v
            _nowSeeded = true
        }
    }
    if _nowSeeded {
        _nowSeed = (_nowSeed * 1664525 + 1013904223) % 2147483647
        return _nowSeed
    }
    return Int(DispatchTime.now().uptimeNanoseconds)
}
func _int(_ v: Any) -> Int {
    if let s = v as? String { return Int(s) ?? 0 }
    if let i = v as? Int { return i }
    if let i = v as? Int64 { return Int(i) }
    if let d = v as? Double { return Int(d) }
    return 0
}
func _mem() -> Int {
    if let status = try? String(contentsOfFile: "/proc/self/status") {
        for line in status.split(separator: "\n") {
            if line.hasPrefix("VmRSS:") {
                let parts = line.split(whereSeparator: { $0 == " " || $0 == "\t" })
                if parts.count >= 2, let kb = Int(parts[1]) {
                    return kb * 1024
                }
            }
        }
    }
    return 0
}
func _idx<T>(_ xs: [T], _ i: Int) -> T? {
    var idx = i
    if idx < 0 { idx += xs.count }
    if idx >= 0 && idx < xs.count { return xs[idx] }
    return xs.first
}
func _append<T>(_ xs: [T], _ v: T) -> [T] {
    var out = xs
    out.append(v)
    return out
}
func _concat<T>(_ a: [T], _ b: [T]) -> [T] {
    var out = a
    out.append(contentsOf: b)
    return out
}
func _len(_ v: Any) -> Int {
    if let s = v as? String { return Array(s).count }
    if let a = v as? [Any] { return a.count }
    if let a = v as? [Double] { return a.count }
    if let a = v as? [Int] { return a.count }
    if let m = v as? [AnyHashable: Any] { return m.count }
    return 0
}
func _slice<T>(_ xs: [T], _ start: Int, _ end: Int) -> [T] {
    var s = start
    var e = end
    if s < 0 { s = 0 }
    if e > xs.count { e = xs.count }
    if s > e { s = e }
    return Array(xs[s..<e])
}
do {
    let _benchMemStart = _mem()
    let _benchStart = _now()
    func longest_subsequence(_ xs: [Int]) -> [Int] {
        let n = _int(((xs).count))
        if (n <= 1) {
            return xs
        }
        let pivot = (_idx(xs, 0) as? Int ?? 0)
        var is_found = false
        var i = 1
        var longest_subseq: [Int] = ([] as! [Int])
        while ((!is_found) && (i < n)) {
            if ((_idx(xs, i) as? Int ?? 0) < pivot) {
                is_found = true
                var temp_array = _slice(Array(xs), i, n)
                temp_array = (longest_subsequence((temp_array as! [Int])) as! [Int])
                if (_int(_len(temp_array)) > _int(((longest_subseq).count))) {
                    longest_subseq = (temp_array as! [Int])
                }
            } else {
                i = _int((i &+ 1))
            }
        }
        var filtered: [Int] = ([] as! [Int])
        var j = 1
        while (j < n) {
            if ((_idx(xs, j) as? Int ?? 0) >= pivot) {
                filtered = (_append(filtered, (_idx(xs, j) as? Int ?? 0)) as! [Int])
            }
            j = _int((j &+ 1))
        }
        var candidate: [Int] = ([] as! [Int])
        candidate = (_append(candidate, pivot) as! [Int])
        candidate = ((_concat(candidate, (longest_subsequence((filtered as! [Int])) as! [Int])) as! [Any?]) as! [Int])
        if (_int(((candidate).count)) > _int(((longest_subseq).count))) {
            return candidate
        } else {
            return longest_subseq
        }
    }
    do {
        assert(((longest_subsequence(({
            var _arr: [Int] = []
            _arr = _append(_arr, 10)
            _arr = _append(_arr, 22)
            _arr = _append(_arr, 9)
            _arr = _append(_arr, 33)
            _arr = _append(_arr, 21)
            _arr = _append(_arr, 50)
            _arr = _append(_arr, 41)
            _arr = _append(_arr, 60)
            _arr = _append(_arr, 80)
            return _arr
        }() as! [Int])) as! [Int]) == ({
            var _arr: [Int] = []
            _arr = _append(_arr, 10)
            _arr = _append(_arr, 22)
            _arr = _append(_arr, 33)
            _arr = _append(_arr, 41)
            _arr = _append(_arr, 60)
            _arr = _append(_arr, 80)
            return _arr
        }() as! [Int])))
        assert(((longest_subsequence(({
            var _arr: [Int] = []
            _arr = _append(_arr, 4)
            _arr = _append(_arr, 8)
            _arr = _append(_arr, 7)
            _arr = _append(_arr, 5)
            _arr = _append(_arr, 1)
            _arr = _append(_arr, 12)
            _arr = _append(_arr, 2)
            _arr = _append(_arr, 3)
            _arr = _append(_arr, 9)
            return _arr
        }() as! [Int])) as! [Int]) == ({
            var _arr: [Int] = []
            _arr = _append(_arr, 1)
            _arr = _append(_arr, 2)
            _arr = _append(_arr, 3)
            _arr = _append(_arr, 9)
            return _arr
        }() as! [Int])))
        assert(((longest_subsequence(({
            var _arr: [Int] = []
            _arr = _append(_arr, 28)
            _arr = _append(_arr, 26)
            _arr = _append(_arr, 12)
            _arr = _append(_arr, 23)
            _arr = _append(_arr, 35)
            _arr = _append(_arr, 39)
            return _arr
        }() as! [Int])) as! [Int]) == ({
            var _arr: [Int] = []
            _arr = _append(_arr, 12)
            _arr = _append(_arr, 23)
            _arr = _append(_arr, 35)
            _arr = _append(_arr, 39)
            return _arr
        }() as! [Int])))
        assert(((longest_subsequence(({
            var _arr: [Int] = []
            _arr = _append(_arr, 9)
            _arr = _append(_arr, 8)
            _arr = _append(_arr, 7)
            _arr = _append(_arr, 6)
            _arr = _append(_arr, 5)
            _arr = _append(_arr, 7)
            return _arr
        }() as! [Int])) as! [Int]) == ({
            var _arr: [Int] = []
            _arr = _append(_arr, 5)
            _arr = _append(_arr, 7)
            return _arr
        }() as! [Int])))
        assert(((longest_subsequence(({
            var _arr: [Int] = []
            _arr = _append(_arr, 1)
            _arr = _append(_arr, 1)
            _arr = _append(_arr, 1)
            return _arr
        }() as! [Int])) as! [Int]) == ({
            var _arr: [Int] = []
            _arr = _append(_arr, 1)
            _arr = _append(_arr, 1)
            _arr = _append(_arr, 1)
            return _arr
        }() as! [Int])))
        assert(((longest_subsequence(([] as! [Int])) as! [Int]) == []))
    }
    let _benchEnd = _now()
    let _benchMemEnd = _mem()
    print("{\n  \"duration_us\": \((_benchEnd - _benchStart) / 1000),\n  \"memory_bytes\": \(_benchMemEnd - _benchMemStart),\n  \"name\": \"main\"\n}")
}
