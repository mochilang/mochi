// Generated by Mochi transpiler v0.10.60 on 2025-08-07 17:04:08 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func longest_subsequence(_ arr: [Int]) -> [Int] {
        ...
        while (i < n) {
            var prev: Int = 0
            while (prev < i) {
                if (((arr[prev] as? Int ?? 0) <= (arr[i] as? Int ?? 0)) && ((_int((((lis[prev] as! [Int])).count)) &+ 1) > _int((((lis[i] as! [Int])).count)))) {
        ...
        i = 0
        while (i < n) {
            if (_int((((lis[i] as! [Int])).count)) > _int(((result).count))) {
                result = (lis[i] as! [Int])
            }
            i = _int((i &+ 1))
        }
