// Generated by Mochi transpiler v0.10.60 on 2025-08-07 17:01:19 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func convert_to_negative(_ img: [[Int]]) -> [[Int]] {
        let h = _int(((img).count))
        let w = _int((((img[0] as! [Int])).count))
        var out: [[Int]] = ([] as! [[Int]])
        var y: Int = 0
        while (y < h) {
            var row: [Int] = ([] as! [Int])
    ...
    func change_contrast(_ img: [[Int]], _ factor: Int) -> [[Int]] {
        ...
        while (y < h) {
            ...
            while (x < w) {
                ...
                var v: Int = ((_int((_int((p &- 128)) &* factor)) / 100) &+ 128)
                v = _int(clamp_byte(v))
    ...
    func img_convolve(_ img: [[Int]], _ kernel: [[Double]]) -> [[Int]] {
        ...
        while (y < h) {
            ...
            while (x < w) {
                var acc: Double = 0.0
                var ky: Int = 0
                while (ky < _int(((kernel).count))) {
                    var kx: Int = 0
                    while (kx < _int((((kernel[0] as! [Double])).count))) {
                        let iy = ((y &+ ky) &- 1)
                        let ix = ((x &+ kx) &- 1)
                        var pixel: Int = 0
                ...
                row = (_append(row, _int(_int(acc))) as! [Int])
                x = _int((x &+ 1))
            }
            out = (_append(out, row) as! [[Int]])
            y = _int((y &+ 1))
    ...
    func median_filter(_ img: [[Int]], _ k: Int) -> [[Int]] {
        ...
        while (y < h) {
            ...
            while (x < w) {
                ...
                row = (_append(row, (sorted[(_int(((sorted).count)) / 2)] as? Int ?? 0)) as! [Int])
                x = _int((x &+ 1))
            }
            out = (_append(out, row) as! [[Int]])
            y = _int((y &+ 1))
            ...
            while (x < w) {
                ...
                while (ky < 3) {
                    ...
                    while (kx < 3) {
                        ...
                        sx = _int((sx &+ ((gx[ky][kx] as? Int ?? 0) &* pixel)))
                        sy = _int((sy &+ ((gy[ky][kx] as? Int ?? 0) &* pixel)))
                        kx = _int((kx &+ 1))
                    }
                    ky = _int((ky &+ 1))
                }
                row = (_append(row, (_int(iabs(sx)) &+ _int(iabs(sy)))) as! [Int])
                x = _int((x &+ 1))
            }
            out = (_append(out, row) as! [[Int]])
            y = _int((y &+ 1))
    ...
    func local_binary_pattern(_ img: [[Int]]) -> [[Int]] {
        ...
        while (y < h) {
            ...
            var x: Int = 0
            while (x < w) {
                row = (_append(row, _int(local_binary_value((img as! [[Int]]), x, y))) as! [Int])
                x = _int((x &+ 1))
            }
            out = (_append(out, row) as! [[Int]])
            y = _int((y &+ 1))
