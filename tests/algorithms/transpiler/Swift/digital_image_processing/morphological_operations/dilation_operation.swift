// Generated by Mochi transpiler v0.10.60 on 2025-08-07 17:00:57 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func dilation(_ image: [[Int]], _ kernel: [[Int]]) -> [[Int]] {
        let img_h = _int(((image).count))
        let img_w = _int((((image[0] as! [Int])).count))
                ...
                while (ky < k_h) {
                    ...
                    while (kx < k_w) {
                        if ((kernel[ky][kx] as? Int ?? 0) == 1) {
                            sum = _int((sum &+ (padded[(i &+ ky)][(j &+ kx)] as? Int ?? 0)))
                        }
                        kx = _int((kx &+ 1))
                    }
                    ky = _int((ky &+ 1))
    ...
    func print_float_matrix(_ mat: [[Double]]) -> Void {
        ...
        while (i < _int(((mat).count))) {
            var line: String = ""
            var j: Int = 0
            while (j < _int((((mat[i] as! [Double])).count))) {
                line = ((line + _p(Double(mat[i][j]))) as! String)
                if (j < (_int((((mat[i] as! [Double])).count)) &- 1)) {
                    line = ((line + " ") as! String)
                }
                j = _int((j &+ 1))
            }
    ...
    func print_int_matrix(_ mat: [[Int]]) -> Void {
        ...
        while (i < _int(((mat).count))) {
            var line: String = ""
            var j: Int = 0
            while (j < _int((((mat[i] as! [Int])).count))) {
                line = ((line + _p((mat[i][j] as? Int ?? 0))) as! String)
                if (j < (_int((((mat[i] as! [Int])).count)) &- 1)) {
                    line = ((line + " ") as! String)
                }
                j = _int((j &+ 1))
            }
