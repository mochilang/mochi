// Generated by Mochi transpiler v0.10.60 on 2025-08-07 17:00:08 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func canny(_ image: [[Double]], _ low: Double, _ high: Double, _ weak: Double, _ strong: Double) -> [[Double]] {
        ...
        let w = _int((((image[0] as! [Double])).count))
    ...
    func print_image(_ img: [[Double]]) {
        var r: Int = 0
        while (r < _int(((img).count))) {
            var c: Int = 0
            var line: String = ""
            while (c < _int((((img[r] as! [Double])).count))) {
                line = (((line + _p(_int(img[r][c]))) + " ") as! String)
                c = _int((c &+ 1))
            }
            print(_p(line))
            r = _int((r &+ 1))
