// Generated by Mochi transpiler v0.10.60 on 2025-08-07 17:00:31 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func my_laplacian(_ src: [[Int]], _ ksize: Int) -> [[Int]] {
        ...
        let rows = _int(((src).count))
        let cols = _int((((src[0] as! [Int])).count))
            ...
            while (j < cols) {
                ...
                while (ki < k) {
                    ...
                    while (kj < k) {
                        ...
                        sum = _int((sum &+ (val &* (kernel[ki][kj] as? Int ?? 0))))
                        kj = _int((kj &+ 1))
                    }
                    ki = _int((ki &+ 1))
                }
    ...
    var r: Int = 0
    while (r < _int(((result).count))) {
        var row_str: String = "["
        var c: Int = 0
        while (c < _int((((result[r] as! [Int])).count))) {
            row_str = ((row_str + _p((result[r][c] as? Int ?? 0))) as! String)
            if ((c &+ 1) < _int((((result[r] as! [Int])).count))) {
