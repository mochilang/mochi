// Generated by Mochi transpiler v0.10.73 on 2025-08-25 22:33:58 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func binary_step(_ vector: [Double]) -> [Int] {
        var out: [Int] = ([] as! [Int])
        var i = 0
        while ((i < _int(((vector).count)))) {
            if ((_num(_idx(vector, i)) >= 0.0)) {
                out = (_append(out, 1) as! [Int])
            } else {
                out = (_append(out, 0) as! [Int])
            }
            i = _int((i &+ 1))
        }
        return out
    ...
    func main() {
        let vector: [Double] = ({
            ...
            _arr = _append(_arr, 1.45)
            _arr = _append(_arr, -3.7000000000000002)
            _arr = _append(_arr, 0.29999999999999999)
            return _arr
        }() as! [Double])
        let result: [Int] = (binary_step((vector as! [Double])) as! [Int])
