// Generated by Mochi transpiler v0.10.60 on 2025-08-07 17:01:23 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func euclidean_distance_sqr(_ p1: [Double], _ p2: [Double]) -> Double {
        let dx = (Double(p1[0]) - Double(p2[0]))
        let dy = (Double(p1[1]) - Double(p2[1]))
        return ((dx * dx) + (dy * dy))
    }
    func column_based_sort(_ arr: [[Double]], _ column: Int) -> [[Double]] {
        var points: [[Double]] = arr
        var i: Int = 0
        while (i < _int(((points).count))) {
            var j: Int = 0
            while (j < (_int(((points).count)) &- 1)) {
                if (Double(points[j][column]) > Double(points[(j &+ 1)][column])) {
                    let tmp: [Double] = (points[j] as! [Double])
                    points = _set(points, j, (points[(j &+ 1)] as! [Double]))
    ...
    func dis_between_closest_pair(_ points: [[Double]], _ count: Int, _ min_dis: Double) -> Double {
        ...
        while (i < (count &- 1)) {
            var j: Int = (i &+ 1)
            while (j < count) {
                let current = Double(euclidean_distance_sqr((points[i] as! [Double]), (points[j] as! [Double])))
                if (current < min_dis) {
    ...
    func closest_pair_of_points_sqr(_ px: [[Double]], _ py: [[Double]], _ count: Int) -> Double {
        ...
        while (i < _int(((px).count))) {
            if (Double(abs(Double((Double(px[i][0]) - Double(px[mid][0]))))) < best) {
                strip = (_append(strip, (px[i] as! [Double])) as! [[Double]])
            }
            i = _int((i &+ 1))
        }
