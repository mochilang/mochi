// Generated by Mochi transpiler v0.10.61 on 2025-08-08 11:39:58 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func get_gas_stations(_ gas_quantities: [Int], _ costs: [Int]) -> [GasStation] {
        ...
        while (i < _int(((gas_quantities).count))) {
            stations = (_append(stations, GasStation(gas_quantity: (gas_quantities[i] as? Int ?? 0), cost: (costs[i] as? Int ?? 0))) as! [GasStation])
            i = _int((i &+ 1))
        }
        return stations
    }
    func can_complete_journey(_ gas_stations: [GasStation]) -> Int {
        ...
        while (i < _int(((gas_stations).count))) {
            total_gas = _int((total_gas &+ gas_stations[i].gas_quantity))
            total_cost = _int((total_cost &+ gas_stations[i].cost))
            i = _int((i &+ 1))
        }
        ...
        i = 0
        while (i < _int(((gas_stations).count))) {
            let station = (gas_stations[i] as! GasStation)
    ...
    let example1: [GasStation] = (get_gas_stations(([1, 2, 3, 4, 5] as! [Int]), ([3, 4, 5, 1, 2] as! [Int])) as! [GasStation])
    print(_p(_p(_int(can_complete_journey((example1 as! [GasStation]))))))
    let example2: [GasStation] = (get_gas_stations(([2, 3, 4] as! [Int]), ([3, 4, 3] as! [Int])) as! [GasStation])
