// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:48:17 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func front(_ d: LinkedDeque) -> String {
        ...
        let head = (d.nodes[d.header] as! Node)
        let idx = head.next
        let node = (d.nodes[idx] as! Node)
        return node.data
    }
    func back(_ d: LinkedDeque) -> String {
        ...
        let tail = (d.nodes[d.trailer] as! Node)
        let idx = tail.prev
        let node = (d.nodes[idx] as! Node)
        return node.data
    }
    func insert(_ d: inout LinkedDeque, _ pred: Int, _ value: String, _ succ: Int) -> LinkedDeque {
        ...
        var pred_node: Node = (nodes[pred] as! Node)
        pred_node.next = Int(new_idx)
        nodes = _set(nodes, pred, (pred_node as! Node))
        var succ_node: Node = (nodes[succ] as! Node)
