// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:53:36 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func sort_list(_ nums: [Int]) -> [Int] {
        ...
        while (i < _int(((nums).count))) {
            arr = (_append(arr, (nums[i] as? Int ?? 0)) as! [Int])
            i = _int((i &+ 1))
        }
        var j: Int = 0
        while (j < _int(((arr).count))) {
            ...
            while (k < _int(((arr).count))) {
                if ((arr[k] as? Int ?? 0) < (arr[j] as? Int ?? 0)) {
                    let tmp = (arr[j] as? Int ?? 0)
                    arr = _set(arr, j, (arr[k] as? Int ?? 0))
    ...
    func str_sll(_ sll: SortedLinkedList) -> String {
        ...
        while (i < _int(((sll.values).count))) {
            res = ((res + _p((sll.values[i] as? Int ?? 0))) as! String)
    ...
    func merge_lists(_ a: SortedLinkedList, _ b: SortedLinkedList) -> SortedLinkedList {
        ...
        while (i < _int(((a.values).count))) {
            combined = (_append(combined, (a.values[i] as? Int ?? 0)) as! [Int])
            i = _int((i &+ 1))
        }
        i = 0
        while (i < _int(((b.values).count))) {
            combined = (_append(combined, (b.values[i] as? Int ?? 0)) as! [Int])
            i = _int((i &+ 1))
        }
