// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:53:59 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func insert(_ key: Int, _ value: Int) {
        ...
        i = _int((level &- 1))
        while (i >= 0) {
            while (((node_forwards[x][i] as? Int ?? 0) != NIL) && ((node_keys[(node_forwards[x][i] as? Int ?? 0)] as? Int ?? 0) < key)) {
                x = (node_forwards[x][i] as? Int ?? 0)
            }
            update = _set(update, i, Int(x))
            i = _int((i &- 1))
        }
        x = (node_forwards[x][0] as? Int ?? 0)
        if ((x != NIL) && ((node_keys[x] as? Int ?? 0) == key)) {
        ...
        i = 0
        while (i < lvl) {
            forwards = _set(forwards, i, (node_forwards[(update[i] as? Int ?? 0)][i] as? Int ?? 0))
            node_forwards[(update[i] as? Int ?? 0)][i] = Int(idx)
            i = _int((i &+ 1))
        }
    ...
    func delete(_ key: Int) {
        ...
        i = 0
        while (i < level) {
            if ((node_forwards[(update[i] as? Int ?? 0)][i] as? Int ?? 0) == x) {
                node_forwards[(update[i] as? Int ?? 0)][i] = (node_forwards[x][i] as? Int ?? 0)
            }
            i = _int((i &+ 1))
        }
        while ((level > 1) && ((node_forwards[0][(level &- 1)] as? Int ?? 0) == NIL)) {
            level = _int((level &- 1))
        }
    }
    func to_string() -> String {
        var s: String = ""
        var x: Int = (node_forwards[0][0] as? Int ?? 0)
        while (x != NIL) {
            if (s != "") {
                s = ((s + " -> ") as! String)
            }
            s = ((((s + _p((node_keys[x] as? Int ?? 0))) + ":") + _p((node_vals[x] as? Int ?? 0))) as! String)
            x = (node_forwards[x][0] as? Int ?? 0)
        }
        return s
    }
