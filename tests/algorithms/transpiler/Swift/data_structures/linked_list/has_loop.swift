// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:53:28 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func has_loop(_ nodes: [Node], _ head: Int) -> Bool {
        ...
        var fast: Int = head
        while (fast != (0 &- 1)) {
            let fast_node1 = (nodes[fast] as! Node)
            if (fast_node1.next == (0 &- 1)) {
                return false
            }
            let fast_node2 = (nodes[fast_node1.next] as! Node)
            if (fast_node2.next == (0 &- 1)) {
                return false
            }
            let slow_node = (nodes[slow] as! Node)
    ...
    func make_nodes(_ values: [Int]) -> [Node] {
        ...
        while (i < _int(((values).count))) {
            ...
            nodes = (_append(nodes, Node(data: (values[i] as? Int ?? 0), next: next_idx)) as! [Node])
            i = _int((i &+ 1))
        }
        return nodes
    }
