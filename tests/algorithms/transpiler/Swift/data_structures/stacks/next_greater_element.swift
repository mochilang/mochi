// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:57:37 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func next_greatest_element_fast(_ xs: [Double]) -> [Double] {
        ...
        while (i < _int(((xs).count))) {
            ...
            var j: Int = (i &+ 1)
            while (j < _int(((xs).count))) {
                let inner = Double(xs[j])
                if (Double(xs[i]) < inner) {
                    next = Double(inner)
                    break
                }
                j = _int((j &+ 1))
    ...
    func next_greatest_element(_ xs: [Double]) -> [Double] {
        ...
        var i: Int = 0
        while (i < _int(((xs).count))) {
            while ((_int(((stack).count)) > 0) && (Double(xs[i]) > Double(xs[(stack[(_int(((stack).count)) &- 1)] as? Int ?? 0)]))) {
                let idx = (stack[(_int(((stack).count)) &- 1)] as? Int ?? 0)
                stack = (Array(stack[0..<(_int(((stack).count)) &- 1)]) as! [Int])
                res = (set_at_float((res as! [Double]), idx, Double(xs[i])) as! [Double])
            }
