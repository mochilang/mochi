// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:57:57 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func empty_stack() -> Stack {
        return Stack(nodes: ([] as! [Node]), head: _int((0 &- 1)))
    }
    func push(_ stack: Stack, _ value: Int) -> Stack {
        var nodes: [Node] = stack.nodes
        let idx = _int(((nodes).count))
        var new_node: Node = Node(data: Int(value), next: (stack.head as? Int ?? 0), prev: _int((0 &- 1)))
        nodes = (_append(nodes, new_node) as! [Node])
        if (stack.head != (0 &- 1)) {
            var head_node: Node = (nodes[stack.head] as! Node)
            head_node.prev = Int(idx)
            nodes = _set(nodes, stack.head, (head_node as! Node))
        }
        return Stack(nodes: (nodes as! [Node]), head: Int(idx))
    }
    func pop(_ stack: Stack) -> PopResult {
        if (stack.head == (0 &- 1)) {
            return PopResult(stack: (stack as! Stack), value: 0, ok: false)
        }
        var nodes: [Node] = stack.nodes
        var head_node: Node = (nodes[stack.head] as! Node)
        ...
        if (next_idx != (0 &- 1)) {
            ...
            next_node.prev = _int((0 &- 1))
            nodes = _set(nodes, next_idx, (next_node as! Node))
        }
        let new_stack = Stack(nodes: (nodes as! [Node]), head: Int(next_idx))
        return PopResult(stack: (new_stack as! Stack), value: Int(value), ok: true)
    }
    func top(_ stack: Stack) -> TopResult {
        ...
        return TopResult(value: (node.data as? Int ?? 0), ok: true)
    }
    ...
    func main() {
        ...
        if t.ok {
            print(_p(("Top element is " + _p(t.value))))
        } else {
            print(_p("Top element is None"))
        }
        print(_p(("Size of the stack is " + _p(_int(size((stack as! Stack)))))))
        var p: PopResult = pop((stack as! Stack))
        stack = (p.stack as! Stack)
        p = (pop((stack as! Stack)) as! PopResult)
        stack = (p.stack as! Stack)
        _ = print_stack((stack as! Stack))
        print(_p(("stack is empty: " + _p(is_empty((stack as! Stack))))))
    }
    _ = main()
    let _benchEnd = _now()
