// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:57:53 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func is_empty(_ s: StackWithQueues) -> Bool {
        return (_int(((s.main_queue).count)) == 0)
    }
    var stack: StackWithQueues = make_stack()
    _ = push(&stack, 1)
    _ = push(&stack, 2)
    _ = push(&stack, 3)
    print(_p(_p(_int(peek((stack as! StackWithQueues))))))
    print(_p(_p(_int(pop(&stack)))))
    print(_p(_p(_int(peek((stack as! StackWithQueues))))))
    print(_p(_p(_int(pop(&stack)))))
    print(_p(_p(_int(pop(&stack)))))
    print(_p(_p(is_empty((stack as! StackWithQueues)))))
    let _benchEnd = _now()
