// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:58:01 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func empty_stack() -> Stack {
        return Stack(nodes: ([] as! [Node]), top: _int(-1))
    }
    func is_empty(_ stack: Stack) -> Bool {
        return (stack.top == _int(-1))
    }
    func push(_ stack: Stack, _ item: String) -> Stack {
        let new_node = Node(value: String(describing: item), next: (stack.top as? Int ?? 0))
        var new_nodes: [Node] = stack.nodes
        new_nodes = (_append(new_nodes, new_node) as! [Node])
        let new_top = (_int(((new_nodes).count)) &- 1)
        return Stack(nodes: (new_nodes as! [Node]), top: Int(new_top))
    }
    func pop(_ stack: Stack) -> PopResult {
        ...
        let new_stack = Stack(nodes: (stack.nodes as! [Node]), top: Int(new_top))
        return PopResult(stack: (new_stack as! Stack), value: String(describing: node.value))
    }
