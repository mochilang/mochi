// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:57:41 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func parse_float(_ token: String) -> Double {
        ...
        var int_part: Int = 0
        while ((idx < _int(((token).count))) && (String(describing: String(Array(String(describing: (token as! String)))[idx..<(idx &+ 1)])) != ".")) {
            int_part = _int(((int_part &* 10) &+ _int(_int(String(describing: String(Array(String(describing: (token as! String)))[idx..<(idx &+ 1)]))))))
            idx = _int((idx &+ 1))
        }
        var result: Double = (1.0 * Double(int_part))
        if ((idx < _int(((token).count))) && (String(describing: String(Array(String(describing: (token as! String)))[idx..<(idx &+ 1)])) == ".")) {
            idx = _int((idx &+ 1))
            var place: Double = 0.1
            while (idx < _int(((token).count))) {
                let digit = _int(_int(String(describing: String(Array(String(describing: (token as! String)))[idx..<(idx &+ 1)]))))
    ...
    func evaluate(_ tokens: [String]) -> Double {
        if (_int(((tokens).count)) == 0) {
            return 0.0
        }
        var stack: [Double] = ([] as! [Double])
        for token in tokens {
            if (((((token == "+") || (token == "-")) || (token == "*")) || (token == "/")) || (token == "^")) {
                if (((token == "+") || (token == "-")) && (_int(((stack).count)) < 2)) {
                    let b = Double(stack[(_int(((stack).count)) &- 1)])
                    ...
                    let b = Double(stack[(_int(((stack).count)) &- 1)])
                    stack = (slice_without_last((stack as! [Double])) as! [Double])
                    let a = Double(stack[(_int(((stack).count)) &- 1)])
                    stack = (slice_without_last((stack as! [Double])) as! [Double])
        ...
        if (_int(((stack).count)) != 1) {
            _ = fatalError("Invalid postfix expression")
        }
        return Double(stack[0])
    }
    print(_p(_p(Double(evaluate((["2", "1", "+", "3", "*"] as! [String]))))))
    print(_p(_p(Double(evaluate((["4", "13", "5", "/", "+"] as! [String]))))))
    print(_p(_p(Double(evaluate((["5", "6", "9", "*", "+"] as! [String]))))))
    print(_p(_p(Double(evaluate((["2", "-", "3", "+"] as! [String]))))))
    print(_p(_p(Double(evaluate(([] as! [String]))))))
    let _benchEnd = _now()
