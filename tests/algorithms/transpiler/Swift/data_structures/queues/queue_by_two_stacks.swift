// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:57:06 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func str_queue(_ q: Queue) -> String {
        ...
        var i: Int = (_int(((q.stack2).count)) &- 1)
        while (i >= 0) {
            items = (_append(items, (q.stack2[i] as? Int ?? 0)) as! [Int])
            i = _int((i &- 1))
        }
        var j: Int = 0
        while (j < _int(((q.stack1).count))) {
            items = (_append(items, (q.stack1[j] as? Int ?? 0)) as! [Int])
            j = _int((j &+ 1))
        }
        var s: String = "Queue(("
        var k: Int = 0
        while (k < _int(((items).count))) {
            s = ((s + _p((items[k] as? Int ?? 0))) as! String)
    ...
    func get(_ q: Queue) -> GetResult {
        ...
        if (_int(((s2).count)) == 0) {
            while (_int(((s1).count)) > 0) {
                ...
                let v = (s1[idx] as? Int ?? 0)
                var new_s1: [Int] = ([] as! [Int])
                var i: Int = 0
                while (i < idx) {
                    new_s1 = (_append(new_s1, (s1[i] as? Int ?? 0)) as! [Int])
                    i = _int((i &+ 1))
                }
                s1 = (new_s1 as! [Int])
