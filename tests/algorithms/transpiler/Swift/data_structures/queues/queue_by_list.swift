// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:57:02 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func str_queue(_ q: Queue) -> String {
        ...
        while (i < _int(((q.entries).count))) {
            s = ((s + _p((q.entries[i] as? Int ?? 0))) as! String)
    ...
    func get(_ q: Queue) -> GetResult {
        ...
        let value = (q.entries[0] as? Int ?? 0)
        var new_entries: [Int] = ([] as! [Int])
        var i: Int = 1
        while (i < _int(((q.entries).count))) {
            new_entries = (_append(new_entries, (q.entries[i] as? Int ?? 0)) as! [Int])
            i = _int((i &+ 1))
        }
    ...
    func rotate(_ q: Queue, _ rotation: Int) -> Queue {
        ...
        while (r < rotation) {
            if (_int(((e).count)) > 0) {
                let first = (e[0] as? Int ?? 0)
                var rest: [Int] = ([] as! [Int])
                var i: Int = 1
                while (i < _int(((e).count))) {
                    rest = (_append(rest, (e[i] as? Int ?? 0)) as! [Int])
                    i = _int((i &+ 1))
                }
    ...
    func get_front(_ q: Queue) -> Int {
        return (q.entries[0] as? Int ?? 0)
    }
    var q: Queue = new_queue(([] as! [Int]))
