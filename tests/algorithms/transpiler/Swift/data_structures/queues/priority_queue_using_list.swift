// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:56:55 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func fpq_new() -> FixedPriorityQueue {
        return FixedPriorityQueue(queues: (([[], [], []] as! [[Any?]]) as! [[Int]]))
    }
    func fpq_enqueue(_ fpq: inout FixedPriorityQueue, _ priority: Int, _ data: Int) -> FixedPriorityQueue {
        ...
        if (_int((((fpq.queues[priority] as! [Int])).count)) >= 100) {
            _ = panic("Maximum queue size is 100")
            return fpq
        }
        var qs: [[Int]] = fpq.queues
        qs = _set(qs, priority, (_append((qs[priority] as! [Int]), data) as! [Int]))
    ...
    func fpq_dequeue(_ fpq: inout FixedPriorityQueue) -> FPQDequeueResult {
        ...
        while (i < _int(((qs).count))) {
            let q: [Int] = (qs[i] as! [Int])
            if (_int(((q).count)) > 0) {
                let val = (q[0] as? Int ?? 0)
                var new_q: [Int] = ([] as! [Int])
                var j: Int = 1
                while (j < _int(((q).count))) {
                    new_q = (_append(new_q, (q[j] as? Int ?? 0)) as! [Int])
                    j = _int((j &+ 1))
                }
    ...
    func fpq_to_string(_ fpq: FixedPriorityQueue) -> String {
        ...
        while (i < _int(((fpq.queues).count))) {
            var q_str: String = "["
            var q: [Int] = (fpq.queues[i] as! [Int])
            var j: Int = 0
            while (j < _int(((q).count))) {
                if (j > 0) {
                    q_str = ((q_str + ", ") as! String)
                }
                q_str = ((q_str + _p((q[j] as? Int ?? 0))) as! String)
                j = _int((j &+ 1))
            }
            q_str = ((q_str + "]") as! String)
        ...
        while (i < _int(((lines).count))) {
            ...
            res = ((res + (lines[i] as! String)) as! String)
            i = _int((i &+ 1))
        }
        return res
    }
    ...
    func epq_dequeue(_ epq: inout ElementPriorityQueue) -> EPQDequeueResult {
        ...
        var min_val: Int = (epq.queue[0] as? Int ?? 0)
        var idx: Int = 0
        var i: Int = 1
        while (i < _int(((epq.queue).count))) {
            let v = (epq.queue[i] as? Int ?? 0)
            if (v < min_val) {
                min_val = Int(v)
                idx = Int(i)
            }
        ...
        while (i < _int(((epq.queue).count))) {
            if (i != idx) {
                new_q = (_append(new_q, (epq.queue[i] as? Int ?? 0)) as! [Int])
            }
            i = _int((i &+ 1))
        }
