// Generated by Mochi transpiler v0.10.60 on 2025-08-07 16:58:24 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func new_node(_ prefix: String, _ is_leaf: Bool) -> RadixNode {
        return RadixNode(prefix: String(describing: prefix), is_leaf: Bool(is_leaf), children: ([:] as! [String: Int]))
    }
    func new_tree() -> RadixTree {
        var nodes: [RadixNode] = ([new_node("", false)] as! [RadixNode])
        return RadixTree(nodes: (nodes as! [RadixNode]))
    }
    func match_prefix(_ node: RadixNode, _ word: String) -> MatchResult {
        var x: Int = 0
        var p: String = node.prefix
        var w: String = word
        var min_len: Int = _int(((p).count))
        if (_int(((w).count)) < min_len) {
            min_len = _int(((w).count))
        }
        while (x < min_len) {
        ...
        let rem_prefix = String(describing: String(Array(String(describing: (p as! String)))[x..<_int(((p).count))]))
        let rem_word = String(describing: String(Array(String(describing: (w as! String)))[x..<_int(((w).count))]))
        return MatchResult(common: String(describing: common), rem_prefix: String(describing: rem_prefix), rem_word: String(describing: rem_word))
    }
    ...
    func insert(_ tree: inout RadixTree, _ idx: Int, _ word: String) {
        ...
        if (!has_key((children as! [String: Int]), (first as! String))) {
            ...
            children = _set(children, String(describing: first), Int(new_idx))
        ...
        new_children = _set(new_children, String(describing: String(Array(String(describing: (res.rem_prefix as! String)))[0..<1])), Int(child_idx))
        let new_idx = _int(((nodes).count))
    ...
    func delete(_ tree: inout RadixTree, _ idx: Int, _ word: String) -> Bool {
        ...
        } else if (_int(((child.children).count)) > 1) {
            child.is_leaf = false
            nodes = _set(nodes, child_idx, (child as! RadixNode))
            tree.nodes = (nodes as! [RadixNode])
        } else {
