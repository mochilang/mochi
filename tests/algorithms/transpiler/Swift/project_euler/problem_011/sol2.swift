// Generated by Mochi transpiler v0.10.66 on 2025-08-14 18:25:01 GMT+7
import Foundation
import Dispatch
...
do {
    ...
    func max_product_four(_ grid: [[Int]]) -> Int {
        ...
        while (i < 17) {
            var j = 0
            while (j < 17) {
                let temp = ((((_idx(_idx(grid, i)!, j) as? Int ?? 0) &* (_idx(_idx(grid, (i &+ 1))!, (j &+ 1)) as? Int ?? 0)) &* (_idx(_idx(grid, (i &+ 2))!, (j &+ 2)) as? Int ?? 0)) &* (_idx(_idx(grid, (i &+ 3))!, (j &+ 3)) as? Int ?? 0))
                if (temp > maximum) {
                    maximum = Int(temp)
                }
                j = _int((j &+ 1))
