...
let rec sum_of_series (first_term: int) (common_diff: int) (num_of_terms: int) =
    ...
    try
        let total: int64 = ((int64 num_of_terms) * (((int64 2) * (int64 first_term)) + ((int64 (num_of_terms - 1)) * (int64 common_diff)))) / (int64 2)
        __ret <- int total
        raise Return
        __ret
    with
...
and test_sum_of_series () =
    ...
    try
        if (sum_of_series (1) (1) (10)) <> 55 then
            failwith ("sum_of_series(1, 1, 10) failed")
        if (sum_of_series (1) (10) (100)) <> 49600 then
            failwith ("sum_of_series(1, 10, 100) failed")
        __ret
    with
        | Return -> __ret
and main () =
    ...
    try
        ...
        test_sum_of_series()
        printfn "%d" (sum_of_series (1) (1) (10))
