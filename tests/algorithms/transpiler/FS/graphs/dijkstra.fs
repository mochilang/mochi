...
let mutable heap: NodeCost array = [|{ node = "E"; cost = 0 }|]
    ...
    while (Seq.length (heap)) > 0 do
        try
            ...
            while i < (Seq.length (heap)) do
                if ((_idx heap (i)).cost) < ((_idx heap (best_idx)).cost) then
                    best_idx <- i
                i <- i + 1
            ...
            if u = "C" then
                ...
                raise Break
            try
                for v in _dictGet G (u) do
                    try
                        if visited.ContainsKey(v) then
                            raise Continue
                        let next_cost = cost + (int (_dictGet G (u).[v]))
                        heap <- Array.append heap [|{ node = v; cost = next_cost }|]
                    with
                    | Continue -> ()
                    | Break -> raise Break
            with
...
let mutable heap2: NodeCost array = [|{ node = "E"; cost = 0 }|]
    ...
    while (Seq.length (heap2)) > 0 do
        try
            ...
            while i2 < (Seq.length (heap2)) do
                if ((_idx heap2 (i2)).cost) < ((_idx heap2 (best2_idx)).cost) then
                    best2_idx <- i2
                i2 <- i2 + 1
            ...
            if u2 = "F" then
                ...
                raise Break
            try
                for v2 in _dictGet G2 (u2) do
                    try
                        if visited2.ContainsKey(v2) then
                            raise Continue
                        let next_cost2 = cost2 + (int (_dictGet G2 (u2).[v2]))
                        heap2 <- Array.append heap2 [|{ node = v2; cost = next_cost2 }|]
                    with
                    | Continue -> ()
                    | Break -> raise Break
            with
...
let mutable heap3: NodeCost array = [|{ node = "E"; cost = 0 }|]
    ...
    while (Seq.length (heap3)) > 0 do
        try
            ...
            while i3 < (Seq.length (heap3)) do
                if ((_idx heap3 (i3)).cost) < ((_idx heap3 (best3_idx)).cost) then
                    best3_idx <- i3
                i3 <- i3 + 1
            ...
            if u3 = "F" then
                ...
                raise Break
            try
                for v3 in _dictGet G3 (u3) do
                    try
                        if visited3.ContainsKey(v3) then
                            raise Continue
                        let next_cost3 = cost3 + (int (_dictGet G3 (u3).[v3]))
                        heap3 <- Array.append heap3 [|{ node = v3; cost = next_cost3 }|]
                    with
                    | Continue -> ()
                    | Break -> raise Break
            with
