...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  def contains(lst, v)
    i = 0
    while i < _len(lst)
      if _eq((__tmp1 = lst; __tmp1.is_a?(Hash) ? __tmp1[i] : _idx(__tmp1, i)), v)
        return true
      end
      i = _add(i, 1)
    end
  ...
  def depth_first_search(graph, start)
    ...
    while _len(stack) > 0
      idx = _len(stack) - 1
      v = (__tmp2 = stack; __tmp2.is_a?(Hash) ? __tmp2[idx] : _idx(__tmp2, idx))
      stack = stack[0...idx]
      neighbors = (__tmp3 = graph; __tmp3.is_a?(Hash) ? __tmp3[v] : _idx(__tmp3, v))
      i = _len(neighbors) - 1
      while i >= 0
        adj = (__tmp4 = neighbors; __tmp4.is_a?(Hash) ? __tmp4[i] : _idx(__tmp4, i))
