...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def create_graph(n, edges)
    ...
    __tmp16.each do |e|
      ...
      g[u] = ((__tmp19 = g; __tmp19.is_a?(Hash) ? __tmp19[u] : _idx(__tmp19, u)) + [v])
    end
    return g
  end
  def main()
    ...
    i = 0
    while i < _len(source)
      edges = (edges + [[(__tmp20 = source; __tmp20.is_a?(Hash) ? __tmp20[i] : _idx(__tmp20, i)), (__tmp21 = target; __tmp21.is_a?(Hash) ? __tmp21[i] : _idx(__tmp21, i))]])
      i = _add(i, 1)
    end
