...
start = _now()
  def validate_initial_digits(cc)
    return cc[0...2] == "34" || cc[0...2] == "35" || cc[0...2] == "37" || cc[0...1] == "4" || cc[0...1] == "5" || cc[0...1] == "6"
  end
  def luhn_validation(cc)
    sum = 0.clone
    double_digit = false.clone
    i = cc.length - 1.clone
    while i >= 0
      n = (cc[i..._add(i, 1)]).to_i.clone
      if double_digit
        n = n * 2.clone
        if n > 9
          n = n - 9.clone
      ...
      sum = _add(sum, n).clone
      double_digit = !double_digit.clone
      i = i - 1.clone
    end
    return sum % 10 == 0
  end
