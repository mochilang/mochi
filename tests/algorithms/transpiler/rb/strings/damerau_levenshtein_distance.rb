...
start = _now()
  def damerau_levenshtein_distance(first_string, second_string)
    ...
    (0...(_add(len1, 1))).each do |_|
      ...
      dp_matrix = (dp_matrix << (row)).clone
    end
    (0...(_add(len1, 1))).each do |i|
      row = dp_matrix[i].clone
      row[0] = i
      dp_matrix[i] = row
    end
    first_row = dp_matrix[0].clone
    ...
    (1...(_add(len1, 1))).each do |i|
      ...
      (1...(_add(len2, 1))).each do |j|
        ...
        value = _add(dp_matrix[i - 1][j], 1).clone
        insertion = _add(row[j - 1], 1)
        if insertion < value
          value = insertion.clone
        end
        ...
        row[j] = value
        if i > 1 && j > 1 && first_string[i - 1...i] == second_string[j - 2...j - 1] && first_string[i - 2...i - 1] == second_string[j - 1...j]
