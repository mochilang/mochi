...
start = _now()
  ...
  def top_k_frequent_words(words, k_value)
    ...
    __tmp1.each do |w|
      heap = (heap << (WordCount.new(word: w, count: $freq_map[w]))).clone
    end
    build_max_heap(heap)
    result = [].clone
    heap_size = heap.length.clone
    limit = k_value.clone
    if limit > heap_size
      limit = heap_size.clone
    end
    j = 0.clone
    while j < limit
      item = heap[0]
      result = (result << (item["word"])).clone
      heap[0] = heap[heap_size - 1]
      heap[heap_size - 1] = item
      heap_size = heap_size - 1.clone
