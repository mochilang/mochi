...
start = _now()
  ...
  def levenshtein_distance(first_word, second_word)
    ...
    previous_row = range_list(_add(second_word.length, 1)).clone
    i = 0.clone
    while i < first_word.length
      c1 = first_word[i]
      current_row = [].clone
      current_row = (current_row << (_add(i, 1))).clone
      j = 0.clone
      while j < second_word.length
        c2 = second_word[j]
        insertions = _add(previous_row[_add(j, 1)], 1)
        deletions = _add(current_row[j], 1)
        substitutions = _add(previous_row[j], ((c1 == c2 ? 0 : 1)))
        min_val = min3(insertions, deletions, substitutions)
        current_row = (current_row << (min_val)).clone
        j = _add(j, 1).clone
      end
