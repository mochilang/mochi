...
start = _now()
  ...
  def is_alnum(c)
    return ("0123456789").include?(c) || ("abcdefghijklmnopqrstuvwxyz").include?(c) || ("ABCDEFGHIJKLMNOPQRSTUVWXYZ").include?(c) || c == " "
  end
  def split_input(text)
    ...
    __tmp2.each_char do |ch|
      if is_alnum(ch)
        ...
        if current != ""
          result = (result << (split_words(current))).clone
          current = "".clone
        end
      end
    end
  ...
  def to_simple_case(text)
    ...
    __tmp3.each do |sub|
      ...
      __tmp4.each do |w|
        res = _add(res, capitalize(w)).clone
      end
    end
    return res
  end
  def to_complex_case(text, upper_flag, sep)
    ...
    __tmp5.each do |sub|
      ...
      __tmp6.each do |w|
        word = (upper_flag ? w.upcase() : w.downcase()).clone
        if first
          res = _add(res, word).clone
          first = false.clone
        else
          res = _add(_add(res, sep), word).clone
        end
      end
    end
    return res
