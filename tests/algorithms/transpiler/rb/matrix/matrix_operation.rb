...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  def add(matrices)
    rows = (__tmp1 = matrices; __tmp1.is_a?(Hash) ? __tmp1[0] : _idx(__tmp1, 0)).length
    cols = (__tmp2 = (__tmp3 = matrices; __tmp3.is_a?(Hash) ? __tmp3[0] : _idx(__tmp3, 0)); __tmp2.is_a?(Hash) ? __tmp2[0] : _idx(__tmp2, 0)).length
    r = 0
    result = []
    while r < rows
      row = []
  ...
  def subtract(a, b)
    rows = a.length
    cols = (__tmp7 = a; __tmp7.is_a?(Hash) ? __tmp7[0] : _idx(__tmp7, 0)).length
    r = 0
    result = []
    while r < rows
      row = []
      c = 0
      while c < cols
        row = (row + [(__tmp8 = (__tmp9 = a; __tmp9.is_a?(Hash) ? __tmp9[r] : _idx(__tmp9, r)); __tmp8.is_a?(Hash) ? __tmp8[c] : _idx(__tmp8, c)) - (__tmp10 = (__tmp11 = b; __tmp11.is_a?(Hash) ? __tmp11[r] : _idx(__tmp11, r)); __tmp10.is_a?(Hash) ? __tmp10[c] : _idx(__tmp10, c))])
        c = _add(c, 1)
      end
      result = (result + [row])
      r = _add(r, 1)
  ...
  def scalar_multiply(matrix, n)
    ...
    while i < matrix.length
      row = []
      j = 0
      while j < (__tmp12 = matrix; __tmp12.is_a?(Hash) ? __tmp12[i] : _idx(__tmp12, i)).length
        row = (row + [(__tmp13 = (__tmp14 = matrix; __tmp14.is_a?(Hash) ? __tmp14[i] : _idx(__tmp14, i)); __tmp13.is_a?(Hash) ? __tmp13[j] : _idx(__tmp13, j)) * n])
        j = _add(j, 1)
      end
      result = (result + [row])
      i = _add(i, 1)
  ...
  def multiply(a, b)
    rowsA = a.length
    colsA = (__tmp15 = a; __tmp15.is_a?(Hash) ? __tmp15[0] : _idx(__tmp15, 0)).length
    rowsB = b.length
    colsB = (__tmp16 = b; __tmp16.is_a?(Hash) ? __tmp16[0] : _idx(__tmp16, 0)).length
    result = []
    i = 0
    while i < rowsA
      row = []
  ...
  def transpose(matrix)
    rows = matrix.length
    cols = (__tmp21 = matrix; __tmp21.is_a?(Hash) ? __tmp21[0] : _idx(__tmp21, 0)).length
    result = []
    c = 0
    while c < cols
      row = []
      r = 0
      while r < rows
        row = (row + [(__tmp22 = (__tmp23 = matrix; __tmp23.is_a?(Hash) ? __tmp23[r] : _idx(__tmp23, r)); __tmp22.is_a?(Hash) ? __tmp22[c] : _idx(__tmp22, c))])
        r = _add(r, 1)
      end
      result = (result + [row])
      c = _add(c, 1)
  ...
  def minor(matrix, row, column)
    ...
    while i < matrix.length
      if !_eq(i, row)
        new_row = []
        j = 0
        while j < (__tmp24 = matrix; __tmp24.is_a?(Hash) ? __tmp24[i] : _idx(__tmp24, i)).length
          if !_eq(j, column)
            new_row = (new_row + [(__tmp25 = (__tmp26 = matrix; __tmp26.is_a?(Hash) ? __tmp26[i] : _idx(__tmp26, i)); __tmp25.is_a?(Hash) ? __tmp25[j] : _idx(__tmp25, j))])
          end
          j = _add(j, 1)
        end
  ...
  def determinant(matrix)
    ...
    det = 0.0
    c = 0
    while c < (__tmp29 = matrix; __tmp29.is_a?(Hash) ? __tmp29[0] : _idx(__tmp29, 0)).length
  ...
  def inverse(matrix)
    ...
    while i < size
      ...
      while j < size
        ...
        row = (row + [(__tmp32 = (__tmp33 = matrix_minor; __tmp33.is_a?(Hash) ? __tmp33[i] : _idx(__tmp33, i)); __tmp32.is_a?(Hash) ? __tmp32[j] : _idx(__tmp32, j)) * sign])
        j = _add(j, 1)
      end
