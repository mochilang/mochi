...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def test_double_factorial()
    if !_eq(double_factorial_recursive(0), 1)
      panic("0!! recursive failed")
    end
    if !_eq(double_factorial_iterative(0), 1)
      panic("0!! iterative failed")
    end
    if !_eq(double_factorial_recursive(1), 1)
      panic("1!! recursive failed")
    end
    if !_eq(double_factorial_iterative(1), 1)
      panic("1!! iterative failed")
    end
    if !_eq(double_factorial_recursive(5), 15)
      panic("5!! recursive failed")
    end
    if !_eq(double_factorial_iterative(5), 15)
      panic("5!! iterative failed")
    end
    if !_eq(double_factorial_recursive(6), 48)
      panic("6!! recursive failed")
    end
    if !_eq(double_factorial_iterative(6), 48)
      panic("6!! iterative failed")
    end
    n = 0
    while n <= 10
      if !_eq(double_factorial_recursive(n), double_factorial_iterative(n))
