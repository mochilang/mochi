...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def find_max_iterative(nums)
    ...
    max_num = (__tmp1 = nums; __tmp1.is_a?(Hash) ? __tmp1[0] : _idx(__tmp1, 0))
    i = 0
    while i < _len(nums)
      x = (__tmp2 = nums; __tmp2.is_a?(Hash) ? __tmp2[i] : _idx(__tmp2, i))
      if x > max_num
        max_num = x
      end
      i = _add(i, 1)
  ...
  def test_find_max()
    ...
    if !_eq(find_max_iterative(arr), 94.0)
      panic("find_max_iterative failed")
    end
    if !_eq(find_max_recursive(arr, 0, _len(arr) - 1), 94.0)
      panic("find_max_recursive failed")
    end
    if !_eq(find_max_recursive(arr, -_len(arr), -1), 94.0)
  ...
  def main()
    ...
    puts(find_max_recursive(nums, 0, _len(nums) - 1))
  end
  main()
