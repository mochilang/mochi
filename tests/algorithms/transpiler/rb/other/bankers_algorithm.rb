...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  def processes_resource_summation(alloc)
    resources = (__tmp1 = alloc; __tmp1.is_a?(Hash) ? __tmp1[0] : _idx(__tmp1, 0)).length
    sums = []
    i = 0
    while i < resources
      total = 0
  ...
  def available_resources(claim, alloc_sum)
    ...
    while i < claim.length
      avail = (avail + [(__tmp4 = claim; __tmp4.is_a?(Hash) ? __tmp4[i] : _idx(__tmp4, i)) - (__tmp5 = alloc_sum; __tmp5.is_a?(Hash) ? __tmp5[i] : _idx(__tmp5, i))])
      i = _add(i, 1)
    end
    return avail
  end
  def need(max, alloc)
    ...
    while i < max.length
      row = []
      j = 0
      while j < (__tmp6 = max; __tmp6.is_a?(Hash) ? __tmp6[0] : _idx(__tmp6, 0)).length
        row = (row + [(__tmp7 = (__tmp8 = max; __tmp8.is_a?(Hash) ? __tmp8[i] : _idx(__tmp8, i)); __tmp7.is_a?(Hash) ? __tmp7[j] : _idx(__tmp7, j)) - (__tmp9 = (__tmp10 = alloc; __tmp10.is_a?(Hash) ? __tmp10[i] : _idx(__tmp10, i)); __tmp9.is_a?(Hash) ? __tmp9[j] : _idx(__tmp9, j))])
        j = _add(j, 1)
      end
      needs = (needs + [row])
      i = _add(i, 1)
