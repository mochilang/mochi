...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def is_balanced(s)
    ...
    while i < _len(s)
      symbol = s[i..._add(i, 1)]
      if _has($OPEN_TO_CLOSED, symbol)
        stack = _append(stack, symbol)
      else
        if _eq(symbol, ")") || _eq(symbol, "]") || _eq(symbol, "}")
          if _eq(_len(stack), 0)
            return false
          end
          top = (__tmp2 = stack; __tmp2.is_a?(Hash) ? __tmp2[_len(stack) - 1] : _idx(__tmp2, _len(stack) - 1))
          if !_eq((__tmp3 = $OPEN_TO_CLOSED; __tmp3.is_a?(Hash) ? __tmp3[top] : _idx(__tmp3, top)), symbol)
            return false
          end
