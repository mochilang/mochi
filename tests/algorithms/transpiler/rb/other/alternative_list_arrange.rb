...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def alternative_list_arrange(first, second)
    ...
    while i < abs_len
      if i < len1
        result = _append(result, (__tmp1 = first; __tmp1.is_a?(Hash) ? __tmp1[i] : _idx(__tmp1, i)))
      end
      if i < len2
        result = _append(result, (__tmp2 = second; __tmp2.is_a?(Hash) ? __tmp2[i] : _idx(__tmp2, i)))
      end
      i = _add(i, 1)
    end
    return result
  ...
  def list_to_string(xs)
    ...
    i = 0
    while i < _len(xs)
      s = _add(s, item_to_string((__tmp3 = xs; __tmp3.is_a?(Hash) ? __tmp3[i] : _idx(__tmp3, i))))
      if i < _len(xs) - 1
        s = _add(s, ", ")
      end
      i = _add(i, 1)
  ...
  Object.const_set(:Int, Struct.new(:value, keyword_init: true))
  Object.send(:remove_const, :Str) if Object.const_defined?(:Str)
  Object.const_set(:Str, Struct.new(:value, keyword_init: true))
