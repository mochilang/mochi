...
start = _now()
  ...
  def max_path_sum(tr)
    ...
    while i >= 0
      ...
      j = 0.clone
      while j < curr.length
        best = max_int(prev[j], prev[_add(j, 1)])
        new_row = (new_row << (_add(curr[j], best))).clone
        j = _add(j, 1).clone
      end
      prev = new_row.clone
      i = i - 1.clone
