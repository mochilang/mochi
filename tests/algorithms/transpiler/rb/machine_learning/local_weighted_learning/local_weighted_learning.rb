...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def transpose(mat)
    ...
    while i < cols
      ...
      j = 0
      while j < rows
        row = (row + [(__tmp2 = (__tmp3 = mat; __tmp3.is_a?(Hash) ? __tmp3[j] : _idx(__tmp3, j)); __tmp2.is_a?(Hash) ? __tmp2[i] : _idx(__tmp2, i))])
        j = _add(j, 1)
      end
      res = (res + [row])
      i = _add(i, 1)
  ...
  def matInv(mat)
    ...
    while i < n
      ...
      j = 0
      while j < n
        row = (row + [(__tmp10 = (__tmp11 = mat; __tmp11.is_a?(Hash) ? __tmp11[i] : _idx(__tmp11, i)); __tmp10.is_a?(Hash) ? __tmp10[j] : _idx(__tmp10, j))])
        j = _add(j, 1)
      end
      j = 0
      while j < n
    ...
    while i < n
      ...
      j = 0
      while j < n
        row = (row + [(__tmp24 = (__tmp25 = aug; __tmp25.is_a?(Hash) ? __tmp25[i] : _idx(__tmp25, i)); __tmp24.is_a?(Hash) ? __tmp24[_add(j, n)] : _idx(__tmp24, _add(j, n)))])
        j = _add(j, 1)
      end
      inv = (inv + [row])
      i = _add(i, 1)
  ...
  def local_weight(point, x_train, y_train, tau)
    ...
    while i < _len(y_train)
      y_col = (y_col + [[(__tmp30 = y_train; __tmp30.is_a?(Hash) ? __tmp30[i] : _idx(__tmp30, i))]])
      i = _add(i, 1)
    end
  ...
  def local_weight_regression(x_train, y_train, tau)
    ...
    while i < m
      ...
      k = 0
      while k < _len(theta)
        weights_vec = (weights_vec + [(__tmp32 = (__tmp33 = theta; __tmp33.is_a?(Hash) ? __tmp33[k] : _idx(__tmp33, k)); __tmp32.is_a?(Hash) ? __tmp32[0] : _idx(__tmp32, 0))])
        k = _add(k, 1)
      end
      pred = 0.0
      j = 0
