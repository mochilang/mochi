...
start = Process.clock_gettime(Process::CLOCK_MONOTONIC, :nanosecond)
  ...
  def sigmoid_vec(v)
    ...
    i = 0
    while i < v.length
      res = (res + [sigmoid((__tmp1 = v; __tmp1.is_a?(Hash) ? __tmp1[i] : _idx(__tmp1, i)))])
      i = _add(i, 1)
    end
    return res
  end
  ...
  def matTvec(mat, vec)
    cols = (__tmp6 = mat; __tmp6.is_a?(Hash) ? __tmp6[0] : _idx(__tmp6, 0)).length
    res = []
    j = 0
    while j < cols
      s = 0.0
  ...
  def vec_sub(a, b)
    ...
    i = 0
    while i < a.length
      res = (res + [(__tmp10 = a; __tmp10.is_a?(Hash) ? __tmp10[i] : _idx(__tmp10, i)) - (__tmp11 = b; __tmp11.is_a?(Hash) ? __tmp11[i] : _idx(__tmp11, i))])
      i = _add(i, 1)
    end
    return res
  end
  def vec_mul(a, b)
    ...
    i = 0
    while i < a.length
      res = (res + [(__tmp12 = a; __tmp12.is_a?(Hash) ? __tmp12[i] : _idx(__tmp12, i)) * (__tmp13 = b; __tmp13.is_a?(Hash) ? __tmp13[i] : _idx(__tmp13, i))])
      i = _add(i, 1)
    end
    return res
  end
  def vec_scalar_mul(v, s)
    ...
    i = 0
    while i < v.length
      res = (res + [(__tmp14 = v; __tmp14.is_a?(Hash) ? __tmp14[i] : _idx(__tmp14, i)) * s])
      i = _add(i, 1)
    end
    return res
  end
  def outer(a, b)
    ...
    while i < a.length
      ...
      j = 0
      while j < b.length
        row = (row + [(__tmp15 = a; __tmp15.is_a?(Hash) ? __tmp15[i] : _idx(__tmp15, i)) * (__tmp16 = b; __tmp16.is_a?(Hash) ? __tmp16[j] : _idx(__tmp16, j))])
        j = _add(j, 1)
      end
      res = (res + [row])
      i = _add(i, 1)
  ...
  def mat_scalar_mul(mat, s)
    ...
    while i < mat.length
      row = []
      j = 0
      while j < (__tmp17 = mat; __tmp17.is_a?(Hash) ? __tmp17[i] : _idx(__tmp17, i)).length
        row = (row + [(__tmp18 = (__tmp19 = mat; __tmp19.is_a?(Hash) ? __tmp19[i] : _idx(__tmp19, i)); __tmp18.is_a?(Hash) ? __tmp18[j] : _idx(__tmp18, j)) * s])
        j = _add(j, 1)
      end
      res = (res + [row])
      i = _add(i, 1)
  ...
  def mat_sub(a, b)
    ...
    while i < a.length
      row = []
      j = 0
      while j < (__tmp20 = a; __tmp20.is_a?(Hash) ? __tmp20[i] : _idx(__tmp20, i)).length
        row = (row + [(__tmp21 = (__tmp22 = a; __tmp22.is_a?(Hash) ? __tmp22[i] : _idx(__tmp22, i)); __tmp21.is_a?(Hash) ? __tmp21[j] : _idx(__tmp21, j)) - (__tmp23 = (__tmp24 = b; __tmp24.is_a?(Hash) ? __tmp24[i] : _idx(__tmp24, i)); __tmp23.is_a?(Hash) ? __tmp23[j] : _idx(__tmp23, j))])
        j = _add(j, 1)
      end
      res = (res + [row])
      i = _add(i, 1)
  ...
  def calc_gradient(y, yhat)
    ...
    i = 0
    while i < y.length
      g = (g + [2.0 * ((__tmp29 = yhat; __tmp29.is_a?(Hash) ? __tmp29[i] : _idx(__tmp29, i)) - (__tmp30 = y; __tmp30.is_a?(Hash) ? __tmp30[i] : _idx(__tmp30, i)))])
      i = _add(i, 1)
    end
    return g
  end
  def train(layers, xdata, ydata, rounds, acc)
    ...
    while r < rounds
      ...
      while i < xdata.length
        ...
        out = (__tmp32 = layers; __tmp32.is_a?(Hash) ? __tmp32[layers.length - 1] : _idx(__tmp32, layers.length - 1)).output
