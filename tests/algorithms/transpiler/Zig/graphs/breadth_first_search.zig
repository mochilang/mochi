...
var g_var: std.AutoHashMap(i64,[]i64) = std.AutoHashMap(i64,[]i64).init(std.heap.page_allocator);
...
fn add_edge(graph_param: *std.AutoHashMap(i64,[]i64), from: i64, to: i64) void {
    if (graph_param.contains(from)) {
        ...
        graph_param.put(from, @constCast(&[1]i64{to})[0..]) catch unreachable;
    }
}
...
pub fn main() void {
    {
        ...
        g_var = std.AutoHashMap(i64, []i64).init(std.heap.page_allocator);
        add_edge(&g_var, 0, 1);
        add_edge(&g_var, 0, 2);
        add_edge(&g_var, 1, 2);
        add_edge(&g_var, 2, 0);
