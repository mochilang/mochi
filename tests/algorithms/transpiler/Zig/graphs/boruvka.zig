// Generated by Mochi Zig transpiler on 2025-08-14 15:55 +0700
...
fn find_component(g: Graph, node: i64) i64 {
    if (g.component.get(node).? == node) {
        return node;
    }
    return find_component(g, g.component.get(node).?);
}

fn set_component(g_param: Graph, node: i64) Graph {
    var g_var: Graph = g_param;
    g_var = g_var;
    if (g_var.component.get(node).? != node) {
        var comp: std.AutoHashMap(i64,i64) = g_var.component;
        comp = comp;
        var k: i64 = 0;
        k = k;
        while (k < g_var.num_nodes) {
            comp.put(k, find_component(g_var, k)) catch unreachable;
            k = k + 1;
        }
        g_var = .{ .num_nodes = g_var.num_nodes, .edges = g_var.edges, .component = comp };
    }
    return g_var;
}

fn union_(g_param_1: Graph, component_size: []i64, u: i64, v: i64) UnionResult {
    var g_var: Graph = g_param_1;
    g_var = g_var;
    ...
    if (comp_size[_idx(comp_size.len, u)] <= comp_size[_idx(comp_size.len, v)]) {
        ...
        g_var = .{ .num_nodes = g_var.num_nodes, .edges = g_var.edges, .component = comp };
        g_var = set_component(g_var, v);
    }
    return .{ .graph = g_var, .component_size = comp_size };
}
...
fn boruvka(g_param_2: Graph) i64 {
    ...
    var num_components: i64 = g_var.num_nodes;
    num_components = num_components;
    var minimum_weight_edge: []Edge = create_empty_edges(g_var.num_nodes);
    minimum_weight_edge = minimum_weight_edge;
    while (num_components > 1) {
        for (g_var.edges) |__it0| {
            const e = __it0;
            const u: i64 = e.u;
            const v: i64 = e.v;
            const w: i64 = e.w;
            const u_comp: i64 = g_var.component.get(u).?;
            const v_comp: i64 = g_var.component.get(v).?;
            if (u_comp != v_comp) {
        ...
        for (minimum_weight_edge) |__it1| {
            ...
            if (e.u != -1) {
                ...
                if (u_comp != v_comp) {
                    ...
                    const res: UnionResult = union_(g_var, component_size, u_comp, v_comp);
                    g_var = res.graph;
        ...
        minimum_weight_edge = create_empty_edges(g_var.num_nodes);
    }
