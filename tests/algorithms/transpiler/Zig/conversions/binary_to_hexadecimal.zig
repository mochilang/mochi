...
fn bin_to_hexadecimal(binary_str: []const u8) []const u8 {
    ...
    const groups: i64 = @divTrunc(@as(i64, @intCast(s.len)), 4) + 1;
    const pad_len: i64 = groups * 4 - @as(i64, @intCast(s.len));
    s = _concat_string(repeat_char("0", pad_len), s);
    const digits: []const u8 = "0123456789abcdef";
    var res: []const u8 = "0x";
    var j: i64 = 0;
    while (j < @as(i64, @intCast(s.len))) {
        const chunk: []const u8 = slice(s, j, j + 4);
        const val: i64 = bits_to_int(chunk);
