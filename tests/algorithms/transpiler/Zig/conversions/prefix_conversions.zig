...
fn convert_si_prefix(known_amount: f64, known_prefix: []const u8, unknown_prefix: []const u8) f64 {
    const kp: []const u8 = _lower(known_prefix);
    const up: []const u8 = _lower(unknown_prefix);
    ...
    const diff: i64 = SI_UNITS_var.get(kp).? - SI_UNITS_var.get(up).?;
...
fn convert_binary_prefix(known_amount: f64, known_prefix: []const u8, unknown_prefix: []const u8) f64 {
    ...
    const diff: i64 = (BINARY_UNITS_var.get(kp).? - BINARY_UNITS_var.get(up).?) * 10;
