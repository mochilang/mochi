...
fn rectangular_to_polar(real: f64, img: f64) []f64 {
    const mod: f64 = round(sqrtApprox(real * real + img * img), 2);
    const ang: f64 = round(deg(atan2Approx(img, real)), 2);
...
fn show(real: f64, img: f64) void {
    const r: []f64 = rectangular_to_polar(real, img);
    std.debug.print("{s}\n", .{_str(r)});
}
