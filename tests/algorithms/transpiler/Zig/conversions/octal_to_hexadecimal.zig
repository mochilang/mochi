...
fn octal_to_hex(octal: []const u8) []const u8 {
    ...
    while (j < @as(i64, @intCast(s.len))) {
        const c: []const u8 = s[@as(usize, @intCast(j))..@as(usize, @intCast(j)) + 1];
    ...
    while (k < @as(i64, @intCast(s.len))) {
        const d: i64 = std.fmt.parseInt(i64, s[@as(usize, @intCast(k))..@as(usize, @intCast(k)) + 1], 10) catch 0;
        decimal = decimal * 8 + d;
        k = k + 1;
    }
...
pub fn main() void {
    {
        ...
        while (t_var < @as(i64, @intCast(nums_var.len))) {
            const num: []const u8 = nums_var[@as(usize, @intCast(t_var))];
            std.debug.print("{s}\n", .{octal_to_hex(num)});
            t_var = t_var + 1;
        }
        const __end = _now();
