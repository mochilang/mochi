...
fn bin_to_octal(bin_string: []const u8) []const u8 {
    ...
    while (i < @as(i64, @intCast(bin_string.len))) {
        const c: []const u8 = bin_string[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1];
    ...
    while (index < @as(i64, @intCast(padded.len))) {
        const group: []const u8 = padded[@as(usize, @intCast(index))..@min(@as(usize, @intCast(index + 3)), @as(usize, @intCast(padded.len)))];
        const b0: i64 = if (std.mem.eql(u8, group[@as(usize, @intCast(0))..@as(usize, @intCast(0)) + 1], "1")) 1 else 0;
        const b1: i64 = if (std.mem.eql(u8, group[@as(usize, @intCast(1))..@as(usize, @intCast(1)) + 1], "1")) 1 else 0;
        const b2: i64 = if (std.mem.eql(u8, group[@as(usize, @intCast(2))..@as(usize, @intCast(2)) + 1], "1")) 1 else 0;
        const oct_val: i64 = b0 * 4 + b1 * 2 + b2;
