...
fn convert_time(time_value: f64, unit_from: []const u8, unit_to: []const u8) f64 {
    ...
    const from: []const u8 = _lower(unit_from);
    const to: []const u8 = _lower(unit_to);
    ...
    const seconds: f64 = time_value * time_chart_var.get(from).?;
    const converted: f64 = seconds * time_chart_inverse_var.get(to).?;
    const scaled: f64 = converted * 1000.0;
    const int_part: i64 = @as(i64, @intFromFloat(scaled + 0.5));
