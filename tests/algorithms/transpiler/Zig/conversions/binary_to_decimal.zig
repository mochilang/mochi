...
fn trim(s: []const u8) []const u8 {
    ...
    while (start < @as(i64, @intCast(s.len))) {
        const ch: []const u8 = s[@as(usize, @intCast(start))..@min(@as(usize, @intCast(start + 1)), @as(usize, @intCast(s.len)))];
    ...
    while (end > start) {
        const ch: []const u8 = s[@as(usize, @intCast(end - 1))..@min(@as(usize, @intCast(end)), @as(usize, @intCast(s.len)))];
        if (!std.mem.eql(u8, ch, " ") and !std.mem.eql(u8, ch, "\n") and !std.mem.eql(u8, ch, "\t") and !std.mem.eql(u8, ch, "\r")) {
            break;
        }
        end = end - 1;
...
fn bin_to_decimal(bin_string: []const u8) i64 {
    const trimmed: []const u8 = trim(bin_string);
    if (trimmed.len == 0) {
    ...
    while (i < @as(i64, @intCast(s.len))) {
        const c: []const u8 = s[@as(usize, @intCast(i))..@min(@as(usize, @intCast(i + 1)), @as(usize, @intCast(s.len)))];
        const digit: i64 = std.fmt.parseInt(i64, c, 10) catch 0;
