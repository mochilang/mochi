...
fn hsv_to_rgb(hue: f64, saturation: f64, value: f64) []i64 {
    ...
    const chroma: f64 = value * saturation;
    const hue_section: f64 = hue / 60.0;
    const second_largest_component: f64 = chroma * (1.0 - absf(fmod(hue_section, 2.0) - 1.0));
    const match_value: f64 = value - chroma;
    var red: i64 = 0;
    var green: i64 = 0;
    var blue: i64 = 0;
...
fn rgb_to_hsv(red: i64, green: i64, blue: i64) []f64 {
    ...
    const float_red: f64 = @as(f64, @floatFromInt(red) / 255.0;
    const float_green: f64 = @as(f64, @floatFromInt(green) / 255.0;
    const float_blue: f64 = @as(f64, @floatFromInt(blue) / 255.0;
    const value: f64 = maxf(float_red, float_green, float_blue);
    const min_val: f64 = minf(float_red, float_green, float_blue);
    const chroma: f64 = value - min_val;
    const saturation: f64 = if (value == 0.0) 0.0 else chroma / value;
    var hue: f64 = 0;
    if (chroma == 0.0) {
        hue = 0.0;
    } else {
...
fn approximately_equal_hsv(hsv1: []f64, hsv2: []f64) bool {
    const check_hue: bool = absf(hsv1[@as(usize, @intCast(0))] - hsv2[@as(usize, @intCast(0))]) < 0.2;
    const check_saturation: bool = absf(hsv1[@as(usize, @intCast(1))] - hsv2[@as(usize, @intCast(1))]) < 0.002;
    const check_value: bool = absf(hsv1[@as(usize, @intCast(2))] - hsv2[@as(usize, @intCast(2))]) < 0.002;
