...
fn gradient(img_param: [][]i64) [][][]f64 {
    const h: i64 = @as(i64, @intCast(img_param.len));
    const w: i64 = @as(i64, @intCast(img_param[@as(usize, @intCast(0))].len));
...
fn harris(img_param: [][]i64, k: f64, window: i64, thresh: f64) [][]i64 {
    const h: i64 = @as(i64, @intCast(img_param.len));
    const w: i64 = @as(i64, @intCast(img_param[@as(usize, @intCast(0))].len));
    const grads: [][][]f64 = gradient(img_param);
    ...
    while (y < h) {
        var x: i64 = 0;
        while (x < w) {
            const gx: f64 = dx[@as(usize, @intCast(y))][@as(usize, @intCast(x))];
            const gy: f64 = dy[@as(usize, @intCast(y))][@as(usize, @intCast(x))];
    ...
    const offset: i64 = @divTrunc(window, 2);
    ...
    while (y < h - offset) {
        ...
        while (x < w - offset) {
            ...
            const det: f64 = wxx * wyy - wxy * wxy;
            const trace: f64 = wxx + wyy;
            const r: f64 = det - k * (trace * trace);
            if (r > thresh) {
