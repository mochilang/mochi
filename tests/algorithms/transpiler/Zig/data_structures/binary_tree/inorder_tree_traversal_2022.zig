// Generated by Mochi Zig transpiler on 2025-08-09 16:35 +0700
...
fn insert(state_param: *TreeState, value: i64) void {
    ...
    current = current;
    var nodes: []Node = state_param.nodes;
    nodes = nodes;
    while (true) {
        var node: Node = nodes[@as(usize, @intCast(current))];
        node = node;
        if (value < node.data) {
            if (node.left == 0 - 1) {
                node.left = new_node(state_param, value);
                nodes[@as(usize, @intCast(current))] = node;
                state_param.nodes = nodes;
                return;
            }
            current = node.left;
        } else {
