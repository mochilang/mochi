...
fn is_empty(q: CircularQueue) bool {
    return q.front == q.rear and q.data[_idx(q.data.len, q.front)].len == 0;
}

fn check_can_perform(q_1: CircularQueue) void {
    if (is_empty(q_1)) {
        @panic("Empty Queue");
    }
}

fn check_is_full(q_2: CircularQueue) void {
    if (q_2.next[_idx(q_2.next.len, q_2.rear)] == q_2.front) {
        @panic("Full Queue");
    }
}

fn peek(q_3: CircularQueue) []const u8 {
    check_can_perform(q_3);
    return q_3.data[_idx(q_3.data.len, q_3.front)];
}
