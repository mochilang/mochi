...
fn conn_recv(conn_param: *ConnMock, size_param: i64) i64 {
...
fn conn_send(conn_param: *ConnMock, data_param: i64) void {
    var data_var: i64 = data_param;
    data_var = data_var;
...
fn make_socket_mock(conn_param: *ConnMock) SocketMock {
    return .{ .bind_called = 0, .listen_called = 0, .accept_called = 0, .shutdown_called = 0, .close_called = 0, .conn = (conn_param).* };
}
...
fn make_file_mock(values_param: []i64) FileMock {
    var values_var: []i64 = values_param;
    values_var = values_var;
    return .{ .read_called = 0, .data = values_var };
}

fn file_read(f_param: *FileMock, size_param_1: i64) i64 {
...
fn send_file(sock_param: *SocketMock, f_param: *FileMock) void {
    socket_bind(sock_param);
    socket_listen(sock_param);
    const conn: ConnMock = socket_accept(sock_param);
    _ = conn_recv(@constCast(&conn), 1024);
    var data: i64 = file_read(f_param, 1024);
    data = data;
    while (data != 0) {
        conn_send(@constCast(&conn), data);
        data = file_read(f_param, 1024);
    }
    conn_close(@constCast(&conn));
    socket_shutdown(sock_param);
    socket_close(sock_param);
}

fn test_send_file_running_as_expected() []const u8 {
    const conn: ConnMock = make_conn_mock();
    const sock: SocketMock = make_socket_mock(@constCast(&conn));
    const f: FileMock = file_open();
    send_file(@constCast(&sock), @constCast(&f));
