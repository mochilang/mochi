...
fn round_to(x_param_1: f64, n_param_1: i64) f64 {
    var x_var: f64 = x_param_1;
    x_var = x_var;
    var n_var: i64 = n_param_1;
    n_var = n_var;
    const m: f64 = pow10(n_var);
    return std.math.floor(x_var * m + 0.5) / m;
}

fn electric_power(voltage_param: f64, current_param: f64, power_param: f64) Result {
    var voltage_var: f64 = voltage_param;
    voltage_var = voltage_var;
    var current_var: f64 = current_param;
    current_var = current_var;
    var power_var: f64 = power_param;
    power_var = power_var;
    var zeros: i64 = 0;
    zeros = zeros;
    if (voltage_var == 0.0) {
    ...
    if (zeros != 1) {
        ...
        if (power_var < 0.0) {
            ...
            if (voltage_var == 0.0) {
                return .{ .name = "voltage", .value = power_var / current_var };
            } else {
                if (current_var == 0.0) {
                    return .{ .name = "current", .value = power_var / voltage_var };
                } else {
                    if (power_var == 0.0) {
                        const p: f64 = absf(voltage_var * current_var);
...
fn str_result(r_param: *Result) []const u8 {
    return _concat_string(_concat_string(_concat_string(_concat_string("Result(name='", r_param.name), "', value="), _str(r_param.value)), ")");
}
