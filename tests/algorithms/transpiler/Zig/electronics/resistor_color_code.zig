...
fn contains(list_param: [][]const u8, value_param: []const u8) bool {
    var list_var: [][]const u8 = list_param;
    list_var = list_var;
    var value_var: []const u8 = value_param;
    value_var = value_var;
    for (list_var) |__it0| {
        const c = __it0;
        if (std.mem.eql(u8, c, value_var)) {
            return true;
        }
    }
    return false;
...
fn get_significant_digits(colors_param: [][]const u8) i64 {
    var colors_var: [][]const u8 = colors_param;
    colors_var = colors_var;
    var digit: i64 = 0;
    digit = digit;
    for (colors_var) |__it1| {
        const color = __it1;
...
fn get_multiplier(color_param: []const u8) f64 {
    var color_var: []const u8 = color_param;
    color_var = color_var;
    if (!(multiplier_color_values_var.contains(color_var))) {
        @panic(_concat_string(color_var, " is not a valid color for multiplier band"));
    }
    return multiplier_color_values_var.get(color_var).?;
}

fn get_tolerance(color_param_1: []const u8) f64 {
    var color_var: []const u8 = color_param_1;
    color_var = color_var;
    if (!(tolerance_color_values_var.contains(color_var))) {
        @panic(_concat_string(color_var, " is not a valid color for tolerance band"));
    }
    return tolerance_color_values_var.get(color_var).?;
}

fn get_temperature_coeffecient(color_param_2: []const u8) i64 {
    var color_var: []const u8 = color_param_2;
    color_var = color_var;
    if (!(temperature_coeffecient_color_values_var.contains(color_var))) {
        @panic(_concat_string(color_var, " is not a valid color for temperature coeffecient band"));
    }
    return temperature_coeffecient_color_values_var.get(color_var).?;
}

fn get_band_type_count(total_param: i64, typ_param: []const u8) i64 {
    var total_var: i64 = total_param;
    total_var = total_var;
    var typ_var: []const u8 = typ_param;
    typ_var = typ_var;
    if (total_var == 3) {
        if (std.mem.eql(u8, typ_var, "significant")) {
            return 2;
        }
        if (std.mem.eql(u8, typ_var, "multiplier")) {
            return 1;
        }
        @panic(_concat_string(typ_var, " is not valid for a 3 band resistor"));
    } else {
        if (total_var == 4) {
            if (std.mem.eql(u8, typ_var, "significant")) {
                return 2;
            ...
            if (std.mem.eql(u8, typ_var, "tolerance")) {
                return 1;
            }
            @panic(_concat_string(typ_var, " is not valid for a 4 band resistor"));
        } else {
            if (total_var == 5) {
                if (std.mem.eql(u8, typ_var, "significant")) {
                    return 3;
                ...
                @panic(_concat_string(typ_var, " is not valid for a 5 band resistor"));
            } else {
                if (total_var == 6) {
                    if (std.mem.eql(u8, typ_var, "significant")) {
                        return 3;
                    ...
                    if (std.mem.eql(u8, typ_var, "temp_coeffecient")) {
                        return 1;
                    }
                    @panic(_concat_string(typ_var, " is not valid for a 6 band resistor"));
                } else {
                    @panic(_concat_string(_str(total_var), " is not a valid number of bands"));
                }
            }
        }
    }
...
fn check_validity(number_of_bands_param: i64, colors_param_1: [][]const u8) bool {
    var number_of_bands_var: i64 = number_of_bands_param;
    number_of_bands_var = number_of_bands_var;
    var colors_var: [][]const u8 = colors_param_1;
    colors_var = colors_var;
    if (number_of_bands_var < 3 or number_of_bands_var > 6) {
        @panic("Invalid number of bands. Resistor bands must be 3 to 6");
    }
    if (number_of_bands_var != @as(i64, @intCast(colors_var.len))) {
        @panic(_concat_string(_concat_string(_concat_string(_concat_string("Expecting ", _str(number_of_bands_var)), " colors, provided "), _str(@as(i64, @intCast(colors_var.len)))), " colors"));
    }
    for (colors_var) |__it2| {
        const color = __it2;
...
fn calculate_resistance(number_of_bands_param_1: i64, color_code_list_param: [][]const u8) []const u8 {
    var number_of_bands_var: i64 = number_of_bands_param_1;
    number_of_bands_var = number_of_bands_var;
    var color_code_list_var: [][]const u8 = color_code_list_param;
    color_code_list_var = color_code_list_var;
    _ = check_validity(number_of_bands_var, color_code_list_var);
    const sig_count: i64 = get_band_type_count(number_of_bands_var, "significant");
    const significant_colors: [][]const u8 = color_code_list_var[@as(usize, @intCast(0))..@min(@as(usize, @intCast(sig_count)), @as(usize, @intCast(color_code_list_var.len)))];
    const significant_digits: i64 = get_significant_digits(significant_colors);
    const multiplier_color: []const u8 = color_code_list_var[_idx(color_code_list_var.len, sig_count)];
    const multiplier: f64 = get_multiplier(multiplier_color);
    var tolerance: f64 = 20.0;
    tolerance = tolerance;
    if (number_of_bands_var >= 4) {
        const tolerance_color: []const u8 = color_code_list_var[_idx(color_code_list_var.len, sig_count + 1)];
        tolerance = get_tolerance(tolerance_color);
    }
    var temp_coeff: i64 = 0;
    temp_coeff = temp_coeff;
    if (number_of_bands_var == 6) {
        const temp_color: []const u8 = color_code_list_var[_idx(color_code_list_var.len, sig_count + 2)];
