...
fn get_random_key() []const u8 {
    ...
    while (j > 0) {
        const k: i64 = rand(j + 1);
        const tmp: []const u8 = chars[@as(usize, @intCast(j))];
...
fn check_valid_key(key_param: []const u8) bool {
    ...
    while (i < @as(i64, @intCast(key_param.len))) {
        const ch: []const u8 = key_param[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1];
        if (used.get(ch).?) {
            return false;
        }
    ...
    i = 0;
    while (i < @as(i64, @intCast(LETTERS_var.len))) {
        const ch: []const u8 = LETTERS_var[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1];
        if (!(used.get(ch).?)) {
            return false;
        }
        i = i + 1;
...
fn translate_message(key_param: []const u8, message: []const u8, mode: []const u8) []const u8 {
    ...
    while (i < @as(i64, @intCast(message.len))) {
        const symbol: []const u8 = message[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1];
        const upper_symbol: []const u8 = char_to_upper(symbol);
        const idx: i64 = index_in(chars_a, upper_symbol);
        if (idx >= 0) {
            const mapped: []const u8 = chars_b[@as(usize, @intCast(idx))..@as(usize, @intCast(idx)) + 1];
            if (is_upper(symbol)) {
...
fn encrypt_message(key_param: []const u8, message: []const u8) []const u8 {
    const res: []const u8 = translate_message(key_param, message, "encrypt");
    return res;
}

fn decrypt_message(key_param: []const u8, message: []const u8) []const u8 {
    const res: []const u8 = translate_message(key_param, message, "decrypt");
    return res;
}
