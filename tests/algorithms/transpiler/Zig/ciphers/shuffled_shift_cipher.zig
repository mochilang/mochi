// Generated by Mochi Zig transpiler on 2025-08-06 23:00 +0700
...
fn unique_sorted(chars: [][]const u8) [][]const u8 {
    ...
    while (i < @as(i64, @intCast(chars.len))) {
        const ch: []const u8 = chars[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1];
        if (!(std.mem.indexOfScalar(i64, uniq, ch) != null)) {
    ...
    while (j < @as(i64, @intCast(uniq.len))) {
        ...
        while (k < @as(i64, @intCast(uniq.len))) {
            if (std.mem.order(u8, uniq[@as(usize, @intCast(k))..@as(usize, @intCast(k)) + 1], uniq[@as(usize, @intCast(min_idx))..@as(usize, @intCast(min_idx)) + 1]) == .lt) {
                min_idx = k;
            }
            k = k + 1;
        }
        if (min_idx != j) {
            const tmp: []const u8 = uniq[@as(usize, @intCast(j))..@as(usize, @intCast(j)) + 1];
            uniq[@as(usize, @intCast(j))] = uniq[@as(usize, @intCast(min_idx))..@as(usize, @intCast(min_idx)) + 1];
...
fn make_key_list(passcode: [][]const u8) [][]const u8 {
    ...
    while (i < @as(i64, @intCast(key_list_options.len))) {
        ...
        if (std.mem.indexOfScalar(i64, breakpoints, ch) != null or i == @as(i64, @intCast(key_list_options.len)) - 1) {
            var k: i64 = @as(i64, @intCast(temp_list.len)) - 1;
            while (k >= 0) {
                keys_l = blk: { var _tmp = std.ArrayList([]const u8).init(std.heap.page_allocator); _tmp.appendSlice(@as([]const []const u8, keys_l)) catch |err| handleError(err); _tmp.append(temp_list[@as(usize, @intCast(k))..@as(usize, @intCast(k)) + 1]) catch |err| handleError(err); break :blk (_tmp.toOwnedSlice() catch |err| handleError(err)); };
                k = k - 1;
            }
            temp_list = &[_]i64{};
        }
...
fn make_shift_key(passcode: [][]const u8) i64 {
    ...
    while (i < @as(i64, @intCast(passcode.len))) {
        codes = blk: { var _tmp = std.ArrayList(i64).init(std.heap.page_allocator); _tmp.appendSlice(@as([]const i64, codes)) catch |err| handleError(err); _tmp.append(ord(passcode[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1])) catch |err| handleError(err); break :blk (_tmp.toOwnedSlice() catch |err| handleError(err)); };
        i = i + 1;
    }
    codes = neg_pos(codes);
    var total: i64 = 0;
...
fn index_of(lst: [][]const u8, ch: []const u8) i64 {
    ...
    while (i < @as(i64, @intCast(lst.len))) {
        if (std.mem.eql(u8, lst[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1], ch)) {
            return i;
        }
        i = i + 1;
    }
...
fn encrypt(c: Cipher, plaintext: []const u8) []const u8 {
    ...
    while (i < @as(i64, @intCast(plaintext.len))) {
        ...
        const new_pos: i64 = @mod(position + c.shift_key, n);
        encoded_var_1 = _concat_string(encoded_var_1, c.key_list[@as(usize, @intCast(new_pos))..@as(usize, @intCast(new_pos)) + 1]);
        i = i + 1;
    }
...
fn decrypt(c: Cipher, encoded_message: []const u8) []const u8 {
    ...
    while (i < @as(i64, @intCast(encoded_message.len))) {
        ...
        decoded = _concat_string(decoded, c.key_list[@as(usize, @intCast(new_pos))..@as(usize, @intCast(new_pos)) + 1]);
        i = i + 1;
    }
    return decoded;
}
