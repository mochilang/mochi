...
fn to_upper(s: []const u8) []const u8 {
    ...
    while (i < @as(i64, @intCast(s.len))) {
        ...
        const idx: i64 = index_in_string(lower, ch);
        if (idx >= 0) {
...
fn remove_duplicates(key: []const u8) []const u8 {
    ...
    while (i < @as(i64, @intCast(key.len))) {
        const ch: []const u8 = key[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1];
...
fn create_cipher_map(key: []const u8) [][]const u8 {
    ...
    const cleaned: []const u8 = remove_duplicates(to_upper(key));
...
fn encipher(message: []const u8, cipher: [][]const u8) []const u8 {
    const alphabet: []const u8 = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    const msg: []const u8 = to_upper(message);
    var res: []const u8 = "";
    var i: i64 = 0;
    while (i < @as(i64, @intCast(msg.len))) {
        const ch: []const u8 = msg[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1];
        const idx: i64 = index_in_string(alphabet, ch);
        if (idx >= 0) {
...
fn decipher(message: []const u8, cipher: [][]const u8) []const u8 {
    ...
    while (i < @as(i64, @intCast(msg.len))) {
        const ch: []const u8 = msg[@as(usize, @intCast(i))..@as(usize, @intCast(i)) + 1];
        const idx: i64 = index_in_list(cipher, ch);
        if (idx >= 0) {
