    ...
    while (p_i < @as(i64, @intCast(plaintext_var.len))) {
        ...
        if (p_idx < 0) {
            ...
            if (k_idx < 0) {
                ...
                var c_idx: i64 = @mod(p_idx + k_idx, 26);
                c_idx = c_idx;
                ciphertext = _concat_string(ciphertext, index_char(c_idx));
                k_i = k_i + 1;
                p_i = p_i + 1;
            }
        }
...
fn decrypt(ciphertext: []const u8, key: []const u8) []const u8 {
    ...
    plaintext = plaintext;
    while (c_i < @as(i64, @intCast(ciphertext.len))) {
        var c_char: []const u8 = ciphertext[@as(usize, @intCast(c_i))..@as(usize, @intCast(c_i)) + 1];
        c_char = c_char;
        var c_idx: i64 = char_index(c_char);
        c_idx = c_idx;
        if (c_idx < 0) {
            plaintext = _concat_string(plaintext, c_char);
        } else {
            var k_char: []const u8 = current_key[@as(usize, @intCast(k_i))..@as(usize, @intCast(k_i)) + 1];
            k_char = k_char;
            var k_idx: i64 = char_index(k_char);
            k_idx = k_idx;
            var p_idx: i64 = @mod(c_idx - k_idx + 26, 26);
            p_idx = p_idx;
