...
fn viterbi(observations_param: [][]const u8, states_param: [][]const u8, start_p_param: *std.StringHashMap(f64), trans_p_param: *std.StringHashMap(std.StringHashMap(f64)), emit_p_param: *std.StringHashMap(std.StringHashMap(f64))) [][]const u8 {
    ...
    while (t < @as(i64, @intCast(observations_var_1.len))) {
        ...
        while (j < @as(i64, @intCast(states_var_1.len))) {
            ...
            while (k < @as(i64, @intCast(states_var_1.len))) {
                ...
                const prob: f64 = prob_prev * @as(f64, @floatFromInt(trans_p_param.get(state0).?.get(state).?)) * @as(f64, @floatFromInt(emit_p_param.get(state).?.get(obs).?));
                if (prob > max_prob) {
                    max_prob = prob;
                    prev_state = state0;
                }
