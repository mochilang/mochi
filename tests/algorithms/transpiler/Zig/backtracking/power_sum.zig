...
fn backtrack(target: i64, exp: i64, current: i64, current_sum: i64) i64 {
    if (current_sum == target) {
        return 1;
    }
    const p: i64 = int_pow(current, exp);
    var count: i64 = 0;
    count = count;
...
fn solve(target: i64, exp: i64) i64 {
    if (!(1 <= target and target <= 1000 and 2 <= exp and exp <= 10)) {
        std.debug.print("{s}\n", .{"Invalid input"});
        return 0;
    }
