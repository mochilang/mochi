...
fn create_state_space_tree(sequence_param: []Value, current: []Value, index: i64, used: []bool) void {
    ...
    while (i < @as(i64, @intCast(sequence_param.len))) {
        if (!(used[@as(usize, @intCast(i))])) {
            var next_current: []Value = blk: { var _tmp = std.ArrayList(Value).init(std.heap.page_allocator); _tmp.appendSlice(@as([]const Value, current)) catch |err| handleError(err); _tmp.append(sequence_param[@as(usize, @intCast(i))]) catch |err| handleError(err); break :blk (_tmp.toOwnedSlice() catch |err| handleError(err)); };
