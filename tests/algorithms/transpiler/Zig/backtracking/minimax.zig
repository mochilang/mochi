...
fn minimax(depth: i64, node_index: i64, is_max: bool, scores: []i64, height: i64) i64 {
    if (depth < 0) {
        @panic(@constCast("Depth cannot be less than 0"));
    }
    if (@as(i64, @intCast(scores.len)) == 0) {
        @panic(@constCast("Scores cannot be empty"));
    }
    if (depth == height) {
...
fn mochi_main() void {
    var scores: []i64 = @constCast(&([8]i64{90, 23, 6, 33, 21, 65, 123, 34423}))[0..];
    scores = scores;
    var height: i64 = tree_height(@as(i64, @intCast(scores.len)));
    height = height;
    std.debug.print("{s}\n", .{_concat_string("Optimal value : ", _str(minimax(0, 0, true, scores, height)))});
}
