// Generated by Mochi transpiler v0.10.63 on 2025-08-11 15:10:35 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
double euclidean_distance_sqr(const std::vector<double>& p1, const std::vector<double>& p2) {
    double dx = (_index(p1, int64_t(0)) - _index(p2, int64_t(0)));
    double dy = (_index(p1, int64_t(1)) - _index(p2, int64_t(1)));
    return ((dx * dx) + (dy * dy));
}
...
std::vector<std::vector<double>> column_based_sort(const std::vector<std::vector<double>>& arr, int64_t column) {
    ...
    while ((i < points.size())) {
        ...
        while ((j < (points.size() - int64_t(1)))) {
            if ((_index(_index(points, j), column) > _index(_index(points, (j + int64_t(1))), column))) {
                std::vector<double> tmp = _index(points, j);
                points[j] = _index(points, (j + int64_t(1)));
...
double dis_between_closest_pair(const std::vector<std::vector<double>>& points, int64_t count, double min_dis) {
    ...
    while ((i < (count - int64_t(1)))) {
        ...
        while ((j < count)) {
            double current = euclidean_distance_sqr(_index(points, i), _index(points, j));
            if ((current < min_dis)) {
                min_dis = current;
            }
            j = (j + int64_t(1));
...
double closest_pair_of_points_sqr(const std::vector<std::vector<double>>& px, const std::vector<std::vector<double>>& py, int64_t count) {
    ...
    int64_t i = int64_t(0);
    while ((i < px.size())) {
        if ((abs((_index(_index(px, i), int64_t(0)) - _index(_index(px, mid), int64_t(0)))) < best)) {
            strip = ([&]{ auto __tmp = strip; __tmp.push_back(_index(px, i)); return __tmp; }());
        }
        i = (i + int64_t(1));
    }
    double strip_best = dis_between_closest_in_strip(strip, strip.size(), best);
    if ((strip_best < best)) {
        best = strip_best;
    }
    return best;
...
int main() {
    {
        ...
        std::cout << (std::string("Distance: ") + ([&]{ std::ostringstream ss; ss << std::boolalpha << closest_pair_of_points(points, points.size()); return ss.str(); }()));
        std::cout << std::endl;
    }
    return 0;
}
