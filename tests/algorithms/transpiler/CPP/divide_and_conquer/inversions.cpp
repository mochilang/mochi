// Generated by Mochi transpiler v0.10.59 on 2025-08-07 09:06:43 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<int64_t> slice_list(const std::vector<int64_t>& arr, int64_t start, int64_t end) {
    ...
    int64_t k = start;
    while ((k < end)) {
        res = ([&]{ auto __tmp = res; __tmp.push_back(arr[k]); return __tmp; }());
        k = (k + int64_t(1));
    }
    return res;
}
...
InvResult count_cross_inversions(const std::vector<int64_t>& p, const std::vector<int64_t>& q) {
    ...
    while (((i < p.size()) && (j < q.size()))) {
        if ((p[i] > q[j])) {
            inv = (inv + (p.size() - i));
            r = ([&]{ auto __tmp = r; __tmp.push_back(q[j]); return __tmp; }());
            j = (j + int64_t(1));
        } else {
            r = ([&]{ auto __tmp = r; __tmp.push_back(p[i]); return __tmp; }());
            i = (i + int64_t(1));
        }
    }
    if ((i < p.size())) {
