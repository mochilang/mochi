// Generated by Mochi transpiler v0.10.63 on 2025-08-11 15:11:02 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
InvResult count_cross_inversions(const std::vector<int64_t>& p, const std::vector<int64_t>& q) {
    ...
    while (((i < p.size()) && (j < q.size()))) {
        if ((_index(p, i) > _index(q, j))) {
            inv = (inv + (p.size() - i));
            r = ([&]{ auto __tmp = r; __tmp.push_back(_index(q, j)); return __tmp; }());
            j = (j + int64_t(1));
        } else {
            r = ([&]{ auto __tmp = r; __tmp.push_back(_index(p, i)); return __tmp; }());
            i = (i + int64_t(1));
        }
    }
    if ((i < p.size())) {
        r = _concat(r, slice_list(p, i, p.size()));
    } else {
        r = _concat(r, slice_list(q, j, q.size()));
    }
...
InvResult count_inversions_recursive(const std::vector<int64_t>& arr) {
    ...
    std::vector<int64_t> q = slice_list(arr, mid, arr.size());
