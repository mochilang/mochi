// Generated by Mochi transpiler v0.10.60 on 2025-08-08 11:18:52 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<Pos> clone_path(const std::vector<Pos>& p) {
    ...
    int64_t i = int64_t(0);
    while ((i < p.size())) {
        res = ([&]{ auto& __tmp = res; __tmp.push_back(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(p)); return __tmp; }());
        i = (i + int64_t(1));
    }
    return res;
}
...
bool contains(const std::vector<Node>& nodes, const Node& node) {
    int64_t i = int64_t(0);
    while ((i < nodes.size())) {
        if (node_equal(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(nodes), node)) {
            return true;
        }
        i = (i + int64_t(1));
    }
...
std::vector<Node> sort_nodes(const std::vector<Node>& nodes) {
    ...
    int64_t i = int64_t(1);
    while ((i < arr.size())) {
        Node key_node = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(arr);
        int64_t j = (i - int64_t(1));
        while ((j >= int64_t(0))) {
            Node temp = ([&](const auto& __v){ auto __i = j; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(arr);
...
std::vector<Node> get_successors(const std::vector<std::vector<int64_t>>& grid, const Node& parent, const Pos& target) {
    ...
    int64_t i = int64_t(0);
    while ((i < delta.size())) {
        Pos d = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(delta);
        int64_t pos_x = (parent.pos_x + d.x);
        int64_t pos_y = (parent.pos_y + d.y);
        if ((((((pos_x >= int64_t(0)) && (pos_x < ([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(grid).size())) && (pos_y >= int64_t(0))) && (pos_y < grid.size())) && (([&](const auto& __v){ auto __i = pos_x; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(([&](const auto& __v){ auto __i = pos_y; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(grid)) == int64_t(0)))) {
...
std::vector<Pos> greedy_best_first(const std::vector<std::vector<int64_t>>& grid, const Pos& init, const Pos& goal) {
    ...
    while ((open_nodes.size() > int64_t(0))) {
        ...
        Node current = ([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(open_nodes);
        std::vector<Node> new_open = {};
        int64_t idx = int64_t(1);
        while ((idx < open_nodes.size())) {
            new_open = ([&]{ auto& __tmp = new_open; __tmp.push_back(([&](const auto& __v){ auto __i = idx; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(open_nodes)); return __tmp; }());
            idx = (idx + int64_t(1));
        }
        open_nodes = new_open;
        ...
        while ((i < successors.size())) {
            Node child = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(successors);
...
void print_grid(const std::vector<std::vector<int64_t>>& grid) {
    int64_t i = int64_t(0);
    while ((i < grid.size())) {
        std::cout << ([&]{ std::ostringstream ss; { auto __tmp = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(grid); ss << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) ss << ' '; ss << __tmp[i]; } ss << "]"; }
 return ss.str(); }());
        std::cout << std::endl;
        i = (i + int64_t(1));
    }
...
int main() {
    {
        ...
        while ((idx < TEST_GRIDS.size())) {
            ...
            std::cout << std::endl;
            std::vector<std::vector<int64_t>> grid = ([&](const auto& __v){ auto __i = idx; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(TEST_GRIDS);
