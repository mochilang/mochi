// Generated by Mochi transpiler v0.10.60 on 2025-08-08 11:17:25 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <map>
...
ExpandResult expand_search(const std::map<int64_t, std::vector<int64_t>>& graph, const std::vector<int64_t>& queue, int64_t head, const std::map<int64_t, int64_t>& parents, const std::map<int64_t, bool>& visited, const std::map<int64_t, bool>& opposite_visited) {
    ...
    int64_t current = ([&](const auto& __v){ auto __i = head; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue);
    head = (head + int64_t(1));
...
std::vector<int64_t> reverse_list(const std::vector<int64_t>& xs) {
    ...
    while ((i > int64_t(0))) {
        i = (i - int64_t(1));
        res = ([&]{ auto& __tmp = res; __tmp.push_back(xs[static_cast<size_t>(i)]); return __tmp; }());
    }
    return res;
}
...
SearchResult bidirectional_search(const std::map<int64_t, std::vector<int64_t>>& g, int64_t start, int64_t goal) {
    ...
    forward_parents[start] = (int64_t(0) - int64_t(1));
    std::map<int64_t, int64_t> backward_parents = {};
    backward_parents[goal] = (int64_t(0) - int64_t(1));
    ...
    while ((j < backward_path.size())) {
        result = ([&]{ auto& __tmp = result; __tmp.push_back(([&](const auto& __v){ auto __i = j; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(backward_path)); return __tmp; }());
        j = (j + int64_t(1));
    }
...
bool is_edge(const std::map<int64_t, std::vector<int64_t>>& g, int64_t u, int64_t v) {
    ...
    int64_t i = int64_t(0);
    while ((i < neighbors.size())) {
        if ((([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(neighbors) == v)) {
            return true;
        }
        i = (i + int64_t(1));
    }
...
bool path_exists(const std::map<int64_t, std::vector<int64_t>>& g, const std::vector<int64_t>& path) {
    ...
    while (((i + int64_t(1)) < path.size())) {
        if (!is_edge(g, ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(path), ([&](const auto& __v){ auto __i = (i + int64_t(1)); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(path))) {
            return false;
        }
        i = (i + int64_t(1));
    }
...
void print_path(const std::map<int64_t, std::vector<int64_t>>& g, int64_t s, int64_t t) {
    ...
    if ((res.ok && path_exists(g, res.path))) {
        std::cout << (((((std::string("Path from ") + ([&]{ std::ostringstream ss; ss << std::boolalpha << s; return ss.str(); }())) + std::string(" to ")) + ([&]{ std::ostringstream ss; ss << std::boolalpha << t; return ss.str(); }())) + std::string(": ")) + ([&]{ std::ostringstream ss; { auto __tmp = res.path; ss << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) ss << ' '; ss << __tmp[i]; } ss << "]"; }
 return ss.str(); }()));
        std::cout << std::endl;
    } else {
