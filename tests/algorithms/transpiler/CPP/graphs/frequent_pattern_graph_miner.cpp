// Generated by Mochi transpiler v0.10.60 on 2025-08-08 11:18:30 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <map>
...
std::vector<std::string> get_distinct_edge(const std::vector<std::vector<std::vector<std::string>>>& edge_array) {
    ...
    for (std::vector<std::vector<std::string>> row : edge_array) {
        for (std::vector<std::string> item : row) {
            std::string e = ([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(item);
...
std::string get_bitcode(const std::vector<std::vector<std::vector<std::string>>>& edge_array, std::string de) {
    ...
    while ((i < edge_array.size())) {
        bool found = false;
        for (std::vector<std::string> item : ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(edge_array)) {
            if ((([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(item) == de)) {
                found = true;
                break;
            }
        }
...
int64_t count_ones(std::string s) {
    ...
    int64_t i = int64_t(0);
    while ((i < s.size())) {
        if ((s.substr(i, (i + int64_t(1)) - i) == std::string("1"))) {
            c = (c + int64_t(1));
        }
        i = (i + int64_t(1));
    }
...
std::vector<std::map<std::string, std::string>> get_frequency_table(const std::vector<std::vector<std::vector<std::string>>>& edge_array) {
    ...
    while ((i < table.size())) {
        ...
        while ((j < table.size())) {
            if (((double)(_parse_int_str(([&](const auto& __m){ auto __it = __m.find(std::string("count")); if (__it == __m.end()) return std::string{}; return __it->second; })(([&](const auto& __v){ auto __i = j; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(table)), int64_t(10))) > (double)(_parse_int_str(([&](const auto& __m){ auto __it = __m.find(std::string("count")); if (__it == __m.end()) return std::string{}; return __it->second; })(([&](const auto& __v){ auto __i = max_i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(table)), int64_t(10))))) {
                max_i = j;
            }
            j = (j + int64_t(1));
        }
        std::map<std::string, std::string> tmp = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(table);
        table[i] = ([&](const auto& __v){ auto __i = max_i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(table);
        table[max_i] = tmp;
        i = (i + int64_t(1));
    }
    return table;
...
ClusterData get_cluster(const NodesData& nodes) {
    ...
    while ((i < nodes.keys.size())) {
        std::string code = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(nodes.keys);
        ...
        if (([&](const auto& __c, const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__c)>, std::string>) { return __c.find(__v) != std::string::npos; } else if constexpr(requires { __c.find(__v); }) { return __c.find(__v) != __c.end(); } else { return std::find(__c.begin(), __c.end(), __v) != __c.end(); } })(clusters, wt)) {
            clusters[wt] = ([&]{ auto __tmp = ([&](const auto& __m){ auto __it = __m.find(wt); if (__it == __m.end()) return std::vector<std::string>{}; return __it->second; })(clusters); __tmp.push_back(code); return __tmp; }());
        } else {
            clusters[wt] = std::vector<std::string>{code};
...
std::vector<int64_t> get_support(const ClusterData& clusters) {
    ...
    while ((i < clusters.weights.size())) {
        int64_t w = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(clusters.weights);
        sup = ([&]{ auto& __tmp = sup; __tmp.push_back((int64_t)((w * int64_t(100)) / clusters.weights.size())); return __tmp; }());
        i = (i + int64_t(1));
    }
    return sup;
}
...
bool contains_bits(std::string a, std::string b) {
    int64_t i = int64_t(0);
    while ((i < a.size())) {
        std::string c1 = a.substr(i, (i + int64_t(1)) - i);
        std::string c2 = b.substr(i, (i + int64_t(1)) - i);
...
std::vector<std::string> create_edge(const NodesData& nodes, std::map<std::string, std::vector<std::string>> graph, const std::vector<std::string>& gkeys, const ClusterData& clusters, int64_t c1, int64_t maxk) {
    ...
    while ((idx1 < codes1.size())) {
        std::string i_code = ([&](const auto& __v){ auto __i = idx1; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(codes1);
        ...
        while ((c2 <= maxk)) {
            ...
            int64_t j = int64_t(0);
            while ((j < codes2.size())) {
                std::string j_code = ([&](const auto& __v){ auto __i = j; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(codes2);
...
GraphData construct_graph(const ClusterData& clusters, const NodesData& nodes) {
    ...
    while ((i < top_codes.size())) {
        std::string code = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(top_codes);
...
void find_freq_subgraph_given_support(int64_t s, const ClusterData& clusters, const GraphData& graph) {
    ...
    int64_t i = int64_t(0);
    while ((i < codes.size())) {
        my_dfs(graph.edges, ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(codes), std::string("Header"), std::vector<std::string>{});
        i = (i + int64_t(1));
    }
}
...
std::vector<std::vector<std::vector<std::string>>> freq_subgraphs_edge_list(const std::vector<std::vector<std::string>>& paths, const NodesData& nodes) {
    ...
    for (std::vector<std::string> path : paths) {
        ...
        while ((j < (path.size() - int64_t(1)))) {
            std::string code = ([&](const auto& __v){ auto __i = j; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(path);
            std::vector<std::string> edge_list = node_edges(nodes, code);
            int64_t e = int64_t(0);
            while ((e < edge_list.size())) {
                std::string edge = ([&](const auto& __v){ auto __i = e; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(edge_list);
                std::string a = edge.substr(int64_t(0), int64_t(1) - int64_t(0));
                std::string b = edge.substr(int64_t(1), int64_t(2) - int64_t(1));
...
void print_all(const NodesData& nodes, const std::vector<int64_t>& support, const ClusterData& clusters, const GraphData& graph, const std::vector<std::vector<std::vector<std::string>>>& freq_subgraph_edge_list) {
    ...
    while ((i < nodes.keys.size())) {
        ...
        std::cout << code;
        std::cout << std::endl;
        { auto __tmp = ([&](const auto& __m){ auto __it = __m.find(code); if (__it == __m.end()) return std::vector<std::string>{}; return __it->second; })(nodes.map); std::cout << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) std::cout << ' '; std::cout << __tmp[i]; } std::cout << "]"; }
        std::cout << std::endl;
        i = (i + int64_t(1));
    }
    std::cout << std::string("\nSupport\n");
    std::cout << std::endl;
    { auto __tmp = support; std::cout << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) std::cout << ' '; std::cout << __tmp[i]; } std::cout << "]"; }
    std::cout << std::endl;
    std::cout << std::string("\nCluster\n");
    std::cout << std::endl;
    int64_t j = int64_t(0);
    while ((j < clusters.weights.size())) {
        int64_t w = ([&](const auto& __v){ auto __i = j; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(clusters.weights);
        std::cout << ((([&]{ std::ostringstream ss; ss << std::boolalpha << w; return ss.str(); }()) + std::string(":")) + ([&]{ std::ostringstream ss; { auto __tmp = ([&](const auto& __m){ auto __it = __m.find(w); if (__it == __m.end()) return std::vector<std::string>{}; return __it->second; })(clusters.clusters); ss << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) ss << ' '; ss << __tmp[i]; } ss << "]"; }
 return ss.str(); }()));
        std::cout << std::endl;
        j = (j + int64_t(1));
    }
    ...
    while ((k < graph.keys.size())) {
        std::string key = ([&](const auto& __v){ auto __i = k; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(graph.keys);
        std::cout << key;
        std::cout << std::endl;
        { auto __tmp = ([&](const auto& __m){ auto __it = __m.find(key); if (__it == __m.end()) return std::vector<std::string>{}; return __it->second; })(graph.edges); std::cout << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) std::cout << ' '; std::cout << __tmp[i]; } std::cout << "]"; }
        std::cout << std::endl;
        k = (k + int64_t(1));
    }
    ...
    for (std::vector<std::vector<std::string>> el : freq_subgraph_edge_list) {
        { auto __tmp = el; std::cout << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) std::cout << ' '; { std::ostringstream __ss;         __ss << ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(__tmp);
