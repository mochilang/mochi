// Generated by Mochi transpiler v0.10.60 on 2025-08-08 11:18:40 GMT+7
#include <iostream>
#include <string>
#include <map>
#include <vector>
...
Graph create_graph(const std::vector<std::string>& vertices, const std::vector<std::vector<std::string>>& edges, bool directed) {
    ...
    for (std::vector<std::string> e : edges) {
        std::string s = ([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(e);
        std::string d = ([&](const auto& __v){ auto __i = int64_t(1); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(e);
...
std::vector<std::string> remove_from_list(const std::vector<std::string>& lst, std::string value) {
    ...
    int64_t i = int64_t(0);
    while ((i < lst.size())) {
        if ((([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(lst) != value)) {
...
std::string to_string(const Graph& graph) {
    return ([&]{ std::ostringstream ss; { auto __tmp = graph.adj; ss << "{"; bool first=true; for(const auto& __p : __tmp){ if(!first) ss << ", "; first=false; ss << __p.first << ': '; any_to_stream(ss, __p.second); } ss << "}"; }
 return ss.str(); }());
}
