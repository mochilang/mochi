// Generated by Mochi transpiler v0.10.60 on 2025-08-08 11:19:35 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <vector>
...
Pos goal = Pos{.x = (n - int64_t(1)), .y = (n - int64_t(1))};
...
bool in_blocks(const Pos& p) {
    int64_t i = int64_t(0);
    while ((i < blocks.size())) {
        if (pos_equal(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(blocks), p)) {
            return true;
        }
        i = (i + int64_t(1));
    }
...
std::vector<PQNode> pq_put(std::vector<PQNode> pq, const Pos& node, double pri) {
    ...
    int64_t i = int64_t(0);
    while ((i < pq.size())) {
        if (pos_equal(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(pq).pos, node)) {
            if ((pri < ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(pq).pri)) {
...
double pq_minkey(const std::vector<PQNode>& pq) {
    ...
    PQNode first = ([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(pq);
    double m = first.pri;
    int64_t i = int64_t(1);
    while ((i < pq.size())) {
        PQNode item = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(pq);
        if ((item.pri < m)) {
            m = item.pri;
        }
        i = (i + int64_t(1));
...
PQPopResult pq_pop_min(const std::vector<PQNode>& pq) {
    PQNode best = ([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(pq);
    int64_t idx = int64_t(0);
    int64_t i = int64_t(1);
    while ((i < pq.size())) {
        if ((([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(pq).pri < best.pri)) {
            best = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(pq);
            idx = i;
        }
        i = (i + int64_t(1));
    }
    ...
    while ((i < pq.size())) {
        if ((i != idx)) {
            new_pq = ([&]{ auto& __tmp = new_pq; __tmp.push_back(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(pq)); return __tmp; }());
        }
        i = (i + int64_t(1));
    }
...
std::vector<Pos> reconstruct(const std::map<std::string, Pos>& back_pointer, const Pos& goal, const Pos& start) {
    ...
    while ((i >= int64_t(0))) {
        rev = ([&]{ auto& __tmp = rev; __tmp.push_back(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(path)); return __tmp; }());
        i = (i - int64_t(1));
    }
    return rev;
}
...
void multi_a_star(const Pos& start, const Pos& goal, int64_t n_heuristic) {
    ...
    while ((i < n_heuristic)) {
        ...
        open_list[i] = pq_put(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(open_list), start, pri);
        i = (i + int64_t(1));
    }
    while ((pq_minkey(([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(open_list)) < INF)) {
        int64_t chosen = int64_t(0);
        i = int64_t(1);
        while ((i < n_heuristic)) {
            if ((pq_minkey(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(open_list)) <= (W2 * pq_minkey(([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(open_list))))) {
                chosen = i;
                break;
            }
            i = (i + int64_t(1));
        ...
        PQPopResult pair = pq_pop_min(([&](const auto& __v){ auto __i = chosen; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(open_list));
        open_list[chosen] = pair.pq;
        PQNode current = pair.node;
        i = int64_t(0);
        while ((i < n_heuristic)) {
            if ((i != chosen)) {
                open_list[i] = pq_remove(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(open_list), current.pos);
            }
            i = (i + int64_t(1));
        }
        ...
        if (pos_equal(current.pos, goal)) {
            ...
            int64_t j = int64_t(0);
            while ((j < path.size())) {
                Pos p = ([&](const auto& __v){ auto __i = j; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(path);
        ...
        int64_t k = int64_t(0);
        while ((k < neighs.size())) {
            Pos nb = ([&](const auto& __v){ auto __i = k; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(neighs);
            if ((valid(nb) && (in_blocks(nb) == false))) {
                ...
                if ((!([&](const auto& __c, const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__c)>, std::string>) { return __c.find(__v) != std::string::npos; } else if constexpr(requires { __c.find(__v); }) { return __c.find(__v) != __c.end(); } else { return std::find(__c.begin(), __c.end(), __v) != __c.end(); } })(g_function, nkey) || (tentative < ([&](const auto& __m){ auto __it = __m.find(nkey); if (__it == __m.end()) return double{}; return __it->second; })(g_function)))) {
                    ...
                    while ((i < n_heuristic)) {
                        ...
                        open_list[i] = pq_put(([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(open_list), nb, pri2);
                        i = (i + int64_t(1));
                    }
                }
            }
