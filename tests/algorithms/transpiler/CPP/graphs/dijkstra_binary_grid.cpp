// Generated by Mochi transpiler v0.10.60 on 2025-08-08 11:18:13 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <vector>
...
std::string path_to_string(const std::vector<Point>& path) {
    ...
    int64_t i = int64_t(0);
    while ((i < path.size())) {
        Point pt = ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(path);
...
Result dijkstra(const std::vector<std::vector<int64_t>>& grid, const Point& source, const Point& destination, bool allow_diagonal) {
    int rows = grid.size();
    int cols = ([&](const auto& __v){ auto __i = int64_t(0); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(grid).size();
    ...
    while ((front < queue.size())) {
        Point current = ([&](const auto& __v){ auto __i = front; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue);
        ...
        int64_t i = int64_t(0);
        while ((i < dx.size())) {
            int64_t nx = (current.x + ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(dx));
            int64_t ny = (current.y + ([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(dy));
            if (((((nx >= int64_t(0)) && (nx < rows)) && (ny >= int64_t(0))) && (ny < cols))) {
                if ((([&](const auto& __v){ auto __i = ny; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(([&](const auto& __v){ auto __i = nx; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(grid)) == int64_t(1))) {
    ...
    if (([&](const auto& __c, const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__c)>, std::string>) { return __c.find(__v) != std::string::npos; } else if constexpr(requires { __c.find(__v); }) { return __c.find(__v) != __c.end(); } else { return std::find(__c.begin(), __c.end(), __v) != __c.end(); } })(dist_map, dest_key)) {
        ...
        while ((k >= int64_t(0))) {
            path = ([&]{ auto& __tmp = path; __tmp.push_back(([&](const auto& __v){ auto __i = k; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(path_rev)); return __tmp; }());
            k = (k - int64_t(1));
        }
...
void print_result(const Result& res) {
    std::cout << ((([&]{ std::ostringstream ss; ss << std::boolalpha << res.distance; return ss.str(); }()) + std::string(", ")) + path_to_string(res.path));
    std::cout << std::endl;
}
