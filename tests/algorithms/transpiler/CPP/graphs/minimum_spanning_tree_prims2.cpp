// Generated by Mochi transpiler v0.10.60 on 2025-08-08 11:19:31 GMT+7
#include <iostream>
#include <string>
#include <map>
#include <vector>
...
std::map<std::string, std::map<std::string, int64_t>> graph = std::map<std::string, std::map<std::string, int64_t>>{};
MSTResult res = prims_algo(graph);
std::map<std::string, int64_t> dist = res.dist;
...
MSTResult prims_algo(const std::map<std::string, std::map<std::string, int64_t>>& graph) {
    ...
    int64_t i = int64_t(1);
    while ((i < queue.size())) {
        if ((([&](const auto& __v){ auto __i = i; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue).weight < ([&](const auto& __v){ auto __i = min_idx; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue).weight)) {
            min_idx = i;
        }
        i = (i + int64_t(1));
    }
    QueueNode start_node = ([&](const auto& __v){ auto __i = min_idx; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue);
    std::string start = start_node.node;
    std::vector<QueueNode> new_q = {};
    int64_t j = int64_t(0);
    while ((j < queue.size())) {
        if ((j != min_idx)) {
            new_q = ([&]{ auto& __tmp = new_q; __tmp.push_back(([&](const auto& __v){ auto __i = j; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue)); return __tmp; }());
        }
        j = (j + int64_t(1));
    }
    queue = new_q;
    ...
    for (const auto& __p : ([&](const auto& __m){ auto __it = __m.find(start); if (__it == __m.end()) return std::map<std::string, int64_t>{}; return __it->second; })(graph)) {
        ...
        if ((([&](const auto& __m){ auto __it = __m.find(neighbour); if (__it == __m.end()) return int64_t{}; return __it->second; })(dist) > (([&](const auto& __m){ auto __it = __m.find(start); if (__it == __m.end()) return int64_t{}; return __it->second; })(dist) + w))) {
            ...
            int64_t k = int64_t(0);
            while ((k < queue.size())) {
                if ((([&](const auto& __v){ auto __i = k; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue).node == neighbour)) {
                    { auto& __tmp = queue[k]; __tmp.weight = ([&](const auto& __m){ auto __it = __m.find(neighbour); if (__it == __m.end()) return int64_t{}; return __it->second; })(dist); }
                    break;
                }
                k = (k + int64_t(1));
            }
    ...
    while ((queue.size() > int64_t(0))) {
        ...
        int64_t p = int64_t(1);
        while ((p < queue.size())) {
            if ((([&](const auto& __v){ auto __i = p; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue).weight < ([&](const auto& __v){ auto __i = best_idx; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue).weight)) {
                best_idx = p;
            }
            p = (p + int64_t(1));
        }
        QueueNode node_entry = ([&](const auto& __v){ auto __i = best_idx; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue);
        std::string node = node_entry.node;
        std::vector<QueueNode> tmp = {};
        int64_t q = int64_t(0);
        while ((q < queue.size())) {
            if ((q != best_idx)) {
                tmp = ([&]{ auto& __tmp = tmp; __tmp.push_back(([&](const auto& __v){ auto __i = q; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue)); return __tmp; }());
            }
            q = (q + int64_t(1));
        }
        queue = tmp;
        for (const auto& __p : ([&](const auto& __m){ auto __it = __m.find(node); if (__it == __m.end()) return std::map<std::string, int64_t>{}; return __it->second; })(graph)) {
            ...
            if ((([&](const auto& __m){ auto __it = __m.find(neighbour); if (__it == __m.end()) return int64_t{}; return __it->second; })(dist) > (([&](const auto& __m){ auto __it = __m.find(node); if (__it == __m.end()) return int64_t{}; return __it->second; })(dist) + w))) {
                ...
                int64_t r = int64_t(0);
                while ((r < queue.size())) {
                    if ((([&](const auto& __v){ auto __i = r; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(queue).node == neighbour)) {
                        { auto& __tmp = queue[r]; __tmp.weight = ([&](const auto& __m){ auto __it = __m.find(neighbour); if (__it == __m.end()) return int64_t{}; return __it->second; })(dist); }
                        break;
                    }
                    r = (r + int64_t(1));
                }
