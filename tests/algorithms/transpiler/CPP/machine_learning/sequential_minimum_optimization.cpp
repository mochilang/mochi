// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:41:39 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
    ...
    while ((passes < max_passes)) {
        ...
        while ((i1 < m)) {
            ...
            if (((((_index(labels, i1) * Ei) < (0.0 - tol)) && (_index(alphas, i1) < c)) || (((_index(labels, i1) * Ei) > tol) && (_index(alphas, i1) > 0.0)))) {
                auto i2 = ((i1 + int64_t(1)) % m);
                double Ej = (predict_raw(samples, labels, alphas, b, samples[static_cast<size_t>(i2)]) - labels[static_cast<size_t>(i2)]);
                double alpha1_old = _index(alphas, i1);
                double alpha2_old = alphas[static_cast<size_t>(i2)];
                double L = 0.0;
                double H = 0.0;
                if ((_index(labels, i1) != labels[static_cast<size_t>(i2)])) {
                ...
                double eta = (((2.0 * dot(_index(samples, i1), samples[static_cast<size_t>(i2)])) - dot(_index(samples, i1), _index(samples, i1))) - dot(samples[static_cast<size_t>(i2)], samples[static_cast<size_t>(i2)]));
                if ((eta >= 0.0)) {
                    i1 = (i1 + int64_t(1));
                    continue;
                }
                alphas[static_cast<size_t>(i2)] = (alpha2_old - ((double)((labels[static_cast<size_t>(i2)] * (Ei - Ej))) / (eta)));
                if ((alphas[static_cast<size_t>(i2)] > H)) {
                    alphas[static_cast<size_t>(i2)] = H;
                }
                if ((alphas[static_cast<size_t>(i2)] < L)) {
                    alphas[static_cast<size_t>(i2)] = L;
                }
                if ((absf((alphas[static_cast<size_t>(i2)] - alpha2_old)) < 1e-05)) {
                    i1 = (i1 + int64_t(1));
                    continue;
                }
                alphas[i1] = (alpha1_old + ((_index(labels, i1) * labels[static_cast<size_t>(i2)]) * (alpha2_old - alphas[static_cast<size_t>(i2)])));
                double b1 = (((b - Ei) - ((_index(labels, i1) * (_index(alphas, i1) - alpha1_old)) * dot(_index(samples, i1), _index(samples, i1)))) - ((labels[static_cast<size_t>(i2)] * (alphas[static_cast<size_t>(i2)] - alpha2_old)) * dot(_index(samples, i1), samples[static_cast<size_t>(i2)])));
                double b2 = (((b - Ej) - ((_index(labels, i1) * (_index(alphas, i1) - alpha1_old)) * dot(_index(samples, i1), samples[static_cast<size_t>(i2)]))) - ((labels[static_cast<size_t>(i2)] * (alphas[static_cast<size_t>(i2)] - alpha2_old)) * dot(samples[static_cast<size_t>(i2)], samples[static_cast<size_t>(i2)])));
                if (((_index(alphas, i1) > 0.0) && (_index(alphas, i1) < c))) {
                    b = b1;
                } else                 if (((alphas[static_cast<size_t>(i2)] > 0.0) && (alphas[static_cast<size_t>(i2)] < c))) {
                    b = b2;
                } else {
                    b = ((double)((b1 + b2)) / (2.0));
                }
...
int main() {
    {
        ...
        labels = std::vector<double>{1.0, 1.0, -1.0, -1.0};
