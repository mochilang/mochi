// Generated by Mochi transpiler v0.10.63 on 2025-08-12 08:49:05 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
void mark_path(std::vector<std::vector<int64_t>>& world, const std::vector<Point>& path) {
    for (Point p : path) {
        world[p.x][p.y] = int64_t(1);
    }
}

void print_world(const std::vector<std::vector<int64_t>>& world) {
    for (std::vector<int64_t> row : world) {
        std::cout << ([&]{ std::ostringstream ss; { auto __tmp = row; ss << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) ss << ' '; ss << __tmp[i]; } ss << "]"; }
 return ss.str(); }());
        std::cout << std::endl;
    }
}
