// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:40:22 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
double ln(double x) {
    ...
    int64_t k = int64_t(0);
    while ((k < int64_t(10))) {
        double denom = to_float(((int64_t(2) * k) + int64_t(1)));
...
double mean(const std::vector<double>& v) {
    ...
    return ((double)(total) / (to_float((int64_t)(v.size()))));
}
...
double categorical_cross_entropy(const std::vector<std::vector<double>>& y_true, const std::vector<std::vector<double>>& y_pred, double epsilon) {
    ...
    while ((i < rows)) {
        ...
        while ((j < _index(y_true, i).size())) {
            ...
            total = (total - (_index(_index(y_true, i), j) * ln(yp)));
            j = (j + int64_t(1));
        }
        i = (i + int64_t(1));
    }
...
double categorical_focal_cross_entropy(const std::vector<std::vector<double>>& y_true, const std::vector<std::vector<double>>& y_pred, const std::vector<double>& alpha, double gamma, double epsilon) {
    ...
    int rows = y_true.size();
    int cols = _index(y_true, int64_t(0)).size();
    ...
    while ((i < rows)) {
        ...
        while ((j < cols)) {
            double yp = clip(_index(_index(y_pred, i), j), epsilon, 1.0);
            row_loss = (row_loss + (((_index(a, j) * powf((1.0 - yp), gamma)) * _index(_index(y_true, i), j)) * ln(yp)));
            j = (j + int64_t(1));
        }
        total = (total - row_loss);
        i = (i + int64_t(1));
    }
    return ((double)(total) / (to_float((int64_t)(rows))));
}
...
double huber_loss(const std::vector<double>& y_true, const std::vector<double>& y_pred, double delta) {
    ...
    return ((double)(total) / (to_float((int64_t)(y_true.size()))));
}
...
double mean_squared_logarithmic_error(const std::vector<double>& y_true, const std::vector<double>& y_pred) {
    ...
    int64_t i = int64_t(0);
    while ((i < y_true.size())) {
        double a = ln((1.0 + _index(y_true, i)));
        double b = ln((1.0 + _index(y_pred, i)));
        double diff = (a - b);
...
double perplexity_loss(const std::vector<std::vector<int64_t>>& y_true, const std::vector<std::vector<std::vector<double>>>& y_pred, double epsilon) {
    ...
    int vocab_size = _index(_index(y_pred, int64_t(0)), int64_t(0)).size();
    int64_t b = int64_t(0);
    ...
    while ((b < batch)) {
        ...
        double mean_log = ((double)(sum_log) / (to_float((int64_t)(sentence_len))));
        double perp = exp(-mean_log);
        total_perp = (total_perp + perp);
        b = (b + int64_t(1));
    }
    return ((double)(total_perp) / (to_float((int64_t)(batch))));
}
...
double kullback_leibler_divergence(const std::vector<double>& y_true, const std::vector<double>& y_pred) {
    ...
    int64_t i = int64_t(0);
    while ((i < y_true.size())) {
        total = (total + (_index(y_true, i) * ln(((double)(_index(y_true, i)) / (_index(y_pred, i))))));
        i = (i + int64_t(1));
    }
    return total;
}
