// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:41:12 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<std::vector<double>> standardize(const std::vector<std::vector<double>>& data) {
    ...
    int n_features = _index(data, int64_t(0)).size();
...
Eigen eigen_decomposition_2x2(const std::vector<std::vector<double>>& matrix) {
    ...
    if ((_index(eigenvalues, int64_t(0)) < _index(eigenvalues, int64_t(1)))) {
        ...
        eigenvalues[int64_t(0)] = _index(eigenvalues, int64_t(1));
        eigenvalues[int64_t(1)] = tmp_val;
        std::vector<double> tmp_vec = _index(eigenvectors, int64_t(0));
        eigenvectors[int64_t(0)] = _index(eigenvectors, int64_t(1));
        eigenvectors[int64_t(1)] = tmp_vec;
    }
...
std::vector<std::vector<double>> matrix_multiply(const std::vector<std::vector<double>>& a, const std::vector<std::vector<double>>& b) {
    int rows_a = a.size();
    int cols_a = _index(a, int64_t(0)).size();
    int rows_b = b.size();
    int cols_b = _index(b, int64_t(0)).size();
    if ((cols_a != rows_b)) {
...
int main() {
    {
        ...
        idx = int64_t(0);
        while ((idx < int64_t(5))) {
            { auto __tmp = _index(result.transformed, idx); std::cout << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) std::cout << ' '; std::cout << __tmp[i]; } std::cout << "]"; }
            std::cout << std::endl;
            idx = (idx + int64_t(1));
        }
        std::cout << std::string("Explained Variance Ratio:");
        std::cout << std::endl;
        { auto __tmp = result.variance_ratio; std::cout << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) std::cout << ' '; std::cout << __tmp[i]; } std::cout << "]"; }
        std::cout << std::endl;
    }
    return 0;
}
