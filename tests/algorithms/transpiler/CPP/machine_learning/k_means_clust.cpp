// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:39:09 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
double distance_sq(const std::vector<double>& a, const std::vector<double>& b) {
    double sum = 0.0;
    for (int i = int64_t(0); i < a.size(); i++ ) {
...
std::vector<int64_t> assign_clusters(const std::vector<std::vector<double>>& data, const std::vector<std::vector<double>>& centroids) {
    ...
    for (int i = int64_t(0); i < data.size(); i++ ) {
        ...
        for (int j = int64_t(1); j < centroids.size(); j++ ) {
...
std::vector<std::vector<double>> revise_centroids(const std::vector<std::vector<double>>& data, int64_t k, const std::vector<int64_t>& assignment) {
    int dim = _index(data, int64_t(0)).size();
    std::vector<std::vector<double>> sums = {};
    std::vector<int64_t> counts = {};
    for (int i = int64_t(0); i < k; i++ ) {
        std::vector<double> row = {};
        for (int j = int64_t(0); j < dim; j++ ) {
    ...
    for (int i = int64_t(0); i < data.size(); i++ ) {
        ...
        counts[c] = (_index(counts, c) + int64_t(1));
        for (int j = int64_t(0); j < dim; j++ ) {
            sums[c][j] = (_index(_index(sums, c), j) + _index(_index(data, i), j));
        }
    }
    std::vector<std::vector<double>> centroids = {};
    for (int i = int64_t(0); i < k; i++ ) {
...
int main() {
 ...
 return ss.str(); }());
        std::cout << std::endl;
        std::cout << ([&]{ std::ostringstream ss; { auto __tmp = result.heterogeneity; ss << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) ss << ' '; ss << __tmp[i]; } ss << "]"; }
 return ss.str(); }());
        std::cout << std::endl;
    }
    return 0;
