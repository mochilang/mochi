// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:39:46 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
    ...
    while ((i < x_items.size())) {
        ...
        while ((j < _index(x_items, i).size())) {
            ...
            while ((k < x_items.size())) {
                double discr = (((_index(_index(x_items, i), j) * ((double)(_index(means, k)) / (variance))) - ((double)((_index(means, k) * _index(means, k))) / ((2.0 * variance)))) + ln(_index(probabilities, k)));
...
int main() {
    {
        ...
        std::vector<int64_t> y = y_generator((int64_t)(counts.size()), counts);
        ...
        { auto __tmp = predicted; std::cout << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) std::cout << ' '; std::cout << __tmp[i]; } std::cout << "]"; }
        std::cout << std::endl;
