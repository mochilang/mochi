// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:40:05 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <any>
...
std::vector<std::vector<double>> matMul(const std::vector<std::vector<double>>& a, const std::vector<std::vector<double>>& b) {
    int a_rows = a.size();
    int a_cols = _index(a, int64_t(0)).size();
    int b_cols = _index(b, int64_t(0)).size();
    std::vector<std::vector<double>> res = {};
    int64_t i = int64_t(0);
    while ((i < a_rows)) {
        std::vector<double> row = {};
...
std::vector<std::vector<double>> matInv(const std::vector<std::vector<double>>& mat) {
    ...
    while ((col < n)) {
        ...
        int64_t j = int64_t(0);
        while ((j < (int64_t(2) * n))) {
            aug[col][j] = ((double)(_index(_index(aug, col), j)) / (pivot));
            j = (j + int64_t(1));
        }
        int64_t r = int64_t(0);
        while ((r < n)) {
            if ((r != col)) {
                ...
                j = int64_t(0);
                while ((j < (int64_t(2) * n))) {
                    aug[r][j] = (_index(_index(aug, r), j) - (factor * _index(_index(aug, col), j)));
                    j = (j + int64_t(1));
                }
            }
            r = (r + int64_t(1));
    ...
    while ((i < n)) {
        ...
        int64_t j = int64_t(0);
        while ((j < n)) {
            row = ([&]{ auto& __tmp = row; __tmp.push_back(_index(aug, i)[static_cast<size_t>((j + n))]); return __tmp; }());
            j = (j + int64_t(1));
        }
...
std::vector<std::vector<double>> weight_matrix(const std::vector<double>& point, const std::vector<std::vector<double>>& x_train, double tau) {
    ...
    while ((j < m)) {
        ...
        weights[j][j] = expApprox(((double)(-diff_sq) / (((2.0 * tau) * tau))));
        j = (j + int64_t(1));
    }
    return weights;
}
