// Generated by Mochi transpiler v0.10.68 on 2025-08-17 20:38:37 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
std::vector<int64_t> shuffle(std::vector<int64_t>& list_int) {
    ...
    while ((i > int64_t(0))) {
        ...
        auto& tmp = list_int[static_cast<size_t>(i)];
...
bool insert_dir(WordSearch& ws, std::string word, int64_t dr, int64_t dc, const std::vector<int64_t>& rows, const std::vector<int64_t>& cols) {
    ...
    while ((ri < rows.size())) {
        ...
        while ((ci < cols.size())) {
            auto& col = cols[static_cast<size_t>(ci)];
            ...
            if (ok) {
                ...
                while ((k < word_len)) {
                    ...
                    auto& row_list = ws.board[static_cast<size_t>(rr2)];
                    row_list[static_cast<size_t>(cc2)] = ([&](const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__v)>, std::string>) return __v.substr(k, (k + int64_t(1)) - k); else return std::vector<typename std::decay_t<decltype(__v)>::value_type>(__v.begin()+k, __v.begin()+(k + int64_t(1))); })(word);
                    k = (k + int64_t(1));
                }
                return true;
            }
...
void generate_board(WordSearch& ws) {
    ...
    while ((i < ws.words.size())) {
        auto& word = ws.words[static_cast<size_t>(i)];
...
std::string visualise(WordSearch& ws, bool add_fake_chars) {
    std::string result = std::string("");
    int64_t r = int64_t(0);
    while ((r < ws.height)) {
        int64_t c = int64_t(0);
        while ((c < ws.width)) {
            auto& ch = _index(ws.board, r)[static_cast<size_t>(c)];
            if ((ch == std::string(""))) {
                if (add_fake_chars) {
                    ch = rand_letter();
                } else {
