// Generated by Mochi transpiler v0.10.61 on 2025-08-08 20:03:17 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <functional>
...
std::vector<Thing> sort_desc(const std::vector<Thing>& items, std::function<double(Thing)> key_func) {
    ...
    while ((j < arr.size())) {
        ...
        while (((k >= int64_t(0)) && ((double)(key_func(_index(arr, k))) < (double)(key_val)))) {
            arr[(k + int64_t(1))] = _index(arr, k);
            k = (k - int64_t(1));
        }
        arr[(k + int64_t(1))] = key_item;
        j = (j + int64_t(1));
    }
    return arr;
}
...
std::string thing_to_string(const Thing& t) {
    return ((((((std::string("Thing(") + t.name) + std::string(", ")) + ([&]{ std::ostringstream ss; ss << std::boolalpha << t.value; return ss.str(); }())) + std::string(", ")) + ([&]{ std::ostringstream ss; ss << std::boolalpha << t.weight; return ss.str(); }())) + std::string(")"));
}
...
int main() {
    {
        ...
        std::cout << std::endl;
        std::cout << ([&]{ std::ostringstream ss; ss << std::boolalpha << res.total_value; return ss.str(); }());
        std::cout << std::endl;
    }
    return 0;
}
