// Generated by Mochi transpiler v0.10.69 on 2025-08-22 13:10:28 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<std::vector<double>> get_data(const std::vector<std::vector<double>>& source_data) {
    ...
    while ((i < source_data.size())) {
        ...
        while ((j < row.size())) {
            ...
            data_lists[static_cast<size_t>(j)] = ([&]{ auto __tmp = _index(data_lists, j); __tmp.push_back(_index(row, j)); return __tmp; }());
            j = (j + int64_t(1));
        }
        i = (i + int64_t(1));
    }
...
std::vector<double> generate_final_scores(const std::vector<std::vector<double>>& score_lists) {
    ...
    while ((i < score_lists.size())) {
        ...
        int64_t j = int64_t(0);
        while ((j < slist.size())) {
            final_scores[static_cast<size_t>(j)] = (_index(final_scores, j) + _index(slist, j));
            j = (j + int64_t(1));
        }
        i = (i + int64_t(1));
    }
...
std::vector<std::vector<double>> procentual_proximity(std::vector<std::vector<double>>& source_data, const std::vector<int64_t>& weights) {
    ...
    while ((i < final_scores.size())) {
        source_data[static_cast<size_t>(i)] = ([&]{ auto __tmp = _index(source_data, i); __tmp.push_back(_index(final_scores, i)); return __tmp; }());
        i = (i + int64_t(1));
    }
    return source_data;
}
