// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:04:43 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::string points_to_polynomial(const std::vector<std::vector<int64_t>>& coordinates) {
    ...
    while ((row < n)) {
        ...
        while ((col < n)) {
            double power = pow_int_float(_index(_index(coordinates, row), int64_t(0)), (int64_t)((n - (col + int64_t(1)))));
    ...
    while ((count < n)) {
        ...
        while ((number < n)) {
            if ((count != number)) {
                ...
                while ((cc < n)) {
                    matrix[number][cc] = (_index(_index(matrix, number), cc) - (_index(_index(matrix, count), cc) * fraction));
                    cc = (cc + int64_t(1));
                }
                vector[number] = (_index(vector, number) - (_index(vector, count) * fraction));
            }
    ...
    while ((count < n)) {
        ...
        solution = ([&]{ auto& __tmp = solution; __tmp.push_back(([&]{ std::ostringstream ss; ss << std::boolalpha << value; return ss.str(); }())); return __tmp; }());
        count = (count + int64_t(1));
    }
    ...
    while ((count < n)) {
        ...
        solved = ((((solved + std::string("x^")) + ([&]{ std::ostringstream ss; any_to_stream(ss, (n - (count + int64_t(1))));
 return ss.str(); }())) + std::string("*")) + coeff);
