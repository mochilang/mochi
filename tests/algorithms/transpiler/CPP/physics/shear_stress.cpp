// Generated by Mochi transpiler v0.10.69 on 2025-08-22 14:02:52 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <iomanip>
...
Result shear_stress(double stress, double tangential_force, double area) {
    ...
    if ((zeros != int64_t(1))) {
        ...
        throw std::runtime_error(std::string("Stress cannot be negative"));
    } else     if ((tangential_force < 0.0)) {
        throw std::runtime_error(std::string("Tangential Force cannot be negative"));
    } else     if ((area < 0.0)) {
        throw std::runtime_error(std::string("Area cannot be negative"));
    } else     if ((stress == 0.0)) {
        return Result{.name = std::string("stress"), .value = ((double)(tangential_force) / (area))};
    } else     if ((tangential_force == 0.0)) {
        return Result{.name = std::string("tangential_force"), .value = (stress * area)};
    } else {
        return Result{.name = std::string("area"), .value = ((double)(tangential_force) / (stress))};
    }
...
std::string str_result(const Result& r) {
    return ((((std::string("Result(name='") + r.name) + std::string("', value=")) + _to_string(r.value)) + std::string(")"));
}
