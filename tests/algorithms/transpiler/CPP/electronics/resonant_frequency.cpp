// Generated by Mochi transpiler v0.10.73 on 2025-08-25 16:45:27 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <iomanip>
...
double resonant_frequency(double inductance, double capacitance) {
    if ((inductance <= 0.0)) {
        throw std::runtime_error(std::string("Inductance cannot be 0 or negative"));
    }
    if ((capacitance <= 0.0)) {
        throw std::runtime_error(std::string("Capacitance cannot be 0 or negative"));
    }
    double denom = ((2.0 * PI) * sqrtApprox((inductance * capacitance)));
    return ((double)(1.0) / (double)(denom));
}
...
int main() {
    {
        ...
        { std::ostringstream __ss; double __dv = resonant_frequency(10.0, 5.0); if(__dv == 0) __dv = 0; if(std::floor(__dv) == __dv) { __ss<<std::fixed<<std::setprecision(1)<<__dv; } else { __ss<<std::defaultfloat<<std::setprecision(6)<<__dv; } std::cout << __ss.str(); }
        std::cout << std::endl;
    }
    return 0;
}
