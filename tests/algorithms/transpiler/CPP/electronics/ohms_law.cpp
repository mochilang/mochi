// Generated by Mochi transpiler v0.10.72 on 2025-08-25 08:43:29 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <map>
...
std::map<std::string, double> ohms_law(double voltage, double current, double resistance);
int main();

std::map<std::string, double> ohms_law(double voltage, double current, double resistance) {
    int64_t zeros = int64_t(0);
    if ((std::fabs(voltage - 0.0) < 1e-6)) {
        zeros = (zeros + int64_t(1));
    }
    if ((std::fabs(current - 0.0) < 1e-6)) {
        zeros = (zeros + int64_t(1));
    }
    ...
    if ((zeros != int64_t(1))) {
        std::cout << std::string("One and only one argument must be 0");
        std::cout << std::endl;
        return {};
    }
    if ((resistance < 0.0)) {
        std::cout << std::string("Resistance cannot be negative");
        std::cout << std::endl;
        return {};
    }
    if ((std::fabs(voltage - 0.0) < 1e-6)) {
        return std::map<std::string, double>{{"voltage", (current * resistance)}};
    }
    if ((std::fabs(current - 0.0) < 1e-6)) {
        return std::map<std::string, double>{{"current", ((double)(voltage) / (double)(resistance))}};
    }
    return std::map<std::string, double>{{"resistance", ((double)(voltage) / (double)(current))}};
}
