// Generated by Mochi transpiler v0.10.64 on 2025-08-12 16:33:17 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<std::string> split(std::string s, std::string sep) {
    ...
    while ((i < s.size())) {
        ...
        if ((ch == sep)) {
            parts = ([&]{ auto __tmp = parts; __tmp.push_back(s.substr(last, i - last)); return __tmp; }());
            last = (i + int64_t(1));
        }
        if (((i + int64_t(1)) == s.size())) {
            parts = ([&]{ auto __tmp = parts; __tmp.push_back(s.substr(last, (i + int64_t(1)) - last)); return __tmp; }());
        }
        i = (i + int64_t(1));
    }
    return parts;
...
std::vector<std::string> extract_links(std::string domain, std::string html) {
    ...
    while ((pos >= int64_t(0))) {
        int64_t start_quote = index_of(html.substr((pos + int64_t(5)), html.size() - (pos + int64_t(5))), std::string("\""));
        if ((start_quote < int64_t(0))) {
            break;
        }
        int64_t rest = (((pos + int64_t(5)) + start_quote) + int64_t(1));
        int64_t end_quote = index_of(html.substr(rest, html.size() - rest), std::string("\""));
        if ((end_quote < int64_t(0))) {
            break;
        }
        std::string link = html.substr(rest, (rest + end_quote) - rest);
        ...
        pos = index_of_substring(html.substr((rest + end_quote), html.size() - (rest + end_quote)), std::string("href="));
        if ((pos >= int64_t(0))) {
...
std::vector<std::string> extract_emails(std::string domain, std::string text) {
    ...
    while ((i < text.size())) {
        if ((([&](const auto& __s){ auto __i = i; if (__i < 0) __i += __s.size(); return std::string(1, __s[static_cast<size_t>(__i)]); })(text) == std::string("@"))) {
            if ((text.substr((i + int64_t(1)), ((i + int64_t(1)) + domain.size()) - (i + int64_t(1))) == domain)) {
                ...
                std::string local = text.substr((j + int64_t(1)), i - (j + int64_t(1)));
