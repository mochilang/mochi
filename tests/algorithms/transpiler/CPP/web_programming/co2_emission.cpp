// Generated by Mochi transpiler v0.10.66 on 2025-08-14 19:15:52 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <iomanip>
...
struct Intensity {
    ...
    int64_t actual;
    std::string index;
    auto operator<=>(const Intensity&) const = default;
};
...
std::ostream& operator<<(std::ostream& os, const Intensity& v) {
    os << '{' << "'forecast': "<< v.forecast
 << ", " << "'actual': "<< v.actual
 << ", " << "'index': "<< "'" << v.index << "'"
 << '}';
    return os;
...
int64_t fetch_last_half_hour();
std::vector<Entry> fetch_from_to(std::string start, std::string end);
int main();

std::string BASE_URL = std::string("https://api.carbonintensity.org.uk/intensity");

int64_t fetch_last_half_hour() {
    Response resp = _fetch_Response(BASE_URL);
    Entry entry = _index(resp.data, int64_t(0));
    return entry.intensity.actual;
}

std::vector<Entry> fetch_from_to(std::string start, std::string end) {
    std::string url = ((((BASE_URL + std::string("/")) + start) + std::string("/")) + end);
    Response resp = _fetch_Response(url);
    return resp.data;
}
...
int main() {
    {
        ...
        std::vector<Entry> entries = fetch_from_to(std::string("2020-10-01"), std::string("2020-10-03"));
        int64_t i = int64_t(0);
        while ((i < entries.size())) {
            ...
            std::cout << std::string(":");
            std::cout << " ";
            std::cout << e.intensity.actual;
            std::cout << std::endl;
            i = (i + int64_t(1));
        }
        int64_t last = fetch_last_half_hour();
        std::cout << std::string("fetch_last_half_hour() =");
        std::cout << " ";
        std::cout << last;
        std::cout << std::endl;
    }
