// Generated by Mochi transpiler v0.10.61 on 2025-08-08 18:18:05 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<Coord> parse_moves(std::string input_str) {
    ...
    while ((i < pairs.size())) {
        ...
        int64_t j = int64_t(0);
        while ((j < pair.size())) {
            std::string ch = pair.substr(j, (j + int64_t(1)) - j);
            if ((ch == std::string(" "))) {
                if ((num != std::string(""))) {
...
void validate_matrix_content(const std::vector<std::string>& matrix, int64_t size) {
    ...
    while ((i < size)) {
        ...
        int64_t j = int64_t(0);
        while ((j < size)) {
            std::string ch = row.substr(j, (j + int64_t(1)) - j);
            if (!is_alnum(ch)) {
...
std::vector<std::vector<std::string>> move_x(std::vector<std::vector<std::string>> matrix_g, int64_t column, int64_t size) {
    ...
    row = int64_t(0);
    while ((row < size)) {
        matrix_g[row][column] = _index(new_list, row);
        row = (row + int64_t(1));
    }
    return matrix_g;
}
...
std::vector<std::vector<std::string>> move_y(std::vector<std::vector<std::string>> matrix_g, int64_t size) {
    ...
    while ((i < empty_cols.size())) {
        ...
        while ((c < size)) {
            int64_t r = int64_t(0);
            while ((r < size)) {
                matrix_g[r][(c - int64_t(1))] = _index(_index(matrix_g, r), c);
                r = (r + int64_t(1));
            }
            c = (c + int64_t(1));
        }
        int64_t r = int64_t(0);
        while ((r < size)) {
            matrix_g[r][(size - int64_t(1))] = std::string("-");
            r = (r + int64_t(1));
        }
        i = (i + int64_t(1));
    }
...
std::vector<std::vector<std::string>> build_matrix(const std::vector<std::string>& matrix) {
    ...
    while ((i < matrix.size())) {
        ...
        int64_t j = int64_t(0);
        while ((j < row.size())) {
            row_list = ([&]{ auto& __tmp = row_list; __tmp.push_back(row.substr(j, (j + int64_t(1)) - j)); return __tmp; }());
            j = (j + int64_t(1));
        }
