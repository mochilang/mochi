// Generated by Mochi transpiler v0.10.61 on 2025-08-08 18:18:10 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::string matrix_to_string(const Matrix& m) {
    ...
    while ((i < m.rows)) {
        ...
        int64_t j = int64_t(0);
        while ((j < m.cols)) {
            s = (s + ([&]{ std::ostringstream ss; ss << std::boolalpha << _index(_index(m.data, i), j); return ss.str(); }()));
...
int main() {
    {
        ...
        std::cout << std::endl;
        std::cout << ([&]{ std::ostringstream ss; { auto __tmp = matrix_columns(m); ss << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) ss << ' '; { std::ostringstream __ss; __ss << __tmp[static_cast<size_t>(i)];
 ss << __ss.str(); }; } ss << "]"; }
 return ss.str(); }());
        std::cout << std::endl;
