// Generated by Mochi transpiler v0.10.69 on 2025-08-22 14:40:44 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
int64_t solution(const std::vector<int64_t>& ciphertext) {
    ...
    while ((j < decoded_text.size())) {
        sum = (sum + ord(([&]{ std::string __s = decoded_text; long long __start = j; long long __end = (j + int64_t(1)); if(__start < 0) __start = 0; if(__end < __start) __end = __start; if(__start > static_cast<long long>(__s.size())) __start = __s.size(); if(__end > static_cast<long long>(__s.size())) __end = __s.size(); return __s.substr(static_cast<size_t>(__start), static_cast<size_t>(__end - __start)); })()));
        j = (j + int64_t(1));
    }
    return sum;
}
