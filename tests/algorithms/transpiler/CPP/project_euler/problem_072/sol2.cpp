// Generated by Mochi transpiler v0.10.63 on 2025-08-11 14:51:03 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
int64_t solution(int64_t limit) {
    ...
    int64_t i = int64_t(0);
    while ((i <= limit)) {
        phi = ([&]{ auto __tmp = phi; __tmp.push_back(i); return __tmp; }());
        i = (i + int64_t(1));
    }
    int64_t n = int64_t(2);
    while ((n <= limit)) {
        if ((_index(phi, n) == n)) {
            int64_t k = n;
            while ((k <= limit)) {
                phi[k] = (_index(phi, k) - (_index(phi, k) / n));
                k = (k + n);
            }
        }
        n = (n + int64_t(1));
