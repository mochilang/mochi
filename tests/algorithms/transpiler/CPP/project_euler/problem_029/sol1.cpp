// Generated by Mochi transpiler v0.10.71 on 2025-08-23 15:50:46 GMT+7
...
cpp_int int_pow(const cpp_int& base, const cpp_int& exp);
...
cpp_int solution(const cpp_int& n) {
    std::vector<cpp_int> powers = {};
    cpp_int limit = (n + int64_t(1));
    for (int a = int64_t(2); a < limit; a++ ) {
        for (int b = int64_t(2); b < limit; b++ ) {
            cpp_int p = int_pow(a, b);
            if (!([&](const auto& __c, const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__c)>, std::string>) { return __c.find(__v) != std::string::npos; } else if constexpr(requires { __c.find(__v); }) { return __c.find(__v) != __c.end(); } else { return std::find(__c.begin(), __c.end(), __v) != __c.end(); } })(powers, p)) {
                powers = ([&]{ auto __tmp = powers; __tmp.push_back(p); return __tmp; }());
            }
        }
    }
    return powers.size();
...
int main() {
    {
        ...
        std::cout << std::string("Number of terms ");
        std::cout << " ";
        std::cout << solution(n);
        std::cout << std::endl;
    }
