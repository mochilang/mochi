// Generated by Mochi transpiler v0.10.71 on 2025-08-23 15:51:15 GMT+7
...
cpp_int solution(const cpp_int& pence);
int main();

cpp_int solution(const cpp_int& pence) {
    std::vector<cpp_int> coins = std::vector<cpp_int>{int64_t(1), int64_t(2), int64_t(5), int64_t(10), int64_t(20), int64_t(50), int64_t(100), int64_t(200)};
    std::vector<cpp_int> ways = {};
    cpp_int i = int64_t(0);
    while ((i <= pence)) {
        ways = ([&]{ auto __tmp = ways; __tmp.push_back(int64_t(0)); return __tmp; }());
        i = (i + int64_t(1));
    }
    ways[static_cast<size_t>(int64_t(0))] = int64_t(1);
    cpp_int idx = int64_t(0);
    while ((idx < coins.size())) {
        cpp_int coin = coins[static_cast<size_t>(idx)];
        cpp_int j = coin;
        while ((j <= pence)) {
            ways[static_cast<size_t>(j)] = (ways[static_cast<size_t>(j)] + ways[static_cast<size_t>((j - coin))]);
            j = (j + int64_t(1));
        }
        idx = (idx + int64_t(1));
    }
    return ways[static_cast<size_t>(pence)];
}
...
int main() {
    {
        ...
        std::cout << solution(int64_t(500));
        std::cout << std::endl;
        std::cout << solution(int64_t(200));
        std::cout << std::endl;
        std::cout << solution(int64_t(50));
        std::cout << std::endl;
