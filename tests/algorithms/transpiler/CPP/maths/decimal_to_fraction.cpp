// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:47:02 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <iomanip>
...
Fraction parse_decimal(std::string s) {
    ...
    std::string first = s.substr(int64_t(0), int64_t(1) - int64_t(0));
    if ((first == std::string("-"))) {
        sign = -int64_t(1);
        idx = int64_t(1);
    ...
    while ((idx < s.size())) {
        std::string c = s.substr(idx, (idx + int64_t(1)) - idx);
    ...
    if (((idx < s.size()) && (s.substr(idx, (idx + int64_t(1)) - idx) == std::string(".")))) {
        idx = (idx + int64_t(1));
        while ((idx < s.size())) {
            std::string c = s.substr(idx, (idx + int64_t(1)) - idx);
            if (((c >= std::string("0")) && (c <= std::string("9")))) {
    ...
    if (((idx < s.size()) && ((s.substr(idx, (idx + int64_t(1)) - idx) == std::string("e")) || (s.substr(idx, (idx + int64_t(1)) - idx) == std::string("E"))))) {
        idx = (idx + int64_t(1));
        int64_t exp_sign = int64_t(1);
        if (((idx < s.size()) && (s.substr(idx, (idx + int64_t(1)) - idx) == std::string("-")))) {
            exp_sign = -int64_t(1);
            idx = (idx + int64_t(1));
        } else         if (((idx < s.size()) && (s.substr(idx, (idx + int64_t(1)) - idx) == std::string("+")))) {
            idx = (idx + int64_t(1));
        }
...
Fraction decimal_to_fraction(double x) {
    return decimal_to_fraction_str(([&]{ std::ostringstream ss; ss << std::boolalpha << x; return ss.str(); }()));
}
