// Generated by Mochi transpiler v0.10.61 on 2025-08-08 16:47:25 GMT+7
#include <iostream>
#include <string>
#include <map>
#include <algorithm>
...
struct TextCounts {
    ...
    std::map<std::string, int64_t> double;
...
std::ostream& operator<<(std::ostream& os, const TextCounts& v) {
 ...
 << ", " << "'double': "<< "{"; bool first_1=true; for(const auto& p: v.double){ if(!first_1) os << ", "; first_1=false; os << p.first; os << ': '; os << p.second; } os << "}"
 << '}';
    return os;
}
...
TextCounts analyze_text(std::string text) {
    ...
    int64_t n = text.size();
    if ((n == int64_t(0))) {
        return TextCounts{.single = single, .double = _double};
    }
    std::string last = text.substr((n - int64_t(1)), n - (n - int64_t(1)));
    ...
    std::string first = text.substr(int64_t(0), int64_t(1) - int64_t(0));
    std::string pair0 = (std::string(" ") + first);
    _double[pair0] = int64_t(1);
    int64_t i = int64_t(0);
    while ((i < (n - int64_t(1)))) {
        std::string ch = text.substr(i, (i + int64_t(1)) - i);
        ...
        std::string seq = text.substr(i, (i + int64_t(2)) - i);
...
void calculate_entropy(std::string text) {
    ...
    int64_t i = int64_t(0);
    while ((i < alphas.size())) {
        std::string ch = alphas.substr(i, (i + int64_t(1)) - i);
    ...
    for (const auto& __p : counts.double) {
        auto seq = __p.first;
        total2 = (total2 + ([&](const auto& __m){ auto __it = __m.find(seq); if (__it == __m.end()) return int64_t{}; return __it->second; })(counts.double));
    }
    double h2 = 0.0;
    int64_t a0 = int64_t(0);
    while ((a0 < alphas.size())) {
        std::string ch0 = alphas.substr(a0, (a0 + int64_t(1)) - a0);
        int64_t a1 = int64_t(0);
        while ((a1 < alphas.size())) {
            std::string ch1 = alphas.substr(a1, (a1 + int64_t(1)) - a1);
            std::string seq = (ch0 + ch1);
            if (([&](const auto& __c, const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__c)>, std::string>) { return __c.find(__v) != std::string::npos; } else if constexpr(requires { __c.find(__v); }) { return __c.find(__v) != __c.end(); } else { return std::find(__c.begin(), __c.end(), __v) != __c.end(); } })(counts.double, seq)) {
                double prob = ((double)((double)(([&](const auto& __m){ auto __it = __m.find(seq); if (__it == __m.end()) return int64_t{}; return __it->second; })(counts.double))) / ((double)(total2)));
