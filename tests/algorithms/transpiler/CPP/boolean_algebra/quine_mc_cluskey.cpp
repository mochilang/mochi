// Generated by Mochi transpiler v0.10.59 on 2025-08-06 20:51:41 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
    ...
    while (true) {
        ...
        while ((i < current.size())) {
            ...
            while ((j < current.size())) {
                std::string k = compare_string(current[i], current[j]);
                if ((k == std::string(""))) {
                    check1[i] = std::string("*");
                    check1[j] = std::string("*");
        ...
        i = int64_t(0);
        while ((i < current.size())) {
            if ((check1[i] == std::string("$"))) {
                pi = ([&]{ auto __tmp = pi; __tmp.push_back(current[i]); return __tmp; }());
            }
            i = (i + int64_t(1));
        }
...
std::vector<std::string> selection(std::vector<std::vector<int64_t>>& chart, const std::vector<std::string>& prime_implicants) {
    ...
    while ((col < chart[int64_t(0)].size())) {
        int64_t count = int64_t(0);
        int64_t row = int64_t(0);
        while ((row < chart.size())) {
            if ((chart[row][col] == int64_t(1))) {
                count = (count + int64_t(1));
            }
            row = (row + int64_t(1));
        }
        if ((count == int64_t(1))) {
            ...
            select[rem] = int64_t(1);
        }
        col = (col + int64_t(1));
    }
    i = int64_t(0);
    while ((i < select.size())) {
        if ((select[i] == int64_t(1))) {
            int64_t j = int64_t(0);
            while ((j < chart[int64_t(0)].size())) {
                if ((chart[i][j] == int64_t(1))) {
                    int64_t r = int64_t(0);
                    while ((r < chart.size())) {
                        chart[r][j] = int64_t(0);
                        r = (r + int64_t(1));
            ...
            temp = ([&]{ auto __tmp = temp; __tmp.push_back(prime_implicants[i]); return __tmp; }());
        }
        i = (i + int64_t(1));
    }
    while (true) {
        ...
        int64_t r = int64_t(0);
        while ((r < chart.size())) {
            counts = ([&]{ auto __tmp = counts; __tmp.push_back(count_ones(chart[r])); return __tmp; }());
            r = (r + int64_t(1));
        }
        int64_t max_n = counts[int64_t(0)];
        int64_t rem = int64_t(0);
        int64_t k = int64_t(1);
        while ((k < counts.size())) {
            if ((counts[k] > max_n)) {
        ...
        temp = ([&]{ auto __tmp = temp; __tmp.push_back(prime_implicants[rem]); return __tmp; }());
        int64_t j = int64_t(0);
        while ((j < chart[int64_t(0)].size())) {
            if ((chart[rem][j] == int64_t(1))) {
                int64_t r2 = int64_t(0);
                while ((r2 < chart.size())) {
                    chart[r2][j] = int64_t(0);
                    r2 = (r2 + int64_t(1));
                }
            }
            j = (j + int64_t(1));
...
std::vector<std::vector<int64_t>> prime_implicant_chart(const std::vector<std::string>& prime_implicants, const std::vector<std::string>& binary) {
    ...
    i = int64_t(0);
    while ((i < prime_implicants.size())) {
        int64_t count = count_char(prime_implicants[i], std::string("_"));
        int64_t j = int64_t(0);
        while ((j < binary.size())) {
...
int main() {
    {
        ...
        std::cout << std::endl;
        std::cout << ([&]{ std::ostringstream ss; { auto __tmp = prime_implicants; ss << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) ss << ' '; ss << __tmp[i]; } ss << "]"; }
 return ss.str(); }());
        std::cout << std::endl;
        ...
        std::cout << std::endl;
        std::cout << ([&]{ std::ostringstream ss; { auto __tmp = essential_prime_implicants; ss << "["; for(size_t i=0;i<__tmp.size();++i){ if(i>0) ss << ' '; ss << __tmp[i]; } ss << "]"; }
 return ss.str(); }());
        std::cout << std::endl;
    }
    return 0;
