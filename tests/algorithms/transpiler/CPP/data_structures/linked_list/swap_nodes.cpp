// Generated by Mochi transpiler v0.10.72 on 2025-08-24 22:21:04 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
LinkedList push(const LinkedList& list, int64_t value);
LinkedList swap_nodes(const LinkedList& list, int64_t v1, int64_t v2);
std::string to_string(const LinkedList& list);
int main();

LinkedList empty_list() {
    return LinkedList{std::vector<int64_t>{}};
}
...
LinkedList swap_nodes(const LinkedList& list, int64_t v1, int64_t v2) {
    ...
    int64_t idx1 = (int64_t(0) - int64_t(1));
    int64_t idx2 = (int64_t(0) - int64_t(1));
    int64_t i = int64_t(0);
    while ((i < list.data.size())) {
        if (((_index(list.data, i) == v1) && (idx1 == (int64_t(0) - int64_t(1))))) {
            idx1 = i;
        }
        if (((_index(list.data, i) == v2) && (idx2 == (int64_t(0) - int64_t(1))))) {
            idx2 = i;
        }
        i = (i + int64_t(1));
    }
    if (((idx1 == (int64_t(0) - int64_t(1))) || (idx2 == (int64_t(0) - int64_t(1))))) {
        return list;
    }
    std::vector<int64_t> res = list.data;
    int64_t temp = _index(res, idx1);
    _index(res, idx1) = _index(res, idx2);
    _index(res, idx2) = temp;
    return LinkedList{res};
}

std::string to_string(const LinkedList& list) {
    return ([&]{ std::ostringstream ss; { auto __tmp1 = list.data; ss << "["; for(size_t i=0;i<__tmp1.size();++i){ if(i>0) ss << ' '; ss << __tmp1[i]; } ss << "]"; }
 return ss.str(); }());
}
...
int main() {
    {
        ...
        std::cout << (std::string("Original Linked List: ") + to_string(ll));
        std::cout << std::endl;
        ll = swap_nodes(ll, int64_t(1), int64_t(4));
        std::cout << (std::string("Modified Linked List: ") + to_string(ll));
        std::cout << std::endl;
        std::cout << std::string("After swapping the nodes whose data is 1 and 4.");
        std::cout << std::endl;
    }
    return 0;
}
