// Generated by Mochi transpiler v0.10.72 on 2025-08-24 22:20:46 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <vector>
...
LinkedList reverse_k_nodes(const LinkedList& list, int64_t k) {
    ...
    while ((i < list.data.size())) {
        ...
        while (((j < k) && ((i + j) < list.data.size()))) {
            group = ([&]{ auto __tmp = group; __tmp.push_back(_index(list.data, (i + j))); return __tmp; }());
            j = (j + int64_t(1));
        }
        if ((group.size() == k)) {
            int64_t g = (k - int64_t(1));
            while ((g >= int64_t(0))) {
                res = ([&]{ auto __tmp = res; __tmp.push_back(_index(group, g)); return __tmp; }());
                g = (g - int64_t(1));
            }
        } else {
            int64_t g = int64_t(0);
...
int main() {
    {
        ...
        LinkedList ll = LinkedList{std::vector<int64_t>{int64_t(1), int64_t(2), int64_t(3), int64_t(4), int64_t(5)}};
