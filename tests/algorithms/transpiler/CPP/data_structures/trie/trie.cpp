// Generated by Mochi transpiler v0.10.59 on 2025-08-07 09:03:02 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <map>
...
Trie new_trie();
std::map<std::string, int64_t> remove_key(std::map<std::string, int64_t>& m, std::string k);
void insert(Trie& trie, std::string word);
void insert_many(Trie& trie, std::vector<std::string>& words);
bool find(Trie& trie, std::string word);
...
void insert(Trie& trie, std::string word) {
    ...
    while ((i < word.size())) {
        ...
        std::map<std::string, int64_t> children = nodes[curr].children;
        if (([&](const auto& __c, const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__c)>, std::string>) { return __c.find(__v) != std::string::npos; } else if constexpr(requires { __c.find(__v); }) { return __c.find(__v) != __c.end(); } else { return std::find(__c.begin(), __c.end(), __v) != __c.end(); } })(children, ch)) {
            child_idx = ([&](const auto& __m){ auto __it = __m.find(ch); return __it != __m.end() ? __it->second : 0; })(children);
        } else {
...
bool find(Trie& trie, std::string word) {
    ...
    while ((i < word.size())) {
        ...
        curr = ([&](const auto& __m){ auto __it = __m.find(ch); return __it != __m.end() ? __it->second : 0; })(children);
        i = (i + int64_t(1));
    }
    Node node = nodes[curr];
    return node.is_leaf;
...
void _delete(Trie& trie, std::string word) {
    ...
    std::function<bool(int64_t, int64_t)> _delete = [=](int64_t idx, int64_t pos) mutable {
        ...
        std::string ch = std::string(1, word[pos]);
        if (!([&](const auto& __c, const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__c)>, std::string>) { return __c.find(__v) != std::string::npos; } else if constexpr(requires { __c.find(__v); }) { return __c.find(__v) != __c.end(); } else { return std::find(__c.begin(), __c.end(), __v) != __c.end(); } })(children, ch)) {
            return false;
        }
        int64_t child_idx = ([&](const auto& __m){ auto __it = __m.find(ch); return __it != __m.end() ? __it->second : 0; })(children);
...
void print_words(Trie& trie) {
    std::function<std::any(int64_t, std::string)> dfs = [=](int64_t idx, std::string word) mutable {
        ...
        for (const auto& __p : node.children) {
            auto key = __p.first;
            dfs(([&](const auto& __m){ auto __it = __m.find(key); return __it != __m.end() ? __it->second : 0; })(node.children), (word + key));
        }
        return std::any();
    };
