// Generated by Mochi transpiler v0.10.59 on 2025-08-07 08:58:41 GMT+7
#include <iostream>
#include <string>
#include <sstream>
#include <vector>
...
FixedPriorityQueue fpq_enqueue(FixedPriorityQueue& fpq, int64_t priority, int64_t data) {
    if (((priority < int64_t(0)) || (priority >= fpq.queues.size()))) {
        throw std::runtime_error(std::string("Valid priorities are 0, 1, and 2"));
        return fpq;
    }
    if ((fpq.queues[priority].size() >= int64_t(100))) {
        throw std::runtime_error(std::string("Maximum queue size is 100"));
        return fpq;
    }
...
FPQDequeueResult fpq_dequeue(FixedPriorityQueue& fpq) {
    ...
    throw std::runtime_error(std::string("All queues are empty"));
...
EPQDequeueResult epq_dequeue(ElementPriorityQueue& epq) {
    if ((epq.queue.size() == int64_t(0))) {
        throw std::runtime_error(std::string("The queue is empty"));
