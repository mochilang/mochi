// Generated by Mochi transpiler v0.10.72 on 2025-08-24 22:21:46 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <map>
...
bool balanced_parentheses(std::string s) {
    ...
    std::map<std::string, std::string> pairs = {{"(", std::string(")")}, {"[", std::string("]")}, {"{", std::string("}")}};
    int64_t i = int64_t(0);
    while ((i < s.size())) {
        ...
        if (([&](const auto& __c, const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__c)>, std::string>) { return __c.find(__v) != std::string::npos; } else if constexpr(requires { __c.find(__v); }) { return __c.find(__v) != __c.end(); } else { return std::find(__c.begin(), __c.end(), __v) != __c.end(); } })(pairs, ch)) {
            ...
            if ((([&](const auto& __m){ auto __it = __m.find(top); if (__it == __m.end()) return std::string{}; return __it->second; })(pairs) != ch)) {
                return false;
            }
...
int main() {
    {
        ...
        while ((idx < tests.size())) {
            std::cout << (balanced_parentheses(_index(tests, idx)) ? "true" : "false");
            std::cout << std::endl;
            idx = (idx + int64_t(1));
        }
    }
