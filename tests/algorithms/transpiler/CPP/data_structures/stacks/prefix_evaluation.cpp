// Generated by Mochi transpiler v0.10.72 on 2025-08-24 22:22:24 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <type_traits>
...
std::vector<double> eval_rec(const std::vector<std::string>& tokens, int64_t pos);
...
bool is_operand(std::string token) {
    ...
    int64_t i = int64_t(0);
    while ((i < token.size())) {
        std::string ch = ([&]{ std::string __s = token; long long __start = i; long long __end = (i + int64_t(1)); if(__start < 0) __start = 0; if(__end < __start) __end = __start; if(__start > static_cast<long long>(__s.size())) __start = __s.size(); if(__end > static_cast<long long>(__s.size())) __end = __s.size(); return __s.substr(static_cast<size_t>(__start), static_cast<size_t>(__end - __start)); })();
        if (!is_digit(ch)) {
            return false;
        }
        i = (i + int64_t(1));
...
double evaluate(std::string expression) {
    ...
    while ((i >= int64_t(0))) {
        std::string token = _index(tokens, i);
        if ((token != std::string(""))) {
            if (is_operand(token)) {
                stack = ([&]{ auto __tmp = stack; __tmp.push_back((double)(to_int(token))); return __tmp; }());
            } else {
                double o1 = _index(stack, (stack.size() - int64_t(1)));
                double o2 = _index(stack, (stack.size() - int64_t(2)));
...
std::vector<double> eval_rec(const std::vector<std::string>& tokens, int64_t pos) {
    std::string token = _index(tokens, pos);
    ...
    double a = _index(left, int64_t(0));
    int64_t p1 = (int64_t)(_index(left, int64_t(1)));
    std::vector<double> right = eval_rec(tokens, p1);
    double b = _index(right, int64_t(0));
    double p2 = _index(right, int64_t(1));
...
int main() {
    {
        ...
        std::cout << _to_string(evaluate(test_expression));
        std::cout << std::endl;
        std::cout << _to_string(evaluate(test_expression2));
        std::cout << std::endl;
        std::cout << _to_string(evaluate_recursive(test_expression3));
        std::cout << std::endl;
    }
    return 0;
}
