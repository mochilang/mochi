// Generated by Mochi transpiler v0.10.59 on 2025-08-07 10:12:53 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<Vec> INITIAL_VECTORS = std::vector<Vec>{VECTOR_1, VECTOR_2, VECTOR_3, VECTOR_1};
std::vector<Vec> example = iterate(std::vector<Vec>{VECTOR_1, VECTOR_3}, int64_t(1));
...
std::vector<Vec> iteration_step(const std::vector<Vec>& vectors) {
    ...
    new_vectors = ([&]{ auto __tmp = new_vectors; __tmp.push_back(vectors[static_cast<size_t>((vectors.size() - int64_t(1)))]); return __tmp; }());
    return new_vectors;
}
...
std::string vec_to_string(const Vec& v) {
    return ((((std::string("(") + ([&]{ std::ostringstream ss; ss << std::boolalpha << v.x; return ss.str(); }())) + std::string(", ")) + ([&]{ std::ostringstream ss; ss << std::boolalpha << v.y; return ss.str(); }())) + std::string(")"));
}
