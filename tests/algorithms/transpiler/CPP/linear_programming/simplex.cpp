// Generated by Mochi transpiler v0.10.66 on 2025-08-14 17:49:44 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <map>
...
std::vector<int64_t> findPivot(const std::vector<std::vector<double>>& t) {
    ...
    double minVal = 0.0;
    for (int64_t j = int64_t(0); j < (_index(t, int64_t(0)).size() - int64_t(1)); j++ ) {
    ...
    double minRatio = 0.0;
    bool first = true;
    for (int64_t i = int64_t(1); i < t.size(); i++ ) {
        double coeff = _index(_index(t, i), col);
        if ((coeff > 0.0)) {
            double rhs = _index(_index(t, i), (_index(t, i).size() - int64_t(1)));
...
std::map<std::string, double> interpret(const std::vector<std::vector<double>>& t, int64_t nVars) {
    ...
    for (int64_t i = int64_t(0); i < nVars; i++ ) {
        int64_t nzRow = -int64_t(1);
        int64_t nzCount = int64_t(0);
        for (int64_t r = int64_t(0); r < t.size(); r++ ) {
...
int main() {
    {
        ...
        std::cout << (std::string("P: ") + _to_string(([&](const auto& __m){ auto __it = __m.find(std::string("P")); if (__it == __m.end()) return double{}; return __it->second; })(res)));
        std::cout << std::endl;
        for (int64_t i = int64_t(0); i < int64_t(2); i++ ) {
            ...
            if (([&](const auto& __c, const auto& __v){ if constexpr(std::is_same_v<std::decay_t<decltype(__c)>, std::string>) { return __c.find(__v) != std::string::npos; } else if constexpr(requires { __c.find(__v); }) { return __c.find(__v) != __c.end(); } else { return std::find(__c.begin(), __c.end(), __v) != __c.end(); } })(res, key)) {
                std::cout << ((key + std::string(": ")) + _to_string(([&](const auto& __m){ auto __it = __m.find(key); if (__it == __m.end()) return double{}; return __it->second; })(res)));
                std::cout << std::endl;
            }
        }
    }
