// Generated by Mochi transpiler v0.10.59 on 2025-08-07 09:03:41 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<std::vector<int64_t>> gaussian_filter(std::vector<std::vector<int64_t>>& image, int64_t k_size, double sigma);
void print_image(std::vector<std::vector<int64_t>>& image);
int main();
    ...
    while ((i < dst_height)) {
        ...
        while ((j < dst_width)) {
            ...
            while ((ki < k_size)) {
                ...
                while ((kj < k_size)) {
                    sum = (sum + ((double)(image[(i + ki)][(j + kj)]) * kernel[ki][kj]));
                    kj = (kj + int64_t(1));
                }
                ki = (ki + int64_t(1));
            }
