// Generated by Mochi transpiler v0.10.60 on 2025-08-07 23:29:13 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<std::vector<int64_t>> gaussian3 = gaussian_filter(img, int64_t(3), 1.0);
std::vector<std::vector<int64_t>> gaussian5 = gaussian_filter(img, int64_t(5), 0.8);
...
std::vector<std::vector<double>> gen_gaussian_kernel(int64_t k_size, double sigma) {
    ...
    while ((i < k_size)) {
        ...
        while ((j < k_size)) {
            ...
            double exponent = -((double)(((x * x) + (y * y))) / (((2.0 * sigma) * sigma)));
            double value = (((double)(1.0) / (((2.0 * PI) * sigma))) * expApprox(exponent));
...
std::vector<std::vector<int64_t>> gaussian_filter(const std::vector<std::vector<int64_t>>& image, int64_t k_size, double sigma) {
    ...
    int dst_height = ((height - k_size) + int64_t(1));
    int dst_width = ((width - k_size) + int64_t(1));
    ...
    while ((i < dst_height)) {
        ...
        while ((j < dst_width)) {
            ...
            while ((ki < k_size)) {
                ...
                while ((kj < k_size)) {
                    sum = (sum + ((double)(([&](const auto& __v){ auto __i = (j + kj); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(([&](const auto& __v){ auto __i = (i + ki); if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(image))) * ([&](const auto& __v){ auto __i = kj; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(([&](const auto& __v){ auto __i = ki; if (__i < 0) __i += __v.size(); return __v[static_cast<size_t>(__i)]; })(kernel))));
                    kj = (kj + int64_t(1));
                }
                ki = (ki + int64_t(1));
            }
