// Generated by Mochi transpiler v0.10.59 on 2025-08-07 09:06:05 GMT+7
#include <iostream>
#include <string>
#include <vector>
#include <sstream>
...
std::vector<std::vector<double>> rgb_to_gray(const std::vector<std::vector<std::vector<int64_t>>>& rgb) {
    ...
    while ((i < rgb.size())) {
        ...
        int64_t j = int64_t(0);
        while ((j < rgb[i].size())) {
            int64_t r = rgb[i][j][int64_t(0)];
            int64_t g = rgb[i][j][int64_t(1)];
            int64_t b = rgb[i][j][int64_t(2)];
...
std::vector<std::vector<int64_t>> dilation(const std::vector<std::vector<int64_t>>& image, const std::vector<std::vector<int64_t>>& kernel) {
    ...
    int img_w = image[int64_t(0)].size();
    int k_h = kernel.size();
    ...
    while ((i < img_h)) {
        int64_t j = int64_t(0);
        while ((j < img_w)) {
            padded[static_cast<size_t>((pad_h + i))][static_cast<size_t>((pad_w + j))] = image[i][j];
            j = (j + int64_t(1));
        }
        i = (i + int64_t(1));
    }
