...
List<String> alphanum_key(String s) {
  ...
  int i = 0;
  while (i < s.length) {
    if (is_digit(_substr(s, i, i + 1))) {
    String _num = "";
    while (i < s.length && is_digit(_substr(s, i, i + 1))) {
    _num = _num + _substr(s, i, i + 1);
    i = i + 1;
  };
    ...
    seg = seg + to_lower(_substr(s, i, i + 1));
    i = i + 1;
  };
    key = [...key, seg];
  }
...
List<String> natural_sort(List<String> arr) {
  ...
  while (k < arr.length) {
    res = [...res, arr[k]];
    keys = ([...keys, alphanum_key(arr[k])] as List<dynamic>).map((e) => (List<String>.from(e) as List<String>)).toList();
    k = k + 1;
  }
  int i = 1;
  while (i < res.length) {
