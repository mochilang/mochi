...
void dg_add_pair(DirectedGraph g, int u, int v, int w) {
  if (g.graph.containsKey(u)) {
    ...
    edges = ([...edges, [w, v]] as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
  ...
  if (!g.graph.containsKey(v)) {
    ...
    m1[v] = ([] as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
    g.graph = m1;
  }
}
...
void dg_remove_pair(DirectedGraph g, int u, int v) {
  if (g.graph.containsKey(u)) {
    ...
    while (i < edges.length) {
    if (edges[i][1] != v) {
    new_edges = ([...new_edges, edges[i]] as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
  }
    i = i + 1;
  };
    Map<int, List<List<int>>> m = g.graph;
...
void g_add_pair(Graph g, int u, int v, int w) {
  ...
  if (g.graph.containsKey(v)) {
    ...
    edges2 = ([...edges2, [w, u]] as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
...
void g_remove_pair(Graph g, int u, int v) {
  ...
  if (g.graph.containsKey(v)) {
    ...
    if (edges2[j][1] != u) {
    new_edges2 = ([...new_edges2, edges2[j]] as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
  }
    j = j + 1;
  };
    Map<int, List<List<int>>> m2 = g.graph;
...
void _main() {
  ...
  print((dg_dfs(dg, -2, -1)).toString());
  print((dg_bfs(dg, -2)).toString());
  print((dg_in_degree(dg, 3)).toString());
  print((dg_out_degree(dg, 0)).toString());
  print((dg_topological_sort(dg)).toString());
  print((dg_has_cycle(dg)).toString());
  Graph ug = g_make_graph();
  g_add_pair(ug, 0, 1, 1);
  g_add_pair(ug, 1, 2, 1);
  g_add_pair(ug, 2, 0, 1);
  print((g_dfs(ug, -2, -1)).toString());
  print((g_bfs(ug, -2)).toString());
  print((g_degree(ug, 1)).toString());
  print((g_has_cycle(ug)).toString());
}
