...
dynamic dfs(Map<int, List<int>> g, int s) {
  ...
  while (stack.length > 0) {
    ...
    if (!found) {
    stack = (stack.sublist(0, stack.length - 1) as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList();
  }
  }
}
...
dynamic bfs(Map<int, List<int>> g, int s) {
  ...
  while (q.length > 0) {
    int u = q[0];
    q = (q.sublist(1, q.length) as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList();
    for (var v in g[u]) {
    if (!visited.containsKey(v)) {
    visited[(v).toInt()] = true;
    q = [...q, v];
...
dynamic dijkstra(Map<int, List<List<int>>> g, int s) {
  ...
  while (known.length < keys.length) {
    ...
    num alt = dist[u]! + w;
    dynamic cur = (dist.containsKey(v) ? dist[(v).toInt()] : 100000);
    if (!known.contains(v) && alt.compareTo(cur) < 0) {
    dist[(v).toInt()] = (alt).toInt();
    path[(v).toInt()] = u;
  }
  }
  }
...
dynamic floyd(List<List<int>> a) {
  ...
  while (i < n) {
    ...
    dist = ([...dist, row] as List).map((e) => (e as List<int>)).toList();
    i = i + 1;
  }
  int k = 0;
  while (k < n) {
...
int prim(Map<int, List<List<int>>> g, int s, int n) {
  ...
  while (known.length < n) {
    ...
    dynamic cur = (dist.containsKey(v) ? dist[(v).toInt()] : 100000);
    if (!known.contains(v) && w.compareTo(cur) < 0) {
    dist[(v).toInt()] = w;
  }
  }
  }
