...
List<List<List<double>>> gradient(List<List<int>> img) {
  ...
  while (y < h - 1) {
    int x = 1;
    while (x < w - 1) {
    while (dx[y]!.length <= x) { dx[y]!.add(0); } dx[y]![x] = (img[y][x + 1] as double) - (img[y][x - 1] as double);
    while (dy[y]!.length <= x) { dy[y]!.add(0); } dy[y]![x] = (img[y + 1][x] as double) - (img[y - 1][x] as double);
    x = x + 1;
  }
    y = y + 1;
  }
