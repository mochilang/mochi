...
List<List<int>> convert_to_negative(List<List<int>> img) {
  ...
  while (y < h) {
    ...
    int x = 0;
    while (x < w) {
    row = (row..add(255 - img[y][x]));
    x = x + 1;
  }
    out = ((out..add(row)) as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
    y = y + 1;
  }
  return out;
}
...
List<List<double>> gen_gaussian_kernel(int n, double sigma) {
  ...
  while (i < n) {
    ...
    k = ((k..add(row)) as List).map((e) => (List<double>.from(e) as List<double>)).toList();
    i = i + 1;
  }
  return k;
}
...
List<List<int>> median_filter(List<List<int>> img, int k) {
  ...
  while (y < h) {
    ...
    row = (row..add(sorted[sorted.length ~/ 2]));
    x = x + 1;
  }
    out = ((out..add(row)) as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
    y = y + 1;
...
List<List<int>> local_binary_pattern(List<List<int>> img) {
  ...
  while (y < h) {
    ...
    int x = 0;
    while (x < w) {
    row = (row..add(local_binary_value(img, x, y)));
    x = x + 1;
  }
    out = ((out..add(row)) as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
    y = y + 1;
