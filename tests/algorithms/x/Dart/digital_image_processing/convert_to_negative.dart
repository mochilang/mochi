// Generated by Mochi transpiler
String _substr(String s, num start, num end) {
  var n = s.length;
  int s0 = start.toInt();
  int e0 = end.toInt();
  if (s0 < 0) s0 += n;
  if (e0 < 0) e0 += n;
  if (s0 < 0) s0 = 0;
  if (s0 > n) s0 = n;
  if (e0 < 0) e0 = 0;
  if (e0 > n) e0 = n;
  if (s0 > e0) s0 = e0;
  return s.substring(s0, e0);
}

List<List<List<int>>> convert_to_negative(List<List<List<int>>> img) {
  List<List<List<int>>> result = <List<List<int>>>[];
  int i = 0;
  while (i < img.length) {
    List<List<int>> row = <List<int>>[];
    int j = 0;
    while (j < img[i].length) {
    List<int> pixel = img[i][j];
    int r = 255 - pixel[0];
    int g = 255 - pixel[1];
    int b = 255 - pixel[2];
    row = ((row..add([r, g, b])) as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
    j = j + 1;
  }
    result = ((result..add(row)) as List).map((e) => ((e as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList() as List<List<int>>)).toList();
    i = i + 1;
  }
  return result;
}

void _main() {
  List<List<List<int>>> image = [[[10, 20, 30], [0, 0, 0]], [[255, 255, 255], [100, 150, 200]]];
  List<List<List<int>>> neg = convert_to_negative(image);
  print(neg);
}

void _start() {
  _main();
}

void main() => _start();
