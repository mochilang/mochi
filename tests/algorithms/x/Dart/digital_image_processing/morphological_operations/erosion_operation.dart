...
List<List<double>> rgb_to_gray(List<List<List<int>>> rgb) {
  ...
  while (i < rgb.length) {
    ...
    gray = ((gray..add(row)) as List).map((e) => (List<double>.from(e) as List<double>)).toList();
    i = i + 1;
  }
  return gray;
}
...
List<List<bool>> gray_to_binary(List<List<double>> gray) {
  ...
  while (i < gray.length) {
    ...
    row = (row..add(gray[i][j] > 127.0 && gray[i][j] <= 255.0));
    j = j + 1;
  }
    binary = ((binary..add(row)) as List).map((e) => (List<bool>.from(e) as List<bool>)).toList();
    i = i + 1;
  }
  return binary;
}
...
List<List<bool>> erosion(List<List<bool>> image, List<List<int>> kernel) {
  ...
  while (y < h + 2 * pad_y) {
    ...
    padded = ((padded..add(row)) as List).map((e) => (List<bool>.from(e) as List<bool>)).toList();
    y = y + 1;
  }
  y = 0;
  while (y < h) {
  ...
  while (y < h) {
    ...
    row_out = (row_out..add(sum == 5));
    x = x + 1;
  }
    output = ((output..add(row_out)) as List).map((e) => (List<bool>.from(e) as List<bool>)).toList();
    y = y + 1;
  }
  return output;
}
