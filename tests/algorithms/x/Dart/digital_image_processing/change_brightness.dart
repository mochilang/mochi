...
List<List<int>> change_brightness(List<List<int>> img, int level) {
  if (level < -255 || level > 255) {
    throw Exception("level must be between -255 and 255");
  }
  ...
  while (i < img.length) {
    ...
    row_res = (row_res..add(clamp(img[i][j] + level)));
    j = j + 1;
  }
    result = ((result..add(row_res)) as List).map((e) => ((e as List).map((e) => (e is BigInt ? e.toInt() : (e as int))).toList() as List<int>)).toList();
    i = i + 1;
  }
  return result;
}
