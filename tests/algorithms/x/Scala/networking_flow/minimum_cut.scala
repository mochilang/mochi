// Generated by Mochi v0.10.61 on 2025-08-08 19:12:45 GMT+7
...
def bfs(graph: ArrayBuffer[ArrayBuffer[BigInt]], s: BigInt, t: BigInt, parent: ArrayBuffer[BigInt]): Boolean = {
  ...
  while (head < BigInt((queue).size)) {
    val u: BigInt = queue.lift((head.toInt).toInt).getOrElse(null)
    head = head + BigInt(1)
    var ind: BigInt = BigInt(0)
    while (ind < BigInt((graph.lift((u.toInt).toInt).getOrElse(ArrayBuffer[BigInt]())).size)) {
      if ((visited.lift((ind.toInt).toInt).getOrElse(false) == false && graph.lift((u.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((ind.toInt).toInt).getOrElse(null) > BigInt(0)).asInstanceOf[Boolean]) {
        queue = queue :+ (ind)
...
def mincut(graph: ArrayBuffer[ArrayBuffer[BigInt]], source: BigInt, sink: BigInt): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < BigInt((g).size)) {
    ...
    while (j < BigInt((g.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]())).size)) {
      row = row :+ (g.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((j.toInt).toInt).getOrElse(null))
      j = j + BigInt(1)
    }
    temp = temp :+ (row)
    i = i + BigInt(1)
  ...
  while (bfs(g, source, sink, parent)) {
    ...
    var s: BigInt = sink
    while (s != source) {
      val p: BigInt = parent.lift((s.toInt).toInt).getOrElse(null)
      val cap: BigInt = g.lift((p.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((s.toInt).toInt).getOrElse(null)
      if (cap < path_flow) {
        path_flow = cap
      }
      s = p
    ...
    var v: BigInt = sink
    while (v != source) {
      val u: BigInt = parent.lift((v.toInt).toInt).getOrElse(null)
      g((u).toInt)((v).toInt) = g.lift((u.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((v.toInt).toInt).getOrElse(null) - path_flow
      g((v).toInt)((u).toInt) = g.lift((v.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((u.toInt).toInt).getOrElse(null) + path_flow
      v = u
    }
  }
  ...
  while (i < BigInt((g).size)) {
    ...
    while (j < BigInt((g.lift((BigInt(0).toInt).toInt).getOrElse(ArrayBuffer[BigInt]())).size)) {
      if ((g.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((j.toInt).toInt).getOrElse(null) == BigInt(0) && temp.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((j.toInt).toInt).getOrElse(null) > BigInt(0)).asInstanceOf[Boolean]) {
