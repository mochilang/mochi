// Generated by Mochi v0.10.61 on 2025-08-08 19:12:33 GMT+7
...
def breadth_first_search(graph: ArrayBuffer[ArrayBuffer[BigInt]], source: BigInt, sink: BigInt, parent: ArrayBuffer[BigInt]): Boolean = {
  ...
  while (head < BigInt((queue).size)) {
    ...
    while (ind < BigInt((row).size)) {
      val capacity: BigInt = row.lift((ind.toInt).toInt).getOrElse(null)
      if ((visited.lift((ind.toInt).toInt).getOrElse(false) == false && capacity > BigInt(0)).asInstanceOf[Boolean]) {
        queue = queue :+ (ind)
...
def ford_fulkerson(graph: ArrayBuffer[ArrayBuffer[BigInt]], source: BigInt, sink: BigInt): BigInt = {
  ...
  while (breadth_first_search(graph, source, sink, parent)) {
    ...
    var s: BigInt = sink
    while (s != source) {
      val prev: BigInt = parent.lift((s.toInt).toInt).getOrElse(null)
      val cap: BigInt = graph.lift((prev.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((s.toInt).toInt).getOrElse(null)
      if (cap < path_flow) {
        path_flow = cap
      }
      s = prev
    ...
    while (v != source) {
      ...
      graph((u).toInt)((v).toInt) = graph.lift((u.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((v.toInt).toInt).getOrElse(null) - path_flow
      graph((v).toInt)((u).toInt) = graph.lift((v.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((u.toInt).toInt).getOrElse(null) + path_flow
      v = u
    }
