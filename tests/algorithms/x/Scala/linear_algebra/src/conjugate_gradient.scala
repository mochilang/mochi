// Generated by Mochi v0.10.59 on 2025-08-07 12:40:56 GMT+7
...
def mat_vec_mul(m: ArrayBuffer[ArrayBuffer[Double]], v: ArrayBuffer[Double]): ArrayBuffer[Double] = {
  ...
  while (i < BigInt((m).size)) {
    ...
    while (j < BigInt((m((i.toInt).toInt)).size)) {
      s = s + m((i.toInt).toInt)((j.toInt).toInt) * v((j.toInt).toInt)
      j = j + BigInt(1)
    }
    res = res :+ s
    i = i + BigInt(1)
...
def main(args: Array[String]): Unit = {
  {
    ...
    val _start = _now()
    println(String.valueOf(x((BigInt(0).toInt).toInt)))
    println(String.valueOf(x((BigInt(1).toInt).toInt)))
    println(String.valueOf(x((BigInt(2).toInt).toInt)))
    val _end = _now()
    System.gc()
