// Generated by Mochi v0.10.59 on 2025-08-07 08:48:48 GMT+7
...
def viterbi(observations: ArrayBuffer[String], states: ArrayBuffer[String], start_p: scala.collection.mutable.Map[String,Double], trans_p: scala.collection.mutable.Map[String,scala.collection.mutable.Map[String,Double]], emit_p: scala.collection.mutable.Map[String,scala.collection.mutable.Map[String,Double]]): ArrayBuffer[String] = {
  if ((BigInt((observations).size) == 0 || BigInt((states).size) == 0).asInstanceOf[Boolean]) {
  ...
  val first_obs: String = observations((0.toInt).toInt)
  var i: BigInt = 0
  while (i < BigInt((states).size)) {
    val state: String = states((i.toInt).toInt)
  ...
  while (t < BigInt((observations).size)) {
    val obs: String = observations((t.toInt).toInt)
    var j: BigInt = 0
    while (j < BigInt((states).size)) {
      val state: String = states((j.toInt).toInt)
      var max_prob: Double = -1.0
      var prev_state: String = ""
      var k: BigInt = 0
      while (k < BigInt((states).size)) {
        val state0: String = states((k.toInt).toInt)
        val obs0: String = observations(((t - 1).toInt).toInt)
  ...
  var path: ArrayBuffer[String] = ArrayBuffer()
  var n: BigInt = 0
  while (n < BigInt((observations).size)) {
    path = path :+ ""
    n = n + 1
  }
  val last_obs: String = observations(((BigInt((observations).size) - 1).toInt).toInt)
  var max_final: Double = -1.0
  var last_state: String = ""
  var m: BigInt = 0
  while (m < BigInt((states).size)) {
    val state: String = states((m.toInt).toInt)
  ...
  val last_index: BigInt = BigInt((observations).size) - 1
  path((last_index).toInt) = last_state
  var idx: BigInt = last_index
  while (idx > 0) {
    val obs: String = observations((idx.toInt).toInt)
    val prev: String = ptrs.getOrElse(key(path((idx.toInt).toInt), obs).toString, "").asInstanceOf[String]
    path((idx - 1).toInt) = prev
    idx = idx - 1
  }
  return path
}
