// Generated by Mochi v0.10.72 on 2025-08-24 22:24:46 GMT+7
...
def get_head_data(ll: LinkedList): BigInt = {
  ...
  var node: Node = ll.asInstanceOf[LinkedList].nodes.lift((ll.asInstanceOf[LinkedList].head_idx.toInt).toInt).getOrElse(null)
...
def get_tail_data(ll: LinkedList): BigInt = {
  ...
  var node: Node = ll.asInstanceOf[LinkedList].nodes.lift((ll.asInstanceOf[LinkedList].tail_idx.toInt).toInt).getOrElse(null)
  return node.asInstanceOf[Node].data
}
...
def insert_before_node(ll: LinkedList, idx: BigInt, new_idx: BigInt): Any = {
  ...
  var p: BigInt = node.asInstanceOf[Node].prev_index
...
def insert_after_node(ll: LinkedList, idx: BigInt, new_idx: BigInt): Any = {
  ...
  var node: Node = nodes.lift((idx.toInt).toInt).getOrElse(null)
  var nxt: BigInt = node.asInstanceOf[Node].next_index
...
def insert(ll: LinkedList, value: BigInt): Any = {
  ...
  var idx: BigInt = BigInt((nodes).size) - BigInt(1)
  ll.asInstanceOf[LinkedList].nodes = nodes
  if (ll.asInstanceOf[LinkedList].head_idx == BigInt(-1)) {
    ll.asInstanceOf[LinkedList].head_idx = idx
    ll.asInstanceOf[LinkedList].tail_idx = idx
...
def insert_at_position(ll: LinkedList, position: BigInt, value: BigInt): Any = {
  ...
  while (current != BigInt(-1)) {
    if (current_pos == position) {
      var nodes: ArrayBuffer[Node] = ll.asInstanceOf[LinkedList].nodes
      nodes = ({nodes += (Node(value, BigInt(-1), BigInt(-1))); nodes})
      var new_idx: BigInt = BigInt((nodes).size) - BigInt(1)
      ll.asInstanceOf[LinkedList].nodes = nodes
      insert_before_node(ll, current, new_idx)
      return
    }
    var node: Node = ll.asInstanceOf[LinkedList].nodes.lift((current.toInt).toInt).getOrElse(null)
...
def delete_value(ll: LinkedList, value: BigInt): Any = {
  var idx: BigInt = get_node(ll, value)
  if (idx == BigInt(-1)) {
    return
  }
  if (idx == ll.asInstanceOf[LinkedList].head_idx) {
    var node: Node = ll.asInstanceOf[LinkedList].nodes.lift((idx.toInt).toInt).getOrElse(null)
...
def to_string(ll: LinkedList): String = {
  ...
  while (current != BigInt(-1)) {
    var node: Node = ll.asInstanceOf[LinkedList].nodes.lift((current.toInt).toInt).getOrElse(null)
    var `val`: String = _str(node.asInstanceOf[Node].data)
    if (first) {
      res = `val`
      first = false
    } else {
