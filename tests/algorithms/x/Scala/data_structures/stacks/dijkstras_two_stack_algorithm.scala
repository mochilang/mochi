// Generated by Mochi v0.10.72 on 2025-08-24 22:27:23 GMT+7
...
def dijkstras_two_stack_algorithm(equation: String): BigInt = {
  ...
  while (idx < BigInt((equation).length)) {
    var ch: String = equation.slice((idx).toInt, (idx + BigInt(1)).toInt)
    if (is_digit(ch)) {
      ...
      if ((((ch == "+" || ch == "-").asInstanceOf[Boolean] || ch == "*").asInstanceOf[Boolean] || ch == "/")) {
        ...
        if (ch == ")") {
          var opr: String = operator_stack.lift(((BigInt((operator_stack).size) - BigInt(1)).toInt).toInt).getOrElse("")
          operator_stack = slice_without_last_string(operator_stack)
          var num1: BigInt = operand_stack.lift(((BigInt((operand_stack).size) - BigInt(1)).toInt).toInt).getOrElse(BigInt(0))
          operand_stack = slice_without_last_int(operand_stack)
          var num2: BigInt = operand_stack.lift(((BigInt((operand_stack).size) - BigInt(1)).toInt).toInt).getOrElse(BigInt(0))
          operand_stack = slice_without_last_int(operand_stack)
          var total: BigInt = (if (opr == "+") num2 + num1 else (if (opr == "-") num2 - num1 else (if (opr == "*") num2 * num1 else num2 / num1)))
