// Generated by Mochi v0.10.72 on 2025-08-24 22:27:38 GMT+7
...
def infix_to_postfix(infix: String): String = {
  ...
  while (i < BigInt((infix).size)) {
    var x: String = infix.slice((i.toInt).toInt, (i.toInt).toInt + 1)
...
def infix_to_prefix(infix: String): String = {
  ...
  while (i >= BigInt(0)) {
    var ch: String = infix.slice((i.toInt).toInt, (i.toInt).toInt + 1)
    if (ch == "(") {
      reversed = reversed + ")"
    } else {
      if (ch == ")") {
...
var PRIORITY: scala.collection.mutable.Map[String,BigInt] = scala.collection.mutable.Map("^" -> (BigInt(3)), "*" -> (BigInt(2)), "/" -> (BigInt(2)), "%" -> (BigInt(2)), "+" -> (BigInt(1)), "-" -> (BigInt(1)))
