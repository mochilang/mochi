// Generated by Mochi v0.10.59 on 2025-08-07 11:24:10 GMT+7
...
def column(m: ArrayBuffer[ArrayBuffer[Double]], idx: BigInt): ArrayBuffer[Double] = {
  ...
  while (i < BigInt((m).size)) {
    col = col :+ m((i.toInt).toInt)((idx.toInt).toInt)
    i = i + BigInt(1)
  }
  return col
}
...
def validate_adjacency_list(graph: ArrayBuffer[ArrayBuffer[BigInt]]): Any = {
  ...
  while (i < BigInt((graph).size)) {
    var j: BigInt = BigInt(0)
    while (j < BigInt((graph((i.toInt).toInt)).size)) {
      val v: BigInt = graph((i.toInt).toInt)((j.toInt).toInt)
      if ((v < BigInt(0) || v >= BigInt((graph).size)).asInstanceOf[Boolean]) {
...
def multiply_matrix_vector(graph: ArrayBuffer[ArrayBuffer[BigInt]], vector: ArrayBuffer[Double]): ArrayBuffer[Double] = {
  ...
  while (i < n) {
    ...
    var j: BigInt = BigInt(0)
    while (j < BigInt((graph((i.toInt).toInt)).size)) {
      val nb: BigInt = graph((i.toInt).toInt)((j.toInt).toInt)
      sum = sum + vector((nb.toInt).toInt)
      j = j + BigInt(1)
    }
    result = result :+ sum
    i = i + BigInt(1)
...
def lanczos_iteration(graph: ArrayBuffer[ArrayBuffer[BigInt]], k: BigInt): LanczosResult = {
  val n: BigInt = BigInt((graph).size)
  if ((k < BigInt(1) || k > n).asInstanceOf[Boolean]) {
  ...
  i = BigInt(0)
  while (i < n) {
    ss = ss + v((i.toInt).toInt) * v((i.toInt).toInt)
    i = i + BigInt(1)
  }
  val vnorm: Double = sqrtApprox(ss)
  i = BigInt(0)
  while (i < n) {
    q((i).toInt)((BigInt(0)).toInt) = v((i.toInt).toInt) / vnorm
    i = i + BigInt(1)
  }
  var beta: Double = 0.0
  var j: BigInt = BigInt(0)
  while (j < k) {
    ...
    val alpha: Double = dot(column(q, j), w)
    w = vector_sub(w, vector_scale(column(q, j), alpha))
    var ss2: Double = 0.0
    var p: BigInt = BigInt(0)
    while (p < n) {
      ss2 = ss2 + w((p.toInt).toInt) * w((p.toInt).toInt)
      p = p + BigInt(1)
    }
    beta = sqrtApprox(ss2)
    ...
    if (j < k - BigInt(1)) {
      ...
      if (beta > 0.0000000001) {
        ...
        while (r < n) {
          q((r).toInt)((j + BigInt(1)).toInt) = wnorm((r.toInt).toInt)
          r = r + BigInt(1)
        }
      }
    }
...
def jacobi_eigen(a_in: ArrayBuffer[ArrayBuffer[Double]], max_iter: BigInt): EigenResult = {
  ...
  _br19.breakable {
    while (iter < max_iter) {
      ...
      var max: Double = absf(a((p.toInt).toInt)((q.toInt).toInt))
      i = BigInt(0)
      while (i < n) {
        var j: BigInt = i + BigInt(1)
        while (j < n) {
          val `val`: Double = absf(a((i.toInt).toInt)((j.toInt).toInt))
          if (`val` > max) {
            max = `val`
            p = i
            q = j
      ...
      val app: Double = a((p.toInt).toInt)((p.toInt).toInt)
      val aqq: Double = a((q.toInt).toInt)((q.toInt).toInt)
      val apq: Double = a((p.toInt).toInt)((q.toInt).toInt)
      val theta: Double = (aqq - app) / (2.0 * apq)
      var t: Double = 1.0 / (absf(theta) + sqrtApprox(theta * theta + 1.0))
      if (theta < 0.0) {
        t = -t
      }
      val c: Double = 1.0 / sqrtApprox(1.0 + t * t)
      val s: Double = t * c
      val tau: Double = s / (1.0 + c)
      ...
      while (k < n) {
        if ((k != p && k != q).asInstanceOf[Boolean]) {
          val akp: Double = a((k.toInt).toInt)((p.toInt).toInt)
          val akq: Double = a((k.toInt).toInt)((q.toInt).toInt)
          a((k).toInt)((p).toInt) = akp - s * (akq + tau * akp)
          a((p).toInt)((k).toInt) = a((k.toInt).toInt)((p.toInt).toInt)
          a((k).toInt)((q).toInt) = akq + s * (akp - tau * akq)
          a((q).toInt)((k).toInt) = a((k.toInt).toInt)((q.toInt).toInt)
        }
        k = k + BigInt(1)
      }
      k = BigInt(0)
      while (k < n) {
        val vkp: Double = v((k.toInt).toInt)((p.toInt).toInt)
        val vkq: Double = v((k.toInt).toInt)((q.toInt).toInt)
  ...
  i = BigInt(0)
  while (i < n) {
    eigenvalues = eigenvalues :+ a((i.toInt).toInt)((i.toInt).toInt)
    i = i + BigInt(1)
  }
...
def sort_eigenpairs(vals: ArrayBuffer[Double], vecs: ArrayBuffer[ArrayBuffer[Double]]): EigenResult = {
  ...
  while (i < n) {
    var j: BigInt = BigInt(0)
    while (j < n - BigInt(1)) {
      if (values((j.toInt).toInt) < values(((j + BigInt(1)).toInt).toInt)) {
        val tmp: Double = values((j.toInt).toInt)
        values((j).toInt) = values(((j + BigInt(1)).toInt).toInt)
        values((j + BigInt(1)).toInt) = tmp
        var r: BigInt = BigInt(0)
        while (r < BigInt((vectors).size)) {
          val tv: Double = vectors((r.toInt).toInt)((j.toInt).toInt)
          vectors((r).toInt)((j).toInt) = vectors((r.toInt).toInt)(((j + BigInt(1)).toInt).toInt)
...
def find_lanczos_eigenvectors(graph: ArrayBuffer[ArrayBuffer[BigInt]], k: BigInt): EigenResult = {
  ...
  val res: LanczosResult = lanczos_iteration(graph, k)
  val eig: EigenResult = jacobi_eigen(res.asInstanceOf[LanczosResult].t, BigInt(50))
  val sorted: EigenResult = sort_eigenpairs(eig.asInstanceOf[EigenResult].values, eig.asInstanceOf[EigenResult].vectors)
  val final_vectors: ArrayBuffer[ArrayBuffer[Double]] = matmul(res.asInstanceOf[LanczosResult].q, sorted.asInstanceOf[EigenResult].vectors)
...
val graph: ArrayBuffer[ArrayBuffer[BigInt]] = ArrayBuffer(ArrayBuffer(BigInt(1), BigInt(2)), ArrayBuffer(BigInt(0), BigInt(2)), ArrayBuffer(BigInt(0), BigInt(1)))

val result: EigenResult = find_lanczos_eigenvectors(graph, BigInt(2))
