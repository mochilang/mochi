// Generated by Mochi v0.10.59 on 2025-08-07 11:24:06 GMT+7
...
def partition_graph(graph: scala.collection.mutable.Map[String,ArrayBuffer[String]]): ArrayBuffer[Pair] = {
  ...
  for (node <- graph.keys.toSeq.sorted) {
    ...
    while (i < BigInt((neigh).size)) {
      lst = lst :+ neigh((i.toInt).toInt)
      i = i + BigInt(1)
    }
    graph_copy.update(node, lst)
  }
  var nodes: ArrayBuffer[String] = graph_copy.keys.toSeq.sorted
  while (BigInt((nodes).size) > BigInt(2)) {
    val u: Any = nodes(((rand_int(BigInt((nodes).size))).toInt).toInt)
    val u_neighbors: ArrayBuffer[String] = graph_copy.getOrElse(u.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]
    val v: String = u_neighbors(((rand_int(BigInt((u_neighbors).size))).toInt).toInt)
    val uv: String = u + v
    ...
    while (i < BigInt((graph_copy.getOrElse(u.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]).size)) {
      val n: String = graph_copy.getOrElse(u.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]((i.toInt).toInt)
      if (((n != u && n != v).asInstanceOf[Boolean] && uv_neighbors.contains(n) == false).asInstanceOf[Boolean]) {
    ...
    while (i < BigInt((graph_copy.getOrElse(v.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]).size)) {
      val n: String = graph_copy.getOrElse(v.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]((i.toInt).toInt)
      if (((n != u && n != v).asInstanceOf[Boolean] && uv_neighbors.contains(n) == false).asInstanceOf[Boolean]) {
        uv_neighbors = uv_neighbors :+ n
      }
      i = i + BigInt(1)
    ...
    while (k < BigInt((uv_neighbors).size)) {
      val nb: String = uv_neighbors((k.toInt).toInt)
    ...
    while (i < BigInt((contracted.getOrElse(u.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]).size)) {
      group = group :+ contracted.getOrElse(u.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]((i.toInt).toInt)
      i = i + BigInt(1)
    }
    i = BigInt(0)
    while (i < BigInt((contracted.getOrElse(v.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]).size)) {
      val `val`: String = contracted.getOrElse(v.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]((i.toInt).toInt)
  ...
  while (j < BigInt((nodes).size)) {
    val n: Any = nodes((j.toInt).toInt)
    groups = groups :+ contracted.getOrElse(n.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]
    j = j + BigInt(1)
  }
  val groupA: ArrayBuffer[String] = groups((BigInt(0).toInt).toInt)
  val groupB: ArrayBuffer[String] = groups((BigInt(1).toInt).toInt)
  var cut: ArrayBuffer[Pair] = ArrayBuffer[Pair]()
  j = BigInt(0)
  while (j < BigInt((groupA).size)) {
    val node: String = groupA((j.toInt).toInt)
    var neigh: ArrayBuffer[String] = graph.getOrElse(node.toString, ArrayBuffer[String]()).asInstanceOf[ArrayBuffer[String]]
    var l: BigInt = BigInt(0)
    while (l < BigInt((neigh).size)) {
      val nb: String = neigh((l.toInt).toInt)
      if ((groupB.contains(nb)).asInstanceOf[Boolean]) {
