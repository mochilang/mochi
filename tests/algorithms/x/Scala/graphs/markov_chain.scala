// Generated by Mochi v0.10.59 on 2025-08-07 11:24:16 GMT+7
...
def transition(current: String, trans: ArrayBuffer[Transition]): String = {
  ...
  val random_value: Double = random()
  for (t <- trans) {
...
def get_transitions(start: String, trans: ArrayBuffer[Transition], steps: BigInt): scala.collection.mutable.Map[String,BigInt] = {
  ...
  while (i < steps) {
    ...
    var count: BigInt = visited.getOrElse(node.toString, null).asInstanceOf[BigInt]
...
def main(): Any = {
  val transitions: ArrayBuffer[Transition] = ArrayBuffer(Transition("a", "a", 0.9), Transition("a", "b", 0.075), Transition("a", "c", 0.025), Transition("b", "a", 0.15), Transition("b", "b", 0.8), Transition("b", "c", 0.05), Transition("c", "a", 0.25), Transition("c", "b", 0.25), Transition("c", "c", 0.5))
  val result: scala.collection.mutable.Map[String,BigInt] = get_transitions("a", transitions, BigInt(5000))
  println(String.valueOf(result.getOrElse("a", null).asInstanceOf[BigInt]) + " " + String.valueOf(result.getOrElse("b", null).asInstanceOf[BigInt]) + " " + String.valueOf(result.getOrElse("c", null).asInstanceOf[BigInt]))
}
