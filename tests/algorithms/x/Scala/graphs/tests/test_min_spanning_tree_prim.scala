// Generated by Mochi v0.10.63 on 2025-08-12 08:48:09 GMT+7
...
def test_prim_successful_result(): Boolean = {
  ...
  for (e <- edges) {
    ...
    val w: BigInt = e.lift((BigInt(2).toInt).toInt).getOrElse(BigInt(0))
  ...
  for (e <- result) {
    val key1: String = _str(e.asInstanceOf[EdgePair].u) + "," + _str(e.asInstanceOf[EdgePair].v)
    val key2: String = _str(e.asInstanceOf[EdgePair].v) + "," + _str(e.asInstanceOf[EdgePair].u)
  ...
  for (ans <- expected) {
    val key: String = _str(ans.lift((BigInt(0).toInt).toInt).getOrElse(BigInt(0))) + "," + _str(ans.lift((BigInt(1).toInt).toInt).getOrElse(BigInt(0)))
