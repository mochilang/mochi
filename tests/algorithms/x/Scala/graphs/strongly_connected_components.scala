// Generated by Mochi v0.10.63 on 2025-08-12 08:47:42 GMT+7
...
def strongly_connected_components(graph: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < n) {
    val v: BigInt = order.lift(((n - i - BigInt(1)).toInt).toInt).getOrElse(BigInt(0))
    if (!(visited.lift((v.toInt).toInt).getOrElse(null)).asInstanceOf[Boolean]) {
...
def main(): Any = {
  ...
  println(_str(strongly_connected_components(test_graph_1)))
  println(_str(strongly_connected_components(test_graph_2)))
}
