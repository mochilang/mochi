// Generated by Mochi v0.10.59 on 2025-08-07 11:16:07 GMT+7
...
def bfs_shortest_path(graph: scala.collection.mutable.Map[String,ArrayBuffer[String]], start: String, goal: String): ArrayBuffer[String] = {
  var explored: ArrayBuffer[String] = ArrayBuffer()
  var queue: ArrayBuffer[ArrayBuffer[String]] = ArrayBuffer(ArrayBuffer(start))
  if (start == goal) {
    return ArrayBuffer(start)
  }
  while (BigInt((queue).size) > 0) {
    val path: ArrayBuffer[String] = queue((0.toInt).toInt)
    queue = queue.slice((1).toInt, (BigInt((queue).size)).toInt)
    val node: String = path(((BigInt((path).size) - 1).toInt).toInt)
    if (!(explored.contains(node)).asInstanceOf[Boolean]) {
      ...
      while (i < BigInt((neighbours).size)) {
        val neighbour: String = neighbours((i.toInt).toInt)
...
def bfs_shortest_path_distance(graph: scala.collection.mutable.Map[String,ArrayBuffer[String]], start: String, target: String): BigInt = {
  if ((contains_key(graph, start) == false || contains_key(graph, target) == false).asInstanceOf[Boolean]) {
    return -1
  }
  if (start == target) {
    return 0
  ...
  dist.update(start, 0)
  dist.update(target, -1)
  while (BigInt((queue).size) > 0) {
    val node: String = queue((0.toInt).toInt)
    queue = queue.slice((1).toInt, (BigInt((queue).size)).toInt)
    if (node == target) {
      if ((dist.getOrElse(target.toString, null).asInstanceOf[BigInt] == -1 || dist.getOrElse(node.toString, null).asInstanceOf[BigInt] < dist.getOrElse(target.toString, null).asInstanceOf[BigInt]).asInstanceOf[Boolean]) {
        dist.update(target, dist.getOrElse(node.toString, null).asInstanceOf[BigInt])
      }
    }
    ...
    while (i < BigInt((adj).size)) {
      val next: String = adj((i.toInt).toInt)
      if (!(visited.contains(next)).asInstanceOf[Boolean]) {
        visited = visited :+ next
        queue = queue :+ next
        dist.update(next, dist.getOrElse(node.toString, null).asInstanceOf[BigInt] + 1)
      }
      i = i + 1
    }
  }
  return dist.getOrElse(target.toString, null).asInstanceOf[BigInt]
}
