// Generated by Mochi v0.10.59 on 2025-08-07 12:50:49 GMT+7
...
def float_to_bits(x: Double): BigInt = {
  ...
  val exp: BigInt = log2_floor(num)
  val pow: Double = pow2_float(exp)
  val normalized: Double = num / pow
  val frac: Double = normalized - 1.0
  val mantissa: BigInt = BigInt((frac * pow2_float(BigInt(23))).toString.toDouble.toInt)
  val exp_bits: BigInt = exp + BigInt(127)
  return lshift(sign, BigInt(31)) + lshift(exp_bits, BigInt(23)) + mantissa
}

def bits_to_float(bits: BigInt): Double = {
  val sign_bit: BigInt = rshift(bits, BigInt(31)) % BigInt(2)
  var sign: Double = 1.0
  if (sign_bit == BigInt(1)) {
    sign = -1.0
  }
  val exp_bits: BigInt = rshift(bits, BigInt(23)) % BigInt(256)
  val exp: BigInt = exp_bits - BigInt(127)
  val mantissa_bits: BigInt = bits % pow2_int(BigInt(23))
  val mantissa: Double = 1.0 + mantissa_bits.toString.toDouble / pow2_float(BigInt(23))
...
def fast_inverse_sqrt(number: Double): Double = {
  ...
  val i: BigInt = float_to_bits(number)
  val magic: BigInt = BigInt(1597463007)
  val y_bits: BigInt = magic - rshift(i, BigInt(1))
...
def test_fast_inverse_sqrt(): Any = {
  ...
  while (i < BigInt(60)) {
    val y: Double = fast_inverse_sqrt(i.toString.toDouble)
    val actual: Double = 1.0 / sqrtApprox(i.toString.toDouble)
...
def main(): Any = {
  ...
  while (i <= BigInt(100)) {
    val diff: Double = 1.0 / sqrtApprox(i.toString.toDouble) - fast_inverse_sqrt(i.toString.toDouble)
    println(String.valueOf(i) + ": " + String.valueOf(diff))
    i = i + BigInt(5)
  }
}
