// Generated by Mochi v0.10.61 on 2025-08-08 16:34:38 GMT+7
...
def proth(number: BigInt): BigInt = {
  ...
  val temp: BigInt = number / BigInt(3)
  ...
  while (block < block_index) {
    var i: BigInt = BigInt(0)
    while (i < increment) {
      val next_val: BigInt = pow2(block + BigInt(1)) + proth_list(((proth_index - BigInt(1)).toInt).toInt)
  ...
  return proth_list(((number - BigInt(1)).toInt).toInt)
}

def main(): Any = {
  var n: BigInt = BigInt(1)
  while (n <= BigInt(10)) {
    val value: BigInt = proth(n)
    println("The " + _str(n) + "th Proth number: " + _str(value))
    n = n + BigInt(1)
  }
}
