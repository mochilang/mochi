// Generated by Mochi v0.10.61 on 2025-08-08 17:34:24 GMT+7
...
def semi_perfect(number: BigInt): Boolean = {
  ...
  val values: ArrayBuffer[BigInt] = factors(number)
  ...
  while (idx < BigInt((values).size)) {
    val v: BigInt = values((idx.toInt).toInt)
    var s: BigInt = number
    while (s >= v) {
      if (possible(((s - v).toInt).toInt)) {
  ...
  return possible((number.toInt).toInt)
}
...
def main(): Any = {
  run_tests()
  val nums: ArrayBuffer[BigInt] = ArrayBuffer(BigInt(69), BigInt(70), BigInt(71))
  var i: BigInt = BigInt(0)
  while (i < BigInt((nums).size)) {
    val n: BigInt = nums((i.toInt).toInt)
    if (weird(n)) {
      println(_str(n) + " is weird.")
    } else {
      println(_str(n) + " is not weird.")
    }
    i = i + BigInt(1)
  }
}
