// Generated by Mochi v0.10.61 on 2025-08-08 17:35:48 GMT+7
  ...
  while (i < n) {
    ...
    while (j < n - BigInt(1)) {
      if (arr((j.toInt).toInt) > arr(((j + BigInt(1)).toInt).toInt)) {
        val temp: BigInt = arr((j.toInt).toInt)
...
def three_sum(nums: ArrayBuffer[BigInt]): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  var i: BigInt = BigInt(0)
  while (i < n - BigInt(2)) {
    if ((i == BigInt(0) || sorted((i.toInt).toInt) != sorted(((i - BigInt(1)).toInt).toInt)).asInstanceOf[Boolean]) {
      var low: BigInt = i + BigInt(1)
      var high: BigInt = n - BigInt(1)
      val c: BigInt = BigInt(0) - sorted((i.toInt).toInt)
      while (low < high) {
        val s: BigInt = sorted((low.toInt).toInt) + sorted((high.toInt).toInt)
        if (s == c) {
          val triple: ArrayBuffer[BigInt] = ArrayBuffer(sorted((i.toInt).toInt), sorted((low.toInt).toInt), sorted((high.toInt).toInt))
          res = res :+ (triple)
          while (low < high && sorted((low.toInt).toInt) == sorted(((low + BigInt(1)).toInt).toInt)) {
            low = low + BigInt(1)
          }
          while (low < high && sorted((high.toInt).toInt) == sorted(((high - BigInt(1)).toInt).toInt)) {
            high = high - BigInt(1)
          }
          low = low + BigInt(1)
          high = high - BigInt(1)
