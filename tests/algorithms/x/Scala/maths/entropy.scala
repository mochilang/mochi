// Generated by Mochi v0.10.59 on 2025-08-07 12:49:48 GMT+7
...
def analyze_text(text: String): TextCounts = {
  ...
  if ((single.contains(last)).asInstanceOf[Boolean]) {
    single.update(last, single.getOrElse(last.toString, null).asInstanceOf[BigInt] + BigInt(1))
  } else {
  ...
  while (i < n - BigInt(1)) {
    ...
    if ((single.contains(ch)).asInstanceOf[Boolean]) {
      single.update(ch, single.getOrElse(ch.toString, null).asInstanceOf[BigInt] + BigInt(1))
    } else {
      single.update(ch, BigInt(1))
    }
    val seq: String = text.slice((i).toInt, (i + BigInt(2)).toInt)
    if ((double.contains(seq)).asInstanceOf[Boolean]) {
      double.update(seq, double.getOrElse(seq.toString, null).asInstanceOf[BigInt] + BigInt(1))
    } else {
...
def calculate_entropy(text: String): Any = {
  ...
  for (ch <- counts.asInstanceOf[TextCounts].single.keys) {
    total1 = total1 + counts.asInstanceOf[TextCounts].single.getOrElse(ch.toString, null).asInstanceOf[BigInt]
  }
  var h1: Double = 0.0
  var i: BigInt = BigInt(0)
  while (i < BigInt((alphas).size)) {
    ...
    if ((counts.asInstanceOf[TextCounts].single.contains(ch)).asInstanceOf[Boolean]) {
      val prob: Double = (counts.asInstanceOf[TextCounts].single.getOrElse(ch.toString, null).asInstanceOf[BigInt]).toString.toDouble / total1.toString.toDouble
      h1 = h1 + prob * log2(prob)
    }
    i = i + BigInt(1)
  }
  val first_entropy: Double = -h1
  println(String.valueOf(round_to_int(first_entropy)) + ".0")
  var total2: BigInt = BigInt(0)
  for (seq <- counts.asInstanceOf[TextCounts].double.keys) {
    total2 = total2 + counts.asInstanceOf[TextCounts].double.getOrElse(seq.toString, null).asInstanceOf[BigInt]
  }
  var h2: Double = 0.0
  ...
  while (a0 < BigInt((alphas).size)) {
    ...
    while (a1 < BigInt((alphas).size)) {
      ...
      if ((counts.asInstanceOf[TextCounts].double.contains(seq)).asInstanceOf[Boolean]) {
        val prob: Double = (counts.asInstanceOf[TextCounts].double.getOrElse(seq.toString, null).asInstanceOf[BigInt]).toString.toDouble / total2.toString.toDouble
  ...
  println(String.valueOf(round_to_int(second_entropy)) + ".0")
  val diff: Double = second_entropy - first_entropy
  println(String.valueOf(round_to_int(diff)) + ".0")
}
