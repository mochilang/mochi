// Generated by Mochi v0.10.61 on 2025-08-08 17:36:05 GMT+7
  ...
  while (a < n) {
    ...
    while (b < n - a - BigInt(1)) {
      if (arr((b.toInt).toInt) > arr(((b + BigInt(1)).toInt).toInt)) {
        val tmp: BigInt = arr((b.toInt).toInt)
  ...
  while (i < BigInt((arr).size) - BigInt(2)) {
    ...
    while (j < BigInt((arr).size) - BigInt(1)) {
      ...
      while (k < BigInt((arr).size)) {
        if (arr((i.toInt).toInt) + arr((j.toInt).toInt) + arr((k.toInt).toInt) == target) {
          return sort3(ArrayBuffer(arr((i.toInt).toInt), arr((j.toInt).toInt), arr((k.toInt).toInt)))
        }
        k = k + BigInt(1)
      }
      j = j + BigInt(1)
...
def triplet_sum2(arr: ArrayBuffer[BigInt], target: BigInt): ArrayBuffer[BigInt] = {
  ...
  while (i < n - BigInt(2)) {
    ...
    while (left < right) {
      val s: BigInt = sorted((i.toInt).toInt) + sorted((left.toInt).toInt) + sorted((right.toInt).toInt)
      if (s == target) {
        return ArrayBuffer(sorted((i.toInt).toInt), sorted((left.toInt).toInt), sorted((right.toInt).toInt))
      }
      if (s < target) {
        left = left + BigInt(1)
      } else {
...
def main(): Any = {
  ...
  println(_str(res((BigInt(0).toInt).toInt)) + " " + _str(res((BigInt(1).toInt).toInt)) + " " + _str(res((BigInt(2).toInt).toInt)))
}
