// Generated by Mochi v0.10.63 on 2025-08-12 08:06:37 GMT+7
...
def test_explicit_euler(): Any = {
  ...
  val last: Double = ys.lift(((BigInt((ys).size) - BigInt(1)).toInt).toInt).getOrElse(0.0)
...
def main(): Any = {
  ...
  val f: (Double, Double) => Any = ((x: Double, y: Double) => y)
  val ys: ArrayBuffer[Double] = explicit_euler(f.asInstanceOf[(Double, Double) => Double], 1.0, 0.0, 0.01, 5.0)
  println(ys.lift(((BigInt((ys).size) - BigInt(1)).toInt).toInt).getOrElse(0.0))
}
