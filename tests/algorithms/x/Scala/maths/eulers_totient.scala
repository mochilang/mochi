// Generated by Mochi v0.10.59 on 2025-08-07 12:50:19 GMT+7
    ...
    while (i <= n) {
      ...
      _br2.breakable {
        while (j < BigInt((primes).size)) {
          val p: BigInt = primes((j.toInt).toInt)
          if (i * p >= n) {
            _br2.break()
          }
          is_prime((i * p).toInt) = false
          if (i % p == BigInt(0)) {
            totients((i * p).toInt) = totients((i.toInt).toInt) * p
            _br2.break()
          }
          totients((i * p).toInt) = totients((i.toInt).toInt) * (p - BigInt(1))
          j = j + BigInt(1)
        }
      }
      i = i + BigInt(1)
...
def test_totient(): Any = {
  ...
  while (idx < BigInt((expected).size)) {
    if (res((idx.toInt).toInt) != expected((idx.toInt).toInt)) {
      throw new RuntimeException(String.valueOf("totient mismatch at " + String.valueOf(idx)))
    }
    idx = idx + BigInt(1)
  }
}
...
def main(): Any = {
  ...
  while (i < n) {
    println(String.valueOf(i) + " has " + String.valueOf(res((i.toInt).toInt)) + " relative primes.")
    i = i + BigInt(1)
  }
}
