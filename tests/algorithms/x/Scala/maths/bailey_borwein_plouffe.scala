// Generated by Mochi v0.10.59 on 2025-08-07 12:47:23 GMT+7
...
def hex_digit(n: BigInt): String = {
  ...
  val letters: ArrayBuffer[String] = ArrayBuffer("a", "b", "c", "d", "e", "f")
  return letters(((n - BigInt(10)).toInt).toInt)
}
...
def subsum(digit_pos_to_extract: BigInt, denominator_addend: BigInt, precision: BigInt): Double = {
  ...
  while (sum_index < digit_pos_to_extract + precision) {
    val denominator: BigInt = BigInt(8) * sum_index + denominator_addend
    if (sum_index < digit_pos_to_extract) {
      val exponent: BigInt = digit_pos_to_extract - BigInt(1) - sum_index
      val exponential_term: BigInt = mod_pow(BigInt(16), exponent, denominator)
      total = total + exponential_term.toString.toDouble / denominator.toString.toDouble
    } else {
      val exponent: BigInt = digit_pos_to_extract - BigInt(1) - sum_index
      val exponential_term: Double = pow_float(16.0, exponent)
...
def bailey_borwein_plouffe(digit_position: BigInt, precision: BigInt): String = {
  ...
  val sum_result: Double = 4.0 * subsum(digit_position, BigInt(1), precision) - 2.0 * subsum(digit_position, BigInt(4), precision) - 1.0 * subsum(digit_position, BigInt(5), precision) - 1.0 * subsum(digit_position, BigInt(6), precision)
  val fraction: Double = sum_result - floor_float(sum_result)
  val digit: BigInt = BigInt((fraction * 16.0).toInt)
  val hd: String = hex_digit(digit)
  return hd
}
...
def main(args: Array[String]): Unit = {
  {
    ...
    println(digits)
    println(bailey_borwein_plouffe(BigInt(5), BigInt(10000)))
    val _end = _now()
    System.gc()
