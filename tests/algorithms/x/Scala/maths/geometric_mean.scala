// Generated by Mochi v0.10.60 on 2025-08-07 16:54:38 GMT+7
...
def nth_root(x: Double, n: BigInt): Double = {
  ...
  var i: BigInt = BigInt(0)
  while (i < BigInt(10)) {
    var denom: Double = Math.pow(guess, (n - BigInt(1)).toDouble)
...
def compute_geometric_mean(nums: ArrayBuffer[Double]): Double = {
  ...
  while (i < BigInt((nums).size)) {
    product = product * nums((i.toInt).toInt)
    i = i + BigInt(1)
  }
  ...
  var mean: Double = Math.pow(Math.abs(product), 1.0 / BigInt((nums).size).toDouble)
  if (product < 0.0) {
    mean = -mean
  }
  val possible: Double = round_nearest(mean)
  if (Math.pow(possible, BigInt((nums).size).toDouble) == product) {
    mean = possible
  }
  return mean
}
