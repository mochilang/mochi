// Generated by Mochi v0.10.71 on 2025-08-24 09:37:22 GMT+7
...
def pad_edge(image: ArrayBuffer[ArrayBuffer[BigInt]], pad_size: BigInt): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < new_height) {
    ...
    while (j < new_width) {
      ...
      row = ({row += (image.lift((src_i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((src_j.toInt).toInt).getOrElse(BigInt(0))); row})
      j = j + BigInt(1)
    }
  ...
  while (i < dst_height) {
    ...
    while (j < dst_width) {
      ...
      while (bi < block_h) {
        ...
        while (bj < block_w) {
          window = ({window += (image.lift(((i + bi).toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift(((j + bj).toInt).toInt).getOrElse(BigInt(0))); window})
          bj = bj + BigInt(1)
        }
        bi = bi + BigInt(1)
      }
      image_array = ({image_array += (window); image_array})
      j = j + BigInt(1)
    }
    i = i + BigInt(1)
  }
...
def flatten(matrix: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[BigInt] = {
  ...
  while (i < BigInt((matrix).size)) {
    ...
    while (j < BigInt((matrix.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]())).size)) {
      out = ({out += (matrix.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((j.toInt).toInt).getOrElse(BigInt(0))); out})
      j = j + BigInt(1)
    }
    i = i + BigInt(1)
  }
