// Generated by Mochi v0.10.66 on 2025-08-15 10:42:51 GMT+7
...
def bit_and(a: BigInt, b: BigInt): BigInt = {
  ...
  while (ua > BigInt(0) || ub > BigInt(0)) {
    if ((ua % BigInt(2) == BigInt(1) && ub % BigInt(2) == BigInt(1))) {
      res = res + bit
    }
    ua = ua / BigInt(2)
    ub = ub / BigInt(2)
...
def bit_xor(a: BigInt, b: BigInt): BigInt = {
  ...
  var bit: BigInt = BigInt(1)
  while (ua > BigInt(0) || ub > BigInt(0)) {
    var abit: BigInt = ua % BigInt(2)
    var bbit: BigInt = ub % BigInt(2)
    if (abit != bbit) {
      res = res + bit
    }
    ua = ua / BigInt(2)
...
def elf_hash(data: String): BigInt = {
  ...
  while (i < BigInt((data).size)) {
    var c: BigInt = ord(data.slice((i).toInt, (i + BigInt(1)).toInt))
    `hash_` = `hash_` * BigInt(16) + c
    var x: BigInt = bit_and(`hash_`, BigInt("4026531840"))
    if (x != BigInt(0)) {
...
def main(args: Array[String]): Unit = {
  {
    ...
    val _start = _now()
    println(_str(_str(elf_hash("lorem ipsum"))))
    val _end = _now()
    System.gc()
