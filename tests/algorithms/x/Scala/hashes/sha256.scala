// Generated by Mochi v0.10.59 on 2025-08-07 12:54:20 GMT+7
...
def bytes_to_hex(bs: ArrayBuffer[BigInt]): String = {
  ...
  while (i < BigInt((bs).size)) {
    res = res + byte_to_hex(bs((i.toInt).toInt))
    i = i + BigInt(1)
  }
  return res
}
...
def main(args: Array[String]): Unit = {
  {
    ...
    val _start = _now()
    println(bytes_to_hex((_sha256("Python".getBytes())).asInstanceOf[ArrayBuffer[BigInt]]))
    println(bytes_to_hex((_sha256("hello world".getBytes())).asInstanceOf[ArrayBuffer[BigInt]]))
    val _end = _now()
    System.gc()
