// Generated by Mochi v0.10.59 on 2025-08-07 11:26:52 GMT+7
...
def build_alphabet(): ArrayBuffer[String] = {
  ...
  while (i < BigInt((ASCII).length)) {
    result = result :+ (ASCII.slice((i.toInt).toInt, (i.toInt).toInt + 1)).toString
    i = i + BigInt(1)
  }
  return result
}
...
def enigma_encrypt(message: String, token: BigInt): String = {
  ...
  def rotator(): Any = {
    var i: BigInt = gear_one((BigInt(0).toInt).toInt)
    ...
    if (gear_one_pos % n == BigInt(0)) {
      i = gear_two((BigInt(0).toInt).toInt)
      ...
      if (gear_two_pos % n == BigInt(0)) {
        i = gear_three((BigInt(0).toInt).toInt)
  ...
  def engine(ch: String): String = {
    ...
    target = gear_one((target.toInt).toInt)
    target = gear_two((target.toInt).toInt)
    target = gear_three((target.toInt).toInt)
    target = reflector((target.toInt).toInt)
    target = index_of_int(gear_three, target)
    target = index_of_int(gear_two, target)
    target = index_of_int(gear_one, target)
    rotator()
    return alphabets((target.toInt).toInt)
  }
