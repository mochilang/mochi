// Generated by Mochi v0.10.59 on 2025-08-07 12:39:33 GMT+7
...
def sha1(message: String): String = {
  ...
  while (j < BigInt(8)) {
    bytes = bytes :+ len_bytes((j.toInt).toInt)
    j = j + BigInt(1)
  }
  ...
  while (pos < BigInt((bytes).size)) {
    ...
    while (j2 < BigInt(64)) {
      block = block :+ bytes(((pos + j2).toInt).toInt)
      j2 = j2 + BigInt(1)
    }
    blocks = blocks :+ block
    pos = pos + BigInt(64)
  ...
  while (bindex < BigInt((blocks).size)) {
    var block: ArrayBuffer[BigInt] = blocks((bindex.toInt).toInt)
    var w: ArrayBuffer[BigInt] = ArrayBuffer[BigInt]()
    var t: BigInt = BigInt(0)
    while (t < BigInt(16)) {
      val j3: BigInt = t * BigInt(4)
      val word: BigInt = ((block((j3.toInt).toInt) * BigInt(256) + block(((j3 + BigInt(1)).toInt).toInt)) * BigInt(256) + block(((j3 + BigInt(2)).toInt).toInt)) * BigInt(256) + block(((j3 + BigInt(3)).toInt).toInt)
      w = w :+ word
      t = t + BigInt(1)
    }
    while (t < BigInt(80)) {
      val tmp: BigInt = bit_xor(bit_xor(bit_xor(w(((t - BigInt(3)).toInt).toInt), w(((t - BigInt(8)).toInt).toInt)), w(((t - BigInt(14)).toInt).toInt)), w(((t - BigInt(16)).toInt).toInt))
    ...
    while (i2 < BigInt(80)) {
      ...
      val temp: BigInt = (rotate_left(a, BigInt(5)) + f + e + kconst + w((i2.toInt).toInt)) % MOD
      e = d
      d = c
