// Generated by Mochi v0.10.59 on 2025-08-07 12:39:18 GMT+7
...
def int_to_bits(n: BigInt, width: BigInt): String = {
  ...
  var num: BigInt = n
  while (num > BigInt(0)) {
    bits = String.valueOf(num % BigInt(2)) + bits
    num = num / BigInt(2)
  }
...
def md5_me(message: String): String = {
  ...
  while (bi < BigInt((blocks).size)) {
    val block: ArrayBuffer[BigInt] = blocks((bi.toInt).toInt)
    var a: BigInt = a0
    var b: BigInt = b0
    var c: BigInt = c0
    var d: BigInt = d0
    ...
    while (i < BigInt(64)) {
      ...
      f = sum_32(f, a)
      f = sum_32(f, added_consts((i.toInt).toInt))
      f = sum_32(f, block((g.toInt).toInt))
      val rotated: BigInt = left_rotate_32(f, shift_amounts((i.toInt).toInt))
