// Generated by Mochi v0.10.59 on 2025-08-07 11:12:52 GMT+7
...
def iteration_step(vectors: ArrayBuffer[Vec]): ArrayBuffer[Vec] = {
  var new_vectors: ArrayBuffer[Vec] = ArrayBuffer()
  var i: BigInt = 0
  while (i < BigInt((vectors).size) - 1) {
    val start: Vec = vectors((i.toInt).toInt)
    val end: Vec = vectors(((i + 1).toInt).toInt)
  ...
  new_vectors = new_vectors :+ vectors(((BigInt((vectors).size) - 1).toInt).toInt)
  return new_vectors
}
...
def vec_to_string(v: Vec): String = {
  return "(" + String.valueOf(v.asInstanceOf[Vec].x) + ", " + String.valueOf(v.asInstanceOf[Vec].y) + ")"
}
...
def vec_list_to_string(lst: ArrayBuffer[Vec]): String = {
  ...
  while (i < BigInt((lst).size)) {
    res = res + vec_to_string(lst((i.toInt).toInt))
    if (i < BigInt((lst).size) - 1) {
      res = res + ", "
    }
    i = i + 1
  }
...
val example: ArrayBuffer[Vec] = iterate(ArrayBuffer(VECTOR_1, VECTOR_3), 1)
