// Generated by Mochi v0.10.63 on 2025-08-12 08:50:00 GMT+7
...
def sort_by_ratio_desc(arr: ArrayBuffer[Item]): ArrayBuffer[Item] = {
  ...
  _br0.breakable {
    while (i < BigInt((arr).size)) {
      val key: Item = arr.lift((i.toInt).toInt).getOrElse(null)
      var j: BigInt = i - BigInt(1)
      val _br1 = new Breaks
      _br1.breakable {
        while (j >= BigInt(0)) {
          val current: Item = arr.lift((j.toInt).toInt).getOrElse(null)
...
def frac_knapsack(vl: ArrayBuffer[Double], wt: ArrayBuffer[Double], w: Double, n: BigInt): Double = {
  ...
  while (i < BigInt((items).size)) {
    val itm: Item = items.lift((i.toInt).toInt).getOrElse(null)
