// Generated by Mochi v0.10.59 on 2025-08-07 11:26:17 GMT+7
...
def insertion_sort(a: ArrayBuffer[BigInt]): ArrayBuffer[BigInt] = {
  ...
  while (i < BigInt((a).size)) {
    ...
    while (j >= BigInt(0) && a((j.toInt).toInt) > key) {
      a((j + BigInt(1)).toInt) = a((j.toInt).toInt)
      j = j - BigInt(1)
    }
...
def minimum_waiting_time(queries: ArrayBuffer[BigInt]): BigInt = {
  ...
  while (i < n) {
    total = total + sorted((i.toInt).toInt) * (n - i - BigInt(1))
    i = i + BigInt(1)
  }
  return total
}
