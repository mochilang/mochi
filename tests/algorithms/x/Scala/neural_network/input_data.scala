// Generated by Mochi v0.10.61 on 2025-08-08 19:20:24 GMT+7
...
def dense_to_one_hot(labels: ArrayBuffer[BigInt], num_classes: BigInt): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < BigInt((labels).size)) {
    ...
    while (j < num_classes) {
      if (j == labels.lift((i.toInt).toInt).getOrElse(null)) {
        row = row :+ (BigInt(1))
      } else {
        row = row :+ (BigInt(0))
      }
...
def next_batch(ds: DataSet, batch_size: BigInt): BatchResult = {
  ...
  if (start + batch_size > ds.asInstanceOf[DataSet].num_examples) {
    ...
    val batch_images = images_rest ++ images_new
    val batch_labels = labels_rest ++ labels_new
