// Generated by Mochi v0.10.61 on 2025-08-08 17:12:21 GMT+7
  ...
  while (i <= size_data - size_kernel) {
    ...
    while (j <= size_data - size_kernel) {
      ...
      while (a < size_kernel) {
        ...
        while (b < size_kernel) {
          sum = sum + data(((i + a).toInt).toInt)(((j + b).toInt).toInt) * kernel((a.toInt).toInt)((b.toInt).toInt)
          b = b + BigInt(1)
        }
        a = a + BigInt(1)
      }
...
def average_pool(map: ArrayBuffer[ArrayBuffer[Double]], size: BigInt): ArrayBuffer[ArrayBuffer[Double]] = {
  ...
  while (i < BigInt((map).size)) {
    var row: ArrayBuffer[Double] = ArrayBuffer[Double]()
    var j: BigInt = BigInt(0)
    while (j < BigInt((map((i.toInt).toInt)).size)) {
      var sum: Double = 0.0
      var a: BigInt = BigInt(0)
      while (a < size) {
        var b: BigInt = BigInt(0)
        while (b < size) {
          sum = sum + map(((i + a).toInt).toInt)(((j + b).toInt).toInt)
          b = b + BigInt(1)
        }
        a = a + BigInt(1)
      }
...
def flatten(maps: ArrayBuffer[ArrayBuffer[ArrayBuffer[Double]]]): ArrayBuffer[Double] = {
  ...
  while (i < BigInt((maps).size)) {
    var j: BigInt = BigInt(0)
    while (j < BigInt((maps((i.toInt).toInt)).size)) {
      var k: BigInt = BigInt(0)
      while (k < BigInt((maps((i.toInt).toInt)((j.toInt).toInt)).size)) {
        out = out :+ (maps((i.toInt).toInt)((j.toInt).toInt)((k.toInt).toInt))
        k = k + BigInt(1)
      }
      j = j + BigInt(1)
    }
...
def vec_mul_mat(v: ArrayBuffer[Double], m: ArrayBuffer[ArrayBuffer[Double]]): ArrayBuffer[Double] = {
  var cols: BigInt = BigInt((m((BigInt(0).toInt).toInt)).size)
  ...
  while (j < cols) {
    ...
    while (i < BigInt((v).size)) {
      sum = sum + v((i.toInt).toInt) * m((i.toInt).toInt)((j.toInt).toInt)
      i = i + BigInt(1)
    }
    res = res :+ (sum)
    j = j + BigInt(1)
...
def vec_map_sig(v: ArrayBuffer[Double]): ArrayBuffer[Double] = {
  ...
  var i: BigInt = BigInt(0)
  while (i < BigInt((v).size)) {
    res = res :+ (sigmoid(v((i.toInt).toInt)))
    i = i + BigInt(1)
  }
  return res
}
...
def forward(cnn: CNN, data: ArrayBuffer[ArrayBuffer[Double]]): ArrayBuffer[Double] = {
  ...
  while (i < BigInt((cnn.asInstanceOf[CNN].conv_kernels).size)) {
    val conv_map: ArrayBuffer[ArrayBuffer[Double]] = convolve(data, cnn.asInstanceOf[CNN].conv_kernels((i.toInt).toInt), cnn.asInstanceOf[CNN].conv_step, cnn.asInstanceOf[CNN].conv_bias((i.toInt).toInt))
...
def train(cnn: CNN, samples: ArrayBuffer[TrainSample], epochs: BigInt): CNN = {
  ...
  while (e < epochs) {
    ...
    while (s < BigInt((samples).size)) {
      val data: ArrayBuffer[ArrayBuffer[Double]] = samples((s.toInt).toInt).image
      val target: ArrayBuffer[Double] = samples((s.toInt).toInt).target
      var maps: ArrayBuffer[ArrayBuffer[ArrayBuffer[Double]]] = ArrayBuffer[ArrayBuffer[ArrayBuffer[Double]]]()
      var i: BigInt = BigInt(0)
      while (i < BigInt((cnn.asInstanceOf[CNN].conv_kernels).size)) {
        val conv_map: ArrayBuffer[ArrayBuffer[Double]] = convolve(data, cnn.asInstanceOf[CNN].conv_kernels((i.toInt).toInt), cnn.asInstanceOf[CNN].conv_step, cnn.asInstanceOf[CNN].conv_bias((i.toInt).toInt))
      ...
      while (j < BigInt((w_out).size)) {
        var k: BigInt = BigInt(0)
        while (k < BigInt((w_out((j.toInt).toInt)).size)) {
          w_out((j).toInt)((k).toInt) = w_out((j.toInt).toInt)((k.toInt).toInt) + cnn.asInstanceOf[CNN].rate_weight * hidden_out((j.toInt).toInt) * pd_out((k.toInt).toInt)
          k = k + BigInt(1)
        }
        j = j + BigInt(1)
      }
      ...
      while (j < BigInt((b_out).size)) {
        b_out((j).toInt) = b_out((j.toInt).toInt) - cnn.asInstanceOf[CNN].rate_bias * pd_out((j.toInt).toInt)
        j = j + BigInt(1)
      }
      var i_h: BigInt = BigInt(0)
      while (i_h < BigInt((w_hidden).size)) {
        var j_h: BigInt = BigInt(0)
        while (j_h < BigInt((w_hidden((i_h.toInt).toInt)).size)) {
          w_hidden((i_h).toInt)((j_h).toInt) = w_hidden((i_h.toInt).toInt)((j_h.toInt).toInt) + cnn.asInstanceOf[CNN].rate_weight * flat((i_h.toInt).toInt) * pd_hidden((j_h.toInt).toInt)
          j_h = j_h + BigInt(1)
        }
        i_h = i_h + BigInt(1)
      }
      ...
      while (j < BigInt((b_hidden).size)) {
        b_hidden((j).toInt) = b_hidden((j.toInt).toInt) - cnn.asInstanceOf[CNN].rate_bias * pd_hidden((j.toInt).toInt)
        j = j + BigInt(1)
      }
      s = s + BigInt(1)
    }
