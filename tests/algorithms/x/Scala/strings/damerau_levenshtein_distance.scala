// Generated by Mochi v0.10.63 on 2025-08-11 15:36:59 GMT+7
...
def damerau_levenshtein_distance(first_string: String, second_string: String): BigInt = {
  ...
  for (i <- (BigInt(0)).toInt until (len1 + BigInt(1)).toInt) {
    ...
    row((BigInt(0)).toInt) = BigInt(i.toString.head.toInt)
  ...
  for (j <- (BigInt(0)).toInt until (len2 + BigInt(1)).toInt) {
    first_row((j).toInt) = BigInt(j.toString.head.toInt)
  }
  ...
  for (i <- (BigInt(1)).toInt until (len1 + BigInt(1)).toInt) {
    ...
    for (j <- (BigInt(1)).toInt until (len2 + BigInt(1)).toInt) {
      ...
      if (insertion < BigInt(value.toString.head.toInt)) {
        value = insertion
      }
      ...
      if (substitution < BigInt(value.toString.head.toInt)) {
        value = substitution
      }
      row((j).toInt) = BigInt(value.toString.head.toInt)
      if ((((BigInt(i.toString.head.toInt) > BigInt(1) && BigInt(j.toString.head.toInt) > BigInt(1)).asInstanceOf[Boolean] && first_string.slice((i - BigInt(1)).toInt, (i).toInt) == second_string.slice((j - BigInt(2)).toInt, (j - BigInt(1)).toInt)).asInstanceOf[Boolean] && first_string.slice((i - BigInt(2)).toInt, (i - BigInt(1)).toInt) == second_string.slice((j - BigInt(1)).toInt, (j).toInt)).asInstanceOf[Boolean]) {
