// Generated by Mochi v0.10.61 on 2025-08-08 17:20:30 GMT+7
  ...
  if (exists) {
    ...
    if (BigInt((store).size) == cache.asInstanceOf[LRUCache].max_capacity) {
      ...
      while (j < BigInt((store).size) - BigInt(1)) {
        new_store = (new_store ++ ArrayBuffer(store((j.toInt).toInt))).asInstanceOf[ArrayBuffer[String]]
        j = j + BigInt(1)
      }
      store = new_store
    }
...
def display(cache: LRUCache): Any = {
  ...
  while (i < BigInt((cache.asInstanceOf[LRUCache].store).size)) {
    println(cache.asInstanceOf[LRUCache].store((i.toInt).toInt))
    i = i + BigInt(1)
  }
}
...
def cache_repr(cache: LRUCache): String = {
  ...
  var i: BigInt = BigInt(0)
  while (i < BigInt((cache.asInstanceOf[LRUCache].store).size)) {
    res = res + repr_item(cache.asInstanceOf[LRUCache].store((i.toInt).toInt))
