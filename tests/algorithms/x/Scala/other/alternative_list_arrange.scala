// Generated by Mochi v0.10.61 on 2025-08-08 17:17:30 GMT+7
...
def alternative_list_arrange(first: ArrayBuffer[Item], second: ArrayBuffer[Item]): ArrayBuffer[Item] = {
  ...
  while (i < abs_len) {
    if (i < len1) {
      result = result :+ (first((i.toInt).toInt))
    }
    if (i < len2) {
      result = result :+ (second((i.toInt).toInt))
    }
    i = i + BigInt(1)
  }
  return result
...
def list_to_string(xs: ArrayBuffer[Item]): String = {
  ...
  while (i < BigInt((xs).size)) {
    s = s + item_to_string(xs((i.toInt).toInt))
