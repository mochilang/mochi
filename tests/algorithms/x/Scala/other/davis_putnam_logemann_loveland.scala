// Generated by Mochi v0.10.61 on 2025-08-08 17:19:24 GMT+7
...
def new_clause(lits: ArrayBuffer[String]): Clause = {
  ...
  while (i < BigInt((lits).size)) {
    val lit: String = lits((i.toInt).toInt)
    m.update(lit, BigInt(0) - BigInt(1))
    names = names :+ (lit)
    i = i + BigInt(1)
  }
...
def assign_clause(c: Clause, model: scala.collection.mutable.Map[String,BigInt]): Clause = {
  ...
  while (i < BigInt((c.asInstanceOf[Clause].names).size)) {
    val lit: String = c.asInstanceOf[Clause].names((i.toInt).toInt)
    val symbol: String = lit.slice((BigInt(0)).toInt, (BigInt(2)).toInt)
    if ((model.contains(symbol)).asInstanceOf[Boolean]) {
      var value: BigInt = model.getOrElse(symbol.toString, null).asInstanceOf[BigInt]
      if ((lit.slice((BigInt((lit).size) - BigInt(1)).toInt, (BigInt((lit).size)).toInt) == "'" && value != BigInt(0) - BigInt(1)).asInstanceOf[Boolean]) {
...
def evaluate_clause(_c: Clause, model: scala.collection.mutable.Map[String,BigInt]): EvalResult = {
  ...
  while (i < BigInt((c.asInstanceOf[Clause].names).size)) {
    val lit: String = c.asInstanceOf[Clause].names((i.toInt).toInt)
    val value: BigInt = c.asInstanceOf[Clause].literals.getOrElse(lit.toString, null).asInstanceOf[BigInt]
    if (value == BigInt(1)) {
      return EvalResult(BigInt(1), c)
    }
    if (value == BigInt(0) - BigInt(1)) {
      return EvalResult(BigInt(0) - BigInt(1), c)
    }
    i = i + BigInt(1)
  }
  ...
  while (i < BigInt((c.asInstanceOf[Clause].names).size)) {
    val lit: String = c.asInstanceOf[Clause].names((i.toInt).toInt)
    if (c.asInstanceOf[Clause].literals.getOrElse(lit.toString, null).asInstanceOf[BigInt] == BigInt(1)) {
      any_true = BigInt(1)
    }
    i = i + BigInt(1)
  }
...
def remove_symbol(symbols: ArrayBuffer[String], s: String): ArrayBuffer[String] = {
  ...
  while (i < BigInt((symbols).size)) {
    if (symbols((i.toInt).toInt) != s) {
      res = res :+ (symbols((i.toInt).toInt))
    }
    i = i + BigInt(1)
  }
  return res
...
def dpll_algorithm(clauses: ArrayBuffer[Clause], symbols: ArrayBuffer[String], model: scala.collection.mutable.Map[String,BigInt]): DPLLResult = {
  ...
  while (i < BigInt((clauses).size)) {
    val ev: EvalResult = evaluate_clause(clauses((i.toInt).toInt), model)
    ...
    if (ev.asInstanceOf[EvalResult].value == BigInt(0)) {
      ...
      if (ev.asInstanceOf[EvalResult].value == BigInt(0) - BigInt(1)) {
        all_true = false
      }
    }
    i = i + BigInt(1)
  ...
  val p: String = symbols((BigInt(0).toInt).toInt)
...
def str_formula(f: Formula): String = {
  ...
  while (i < BigInt((f.asInstanceOf[Formula].clauses).size)) {
    line = line + str_clause(f.asInstanceOf[Formula].clauses((i.toInt).toInt))
