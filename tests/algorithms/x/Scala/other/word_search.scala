// Generated by Mochi v0.10.62 on 2025-08-09 10:39:12 GMT+7
...
def insert_dir(ws: WordSearch, word: String, dr: BigInt, dc: BigInt, rows: ArrayBuffer[BigInt], cols: ArrayBuffer[BigInt]): Boolean = {
  ...
  _br3.breakable {
    while (ri < BigInt((rows).size)) {
      _ct3.breakable {
        val row: BigInt = rows.lift((ri.toInt).toInt).getOrElse(null)
        ...
        _br4.breakable {
          while (ci < BigInt((cols).size)) {
            _ct4.breakable {
              val col: BigInt = cols.lift((ci.toInt).toInt).getOrElse(null)
              val end_r: BigInt = row + dr * (word_len - BigInt(1))
              val end_c: BigInt = col + dc * (word_len - BigInt(1))
              if ((((end_r < BigInt(0) || end_r >= ws.asInstanceOf[WordSearch].height).asInstanceOf[Boolean] || end_c < BigInt(0)).asInstanceOf[Boolean] || end_c >= ws.asInstanceOf[WordSearch].width).asInstanceOf[Boolean]) {
                ci = ci + BigInt(1)
                _ct4.break()
              }
...
def generate_board(ws: WordSearch): Any = {
  ...
  while (i < BigInt((ws.asInstanceOf[WordSearch].words).size)) {
    ...
    insert_dir(ws, word, dirs_r.lift((d.toInt).toInt).getOrElse(null), dirs_c.lift((d.toInt).toInt).getOrElse(null), rows, cols)
    i = i + BigInt(1)
  }
}
