// Generated by Mochi v0.10.69 on 2025-08-19 16:43:36 GMT+7
...
def get_data(source_data: ArrayBuffer[ArrayBuffer[Double]]): ArrayBuffer[ArrayBuffer[Double]] = {
  ...
  while (i < BigInt((source_data).size)) {
    var row: ArrayBuffer[Double] = source_data.lift((i.toInt).toInt).getOrElse(ArrayBuffer[Double]())
...
def calculate_each_score(data_lists: ArrayBuffer[ArrayBuffer[Double]], weights: ArrayBuffer[BigInt]): ArrayBuffer[ArrayBuffer[Double]] = {
  ...
  while (i < BigInt((data_lists).size)) {
    var dlist: ArrayBuffer[Double] = data_lists.lift((i.toInt).toInt).getOrElse(ArrayBuffer[Double]())
    var weight: BigInt = weights.lift((i.toInt).toInt).getOrElse(BigInt(0))
    ...
    while (j < BigInt((dlist).size)) {
      var `val`: Double = dlist.lift((j.toInt).toInt).getOrElse(0.0)
      if (`val` < mind) {
        mind = `val`
      }
      if (`val` > maxd) {
    ...
    if (weight == BigInt(0)) {
      while (j < BigInt((dlist).size)) {
        var item: Double = dlist.lift((j.toInt).toInt).getOrElse(0.0)
        if (maxd - mind == 0.0) {
          score = (score :+ (1.0))
        } else {
...
def generate_final_scores(score_lists: ArrayBuffer[ArrayBuffer[Double]]): ArrayBuffer[Double] = {
  var count: BigInt = BigInt((score_lists.lift((BigInt(0).toInt).toInt).getOrElse(ArrayBuffer[Double]())).size)
  ...
  while (i < BigInt((score_lists).size)) {
    var slist: ArrayBuffer[Double] = score_lists.lift((i.toInt).toInt).getOrElse(ArrayBuffer[Double]())
    var j: BigInt = BigInt(0)
...
def procentual_proximity(source_data: ArrayBuffer[ArrayBuffer[Double]], weights: ArrayBuffer[BigInt]): ArrayBuffer[ArrayBuffer[Double]] = {
  var data_lists: ArrayBuffer[ArrayBuffer[Double]] = get_data(source_data)
  var score_lists: ArrayBuffer[ArrayBuffer[Double]] = calculate_each_score(data_lists, weights)
  var final_scores: ArrayBuffer[Double] = generate_final_scores(score_lists)
  var i: BigInt = BigInt(0)
...
def main(args: Array[String]): Unit = {
  {
    ...
    var result: ArrayBuffer[ArrayBuffer[Double]] = procentual_proximity(vehicles, weights)
