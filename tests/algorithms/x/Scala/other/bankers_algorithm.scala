// Generated by Mochi v0.10.61 on 2025-08-08 17:19:12 GMT+7
...
def processes_resource_summation(alloc: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[BigInt] = {
  val resources: BigInt = BigInt((alloc((BigInt(0).toInt).toInt)).size)
  ...
  while (i < resources) {
    ...
    while (j < BigInt((alloc).size)) {
      total = total + alloc((j.toInt).toInt)((i.toInt).toInt)
      j = j + BigInt(1)
    }
    sums = sums :+ (total)
    i = i + BigInt(1)
...
def available_resources(claim: ArrayBuffer[BigInt], alloc_sum: ArrayBuffer[BigInt]): ArrayBuffer[BigInt] = {
  ...
  while (i < BigInt((claim).size)) {
    avail = avail :+ (claim((i.toInt).toInt) - alloc_sum((i.toInt).toInt))
    i = i + BigInt(1)
  }
  return avail
}
...
def need(max: ArrayBuffer[ArrayBuffer[BigInt]], alloc: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < BigInt((max).size)) {
    ...
    var j: BigInt = BigInt(0)
    while (j < BigInt((max((BigInt(0).toInt).toInt)).size)) {
      row = row :+ (max((i.toInt).toInt)((j.toInt).toInt) - alloc((i.toInt).toInt)((j.toInt).toInt))
      j = j + BigInt(1)
    }
    needs = needs :+ (row)
    i = i + BigInt(1)
...
def pretty_print(claim: ArrayBuffer[BigInt], alloc: ArrayBuffer[ArrayBuffer[BigInt]], max: ArrayBuffer[ArrayBuffer[BigInt]]): Any = {
  ...
  while (i < BigInt((alloc).size)) {
    var row: ArrayBuffer[BigInt] = alloc((i.toInt).toInt)
    var line: String = "P" + _str(i + BigInt(1)) + "       "
    var j: BigInt = BigInt(0)
    while (j < BigInt((row).size)) {
      line = line + _str(row((j.toInt).toInt))
  ...
  i = BigInt(0)
  while (i < BigInt((max).size)) {
    var row: ArrayBuffer[BigInt] = max((i.toInt).toInt)
    var line: String = "P" + _str(i + BigInt(1)) + "       "
    var j: BigInt = BigInt(0)
    while (j < BigInt((row).size)) {
      line = line + _str(row((j.toInt).toInt))
  ...
  while (i < BigInt((claim).size)) {
    ...
    usage = usage + _str(claim((i.toInt).toInt))
    i = i + BigInt(1)
  }
  ...
  while (i < BigInt((avail).size)) {
    ...
    avail_str = avail_str + _str(avail((i.toInt).toInt))
    i = i + BigInt(1)
  }
        ...
        while (p < BigInt((need_list).size)) {
          if (!finished((p.toInt).toInt)) {
            ...
            if (exec) {
              ...
              r = BigInt(0)
              while (r < BigInt((avail).size)) {
                avail((r).toInt) = avail((r.toInt).toInt) + alloc((p.toInt).toInt)((r.toInt).toInt)
                r = r + BigInt(1)
              }
              var avail_str: String = ""
              r = BigInt(0)
              while (r < BigInt((avail).size)) {
                ...
                avail_str = avail_str + _str(avail((r.toInt).toInt))
                r = r + BigInt(1)
              }
