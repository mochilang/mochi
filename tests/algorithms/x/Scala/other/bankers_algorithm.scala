// Generated by Mochi v0.10.61 on 2025-08-08 19:23:42 GMT+7
...
def processes_resource_summation(alloc: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[BigInt] = {
  ...
  while (i < resources) {
    ...
    while (j < BigInt((alloc).size)) {
      total = total + alloc.lift((j.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((i.toInt).toInt).getOrElse(null)
      j = j + BigInt(1)
    }
    sums = sums :+ (total)
    i = i + BigInt(1)
...
def available_resources(claim: ArrayBuffer[BigInt], alloc_sum: ArrayBuffer[BigInt]): ArrayBuffer[BigInt] = {
  ...
  while (i < BigInt((claim).size)) {
    avail = avail :+ (claim.lift((i.toInt).toInt).getOrElse(null) - alloc_sum.lift((i.toInt).toInt).getOrElse(null))
    i = i + BigInt(1)
  }
  return avail
}
...
def need(max: ArrayBuffer[ArrayBuffer[BigInt]], alloc: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < BigInt((max).size)) {
    ...
    while (j < BigInt((max.lift((BigInt(0).toInt).toInt).getOrElse(ArrayBuffer[BigInt]())).size)) {
      row = row :+ (max.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((j.toInt).toInt).getOrElse(null) - alloc.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((j.toInt).toInt).getOrElse(null))
      j = j + BigInt(1)
    }
    needs = needs :+ (row)
    i = i + BigInt(1)
...
def pretty_print(claim: ArrayBuffer[BigInt], alloc: ArrayBuffer[ArrayBuffer[BigInt]], max: ArrayBuffer[ArrayBuffer[BigInt]]): Any = {
  ...
  while (i < BigInt((alloc).size)) {
    ...
    while (j < BigInt((row).size)) {
      line = line + _str(row.lift((j.toInt).toInt).getOrElse(null))
  ...
  while (i < BigInt((claim).size)) {
    ...
    usage = usage + _str(claim.lift((i.toInt).toInt).getOrElse(null))
    i = i + BigInt(1)
  }
  ...
  while (i < BigInt((avail).size)) {
    ...
    avail_str = avail_str + _str(avail.lift((i.toInt).toInt).getOrElse(null))
    i = i + BigInt(1)
  }
          ...
          if (!finished.lift((p.toInt).toInt).getOrElse(false)) {
            ...
            _br14.breakable {
              while (r < BigInt((avail).size)) {
                if (need_list.lift((p.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((r.toInt).toInt).getOrElse(null) > avail.lift((r.toInt).toInt).getOrElse(null)) {
                  exec = false
                  _br14.break()
                }
                r = r + BigInt(1)
            ...
            if (exec) {
              ...
              r = BigInt(0)
              while (r < BigInt((avail).size)) {
                avail((r).toInt) = avail.lift((r.toInt).toInt).getOrElse(null) + alloc.lift((p.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((r.toInt).toInt).getOrElse(null)
                r = r + BigInt(1)
              }
              var avail_str: String = ""
              r = BigInt(0)
              while (r < BigInt((avail).size)) {
                ...
                avail_str = avail_str + _str(avail.lift((r.toInt).toInt).getOrElse(null))
                r = r + BigInt(1)
              }
