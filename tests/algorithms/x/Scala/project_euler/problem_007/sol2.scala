// Generated by Mochi v0.10.62 on 2025-08-09 16:38:58 GMT+7
...
def solution(nth: BigInt): BigInt = {
  ...
  while (BigInt((primes).size) < nth) {
    if (is_prime(num)) {
      primes = primes :+ (num)
    }
    num = num + BigInt(1)
  }
  return primes.lift(((BigInt((primes).size) - BigInt(1)).toInt).toInt).getOrElse(null)
}

val ans: BigInt = solution(BigInt(10001))
...
def main(args: Array[String]): Unit = {
  {
    ...
    val _start = _now()
    println("solution(10001) = " + _str(ans))
    val _end = _now()
    System.gc()
