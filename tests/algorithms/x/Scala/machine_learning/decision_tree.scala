// Generated by Mochi v0.10.59 on 2025-08-07 12:42:53 GMT+7
...
case class Branch(var decision_boundary: Double, var left: Any, var right: Any) extends Tree
...
def mean(vals: ArrayBuffer[Double]): Double = {
  ...
  while (i < BigInt((vals).size)) {
    sum = sum + vals((i.toInt).toInt)
    i = i + BigInt(1)
  }
  return sum / (BigInt((vals).size)).toString.toDouble
}
...
def mean_squared_error(labels: ArrayBuffer[Double], prediction: Double): Double = {
  ...
  while (i < BigInt((labels).size)) {
    val diff: Double = labels((i.toInt).toInt) - prediction
    total = total + diff * diff
    i = i + BigInt(1)
  }
  return total / (BigInt((labels).size)).toString.toDouble
}
...
def train_tree(x: ArrayBuffer[Double], y: ArrayBuffer[Double], depth: BigInt, min_leaf_size: BigInt): Tree = {
  ...
  if (best_split != BigInt(0)) {
    ...
    val boundary: Double = x((best_split.toInt).toInt)
...
def main(): Any = {
  ...
  while (i < BigInt((test_cases).size)) {
    predictions = predictions :+ predict(tree.asInstanceOf[Tree], test_cases((i.toInt).toInt))
    i = i + BigInt(1)
  }
  var sum_err: Double = 0.0
  i = BigInt(0)
  while (i < BigInt((test_cases).size)) {
    val diff: Double = predictions((i.toInt).toInt) - test_cases((i.toInt).toInt)
    sum_err = sum_err + diff * diff
    i = i + BigInt(1)
  }
  val avg_error: Double = sum_err / (BigInt((test_cases).size)).toString.toDouble
  println("Test values: " + String.valueOf(test_cases))
  println("Predictions: " + String.valueOf(predictions))
  println("Average error: " + String.valueOf(avg_error))
}
