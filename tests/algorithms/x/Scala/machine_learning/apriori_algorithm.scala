// Generated by Mochi v0.10.63 on 2025-08-12 08:59:22 GMT+7
...
def combinations_lists(xs: ArrayBuffer[ArrayBuffer[String]], k: BigInt): ArrayBuffer[ArrayBuffer[ArrayBuffer[String]]] = {
  ...
  var i: BigInt = BigInt(0)
  while (i < BigInt((xs).size)) {
    val head: ArrayBuffer[String] = xs.lift((i.toInt).toInt).getOrElse(ArrayBuffer[String]())
  ...
  while (i < BigInt((res).size)) {
    ...
    while (j < BigInt((res).size)) {
      if (res.lift((j.toInt).toInt).getOrElse("") < res.lift((i.toInt).toInt).getOrElse("")) {
        val tmp: String = res.lift((i.toInt).toInt).getOrElse("")
...
def apriori(data: ArrayBuffer[ArrayBuffer[String]], min_support: BigInt): ArrayBuffer[Itemset] = {
  var itemset: ArrayBuffer[ArrayBuffer[ArrayBuffer[String]]] = ArrayBuffer[ArrayBuffer[ArrayBuffer[String]]]()
  ...
  while (BigInt((itemset).size) > BigInt(0)) {
    ...
    for (transaction <- data) {
      ...
      while (j < BigInt((itemset).size)) {
        val candidate: ArrayBuffer[String] = itemset.lift((j.toInt).toInt).getOrElse(ArrayBuffer[String]())
...
def main(args: Array[String]): Unit = {
  {
    ...
    for (fi <- frequent_itemsets) {
      println(itemset_to_string(fi.asInstanceOf[Itemset].items) + ": " + _str(fi.asInstanceOf[Itemset].support))
    }
    val _end = _now()
    System.gc()
