// Generated by Mochi v0.10.59 on 2025-08-07 12:42:30 GMT+7
...
def get_neighbours(p: Point, x_limit: BigInt, y_limit: BigInt): ArrayBuffer[Point] = {
  val deltas: ArrayBuffer[Point] = ArrayBuffer(Point(BigInt(0) - BigInt(1), BigInt(0) - BigInt(1)), Point(BigInt(0) - BigInt(1), BigInt(0)), Point(BigInt(0) - BigInt(1), BigInt(1)), Point(BigInt(0), BigInt(0) - BigInt(1)), Point(BigInt(0), BigInt(1)), Point(BigInt(1), BigInt(0) - BigInt(1)), Point(BigInt(1), BigInt(0)), Point(BigInt(1), BigInt(1)))
...
def get_node(nodes: ArrayBuffer[Node], p: Point): Node = {
  ...
  return Node(p, Point(BigInt(0) - BigInt(1), BigInt(0) - BigInt(1)), BigInt(0), BigInt(0), BigInt(0))
}
...
def astar(x_limit: BigInt, y_limit: BigInt, start: Point, goal: Point): ArrayBuffer[Point] = {
  ...
  open = open :+ Node(start, Point(BigInt(0) - BigInt(1), BigInt(0) - BigInt(1)), BigInt(0), BigInt(0), BigInt(0))
  var current: Node = open((BigInt(0).toInt).toInt)
  val _br3 = new Breaks
  val _ct3 = new Breaks
  _br3.breakable {
    while (BigInt((open).size) > BigInt(0)) {
      _ct3.breakable {
        ...
        while (i < BigInt((open).size)) {
          if (open((i.toInt).toInt).f < open((min_index.toInt).toInt).f) {
            min_index = i
          }
          i = i + BigInt(1)
        }
        current = open((min_index.toInt).toInt)
        var new_open: ArrayBuffer[Node] = ArrayBuffer[Node]()
        var j: BigInt = BigInt(0)
        while (j < BigInt((open).size)) {
          if (j != min_index) {
            new_open = new_open :+ open((j.toInt).toInt)
          }
          j = j + BigInt(1)
        }
        open = new_open
  ...
  while (!(current.asInstanceOf[Node].parent.asInstanceOf[Point].x == BigInt(0) - BigInt(1) && current.asInstanceOf[Node].parent.asInstanceOf[Point].y == BigInt(0) - BigInt(1)).asInstanceOf[Boolean]) {
...
def main(args: Array[String]): Unit = {
  {
    ...
    val _start = _now()
    println("path from (" + String.valueOf(start.x) + ", " + String.valueOf(start.y) + ") to (" + String.valueOf(goal.x) + ", " + String.valueOf(goal.y) + ")")
