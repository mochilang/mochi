// Generated by Mochi v0.10.61 on 2025-08-08 17:03:53 GMT+7
...
def populate_current_row(triangle: ArrayBuffer[ArrayBuffer[BigInt]], current_row_idx: BigInt): ArrayBuffer[BigInt] = {
  ...
  while (i <= current_row_idx) {
    if ((i == BigInt(0) || i == current_row_idx).asInstanceOf[Boolean]) {
      ...
      val left: BigInt = triangle(((current_row_idx - BigInt(1)).toInt).toInt)(((i - BigInt(1)).toInt).toInt)
      val right: BigInt = triangle(((current_row_idx - BigInt(1)).toInt).toInt)((i.toInt).toInt)
      row = row :+ (left + right)
    }
    i = i + BigInt(1)
  }
...
def row_to_string(row: ArrayBuffer[BigInt], total_rows: BigInt, row_idx: BigInt): String = {
  ...
  while (c <= row_idx) {
    line = line + _str(row((c.toInt).toInt))
    if (c != row_idx) {
      line = line + " "
    }
    c = c + BigInt(1)
...
def print_pascal_triangle(num_rows: BigInt): Any = {
  ...
  while (r < num_rows) {
    val line: String = row_to_string(triangle((r.toInt).toInt), num_rows, r)
    println(line)
    r = r + BigInt(1)
  }
}
