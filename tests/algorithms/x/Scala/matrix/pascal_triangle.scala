// Generated by Mochi v0.10.61 on 2025-08-08 19:11:12 GMT+7
...
def populate_current_row(triangle: ArrayBuffer[ArrayBuffer[BigInt]], current_row_idx: BigInt): ArrayBuffer[BigInt] = {
  ...
  while (i <= current_row_idx) {
    if ((i == BigInt(0) || i == current_row_idx).asInstanceOf[Boolean]) {
      ...
      val left: BigInt = triangle.lift(((current_row_idx - BigInt(1)).toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift(((i - BigInt(1)).toInt).toInt).getOrElse(null)
      val right: BigInt = triangle.lift(((current_row_idx - BigInt(1)).toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((i.toInt).toInt).getOrElse(null)
      row = row :+ (left + right)
    }
    i = i + BigInt(1)
  }
...
def row_to_string(row: ArrayBuffer[BigInt], total_rows: BigInt, row_idx: BigInt): String = {
  ...
  while (c <= row_idx) {
    line = line + _str(row.lift((c.toInt).toInt).getOrElse(null))
    if (c != row_idx) {
      line = line + " "
    }
    c = c + BigInt(1)
