// Generated by Mochi v0.10.61 on 2025-08-08 19:10:36 GMT+7
...
def depth_first_search(row: BigInt, col: BigInt, seen: scala.collection.mutable.Map[String,Boolean], mat: ArrayBuffer[ArrayBuffer[BigInt]]): BigInt = {
  ...
  if (((is_safe(row, col, rows, cols) && !has(seen, key)).asInstanceOf[Boolean] && mat.lift((row.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((col.toInt).toInt).getOrElse(null) == BigInt(1)).asInstanceOf[Boolean]) {
...
def find_max_area(mat: ArrayBuffer[ArrayBuffer[BigInt]]): BigInt = {
  ...
  while (r < rows) {
    ...
    while (c < cols) {
      if (line.lift((c.toInt).toInt).getOrElse(null) == BigInt(1)) {
