// Generated by Mochi v0.10.61 on 2025-08-08 19:11:23 GMT+7
...
def transpose(mat: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < n) {
    ...
    while (j < n) {
      row = row :+ (mat.lift((j.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((i.toInt).toInt).getOrElse(null))
      j = j + BigInt(1)
    }
    result = result :+ (row)
    i = i + BigInt(1)
...
def reverse_column(mat: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < BigInt((mat).size)) {
    ...
    while (j >= BigInt(0)) {
      row = row :+ (mat.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((j.toInt).toInt).getOrElse(null))
      j = j - BigInt(1)
    }
    result = result :+ (row)
    i = i + BigInt(1)
...
def row_to_string(row: ArrayBuffer[BigInt]): String = {
  ...
  while (i < BigInt((row).size)) {
    if (i == BigInt(0)) {
      line = (_str(row.lift((i.toInt).toInt).getOrElse(null))).toString
    } else {
      line = line + " " + _str(row.lift((i.toInt).toInt).getOrElse(null))
    }
    i = i + BigInt(1)
  }
  return line
