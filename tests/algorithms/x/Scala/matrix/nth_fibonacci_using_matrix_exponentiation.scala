// Generated by Mochi v0.10.61 on 2025-08-08 17:03:45 GMT+7
...
def multiply(matrix_a: ArrayBuffer[ArrayBuffer[BigInt]], matrix_b: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < n) {
    ...
    while (j < n) {
      ...
      while (k < n) {
        `val` = `val` + matrix_a((i.toInt).toInt)((k.toInt).toInt) * matrix_b((k.toInt).toInt)((j.toInt).toInt)
        k = k + BigInt(1)
      }
      row = row :+ (`val`)
      j = j + BigInt(1)
...
def nth_fibonacci_matrix(n: BigInt): BigInt = {
  ...
  return res_matrix((BigInt(0).toInt).toInt)((BigInt(0).toInt).toInt)
}
...
def main(): Any = {
  ...
  while (i < BigInt((ordinals).size)) {
    val ordinal: String = ordinals((i.toInt).toInt)
