// Generated by Mochi v0.10.61 on 2025-08-08 19:10:59 GMT+7
...
def multiply(matrix_a: ArrayBuffer[ArrayBuffer[BigInt]], matrix_b: ArrayBuffer[ArrayBuffer[BigInt]]): ArrayBuffer[ArrayBuffer[BigInt]] = {
  ...
  while (i < n) {
    ...
    while (j < n) {
      ...
      while (k < n) {
        `val` = `val` + matrix_a.lift((i.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((k.toInt).toInt).getOrElse(null) * matrix_b.lift((k.toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((j.toInt).toInt).getOrElse(null)
        k = k + BigInt(1)
      }
      row = row :+ (`val`)
      j = j + BigInt(1)
...
def nth_fibonacci_matrix(n: BigInt): BigInt = {
  ...
  return res_matrix.lift((BigInt(0).toInt).toInt).getOrElse(ArrayBuffer[BigInt]()).lift((BigInt(0).toInt).toInt).getOrElse(null)
}
