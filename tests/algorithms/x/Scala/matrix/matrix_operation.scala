// Generated by Mochi v0.10.61 on 2025-08-08 17:03:18 GMT+7
...
def add(matrices: ArrayBuffer[ArrayBuffer[ArrayBuffer[Double]]]): ArrayBuffer[ArrayBuffer[Double]] = {
  val rows: BigInt = BigInt((matrices((BigInt(0).toInt).toInt)).size)
  val cols: BigInt = BigInt((matrices((BigInt(0).toInt).toInt)((BigInt(0).toInt).toInt)).size)
  ...
  while (r < rows) {
    ...
    while (c < cols) {
      ...
      while (m < BigInt((matrices).size)) {
        sum = sum + matrices((m.toInt).toInt)((r.toInt).toInt)((c.toInt).toInt)
        m = m + BigInt(1)
      }
      row = row :+ (sum)
      c = c + BigInt(1)
...
def subtract(a: ArrayBuffer[ArrayBuffer[Double]], b: ArrayBuffer[ArrayBuffer[Double]]): ArrayBuffer[ArrayBuffer[Double]] = {
  ...
  while (r < rows) {
    ...
    var c: BigInt = BigInt(0)
    while (c < cols) {
      row = row :+ (a((r.toInt).toInt)((c.toInt).toInt) - b((r.toInt).toInt)((c.toInt).toInt))
      c = c + BigInt(1)
    }
    result = result :+ (row)
    r = r + BigInt(1)
...
def scalar_multiply(matrix: ArrayBuffer[ArrayBuffer[Double]], n: Double): ArrayBuffer[ArrayBuffer[Double]] = {
  ...
  while (i < BigInt((matrix).size)) {
    ...
    while (j < BigInt((matrix((i.toInt).toInt)).size)) {
      row = row :+ (matrix((i.toInt).toInt)((j.toInt).toInt) * n)
      j = j + BigInt(1)
    }
    result = result :+ (row)
    i = i + BigInt(1)
...
def multiply(a: ArrayBuffer[ArrayBuffer[Double]], b: ArrayBuffer[ArrayBuffer[Double]]): ArrayBuffer[ArrayBuffer[Double]] = {
  ...
  val colsA: BigInt = BigInt((a((BigInt(0).toInt).toInt)).size)
  val rowsB: BigInt = BigInt((b).size)
  val colsB: BigInt = BigInt((b((BigInt(0).toInt).toInt)).size)
  var result: ArrayBuffer[ArrayBuffer[Double]] = ArrayBuffer[ArrayBuffer[Double]]()
  var i: BigInt = BigInt(0)
  while (i < rowsA) {
    var row: ArrayBuffer[Double] = ArrayBuffer[Double]()
...
def transpose(matrix: ArrayBuffer[ArrayBuffer[Double]]): ArrayBuffer[ArrayBuffer[Double]] = {
  ...
  while (c < cols) {
    ...
    var r: BigInt = BigInt(0)
    while (r < rows) {
      row = row :+ (matrix((r.toInt).toInt)((c.toInt).toInt))
      r = r + BigInt(1)
    }
    result = result :+ (row)
    c = c + BigInt(1)
...
def minor(matrix: ArrayBuffer[ArrayBuffer[Double]], row: BigInt, column: BigInt): ArrayBuffer[ArrayBuffer[Double]] = {
  ...
  while (i < BigInt((matrix).size)) {
    if (i != row) {
      ...
      while (j < BigInt((matrix((i.toInt).toInt)).size)) {
        if (j != column) {
          new_row = new_row :+ (matrix((i.toInt).toInt)((j.toInt).toInt))
        }
        j = j + BigInt(1)
      }
...
def determinant(matrix: ArrayBuffer[ArrayBuffer[Double]]): Double = {
  if (BigInt((matrix).size) == BigInt(1)) {
    return matrix((BigInt(0).toInt).toInt)((BigInt(0).toInt).toInt)
  }
  var det: Double = 0.0
  var c: BigInt = BigInt(0)
  while (c < BigInt((matrix((BigInt(0).toInt).toInt)).size)) {
    val sub: ArrayBuffer[ArrayBuffer[Double]] = minor(matrix, BigInt(0), c)
    val sign: Double = (if (c % BigInt(2) == BigInt(0)) 1.0 else -1.0)
    det = det + matrix((BigInt(0).toInt).toInt)((c.toInt).toInt) * determinant(sub) * sign
    c = c + BigInt(1)
  }
  return det
}
...
def inverse(matrix: ArrayBuffer[ArrayBuffer[Double]]): ArrayBuffer[ArrayBuffer[Double]] = {
  ...
  while (i < size) {
    ...
    while (j < size) {
      ...
      row = row :+ (matrix_minor((i.toInt).toInt)((j.toInt).toInt) * sign)
      j = j + BigInt(1)
    }
