// Generated by Mochi v0.10.61 on 2025-08-08 17:02:30 GMT+7
...
def parse_moves(input_str: String): ArrayBuffer[Coord] = {
  val pairs = ArrayBuffer(input_str.split(","): _*)
  var moves: ArrayBuffer[Coord] = ArrayBuffer[Coord]()
  var i: BigInt = BigInt(0)
  while (i < BigInt((pairs).size)) {
    val pair: String = pairs((i.toInt).toInt)
    ...
    val x: BigInt = to_int(numbers((BigInt(0).toInt).toInt))
    val y: BigInt = to_int(numbers((BigInt(1).toInt).toInt))
...
def validate_matrix_content(matrix: ArrayBuffer[String], size: BigInt): Any = {
  ...
  var i: BigInt = BigInt(0)
  while (i < size) {
    val row: String = matrix((i.toInt).toInt)
...
def validate_moves(moves: ArrayBuffer[Coord], size: BigInt): Any = {
  ...
  while (i < BigInt((moves).size)) {
    val mv: Coord = moves((i.toInt).toInt)
...
def find_repeat(matrix_g: ArrayBuffer[ArrayBuffer[String]], row: BigInt, _column: BigInt, size: BigInt): ArrayBuffer[Coord] = {
  ...
  val color: String = matrix_g((column.toInt).toInt)((row.toInt).toInt)
  if (color == "-") {
    return repeated
  }
  ...
  while (BigInt((stack).size) > BigInt(0)) {
    _ct8.breakable {
      ...
      val pos: Coord = stack((idx.toInt).toInt)
      ...
      visited = visited :+ (pos)
      if (matrix_g((pos.asInstanceOf[Coord].x.toInt).toInt)((pos.asInstanceOf[Coord].y.toInt).toInt) == color) {
...
def move_x(matrix_g: ArrayBuffer[ArrayBuffer[String]], column: BigInt, size: BigInt): ArrayBuffer[ArrayBuffer[String]] = {
  ...
  while (row < size) {
    val `val`: String = matrix_g((row.toInt).toInt)((column.toInt).toInt)
    if (`val` != "-") {
  ...
  row = BigInt(0)
  while (row < size) {
    matrix_g((row).toInt)((column).toInt) = new_list((row.toInt).toInt)
    row = row + BigInt(1)
  }
  return matrix_g
}
...
def move_y(matrix_g: ArrayBuffer[ArrayBuffer[String]], size: BigInt): ArrayBuffer[ArrayBuffer[String]] = {
  ...
  _br11.breakable {
    while (column >= BigInt(0)) {
      ...
      _br12.breakable {
        while (row < size) {
          if (matrix_g((row.toInt).toInt)((column.toInt).toInt) != "-") {
            all_empty = false
            _br12.break()
          }
          row = row + BigInt(1)
  ...
  while (i < BigInt((empty_cols).size)) {
    val col: BigInt = empty_cols((i.toInt).toInt)
    var c: BigInt = col + BigInt(1)
    while (c < size) {
      var r: BigInt = BigInt(0)
      while (r < size) {
        matrix_g((r).toInt)((c - BigInt(1)).toInt) = matrix_g((r.toInt).toInt)((c.toInt).toInt)
        r = r + BigInt(1)
      }
      c = c + BigInt(1)
    }
...
def play(_matrix_g: ArrayBuffer[ArrayBuffer[String]], pos_x: BigInt, pos_y: BigInt, size: BigInt): PlayResult = {
  ...
  if (BigInt((same_colors).size) != BigInt(0)) {
    ...
    while (i < BigInt((same_colors).size)) {
      val p: Coord = same_colors((i.toInt).toInt)
