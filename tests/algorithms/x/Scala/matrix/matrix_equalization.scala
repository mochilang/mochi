// Generated by Mochi v0.10.61 on 2025-08-08 17:02:59 GMT+7
...
def unique(nums: ArrayBuffer[BigInt]): ArrayBuffer[BigInt] = {
  ...
  _br0.breakable {
    while (i < BigInt((nums).size)) {
      val v: BigInt = nums((i.toInt).toInt)
      var found: Boolean = false
      var j: BigInt = BigInt(0)
      val _br1 = new Breaks
      _br1.breakable {
        while (j < BigInt((res).size)) {
          if (res((j.toInt).toInt) == v) {
            found = true
            _br1.break()
          }
          j = j + BigInt(1)
...
def array_equalization(vector: ArrayBuffer[BigInt], step_size: BigInt): BigInt = {
  ...
  while (i < BigInt((elems).size)) {
    val target: BigInt = elems((i.toInt).toInt)
    var idx: BigInt = BigInt(0)
    var updates: BigInt = BigInt(0)
    while (idx < BigInt((vector).size)) {
      if (vector((idx.toInt).toInt) != target) {
