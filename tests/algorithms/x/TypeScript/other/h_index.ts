// Generated by Mochi v0.10.59 on 2025-08-07 08:17:44 GMT+7
...
function h_index(citations: number[]): number {
  ...
  while ((idx < _len(citations))) {
    if ((citations[(idx) < 0 ? citations.length + (idx) : idx] < 0)) {
  ...
  let i: number = 0;
  while ((i < n)) {
    if ((sorted[(Math.trunc(((n - 1) - i))) < 0 ? sorted.length + (Math.trunc(((n - 1) - i))) : Math.trunc(((n - 1) - i))] <= i)) {
      return i;
    }
    i = (i + 1);
  }
...
(() => {
  ...
  const _start = _now()
  console.log(_str(_str(h_index([3, 0, 6, 1, 5]))));
  console.log(_str(_str(h_index([1, 3, 1]))));
  console.log(_str(_str(h_index([1, 2, 3]))));
  const _end = _now()
