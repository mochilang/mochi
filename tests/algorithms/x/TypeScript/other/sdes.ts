// Generated by Mochi v0.10.59 on 2025-08-07 08:17:56 GMT+7

function apply_table(inp: string, table: number[]): string {
  let res: string = "";
  let i: number = 0;
  while ((i < _len(table))) {
    let idx: number = (table[(i) < 0 ? table.length + (i) : i] - 1);
    if ((idx < 0)) {
      idx = (_len(inp) - 1);
    }
...
function apply_sbox(s: number[][], data: string): string {
  ...
  let val: number = s[(row) < 0 ? s.length + (row) : row][(col) < 0 ? s[(row) < 0 ? s.length + (row) : row].length + (col) : col];
