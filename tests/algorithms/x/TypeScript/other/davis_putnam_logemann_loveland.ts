// Generated by Mochi v0.10.59 on 2025-08-07 08:17:37 GMT+7
...
function new_clause(lits: string[]): Clause {
  ...
  let i: number = 0;
  while ((i < _len(lits))) {
    let lit: string = lits[(i) < 0 ? lits.length + (i) : i];
    m[(lit) < 0 ? m.length + (lit) : lit] = (0 - 1);
    names.push(lit);
    i = (i + 1);
  }
...
function assign_clause(c: Clause, model: Record<string, number>): Clause {
  ...
  while ((i < _len(c.names))) {
    let lit: string = c.names[(i) < 0 ? c.names.length + (i) : i];
    let _symbol: string = (lit).substring(0, 2);
    if ((_symbol in model)) {
      let value: number = model[(_symbol) < 0 ? model.length + (_symbol) : _symbol];
      if ((((lit).substring((_len(lit) - 1), _len(lit)) == "'") && (value != (0 - 1)))) {
        value = (1 - value);
      }
      lits[(lit) < 0 ? lits.length + (lit) : lit] = value;
    }
    i = (i + 1);
  }
  c.literals = lits;
...
function evaluate_clause(c: Clause, model: Record<string, number>): EvalResult {
  ...
  while ((i < _len(c.names))) {
    let lit: string = c.names[(i) < 0 ? c.names.length + (i) : i];
    let value: number = c.literals[(lit) < 0 ? c.literals.length + (lit) : lit];
    if ((value == 1)) {
      return {"value": 1, "clause": c};
    }
    if ((value == (0 - 1))) {
  ...
  while ((i < _len(c.names))) {
    let lit: string = c.names[(i) < 0 ? c.names.length + (i) : i];
    if ((c.literals[(lit) < 0 ? c.literals.length + (lit) : lit] == 1)) {
      any_true = 1;
    }
    i = (i + 1);
  }
...
function remove_symbol(symbols: string[], s: string): string[] {
  ...
  while ((i < _len(symbols))) {
    if ((symbols[(i) < 0 ? symbols.length + (i) : i] != s)) {
      res.push(symbols[(i) < 0 ? symbols.length + (i) : i]);
    }
    i = (i + 1);
  }
  return res;
...
function dpll_algorithm(clauses: Clause[], symbols: string[], model: Record<string, number>): DPLLResult {
  ...
  while ((i < _len(clauses))) {
    let ev: EvalResult = evaluate_clause(clauses[(i) < 0 ? clauses.length + (i) : i], model);
    clauses[(i) < 0 ? clauses.length + (i) : i] = ev.clause;
    if ((ev.value == 0)) {
  ...
  let p: string = symbols[(Math.trunc(0)) < 0 ? symbols.length + (Math.trunc(0)) : Math.trunc(0)];
  let rest: string[] = remove_symbol(symbols, p);
  let tmp1: Record<string, number> = model;
  let tmp2: Record<string, number> = model;
  tmp1[(p) < 0 ? tmp1.length + (p) : p] = 1;
  tmp2[(p) < 0 ? tmp2.length + (p) : p] = 0;
...
function str_formula(f: Formula): string {
  ...
  while ((i < _len(f.clauses))) {
    line = (line + str_clause(f.clauses[(i) < 0 ? f.clauses.length + (i) : i]));
