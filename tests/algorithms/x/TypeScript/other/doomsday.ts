// Generated by Mochi v0.10.69 on 2025-08-22 13:10:58 GMT+7
...
function get_week_day(year: number, month: number, day: number): string {
  ...
  let century_anchor: bigint = (((5 * (century % 4)) + 2) % 7);
  let centurian: bigint = (year % 100);
  let centurian_m: bigint = (centurian % 12);
  let dooms_day: bigint = ((((Math.trunc(centurian / 12) + centurian_m) + Math.trunc(centurian_m / 4)) + century_anchor) % 7);
  ...
  let week_day: bigint = (((dooms_day + day) - day_anchor) % 7);
  if ((week_day < 0)) {
    week_day = (week_day + 7);
  }
