// Generated by Mochi v0.10.59 on 2025-08-07 08:17:38 GMT+7
...
function get_week_day(year: number, month: number, day: number): string {
  ...
  let day_anchor: number = ((((year % 4) != 0) || ((centurian == 0) && ((year % 400) != 0))) ? DOOMSDAY_NOT_LEAP[(Math.trunc((month - 1))) < 0 ? DOOMSDAY_NOT_LEAP.length + (Math.trunc((month - 1))) : Math.trunc((month - 1))] : DOOMSDAY_LEAP[(Math.trunc((month - 1))) < 0 ? DOOMSDAY_LEAP.length + (Math.trunc((month - 1))) : Math.trunc((month - 1))]);
  let week_day: number = (((dooms_day + day) - day_anchor) % 7);
  if ((week_day < 0)) {
    week_day = (week_day + 7);
  }
  return WEEK_DAY_NAMES[(week_day) < 0 ? WEEK_DAY_NAMES.length + (week_day) : week_day];
}
var _nowSeed = 0;
