// Generated by Mochi v0.10.69 on 2025-08-22 13:11:01 GMT+7

export interface EasterDate { month: number; day: number }
function gauss_easter(year: number): EasterDate {
  let metonic_cycle: bigint = (year % 19);
  let julian_leap_year: bigint = (year % 4);
  let non_leap_year: bigint = (year % 7);
  let leap_day_inhibits: number = Math.trunc(year / 100);
  let lunar_orbit_correction: bigint = Math.trunc((13 + (8 * leap_day_inhibits)) / 25);
