// Generated by Mochi v0.10.59 on 2025-08-07 08:17:46 GMT+7
...
function lfu_get(cache: LFUCache, key: number): GetResult {
  ...
  let e: Entry = entries[(idx) < 0 ? entries.length + (idx) : idx];
  e.freq = (e.freq + 1);
  let new_tick: number = (cache.tick + 1);
  e.order = new_tick;
  entries[(idx) < 0 ? entries.length + (idx) : idx] = e;
...
function remove_lfu(entries: Entry[]): Entry[] {
  ...
  while ((i < _len(entries))) {
    let e: Entry = entries[(i) < 0 ? entries.length + (i) : i];
    let m: Entry = entries[(min_idx) < 0 ? entries.length + (min_idx) : min_idx];
  ...
  while ((j < _len(entries))) {
    if ((j != min_idx)) {
      res.push(entries[(j) < 0 ? entries.length + (j) : j]);
    }
    j = (j + 1);
  }
  return res;
