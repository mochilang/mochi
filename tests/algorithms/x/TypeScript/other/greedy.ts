// Generated by Mochi v0.10.59 on 2025-08-07 08:17:41 GMT+7
...
function build_menu(names: string[], values: number[], weights: number[]): Thing[] {
  ...
  while ((((i < _len(values)) && (i < _len(names))) && (i < _len(weights)))) {
    menu.push({"name": names[(i) < 0 ? names.length + (i) : i], "value": values[(i) < 0 ? values.length + (i) : i], "weight": weights[(i) < 0 ? weights.length + (i) : i]});
    i = (i + 1);
  }
  return menu;
}
function sort_desc(items: Thing[], key_func): Thing[] {
  ...
  let i: number = 0;
  while ((i < _len(items))) {
    arr.push(items[(i) < 0 ? items.length + (i) : i]);
    i = (i + 1);
  }
  let j: number = 1;
  while ((j < _len(arr))) {
    let key_item: Thing = arr[(j) < 0 ? arr.length + (j) : j];
    let key_val: number = key_func(key_item);
    let k: number = (j - 1);
    while (((k >= 0) && (key_func(arr[(k) < 0 ? arr.length + (k) : k]) < key_val))) {
      arr[((k + 1)) < 0 ? arr.length + ((k + 1)) : (k + 1)] = arr[(k) < 0 ? arr.length + (k) : k];
      k = (k - 1);
    }
    arr[((k + 1)) < 0 ? arr.length + ((k + 1)) : (k + 1)] = key_item;
    j = (j + 1);
  }
  return arr;
}
...
function greedy(items: Thing[], max_cost: number, key_func): GreedyResult {
  ...
  while ((i < _len(items_copy))) {
    let it: Thing = items_copy[(i) < 0 ? items_copy.length + (i) : i];
...
function list_to_string(ts: Thing[]): string {
  ...
  let i: number = 0;
  while ((i < _len(ts))) {
    s = (s + thing_to_string(ts[(i) < 0 ? ts.length + (i) : i]));
    if ((i < (_len(ts) - 1))) {
      s = (s + ", ");
    }
    i = (i + 1);
...
res = greedy(foods, 500.0, get_value)
  ...
  console.log(_str(_str(res.total_value)));
  const _end = _now()
