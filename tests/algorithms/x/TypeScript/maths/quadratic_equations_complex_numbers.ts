// Generated by Mochi v0.10.59 on 2025-08-07 00:18:26 GMT+7
...
function main() {
  ...
  if ((_len(roots) == 2)) {
    console.log(_str(((("The solutions are: " + root_str(roots[(Math.trunc(0)) < 0 ? roots.length + (Math.trunc(0)) : Math.trunc(0)])) + " and ") + root_str(roots[(Math.trunc(1)) < 0 ? roots.length + (Math.trunc(1)) : Math.trunc(1)]))));
  }
}
var _nowSeed = 0;
