// Generated by Mochi v0.10.59 on 2025-08-07 00:18:28 GMT+7
...
function fft(a: Complex[], invert: boolean): Complex[] {
  let n: number = _len(a);
  if ((n == 1)) {
    return [a[(Math.trunc(0)) < 0 ? a.length + (Math.trunc(0)) : Math.trunc(0)]];
  }
  let a0: Complex[] = [];
  let a1: Complex[] = [];
  let i: number = 0;
  while ((i < Math.trunc(n / 2))) {
    a0.push(a[(Math.trunc((2 * i))) < 0 ? a.length + (Math.trunc((2 * i))) : Math.trunc((2 * i))]);
    a1.push(a[(Math.trunc(((2 * i) + 1))) < 0 ? a.length + (Math.trunc(((2 * i) + 1))) : Math.trunc(((2 * i) + 1))]);
    i = (i + 1);
  }
  ...
  i = 0;
  while ((i < Math.trunc(n / 2))) {
    let t: Complex = c_mul(w, y1[(i) < 0 ? y1.length + (i) : i]);
    let u: Complex = y0[(i) < 0 ? y0.length + (i) : i];
    ...
    y[(i) < 0 ? y.length + (i) : i] = even;
    y[((i + Math.trunc(n / 2))) < 0 ? y.length + ((i + Math.trunc(n / 2))) : (i + Math.trunc(n / 2))] = odd;
    w = c_mul(w, wn);
    i = (i + 1);
  }
  return y;
...
function multiply_poly(a: number[], b: number[]): number[] {
  ...
  let i: number = 0;
  while ((i < _len(a))) {
    fa[(i) < 0 ? fa.length + (i) : i] = {"re": a[(i) < 0 ? a.length + (i) : i], "im": 0.0};
    i = (i + 1);
  }
  i = 0;
  while ((i < _len(b))) {
    fb[(i) < 0 ? fb.length + (i) : i] = {"re": b[(i) < 0 ? b.length + (i) : i], "im": 0.0};
    i = (i + 1);
  }
  fa = fft(fa, false);
  fb = fft(fb, false);
  i = 0;
  while ((i < n)) {
    fa[(i) < 0 ? fa.length + (i) : i] = c_mul(fa[(i) < 0 ? fa.length + (i) : i], fb[(i) < 0 ? fb.length + (i) : i]);
    i = (i + 1);
  }
  fa = fft(fa, true);
  let res: number[] = [];
  ...
  while ((i < ((_len(a) + _len(b)) - 1))) {
    let val: Complex = fa[(i) < 0 ? fa.length + (i) : i];
    res.push(round_to(val.re, 8));
    i = (i + 1);
  }
  while (((_len(res) > 0) && (res[(Math.trunc((_len(res) - 1))) < 0 ? res.length + (Math.trunc((_len(res) - 1))) : Math.trunc((_len(res) - 1))] == 0.0))) {
