// Generated by Mochi v0.10.59 on 2025-08-07 00:17:09 GMT+7
...
function dual_add(a: Dual, b: Dual): Dual {
  ...
  while ((i < _len(a.duals))) {
    s_dual.push(a.duals[(i) < 0 ? a.duals.length + (i) : i]);
    i = (i + 1);
  }
  let o_dual: number[] = [];
  let j: number = 0;
  while ((j < _len(b.duals))) {
    o_dual.push(b.duals[(j) < 0 ? b.duals.length + (j) : j]);
    j = (j + 1);
  }
  if ((_len(s_dual) > _len(o_dual))) {
    let diff: number = (_len(s_dual) - _len(o_dual));
    let k: number = 0;
    while ((k < diff)) {
      o_dual.push(1.0);
      k = (k + 1);
    ...
    if ((_len(s_dual) < _len(o_dual))) {
      let diff2: number = (_len(o_dual) - _len(s_dual));
      let k2: number = 0;
      while ((k2 < diff2)) {
        s_dual.push(1.0);
        k2 = (k2 + 1);
  ...
  while ((idx < _len(s_dual))) {
    new_duals.push((s_dual[(idx) < 0 ? s_dual.length + (idx) : idx] + o_dual[(idx) < 0 ? o_dual.length + (idx) : idx]));
    idx = (idx + 1);
  }
...
function dual_add_real(a: Dual, b: number): Dual {
  ...
  let i: number = 0;
  while ((i < _len(a.duals))) {
    ds.push(a.duals[(i) < 0 ? a.duals.length + (i) : i]);
    i = (i + 1);
  }
  return {"real": (a.real + b), "duals": ds};
}
function dual_mul(a: Dual, b: Dual): Dual {
  let new_len: number = ((_len(a.duals) + _len(b.duals)) + 1);
  let new_duals: number[] = [];
  let idx: number = 0;
  while ((idx < new_len)) {
    new_duals.push(0.0);
  ...
  while ((i < _len(a.duals))) {
    ...
    while ((j < _len(b.duals))) {
      ...
      let val: number = (new_duals[(pos) < 0 ? new_duals.length + (pos) : pos] + (a.duals[(i) < 0 ? a.duals.length + (i) : i] * b.duals[(j) < 0 ? b.duals.length + (j) : j]));
      new_duals[(pos) < 0 ? new_duals.length + (pos) : pos] = val;
      j = (j + 1);
    }
    i = (i + 1);
  }
  ...
  while ((k < _len(a.duals))) {
    let val: number = (new_duals[(k) < 0 ? new_duals.length + (k) : k] + (a.duals[(k) < 0 ? a.duals.length + (k) : k] * b.real));
    new_duals[(k) < 0 ? new_duals.length + (k) : k] = val;
    k = (k + 1);
  }
  let l: number = 0;
  while ((l < _len(b.duals))) {
    let val: number = (new_duals[(l) < 0 ? new_duals.length + (l) : l] + (b.duals[(l) < 0 ? b.duals.length + (l) : l] * a.real));
    new_duals[(l) < 0 ? new_duals.length + (l) : l] = val;
    l = (l + 1);
  }
  return {"real": (a.real * b.real), "duals": new_duals};
}
function dual_mul_real(a: Dual, b: number): Dual {
  ...
  let i: number = 0;
  while ((i < _len(a.duals))) {
    ds.push((a.duals[(i) < 0 ? a.duals.length + (i) : i] * b));
    i = (i + 1);
  }
  return {"real": (a.real * b), "duals": ds};
}
...
function differentiate(func, position: number, order: number): number {
  ...
  return (result.duals[(Math.trunc((order - 1))) < 0 ? result.duals.length + (Math.trunc((order - 1))) : Math.trunc((order - 1))] * factorial(order));
}
