// Generated by Mochi v0.10.59 on 2025-08-07 00:17:10 GMT+7
...
function analyze_text(text: string): TextCounts {
  ...
  if ((last in single)) {
    single[(last) < 0 ? single.length + (last) : last] = (single[(last) < 0 ? single.length + (last) : last] + 1);
  } else {
    single[(last) < 0 ? single.length + (last) : last] = 1;
  }
  let first = (text).substring(0, 1);
  let pair0: string = (" " + first);
  double[(pair0) < 0 ? double.length + (pair0) : pair0] = 1;
  let i: number = 0;
  while ((i < (n - 1))) {
    let ch: string = (text).substring(i, (i + 1));
    if ((ch in single)) {
      single[(ch) < 0 ? single.length + (ch) : ch] = (single[(ch) < 0 ? single.length + (ch) : ch] + 1);
    } else {
      single[(ch) < 0 ? single.length + (ch) : ch] = 1;
    }
    let seq: string = (text).substring(i, (i + 2));
    if ((seq in double)) {
      double[(seq) < 0 ? double.length + (seq) : seq] = (double[(seq) < 0 ? double.length + (seq) : seq] + 1);
    } else {
      double[(seq) < 0 ? double.length + (seq) : seq] = 1;
    }
    i = (i + 1);
  }
  return {single, double};
...
function calculate_entropy(text: string) {
  ...
  for (const ch in counts.single) {
    total1 = (total1 + counts.single[(ch) < 0 ? counts.single.length + (ch) : ch]);
  }
  let h1: number = 0.0;
  let i: number = 0;
  while ((i < _len(alphas))) {
    ...
    if ((ch in counts.single)) {
      let prob: number = (counts.single[(ch) < 0 ? counts.single.length + (ch) : ch] / total1);
  ...
  for (const seq in counts.double) {
    total2 = (total2 + counts.double[(seq) < 0 ? counts.double.length + (seq) : seq]);
  }
  let h2: number = 0.0;
  let a0: number = 0;
  while ((a0 < _len(alphas))) {
    ...
    while ((a1 < _len(alphas))) {
      ...
      if ((seq in counts.double)) {
        let prob: number = (counts.double[(seq) < 0 ? counts.double.length + (seq) : seq] / total2);
