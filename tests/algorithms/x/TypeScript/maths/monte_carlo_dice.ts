// Generated by Mochi v0.10.59 on 2025-08-07 00:17:51 GMT+7
...
function throw_dice(num_throws: number, num_dice: number): number[] {
  ...
  let max_sum: number = ((num_dice * 6) + 1);
  let i: number = 0;
  while ((i < max_sum)) {
    count_of_sum.push(0);
    i = (i + 1);
  ...
  while ((t < num_throws)) {
    ...
    count_of_sum[(s) < 0 ? count_of_sum.length + (s) : s] = (count_of_sum[(s) < 0 ? count_of_sum.length + (s) : s] + 1);
    t = (t + 1);
  }
  let probability: number[] = [];
  i = num_dice;
  while ((i < max_sum)) {
    let p: number = ((count_of_sum[(i) < 0 ? count_of_sum.length + (i) : i] * 100.0) / num_throws);
