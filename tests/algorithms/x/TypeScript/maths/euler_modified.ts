// Generated by Mochi v0.10.59 on 2025-08-07 00:17:12 GMT+7
...
function euler_modified(ode_func, y0: number, x0: number, step: number, x_end: number): number[] {
  ...
  let k: number = 0;
  while ((k < n)) {
    let y_predict: number = (y[(k) < 0 ? y.length + (k) : k] + (step * ode_func(x, y[(k) < 0 ? y.length + (k) : k])));
    let slope1: number = ode_func(x, y[(k) < 0 ? y.length + (k) : k]);
    let slope2: number = ode_func((x + step), y_predict);
    let y_next: number = (y[(k) < 0 ? y.length + (k) : k] + ((step / 2.0) * (slope1 + slope2)));
    y.push(y_next);
    x = (x + step);
    k = (k + 1);
  }
...
function main() {
  ...
  console.log(_str(y1[(Math.trunc((_len(y1) - 1))) < 0 ? y1.length + (Math.trunc((_len(y1) - 1))) : Math.trunc((_len(y1) - 1))]));
  let y2: number[] = euler_modified(f2, 1.0, 0.0, 0.1, 0.3);
  console.log(_str(y2[(Math.trunc((_len(y2) - 1))) < 0 ? y2.length + (Math.trunc((_len(y2) - 1))) : Math.trunc((_len(y2) - 1))]));
}
var _nowSeed = 0;
