// Generated by Mochi v0.10.59 on 2025-08-07 00:18:25 GMT+7
...
function vector_norm(v: number[]): number {
  ...
  let i: number = 0;
  while ((i < _len(v))) {
    sum = (sum + (v[(i) < 0 ? v.length + (i) : i] * v[(i) < 0 ? v.length + (i) : i]));
    i = (i + 1);
  }
...
function matmul(a: number[][], b: number[][]): number[][] {
  let m: number = _len(a);
  let n: number = _len(a[(Math.trunc(0)) < 0 ? a.length + (Math.trunc(0)) : Math.trunc(0)]);
...
function qr_decomposition(a: number[][]): QR {
  ...
  while ((k < (t - 1))) {
    ...
    let i: number = k;
    while ((i < m)) {
      x.push(r[(i) < 0 ? r.length + (i) : i][(k) < 0 ? r[(i) < 0 ? r.length + (i) : i].length + (k) : k]);
      i = (i + 1);
    }
    let e1: number[] = [];
    i = 0;
    ...
    let s: number = (sign(x[(Math.trunc(0)) < 0 ? x.length + (Math.trunc(0)) : Math.trunc(0)]) * alpha);
    let v: number[] = [];
    i = 0;
    while ((i < _len(x))) {
      v.push((x[(i) < 0 ? x.length + (i) : i] + (s * e1[(i) < 0 ? e1.length + (i) : i])));
      i = (i + 1);
    }
    let vnorm: number = vector_norm(v);
    i = 0;
    while ((i < _len(v))) {
      v[(i) < 0 ? v.length + (i) : i] = (v[(i) < 0 ? v.length + (i) : i] / vnorm);
      i = (i + 1);
    }
    ...
    while ((i < size)) {
      ...
      while ((j < size)) {
        ...
        row.push((delta - ((2.0 * v[(i) < 0 ? v.length + (i) : i]) * v[(j) < 0 ? v.length + (j) : j])));
        j = (j + 1);
      }
      qk_small.push(row);
      i = (i + 1);
    ...
    while ((i < size)) {
      let j: number = 0;
      while ((j < size)) {
        qk[((k + i)) < 0 ? qk.length + ((k + i)) : (k + i)][((k + j)) < 0 ? qk[((k + i)) < 0 ? qk.length + ((k + i)) : (k + i)].length + ((k + j)) : (k + j)] = qk_small[(i) < 0 ? qk_small.length + (i) : i][(j) < 0 ? qk_small[(i) < 0 ? qk_small.length + (i) : i].length + (j) : j];
        j = (j + 1);
      }
      i = (i + 1);
    }
