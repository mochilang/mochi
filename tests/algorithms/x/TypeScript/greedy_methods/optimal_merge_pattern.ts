// Generated by Mochi v0.10.59 on 2025-08-07 00:03:04 GMT+7

function index_of_min(xs: number[]): number {
  let min_idx: number = 0;
  let i: number = 1;
  while ((i < _len(xs))) {
    if ((xs[(i) < 0 ? xs.length + (i) : i] < xs[(min_idx) < 0 ? xs.length + (min_idx) : min_idx])) {
      min_idx = i;
    }
    i = (i + 1);
  }
...
function optimal_merge_pattern(files: number[]): number {
  ...
  while ((_len(arr) > 1)) {
    ...
    while ((k < 2)) {
      ...
      temp = (temp + arr[(min_idx) < 0 ? arr.length + (min_idx) : min_idx]);
