// Generated by Mochi v0.10.59 on 2025-08-06 23:29:01 GMT+7
...
function get_gas_stations(gas_quantities: number[], costs: number[]): GasStation[] {
  ...
  while ((i < _len(gas_quantities))) {
    stations.push({"gas_quantity": gas_quantities[(i) < 0 ? gas_quantities.length + (i) : i], "cost": costs[(i) < 0 ? costs.length + (i) : i]});
    i = (i + 1);
  }
  return stations;
}
function can_complete_journey(gas_stations: GasStation[]): number {
  ...
  while ((i < _len(gas_stations))) {
    total_gas = (total_gas + gas_stations[(i) < 0 ? gas_stations.length + (i) : i].gas_quantity);
    total_cost = (total_cost + gas_stations[(i) < 0 ? gas_stations.length + (i) : i].cost);
    i = (i + 1);
  }
  ...
  i = 0;
  while ((i < _len(gas_stations))) {
    let station: GasStation = gas_stations[(i) < 0 ? gas_stations.length + (i) : i];
...
example1 = get_gas_stations([1, 2, 3, 4, 5], [3, 4, 5, 1, 2])
  console.log(_str(_str(can_complete_journey(example1))));
example2 = get_gas_stations([2, 3, 4], [3, 4, 3])
  console.log(_str(_str(can_complete_journey(example2))));
  const _end = _now()
