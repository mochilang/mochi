// Generated by Mochi v0.10.59 on 2025-08-06 23:28:59 GMT+7
  ...
  while ((i < _len(arr))) {
    ...
    while ((j >= 0)) {
      ...
      if (((current.value / current.weight) < (key.value / key.weight))) {
        arr[(Math.trunc((j + 1))) < 0 ? arr.length + (Math.trunc((j + 1))) : Math.trunc((j + 1))] = current;
        j = (j - 1);
      } else {
        break
      }
    }
    arr[(Math.trunc((j + 1))) < 0 ? arr.length + (Math.trunc((j + 1))) : Math.trunc((j + 1))] = key;
    i = (i + 1);
  }
  return arr;
}
