// Generated by Mochi v0.10.59 on 2025-08-07 00:02:39 GMT+7
...
function key_fn(start: Pos, i: number, goal: Pos, g_func: Record<string, number>): number {
  let g: number = g_func[(pos_key(start)) < 0 ? g_func.length + (pos_key(start)) : pos_key(start)];
...
function reconstruct(back_pointer: Record<string, Pos>, goal: Pos, start: Pos): Pos[] {
  ...
  while (!pos_equal(current, start)) {
    current = back_pointer[(key) < 0 ? back_pointer.length + (key) : key];
    key = pos_key(current);
    path.push(current);
  }
  let rev: Pos[] = [];
...
function multi_a_star(start: Pos, goal: Pos, n_heuristic: number) {
  ...
  g_function[(pos_key(start)) < 0 ? g_function.length + (pos_key(start)) : pos_key(start)] = 0.0;
  g_function[(pos_key(goal)) < 0 ? g_function.length + (pos_key(goal)) : pos_key(goal)] = INF;
  back_pointer[(pos_key(start)) < 0 ? back_pointer.length + (pos_key(start)) : pos_key(start)] = {"x": -1, "y": -1};
  back_pointer[(pos_key(goal)) < 0 ? back_pointer.length + (pos_key(goal)) : pos_key(goal)] = {"x": -1, "y": -1};
  visited[(pos_key(start)) < 0 ? visited.length + (pos_key(start)) : pos_key(start)] = true;
  let i: number = 0;
  while ((i < n_heuristic)) {
    open_list.push([]);
  ...
  while ((pq_minkey(open_list[(Math.trunc(0)) < 0 ? open_list.length + (Math.trunc(0)) : Math.trunc(0)]) < INF)) {
    ...
    visited[(ckey) < 0 ? visited.length + (ckey) : ckey] = true;
    ...
    while ((k < _len(neighs))) {
      ...
      if ((valid(nb) && (in_blocks(nb) == false))) {
        ...
        let tentative: number = (g_function[(ckey) < 0 ? g_function.length + (ckey) : ckey] + 1.0);
        if ((!(nkey in g_function) || (tentative < g_function[(nkey) < 0 ? g_function.length + (nkey) : nkey]))) {
          g_function[(nkey) < 0 ? g_function.length + (nkey) : nkey] = tentative;
          back_pointer[(nkey) < 0 ? back_pointer.length + (nkey) : nkey] = current.pos;
          i = 0;
          while ((i < n_heuristic)) {
