// Generated by Mochi v0.10.59 on 2025-08-07 00:01:36 GMT+7
...
function breath_first_search(g: Graph): Graph {
  ...
  parent[(g.source) < 0 ? parent.length + (g.source) : g.source] = g.source;
  let queue: string[] = [g.source];
  let idx: number = 0;
  while ((idx < _len(queue))) {
    let vertex: string = queue[(idx) < 0 ? queue.length + (idx) : idx];
    for (const adj of g.graph[(vertex) < 0 ? g.graph.length + (vertex) : vertex]) {
      if (!(adj in parent)) {
        parent[(adj) < 0 ? parent.length + (adj) : adj] = vertex;
        queue.push(adj);
      }
    }
    idx = (idx + 1);
...
function shortest_path(g: Graph, target: string): string {
  ...
  let p: string = g.parent[(target) < 0 ? g.parent.length + (target) : target];
