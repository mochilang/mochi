// Generated by Mochi v0.10.59 on 2025-08-07 00:02:06 GMT+7
...
function get_distinct_edge(edge_array: string[][][]): string[] {
  ...
  for (const row of edge_array) {
    for (const item of row) {
      let e: string = item[(Math.trunc(0)) < 0 ? item.length + (Math.trunc(0)) : Math.trunc(0)];
...
function get_bitcode(edge_array: string[][][], de: string): string {
  ...
  while ((i < _len(edge_array))) {
    ...
    for (const item of edge_array[(i) < 0 ? edge_array.length + (i) : i]) {
      if ((item[(Math.trunc(0)) < 0 ? item.length + (Math.trunc(0)) : Math.trunc(0)] == de)) {
        found = true;
        break
      }
    }
...
function get_frequency_table(edge_array: string[][][]): Record<string, string>[] {
  ...
  while ((i < _len(table))) {
    ...
    let j: number = (i + 1);
    while ((j < _len(table))) {
      if ((toi(table[(j) < 0 ? table.length + (j) : j].count) > toi(table[(max_i) < 0 ? table.length + (max_i) : max_i].count))) {
        max_i = j;
      }
      j = (j + 1);
    }
    let tmp: Record<string, string> = table[(i) < 0 ? table.length + (i) : i];
    table[(i) < 0 ? table.length + (i) : i] = table[(max_i) < 0 ? table.length + (max_i) : max_i];
    table[(max_i) < 0 ? table.length + (max_i) : max_i] = tmp;
    i = (i + 1);
  }
  return table;
}
function get_nodes(freq_table: Record<string, string>[]): NodesData {
  ...
  for (const f of freq_table) {
    ...
    if ((code in nodes)) {
      nodes[(code) < 0 ? nodes.length + (code) : code] = [...nodes[(code) < 0 ? nodes.length + (code) : code], edge];
    } else {
      nodes[(code) < 0 ? nodes.length + (code) : code] = [edge];
      keys.push(code);
    }
  }
...
function get_cluster(nodes: NodesData): ClusterData {
  ...
  while ((i < _len(nodes.keys))) {
    let code: string = nodes.keys[(i) < 0 ? nodes.keys.length + (i) : i];
    let wt: number = count_ones(code);
    if ((wt in clusters)) {
      clusters[(wt) < 0 ? clusters.length + (wt) : wt] = [...clusters[(wt) < 0 ? clusters.length + (wt) : wt], code];
    } else {
      clusters[(wt) < 0 ? clusters.length + (wt) : wt] = [code];
      weights.push(wt);
    }
    i = (i + 1);
  }
...
function get_support(clusters: ClusterData): number[] {
  ...
  while ((i < _len(clusters.weights))) {
    let w: number = clusters.weights[(i) < 0 ? clusters.weights.length + (i) : i];
...
function get_cluster_codes(clusters: ClusterData, wt: number): string[] {
  if ((wt in clusters.clusters)) {
    return clusters.clusters[(wt) < 0 ? clusters.clusters.length + (wt) : wt];
  }
  return [];
}
function create_edge(nodes: NodesData, graph: Record<string, string[]>, gkeys: string[], clusters: ClusterData, c1: number, maxk: number): string[] {
  ...
  while ((idx1 < _len(codes1))) {
    let i_code: string = codes1[(idx1) < 0 ? codes1.length + (idx1) : idx1];
    let count: number = 0;
    let c2: number = (c1 + 1);
    while ((c2 <= maxk)) {
      ...
      let j: number = 0;
      while ((j < _len(codes2))) {
        let j_code: string = codes2[(j) < 0 ? codes2.length + (j) : j];
        if (contains_bits(i_code, j_code)) {
          if ((i_code in graph)) {
            graph[(i_code) < 0 ? graph.length + (i_code) : i_code] = [...graph[(i_code) < 0 ? graph.length + (i_code) : i_code], j_code];
          } else {
            graph[(i_code) < 0 ? graph.length + (i_code) : i_code] = [j_code];
...
function construct_graph(clusters: ClusterData, nodes: NodesData): GraphData {
  ...
  while ((i < _len(top_codes))) {
    let code: string = top_codes[(i) < 0 ? top_codes.length + (i) : i];
    graph.Header = [...graph.Header, code];
    graph[(code) < 0 ? graph.length + (code) : code] = ["Header"];
    keys.push(code);
    i = (i + 1);
  }
  let c: number = 1;
...
function my_dfs(graph: Record<string, string[]>, start: string, end: string, path: string[]) {
  ...
  for (const node of graph[(start) < 0 ? graph.length + (start) : start]) {
...
function find_freq_subgraph_given_support(s: number, clusters: ClusterData, graph: GraphData) {
  ...
  let i: number = 0;
  while ((i < _len(codes))) {
    my_dfs(graph.edges, codes[(i) < 0 ? codes.length + (i) : i], "Header", []);
    i = (i + 1);
  }
}
function node_edges(nodes: NodesData, code: string): string[] {
  return nodes.map[(code) < 0 ? nodes.map.length + (code) : code];
}
function freq_subgraphs_edge_list(paths: string[][], nodes: NodesData): string[][][] {
  ...
  for (const path of paths) {
    ...
    let j: number = 0;
    while ((j < (_len(path) - 1))) {
      let code: string = path[(j) < 0 ? path.length + (j) : j];
      let edge_list: string[] = node_edges(nodes, code);
      let e: number = 0;
      while ((e < _len(edge_list))) {
        let edge: string = edge_list[(e) < 0 ? edge_list.length + (e) : e];
...
function print_all(nodes: NodesData, support: number[], clusters: ClusterData, graph: GraphData, freq_subgraph_edge_list: string[][][]) {
  ...
  while ((i < _len(nodes.keys))) {
    ...
    console.log(_str("[" + (nodes.map[(code) < 0 ? nodes.map.length + (code) : code]).join(' ') + "]"));
    i = (i + 1);
  }
  console.log(_str("\nSupport\n"));
  ...
  while ((j < _len(clusters.weights))) {
    let w: number = clusters.weights[(j) < 0 ? clusters.weights.length + (j) : j];
    console.log(_str(((_str(w) + ":") + _str(clusters.clusters[(w) < 0 ? clusters.clusters.length + (w) : w]))));
    j = (j + 1);
  }
  console.log(_str("\nGraph\n"));
  let k: number = 0;
  while ((k < _len(graph.keys))) {
    let key: string = graph.keys[(k) < 0 ? graph.keys.length + (k) : k];
    console.log(_str(key));
    console.log(_str("[" + (graph.edges[(key) < 0 ? graph.edges.length + (key) : key]).join(' ') + "]"));
    k = (k + 1);
  }
