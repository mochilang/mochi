// Generated by Mochi v0.10.59 on 2025-08-07 00:02:02 GMT+7
...
function dfs(u: number, graph: Record<number, number[]>, visited_edge: boolean[][], path: number[]): number[] {
  path.push(u);
  if ((u in graph)) {
    let neighbors: number[] = graph[(u) < 0 ? graph.length + (u) : u];
    let i: number = 0;
    while ((i < _len(neighbors))) {
      let v: number = neighbors[(i) < 0 ? neighbors.length + (i) : i];
      if ((visited_edge[(u) < 0 ? visited_edge.length + (u) : u][(v) < 0 ? visited_edge[(u) < 0 ? visited_edge.length + (u) : u].length + (v) : v] == false)) {
        visited_edge[(u) < 0 ? visited_edge.length + (u) : u][(v) < 0 ? visited_edge[(u) < 0 ? visited_edge.length + (u) : u].length + (v) : v] = true;
        visited_edge[(v) < 0 ? visited_edge.length + (v) : v][(u) < 0 ? visited_edge[(v) < 0 ? visited_edge.length + (v) : v].length + (u) : u] = true;
...
function check_circuit_or_path(graph: Record<number, number[]>, max_node: number): CheckResult {
  ...
  while ((i < max_node)) {
    if ((i in graph)) {
      if (((_len(graph[(i) < 0 ? graph.length + (i) : i]) % 2) == 1)) {
