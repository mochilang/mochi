// Generated by Mochi v0.10.59 on 2025-08-07 00:01:49 GMT+7
...
result = -1
  while ((_len(heap) > 0)) {
    ...
    let i: number = 1;
    while ((i < _len(heap))) {
      if ((heap[(i) < 0 ? heap.length + (i) : i].cost < heap[(best_idx) < 0 ? heap.length + (best_idx) : best_idx].cost)) {
        best_idx = i;
      }
      i = (i + 1);
    }
    let best: NodeCost = heap[(best_idx) < 0 ? heap.length + (best_idx) : best_idx];
    let new_heap: NodeCost[] = [];
    let j: number = 0;
    while ((j < _len(heap))) {
      if ((j != best_idx)) {
        new_heap.push(heap[(j) < 0 ? heap.length + (j) : j]);
      }
      j = (j + 1);
    }
    heap = new_heap;
    ...
    for (const v of G[(u) < 0 ? G.length + (u) : u]) {
      ...
      let next_cost = (cost + G[(u) < 0 ? G.length + (u) : u][(v) < 0 ? G[(u) < 0 ? G.length + (u) : u].length + (v) : v]);
...
result2 = -1
  while ((_len(heap2) > 0)) {
    ...
    let i2: number = 1;
    while ((i2 < _len(heap2))) {
      if ((heap2[(i2) < 0 ? heap2.length + (i2) : i2].cost < heap2[(best2_idx) < 0 ? heap2.length + (best2_idx) : best2_idx].cost)) {
        best2_idx = i2;
      }
      i2 = (i2 + 1);
    }
    let best2: NodeCost = heap2[(best2_idx) < 0 ? heap2.length + (best2_idx) : best2_idx];
    let new_heap2: NodeCost[] = [];
    let j2: number = 0;
    while ((j2 < _len(heap2))) {
      if ((j2 != best2_idx)) {
        new_heap2.push(heap2[(j2) < 0 ? heap2.length + (j2) : j2]);
      }
      j2 = (j2 + 1);
    }
    heap2 = new_heap2;
    ...
    visited2[(u2) < 0 ? visited2.length + (u2) : u2] = true;
    if ((u2 == "F")) {
      result2 = cost2;
      break
    }
    for (const v2 of G2[(u2) < 0 ? G2.length + (u2) : u2]) {
      if ((v2 in visited2)) {
        continue
      }
      let next_cost2 = (cost2 + G2[(u2) < 0 ? G2.length + (u2) : u2][(v2) < 0 ? G2[(u2) < 0 ? G2.length + (u2) : u2].length + (v2) : v2]);
...
result3 = -1
  while ((_len(heap3) > 0)) {
    ...
    let i3: number = 1;
    while ((i3 < _len(heap3))) {
      if ((heap3[(i3) < 0 ? heap3.length + (i3) : i3].cost < heap3[(best3_idx) < 0 ? heap3.length + (best3_idx) : best3_idx].cost)) {
        best3_idx = i3;
      }
      i3 = (i3 + 1);
    }
    let best3: NodeCost = heap3[(best3_idx) < 0 ? heap3.length + (best3_idx) : best3_idx];
    let new_heap3: NodeCost[] = [];
    let j3: number = 0;
    while ((j3 < _len(heap3))) {
      if ((j3 != best3_idx)) {
        new_heap3.push(heap3[(j3) < 0 ? heap3.length + (j3) : j3]);
      }
      j3 = (j3 + 1);
    }
    heap3 = new_heap3;
    ...
    visited3[(u3) < 0 ? visited3.length + (u3) : u3] = true;
    if ((u3 == "F")) {
      result3 = cost3;
      break
    }
    for (const v3 of G3[(u3) < 0 ? G3.length + (u3) : u3]) {
      if ((v3 in visited3)) {
        continue
      }
      let next_cost3 = (cost3 + G3[(u3) < 0 ? G3.length + (u3) : u3][(v3) < 0 ? G3[(u3) < 0 ? G3.length + (u3) : u3].length + (v3) : v3]);
