// Generated by Mochi v0.10.59 on 2025-08-06 23:28:48 GMT+7
...
function prims_algo(graph: Record<string, Record<string, number>>): MSTResult {
  ...
  queue = new_q;
  dist[start] = 0;
  for (const neighbour in Object.keys(graph[start])) {
    ...
    if ((dist[neighbour] > (dist[start] + w))) {
      ...
      while ((k < _len(queue))) {
        if ((queue[(k) < 0 ? queue.length + (k) : k].node == neighbour)) {
          queue[(k) < 0 ? queue.length + (k) : k].weight = dist[neighbour];
          break
        }
        k = (k + 1);
      }
  ...
  while ((_len(queue) > 0)) {
    ...
    queue = tmp;
    for (const neighbour in Object.keys(graph[node])) {
      ...
      if ((dist[neighbour] > (dist[node] + w))) {
        ...
        while ((r < _len(queue))) {
          if ((queue[(r) < 0 ? queue.length + (r) : r].node == neighbour)) {
            queue[(r) < 0 ? queue.length + (r) : r].weight = dist[neighbour];
            break
          }
          r = (r + 1);
        }
