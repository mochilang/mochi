// Generated by Mochi v0.10.61 on 2025-08-08 15:56:51 GMT+7
...
function prims_algorithm(adjacency: Record<number, Neighbor[]>): EdgePair[] {
  ...
  while ((count < total)) {
    ...
    for (const u_str in adjacency) {
      ...
      if (visited[(()=>{const _mochi_idx = u; return _mochi_idx < 0 ? visited.length + _mochi_idx : _mochi_idx;})()]) {
        for (const n of adjacency[(()=>{const _mochi_idx = u; return _mochi_idx < 0 ? adjacency.length + _mochi_idx : _mochi_idx;})()]) {
          if ((!visited[(()=>{const _mochi_idx = Math.trunc(n.node); return _mochi_idx < 0 ? visited.length + _mochi_idx : _mochi_idx;})()] && (n.cost < best_cost))) {
            best_cost = n.cost;
            best_u = u;
            best_v = n.node;
          }
...
function test_prim_successful_result(): boolean {
  ...
  for (const e of edges) {
    ...
    adjacency[(()=>{const _mochi_idx = u; return _mochi_idx < 0 ? adjacency.length + _mochi_idx : _mochi_idx;})()].push({"node": v, "cost": w});
    adjacency[(()=>{const _mochi_idx = v; return _mochi_idx < 0 ? adjacency.length + _mochi_idx : _mochi_idx;})()].push({"node": u, "cost": w});
  }
  ...
  for (const ans of expected) {
    ...
    if (!seen[(()=>{const _mochi_idx = key; return _mochi_idx < 0 ? seen.length + _mochi_idx : _mochi_idx;})()]) {
      return false;
    }
  }
  return true;
