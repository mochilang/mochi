...
function complete_graph(vertices_number: number): Record<number, number[]> {
  ...
  while ((i < vertices_number)) {
    ...
    graph[(()=>{const _mochi_idx = i; return _mochi_idx < 0 ? graph.length + _mochi_idx : _mochi_idx;})()] = neighbors;
    i = (i + 1);
  }
  return graph;
}
function random_graph(vertices_number: number, probability: number, directed: boolean): Record<number, number[]> {
  ...
  let i: number = 0;
  while ((i < vertices_number)) {
    graph[(()=>{const _mochi_idx = i; return _mochi_idx < 0 ? graph.length + _mochi_idx : _mochi_idx;})()] = [];
    i = (i + 1);
  }
  if ((probability >= 1.0)) {
  ...
  while ((i < vertices_number)) {
    ...
    while ((j < vertices_number)) {
      if ((random() < probability)) {
        graph[(()=>{const _mochi_idx = i; return _mochi_idx < 0 ? graph.length + _mochi_idx : _mochi_idx;})()] = [...graph[i], j];
        if (!directed) {
          graph[(()=>{const _mochi_idx = j; return _mochi_idx < 0 ? graph.length + _mochi_idx : _mochi_idx;})()] = [...graph[j], i];
        }
      }
      j = (j + 1);
    }
