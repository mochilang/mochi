// Generated by Mochi v0.10.59 on 2025-08-07 00:02:27 GMT+7
...
function get_nodes(trans: Transition[]): string[] {
  ...
  for (const t of trans) {
    seen[(t.src) < 0 ? seen.length + (t.src) : t.src] = true;
    seen[(t.dst) < 0 ? seen.length + (t.dst) : t.dst] = true;
  }
  let nodes: string[] = [];
...
function get_transitions(start: string, trans: Transition[], steps: number): Record<string, number> {
  ...
  for (const node of get_nodes(trans)) {
    let one: number = 1;
    visited[(node) < 0 ? visited.length + (node) : node] = one;
  }
  let node: string = start;
  let i: number = 0;
  while ((i < steps)) {
    node = transition(node, trans);
    let count: number = visited[(node) < 0 ? visited.length + (node) : node];
    count = (count + 1);
    visited[(node) < 0 ? visited.length + (node) : node] = count;
    i = (i + 1);
  }
  return visited;
}
