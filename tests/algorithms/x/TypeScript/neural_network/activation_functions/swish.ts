// Generated by Mochi v0.10.59 on 2025-08-07 08:17:28 GMT+7
...
function sigmoid(vector: number[]): number[] {
  ...
  let i: number = 0;
  while ((i < _len(vector))) {
    let v: number = vector[(i) < 0 ? vector.length + (i) : i];
...
function approx_equal_list(a: number[], b: number[], eps: number): boolean {
  ...
  let i: number = 0;
  while ((i < _len(a))) {
    if (!approx_equal(a[(i) < 0 ? a.length + (i) : i], b[(i) < 0 ? b.length + (i) : i], eps)) {
      return false;
    }
    i = (i + 1);
  }
...
function main() {
  test_swish();
  console.log(_str(_str(sigmoid([-1.0, 1.0, 2.0]))));
  console.log(_str(_str(sigmoid_linear_unit([-1.0, 1.0, 2.0]))));
  console.log(_str(_str(swish([-1.0, 1.0, 2.0], 2.0))));
  console.log(_str(_str(swish([-2.0], 1.0))));
}
var _nowSeed = 0;
