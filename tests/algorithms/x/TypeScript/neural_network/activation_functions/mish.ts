// Generated by Mochi v0.10.59 on 2025-08-07 08:17:22 GMT+7
...
function mish(vector: number[]): number[] {
  let result: number[] = [];
  let i: number = 0;
  while ((i < _len(vector))) {
    let x: number = vector[(i) < 0 ? vector.length + (i) : i];
    let sp: number = softplus(x);
    let y: number = (x * tanh_approx(sp));
    result.push(y);
    i = (i + 1);
  }
  return result;
...
function main() {
  ...
  console.log(_str(_str(mish(v1))));
  console.log(_str(_str(mish(v2))));
}
var _nowSeed = 0;
