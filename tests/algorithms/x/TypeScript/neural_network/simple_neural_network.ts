// Generated by Mochi v0.10.63 on 2025-08-11 14:52:56 GMT+7
...
function sigmoid_derivative(sig_val: number): number {
  return (sig_val * (1.0 - sig_val));
}
let INITIAL_VALUE: number = 0.02;
function forward_propagation(expected: number, number_propagations: number): number {
  let weight: number = ((2.0 * randint(1, 100)) - 1.0);
  let layer_1: number = 0.0;
  let i: number = 0;
  while ((i < number_propagations)) {
    layer_1 = sigmoid((INITIAL_VALUE * weight));
    let layer_1_error: number = ((expected / 100.0) - layer_1);
    let layer_1_delta: number = (layer_1_error * sigmoid_derivative(layer_1));
