// Generated by Mochi v0.10.59 on 2025-08-06 23:29:39 GMT+7
...
function mean(vals: number[]): number {
  ...
  let i: number = 0;
  while ((i < _len(vals))) {
    sum = (sum + vals[(i) < 0 ? vals.length + (i) : i]);
    i = (i + 1);
  }
...
function mean_squared_error(labels: number[], prediction: number): number {
  ...
  let i: number = 0;
  while ((i < _len(labels))) {
    let diff: number = (labels[(i) < 0 ? labels.length + (i) : i] - prediction);
...
function train_tree(x: number[], y: number[], depth: number, min_leaf_size: number): Tree {
  ...
  if ((best_split != 0)) {
    ...
    let boundary: number = x[(best_split) < 0 ? x.length + (best_split) : best_split];
...
function main() {
  ...
  while ((i < _len(test_cases))) {
    predictions.push(predict(tree, test_cases[(i) < 0 ? test_cases.length + (i) : i]));
    i = (i + 1);
  }
  let sum_err: number = 0.0;
  i = 0;
  while ((i < _len(test_cases))) {
    let diff: number = (predictions[(i) < 0 ? predictions.length + (i) : i] - test_cases[(i) < 0 ? test_cases.length + (i) : i]);
