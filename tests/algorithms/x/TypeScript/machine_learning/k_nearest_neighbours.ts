// Generated by Mochi v0.10.59 on 2025-08-07 00:29:57 GMT+7
...
function make_knn(train_data: number[][], train_target: number[], class_labels: string[]): KNN {
  ...
  while ((i < _len(train_data))) {
    let pl: PointLabel = {"point": train_data[(i) < 0 ? train_data.length + (i) : i], "label": train_target[(i) < 0 ? train_target.length + (i) : i]};
    items.push(pl);
    i = (i + 1);
  }
...
function classify(knn: KNN, pred_point: number[], k: number): string {
  ...
  while ((i < _len(knn.data))) {
    let d = euclidean_distance(knn.data[(i) < 0 ? knn.data.length + (i) : i].point, pred_point);
    distances.push({"dist": d, "label": knn.data[(i) < 0 ? knn.data.length + (i) : i].label});
    i = (i + 1);
  }
  let votes: number[] = [];
  let count: number = 0;
  while ((count < k)) {
    ...
    while ((j < _len(distances))) {
      if ((distances[(j) < 0 ? distances.length + (j) : j].dist < distances[(min_index) < 0 ? distances.length + (min_index) : min_index].dist)) {
        min_index = j;
      }
      j = (j + 1);
    }
    votes.push(distances[(min_index) < 0 ? distances.length + (min_index) : min_index].label);
    distances[(min_index) < 0 ? distances.length + (min_index) : min_index].dist = 1e18;
    count = (count + 1);
  }
  let tally: number[] = [];
  let t: number = 0;
  ...
  let v: number = 0;
  while ((v < _len(votes))) {
    let lbl: number = votes[(v) < 0 ? votes.length + (v) : v];
    tally[(lbl) < 0 ? tally.length + (lbl) : lbl] = (tally[(lbl) < 0 ? tally.length + (lbl) : lbl] + 1);
    v = (v + 1);
  }
  let max_idx: number = 0;
  let m: number = 1;
  while ((m < _len(tally))) {
    if ((tally[(m) < 0 ? tally.length + (m) : m] > tally[(max_idx) < 0 ? tally.length + (max_idx) : max_idx])) {
      max_idx = m;
    }
    m = (m + 1);
  }
  return knn.labels[(max_idx) < 0 ? knn.labels.length + (max_idx) : max_idx];
}
