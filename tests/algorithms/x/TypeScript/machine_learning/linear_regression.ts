// Generated by Mochi v0.10.59 on 2025-08-07 00:16:26 GMT+7

function dot(x: number[], y: number[]): number {
  let sum: number = 0.0;
  let i: number = 0;
  while ((i < _len(x))) {
    sum = (sum + (x[(i) < 0 ? x.length + (i) : i] * y[(i) < 0 ? y.length + (i) : i]));
    i = (i + 1);
  }
  return sum;
}
function run_steep_gradient_descent(data_x: number[][], data_y: number[], len_data: number, alpha: number, theta: number[]): number[] {
  ...
  let i: number = 0;
  while ((i < len_data)) {
    let prediction: number = dot(theta, data_x[(i) < 0 ? data_x.length + (i) : i]);
    let error: number = (prediction - data_y[(i) < 0 ? data_y.length + (i) : i]);
    let k: number = 0;
    while ((k < _len(theta))) {
      gradients[(k) < 0 ? gradients.length + (k) : k] = (gradients[(k) < 0 ? gradients.length + (k) : k] + (error * data_x[(i) < 0 ? data_x.length + (i) : i][(k) < 0 ? data_x[(i) < 0 ? data_x.length + (i) : i].length + (k) : k]));
      k = (k + 1);
    }
    i = (i + 1);
  }
  ...
  let g: number = 0;
  while ((g < _len(theta))) {
    t.push((theta[(g) < 0 ? theta.length + (g) : g] - ((alpha / len_data) * gradients[(g) < 0 ? gradients.length + (g) : g])));
    g = (g + 1);
  }
  return t;
}
function sum_of_square_error(data_x: number[][], data_y: number[], len_data: number, theta: number[]): number {
  ...
  while ((i < len_data)) {
    let prediction: number = dot(theta, data_x[(i) < 0 ? data_x.length + (i) : i]);
    let diff: number = (prediction - data_y[(i) < 0 ? data_y.length + (i) : i]);
...
function run_linear_regression(data_x: number[][], data_y: number[]): number[] {
  ...
  let no_features: number = _len(data_x[(Math.trunc(0)) < 0 ? data_x.length + (Math.trunc(0)) : Math.trunc(0)]);
...
function mean_absolute_error(predicted_y: number[], original_y: number[]): number {
  ...
  while ((i < _len(predicted_y))) {
    let diff: number = absf((predicted_y[(i) < 0 ? predicted_y.length + (i) : i] - original_y[(i) < 0 ? original_y.length + (i) : i]));
    total = (total + diff);
    i = (i + 1);
  }
...
theta = run_linear_regression(data_x, data_y)
  ...
  while ((i < _len(theta))) {
    console.log(_str(_str(theta[(i) < 0 ? theta.length + (i) : i])));
    i = (i + 1);
  }
