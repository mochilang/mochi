// Generated by Mochi v0.10.59 on 2025-08-06 23:29:38 GMT+7
...
function mean(data: number[]): number {
  ...
  while ((i < n)) {
    total = (total + data[(i) < 0 ? data.length + (i) : i]);
    i = (i + 1);
  }
  return (total / n);
}
function stdev(data: number[]): number {
  ...
  let i: number = 0;
  while ((i < n)) {
    let diff: number = (data[(i) < 0 ? data.length + (i) : i] - m);
...
function normalization(data: number[], ndigits: number): number[] {
  let x_min: number = (() => { const arr = data; return arr.length === 0 ? 0 : Math.min(...arr); })();
  let x_max: number = (() => { const arr = data; return arr.length === 0 ? 0 : Math.max(...arr); })();
  let denom = (x_max - x_min);
  let result: number[] = [];
  let i: number = 0;
  let n: number = _len(data);
  while ((i < n)) {
    let norm = ((data[(i) < 0 ? data.length + (i) : i] - x_min) / denom);
...
function standardization(data: number[], ndigits: number): number[] {
  ...
  let n: number = _len(data);
  while ((i < n)) {
    let z: number = ((data[(i) < 0 ? data.length + (i) : i] - mu) / sigma);
