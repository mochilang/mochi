// Generated by Mochi v0.10.59 on 2025-08-06 23:29:36 GMT+7
...
function astar(x_limit: number, y_limit: number, start: Point, goal: Point): Point[] {
  ...
  let current: Node = open[(Math.trunc(0)) < 0 ? open.length + (Math.trunc(0)) : Math.trunc(0)];
  while ((_len(open) > 0)) {
    let min_index: number = 0;
    let i: number = 1;
    while ((i < _len(open))) {
      if ((open[(i) < 0 ? open.length + (i) : i].f < open[(min_index) < 0 ? open.length + (min_index) : min_index].f)) {
        min_index = i;
      }
      i = (i + 1);
    }
    current = open[(min_index) < 0 ? open.length + (min_index) : min_index];
    let new_open: Node[] = [];
    let j: number = 0;
    while ((j < _len(open))) {
      if ((j != min_index)) {
        new_open.push(open[(j) < 0 ? open.length + (j) : j]);
      }
      j = (j + 1);
    }
    open = new_open;
  ...
  while ((k >= 0)) {
    rev.push(path[(k) < 0 ? path.length + (k) : k]);
    k = (k - 1);
  }
  return rev;
}
...
function mark_path(world: number[][], path: Point[]) {
  for (const p of path) {
    world[(Math.trunc(p.x)) < 0 ? world.length + (Math.trunc(p.x)) : Math.trunc(p.x)][(Math.trunc(p.y)) < 0 ? world[(Math.trunc(p.x)) < 0 ? world.length + (Math.trunc(p.x)) : Math.trunc(p.x)].length + (Math.trunc(p.y)) : Math.trunc(p.y)] = 1;
  }
}
