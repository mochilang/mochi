// Generated by Mochi v0.10.59 on 2025-08-06 21:31:26 GMT+7
...
function pop_back(dq: Deque): PopResult {
  ...
  return {"deque": {"data": res}, "value": dq.data[Math.trunc((_len(dq.data) - 1))]};
}
function pop_front(dq: Deque): PopResult {
  ...
  return {"deque": {"data": res}, "value": dq.data[Math.trunc(0)]};
}
...
function to_string(dq: Deque): string {
  ...
  let s: string = ("[" + _str(dq.data[Math.trunc(0)]));
  let i: number = 1;
  while ((i < _len(dq.data))) {
    s = ((s + ", ") + _str(dq.data[i]));
    i = (i + 1);
