// Generated by Mochi v0.10.59 on 2025-08-06 21:30:54 GMT+7
...
function front(d: LinkedDeque): string {
  ...
  let head: Node = d.nodes[Math.trunc(d.header)];
  let idx: number = head.next;
  let node: Node = d.nodes[idx];
  return node.data;
}
function back(d: LinkedDeque): string {
  ...
  let tail: Node = d.nodes[Math.trunc(d.trailer)];
...
function insert(d: LinkedDeque, pred: number, value: string, succ: number): LinkedDeque {
  ...
  let pred_node: Node = nodes[pred];
  pred_node.next = new_idx;
  nodes[pred] = pred_node;
  let succ_node: Node = nodes[succ];
