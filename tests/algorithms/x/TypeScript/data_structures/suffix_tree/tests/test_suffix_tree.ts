// Generated by Mochi v0.10.59 on 2025-08-06 21:32:06 GMT+7
...
function suffix_tree_search(st: SuffixTree, pattern: string): boolean {
  ...
  while ((i <= (n - m))) {
    ...
    while ((j < m)) {
      if ((st.text[Math.trunc((i + j))] != pattern[j])) {
        found = false;
        break
      }
      j = (j + 1);
...
st = suffix_tree_new(text)
  while ((i < _len(patterns_exist))) {
    console.log(_str(_str(suffix_tree_search(st, patterns_exist[i]))));
    i = (i + 1);
  }
  i = 0;
  while ((i < _len(patterns_none))) {
    console.log(_str(_str(suffix_tree_search(st, patterns_none[i]))));
    i = (i + 1);
  }
  ...
  while ((i < _len(substrings))) {
    console.log(_str(_str(suffix_tree_search(st, substrings[i]))));
    i = (i + 1);
  }
  const _end = _now()
