// Generated by Mochi v0.10.59 on 2025-08-07 08:18:20 GMT+7
...
function update_system(system: BodySystem, delta_time: number): BodySystem {
  ...
  let i: number = 0;
  while ((i < _len(bodies))) {
    let body1: Body = bodies[(i) < 0 ? bodies.length + (i) : i];
    let force_x: number = 0.0;
    let force_y: number = 0.0;
    let j: number = 0;
    while ((j < _len(bodies))) {
      if ((i != j)) {
        let body2: Body = bodies[(j) < 0 ? bodies.length + (j) : j];
    ...
    bodies[(i) < 0 ? bodies.length + (i) : i] = body1;
    i = (i + 1);
  }
  i = 0;
  while ((i < _len(bodies))) {
    let body: Body = bodies[(i) < 0 ? bodies.length + (i) : i];
    body = update_position(body, (delta_time * system.time_factor));
    bodies[(i) < 0 ? bodies.length + (i) : i] = body;
    i = (i + 1);
  }
...
function main() {
  ...
  let b1_after: Body = sys1.bodies[(Math.trunc(0)) < 0 ? sys1.bodies.length + (Math.trunc(0)) : Math.trunc(0)];
  ...
  let b2_after: Body = sys2.bodies[(Math.trunc(0)) < 0 ? sys2.bodies.length + (Math.trunc(0)) : Math.trunc(0)];
