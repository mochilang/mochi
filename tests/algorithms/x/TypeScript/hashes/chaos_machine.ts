// Generated by Mochi v0.10.59 on 2025-08-06 23:29:06 GMT+7
...
function push(m: Machine, seed: number): Machine {
  ...
  let i: number = 0;
  while ((i < _len(buf))) {
    let value: number = buf[(i) < 0 ? buf.length + (i) : i];
    let e: number = ((1.0 * seed) / value);
    let next_value: number = (buf[(Math.trunc(((i + 1) % size))) < 0 ? buf.length + (Math.trunc(((i + 1) % size))) : Math.trunc(((i + 1) % size))] + e);
    next_value = (next_value - (1.0 * Math.trunc(next_value)));
    let r: number = (par[(i) < 0 ? par.length + (i) : i] + e);
    r = (r - (1.0 * Math.trunc(r)));
    r = (r + 3.0);
    buf[(i) < 0 ? buf.length + (i) : i] = round_dec(((r * next_value) * (1.0 - next_value)), 10);
    par[(i) < 0 ? par.length + (i) : i] = r;
    i = (i + 1);
  }
...
function pull(m: Machine): PullResult {
  ...
  let i: number = 0;
  while ((i < t)) {
    let r: number = par[(key) < 0 ? par.length + (key) : key];
    let value: number = buf[(key) < 0 ? buf.length + (key) : key];
    buf[(key) < 0 ? buf.length + (key) : key] = round_dec(((r * value) * (1.0 - value)), 10);
    let new_r: number = (((1.0 * m.time) * 0.01) + (r * 1.01));
    new_r = (new_r - (1.0 * Math.trunc(new_r)));
    par[(key) < 0 ? par.length + (key) : key] = (new_r + 3.0);
    i = (i + 1);
  }
  let x: number = Math.trunc((buf[(Math.trunc(((key + 2) % size))) < 0 ? buf.length + (Math.trunc(((key + 2) % size))) : Math.trunc(((key + 2) % size))] * 10000000000.0));
  let y: number = Math.trunc((buf[(Math.trunc((((key + size) - 2) % size))) < 0 ? buf.length + (Math.trunc((((key + size) - 2) % size))) : Math.trunc((((key + size) - 2) % size))] * 10000000000.0));
  let new_machine: Machine = {"buffer": buf, "params": par, "time": (m.time + 1)};
  let value: number = (xorshift(x, y) % 4294967295);
...
res = pull(machine)
  ...
  console.log(_str("[" + (res.machine.buffer).join(' ') + "]"));
  console.log(_str("[" + (res.machine.params).join(' ') + "]"));
  const _end = _now()
