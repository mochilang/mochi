// Generated by Mochi v0.10.59 on 2025-08-06 21:51:20 GMT+7
...
function convert_to_negative(img: number[][]): number[][] {
  ...
  while ((y < h)) {
    ...
    let x: number = 0;
    while ((x < w)) {
      row.push((255 - img[(y) < 0 ? img.length + (y) : y][(x) < 0 ? img[(y) < 0 ? img.length + (y) : y].length + (x) : x]));
      x = (x + 1);
    }
    out.push(row);
    y = (y + 1);
...
function change_contrast(img: number[][], factor: number): number[][] {
  ...
  while ((y < h)) {
    ...
    let x: number = 0;
    while ((x < w)) {
      let p: number = img[(y) < 0 ? img.length + (y) : y][(x) < 0 ? img[(y) < 0 ? img.length + (y) : y].length + (x) : x];
  ...
  while ((y < h)) {
    ...
    while ((x < w)) {
      ...
      while ((ky < _len(kernel))) {
        let kx: number = 0;
        while ((kx < _len(kernel[(Math.trunc(0)) < 0 ? kernel.length + (Math.trunc(0)) : Math.trunc(0)]))) {
          let iy: number = ((y + ky) - 1);
          let ix: number = ((x + kx) - 1);
          let pixel: number = 0;
          if (((((iy >= 0) && (iy < h)) && (ix >= 0)) && (ix < w))) {
            pixel = img[(iy) < 0 ? img.length + (iy) : iy][(ix) < 0 ? img[(iy) < 0 ? img.length + (iy) : iy].length + (ix) : ix];
          }
          acc = (acc + (kernel[(ky) < 0 ? kernel.length + (ky) : ky][(kx) < 0 ? kernel[(ky) < 0 ? kernel.length + (ky) : ky].length + (kx) : kx] * (1.0 * pixel)));
          kx = (kx + 1);
        }
        ky = (ky + 1);
      }
...
function median_filter(img: number[][], k: number): number[][] {
  ...
  while ((y < h)) {
    ...
    while ((x < w)) {
      ...
      row.push(sorted[(Math.trunc(Math.trunc(_len(sorted) / 2))) < 0 ? sorted.length + (Math.trunc(Math.trunc(_len(sorted) / 2))) : Math.trunc(Math.trunc(_len(sorted) / 2))]);
      x = (x + 1);
    }
    out.push(row);
    y = (y + 1);
    ...
    while ((x < w)) {
      ...
      while ((ky < 3)) {
        ...
        while ((kx < 3)) {
          ...
          sx = (sx + (gx[(ky) < 0 ? gx.length + (ky) : ky][(kx) < 0 ? gx[(ky) < 0 ? gx.length + (ky) : ky].length + (kx) : kx] * pixel));
          sy = (sy + (gy[(ky) < 0 ? gy.length + (ky) : ky][(kx) < 0 ? gy[(ky) < 0 ? gy.length + (ky) : ky].length + (kx) : kx] * pixel));
          kx = (kx + 1);
        }
        ky = (ky + 1);
      }
    ...
    while ((dx <= 1)) {
      if (!((dx == 0) && (dy == 0))) {
        ...
        if (((((ny >= 0) && (ny < h)) && (nx >= 0)) && (nx < w))) {
          val = img[(ny) < 0 ? img.length + (ny) : ny][(nx) < 0 ? img[(ny) < 0 ? img.length + (ny) : ny].length + (nx) : nx];
        }
        neighbors.push(val);
      }
      dx = (dx + 1);
...
function local_binary_value(img: number[][], x: number, y: number): number {
  let center: number = img[(y) < 0 ? img.length + (y) : y][(x) < 0 ? img[(y) < 0 ? img.length + (y) : y].length + (x) : x];
  let neighbors: number[] = get_neighbors_pixel(img, x, y);
  let v: number = 0;
  let i: number = 0;
  while ((i < _len(neighbors))) {
    if ((neighbors[(i) < 0 ? neighbors.length + (i) : i] >= center)) {
      v = (v + pow2(i));
    }
    i = (i + 1);
  }
