...
function simple_moving_average(data: number[], window_size: number): SMAValue[] {
  ...
  let i: number = 0;
  while ((i < _len(data))) {
    window_sum = (window_sum + data[(i) < 0 ? data.length + (i) : i]);
    if ((i >= window_size)) {
      window_sum = (window_sum - data[(Math.trunc((i - window_size))) < 0 ? data.length + (Math.trunc((i - window_size))) : Math.trunc((i - window_size))]);
    }
    if ((i >= (window_size - 1))) {
...
sma_values = simple_moving_average(data, window_size)
  while ((idx < _len(sma_values))) {
    let item: SMAValue = sma_values[(idx) < 0 ? sma_values.length + (idx) : idx];
    if (item.ok) {
