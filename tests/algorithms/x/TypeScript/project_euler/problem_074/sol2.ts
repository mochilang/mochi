// Generated by Mochi v0.10.59 on 2025-08-07 08:54:24 GMT+7
...
function digit_factorial_sum(_number: number): number {
  ...
  if ((_number == 0)) {
    return DIGIT_FACTORIAL[(Math.trunc(0)) < 0 ? DIGIT_FACTORIAL.length + (Math.trunc(0)) : Math.trunc(0)];
  }
  let n: number = _number;
  let total: number = 0;
  while ((n > 0)) {
    let digit: number = (n % 10);
    total = (total + DIGIT_FACTORIAL[(digit) < 0 ? DIGIT_FACTORIAL.length + (digit) : digit]);
    n = Math.trunc(n / 10);
  }
  return total;
}
function chain_len(n: number, limit: number): number {
  ...
  while ((((cur in seen) == false) && (length <= limit))) {
    seen[(cur) < 0 ? seen.length + (cur) : cur] = true;
    length = (length + 1);
...
(() => {
  ...
  const _start = _now()
  console.log(_str(_str(solution(60, 1000000))));
  const _end = _now()
