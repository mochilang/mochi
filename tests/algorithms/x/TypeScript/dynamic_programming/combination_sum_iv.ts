// Generated by Mochi v0.10.59 on 2025-08-06 21:51:47 GMT+7
...
function count_recursive(array: number[], target: number): number {
  ...
  let i: number = 0;
  while ((i < _len(array))) {
    total = (total + count_recursive(array, (target - array[(i) < 0 ? array.length + (i) : i])));
    i = (i + 1);
  }
  return total;
}
...
function count_dp(array: number[], target: number, dp: number[]): number {
  ...
  if ((dp[(target) < 0 ? dp.length + (target) : target] > (0 - 1))) {
    return dp[(target) < 0 ? dp.length + (target) : target];
  }
  let total: number = 0;
  let i: number = 0;
  while ((i < _len(array))) {
    total = (total + count_dp(array, (target - array[(i) < 0 ? array.length + (i) : i]), dp));
    i = (i + 1);
  }
  dp[(target) < 0 ? dp.length + (target) : target] = total;
  return total;
}
...
function combination_sum_iv_bottom_up(n: number, array: number[], target: number): number {
  ...
  dp[(0) < 0 ? dp.length + (0) : 0] = 1;
  let i: number = 1;
  while ((i <= target)) {
    let j: number = 0;
    while ((j < n)) {
      if (((i - array[(j) < 0 ? array.length + (j) : j]) >= 0)) {
        dp[(i) < 0 ? dp.length + (i) : i] = (dp[(i) < 0 ? dp.length + (i) : i] + dp[(Math.trunc((i - array[(j) < 0 ? array.length + (j) : j]))) < 0 ? dp.length + (Math.trunc((i - array[(j) < 0 ? array.length + (j) : j]))) : Math.trunc((i - array[(j) < 0 ? array.length + (j) : j]))]);
      }
      j = (j + 1);
    }
    i = (i + 1);
