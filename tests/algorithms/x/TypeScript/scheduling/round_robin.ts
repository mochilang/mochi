// Generated by Mochi v0.10.59 on 2025-08-07 08:54:45 GMT+7
...
function calculate_waiting_times(burst_times: number[]): number[] {
  ...
  while ((i < _len(burst_times))) {
    rem.push(burst_times[(i) < 0 ? burst_times.length + (i) : i]);
    i = (i + 1);
  }
  let waiting: number[] = [];
  i = 0;
  ...
  while (true) {
    ...
    while ((j < _len(burst_times))) {
      if ((rem[(j) < 0 ? rem.length + (j) : j] > 0)) {
        done = false;
        if ((rem[(j) < 0 ? rem.length + (j) : j] > quantum)) {
          t = (t + quantum);
          rem[(j) < 0 ? rem.length + (j) : j] = (rem[(j) < 0 ? rem.length + (j) : j] - quantum);
        } else {
          t = (t + rem[(j) < 0 ? rem.length + (j) : j]);
          waiting[(j) < 0 ? waiting.length + (j) : j] = (t - burst_times[(j) < 0 ? burst_times.length + (j) : j]);
          rem[(j) < 0 ? rem.length + (j) : j] = 0;
        }
      }
      j = (j + 1);
    }
...
function calculate_turn_around_times(burst_times: number[], waiting_times: number[]): number[] {
  ...
  let i: number = 0;
  while ((i < _len(burst_times))) {
    result.push((burst_times[(i) < 0 ? burst_times.length + (i) : i] + waiting_times[(i) < 0 ? waiting_times.length + (i) : i]));
    i = (i + 1);
  }
  return result;
}
...
function format_float_5(x: number): string {
  ...
  let frac_part: number = (scaled % 100000);
...
function main() {
  ...
  let i: number = 0;
  while ((i < _len(burst_times))) {
    let line: string = ((((((("  " + _str((i + 1))) + "\t\t  ") + _str(burst_times[(i) < 0 ? burst_times.length + (i) : i])) + "\t\t  ") + _str(waiting_times[(i) < 0 ? waiting_times.length + (i) : i])) + "\t\t  ") + _str(turn_around_times[(i) < 0 ? turn_around_times.length + (i) : i]));
