// Generated by Mochi v0.10.59 on 2025-08-07 08:17:09 GMT+7
...
function reverse_row(mat: number[][]): number[][] {
  ...
  while ((i >= 0)) {
    result.push(mat[(i) < 0 ? mat.length + (i) : i]);
    i = (i - 1);
  }
  return result;
}
function reverse_column(mat: number[][]): number[][] {
  ...
  while ((i < _len(mat))) {
    let row: number[] = [];
    let j: number = (_len(mat[(i) < 0 ? mat.length + (i) : i]) - 1);
    while ((j >= 0)) {
...
function row_to_string(row: number[]): string {
  ...
  while ((i < _len(row))) {
    if ((i == 0)) {
      line = _str(row[(i) < 0 ? row.length + (i) : i]);
    } else {
      line = ((line + " ") + _str(row[(i) < 0 ? row.length + (i) : i]));
    }
    i = (i + 1);
  }
  return line;
...
function print_matrix(mat: number[][]) {
  let i: number = 0;
  while ((i < _len(mat))) {
    console.log(_str(row_to_string(mat[(i) < 0 ? mat.length + (i) : i])));
    i = (i + 1);
  }
}
var _nowSeed = 0;
