// Generated by Mochi v0.10.66 on 2025-08-16 09:40:36 GMT+7
...
function median(matrix: number[][]): number {
  ...
  while ((i < _len(matrix))) {
    ...
    let j: number = 0;
    while ((j < _len(row))) {
      linear.push(row[(()=>{const _mochi_idx = j; return _mochi_idx < 0 ? row.length + _mochi_idx : _mochi_idx;})()]);
      j = (j + 1);
    }
    i = (i + 1);
  }
  ...
  return sorted[(()=>{const _mochi_idx = mid; return _mochi_idx < 0 ? sorted.length + _mochi_idx : _mochi_idx;})()];
}
...
(() => {
  ...
  const _start = _now()
  console.log(_str(median(matrix1)));
  console.log(_str(median(matrix2)));
  const _end = _now()
