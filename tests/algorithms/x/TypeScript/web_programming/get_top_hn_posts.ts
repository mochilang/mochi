// Generated by Mochi v0.10.59 on 2025-08-07 08:57:16 GMT+7
...
async function hackernews_top_stories(max_stories: number): Story[] {
  ...
  let i: number = 0;
  while ((i < _len(ids))) {
    stories.push(get_hackernews_story(ids[(i) < 0 ? ids.length + (i) : i]));
    i = (i + 1);
  }
  return stories;
}
async function hackernews_top_stories_as_markdown(max_stories: number): string {
  let stories: Story[] = hackernews_top_stories(max_stories);
  let output: string = "";
  let i: number = 0;
...
(async () => {
  ...
  const _start = _now()
  console.log(_str(hackernews_top_stories_as_markdown(5)));
  const _end = _now()
