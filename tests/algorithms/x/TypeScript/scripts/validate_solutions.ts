// Generated by Mochi v0.10.59 on 2025-08-07 08:54:49 GMT+7

let HEX: string = "0123456789abcdef";
function byte_to_hex(b: number): string {
  let hi: number = Math.trunc(b / 16);
  let lo: number = (b % 16);
  return (HEX[(hi) < 0 ? HEX.length + (hi) : hi] + HEX[(lo) < 0 ? HEX.length + (lo) : lo]);
}
function bytes_to_hex(bs: number[]): string {
  let res: string = "";
  let i: number = 0;
  while ((i < _len(bs))) {
    res = (res + byte_to_hex(bs[(i) < 0 ? bs.length + (i) : i]));
    i = (i + 1);
  }
  return res;
}
