// Generated by Mochi v0.10.59 on 2025-08-06 23:29:25 GMT+7
...
function mat_vec_mul(m: number[][], v: number[]): number[] {
  ...
  while ((i < _len(m))) {
    ...
    while ((j < _len(m[(i) < 0 ? m.length + (i) : i]))) {
      s = (s + (m[(i) < 0 ? m.length + (i) : i][(j) < 0 ? m[(i) < 0 ? m.length + (i) : i].length + (j) : j] * v[(j) < 0 ? v.length + (j) : j]));
      j = (j + 1);
    }
    res.push(s);
    i = (i + 1);
...
x = conjugate_gradient(A, b, 1000, 1e-08)
  console.log(_str(_str(x[(Math.trunc(0)) < 0 ? x.length + (Math.trunc(0)) : Math.trunc(0)])));
  console.log(_str(_str(x[(Math.trunc(1)) < 0 ? x.length + (Math.trunc(1)) : Math.trunc(1)])));
  console.log(_str(_str(x[(Math.trunc(2)) < 0 ? x.length + (Math.trunc(2)) : Math.trunc(2)])));
  const _end = _now()
