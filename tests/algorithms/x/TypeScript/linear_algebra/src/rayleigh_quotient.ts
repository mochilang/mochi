// Generated by Mochi v0.10.59 on 2025-08-06 23:29:31 GMT+7
...
function conj_vector(v: Complex[]): Complex[] {
  ...
  let i: number = 0;
  while ((i < _len(v))) {
    res.push(complex_conj(v[(i) < 0 ? v.length + (i) : i]));
    i = (i + 1);
  }
  return res;
}
function vec_mat_mul(v: Complex[], m: Complex[][]): Complex[] {
  let result: Complex[] = [];
  let col: number = 0;
  while ((col < _len(m[(Math.trunc(0)) < 0 ? m.length + (Math.trunc(0)) : Math.trunc(0)]))) {
    let sum: Complex = {"re": 0.0, "im": 0.0};
    let row: number = 0;
    while ((row < _len(v))) {
      sum = complex_add(sum, complex_mul(v[(row) < 0 ? v.length + (row) : row], m[(row) < 0 ? m.length + (row) : row][(col) < 0 ? m[(row) < 0 ? m.length + (row) : row].length + (col) : col]));
      row = (row + 1);
    }
    result.push(sum);
    col = (col + 1);
...
function dot(a: Complex[], b: Complex[]): Complex {
  ...
  let i: number = 0;
  while ((i < _len(a))) {
    sum = complex_add(sum, complex_mul(a[(i) < 0 ? a.length + (i) : i], b[(i) < 0 ? b.length + (i) : i]));
    i = (i + 1);
  }
  return sum;
}
function is_hermitian(m: Complex[][]): boolean {
  ...
  while ((i < _len(m))) {
    let j: number = 0;
    while ((j < _len(m))) {
      if (!complex_eq(m[(i) < 0 ? m.length + (i) : i][(j) < 0 ? m[(i) < 0 ? m.length + (i) : i].length + (j) : j], complex_conj(m[(j) < 0 ? m.length + (j) : j][(i) < 0 ? m[(j) < 0 ? m.length + (j) : j].length + (i) : i]))) {
        return false;
      }
      j = (j + 1);
    }
