// Generated by Mochi v0.10.59 on 2025-08-06 21:53:19 GMT+7
...
function file_read(f: FileMock, size: number): number {
  if ((f.read_called < _len(f.data))) {
    let value: number = f.data[(Math.trunc(f.read_called)) < 0 ? f.data.length + (Math.trunc(f.read_called)) : Math.trunc(f.read_called)];
