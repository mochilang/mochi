// Generated by Mochi v0.10.59 on 2025-08-06 22:14:55 GMT+7
...
function julia_demo() {
  ...
  while ((y < _len(grid))) {
    ...
    let row_exp: number[] = [];
    let x: number = 0;
    while ((x < _len(grid[(y) < 0 ? grid.length + (y) : y]))) {
      let z0: Complex = grid[(y) < 0 ? grid.length + (y) : y][(x) < 0 ? grid[(y) < 0 ? grid.length + (y) : y].length + (x) : x];
  ...
  console.log(_str("[" + (poly_result).join(' ') + "]"));
  console.log(_str("[" + (exp_result).join(' ') + "]"));
}
var _nowSeed = 0;
