// Generated by Mochi v0.10.59 on 2025-08-06 22:14:58 GMT+7
...
function iteration_step(vectors: Vec[]): Vec[] {
  ...
  while ((i < (_len(vectors) - 1))) {
    let start: Vec = vectors[(i) < 0 ? vectors.length + (i) : i];
    let end: Vec = vectors[(Math.trunc((i + 1))) < 0 ? vectors.length + (Math.trunc((i + 1))) : Math.trunc((i + 1))];
  ...
  new_vectors.push(vectors[(Math.trunc((_len(vectors) - 1))) < 0 ? vectors.length + (Math.trunc((_len(vectors) - 1))) : Math.trunc((_len(vectors) - 1))]);
  return new_vectors;
}
...
function vec_list_to_string(lst: Vec[]): string {
  ...
  let i: number = 0;
  while ((i < _len(lst))) {
    res = (res + vec_to_string(lst[(i) < 0 ? lst.length + (i) : i]));
    if ((i < (_len(lst) - 1))) {
      res = (res + ", ");
    }
    i = (i + 1);
