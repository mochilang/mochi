// Generated by Mochi v0.10.59 on 2025-08-06 21:51:27 GMT+7
...
function count_inversions_bf(arr: number[]): number {
  ...
  while ((i < (n - 1))) {
    let j: number = (i + 1);
    while ((j < n)) {
      if ((arr[(i) < 0 ? arr.length + (i) : i] > arr[(j) < 0 ? arr.length + (j) : j])) {
        inv = (inv + 1);
      }
      j = (j + 1);
    }
...
function count_cross_inversions(p: number[], q: number[]): InvResult {
  ...
  while (((i < _len(p)) && (j < _len(q)))) {
    if ((p[(i) < 0 ? p.length + (i) : i] > q[(j) < 0 ? q.length + (j) : j])) {
      inv = (inv + (_len(p) - i));
      r.push(q[(j) < 0 ? q.length + (j) : j]);
      j = (j + 1);
    } else {
      r.push(p[(i) < 0 ? p.length + (i) : i]);
      i = (i + 1);
    }
  }
  if ((i < _len(p))) {
