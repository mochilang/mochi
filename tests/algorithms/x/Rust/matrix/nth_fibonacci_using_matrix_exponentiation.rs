...
fn main() {
    ...
    fn nth_fibonacci_matrix(mut n: i64) -> i64 {
    if (n <= 1) {
        return n
    }
    ...
    fn mochi_main() {
    let ordinals: Vec<String> = vec![String::from("0th").clone().clone(), String::from("1st").clone().clone(), String::from("2nd").clone().clone(), String::from("3rd").clone().clone(), String::from("10th").clone().clone(), String::from("100th").clone().clone(), String::from("1000th").clone().clone()];
    let mut i: i64 = 0;
    while (i < (ordinals.len() as i64)) {
        let ordinal: String = ordinals[i as usize].clone().clone();
        let n: i64 = parse_number(&ordinal);
