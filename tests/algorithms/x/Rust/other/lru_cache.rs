...
fn main() {
    ...
    let mut new_cache = |mut cap: i64| -> LRUCache {
    ...
    let mut lru_get = |mut c: LRUCache, mut key: i64| -> GetResult {
    ...
    let mut lru_put = |mut c: LRUCache, mut key: i64, mut value: i64| -> LRUCache {
    let mut cache: LRUCache = c.clone();
    let key_str: String = key.to_string().clone();
    if !cache.cache.clone().contains_key(&key_str) {
