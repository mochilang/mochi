...
fn main() {
    ...
    let mut evaluate_clause = |mut c: Clause, mut model: HashMap<String, i64>| -> EvalResult {
    let mut i: i64 = 0;
    while (i < (c.names.clone().len() as i64)) {
        let lit: String = c.names.clone()[i as usize].clone().clone();
    ...
    let mut dpll_algorithm = {
fn dpll_algorithm(evaluate_clause: &mut dyn FnMut(Clause, HashMap<String, i64>) -> EvalResult, remove_symbol: &mut dyn FnMut(Vec<String>, String) -> Vec<String>, clauses: &mut Vec<Clause>, mut symbols: Vec<String>, mut model: HashMap<String, i64>) -> DPLLResult {
    ...
    let res1: DPLLResult = dpll_algorithm(evaluate_clause, remove_symbol, clauses.clone(), rest.clone(), tmp1.clone());
    if res1.sat {
        return res1
    }
    return dpll_algorithm(evaluate_clause, remove_symbol, clauses.clone(), rest.clone(), tmp2.clone())
}
|clauses: &mut Vec<Clause>, mut symbols: Vec<String>, mut model: HashMap<String, i64>| -> DPLLResult { dpll_algorithm(&mut evaluate_clause, &mut remove_symbol, clauses, symbols, model) }
};
    ...
    let mut str_formula = |mut f: Formula| -> String {
    let mut line: String = String::from("{");
    let mut i: i64 = 0;
    ...
    let clauses: Vec<Clause> = vec![clause1.clone(), clause2.clone()];
    ...
    let result: DPLLResult = dpll_algorithm(clauses.clone(), symbols.clone(), model.clone());
    if result.sat {
