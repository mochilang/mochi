...
fn main() {
    ...
    let mut merge_sort = {
fn merge_sort(merge: &mut dyn FnMut(Vec<i64>, Vec<i64>) -> Vec<i64>, subarray: &mut dyn FnMut(Vec<i64>, i64, i64) -> Vec<i64>, mut array: Vec<i64>) -> Vec<i64> {
    ...
    let sorted_left: Vec<i64> = merge_sort(merge, subarray, left_half.clone());
    let sorted_right: Vec<i64> = merge_sort(merge, subarray, right_half.clone());
    return merge(sorted_left.clone(), sorted_right.clone())
}
|mut array: Vec<i64>| -> Vec<i64> { merge_sort(&mut merge, &mut subarray, array) }
};
    let mut h_index = |mut citations: Vec<i64>| -> i64 {
    let mut idx: i64 = 0;
