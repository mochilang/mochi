...
fn main() {
    ...
    return (2.0 * sum)
        ...
        unsafe fn gaussian_distribution(mut mean: f64, mut std_dev: f64, mut instance_count: i64) -> Vec<f64> {
    let mut res: Vec<f64> = vec![];
    let mut i: i64 = 0;
    while (i < instance_count) {
        let mut u1: f64 = random();
        let mut u2: f64 = random();
        let mut r: f64 = sqrtApprox((-2.0 * ln(u1)));
        let mut theta: f64 = (g_TWO_PI * u2);
        let mut z: f64 = (r * cos(theta));
        res = { let mut _v = res.clone(); _v.push((mean + (z * std_dev))); _v };
        i = (i + 1);
    }
    return res
};
        unsafe fn y_generator(mut class_count: i64, mut instance_count: Vec<i64>) -> Vec<i64> {
    ...
    return res
        ...
        unsafe fn calculate_mean(mut instance_count: i64, mut items: Vec<f64>) -> f64 {
    ...
    return (total / (instance_count as f64))
        ...
        unsafe fn calculate_probabilities(mut instance_count: i64, mut total_count: i64) -> f64 {
    return ((instance_count as f64) / (total_count as f64))
};
        unsafe fn calculate_variance(mut items: Vec<Vec<f64>>, mut means: Vec<f64>, mut total_count: i64) -> f64 {
    let mut squared_diff: Vec<f64> = vec![];
    let mut i: i64 = 0;
    while (i < (items.len() as i64)) {
        let mut j: i64 = 0;
        while (j < (items[i as usize].clone().len() as i64)) {
            let mut diff: f64 = (items[i as usize].clone()[j as usize] - means[i as usize]);
    ...
    let mut n_classes: i64 = (means.len() as i64);
    return ((1.0 / ((total_count - n_classes) as f64)) * sum_sq)
};
        unsafe fn predict_y_values(mut x_items: Vec<Vec<f64>>, mut means: Vec<f64>, mut variance: f64, mut probabilities: Vec<f64>) -> Vec<i64> {
    ...
    while (i < (x_items.len() as i64)) {
        ...
        while (j < (x_items[i as usize].clone().len() as i64)) {
            ...
            while (k < (x_items.len() as i64)) {
                let mut discr: f64 = (((x_items[i as usize].clone()[j as usize] * (means[k as usize] / variance)) - ((means[k as usize] * means[k as usize]) / (2.0 * variance))) + ln(probabilities[k as usize]));
    ...
    return results
        ...
        unsafe fn accuracy(mut actual_y: Vec<i64>, mut predicted_y: Vec<i64>) -> f64 {
    ...
    g_seed = 1;
    let mut counts: Vec<i64> = vec![20, 20, 20];
    let mut means: Vec<f64> = vec![5.0, 10.0, 15.0];
    let mut std_dev: f64 = 1.0;
    ...
    let mut y: Vec<i64> = y_generator((counts.len() as i64), counts.clone());
    ...
    let mut variance: f64 = calculate_variance(x.clone(), actual_means.clone(), total_count);
    let mut predicted: Vec<i64> = predict_y_values(x.clone(), actual_means.clone(), variance, probabilities.clone());
