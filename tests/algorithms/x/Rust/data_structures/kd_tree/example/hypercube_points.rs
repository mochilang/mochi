...
fn main() {
    ...
    return ((rand() as f64) / 2147483648.0)
        ...
        fn hypercube_points(mut num_points: i64, mut hypercube_size: f64, mut num_dimensions: i64) -> Vec<Vec<f64>> {
    ...
    while (i < num_points) {
        ...
        while (j < num_dimensions) {
            let value: f64 = (hypercube_size * random());
    ...
    return points
        ...
        let pts: Vec<Vec<f64>> = hypercube_points(3, 1.0, 2);
        println!("{:?}", format!("{:?}", pts));
        let _end: i64 = _now();
