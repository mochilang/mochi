...
fn main() {
    ...
    let new_main: Vec<i64> = s.temp_queue.clone();
    ...
    let item: i64 = s.main_queue.clone()[0 as usize];
    (*s).main_queue = s.main_queue.clone()[1 as usize..(s.main_queue.clone().len() as i64) as usize].to_vec();
    return item
};
    fn peek(mut s: StackWithQueues) -> i64 {
    if ((s.main_queue.clone().len() as i64) == 0) {
        panic!("peek from empty stack");
    }
    return s.main_queue.clone()[0 as usize]
};
    fn is_empty(mut s: StackWithQueues) -> bool {
    ...
    push(&mut stack, 2);
    push(&mut stack, 3);
    println!("{}", peek(stack.clone()).to_string());
    println!("{}", pop(&mut stack).to_string());
    println!("{}", peek(stack.clone()).to_string());
    println!("{}", pop(&mut stack).to_string());
    println!("{}", pop(&mut stack).to_string());
    println!("{}", is_empty(stack.clone()).to_string());
    let _end: i64 = _now();
