...
fn main() {
    ...
    return Stack {nodes: vec![], head: (-1)}
};
    fn push(mut stack: Stack, mut value: i64) -> Stack {
    let mut nodes: Vec<Node> = stack.nodes.clone();
    let idx: i64 = (nodes.len() as i64);
    let mut new_node: Node = Node {data: value, next: stack.head, prev: (-1)};
    ...
    return Stack {nodes: nodes.clone(), head: idx}
};
    ...
    let mut nodes: Vec<Node> = stack.nodes.clone();
    let mut head_node: Node = nodes[stack.head as usize].clone();
    let value: i64 = head_node.data;
    let next_idx: i64 = head_node.next;
    if (next_idx != (-1)) {
        let mut next_node: Node = nodes[next_idx as usize].clone();
        next_node.prev = (-1);
        nodes[next_idx as usize] = next_node.clone();
    }
    let new_stack: Stack = Stack {nodes: nodes.clone(), head: next_idx};
    return PopResult {stack: new_stack.clone(), value: value, ok: true}
};
    fn top(mut stack: Stack) -> TopResult {
    if (stack.head == (-1)) {
        return TopResult {value: 0, ok: false}
    }
    let node: Node = stack.nodes.clone()[stack.head as usize].clone();
    return TopResult {value: node.data, ok: true}
};
    fn size(mut stack: Stack) -> i64 {
    let mut count: i64 = 0;
    let mut idx: i64 = stack.head;
    while (idx != (-1)) {
        count = (count + 1);
        let node: Node = stack.nodes.clone()[idx as usize].clone();
        idx = node.next;
    }
    return count
};
    ...
    stack = push(stack.clone(), 4);
    stack = push(stack.clone(), 5);
    stack = push(stack.clone(), 6);
    stack = push(stack.clone(), 7);
    print_stack(stack.clone());
    let t: TopResult = top(stack.clone());
    if t.ok {
        println!("{}", format!("{}{}", "Top element is ", t.value.to_string()));
    } else {
        println!("{}", "Top element is None");
    }
    println!("{}", format!("{}{}", "Size of the stack is ", size(stack.clone()).to_string()));
    let mut p: PopResult = pop(stack.clone());
    stack = p.stack.clone();
    p = pop(stack.clone());
    stack = p.stack.clone();
    print_stack(stack.clone());
    println!("{}", format!("{}{}", "stack is empty: ", is_empty(stack.clone()).to_string()));
};
    mochi_main();
    let _end: i64 = _now();
