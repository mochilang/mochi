...
fn main() {
        let _start: i64 = _now();
    fn calculation_span(mut price: Vec<i64>) -> Vec<i64> {
    let n: i64 = (price.len() as i64);
    ...
    for i in 1..n {
        ...
        let s: i64 = if ((st.len() as i64) <= 0) { (i + 1) } else { (i - st[((st.len() as i64) - 1) as usize]) };
        span = { let mut _v = span.clone(); _v.push(s); _v };
        st = { let mut _v = st.clone(); _v.push(i); _v };
    }
    return span
};
    fn print_array(mut arr: Vec<i64>) {
    for i in 0..(arr.len() as i64) {
        println!("{}", arr[i as usize]);
    }
};
    let price: Vec<i64> = vec![10, 4, 5, 90, 120, 80];
    let spans: Vec<i64> = calculation_span(price.clone());
