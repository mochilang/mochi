...
fn main() {
    ...
    if ((token.len() as i64) > 0) {
        let first: String = { let tmp = &token; tmp.chars().skip(0 as usize).take((1 - 0) as usize).collect::<String>() }.clone();
        if (first.as_str() == "-") {
            sign = -1.0;
            idx = 1;
    ...
    if ((idx < (token.len() as i64)) && ({ let tmp = &token; tmp.chars().skip(idx as usize).take(((idx + 1) - idx) as usize).collect::<String>() }.as_str() == ".")) {
        ...
        while (idx < (token.len() as i64)) {
            let digit = { let n: i64 = { let tmp = &token; tmp.chars().skip(idx as usize).take(((idx + 1) - idx) as usize).collect::<String>() }.parse().unwrap(); n };
    ...
    fn apply_op(mut a: f64, mut b: f64, mut op: String) -> f64 {
    if (op.as_str() == "+") {
        return (a + b)
    }
    if (op.as_str() == "-") {
    ...
    for token in tokens.iter().cloned() {
        if (((((token.as_str() == "+") || (token.as_str() == "-")) || (token.as_str() == "*")) || (token.as_str() == "/")) || (token.as_str() == "^")) {
            if (((token.as_str() == "+") || (token.as_str() == "-")) && ((stack.len() as i64) < 2)) {
                let b: f64 = stack[((stack.len() as i64) - 1) as usize];
                ...
                let b: f64 = stack[((stack.len() as i64) - 1) as usize];
                stack = slice_without_last(stack.clone());
                let a: f64 = stack[((stack.len() as i64) - 1) as usize];
                stack = slice_without_last(stack.clone());
                let result: f64 = apply_op(a, b, token.clone());
    ...
    println!("{}", evaluate(vec![String::from("2").clone(), String::from("1").clone(), String::from("+").clone(), String::from("3").clone(), String::from("*").clone()]).to_string());
    println!("{}", evaluate(vec![String::from("4").clone(), String::from("13").clone(), String::from("5").clone(), String::from("/").clone(), String::from("+").clone()]).to_string());
    println!("{}", evaluate(vec![String::from("5").clone(), String::from("6").clone(), String::from("9").clone(), String::from("*").clone(), String::from("+").clone()]).to_string());
    println!("{}", evaluate(vec![String::from("2").clone(), String::from("-").clone(), String::from("3").clone(), String::from("+").clone()]).to_string());
