...
fn main() {
        let _start: i64 = _now();
    fn new_suffix_tree_node(mut children: HashMap<String, i64>, mut is_end_of_string: bool, mut start: i64, mut end: i64, mut suffix_link: i64) -> SuffixTreeNode {
    ...
    return new_suffix_tree_node(HashMap::new(), false, (-1), (-1), (-1))
};
    ...
    let root: SuffixTreeNode = new_suffix_tree_node(HashMap::from([(String::from("a"), 1)]), false, (-1), (-1), (-1));
    let leaf: SuffixTreeNode = new_suffix_tree_node(HashMap::new(), true, 0, 2, 0);
    let mut nodes: Vec<SuffixTreeNode> = vec![root.clone(), leaf.clone()];
    let mut root_check: SuffixTreeNode = nodes[0 as usize].clone();
    let mut leaf_check: SuffixTreeNode = nodes[1 as usize].clone();
    println!("{}", has_key(root_check.children.clone().clone(), String::from("a")).to_string());
