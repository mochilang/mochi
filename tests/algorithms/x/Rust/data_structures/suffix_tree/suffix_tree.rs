...
fn main() {
    ...
    fn build_suffix_tree(mut tree: SuffixTree) -> SuffixTree {
    let text: String = tree.text.clone().clone();
    ...
    while (i < n) {
        ...
        t = add_suffix(t.clone(), suffix.clone(), i);
        i = (i + 1);
    }
    return t
};
    fn new_suffix_tree(mut text: String) -> SuffixTree {
    let mut tree: SuffixTree = SuffixTree {text: text.clone(), nodes: vec![]};
    tree.nodes = { let mut _v = tree.nodes.clone().clone(); _v.push(new_node()); _v };
    tree = build_suffix_tree(tree.clone());
    return tree
};
    fn search(mut tree: SuffixTree, mut pattern: String) -> bool {
    let mut node_idx: i64 = 0;
    let mut i: i64 = 0;
    let nodes: Vec<Node> = tree.nodes.clone();
    while (i < (pattern.len() as i64)) {
        let ch: String = { let tmp = &pattern; tmp.chars().skip(i as usize).take(((i + 1) - i) as usize).collect::<String>() }.clone();
        let node: Node = nodes[node_idx as usize].clone();
    ...
    let st: SuffixTree = new_suffix_tree(String::from("bananas"));
    println!("{}", search(st.clone(), String::from("ana")).to_string());
    println!("{}", search(st.clone(), String::from("apple")).to_string());
    let _end: i64 = _now();
