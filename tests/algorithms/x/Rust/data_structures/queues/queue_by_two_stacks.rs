...
fn main() {
    ...
    return Queue {stack1: items.clone(), stack2: vec![]}
};
    ...
    let mut i: i64 = ((q.stack2.clone().len() as i64) - 1);
    while (i >= 0) {
    ...
    return Queue {stack1: s1.clone(), stack2: q.stack2.clone()}
};
    ...
    if ((s2.len() as i64) == 0) {
        while ((s1.len() as i64) > 0) {
            let idx: i64 = ((s1.len() as i64) - 1);
            let v: i64 = s1[idx as usize];
    ...
    let idx2: i64 = ((s2.len() as i64) - 1);
    let value: i64 = s2[idx2 as usize];
    ...
    s2 = new_s2.clone();
    return GetResult {queue: Queue {stack1: s1.clone(), stack2: s2.clone()}, value: value}
};
    let mut q: Queue = new_queue(vec![10, 20, 30]);
    let r1: GetResult = get(q.clone());
    q = r1.queue.clone();
    println!("{}", r1.value);
    q = put(q.clone(), 40);
    let r2: GetResult = get(q.clone());
    q = r2.queue.clone();
    println!("{}", r2.value);
    let r3: GetResult = get(q.clone());
    q = r3.queue.clone();
    println!("{}", r3.value);
    println!("{}", len_queue(q.clone()));
    let r4: GetResult = get(q.clone());
    q = r4.queue.clone();
    println!("{}", r4.value);
    let _end: i64 = _now();
