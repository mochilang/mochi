...
fn main() {
    ...
    return LinkedQueue {nodes: vec![], front: (-1), rear: (-1)}
};
    fn is_empty(mut q: LinkedQueue) -> bool {
    return (q.front == (-1))
};
    fn put(q: &mut LinkedQueue, mut item: String) {
    let node: Node = Node {data: item.clone(), next: (-1)};
    (*q).nodes = { let mut _v = q.nodes.clone().clone(); _v.push(node.clone()); _v };
    let idx: i64 = ((q.nodes.clone().len() as i64) - 1);
    if (q.front == (-1)) {
        (*q).front = idx;
        (*q).rear = idx;
    } else {
    ...
    let idx: i64 = q.front;
    let node: Node = q.nodes.clone()[idx as usize].clone();
    (*q).front = node.next;
    if (q.front == (-1)) {
        (*q).rear = (-1);
    }
    ...
    fn length(mut q: LinkedQueue) -> i64 {
    let mut count: i64 = 0;
    ...
    fn to_string(mut q: LinkedQueue) -> String {
    ...
    println!("{}", is_empty(queue.clone()).to_string());
    put(&mut queue, String::from("5"));
    put(&mut queue, String::from("9"));
    put(&mut queue, String::from("python"));
    println!("{}", is_empty(queue.clone()).to_string());
    println!("{}", get(&mut queue));
    put(&mut queue, String::from("algorithms"));
    println!("{}", get(&mut queue));
    println!("{}", get(&mut queue));
    println!("{}", get(&mut queue));
    println!("{}", is_empty(queue.clone()).to_string());
