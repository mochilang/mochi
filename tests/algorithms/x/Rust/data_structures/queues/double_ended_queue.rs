...
fn main() {
    ...
    fn push_back(mut dq: Deque, mut value: i64) -> Deque {
    return Deque {data: { let mut _v = dq.data.clone().clone(); _v.push(value); _v }}
};
    fn push_front(mut dq: Deque, mut value: i64) -> Deque {
    ...
    return Deque {data: res.clone()}
};
    fn extend_back(mut dq: Deque, mut values: Vec<i64>) -> Deque {
    ...
    fn extend_front(mut dq: Deque, mut values: Vec<i64>) -> Deque {
    let mut res: Vec<i64> = vec![];
    let mut i: i64 = ((values.len() as i64) - 1);
    while (i >= 0) {
        res = { let mut _v = res.clone(); _v.push(values[i as usize]); _v };
        i = (i - 1);
    }
    ...
    fn pop_back(mut dq: Deque) -> PopResult {
    ...
    return PopResult {deque: Deque {data: res.clone()}, value: dq.data.clone()[((dq.data.clone().len() as i64) - 1) as usize]}
};
    fn pop_front(mut dq: Deque) -> PopResult {
    if ((dq.data.clone().len() as i64) == 0) {
    ...
    return PopResult {deque: Deque {data: res.clone()}, value: dq.data.clone()[0 as usize]}
};
    fn is_empty(mut dq: Deque) -> bool {
    return ((dq.data.clone().len() as i64) == 0)
};
    fn length(mut dq: Deque) -> i64 {
    return (dq.data.clone().len() as i64)
};
    fn to_string(mut dq: Deque) -> String {
    if ((dq.data.clone().len() as i64) == 0) {
        return String::from("[]")
    }
    ...
    dq = push_back(dq.clone(), 2);
    dq = push_front(dq.clone(), 1);
    dq = extend_back(dq.clone(), vec![3, 4]);
    dq = extend_front(dq.clone(), vec![0]);
    println!("{}", to_string(dq.clone()));
    let mut r: PopResult = pop_back(dq.clone());
    dq = r.deque.clone();
    println!("{}", r.value);
    r = pop_front(dq.clone());
    dq = r.deque.clone();
    println!("{}", r.value);
    println!("{}", to_string(dq.clone()));
