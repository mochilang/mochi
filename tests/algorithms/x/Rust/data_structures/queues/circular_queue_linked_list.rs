...
fn main() {
    ...
    return CircularQueue {data: data.clone(), next: next.clone(), prev: prev.clone(), front: 0, rear: 0}
};
    fn is_empty(mut q: CircularQueue) -> bool {
    return ((q.front == q.rear) && (q.data.clone()[q.front as usize].clone().as_str() == ""))
};
    fn check_can_perform(mut q: CircularQueue) {
    if is_empty(q.clone()) {
        panic!("Empty Queue");
    }
};
    fn check_is_full(mut q: CircularQueue) {
    if (q.next.clone()[q.rear as usize] == q.front) {
        panic!("Full Queue");
    }
};
    fn peek(mut q: CircularQueue) -> String {
    check_can_perform(q.clone());
    return q.data.clone()[q.front as usize].clone().clone()
};
    fn enqueue(mut q: CircularQueue, mut value: String) -> CircularQueue {
    check_is_full(q.clone());
    if !is_empty(q.clone()) {
        q.rear = q.next.clone()[q.rear as usize];
    }
    let mut data: Vec<String> = q.data.clone();
    data[q.rear as usize] = value.clone();
    q.data = data.clone();
    return q
};
    ...
    check_can_perform(q.clone());
    let mut data: Vec<String> = q.data.clone();
    let val: String = data[q.front as usize].clone().clone();
    ...
    println!("{}", is_empty(q.clone()).to_string());
    q = enqueue(q.clone(), String::from("a"));
    q = enqueue(q.clone(), String::from("b"));
    println!("{}", peek(q.clone()));
