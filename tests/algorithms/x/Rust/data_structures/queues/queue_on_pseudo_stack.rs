...
fn main() {
    ...
    return Queue {stack: s.clone(), length: (q.length + 1)}
};
    ...
    while ((i < rotation) && ((s.len() as i64) > 0)) {
        let temp: i64 = s[0 as usize];
        s = drop_first(s.clone());
        s = { let mut _v = s.clone(); _v.push(temp); _v };
        i = (i + 1);
    }
    return Queue {stack: s.clone(), length: q.length}
};
    fn get(mut q: Queue) -> GetResult {
    if (q.length == 0) {
        panic!("queue empty");
    }
    let mut q1: Queue = rotate(q.clone(), 1);
    let v: i64 = q1.stack.clone()[(q1.length - 1) as usize];
    let mut s: Vec<i64> = drop_last(q1.stack.clone().clone());
    let mut q2: Queue = Queue {stack: s.clone(), length: q1.length};
    q2 = rotate(q2.clone(), (q2.length - 1));
    q2 = Queue {stack: q2.stack.clone(), length: (q2.length - 1)};
    return GetResult {queue: q2.clone(), value: v}
};
    fn front(mut q: Queue) -> FrontResult {
    let r: GetResult = get(q.clone());
    let mut q2: Queue = put(r.queue.clone().clone(), r.value);
    q2 = rotate(q2.clone(), (q2.length - 1));
    return FrontResult {queue: q2.clone(), value: r.value}
};
    fn size(mut q: Queue) -> i64 {
    return q.length
};
    fn to_string(mut q: Queue) -> String {
    ...
    q = put(q.clone(), 2);
    q = put(q.clone(), 3);
    println!("{}", to_string(q.clone()));
    let g: GetResult = get(q.clone());
    q = g.queue.clone();
    println!("{}", g.value);
    println!("{}", to_string(q.clone()));
    let f: FrontResult = front(q.clone());
    q = f.queue.clone();
    println!("{}", f.value);
    println!("{}", to_string(q.clone()));
    println!("{}", size(q.clone()));
};
    mochi_main();
    let _end: i64 = _now();
