...
fn main() {
    ...
    return CircularQueue {data: arr.clone(), front: 0, rear: 0, size: 0, capacity: capacity}
};
    fn length(mut q: CircularQueue) -> i64 {
    return q.size
};
    fn is_empty(mut q: CircularQueue) -> bool {
    return (q.size == 0)
};
    fn front(mut q: CircularQueue) -> i64 {
    if is_empty(q.clone()) {
        return 0
    }
    ...
    let value: i64 = q.data.clone()[q.front as usize];
    ...
    println!("{}", if is_empty(q.clone()) { 1 } else { 0 });
    q = enqueue(q.clone(), 10);
    println!("{}", if is_empty(q.clone()) { 1 } else { 0 });
    q = enqueue(q.clone(), 20);
    q = enqueue(q.clone(), 30);
    println!("{}", front(q.clone()));
    let mut r: DequeueResult = dequeue(&mut q);
    q = r.queue.clone();
    println!("{}", r.value);
    println!("{}", front(q.clone()));
    println!("{}", length(q.clone()));
};
    mochi_main();
    let _end: i64 = _now();
