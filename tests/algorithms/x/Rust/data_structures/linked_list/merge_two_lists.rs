...
fn main() {
    ...
    fn len_sll(mut sll: SortedLinkedList) -> i64 {
    return (sll.values.clone().len() as i64)
};
    fn str_sll(mut sll: SortedLinkedList) -> String {
    ...
    fn merge_lists(mut a: SortedLinkedList, mut b: SortedLinkedList) -> SortedLinkedList {
    ...
    let mut mochi_main = || {
    let test_data_odd: Vec<i64> = vec![3, 9, -11, 0, 7, 5, 1, -1];
    let test_data_even: Vec<i64> = vec![4, 6, 2, 0, 8, 10, 3, -2];
    let sll_one: SortedLinkedList = make_sorted_linked_list(test_data_odd.clone());
    let sll_two: SortedLinkedList = make_sorted_linked_list(test_data_even.clone());
    let merged: SortedLinkedList = merge_lists(sll_one.clone(), sll_two.clone());
    println!("{}", len_sll(merged.clone()).to_string());
    println!("{}", str_sll(merged.clone()));
};
    mochi_main();
    let _end: i64 = _now();
