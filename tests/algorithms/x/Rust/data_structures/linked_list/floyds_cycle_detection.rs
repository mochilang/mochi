...
fn main() {
        let _start: i64 = _now();
    let NULL: i64 = (-1);
    let mut empty_list = || -> LinkedList {
    return LinkedList {next: vec![], head: NULL}
};
    let mut add_node = |mut list: LinkedList, mut value: i64| -> LinkedList {
    let mut nexts: Vec<i64> = list.next.clone();
    let new_index: i64 = (nexts.len() as i64);
    nexts = { let mut _v = nexts.clone(); _v.push(NULL); _v };
    if (list.head == NULL) {
        return LinkedList {next: nexts.clone(), head: new_index}
    }
    ...
    return LinkedList {next: new_nexts.clone(), head: list.head}
};
    let mut set_next = |mut list: LinkedList, mut index: i64, mut next_index: i64| -> LinkedList {
    let mut nexts: Vec<i64> = list.next.clone();
    let mut new_nexts: Vec<i64> = vec![];
    let mut i: i64 = 0;
    while (i < (nexts.len() as i64)) {
    ...
    let mut detect_cycle = |mut list: LinkedList| -> bool {
    if (list.head == NULL) {
        return false
    }
    let nexts: Vec<i64> = list.next.clone();
    ...
    ll = add_node(ll.clone(), 1);
    ll = add_node(ll.clone(), 2);
    ll = add_node(ll.clone(), 3);
    ll = add_node(ll.clone(), 4);
    ll = set_next(ll.clone(), 3, 1);
    println!("{}", if detect_cycle(ll.clone()) { 1 } else { 0 });
};
    mochi_main();
    let _end: i64 = _now();
