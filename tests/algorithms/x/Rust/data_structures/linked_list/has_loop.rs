...
fn main() {
    ...
    while (fast != (-1)) {
        let fast_node1: Node = nodes[fast as usize].clone();
        if (fast_node1.next == (-1)) {
            return false
        }
        let fast_node2: Node = nodes[fast_node1.next as usize].clone();
        if (fast_node2.next == (-1)) {
            return false
        }
        let slow_node: Node = nodes[slow as usize].clone();
        slow = slow_node.next;
    ...
    while (i < (values.len() as i64)) {
        let next_idx: i64 = if (i == ((values.len() as i64) - 1)) { (-1) } else { (i + 1) };
    ...
    println!("{}", has_loop(list1.clone(), 0).to_string());
    let list2: Vec<Node> = make_nodes(vec![5, 6, 5, 6]);
    println!("{}", has_loop(list2.clone(), 0).to_string());
    let list3: Vec<Node> = make_nodes(vec![1]);
