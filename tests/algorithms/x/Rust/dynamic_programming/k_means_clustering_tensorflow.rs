...
fn main() {
    ...
    while (i < dim) {
        ...
        res = { let mut _v = res.clone(); _v.push((total / (vectors.len() as i64))); _v };
        i = (i + 1);
    }
    return res
};
    ...
    fn mochi_main() {
    let mut vectors: Vec<Vec<f64>> = vec![vec![1.0, 2.0], vec![1.5, 1.8], vec![5.0, 8.0], vec![8.0, 8.0], vec![1.0, 0.6], vec![9.0, 11.0]];
    let mut result: KMeansResult = k_means(vectors.clone(), 2, 5);
