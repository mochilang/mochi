...
let rec is_pangram input_str =
  ...
  (try while (!i < String.length (input_str)) do
    try
  let c = String.lowercase_ascii (String.make 1 (String.get (input_str) !i)) in
...
and is_pangram_faster input_str =
  ...
  (try while (!j < String.length (input_str)) do
    try
  let c = String.lowercase_ascii (String.make 1 (String.get (input_str) !j)) in
  let k = ref (0) in
  (try while (!k < 26) do
    try
  if (String.make 1 (String.get (alphabet) !k) = c) then (
...
and is_pangram_fastest input_str =
  ...
  (try while (!i < String.length (alphabet)) do
    try
  let letter = String.make 1 (String.get (alphabet) !i) in
