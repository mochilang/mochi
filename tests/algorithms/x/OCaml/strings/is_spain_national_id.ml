...
let rec to_upper s =
  ...
  (try while (!j < String.length (_lower)) do
    try
  if (String.make 1 (String.get (_lower) !j) = ch) then (
  converted := String.make 1 (String.get (_upper) !j);
  raise Break;
  );
  j := (!j + 1);
    with Continue -> ()
...
and clean_id spanish_id =
  ...
  (try while (!i < String.length (upper_id)) do
    try
  let ch = String.make 1 (String.get (upper_id) !i) in
  if (ch <> "-") then (
  cleaned := (!cleaned ^ ch);
  );
  i := (!i + 1);
...
and is_spain_national_id spanish_id =
  ...
  (try while (!i < 8) do
    try
  if not (is_digit (String.make 1 (String.get (sid) !i))) then (
  (failwith (_error_msg));
  );
  i := (!i + 1);
    with Continue -> ()
  ...
  let letter = String.make 1 (String.get (sid) 8) in
  if is_digit (letter) then (
  (failwith (_error_msg));
  );
  let expected = String.make 1 (String.get (_lookup_letters) ((number mod 23 + 23) mod 23)) in
