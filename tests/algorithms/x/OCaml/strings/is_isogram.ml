...
and is_alpha c =
  ...
  (try
  let code = ord (c) in
  __ret := (Obj.magic ((((code >= 65) && (code <= 90)) || ((code >= 97) && (code <= 122)))) : bool); raise Return
  with Return -> !__ret)
...
let () =
  ...
  let start = _now () in
  print_endline ((string_of_bool (is_isogram ("Uncopyrightable"))));
  print_endline ((string_of_bool (is_isogram ("allowance"))));
  let finish = _now () in
  let mem_end = _mem () in
