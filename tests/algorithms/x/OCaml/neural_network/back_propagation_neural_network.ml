...
and sigmoid_derivative out =
  ...
  (try while (!i < List.length (out)) do
    try
  let val_ = (let __l = out in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
...
and matvec mat vec =
  ...
  (try while (!j < List.length (vec)) do
    try
  s := (!s +. ((let __l = (let __l = mat in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) *. (let __l = vec in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))));
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
...
and matTvec mat vec =
  ...
  let mat = (Obj.magic mat : float list list) in
  let vec = (Obj.magic vec : float list) in
  let cols = ref (List.length ((let __l = mat in let __i = 0 in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)))) in
  let res = ref (([] : (float) list)) in
  let j = ref (0) in
  (try while (!j < !cols) do
    try
  ...
  (try while (!i < List.length (mat)) do
    try
  s := (!s +. ((let __l = (let __l = mat in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) *. (let __l = vec in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  res := (Obj.magic (List.append (!res) [(Obj.magic (!s) : float)]) : float list);
...
and vec_scalar_mul v s =
  ...
  (try while (!i < List.length (v)) do
    try
  res := (Obj.magic (List.append (!res) [(Obj.magic (((let __l = v in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) *. s)) : float)]) : float list);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  __ret := (Obj.magic (!res) : float list); raise Return
...
and outer a b =
  ...
  (try while (!j < List.length (b)) do
    try
  row := (Obj.magic (List.append (!row) [(Obj.magic (((let __l = a in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) *. (let __l = b in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)))) : float)]) : float list);
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
...
and mat_scalar_mul mat s =
  ...
  (try while (!j < List.length ((let __l = mat in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)))) do
    try
  row := (Obj.magic (List.append (!row) [(Obj.magic (((let __l = (let __l = mat in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) *. s)) : float)]) : float list);
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
  res := (Obj.magic (List.append (!res) [(Obj.magic (!row) : float list)]) : float list list);
...
and mat_sub a b =
  ...
  let row = ref (([] : (float) list)) in
  let j = ref (0) in
  (try while (!j < List.length ((let __l = a in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)))) do
    try
...
and init_layer units back_units lr =
  ...
  __ret := (Obj.magic ([(__str ("units"), Obj.repr (units)); (__str ("weight"), Obj.repr (random_matrix (Obj.repr (units)) (Obj.repr (back_units)))); (__str ("bias"), Obj.repr (random_vector (Obj.repr (units)))); (__str ("output"), Obj.repr ([])); (__str ("xdata"), Obj.repr ([])); (__str ("learn_rate"), Obj.repr (lr))]) : (string * Obj.t) list); raise Return
  with Return -> !__ret)
...
and forward layers x =
  ...
  (try while (!i < List.length (!layers)) do
    try
  let layer = ref ((let __l = !layers in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) in
  layer := ((__str ("xdata"), Obj.repr (Obj.magic (!data) : float list)) :: List.remove_assoc (__str ("xdata")) (Obj.magic (!layer) : (string * Obj.t) list));
  if (!i = 0) then (
  layer := ((__str ("output"), Obj.repr (Obj.magic (!data) : float list)) :: List.remove_assoc (__str ("output")) (Obj.magic (!layer) : (string * Obj.t) list));
  ) else (
  let z = ref (vec_sub (matvec ((Obj.obj (List.assoc (__str ("weight")) (!layer) : Obj.t) : float list list)) (!data)) ((Obj.obj (List.assoc (__str ("bias")) (!layer) : Obj.t) : float list))) in
  layer := ((__str ("output"), Obj.repr (Obj.magic (sigmoid_vec (!z)) : float list)) :: List.remove_assoc (__str ("output")) (Obj.magic (!layer) : (string * Obj.t) list));
  data := (Obj.magic (Obj.obj (List.assoc (__str ("output")) (!layer) : Obj.t) : float list) : float list);
  );
...
and backward layers grad =
  ...
  (try while (!i > 0) do
    try
  let layer = ref ((let __l = !layers in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) in
  let deriv = ref (sigmoid_derivative ((Obj.obj (List.assoc (__str ("output")) (!layer) : Obj.t) : float list))) in
  let delta = ref (vec_mul (!g) (!deriv)) in
  let grad_w = ref (outer (!delta) ((Obj.obj (List.assoc (__str ("xdata")) (!layer) : Obj.t) : float list))) in
  layer := ((__str ("weight"), Obj.repr (Obj.magic (mat_sub ((Obj.obj (List.assoc (__str ("weight")) (!layer) : Obj.t) : float list list)) (mat_scalar_mul (!grad_w) (Obj.repr ((Obj.obj (List.assoc (__str ("learn_rate")) (!layer) : Obj.t) : float))))) : float list list)) :: List.remove_assoc (__str ("weight")) (Obj.magic (!layer) : (string * Obj.t) list));
  layer := ((__str ("bias"), Obj.repr (Obj.magic (vec_sub ((Obj.obj (List.assoc (__str ("bias")) (!layer) : Obj.t) : float list)) (vec_scalar_mul (!delta) (Obj.repr ((Obj.obj (List.assoc (__str ("learn_rate")) (!layer) : Obj.t) : float))))) : float list)) :: List.remove_assoc (__str ("bias")) (Obj.magic (!layer) : (string * Obj.t) list));
  g := (Obj.magic matTvec ((Obj.obj (List.assoc (__str ("weight")) (!layer) : Obj.t) : float list list)) (!delta) : float list);
  layers := (List.mapi (fun __i __x -> if __i = !i then !layer else __x) (!layers));
  i := (!i - 1);
    with Continue -> ()
  done with Break -> ());
...
and calc_loss y yhat =
  ...
  (try while (!i < List.length (y)) do
    try
  let d = ((let __l = y in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) -. (let __l = yhat in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) in
  s := (!s +. (d *. d));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
and calc_gradient y yhat =
  ...
  (try while (!i < List.length (y)) do
    try
  g := (Obj.magic (List.append (!g) [(Obj.magic ((2.0 *. ((let __l = yhat in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) -. (let __l = y in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))))) : float)]) : float list);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
and train layers xdata ydata rounds acc =
  ...
  (try while (!i < List.length (xdata)) do
    try
  layers := forward (layers) ((let __l = xdata in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)));
  let out = ref ((Obj.obj (List.assoc (__str ("output")) ((let __l = !layers in let __i = (List.length (!layers) - 1) in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : Obj.t) : float list)) in
  let grad = ref (calc_gradient ((let __l = ydata in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) (!out)) in
...
and create_data () =
  ...
  __ret := (Obj.magic ([(__str ("x"), Obj.repr (!x)); (__str ("y"), Obj.repr (!y))]) : (string * Obj.t) list); raise Return
  with Return -> !__ret)
...
and main () =
  ...
  let x = ref ((Obj.obj (List.assoc (__str ("x")) (data) : Obj.t) : float list list)) in
  let y = ref ((Obj.obj (List.assoc (__str ("y")) (data) : Obj.t) : float list list)) in
