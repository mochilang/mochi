...
let rec multiply matrix_a matrix_b =
  ...
  (try while (!k < n) do
    try
  val_ := (!val_ + ((let __l = (let __l = matrix_a in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = !k in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) * (let __l = (let __l = matrix_b in let __i = !k in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))));
  k := (!k + 1);
    with Continue -> ()
  done with Break -> ());
...
and nth_fibonacci_matrix n =
  ...
  __ret := (Obj.magic ((let __l = (let __l = !res_matrix in let __i = 0 in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = 0 in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : int); raise Return
  with Return -> !__ret)
...
and main () =
  ...
  (try while (!i < List.length (!ordinals)) do
    try
  let ordinal = (let __l = !ordinals in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
  let n = parse_number (ordinal) in
  let msg = ((((ordinal ^ " fibonacci number using matrix exponentiation is ") ^ (string_of_int (nth_fibonacci_matrix (Obj.repr (n))))) ^ " and using bruteforce is ") ^ (string_of_int (nth_fibonacci_bruteforce (Obj.repr (n))))) in
  print_endline (msg);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
