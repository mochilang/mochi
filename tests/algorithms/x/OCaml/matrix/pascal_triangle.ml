...
let rec populate_current_row triangle current_row_idx =
  ...
  let left = (let __l = (let __l = triangle in let __i = (current_row_idx - 1) in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = (!i - 1) in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
  let right = (let __l = (let __l = triangle in let __i = (current_row_idx - 1) in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
...
and row_to_string row total_rows row_idx =
  ...
  (try while (!c <= row_idx) do
    try
  line := (!line ^ (string_of_int ((let __l = row in let __i = !c in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)))));
  if (!c <> row_idx) then (
  line := (!line ^ " ");
  );
  c := (!c + 1);
...
and print_pascal_triangle num_rows =
  ...
  (try while (!r < num_rows) do
    try
  let line = row_to_string ((let __l = !triangle in let __i = !r in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) (Obj.repr (num_rows)) (Obj.repr (!r)) in
  print_endline (line);
  r := (!r + 1);
    with Continue -> ()
  done with Break -> ());
