...
and median matrix =
  ...
  let linear = ref (([] : (int) list)) in
  let i = ref (0) in
  (try while (!i < List.length (matrix)) do
    try
  let row = ref ((let __l = matrix in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) in
  let j = ref (0) in
  (try while (!j < List.length (!row)) do
    try
  linear := (Obj.magic (List.append (!linear) [(Obj.magic ((let __l = !row in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : int)]) : int list);
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
  i := (!i + 1);
  ...
  let sorted = ref (bubble_sort (!linear)) in
  let mid = ((List.length (!sorted) - 1) / 2) in
  __ret := (Obj.magic ((let __l = !sorted in let __i = mid in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : int); raise Return
  with Return -> !__ret)


let matrix1 = ref ([[1; 3; 5]; [2; 6; 9]; [3; 6; 9]])
let () =
  let mem_start = _mem () in
  let start = _now () in
  print_endline ((string_of_int (median (!matrix1))));
  let matrix2 = ref ([[1; 2; 3]; [4; 5; 6]]) in
  print_endline ((string_of_int (median (!matrix2))));
  let finish = _now () in
  let mem_end = _mem () in
