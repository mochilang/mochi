...
and dfs2 u reversed_graph visit component =
  ...
  component := (Obj.magic dfs2 (Obj.repr (v)) (reversed_graph) (visit) (component) : int list);
    with Continue -> ()) (List.nth (reversed_graph) (u)) with Break -> ());
  __ret := (Obj.magic (!component) : int list); raise Return
  with Return -> !__ret)
...
and kosaraju graph =
  ...
  (try List.iter (fun v ->
    try
  reversed_graph := (List.mapi (fun __i __x -> if __i = v then (List.append (List.nth (!reversed_graph) (v)) [(Obj.magic (!i) : int)]) else __x) (!reversed_graph));
    with Continue -> ()) (List.nth (graph) (!i)) with Break -> ());
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  ...
  (try while (!i < n) do
    try
  if (List.nth (!visit) (!i) = false) then (
  ...
  (try while (!idx >= 0) do
    try
  let node = List.nth (!stack) (!idx) in
  if (List.nth (!visit) (node) = false) then (
...
and main () =
  ...
  (try while (!i < List.length (!comps)) do
    try
  print_endline (__show_list (List.nth (!comps) (!i)));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
    !__ret
