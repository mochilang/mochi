...
let rec lin_search left right array target =
  ...
  (try while (!i < right) do
    try
  if ((let __l = array in let __i = !i in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) = target) then (
...
and ite_ternary_search array target =
  ...
  if ((let __l = array in let __i = one_third in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) = target) then (
  __ret := (Obj.magic (one_third) : int); raise Return
  );
  if ((let __l = array in let __i = two_third in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) = target) then (
  __ret := (Obj.magic (two_third) : int); raise Return
  );
  if (target < (let __l = array in let __i = one_third in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0)) then (
  right := (one_third - 1);
  ) else (
  if ((let __l = array in let __i = two_third in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) < target) then (
  left := (two_third + 1);
...
and main () =
  ...
  print_endline ((string_of_int (Obj.magic (ite_ternary_search (!test_list) (Obj.repr (3))) : int)));
  print_endline ((string_of_int (Obj.magic (ite_ternary_search (!test_list) (Obj.repr (13))) : int)));
  print_endline ((string_of_int (Obj.magic (rec_ternary_search (Obj.repr (0)) (Obj.repr ((List.length (!test_list) - 1))) (!test_list) (Obj.repr (3))) : int)));
  print_endline ((string_of_int (Obj.magic (rec_ternary_search (Obj.repr (0)) (Obj.repr ((List.length (!test_list) - 1))) (!test_list) (Obj.repr (13))) : int)));
    !__ret
  with Return -> !__ret)
