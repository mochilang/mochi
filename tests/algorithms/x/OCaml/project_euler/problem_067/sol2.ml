...
and max_path_sum tr =
  let __ret = ref 0 in
  (try
  let tr = (Obj.magic tr : int list list) in
  let prev = ref ((let __l = tr in let __i = (List.length (tr) - 1) in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> [])) in
  let i = ref ((List.length (tr) - 2)) in
  (try while (!i >= 0) do
    try
  let curr = ref ((let __l = tr in let __i = !i in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> [])) in
  let new_row = ref (([] : (int) list)) in
  let j = ref (0) in
  (try while (!j < List.length (!curr)) do
    try
  let best = max_int (Obj.repr ((let __l = !prev in let __i = !j in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0))) (Obj.repr ((let __l = !prev in let __i = (!j + 1) in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0))) in
  new_row := (Obj.magic ((List.append (!new_row) [(Obj.magic (((let __l = !curr in let __i = !j in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) + best)) : int)])) : int list);
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
  prev := (Obj.magic (!new_row) : int list);
  i := (!i - 1);
    with Continue -> ()
  done with Break -> ());
  __ret := (Obj.magic ((let __l = !prev in let __i = 0 in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0)) : int); raise Return
  with Return -> !__ret)
...
let () =
  ...
  print_endline ((string_of_int (Obj.magic (max_path_sum (!triangle)) : int)));
