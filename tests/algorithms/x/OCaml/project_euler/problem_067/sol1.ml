...
let rec max_path tri =
  ...
  (try while (!j < List.length ((let __l = !a in let __i = !i in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> []))) do
    try
  let number1 = if (!j <> List.length ((let __l = !a in let __i = (!i - 1) in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> []))) then (let __l = (let __l = !a in let __i = (!i - 1) in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> []) in let __i = !j in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) else 0 in
  let number2 = if (!j > 0) then (let __l = (let __l = !a in let __i = (!i - 1) in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> []) in let __i = (!j - 1) in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) else 0 in
  a := (List.mapi (fun __i __x -> if __i = !i then (List.mapi (fun __i __x -> if __i = !j then ((let __l = (let __l = !a in let __i = !i in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> []) in let __i = !j in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) + (List.fold_left max min_int [number1; number2])) else __x) ((let __l = !a in let __i = !i in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> []))) else __x) (!a));
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
  i := (!i + 1);
  ...
  __ret := (Obj.magic ((List.fold_left max min_int (let __l = !a in let __i = (List.length (!a) - 1) in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> []))) : int); raise Return
  with Return -> !__ret)
...
let () =
  ...
  print_endline ((string_of_int (Obj.magic (max_path (!triangle)) : int)));
