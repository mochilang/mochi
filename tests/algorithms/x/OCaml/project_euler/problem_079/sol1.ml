...
and contains xs c =
  ...
  (try while (!i < List.length (xs)) do
    try
  if ((let __l = xs in let __i = !i in if __i < 0 then "" else match List.nth_opt __l __i with Some v -> (Obj.magic v : string) | None -> "") = c) then (
...
and unique_chars logins =
  ...
  (try while (!i < List.length (logins)) do
    try
  let login = (let __l = logins in let __i = !i in if __i < 0 then "" else match List.nth_opt __l __i with Some v -> (Obj.magic v : string) | None -> "") in
  let j = ref (0) in
...
and search chars current logins =
  ...
  (try while (!i < List.length (chars)) do
    try
  let c = (let __l = chars in let __i = !i in if __i < 0 then "" else match List.nth_opt __l __i with Some v -> (Obj.magic v : string) | None -> "") in
...
let logins2 = ref (["426"; "281"; "061"; "819"; "268"; "406"; "420"; "428"; "209"; "689"; "019"; "421"; "469"; "261"; "681"; "201"])
let () =
  let bench_mem_start = _mem () in
  let bench_start = _now () in
  print_endline ((string_of_int (Obj.magic (find_secret_passcode (!logins1)) : int)));
  print_endline ((string_of_int (Obj.magic (find_secret_passcode (!logins2)) : int)));
