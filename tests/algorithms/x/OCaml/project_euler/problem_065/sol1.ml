...
let rec sum_digits num =
  ...
  let n = ref (num) in
  let digit_sum = ref (0) in
  (try while (!n > 0) do
    try
  digit_sum := (!digit_sum + ((!n mod 10 + 10) mod 10));
  n := (!n / 10);
    with Continue -> ()
  done with Break -> ());
  __ret := (Obj.magic (!digit_sum) : int); raise Return
  with Return -> !__ret)
...
and solution max_n =
  ...
  let pre_numerator = ref (1) in
  let cur_numerator = ref (2) in
  let i = ref (2) in
  (try while (!i <= max_n) do
    try
  let temp = !pre_numerator in
  let e_cont = ref (1) in
  if (((!i mod 3 + 3) mod 3) = 0) then (
  e_cont := ((2 * !i) / 3);
  );
  ...
  __ret := (Obj.magic (sum_digits (Obj.repr (!cur_numerator))) : int); raise Return
  with Return -> !__ret)
...
let () =
  ...
  print_endline (string_of_int (solution (Obj.repr (9))));
  print_endline (string_of_int (solution (Obj.repr (10))));
  print_endline (string_of_int (solution (Obj.repr (50))));
  print_endline (string_of_int (solution (Obj.repr (100))));
