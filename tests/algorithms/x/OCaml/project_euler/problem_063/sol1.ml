...
and solution max_base max_power =
  let __ret = ref 0 in
  (try
  let max_base = (Obj.magic max_base : int) in
  let max_power = (Obj.magic max_power : int) in
  let total = ref (0) in
  let base = ref (1) in
  (try while (!base < max_base) do
    try
  let power = ref (1) in
  (try while (!power < max_power) do
    try
  let digits = num_digits (Obj.repr (pow (Obj.repr (!base)) (Obj.repr (!power)))) in
...
let () =
  ...
  print_endline (("solution(10, 22) = " ^ (string_of_int (Obj.magic (solution (Obj.repr (10)) (Obj.repr (22))) : int))));
