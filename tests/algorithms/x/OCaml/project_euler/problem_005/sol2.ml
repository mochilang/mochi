...
and lcm x y =
  ...
  __ret := (Obj.magic (((x * y) / gcd (Obj.repr (x)) (Obj.repr (y)))) : int); raise Return
  with Return -> !__ret)
...
and main () =
  ...
  (try
  print_endline ((string_of_int (Obj.magic (solution (Obj.repr (20))) : int)));
    !__ret
  with Return -> !__ret)
