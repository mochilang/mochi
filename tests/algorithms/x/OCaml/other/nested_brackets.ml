...
let _open_to_closed = ref ([(__str ("("), ")"); (__str ("["), "]"); (__str ("{"), "}")])
...
and is_balanced s =
  ...
  if (List.mem_assoc (__str (symbol)) !_open_to_closed) then (
  ...
  let top = (let __l = !stack in let __i = (List.length (!stack) - 1) in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
  if (List.assoc (__str (top)) (!_open_to_closed) <> symbol) then (
  __ret := (Obj.magic (false) : bool); raise Return
  );
