...
and majority_vote votes votes_needed_to_win =
  ...
  (try while (!i < List.length (votes)) do
    try
  let v = (let __l = votes in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
  let idx = index_of (!candidates) (Obj.repr (v)) in
  if (idx <> (0 - 1)) then (
  counts := (List.mapi (fun __i __x -> if __i = idx then ((let __l = !counts in let __i = idx in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) + 1) else __x) (!counts));
  ) else (
  ...
  (try while (!j < List.length (!candidates)) do
    try
  if ((let __l = !counts in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) > 0) then (
  new_candidates := (Obj.magic (List.append (!new_candidates) [(Obj.magic ((let __l = !candidates in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : int)]) : int list);
  new_counts := (Obj.magic (List.append (!new_counts) [(Obj.magic ((let __l = !counts in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : int)]) : int list);
  );
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
  ...
  let idx = index_of (!candidates) (Obj.repr (v)) in
  if (idx <> (0 - 1)) then (
  final_counts := (List.mapi (fun __i __x -> if __i = idx then ((let __l = !final_counts in let __i = idx in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) + 1) else __x) (!final_counts));
  );
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  ...
  (try while (!j < List.length (!candidates)) do
    try
  if (((let __l = !final_counts in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) * votes_needed_to_win) > List.length (votes)) then (
  result := (Obj.magic (List.append (!result) [(Obj.magic ((let __l = !candidates in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : int)]) : int list);
  );
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
