...
and apply_sbox s data =
  ...
  let val_ = (let __l = (let __l = s in let __i = row in if __i < 0 then [] else match List.nth_opt __l __i with Some v -> v | None -> []) in let __i = col in if __i < 0 then 0 else match List.nth_opt __l __i with Some v -> (Obj.magic v : int) | None -> 0) in
