...
and shuffle list_int =
  ...
  let tmp = (let __l = !list_int in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
  list_int := (List.mapi (fun __i __x -> if __i = !i then (let __l = !list_int in let __i = j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) else __x) (!list_int));
...
and make_word_search words width height =
  ...
  __ret := (Obj.magic ([(__str ("words"), Obj.repr (words)); (__str ("width"), Obj.repr (width)); (__str ("height"), Obj.repr (height)); (__str ("board"), Obj.repr (!board))]) : (string * Obj.t) list); raise Return
  with Return -> !__ret)
...
and insert_dir ws word dr dc rows cols =
  ...
  (try while (!ri < List.length (rows)) do
    try
  let row = (let __l = rows in let __i = !ri in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
  let ci = ref (0) in
  (try while (!ci < List.length (cols)) do
    try
  let col = (let __l = cols in let __i = !ci in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
  let end_r = (row + (dr * (word_len - 1))) in
  let end_c = (col + (dc * (word_len - 1))) in
  if ((((end_r < 0) || (end_r >= (Obj.obj (List.assoc (__str ("height")) (ws) : Obj.t) : int))) || (end_c < 0)) || (end_c >= (Obj.obj (List.assoc (__str ("width")) (ws) : Obj.t) : int))) then (
  ci := (!ci + 1);
  raise Continue;
  );
  let k = ref (0) in
  ...
  let rr = (row + (dr * !k)) in
  let cc = (col + (dc * !k)) in
  if ((let __l = (let __l = (Obj.obj (List.assoc (__str ("board")) (ws) : Obj.t) : string list list) in let __i = rr in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = cc in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) <> "") then (
  ok := false;
  raise Break;
  );
  k := (!k + 1);
  ...
  let rr2 = (row + (dr * !k)) in
  let cc2 = (col + (dc * !k)) in
  let row_list = ref ((let __l = (Obj.obj (List.assoc (__str ("board")) (ws) : Obj.t) : string list list) in let __i = rr2 in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) in
...
and generate_board ws =
  ...
  let i = ref (0) in
  (try while (!i < List.length ((Obj.obj (List.assoc (__str ("words")) (ws) : Obj.t) : string list))) do
    try
  let word = (let __l = (Obj.obj (List.assoc (__str ("words")) (ws) : Obj.t) : string list) in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
  let rows = ref (([] : (int) list)) in
  let r = ref (0) in
  (try while (!r < (Obj.obj (List.assoc (__str ("height")) (ws) : Obj.t) : int)) do
    try
  ...
  let c = ref (0) in
  (try while (!c < (Obj.obj (List.assoc (__str ("width")) (ws) : Obj.t) : int)) do
    try
  ...
  ignore (insert_dir ((Obj.magic ws : (string * Obj.t) list)) (word) (Obj.repr ((let __l = !dirs_r in let __i = d in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)))) (Obj.repr ((let __l = !dirs_c in let __i = d in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)))) (!rows) (!cols));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
    !__ret
...
and visualise ws add_fake_chars =
  ...
  (try while (!c < (Obj.obj (List.assoc (__str ("width")) (ws) : Obj.t) : int)) do
    try
  let ch = ref ((let __l = (let __l = (Obj.obj (List.assoc (__str ("board")) (ws) : Obj.t) : string list list) in let __i = !r in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in let __i = !c in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) in
  if (!ch = "") then (
