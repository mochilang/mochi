...
and josephus_iterative num_people step_size =
  ...
  __ret := (Obj.magic ((let __l = !circle in let __i = 0 in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : int); raise Return
  with Return -> !__ret)
...
let () =
  ...
  print_endline ((string_of_int (find_winner (Obj.repr (7)) (Obj.repr (3)))));
  print_endline ((string_of_int (josephus_iterative (Obj.repr (7)) (Obj.repr (3)))));
  let finish = _now () in
  let mem_end = _mem () in
