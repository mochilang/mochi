...
let rec bubble_sort nums =
  ...
  (try while (!i < List.length (nums)) do
    try
  arr := (Obj.magic (List.append (!arr) [(Obj.magic ((let __l = nums in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : float)]) : float list);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  ...
  (try while (!b < ((!n - !a) - 1)) do
    try
  if ((let __l = !arr in let __i = !b in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) > (let __l = !arr in let __i = (!b + 1) in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) then (
  let temp = (let __l = !arr in let __i = !b in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) in
  arr := (List.mapi (fun __i __x -> if __i = !b then (let __l = !arr in let __i = (!b + 1) in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) else __x) (!arr));
...
and find_median nums =
  ...
  if (mod_ <> 0) then (
  __ret := (Obj.magic ((let __l = nums in let __i = div in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : float); raise Return
  );
  __ret := (Obj.magic ((((let __l = nums in let __i = div in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i)) +. (let __l = nums in let __i = (div - 1) in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) /. 2.0)) : float); raise Return
  with Return -> !__ret)
...
and interquartile_range nums =
  ...
  (try while (!i < div) do
    try
  lower := (Obj.magic (List.append (!lower) [(Obj.magic ((let __l = !sorted in let __i = !i in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : float)]) : float list);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  ...
  (try while (!j < length) do
    try
  upper := (Obj.magic (List.append (!upper) [(Obj.magic ((let __l = !sorted in let __i = !j in let __len = List.length __l in List.nth __l (if __i >= 0 then __i else __len + __i))) : float)]) : float list);
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
...
and main () =
  ...
  print_endline ((Printf.sprintf "%.16g" (interquartile_range ([4.0; 1.0; 2.0; 3.0; 2.0]))));
    !__ret
  with Return -> !__ret)
