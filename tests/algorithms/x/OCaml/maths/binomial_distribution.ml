...
and binomial_distribution successes trials prob =
  let __ret = ref 0.0 in
  (try
  let successes = (Obj.magic successes : int) in
  ...
  let probability = (pow_float (Obj.repr (prob)) (Obj.repr (successes)) *. pow_float (Obj.repr ((1.0 -. prob))) (Obj.repr ((trials - successes)))) in
  let numerator = float_of_int (factorial (Obj.repr (trials))) in
  let denominator = float_of_int ((factorial (Obj.repr (successes)) * factorial (Obj.repr ((trials - successes))))) in
  let coefficient = (numerator /. denominator) in
  __ret := (Obj.magic ((probability *. coefficient)) : float); raise Return
  with Return -> !__ret)
