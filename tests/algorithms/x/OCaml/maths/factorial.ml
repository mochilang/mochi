...
and factorial_recursive n =
  ...
  __ret := (Obj.magic ((n * factorial_recursive (Obj.repr ((n - 1))))) : int); raise Return
  with Return -> !__ret)
...
and test_factorial () =
  ...
  (try while (!i <= 10) do
    try
  if (factorial (Obj.repr (!i)) <> factorial_recursive (Obj.repr (!i))) then (
  (failwith ("mismatch between factorial and factorial_recursive"));
  );
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  if (factorial (Obj.repr (6)) <> 720) then (
...
and main () =
  ...
  print_endline (string_of_int (factorial (Obj.repr (6))));
    !__ret
  with Return -> !__ret)
