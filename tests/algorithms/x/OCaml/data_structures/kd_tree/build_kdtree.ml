...
let rec sort_points points axis =
  ...
  (try while (!j < (List.length (!arr) - 1)) do
    try
  if (List.nth (List.nth (!arr) (!j)) (axis) > List.nth (List.nth (!arr) ((!j + 1))) (axis)) then (
  let tmp = ref (List.nth (!arr) (!j)) in
...
and build_kdtree points depth =
  ...
  let k = List.length (List.nth (points) (0)) in
  ...
  tree := (List.append (!tree) [(Obj.magic ([(__str ("point"), Obj.repr (List.nth (!sorted) (median_idx))); (__str ("left"), Obj.repr ((0 - 1))); (__str ("right"), Obj.repr ((0 - 1)))]) : (string * Obj.t) list)]);
  let left_idx = build_kdtree (!left_points) (Obj.repr ((depth + 1))) in
  let right_idx = build_kdtree (!right_points) (Obj.repr ((depth + 1))) in
  let node = ref (List.nth (!tree) (!idx)) in
  node := ((__str ("left"), Obj.repr (left_idx)) :: List.remove_assoc (__str ("left")) (Obj.magic (!node) : (string * Obj.t) list));
  node := ((__str ("right"), Obj.repr (right_idx)) :: List.remove_assoc (__str ("right")) (Obj.magic (!node) : (string * Obj.t) list));
...
let () =
  ...
  let start = _now () in
  let root = build_kdtree (!pts) (Obj.repr (0)) in
  print_endline ((__str (!tree)));
