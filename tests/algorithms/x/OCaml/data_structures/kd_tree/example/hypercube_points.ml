...
and hypercube_points num_points hypercube_size num_dimensions =
  let __ret = ref ([] : (float list) list) in
  (try
  let num_points = (Obj.magic num_points : int) in
  let hypercube_size = (Obj.magic hypercube_size : float) in
  let num_dimensions = (Obj.magic num_dimensions : int) in
  let points = ref (([] : (float list) list)) in
  let i = ref (0) in
  (try while (!i < num_points) do
    try
  let point = ref (([] : (float) list)) in
  let j = ref (0) in
  (try while (!j < num_dimensions) do
    try
  let value = (hypercube_size *. random ()) in
  point := (List.append (!point) [(Obj.magic (value) : float)]);
  j := (!j + 1);
    with Continue -> ()
  done with Break -> ());
  points := (List.append (!points) [(Obj.magic (!point) : float list)]);
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
...
let pts = ref (hypercube_points (Obj.repr (3)) (Obj.repr (1.0)) (Obj.repr (2)))
let () =
  let mem_start = _mem () in
  let start = _now () in
  print_endline (__show_list !pts);
  let finish = _now () in
  let mem_end = _mem () in
