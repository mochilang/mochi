...
and main () =
  ...
  (try
  let cll = ref (empty_list ()) in
  let i = ref (0) in
  (try while (!i < 5) do
    try
  cll := insert_tail (!cll) (Obj.repr ((!i + 1)));
  i := (!i + 1);
    with Continue -> ()
  done with Break -> ());
  print_endline ((to_string (!cll)));
  cll := insert_tail (!cll) (Obj.repr (6));
  print_endline ((to_string (!cll)));
  cll := insert_head (!cll) (Obj.repr (0));
  print_endline ((to_string (!cll)));
  let res = ref (delete_front (!cll)) in
  cll := (Obj.obj (List.assoc (__str ("list")) (!res) : Obj.t) : ( string * Obj.t ) list);
  print_endline (string_of_int (Obj.obj (List.assoc (__str ("value")) (!res) : Obj.t) : int));
  res := delete_tail (!cll);
  cll := (Obj.obj (List.assoc (__str ("list")) (!res) : Obj.t) : ( string * Obj.t ) list);
  print_endline (string_of_int (Obj.obj (List.assoc (__str ("value")) (!res) : Obj.t) : int));
  res := delete_nth (!cll) (Obj.repr (2));
  cll := (Obj.obj (List.assoc (__str ("list")) (!res) : Obj.t) : ( string * Obj.t ) list);
  print_endline (string_of_int (Obj.obj (List.assoc (__str ("value")) (!res) : Obj.t) : int));
  print_endline ((to_string (!cll)));
    !__ret
