    ...
    elseif type(v) == 'string' then
      ...
      elseif type(v) == 'table' and (v[1] == nil) then
        ...
        elseif type(v) == 'string' then
          ...
          local hist = make_list(256, 0)
          local i = 0
          while (i < height) do
            local j = 0
            while (j < width) do
              local val = image[i + 1][j + 1]
              hist[val + 1] = (hist[val + 1] + 1)
              j = (j + 1)
            end
            i = (i + 1)
          end
          local mapping = make_list(256, 0)
          local cumulative = 0
          local total = (height * width)
          local h = 0
          while (h < 256) do
            cumulative = (cumulative + hist[h + 1])
            mapping[h + 1] = ((255 * cumulative) // total)
            h = (h + 1)
          end
          i = 0
          while (i < height) do
