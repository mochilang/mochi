  ...
  function get_significant_digits(colors)
    ...
    for _, color in ipairs(colors) do
      if (not (significant_figures_color_values[color] ~= nil)) then
        panic((tostring(color) .. " is not a valid color for significant figure bands"))
      end
      ...
      if (total == 4) then
        ...
        if (total == 5) then
          ...
          if (total == 6) then
            ...
            panic((tostring(total) .. " is not a valid number of bands"))
          end
        end
      end
    end
    ...
    elseif type(v) == 'table' and (v[1] == nil) then
      ...
      elseif type(v) == 'string' then
        ...
        elseif type(v) == 'table' then
          ...
          panic((((("Expecting " .. tostring(number_of_bands)) .. " colors, provided ") .. tostring((function(v)
          if type(v) == 'table' and v.items ~= nil then
            return #v.items
          elseif type(v) == 'table' and (v[1] == nil) then
              local c = 0
            ...
            for _, color in ipairs(colors) do
              ...
              return false
            end)(valid_colors, color)) then
              panic((tostring(color) .. " is not a valid color"))
            end
          end
          return true
        end
        ...
        local resistance_str = tostring(resistance_value)
        if (resistance_value == math.floor(tonumber(resistance_value) or 0)) then
          resistance_str = tostring(math.floor(tonumber(resistance_value) or 0))
        end
        local answer = (((resistance_str .. "Ω ±") .. tostring(tolerance)) .. "% ")
        if (temp_coeff ~= 0) then
          answer = ((answer .. tostring(temp_coeff)) .. " ppm/K")
        end
        return answer
      end
      ...
      multiplier_color_values = {__name = "GenType2", __order = {"Black", "Brown", "Red", "Orange", "Yellow", "Green", "Blue", "Violet", "Grey", "White", "Gold", "Silver"}, Black = 1, Brown = 10, Red = 100, Orange = 1000, Yellow = 10000, Green = 100000, Blue = 1e+06, Violet = 1e+07, Grey = 1e+08, White = 1e+09, Gold = 0.1, Silver = 0.01}
      tolerance_color_values = {__name = "GenType3", __order = {"Brown", "Red", "Orange", "Yellow", "Green", "Blue", "Violet", "Grey", "Gold", "Silver"}, Brown = 1, Red = 2, Orange = 0.05, Yellow = 0.02, Green = 0.5, Blue = 0.25, Violet = 0.1, Grey = 0.01, Gold = 5, Silver = 10}
