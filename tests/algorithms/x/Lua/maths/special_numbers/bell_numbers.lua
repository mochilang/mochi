...
do
  ...
  function binomial_coefficient(total_elements, elements_to_choose)
    if ((elements_to_choose == 0) or (elements_to_choose == total_elements)) then
      return 1
    end
    local k = elements_to_choose
  ...
  while (i <= max_set_length) do
    local j = 0
    while (j < i) do
      bell[i + 1] = (bell[i + 1] + (binomial_coefficient((i - 1), j) * bell[j + 1]))
      j = (j + 1)
    end
    i = (i + 1)
  end
