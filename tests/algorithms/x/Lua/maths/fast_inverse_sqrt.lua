...
do
  ...
  function float_to_bits(x)
    ...
    local frac = (normalized - 1)
    local mantissa = math.floor(tonumber((frac * pow2_float(23))) or 0)
  ...
  function bits_to_float(bits)
    ...
    local mantissa = (1 + (mantissa_bits / pow2_float(23)))
  ...
  function test_fast_inverse_sqrt()
    if (absf((fast_inverse_sqrt(10) - 0.3156857923527257)) > 0.0001) then
      _panic("fast_inverse_sqrt(10) failed")
    end
    if (absf((fast_inverse_sqrt(4) - 0.49915357479239103)) > 0.0001) then
  ...
  function main()
    ...
    local i = 5
    while (i <= 100) do
      local diff = ((1 / sqrtApprox(i)) - fast_inverse_sqrt(i))
      print((((type(((tostring(i) .. ": ") .. tostring(diff))) == "table")) and (
      (function(v)
      ...
      if type(x) == "table" then
        ...
        return encode(v)
      end)(((tostring(i) .. ": ") .. tostring(diff)))) or (((tostring(i) .. ": ") .. tostring(diff)))))
      i = (i + 5)
    end
  end
  main()
