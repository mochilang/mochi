...
function insert_double_hash(values, size, prime, value)
  local vals = values
  local idx = hash1(size, value)
  local step = hash2(prime, value)
  local count = 0
  while ((vals[idx + 1] ~= (-1)) and (count < size)) do
    idx = ((idx + step) % size)
    count = (count + 1)
  end
  if (vals[idx + 1] == (-1)) then
    vals[idx + 1] = value
  end
  return vals
end
function table_keys(values)
  ...
  elseif type(v) == 'table' and (v[1] == nil) then
    ...
    function run_example(size, data)
      local prime = prev_prime(size)
      local _table = create_table(size)
      local i = 0
      while (i < (function(v)
      if type(v) == 'table' and v.items ~= nil then
        return #v.items
      elseif type(v) == 'table' and (v[1] == nil) then
        ...
        elseif type(v) == 'string' then
          ...
          elseif type(v) == 'table' then
            ...
            _table = insert_double_hash(_table, size, prime, data[i + 1])
            i = (i + 1)
          end
          print((((type(
          (function(v)
          ...
          if type(x) == "table" then
            ...
            return encode(v)
          end)(table_keys(_table))) == "table")) and (
          (function(v)
          local function encode(x)
          if type(x) == "table" then
            if x.__name and x.__order then
