...
do
  ...
  function max_heapify(h, heap_size, index)
    local largest = index
    local left = left_child_idx(index)
    local right = right_child_idx(index)
    if ((left < heap_size) and (h[left + 1] > h[largest + 1])) then
      largest = left
    end
    if ((right < heap_size) and (h[right + 1] > h[largest + 1])) then
      largest = right
    end
    if (largest ~= index) then
      local temp = h[index + 1]
      h[index + 1] = h[largest + 1]
      h[largest + 1] = temp
  ...
  function build_max_heap(h)
    ...
    elseif type(v) == 'table' and (v[1] == nil) then
      ...
      function extract_max(h, heap_size)
        local max_value = h[0 + 1]
        h[0 + 1] = h[(heap_size - 1) + 1]
        max_heapify(h, (heap_size - 1), 0)
        return max_value
      end
      function insert(h, heap_size, value)
        if (heap_size < (function(v)
        if type(v) == 'table' and v.items ~= nil then
          return #v.items
        elseif type(v) == 'table' and (v[1] == nil) then
            local c = 0
        ...
        print((((type(heap_to_string(heap, size)) == "table")) and (
        (function(v)
        ...
        if type(x) == "table" then
          ...
          return encode(v)
        end)(heap_to_string(heap, size))) or (heap_to_string(heap, size))))
        m = extract_max(heap, size)
        size = (size - 1)
        print((((type(tostring(m)) == "table")) and (
        (function(v)
        local function encode(x)
        if type(x) == "table" then
          if x.__name and x.__order then
