  ...
  function lu_decomposition(mat)
        ...
        elseif type(v) == 'table' and (v[1] == nil) then
          ...
          elseif type(v) == 'string' then
            ...
            while (i < n) do
              ...
              local j = 0
              while (j < n) do
                lrow = (function(lst, item)
                local res = {table.unpack(lst or {})}
                table.insert(res, item)
                return res
              end)(lrow, 0)
              urow = (function(lst, item)
              local res = {table.unpack(lst or {})}
              table.insert(res, item)
              return res
            end)(urow, 0)
    ...
    while (i < n) do
      ...
      while (j1 < i) do
        ...
        local k = 0
        while (k < i) do
          total = (total + (lower[i + 1][k + 1] * upper[k + 1][j1 + 1]))
          k = (k + 1)
        end
        if (upper[j1 + 1][j1 + 1] == 0) then
          panic("No LU decomposition exists")
        end
        lower[i + 1][j1 + 1] = ((mat[i + 1][j1 + 1] - total) / upper[j1 + 1][j1 + 1])
        j1 = (j1 + 1)
      end
      lower[i + 1][i + 1] = 1
      local j2 = i
      while (j2 < n) do
        ...
        local k2 = 0
        while (k2 < i) do
          total2 = (total2 + (lower[i + 1][k2 + 1] * upper[k2 + 1][j2 + 1]))
          k2 = (k2 + 1)
        end
        upper[i + 1][j2 + 1] = (mat[i + 1][j2 + 1] - total2)
        j2 = (j2 + 1)
      end
      i = (i + 1)
    end
