...
do
  ...
  function round_dec(x, n)
    ...
    local y = ((x * m10) + 0.5)
    return ((1 * math.floor(tonumber(y) or 0)) / m10)
  end
  function reset()
    return {buffer = K, params = {0, 0, 0, 0, 0}, time = 0}
  end
    ...
    elseif type(v) == 'table' and (v[1] == nil) then
      ...
      elseif type(v) == 'string' then
        ...
        elseif type(v) == 'table' then
          ...
          local value = buf[i + 1]
          local e = ((1 * seed) / value)
          local next_value = (buf[((i + 1) % size) + 1] + e)
          next_value = (next_value - (1 * math.floor(tonumber(next_value) or 0)))
          local r = (par[i + 1] + e)
          r = (r - (1 * math.floor(tonumber(r) or 0)))
          r = (r + 3)
          buf[i + 1] = round_dec(((r * next_value) * (1 - next_value)), 10)
          par[i + 1] = r
          i = (i + 1)
        end
      ...
      function pull(m)
        ...
        while (i < t) do
          ...
          buf[key + 1] = round_dec(((r * value) * (1 - value)), 10)
          local new_r = (((1 * m.time) * 0.01) + (r * 1.01))
          new_r = (new_r - (1 * math.floor(tonumber(new_r) or 0)))
          par[key + 1] = (new_r + 3)
          i = (i + 1)
        end
      ...
      i = 0
      while (i < 100) do
        machine = push((function(_t) local _c={} for k,v in pairs(_t) do _c[k]=v end return _c end)(machine), i)
        i = (i + 1)
      end
      res = pull(machine)
      print(res.value)
