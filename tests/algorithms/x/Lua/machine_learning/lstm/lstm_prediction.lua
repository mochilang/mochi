  ...
  elseif type(v) == 'table' and (v[1] == nil) then
    ...
    elseif type(v) == 'string' then
      ...
      while (t >= 0) do
        ...
        local da_o = ((do_t * o_t) * (1 - o_t))
        local dc = (((dh_next * o_t) * (1 - (tanh_c * tanh_c))) + dc_next)
        local di_t = (dc * g_t)
        local da_i = ((di_t * i_t) * (1 - i_t))
        local dg_t = (dc * i_t)
        local da_g = (dg_t * (1 - (g_t * g_t)))
        local df_t = (dc * c_prev)
        local da_f = ((df_t * f_t) * (1 - f_t))
    ...
    function init_weights()
      return {w_i = 0.1, u_i = 0.2, b_i = 0, w_f = 0.1, u_f = 0.2, b_f = 0, w_o = 0.1, u_o = 0.2, b_o = 0, w_c = 0.1, u_c = 0.2, b_c = 0, w_y = 0.1, b_y = 0}
    end
      ...
      while (ep < epochs) do
        ...
        elseif type(v) == 'table' and (v[1] == nil) then
          ...
          elseif type(v) == 'string' then
            ...
            elseif type(v) == 'table' then
              ...
              w = backward(seq, target, (function(_t) local _c={} for k,v in pairs(_t) do _c[k]=v end return _c end)(w), state, lr)
              j = (j + 1)
            end
            ep = (ep + 1)
          end
        ...
        function predict(seq, w)
          ...
          elseif type(v) == 'table' and (v[1] == nil) then
            ...
            print((((type(("Predicted value: " .. tostring(pred))) == "table")) and (
            (function(v)
            ...
            if type(x) == "table" then
              ...
              return encode(v)
            end)(("Predicted value: " .. tostring(pred)))) or (("Predicted value: " .. tostring(pred)))))
