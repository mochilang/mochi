      ...
      function predict_raw(samples, labels, alphas, b, x)
        ...
        elseif type(v) == 'table' and (v[1] == nil) then
          ...
          function smo_train(samples, labels, c, tol, max_passes)
            ...
            elseif type(v) == 'table' and (v[1] == nil) then
              ...
              while (passes < max_passes) do
                ...
                while (i1 < m) do
                  ...
                  if ((((labels[i1 + 1] * Ei) < (0 - tol)) and (alphas[i1 + 1] < c)) or (((labels[i1 + 1] * Ei) > tol) and (alphas[i1 + 1] > 0))) then
                    local i2 = ((i1 + 1) % m)
                    ...
                    if (labels[i1 + 1] ~= labels[i2 + 1]) then
                      L = maxf(0, (alpha2_old - alpha1_old))
                      H = minf(c, ((c + alpha2_old) - alpha1_old))
                    else
                      L = maxf(0, ((alpha2_old + alpha1_old) - c))
                    ...
                    local eta = (((2 * dot(samples[i1 + 1], samples[i2 + 1])) - dot(samples[i1 + 1], samples[i1 + 1])) - dot(samples[i2 + 1], samples[i2 + 1]))
                    if (eta >= 0) then
                      i1 = (i1 + 1)
                      goto __cont_2
                    end
                    ...
                    if ((alphas[i1 + 1] > 0) and (alphas[i1 + 1] < c)) then
                      b = b1
                    else
                      if ((alphas[i2 + 1] > 0) and (alphas[i2 + 1] < c)) then
                        b = b2
                      else
                        b = ((b1 + b2) / 2)
                      end
            ...
            print((((type(predict(samples, labels, model, {1.5, 1})) == "table")) and (
            (function(v)
            ...
            if type(x) == "table" then
              ...
              return encode(v)
            end)(predict(samples, labels, model, {1.5, 1}))) or (predict(samples, labels, model, {1.5, 1}))))
