...
do
  ...
  function mod_inverse(a, m)
    if (gcd(a, m) ~= 1) then
      panic((((("mod inverse of " .. tostring(a)) .. " and ") .. tostring(m)) .. " does not exist"))
    end
    local u1 = 1
    local u2 = 0
    local u3 = a
        ...
        elseif type(v) == 'table' and (v[1] == nil) then
          ...
          elseif type(v) == 'string' then
            ...
            if (((key_a < 0) or (key_b < 0)) or (key_b > (m - 1))) then
              panic(("Key A must be greater than 0 and key B must be between 0 and " .. tostring((m - 1))))
            end
            if (gcd(key_a, m) ~= 1) then
              panic((((("Key A " .. tostring(key_a)) .. " and the symbol set size ") .. tostring(m)) .. " are not relatively prime. Choose a different key."))
            end
          end
