...
defmodule Main do
  ...
  def is_safe(grid, visited, row, col) do
    try do
      ...
      visited_cell = Enum.at(Enum.at(visited, row, []), col, false)
      within_bounds = row >= 0 && row < rows && col >= 0 && col < cols
      not_visited = visited_cell == false
      throw {:return, within_bounds && not_visited && Enum.at(Enum.at(grid, row, []), col, 0) == 1}
    catch
      {:return, val} -> val
    end
  end
  def dfs(grid, visited, row, col) do
    try do
      ...
      while_fun = fn while_fun, k ->
        if k < 8 do
          new_row = row + Enum.at(row_nbr, k, 0)
          new_col = col + Enum.at(col_nbr, k, 0)
      ...
      while_fun_4 = fn while_fun_4, count, i ->
        if i < rows do
          ...
          while_fun_5 = fn while_fun_5, count, j ->
            if j < cols do
              {count} = if !Enum.at(Enum.at(visited, i, []), j, false) && Enum.at(Enum.at(grid, i, []), j, 0) == 1 do
