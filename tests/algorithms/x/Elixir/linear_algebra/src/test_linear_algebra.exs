...
defmodule Main do
        ...
        if i < _len(m) do
          ...
          while_fun_15 = fn while_fun_15, j, s ->
            if j < _len(Enum.at(m, 0)) do
              s = (s <> int_to_string(Enum.at(Enum.at(m, i), j)))
    ...
    try do
      ...
      while_fun_18 = fn while_fun_18, c, det ->
        if c < n do
          ...
          det = det + sign * Enum.at(Enum.at(m, 0), c) * determinant(sub)
          c = c + 1
      ...
      while_fun_21 = fn while_fun_21, i, res ->
        if i < _len(m) do
          ...
          while_fun_22 = fn while_fun_22, j, row ->
            if j < _len(Enum.at(m, 0)) do
              row = (row ++ [Enum.at(Enum.at(m, i), j) * s])
              j = j + 1
  ...
  def matrix_component(m, i, j) do
    try do
      throw {:return, Enum.at(Enum.at(m, i), j)}
    catch
      {:return, val} -> val
    end
  end
