  ...
  def points_to_polynomial(coordinates) do
    try do
        ...
        if j < _len(coordinates) do
          ...
          while_fun_6 = fn while_fun_6, k ->
            if k < _len(coordinates) do
              if Enum.at(Enum.at(coordinates, j), 0) == Enum.at(Enum.at(coordinates, k), 0) && Enum.at(Enum.at(coordinates, j), 1) == Enum.at(Enum.at(coordinates, k), 1) do
                raise("The program cannot work out a fitting polynomial.")
              end
              k = k + 1
      ...
      while_fun_7 = fn while_fun_7, i, set_x ->
        if i < _len(coordinates) do
          x_val = Enum.at(Enum.at(coordinates, i), 0)
      ...
      if _len(set_x) == 1 do
        throw {:return, ("x=" <> Kernel.to_string(Enum.at(Enum.at(coordinates, 0), 0)))}
      end
      ...
      while_fun_8 = fn while_fun_8, matrix, row ->
        if row < n do
          ...
          while_fun_9 = fn while_fun_9, col, line ->
            if col < n do
              power = pow_int_float(Enum.at(Enum.at(coordinates, row), 0), n - (col + 1))
              line = (line ++ [power])
              col = col + 1
      ...
      while_fun_10 = fn while_fun_10, row, vector ->
        if row < n do
          vector = (vector ++ [:erlang.float(Enum.at(Enum.at(coordinates, row), 1))])
          row = row + 1
          ...
          while_fun_12 = fn while_fun_12, matrix, number, vector ->
            if number < n do
              {matrix, vector} = if count != number do
                fraction = Enum.at(Enum.at(matrix, number), count) / Enum.at(Enum.at(matrix, count), count)
                cc = 0
                while_fun_13 = fn while_fun_13, cc, matrix ->
                  if cc < n do
                    matrix = List.replace_at(matrix, number, List.replace_at(Enum.at(matrix, number), cc, Enum.at(Enum.at(matrix, number), cc) - Enum.at(Enum.at(matrix, count), cc) * fraction))
                    cc = cc + 1
      ...
      while_fun_14 = fn while_fun_14, count, solution ->
        if count < n do
          value = Enum.at(vector, count) / Enum.at(Enum.at(matrix, count), count)
