        ...
        if head < _len(queue) do
          ...
          while_fun_3 = fn while_fun_3, ind, parent, queue, visited ->
            if ind < _len(Enum.at(graph, u)) do
              {parent, queue, visited} = if Enum.at(visited, ind) == false && Enum.at(Enum.at(graph, u), ind) > 0 do
                queue = (queue ++ [ind])
      ...
      while_fun_7 = fn while_fun_7, g ->
        if bfs(g, source, sink, parent) do
          ...
          while_fun_9 = fn while_fun_9, g, v ->
            if v != source do
              u = Enum.at(parent, v)
              g = List.replace_at(g, u, List.replace_at(Enum.at(g, u), v, Enum.at(Enum.at(g, u), v) - path_flow))
              g = List.replace_at(g, v, List.replace_at(Enum.at(g, v), u, Enum.at(Enum.at(g, v), u) + path_flow))
              v = u
        ...
        if i < _len(g) do
          ...
          while_fun_11 = fn while_fun_11, j, res ->
            if j < _len(Enum.at(g, 0)) do
              {res} = if Enum.at(Enum.at(g, i), j) == 0 && Enum.at(Enum.at(temp, i), j) > 0 do
                res = (res ++ [[i, j]])
                {res}
              else
                {res}
