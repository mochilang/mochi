      ...
      while_fun = fn while_fun, i, sums ->
        if i < resources do
          ...
          while_fun_2 = fn while_fun_2, j, total ->
            if j < _len(alloc) do
              total = total + Enum.at(Enum.at(alloc, j), i)
              j = j + 1
        ...
        if i < _len(max) do
          ...
          while_fun_5 = fn while_fun_5, j, row ->
            if j < _len(Enum.at(max, 0)) do
              row = (row ++ [Enum.at(Enum.at(max, i), j) - Enum.at(Enum.at(alloc, i), j)])
              j = j + 1
            ...
            if p < _len(need_list) do
              {avail, finished, remaining, safe} = if !Enum.at(finished, p) do
                ...
                while_fun_15 = fn while_fun_15, exec, r ->
                  if r < _len(avail) do
                    {exec} = if Enum.at(Enum.at(need_list, p), r) > Enum.at(avail, r) do
                      exec = false
                      throw {:break, {exec, r}}
                      {exec}
                    else
                ...
                {avail, finished, r, remaining, safe} = if exec do
                  ...
                  while_fun_16 = fn while_fun_16, avail, r ->
                    if r < _len(avail) do
                      avail = List.replace_at(avail, r, Enum.at(avail, r) + Enum.at(Enum.at(alloc, p), r))
                      r = r + 1
