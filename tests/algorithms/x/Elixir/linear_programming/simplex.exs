  ...
  def findPivot(t) do
    try do
      ...
      {first, minRatio, row} = Enum.reduce((1..(_len(t) - 1)//1), {first, minRatio, row}, fn i, {first, minRatio, row} ->
        coeff = Enum.at(Enum.at(t, i), col)
        {first, minRatio, row} = if coeff > 0.0 do
          rhs = Enum.at(Enum.at(t, i), _len(Enum.at(t, i)) - 1)
          ratio = rhs / coeff
  ...
  def interpret(t, nVars) do
    try do
      ...
      {result} = Enum.reduce((0..(nVars - 1)//1), {result}, fn i, {result} ->
        ...
        {result} = if nzCount == 1 && Enum.at(Enum.at(t, nzRow), i) == 1.0 do
          result = Map.put(result, ("x" <> Kernel.to_string(i + 1)), Enum.at(Enum.at(t, nzRow), lastCol))
          {result}
        else
          {result}
        end
