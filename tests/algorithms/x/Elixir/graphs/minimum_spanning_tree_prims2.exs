...
defmodule Main do
  ...
  def prims_algo(graph) do
    try do
      ...
      parent = %{}
      queue = []
      {dist, parent, queue} = Enum.reduce(Map.keys(graph), {dist, parent, queue}, fn node_, {dist, parent, queue} ->
      ...
      {dist, parent, queue} = Enum.reduce(Map.keys(graph[start]), {dist, parent, queue}, fn neighbour, {dist, parent, queue} ->
        try do
          w = graph[start][neighbour]
          {dist, parent, queue} = if dist[neighbour] > dist[start] + w do
          ...
          :continue -> {dist, parent, queue}
        end
        {dist, parent, queue}
      end)
      while_fun_4 = fn while_fun_4, dist, parent, queue ->
        if _len(queue) > 0 do
          best_idx = 0
          p = 1
          ...
          queue = tmp
          {dist, parent, queue} = Enum.reduce(Map.keys(graph[node_]), {dist, parent, queue}, fn neighbour, {dist, parent, queue} ->
            try do
              w = graph[node_][neighbour]
              {dist, parent, queue} = if dist[neighbour] > dist[node_] + w do
          ...
          while_fun_4.(while_fun_4, dist, parent, queue)
        else
          {dist, parent, queue}
        end
      end
      {dist, parent, queue} = try do
          while_fun_4.(while_fun_4, dist, parent, queue)
        catch
          {:break, {dist, parent, queue}} -> {dist, parent, queue}
        end
