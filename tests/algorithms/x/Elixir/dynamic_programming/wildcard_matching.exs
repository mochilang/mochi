      ...
      while_fun_3 = fn while_fun_3, dp, j ->
        if j <= m do
          {dp} = if _slice(p, j - 1, j - (j - 1)) == "*" do
            dp = List.replace_at(dp, 0, List.replace_at(Enum.at(dp, 0), j, Enum.at(Enum.at(dp, 0), j - 1)))
            {dp}
          else
            {dp}
          end
        ...
        if i <= n do
          ...
          while_fun_5 = fn while_fun_5, dp, j2 ->
            if j2 <= m do
              ...
              {dp} = if pc == sc || pc == "?" do
                dp = List.replace_at(dp, i, List.replace_at(Enum.at(dp, i), j2, Enum.at(Enum.at(dp, i - 1), j2 - 1)))
                {dp}
              else
                {dp} = if pc == "*" do
                  {dp} = if Enum.at(Enum.at(dp, i - 1), j2) || Enum.at(Enum.at(dp, i), j2 - 1) do
