...
calc_profit(Profit, Weight, Max_weight) ->
    try
        ...
        true -> throw({return, #{"ok" => false, "value" => 0, "error" => "The length of profit and weight must be same."}});
        _ -> ok
    end,
        case (Max_weight =< 0) of
        true -> throw({return, #{"ok" => false, "value" => 0, "error" => "max_weight must greater than zero."}});
        _ -> ok
    end,
        I = 0,
    ...
    case (I < erlang:length(Profit)) of
        ...
        true -> throw({return, #{"ok" => false, "value" => 0, "error" => "Profit can not be negative."}});
        _ -> ok
    end,
        ...
        true -> throw({return, #{"ok" => false, "value" => 0, "error" => "Weight can not be negative."}});
        _ -> ok
    end,
            I_2 = (I + 1),
...
test_sorted() ->
    try
        ...
        (maps:get("ok", Res, nil) andalso (maps:get("value", Res, nil) == 210))
    catch {return, Ret} -> Ret end.
