...
sarimax_predictor(Train_user, Train_match, Test_match) ->
    ...
    case (I_19 < N_4) of
        true ->
            X_5 = lists:append((case X_4 of nil -> []; _ -> X_4 end), [[1, (case erlang:is_map(Train_user) of true -> maps:get((I_19 - 1), Train_user, nil); _ -> case (I_19 - 1) < 0 of true -> mochi_nth(erlang:length(Train_user) + (I_19 - 1) + 1, Train_user); _ -> mochi_nth((I_19 - 1) + 1, Train_user) end end), (case erlang:is_map(Train_match) of true -> maps:get(I_19, Train_match, nil); _ -> case I_19 < 0 of true -> mochi_nth(erlang:length(Train_match) + I_19 + 1, Train_match); _ -> mochi_nth(I_19 + 1, Train_match) end end)]]),
...
percentile(Data, Q) ->
    try
        ...
        Pos = mochi_safe_mul(mochi_safe_div(Q, 100), int_to_float((N_5 - 1))),
        Idx_2 = floor_int(Pos),
        Frac = (Pos - int_to_float(Idx_2)),
        case ((Idx_2 + 1) < N_5) of
        true -> throw({return, (mochi_safe_mul((case erlang:is_map(Sorted) of true -> maps:get(Idx_2, Sorted, nil); _ -> case Idx_2 < 0 of true -> mochi_nth(erlang:length(Sorted) + Idx_2 + 1, Sorted); _ -> mochi_nth(Idx_2 + 1, Sorted) end end), (1 - Frac)) + mochi_safe_mul((case erlang:is_map(Sorted) of true -> maps:get((Idx_2 + 1), Sorted, nil); _ -> case (Idx_2 + 1) < 0 of true -> mochi_nth(erlang:length(Sorted) + (Idx_2 + 1) + 1, Sorted); _ -> mochi_nth((Idx_2 + 1) + 1, Sorted) end end), Frac))});
        _ -> ok
    end,
...
interquartile_range_checker(Train_user_3) ->
    try
        Q1 = percentile(Train_user_3, 25),
        Q3 = percentile(Train_user_3, 75),
        Iqr = (Q3 - Q1),
...
main() ->
    try
        Vote = [linear_regression_prediction([2, 3, 4, 5], [5, 3, 4, 6], [3, 1, 2, 4], [2], [2]), sarimax_predictor([4, 2, 6, 8], [3, 1, 2, 4], [2]), support_vector_regressor([[5, 2], [1, 5], [6, 2]], [[3, 2]], [2, 1, 4])],
        ...
        io:format("~ts~n", [mochi_repr(data_safety_checker(Vote, 5))]),
        nil
    catch {return, Ret} -> Ret end.
