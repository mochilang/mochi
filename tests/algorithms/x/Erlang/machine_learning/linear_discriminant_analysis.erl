...
{I_11, J_3, K_6, Means_2, Probabilities, Results, Temp_2, Variance, X_items} = Fun_10(I_11, J_3, K_5, Means_2, Probabilities, Results, Temp, Variance, X_items),
    ...
    case (T_2 < erlang:length(Temp_2)) of
        true ->
            case ((case erlang:is_map(Temp_2) of true -> maps:get(T_2, Temp_2, nil); _ -> case T_2 < 0 of true -> mochi_nth(erlang:length(Temp_2) + T_2 + 1, Temp_2); _ -> mochi_nth(T_2 + 1, Temp_2) end end) > Max_val) of
...
accuracy(Actual_y, Predicted_y) ->
    ...
    case (I_13 < erlang:length(Actual_y)) of
        true ->
            case ((case erlang:is_map(Actual_y) of true -> maps:get(I_13, Actual_y, nil); _ -> case I_13 < 0 of true -> mochi_nth(erlang:length(Actual_y) + I_13 + 1, Actual_y); _ -> mochi_nth(I_13 + 1, Actual_y) end end) == (case erlang:is_map(Predicted_y) of true -> maps:get(I_13, Predicted_y, nil); _ -> case I_13 < 0 of true -> mochi_nth(erlang:length(Predicted_y) + I_13 + 1, Predicted_y); _ -> mochi_nth(I_13 + 1, Predicted_y) end end)) of
