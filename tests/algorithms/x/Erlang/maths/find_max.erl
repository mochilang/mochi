...
find_max_iterative(Nums) ->
    try
        ...
        true -> erlang:error("find_max_iterative() arg is an empty sequence");
        _ -> ok
    end,
...
find_max_recursive(Nums_2, Left, Right) ->
    try
        ...
        case (N_2 == 0) of
        true -> erlang:error("find_max_recursive() arg is an empty sequence");
        _ -> ok
    end,
...
test_find_max() ->
    try
        ...
        true -> erlang:error("find_max_iterative failed");
        _ -> ok
    end,
        case (find_max_recursive(Arr, 0, (erlang:length(Arr) - 1)) /= 94.0) of
        true -> erlang:error("find_max_recursive failed");
        _ -> ok
    end,
        case (find_max_recursive(Arr, -erlang:length(Arr), -1) /= 94.0) of
        true -> erlang:error("negative index handling failed");
        _ -> ok
    end,
        nil
    catch {return, Ret} -> Ret end.
