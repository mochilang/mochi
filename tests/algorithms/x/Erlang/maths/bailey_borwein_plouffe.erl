...
subsum(Digit_pos_to_extract, Denominator_addend, Precision) ->
    ...
    case (Sum_index < (Digit_pos_to_extract + Precision)) of
        true ->
            ...
            case (Sum_index < Digit_pos_to_extract) of
...
bailey_borwein_plouffe(Digit_position, Precision_2) ->
    try
        ...
        true -> erlang:error("Digit position must be a positive integer");
        _ -> ok
    end,
        case (Precision_2 < 0) of
        true -> erlang:error("Precision must be a nonnegative integer");
        _ -> ok
    end,
