...
prime_sieve(Num) ->
    try
        case (Num =< 0) of
        true -> erlang:error("Invalid input, please enter a positive integer.");
        _ -> ok
    end,
        Sieve = [],
        I = 0,
...
{I_2, Num, Sieve_2} = Fun_2(I, Num, Sieve),
    ...
    case (Start =< End) of
        true ->
            case (case erlang:is_map(Sieve_2) of true -> maps:get(Start, Sieve_2, nil); _ -> case Start < 0 of true -> mochi_nth(erlang:length(Sieve_2) + Start + 1, Sieve_2); _ -> mochi_nth(Start + 1, Sieve_2) end end) of
        true -> Prime_2 = lists:append((case Prime of nil -> []; _ -> Prime end), [Start]),
            J = (Start * Start),
            Fun_3 = fun Fun_3_loop(End, I_2, J, Num, Prime_2, Sieve_2, Start) ->
    case (J =< Num) of
        true ->
            case (case erlang:is_map(Sieve_2) of true -> maps:get(J, Sieve_2, nil); _ -> case J < 0 of true -> mochi_nth(erlang:length(Sieve_2) + J + 1, Sieve_2); _ -> mochi_nth(J + 1, Sieve_2) end end) of
...
{End, I_2, Num, Prime_3, Sieve_5, Start_2} = Fun_5(End, I_2, Num, Prime, Sieve_2, Start),
    ...
    case (K =< Num) of
        true ->
            case (case erlang:is_map(Sieve_5) of true -> maps:get(K, Sieve_5, nil); _ -> case K < 0 of true -> mochi_nth(erlang:length(Sieve_5) + K + 1, Sieve_5); _ -> mochi_nth(K + 1, Sieve_5) end end) of
