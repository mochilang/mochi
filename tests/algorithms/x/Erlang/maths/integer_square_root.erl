...
integer_square_root(Num) ->
    try
        case (Num < 0) of
        true -> erlang:error("num must be non-negative integer");
        _ -> ok
    end,
        case (Num < 2) of
...
test_integer_square_root() ->
    ...
    case (I < erlang:length(Expected)) of
        ...
        true -> erlang:error(("test failed at index " ++ mochi_str(I)));
        _ -> ok
    end,
            I_2 = (I + 1),
...
{Expected, I_2} = Fun_2(Expected, I),
        ...
        true -> erlang:error("sqrt of 625 incorrect");
        _ -> ok
    end,
        case (integer_square_root(2147483647) /= 46340) of
        true -> erlang:error("sqrt of max int incorrect");
        _ -> ok
    end,
        nil
    catch {return, Ret} -> Ret end.
