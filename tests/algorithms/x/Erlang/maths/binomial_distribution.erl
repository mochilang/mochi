...
factorial(N) ->
    try
        case (N < 0) of
        true -> erlang:error("factorial is undefined for negative numbers");
        _ -> ok
    end,
        Result = 1,
        I = 2,
...
binomial_distribution(Successes, Trials, Prob) ->
    try
        ...
        true -> erlang:error("successes must be lower or equal to trials");
        _ -> ok
    end,
        case ((Trials < 0) orelse (Successes < 0)) of
        true -> erlang:error("the function is defined for non-negative integers");
        _ -> ok
    end,
        case mochi_not(((0.0 < Prob) andalso (Prob < 1.0))) of
        true -> erlang:error("prob has to be in range of 1 - 0");
        _ -> ok
    end,
