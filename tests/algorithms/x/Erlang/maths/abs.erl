...
abs_min(X) ->
    try
        ...
        true -> erlang:error("abs_min() arg is an empty sequence");
        _ -> ok
    end,
    ...
    case (Idx < erlang:length(X)) of
        true ->
            ...
            case (abs_val(float(I)) < abs_val(float(J))) of
        true -> J_2 = I,
            J_3 = J_2;
        _ -> J_3 = J
    end,
...
abs_max(X_2) ->
    try
        ...
        true -> erlang:error("abs_max() arg is an empty sequence");
        _ -> ok
    end,
    ...
    case (Idx_3 < erlang:length(X_2)) of
        true ->
            ...
            case (abs_val(float(I_2)) > abs_val(float(J_4))) of
        true -> J_5 = I_2,
            J_6 = J_5;
        _ -> J_6 = J_4
    end,
...
abs_max_sort(X_3) ->
    try
        ...
        true -> erlang:error("abs_max_sort() arg is an empty sequence");
        _ -> ok
    end,
        Arr = [],
        I_3 = 0,
...
{Arr_2, I_4, X_3} = Fun_3(Arr, I_3, X_3),
    ...
    case (B < ((N - A) - 1)) of
        true ->
            case (abs_val(float((case erlang:is_map(Arr_2) of true -> maps:get(B, Arr_2, nil); _ -> case B < 0 of true -> mochi_nth(erlang:length(Arr_2) + B + 1, Arr_2); _ -> mochi_nth(B + 1, Arr_2) end end))) > abs_val(float((case erlang:is_map(Arr_2) of true -> maps:get((B + 1), Arr_2, nil); _ -> case (B + 1) < 0 of true -> mochi_nth(erlang:length(Arr_2) + (B + 1) + 1, Arr_2); _ -> mochi_nth((B + 1) + 1, Arr_2) end end)))) of
...
test_abs_val() ->
    try
        ...
        true -> erlang:error("abs_val(0) failed");
        _ -> ok
    end,
        case (abs_val(34.0) /= 34.0) of
        true -> erlang:error("abs_val(34) failed");
        _ -> ok
    end,
        case (abs_val(-100000000000.0) /= 100000000000.0) of
        true -> erlang:error("abs_val large failed");
        _ -> ok
    end,
        A_3 = [-3, -1, 2, -11],
        case (abs_max(A_3) /= -11) of
        true -> erlang:error("abs_max failed");
        _ -> ok
    end,
        case (abs_max_sort(A_3) /= -11) of
        true -> erlang:error("abs_max_sort failed");
        _ -> ok
    end,
        case (abs_min(A_3) /= -1) of
        true -> erlang:error("abs_min failed");
        _ -> ok
    end,
        nil
    catch {return, Ret} -> Ret end.
