...
-export([main/1, odd_even_transposition/1]).
...
odd_even_transposition(Arr) ->
    try
        N = erlang:length(Arr),
        Pass = 0,
        Fun_2 = fun Fun_2_loop(Arr, N, Pass) ->
    case (Pass < N) of
        true ->
            I = mochi_mod(Pass, 2),
            Fun = fun Fun_loop(Arr, I, N, Pass) ->
    case (I < (N - 1)) of
        true ->
        ...
        _ -> Arr_4 = Arr,
            ...
            I_2 = (I + 2),
            Fun_loop(Arr_4, I_2, N, Pass);
        _ -> {Arr, I, N, Pass}
    end
end,
{Arr_4, I_2, N, Pass} = Fun(Arr, I, N, Pass),
            Pass_2 = (Pass + 1),
            Fun_2_loop(Arr_4, N, Pass_2);
        _ -> {Arr, N, Pass}
    end
end,
{Arr_4, N, Pass_2} = Fun_2(Arr, N, Pass),
        Arr_4
...
main(_) ->
    ...
    io:format("~ts~n", [mochi_str(odd_even_transposition([5.0, 4.0, 3.0, 2.0, 1.0]))]),
    io:format("~ts~n", [mochi_str(odd_even_transposition([13.0, 11.0, 18.0, 0.0, -1.0]))]),
    io:format("~ts~n", [mochi_str(odd_even_transposition([-0.1, 1.1, 0.1, -2.9]))]),
