...
test_prim_successful_result() ->
    ...
    case List of
        [] -> {Adjacency_2, Edges};
        [E|E_rest] ->
            U_2 = (case erlang:is_map(E) of true -> maps:get(0, E, nil); _ -> case 0 < 0 of true -> lists:nth(erlang:length(E) + 0 + 1, E); _ -> lists:nth(0 + 1, E) end end),
            V = (case erlang:is_map(E) of true -> maps:get(1, E, nil); _ -> case 1 < 0 of true -> lists:nth(erlang:length(E) + 1 + 1, E); _ -> lists:nth(1 + 1, E) end end),
            W = (case erlang:is_map(E) of true -> maps:get(2, E, nil); _ -> case 2 < 0 of true -> lists:nth(erlang:length(E) + 2 + 1, E); _ -> lists:nth(2 + 1, E) end end),
...
{Adjacency_8, Edges, Result, Seen_3} = Fun_7(Result, Adjacency_8, Edges, Result, Seen),
    ...
    case List of
        ...
        [Ans|Ans_rest] ->
            Key = ((mochi_str((case erlang:is_map(Ans) of true -> maps:get(0, Ans, nil); _ -> case 0 < 0 of true -> lists:nth(erlang:length(Ans) + 0 + 1, Ans); _ -> lists:nth(0 + 1, Ans) end end)) ++ ",") ++ mochi_str((case erlang:is_map(Ans) of true -> maps:get(1, Ans, nil); _ -> case 1 < 0 of true -> lists:nth(erlang:length(Ans) + 1 + 1, Ans); _ -> lists:nth(1 + 1, Ans) end end))),
