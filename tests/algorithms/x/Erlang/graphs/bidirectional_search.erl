...
expand_search(Graph, Queue, Head, Parents, Visited, Opposite_visited) ->
    try
        ...
        Current = (case erlang:is_map(Queue) of true -> maps:get(Head, Queue, nil); _ -> case Head < 0 of true -> lists:nth(erlang:length(Queue) + Head + 1, Queue); _ -> lists:nth(Head + 1, Queue) end end),
        Head_2 = (Head + 1),
    ...
    case (I < erlang:length(Neighbors)) of
        true ->
            try
                Neighbor = (case erlang:is_map(Neighbors) of true -> maps:get(I, Neighbors, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(Neighbors) + I + 1, Neighbors); _ -> lists:nth(I + 1, Neighbors) end end),
...
reverse_list(Xs) ->
    ...
    case (I_5 > 0) of
        true ->
            I_6 = (I_5 - 1),
            Res_2 = lists:append((case Res of nil -> []; _ -> Res end), [(case erlang:is_map(Xs) of true -> maps:get(I_6, Xs, nil); _ -> case I_6 < 0 of true -> lists:nth(erlang:length(Xs) + I_6 + 1, Xs); _ -> lists:nth(I_6 + 1, Xs) end end)]),
...
{Backward_head_2, Backward_parents_3, Backward_queue_2, Backward_visited_3, Forward_head_2, Forward_parents_3, Forward_queue_2, Forward_visited_3, G, Goal, Intersection_5, Start} = Fun_4(Backward_head, Backward_parents_2, Backward_queue, Backward_visited_2, Forward_head, Forward_parents_2, Forward_queue, Forward_visited_2, G, Goal, Intersection, Start),
        ...
        Back_start = (case erlang:is_map(Backward_parents_3) of true -> maps:get(Intersection_5, Backward_parents_3, nil); _ -> case Intersection_5 < 0 of true -> lists:nth(erlang:length(Backward_parents_3) + Intersection_5 + 1, Backward_parents_3); _ -> lists:nth(Intersection_5 + 1, Backward_parents_3) end end),
    ...
    case (J < erlang:length(Backward_path)) of
        true ->
            Result_2 = lists:append((case Result of nil -> []; _ -> Result end), [(case erlang:is_map(Backward_path) of true -> maps:get(J, Backward_path, nil); _ -> case J < 0 of true -> lists:nth(erlang:length(Backward_path) + J + 1, Backward_path); _ -> lists:nth(J + 1, Backward_path) end end)]),
            J_2 = (J + 1),
...
is_edge(G_2, U, V_3) ->
    ...
    case (I_7 < erlang:length(Neighbors_2)) of
        true ->
            case ((case erlang:is_map(Neighbors_2) of true -> maps:get(I_7, Neighbors_2, nil); _ -> case I_7 < 0 of true -> lists:nth(erlang:length(Neighbors_2) + I_7 + 1, Neighbors_2); _ -> lists:nth(I_7 + 1, Neighbors_2) end end) == V_3) of
...
path_exists(G_3, Path_3) ->
    ...
    case ((I_9 + 1) < erlang:length(Path_3)) of
        true ->
            case mochi_not(is_edge(G_3, (case erlang:is_map(Path_3) of true -> maps:get(I_9, Path_3, nil); _ -> case I_9 < 0 of true -> lists:nth(erlang:length(Path_3) + I_9 + 1, Path_3); _ -> lists:nth(I_9 + 1, Path_3) end end), (case erlang:is_map(Path_3) of true -> maps:get((I_9 + 1), Path_3, nil); _ -> case (I_9 + 1) < 0 of true -> lists:nth(erlang:length(Path_3) + (I_9 + 1) + 1, Path_3); _ -> lists:nth((I_9 + 1) + 1, Path_3) end end))) of
        true -> throw({return, false});
        _ -> ok
    end,
            I_10 = (I_9 + 1),
