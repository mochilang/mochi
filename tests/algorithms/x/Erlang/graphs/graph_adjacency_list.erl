...
{Adj_2, Directed, Edges, Vertices} = Fun(Vertices, Adj, Directed, Edges, Vertices),
    ...
    case List of
        ...
        [E|E_rest] ->
            S = (case erlang:is_map(E) of true -> maps:get(0, E, nil); _ -> case 0 < 0 of true -> lists:nth(erlang:length(E) + 0 + 1, E); _ -> lists:nth(0 + 1, E) end end),
            D = (case erlang:is_map(E) of true -> maps:get(1, E, nil); _ -> case 1 < 0 of true -> lists:nth(erlang:length(E) + 1 + 1, E); _ -> lists:nth(1 + 1, E) end end),
...
add_vertex(Graph, V_2) ->
    try
        ...
        true -> erlang:error("vertex exists");
        _ -> ok
    end,
        Adj_10 = #{},
...
remove_from_list(Lst, Value) ->
    ...
    case (I < erlang:length(Lst)) of
        true ->
            case ((case erlang:is_map(Lst) of true -> maps:get(I, Lst, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(Lst) + I + 1, Lst); _ -> lists:nth(I + 1, Lst) end end) /= Value) of
        true -> Res_2 = lists:append((case Res of nil -> []; _ -> Res end), [(case erlang:is_map(Lst) of true -> maps:get(I, Lst, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(Lst) + I + 1, Lst); _ -> lists:nth(I + 1, Lst) end end)]),
            Res_3 = Res_2;
        _ -> Res_3 = Res
    end,
            I_2 = (I + 1),
...
add_edge(Graph_2, S_2, D_2) ->
    try
        ...
        true -> erlang:error("vertex missing");
        _ -> ok
    end,
        case contains_edge(Graph_2, S_2, D_2) of
        true -> erlang:error("edge exists");
        _ -> ok
    end,
        Adj_13 = #{},
...
remove_edge(Graph_3, S_3, D_3) ->
    try
        ...
        true -> erlang:error("edge missing");
        _ -> ok
    end,
        Adj_18 = #{},
