...
bfs_shortest_path(Graph, Start, Goal) ->
    ...
    case (erlang:length(Queue) > 0) of
        true ->
            Path = (case erlang:is_map(Queue) of true -> maps:get(0, Queue, nil); _ -> case 0 < 0 of true -> lists:nth(erlang:length(Queue) + 0 + 1, Queue); _ -> lists:nth(0 + 1, Queue) end end),
            ...
            Node = (case erlang:is_map(Path) of true -> maps:get((erlang:length(Path) - 1), Path, nil); _ -> case (erlang:length(Path) - 1) < 0 of true -> lists:nth(erlang:length(Path) + (erlang:length(Path) - 1) + 1, Path); _ -> lists:nth((erlang:length(Path) - 1) + 1, Path) end end),
    ...
    case (I_3 < erlang:length(Neighbours)) of
        true ->
            Neighbour = (case erlang:is_map(Neighbours) of true -> maps:get(I_3, Neighbours, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Neighbours) + I_3 + 1, Neighbours); _ -> lists:nth(I_3 + 1, Neighbours) end end),
            New_path = Path,
...
bfs_shortest_path_distance(Graph_2, Start_2, Target) ->
    ...
    case (erlang:length(Queue_5) > 0) of
        true ->
            Node_2 = (case erlang:is_map(Queue_5) of true -> maps:get(0, Queue_5, nil); _ -> case 0 < 0 of true -> lists:nth(erlang:length(Queue_5) + 0 + 1, Queue_5); _ -> lists:nth(0 + 1, Queue_5) end end),
    ...
    case (I_6 < erlang:length(Adj)) of
        true ->
            Next = (case erlang:is_map(Adj) of true -> maps:get(I_6, Adj, nil); _ -> case I_6 < 0 of true -> lists:nth(erlang:length(Adj) + I_6 + 1, Adj); _ -> lists:nth(I_6 + 1, Adj) end end),
