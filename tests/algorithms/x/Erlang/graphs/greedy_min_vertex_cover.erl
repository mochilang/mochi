...
remove_value(Lst, Val) ->
    ...
    case (I < erlang:length(Lst)) of
        true ->
            case ((case erlang:is_map(Lst) of true -> maps:get(I, Lst, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(Lst) + I + 1, Lst); _ -> lists:nth(I + 1, Lst) end end) /= Val) of
...
{Cover, G, Graph, Max_deg_3, Max_v_3} = Fun_2(maps:keys(G), Cover, G, Graph, Max_deg, Max_v),
    ...
    case (I_3 < erlang:length(Neighbors)) of
        true ->
            N = (case erlang:is_map(Neighbors) of true -> maps:get(I_3, Neighbors, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Neighbors) + I_3 + 1, Neighbors); _ -> lists:nth(I_3 + 1, Neighbors) end end),
