...
path_to_string(Path) ->
    ...
    case (I < erlang:length(Path)) of
        true ->
            Pt = (case erlang:is_map(Path) of true -> maps:get(I, Path, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(Path) + I + 1, Path); _ -> lists:nth(I + 1, Path) end end),
...
dijkstra(Grid, Source, Destination, Allow_diagonal) ->
    try
        ...
        Cols = erlang:length((case erlang:is_map(Grid) of true -> maps:get(0, Grid, nil); _ -> case 0 < 0 of true -> lists:nth(erlang:length(Grid) + 0 + 1, Grid); _ -> lists:nth(0 + 1, Grid) end end)),
        Dx = [-1, 1, 0, 0],
        Dy = [0, 0, -1, 1],
    ...
    case (Front < erlang:length(Queue)) of
        true ->
            try
                Current = (case erlang:is_map(Queue) of true -> maps:get(Front, Queue, nil); _ -> case Front < 0 of true -> lists:nth(erlang:length(Queue) + Front + 1, Queue); _ -> lists:nth(Front + 1, Queue) end end),
                Front_2 = (Front + 1),
                Cur_key = key(Current),
    ...
    case (I_3 < erlang:length(Dx_3)) of
        true ->
            Nx = (maps:get("x", Current, nil) + (case erlang:is_map(Dx_3) of true -> maps:get(I_3, Dx_3, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Dx_3) + I_3 + 1, Dx_3); _ -> lists:nth(I_3 + 1, Dx_3) end end)),
            Ny = (maps:get("y", Current, nil) + (case erlang:is_map(Dy_3) of true -> maps:get(I_3, Dy_3, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Dy_3) + I_3 + 1, Dy_3); _ -> lists:nth(I_3 + 1, Dy_3) end end)),
            case ((((Nx >= 0) andalso (Nx < Rows)) andalso (Ny >= 0)) andalso (Ny < Cols)) of
        true -> case ((case erlang:is_map((case erlang:is_map(Grid) of true -> maps:get(Nx, Grid, nil); _ -> case Nx < 0 of true -> lists:nth(erlang:length(Grid) + Nx + 1, Grid); _ -> lists:nth(Nx + 1, Grid) end end)) of true -> maps:get(Ny, (case erlang:is_map(Grid) of true -> maps:get(Nx, Grid, nil); _ -> case Nx < 0 of true -> lists:nth(erlang:length(Grid) + Nx + 1, Grid); _ -> lists:nth(Nx + 1, Grid) end end), nil); _ -> case Ny < 0 of true -> lists:nth(erlang:length((case erlang:is_map(Grid) of true -> maps:get(Nx, Grid, nil); _ -> case Nx < 0 of true -> lists:nth(erlang:length(Grid) + Nx + 1, Grid); _ -> lists:nth(Nx + 1, Grid) end end)) + Ny + 1, (case erlang:is_map(Grid) of true -> maps:get(Nx, Grid, nil); _ -> case Nx < 0 of true -> lists:nth(erlang:length(Grid) + Nx + 1, Grid); _ -> lists:nth(Nx + 1, Grid) end end)); _ -> lists:nth(Ny + 1, (case erlang:is_map(Grid) of true -> maps:get(Nx, Grid, nil); _ -> case Nx < 0 of true -> lists:nth(erlang:length(Grid) + Nx + 1, Grid); _ -> lists:nth(Nx + 1, Grid) end end)) end end) == 1) of
        true -> N_key = ((mochi_str(Nx) ++ ",") ++ mochi_str(Ny)),
            case mochi_not(maps:is_key(N_key, Dist_map)) of
        true -> Dist_map_2 = maps:put(N_key, (maps:get(Cur_key, Dist_map, nil) + 1), Dist_map),
...
{INF, Allow_diagonal, Cols, Dest_key, Destination, Dist_map_5, Dx_3, Dy_3, Front_2, Grid, Path_rev_2, Prev_5, Queue_5, Rows, Source, Step_key_2, Step_pt_2} = Fun_4(INF, Allow_diagonal, Cols, Dest_key, Destination, Dist_map_5, Dx_3, Dy_3, Front_2, Grid, Path_rev, Prev_5, Queue_5, Rows, Source, Step_key, Step_pt),
    ...
    case (K >= 0) of
        true ->
            Path_3 = lists:append((case Path_2 of nil -> []; _ -> Path_2 end), [(case erlang:is_map(Path_rev_2) of true -> maps:get(K, Path_rev_2, nil); _ -> case K < 0 of true -> lists:nth(erlang:length(Path_rev_2) + K + 1, Path_rev_2); _ -> lists:nth(K + 1, Path_rev_2) end end)]),
            K_2 = (K - 1),
