...
dfs(U, Graph, Visit, Stack) ->
    try
        case (case erlang:is_map(Visit) of true -> maps:get(U, Visit, nil); _ -> case U < 0 of true -> lists:nth(erlang:length(Visit) + U + 1, Visit); _ -> lists:nth(U + 1, Visit) end end) of
...
{Graph, Stack_2, U, Visit_2} = Fun((case erlang:is_map(Graph) of true -> maps:get(U, Graph, nil); _ -> case U < 0 of true -> lists:nth(erlang:length(Graph) + U + 1, Graph); _ -> lists:nth(U + 1, Graph) end end), Graph, Stack, U, Visit_2),
...
dfs2(U_2, Reversed_graph, Visit_3, Component) ->
    try
        case (case erlang:is_map(Visit_3) of true -> maps:get(U_2, Visit_3, nil); _ -> case U_2 < 0 of true -> lists:nth(erlang:length(Visit_3) + U_2 + 1, Visit_3); _ -> lists:nth(U_2 + 1, Visit_3) end end) of
...
{Component_3, Reversed_graph, U_2, Visit_4} = Fun_2((case erlang:is_map(Reversed_graph) of true -> maps:get(U_2, Reversed_graph, nil); _ -> case U_2 < 0 of true -> lists:nth(erlang:length(Reversed_graph) + U_2 + 1, Reversed_graph); _ -> lists:nth(U_2 + 1, Reversed_graph) end end), Component_2, Reversed_graph, U_2, Visit_4),
        Component_3
    catch {return, Ret} -> Ret end.
...
{Graph_2, I_2, N, Reversed_graph_3} = Fun_3(Graph_2, I, N, Reversed_graph_2),
    ...
    case List of
        ...
        [V_3|V_3_rest] ->
            Reversed_graph_4 = lists:sublist(Reversed_graph_3, V_3) ++ [lists:append((case (case erlang:is_map(Reversed_graph_3) of true -> maps:get(V_3, Reversed_graph_3, nil); _ -> case V_3 < 0 of true -> lists:nth(erlang:length(Reversed_graph_3) + V_3 + 1, Reversed_graph_3); _ -> lists:nth(V_3 + 1, Reversed_graph_3) end end) of nil -> []; _ -> (case erlang:is_map(Reversed_graph_3) of true -> maps:get(V_3, Reversed_graph_3, nil); _ -> case V_3 < 0 of true -> lists:nth(erlang:length(Reversed_graph_3) + V_3 + 1, Reversed_graph_3); _ -> lists:nth(V_3 + 1, Reversed_graph_3) end end) end), [I_3])] ++ lists:nthtail(V_3 + 1, Reversed_graph_3),
            Fun_4_loop(V_3_rest, Graph_2, I_3, N, Reversed_graph_4);
        _ -> {Graph_2, I_3, N, Reversed_graph_3}
    end
end,
{Graph_2, I_3, N, Reversed_graph_4} = Fun_4((case erlang:is_map(Graph_2) of true -> maps:get(I_3, Graph_2, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Graph_2) + I_3 + 1, Graph_2); _ -> lists:nth(I_3 + 1, Graph_2) end end), Graph_2, I_3, N, Reversed_graph_3),
            I_4 = (I_3 + 1),
...
{Graph_2, I_6, N, Reversed_graph_4, Visit_6} = Fun_6(Graph_2, I_5, N, Reversed_graph_4, Visit_5),
    ...
    case (I_7 < N) of
        true ->
            case ((case erlang:is_map(Visit_6) of true -> maps:get(I_7, Visit_6, nil); _ -> case I_7 < 0 of true -> lists:nth(erlang:length(Visit_6) + I_7 + 1, Visit_6); _ -> lists:nth(I_7 + 1, Visit_6) end end) == false) of
...
{Graph_2, I_10, N, Reversed_graph_4, Stack_6, Visit_7} = Fun_8(Graph_2, I_9, N, Reversed_graph_4, Stack_6, Visit_6),
    ...
    case (Idx >= 0) of
        true ->
            Node = (case erlang:is_map(Stack_6) of true -> maps:get(Idx, Stack_6, nil); _ -> case Idx < 0 of true -> lists:nth(erlang:length(Stack_6) + Idx + 1, Stack_6); _ -> lists:nth(Idx + 1, Stack_6) end end),
            case ((case erlang:is_map(Visit_7) of true -> maps:get(Node, Visit_7, nil); _ -> case Node < 0 of true -> lists:nth(erlang:length(Visit_7) + Node + 1, Visit_7); _ -> lists:nth(Node + 1, Visit_7) end end) == false) of
        true -> Component_4 = [],
...
main() ->
    ...
    case (I_11 < erlang:length(Comps)) of
        true ->
            io:format("~ts~n", [mochi_repr((case erlang:is_map(Comps) of true -> maps:get(I_11, Comps, nil); _ -> case I_11 < 0 of true -> lists:nth(erlang:length(Comps) + I_11 + 1, Comps); _ -> lists:nth(I_11 + 1, Comps) end end))]),
            I_12 = (I_11 + 1),
