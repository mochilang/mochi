...
atanapprox(X_5) ->
    try
        case (X_5 > 1) of
        true -> throw({return, (mochi_safe_div(3.141592653589793, 2) - mochi_safe_div(X_5, (mochi_safe_mul(X_5, X_5) + 0.28)))});
        _ -> ok
    end,
        case (X_5 < -1) of
        true -> throw({return, (mochi_safe_div(-3.141592653589793, 2) - mochi_safe_div(X_5, (mochi_safe_mul(X_5, X_5) + 0.28)))});
        ...
        mochi_safe_div(X_5, (1 + mochi_safe_mul(mochi_safe_mul(0.28, X_5), X_5)))
    catch {return, Ret} -> Ret end.
...
asinapprox(X_7) ->
    try
        Denom = sqrtapprox((1 - mochi_safe_mul(X_7, X_7))),
...
haversine_distance(Lat1, Lon1, Lat2, Lon2) ->
    try
        Flattening = mochi_safe_div((6.378137e+06 - 6.356752314245e+06), 6.378137e+06),
        Phi_1 = atanapprox(mochi_safe_mul((1 - Flattening), tan_approx(to_radians(Lat1)))),
        Phi_2 = atanapprox(mochi_safe_mul((1 - Flattening), tan_approx(to_radians(Lat2)))),
        Lambda_1 = to_radians(Lon1),
        Lambda_2 = to_radians(Lon2),
        Sin_sq_phi = sin_taylor(mochi_safe_div((Phi_2 - Phi_1), 2)),
        Sin_sq_lambda = sin_taylor(mochi_safe_div((Lambda_2 - Lambda_1), 2)),
        ...
        mochi_safe_mul(mochi_safe_mul(2, 6.378137e+06), asinapprox(H_value))
    catch {return, Ret} -> Ret end.
...
main(_) ->
    ...
    erlang:put('AXIS_A', 6.378137e+06),
    erlang:put('AXIS_B', 6.356752314245e+06),
    erlang:put('RADIUS', 6.378137e+06),
    erlang:put('SAN_FRANCISCO', [37.774856, -122.424227]),
    erlang:put('YOSEMITE', [37.864742, -119.537521]),
    io:format("~ts~n", [mochi_str(haversine_distance((case erlang:is_map(erlang:get('SAN_FRANCISCO')) of true -> maps:get(0, erlang:get('SAN_FRANCISCO'), nil); _ -> case 0 < 0 of true -> lists:nth(erlang:length(erlang:get('SAN_FRANCISCO')) + 0 + 1, erlang:get('SAN_FRANCISCO')); _ -> lists:nth(0 + 1, erlang:get('SAN_FRANCISCO')) end end), (case erlang:is_map(erlang:get('SAN_FRANCISCO')) of true -> maps:get(1, erlang:get('SAN_FRANCISCO'), nil); _ -> case 1 < 0 of true -> lists:nth(erlang:length(erlang:get('SAN_FRANCISCO')) + 1 + 1, erlang:get('SAN_FRANCISCO')); _ -> lists:nth(1 + 1, erlang:get('SAN_FRANCISCO')) end end), (case erlang:is_map(erlang:get('YOSEMITE')) of true -> maps:get(0, erlang:get('YOSEMITE'), nil); _ -> case 0 < 0 of true -> lists:nth(erlang:length(erlang:get('YOSEMITE')) + 0 + 1, erlang:get('YOSEMITE')); _ -> lists:nth(0 + 1, erlang:get('YOSEMITE')) end end), (case erlang:is_map(erlang:get('YOSEMITE')) of true -> maps:get(1, erlang:get('YOSEMITE'), nil); _ -> case 1 < 0 of true -> lists:nth(erlang:length(erlang:get('YOSEMITE')) + 1 + 1, erlang:get('YOSEMITE')); _ -> lists:nth(1 + 1, erlang:get('YOSEMITE')) end end)))]),
