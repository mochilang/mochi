...
coulombs_law(Force, Charge1, Charge2, Distance) ->
    try
        ...
        case (Distance < 0) of
        true -> erlang:error("Distance cannot be negative");
        _ -> ok
    end,
        case (Force == 0) of
        true -> F = mochi_safe_div(mochi_safe_mul(8.988e+09, Charge_product), mochi_safe_mul(Distance, Distance)),
            throw({return, #{"force" => F}}),
            F_2 = F;
        _ -> F_2 = nil
    end,
        case (Charge1 == 0) of
        true -> C1 = mochi_safe_div(mochi_safe_mul(erlang:abs(Force), mochi_safe_mul(Distance, Distance)), mochi_safe_mul(8.988e+09, Charge2)),
            throw({return, #{"charge1" => C1}}),
            C1_2 = C1;
        _ -> C1_2 = nil
    end,
        case (Charge2 == 0) of
        true -> C2 = mochi_safe_div(mochi_safe_mul(erlang:abs(Force), mochi_safe_mul(Distance, Distance)), mochi_safe_mul(8.988e+09, Charge1)),
            throw({return, #{"charge2" => C2}}),
            C2_2 = C2;
        _ -> C2_2 = nil
    end,
        D = sqrtapprox(mochi_safe_div(mochi_safe_mul(8.988e+09, Charge_product), erlang:abs(Force))),
        #{"distance" => D}
    catch {return, Ret} -> Ret end.
...
main(_) ->
    ...
    erlang:put('COULOMBS_CONSTANT', 8.988e+09),
