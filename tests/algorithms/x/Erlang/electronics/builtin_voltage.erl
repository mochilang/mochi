...
{K_3, X_2, Y_3} = Fun_4(K_2, X_2, Y_2),
        (ln_series(Y_3) + mochi_safe_mul(float(K_3), ln_series(10)))
    catch {return, Ret} -> Ret end.

builtin_voltage(Donor_conc, Acceptor_conc, Intrinsic_conc) ->
    try
        case (Donor_conc =< 0) of
        true -> erlang:error("Donor concentration should be positive");
        _ -> ok
    end,
        case (Acceptor_conc =< 0) of
        true -> erlang:error("Acceptor concentration should be positive");
        _ -> ok
    end,
        case (Intrinsic_conc =< 0) of
        true -> erlang:error("Intrinsic concentration should be positive");
        _ -> ok
    end,
        case (Donor_conc =< Intrinsic_conc) of
        true -> erlang:error("Donor concentration should be greater than intrinsic concentration");
        _ -> ok
    end,
        case (Acceptor_conc =< Intrinsic_conc) of
        true -> erlang:error("Acceptor concentration should be greater than intrinsic concentration");
        _ -> ok
    end,
        mochi_safe_div(mochi_safe_mul(mochi_safe_mul(erlang:get('BOLTZMANN'), 300), ln(mochi_safe_div(mochi_safe_mul(Donor_conc, Acceptor_conc), mochi_safe_mul(Intrinsic_conc, Intrinsic_conc)))), erlang:get('ELECTRON_VOLT'))
    catch {return, Ret} -> Ret end.
