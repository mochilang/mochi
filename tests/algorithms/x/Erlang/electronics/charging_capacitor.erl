...
{Is_neg_3, N_2, Sum_2, Term_2, X, Y_3} = Fun(Is_neg_3, N, Sum, Term, X, Y_3),
        case Is_neg_3 of
        true -> throw({return, mochi_safe_div(1, Sum_2)});
        _ -> ok
    end,
        Sum_2
...
round3(X_2) ->
    try
        Scaled = mochi_safe_mul(X_2, 1000),
        case (Scaled >= 0) of
        ...
        mochi_safe_div(float(Scaled_int), 1000)
    catch {return, Ret} -> Ret end.
...
charging_capacitor(Source_voltage, Resistance, Capacitance, Time_sec) ->
    try
        ...
        true -> erlang:error("Capacitance must be positive.");
        _ -> ok
    end,
        Exponent = mochi_safe_div(-Time_sec, mochi_safe_mul(Resistance, Capacitance)),
        Voltage = mochi_safe_mul(Source_voltage, (1 - expapprox(Exponent))),
        round3(Voltage)
    catch {return, Ret} -> Ret end.
...
main(_) ->
    ...
    io:format("~ts~n", [mochi_repr(charging_capacitor(2.2, 3.5, 2.4, 9))]),
    io:format("~ts~n", [mochi_repr(charging_capacitor(15, 200, 20, 2))]),
    io:format("~ts~n", [mochi_repr(charging_capacitor(20, 2000, 0.0003, 4))]),
