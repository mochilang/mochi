...
schur_complement(Mat_a, Mat_b, Mat_c, Pseudo_inv) ->
    try
        ...
        true -> erlang:error("Matrix A must be square");
        _ -> ok
    end,
        case (A_rows /= erlang:length(Mat_b)) of
        true -> erlang:error("Expected the same number of rows for A and B");
        _ -> ok
    end,
        ...
        true -> erlang:error("Expected the same number of columns for B and C");
        _ -> ok
    end,
        A_inv = nil,
