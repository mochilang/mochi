...
{I, J_2, Matrix, N, Sum_3} = Fun(I, J, Matrix, N, Sum),
        ...
        true -> erlang:error("Coefficient matrix is not strictly diagonally dominant");
        _ -> ok
    end,
            I_2 = (I + 1),
...
jacobi_iteration_method(Coefficient, Constant, Init_val, Iterations) ->
    try
        ...
        case (N_2 == 0) of
        true -> erlang:error("Coefficient matrix cannot be empty");
        _ -> ok
    end,
        case (erlang:length(Constant) /= N_2) of
        true -> erlang:error("Constant vector length must equal number of rows in coefficient matrix");
        _ -> ok
    end,
        case (erlang:length(Init_val) /= N_2) of
        true -> erlang:error("Initial values count must match matrix size");
        _ -> ok
    end,
        R = 0,
    ...
    case (R < N_2) of
        ...
        true -> erlang:error("Coefficient matrix must be square");
        _ -> ok
    end,
            R_2 = (R + 1),
...
{Coefficient, Constant, Init_val, Iterations, N_2, R_2} = Fun_3(Coefficient, Constant, Init_val, Iterations, N_2, R),
        ...
        true -> erlang:error("Iterations must be at least 1");
        _ -> ok
    end,
...
main(_) ->
    ...
    erlang:put('coefficient', [[4, 1, 1], [1, 5, 2], [1, 2, 4]]),
    erlang:put('constant', [2, -6, -4]),
