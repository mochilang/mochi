...
index_of_min(Xs) ->
    ...
    case (I < erlang:length(Xs)) of
        true ->
            case ((case erlang:is_map(Xs) of true -> maps:get(I, Xs, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(Xs) + I + 1, Xs); _ -> lists:nth(I + 1, Xs) end end) < (case erlang:is_map(Xs) of true -> maps:get(Min_idx, Xs, nil); _ -> case Min_idx < 0 of true -> lists:nth(erlang:length(Xs) + Min_idx + 1, Xs); _ -> lists:nth(Min_idx + 1, Xs) end end)) of
        true -> Min_idx_2 = I,
            Min_idx_3 = Min_idx_2;
        _ -> Min_idx_3 = Min_idx
    end,
...
remove_at(Xs_2, Idx) ->
    ...
    case (I_3 < erlang:length(Xs_2)) of
        true ->
            case (I_3 /= Idx) of
        true -> Res_2 = lists:append((case Res of nil -> []; _ -> Res end), [(case erlang:is_map(Xs_2) of true -> maps:get(I_3, Xs_2, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Xs_2) + I_3 + 1, Xs_2); _ -> lists:nth(I_3 + 1, Xs_2) end end)]),
            Res_3 = Res_2;
        _ -> Res_3 = Res
    end,
            I_4 = (I_3 + 1),
...
optimal_merge_pattern(Files) ->
    ...
    case (K < 2) of
        true ->
            ...
            Temp_2 = (Temp + (case erlang:is_map(Arr) of true -> maps:get(Min_idx_4, Arr, nil); _ -> case Min_idx_4 < 0 of true -> lists:nth(erlang:length(Arr) + Min_idx_4 + 1, Arr); _ -> lists:nth(Min_idx_4 + 1, Arr) end end)),
