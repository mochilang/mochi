...
sort_by_ratio_desc(Arr) ->
    ...
    case (J >= 0) of
        true ->
            try
                Current = (case erlang:is_map(Arr) of true -> maps:get(J, Arr, nil); _ -> case J < 0 of true -> lists:nth(erlang:length(Arr) + J + 1, Arr); _ -> lists:nth(J + 1, Arr) end end),
...
sum_first(Arr_5, K) ->
    ...
    case ((I_3 < K) andalso (I_3 < erlang:length(Arr_5))) of
        true ->
            S_2 = (S + (case erlang:is_map(Arr_5) of true -> maps:get(I_3, Arr_5, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Arr_5) + I_3 + 1, Arr_5); _ -> lists:nth(I_3 + 1, Arr_5) end end)),
            I_4 = (I_3 + 1),
...
frac_knapsack(Vl, Wt, W, N) ->
    ...
    case ((I_5 < erlang:length(Vl)) andalso (I_5 < erlang:length(Wt))) of
        true ->
            Items_2 = lists:append((case Items of nil -> []; _ -> Items end), [#{"value" => (case erlang:is_map(Vl) of true -> maps:get(I_5, Vl, nil); _ -> case I_5 < 0 of true -> lists:nth(erlang:length(Vl) + I_5 + 1, Vl); _ -> lists:nth(I_5 + 1, Vl) end end), "weight" => (case erlang:is_map(Wt) of true -> maps:get(I_5, Wt, nil); _ -> case I_5 < 0 of true -> lists:nth(erlang:length(Wt) + I_5 + 1, Wt); _ -> lists:nth(I_5 + 1, Wt) end end)}]),
            I_6 = (I_5 + 1),
...
{I_6, Items_2, N, Vl, W, Wt} = Fun_4(I_5, Items, N, Vl, W, Wt),
    ...
    case (I_7 < erlang:length(Items_3)) of
        true ->
            Itm = (case erlang:is_map(Items_3) of true -> maps:get(I_7, Items_3, nil); _ -> case I_7 < 0 of true -> lists:nth(erlang:length(Items_3) + I_7 + 1, Items_3); _ -> lists:nth(I_7 + 1, Items_3) end end),
...
{I_8, Items_3, N, Values_2, Vl, W, Weights_2, Wt} = Fun_5(I_7, Items_3, N, Values, Vl, W, Weights, Wt),
    ...
    case (I_9 < erlang:length(Weights_2)) of
        true ->
            Total_2 = (Total + (case erlang:is_map(Weights_2) of true -> maps:get(I_9, Weights_2, nil); _ -> case I_9 < 0 of true -> lists:nth(erlang:length(Weights_2) + I_9 + 1, Weights_2); _ -> lists:nth(I_9 + 1, Weights_2) end end)),
...
{Acc_2, I_10, Items_3, N, Total_2, Values_2, Vl, W, Weights_2, Wt} = Fun_6(Acc, I_9, Items_3, N, Total, Values_2, Vl, W, Weights_2, Wt),
    ...
    case ((K_2 < erlang:length(Acc_2)) andalso (W >= (case erlang:is_map(Acc_2) of true -> maps:get(K_2, Acc_2, nil); _ -> case K_2 < 0 of true -> lists:nth(erlang:length(Acc_2) + K_2 + 1, Acc_2); _ -> lists:nth(K_2 + 1, Acc_2) end end))) of
        true ->
            K_3 = (K_2 + 1),
...
{Acc_2, I_10, Items_3, K_3, N, Total_2, Values_2, Vl, W, Weights_2, Wt} = Fun_7(Acc_2, I_10, Items_3, K_2, N, Total_2, Values_2, Vl, W, Weights_2, Wt),
        ...
        case (K_3 /= N) of
        true -> throw({return, (sum_first(Values_2, K_3) + mochi_safe_div(mochi_safe_mul((W - (case erlang:is_map(Acc_2) of true -> maps:get((K_3 - 1), Acc_2, nil); _ -> case (K_3 - 1) < 0 of true -> lists:nth(erlang:length(Acc_2) + (K_3 - 1) + 1, Acc_2); _ -> lists:nth((K_3 - 1) + 1, Acc_2) end end)), (case erlang:is_map(Values_2) of true -> maps:get(K_3, Values_2, nil); _ -> case K_3 < 0 of true -> lists:nth(erlang:length(Values_2) + K_3 + 1, Values_2); _ -> lists:nth(K_3 + 1, Values_2) end end)), (case erlang:is_map(Weights_2) of true -> maps:get(K_3, Weights_2, nil); _ -> case K_3 < 0 of true -> lists:nth(erlang:length(Weights_2) + K_3 + 1, Weights_2); _ -> lists:nth(K_3 + 1, Weights_2) end end)))});
        _ -> ok
    end,
...
main(_) ->
    ...
    erlang:put('vl', [60, 100, 120]),
    erlang:put('wt', [10, 20, 30]),
    erlang:put('result', frac_knapsack(erlang:get('vl'), erlang:get('wt'), 50, 3)),
