...
get_gas_stations(Gas_quantities, Costs) ->
    ...
    case (I < erlang:length(Gas_quantities)) of
        true ->
            Stations_2 = lists:append((case Stations of nil -> []; _ -> Stations end), [#{"gas_quantity" => (case erlang:is_map(Gas_quantities) of true -> maps:get(I, Gas_quantities, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(Gas_quantities) + I + 1, Gas_quantities); _ -> lists:nth(I + 1, Gas_quantities) end end), "cost" => (case erlang:is_map(Costs) of true -> maps:get(I, Costs, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(Costs) + I + 1, Costs); _ -> lists:nth(I + 1, Costs) end end)}]),
            I_2 = (I + 1),
...
can_complete_journey(Gas_stations) ->
    ...
    case (I_3 < erlang:length(Gas_stations)) of
        true ->
            Total_gas_2 = (Total_gas + maps:get("gas_quantity", (case erlang:is_map(Gas_stations) of true -> maps:get(I_3, Gas_stations, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Gas_stations) + I_3 + 1, Gas_stations); _ -> lists:nth(I_3 + 1, Gas_stations) end end), nil)),
            Total_cost_2 = (Total_cost + maps:get("cost", (case erlang:is_map(Gas_stations) of true -> maps:get(I_3, Gas_stations, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Gas_stations) + I_3 + 1, Gas_stations); _ -> lists:nth(I_3 + 1, Gas_stations) end end), nil)),
            I_4 = (I_3 + 1),
...
{Gas_stations, I_4, Total_cost_2, Total_gas_2} = Fun_2(Gas_stations, I_3, Total_cost, Total_gas),
    ...
    case (I_5 < erlang:length(Gas_stations)) of
        true ->
            Station = (case erlang:is_map(Gas_stations) of true -> maps:get(I_5, Gas_stations, nil); _ -> case I_5 < 0 of true -> lists:nth(erlang:length(Gas_stations) + I_5 + 1, Gas_stations); _ -> lists:nth(I_5 + 1, Gas_stations) end end),
