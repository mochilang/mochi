...
catalan_numbers(Upper_limit) ->
    try
        ...
        true -> erlang:error("Limit for the Catalan sequence must be >= 0"),
            throw({return, []});
        _ -> ok
    end,
        Catalans = [1],
    ...
    case (J < N) of
        true ->
            Next_val_2 = (Next_val + ((case erlang:is_map(Catalans) of true -> maps:get(J, Catalans, nil); _ -> case J < 0 of true -> lists:nth(erlang:length(Catalans) + J + 1, Catalans); _ -> lists:nth(J + 1, Catalans) end end) * (case erlang:is_map(Catalans) of true -> maps:get(((N - J) - 1), Catalans, nil); _ -> case ((N - J) - 1) < 0 of true -> lists:nth(erlang:length(Catalans) + ((N - J) - 1) + 1, Catalans); _ -> lists:nth(((N - J) - 1) + 1, Catalans) end end))),
            J_2 = (J + 1),
