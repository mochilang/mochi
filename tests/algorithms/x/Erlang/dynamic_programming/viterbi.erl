...
viterbi(Observations, States, Start_p, Trans_p, Emit_p) ->
    try
        ...
        true -> erlang:error("empty parameters");
        _ -> ok
    end,
        Probs = #{},
        Ptrs = #{},
        First_obs = (case erlang:is_map(Observations) of true -> maps:get(0, Observations, nil); _ -> case 0 < 0 of true -> lists:nth(erlang:length(Observations) + 0 + 1, Observations); _ -> lists:nth(0 + 1, Observations) end end),
        I = 0,
        Fun = fun Fun_loop(Emit_p, First_obs, I, Observations, Probs, Ptrs, Start_p, States, Trans_p) ->
    case (I < erlang:length(States)) of
        true ->
            State_2 = (case erlang:is_map(States) of true -> maps:get(I, States, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(States) + I + 1, States); _ -> lists:nth(I + 1, States) end end),
...
{Emit_p, First_obs, I_2, Observations, Probs_2, Ptrs_2, Start_p, States, Trans_p} = Fun(Emit_p, First_obs, I, Observations, Probs, Ptrs, Start_p, States, Trans_p),
    ...
    case (T < erlang:length(Observations)) of
        true ->
            Obs_2 = (case erlang:is_map(Observations) of true -> maps:get(T, Observations, nil); _ -> case T < 0 of true -> lists:nth(erlang:length(Observations) + T + 1, Observations); _ -> lists:nth(T + 1, Observations) end end),
            J = 0,
    ...
    case (J < erlang:length(States)) of
        true ->
            State_3 = (case erlang:is_map(States) of true -> maps:get(J, States, nil); _ -> case J < 0 of true -> lists:nth(erlang:length(States) + J + 1, States); _ -> lists:nth(J + 1, States) end end),
            Max_prob = -1,
            Prev_state = "",
            K = 0,
    ...
    case (K < erlang:length(States)) of
        true ->
            State0 = (case erlang:is_map(States) of true -> maps:get(K, States, nil); _ -> case K < 0 of true -> lists:nth(erlang:length(States) + K + 1, States); _ -> lists:nth(K + 1, States) end end),
            Obs0 = (case erlang:is_map(Observations) of true -> maps:get((T - 1), Observations, nil); _ -> case (T - 1) < 0 of true -> lists:nth(erlang:length(Observations) + (T - 1) + 1, Observations); _ -> lists:nth((T - 1) + 1, Observations) end end),
...
{Emit_p, First_obs, I_2, N_2, Observations, Path_2, Probs_3, Ptrs_3, Start_p, States, T_2, Trans_p} = Fun_5(Emit_p, First_obs, I_2, N, Observations, Path, Probs_3, Ptrs_3, Start_p, States, T_2, Trans_p),
        Last_obs = (case erlang:is_map(Observations) of true -> maps:get((erlang:length(Observations) - 1), Observations, nil); _ -> case (erlang:length(Observations) - 1) < 0 of true -> lists:nth(erlang:length(Observations) + (erlang:length(Observations) - 1) + 1, Observations); _ -> lists:nth((erlang:length(Observations) - 1) + 1, Observations) end end),
        Max_final = -1,
        Last_state = "",
        M = 0,
    ...
    case (M < erlang:length(States)) of
        true ->
            State_4 = (case erlang:is_map(States) of true -> maps:get(M, States, nil); _ -> case M < 0 of true -> lists:nth(erlang:length(States) + M + 1, States); _ -> lists:nth(M + 1, States) end end),
...
{Emit_p, First_obs, I_2, Last_obs, Last_state_3, M_2, Max_final_3, N_2, Observations, Path_2, Probs_3, Ptrs_3, Start_p, States, T_2, Trans_p} = Fun_6(Emit_p, First_obs, I_2, Last_obs, Last_state, M, Max_final, N_2, Observations, Path_2, Probs_3, Ptrs_3, Start_p, States, T_2, Trans_p),
    ...
    case (Idx > 0) of
        true ->
            Obs_3 = (case erlang:is_map(Observations) of true -> maps:get(Idx, Observations, nil); _ -> case Idx < 0 of true -> lists:nth(erlang:length(Observations) + Idx + 1, Observations); _ -> lists:nth(Idx + 1, Observations) end end),
            Prev = maps:get(key((case erlang:is_map(Path_3) of true -> maps:get(Idx, Path_3, nil); _ -> case Idx < 0 of true -> lists:nth(erlang:length(Path_3) + Idx + 1, Path_3); _ -> lists:nth(Idx + 1, Path_3) end end), Obs_3), Ptrs_3, nil),
...
join_words(Words) ->
    ...
    case (I_3 < erlang:length(Words)) of
        ...
        _ -> Res_3 = Res
            ...
            Res_4 = (Res_3 ++ (case erlang:is_map(Words) of true -> maps:get(I_3, Words, nil); _ -> case I_3 < 0 of true -> lists:nth(erlang:length(Words) + I_3 + 1, Words); _ -> lists:nth(I_3 + 1, Words) end end)),
            I_4 = (I_3 + 1),
