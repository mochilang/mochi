...
{Dict, Dp_2, I_2, N, S, Words_2} = Fun_2(Dict, Dp, I, N, S, Words_2),
    ...
    case (J < I_3) of
        true ->
            case (case erlang:is_map(Dp_3) of true -> maps:get(J, Dp_3, nil); _ -> case J < 0 of true -> lists:nth(erlang:length(Dp_3) + J + 1, Dp_3); _ -> lists:nth(J + 1, Dp_3) end end) of
...
{Dict, Dp_6, I_4, N, S, Words_2} = Fun_4(Dict, Dp_3, I_3, N, S, Words_2),
        (case erlang:is_map(Dp_6) of true -> maps:get(N, Dp_6, nil); _ -> case N < 0 of true -> lists:nth(erlang:length(Dp_6) + N + 1, Dp_6); _ -> lists:nth(N + 1, Dp_6) end end)
    catch {return, Ret} -> Ret end.
