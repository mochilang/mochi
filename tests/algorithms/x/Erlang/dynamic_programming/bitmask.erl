...
count_assignments(Person, Task_performed, Used) ->
    try
        ...
        Total = 0,
        Tasks = (case erlang:is_map(Task_performed) of true -> maps:get(Person, Task_performed, nil); _ -> case Person < 0 of true -> lists:nth(erlang:length(Task_performed) + Person + 1, Task_performed); _ -> lists:nth(Person + 1, Task_performed) end end),
        I = 0,
        Fun = fun Fun_loop(I, Person, Task_performed, Tasks, Total, Used) ->
    case (I < erlang:length(Tasks)) of
        true ->
            T = (case erlang:is_map(Tasks) of true -> maps:get(I, Tasks, nil); _ -> case I < 0 of true -> lists:nth(erlang:length(Tasks) + I + 1, Tasks); _ -> lists:nth(I + 1, Tasks) end end),
