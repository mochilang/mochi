...
neg_pos(Iterlist) ->
    ...
    case (I_7 < length(Iterlist)) of
        true ->
            Iterlist_2 = lists:sublist(Iterlist, I_7) ++ [-(case erlang:is_map(Iterlist) of true -> maps:get(I_7, Iterlist, nil); _ -> lists:nth(I_7 + 1, Iterlist) end)] ++ lists:nthtail(I_7 + 1, Iterlist),
            I_8 = (I_7 + 2),
...
passcode_creator() ->
    try
        ...
        Seed = mochi_now(),
        Length = (10 + (Seed rem 11)),
        Password = [],
        I_9 = 0,
        Fun_5 = fun Fun_5_loop(Choices, I_9, Length, Password, Seed) ->
    case (I_9 < Length) of
        true ->
            Seed_2 = (((Seed * 1103515245) + 12345) rem 2147483647),
            Idx = (Seed_2 rem length(Choices)),
            Password_2 = lists:append(Password, [string:substr(Choices, Idx + 1, ((Idx + 1) - Idx))]),
            I_10 = (I_9 + 1),
...
unique_sorted(Chars) ->
    ...
    case (I_11 < length(Chars)) of
        true ->
            Ch_2 = (case erlang:is_map(Chars) of true -> maps:get(I_11, Chars, nil); _ -> lists:nth(I_11 + 1, Chars) end),
            case mochi_not(mochi_member(Ch_2, Uniq)) of
        true -> Uniq_2 = lists:append(Uniq, [Ch_2]),
            Uniq_3 = Uniq_2;
        _ -> Uniq_3 = Uniq
    end,
            I_12 = (I_11 + 1),
...
{Chars, I_12, Uniq_3} = Fun_6(Chars, I_11, Uniq),
    ...
    case (J < length(Uniq_3)) of
        true ->
            K = (J + 1),
            Min_idx = J,
            Fun_7 = fun Fun_7_loop(Chars, I_12, J, K, Min_idx, Uniq_3) ->
    case (K < length(Uniq_3)) of
        true ->
            case ((case erlang:is_map(Uniq_3) of true -> maps:get(K, Uniq_3, nil); _ -> lists:nth(K + 1, Uniq_3) end) < (case erlang:is_map(Uniq_3) of true -> maps:get(Min_idx, Uniq_3, nil); _ -> lists:nth(Min_idx + 1, Uniq_3) end)) of
        true -> Min_idx_2 = K,
            Min_idx_3 = Min_idx_2;
        _ -> Min_idx_3 = Min_idx
    end,
...
{Chars, I_12, J, K_2, Min_idx_3, Uniq_3} = Fun_7(Chars, I_12, J, K, Min_idx, Uniq_3),
            case (Min_idx_3 /= J) of
        true -> Tmp = (case erlang:is_map(Uniq_3) of true -> maps:get(J, Uniq_3, nil); _ -> lists:nth(J + 1, Uniq_3) end),
            Uniq_4 = lists:sublist(Uniq_3, J) ++ [(case erlang:is_map(Uniq_3) of true -> maps:get(Min_idx_3, Uniq_3, nil); _ -> lists:nth(Min_idx_3 + 1, Uniq_3) end)] ++ lists:nthtail(J + 1, Uniq_3),
...
make_key_list(Passcode) ->
    ...
    case (I_13 < length(Key_list_options)) of
        true ->
            Ch_3 = string:substr(Key_list_options, I_13 + 1, ((I_13 + 1) - I_13)),
            Temp_list_2 = lists:append(Temp_list, [Ch_3]),
            case (mochi_member(Ch_3, Breakpoints) orelse (I_13 == (length(Key_list_options) - 1))) of
        true -> K_3 = (length(Temp_list_2) - 1),
            Fun_9 = fun Fun_9_loop(Breakpoints, Ch_3, I_13, K_3, Key_list_options, Keys_l, Passcode, Temp_list_2) ->
    case (K_3 >= 0) of
        true ->
            Keys_l_2 = lists:append(Keys_l, [(case erlang:is_map(Temp_list_2) of true -> maps:get(K_3, Temp_list_2, nil); _ -> lists:nth(K_3 + 1, Temp_list_2) end)]),
            K_4 = (K_3 - 1),
...
make_shift_key(Passcode_2) ->
    ...
    case (I_15 < length(Passcode_2)) of
        true ->
            Codes_2 = lists:append(Codes, [ord((case erlang:is_map(Passcode_2) of true -> maps:get(I_15, Passcode_2, nil); _ -> lists:nth(I_15 + 1, Passcode_2) end))]),
            I_16 = (I_15 + 1),
...
{Codes_2, I_16, Passcode_2} = Fun_12(Codes, I_15, Passcode_2),
    ...
    case (I_17 < length(Codes_3)) of
        true ->
            Total_2 = (Total + (case erlang:is_map(Codes_3) of true -> maps:get(I_17, Codes_3, nil); _ -> lists:nth(I_17 + 1, Codes_3) end)),
            I_18 = (I_17 + 1),
...
new_cipher(Passcode_str) ->
    ...
    case (I_19 < length(Passcode_str)) of
        true ->
            Passcode_5 = lists:append(Passcode_3, [string:substr(Passcode_str, I_19 + 1, ((I_19 + 1) - I_19))]),
            I_20 = (I_19 + 1),
...
index_of(Lst, Ch_4) ->
    ...
    case (I_22 < length(Lst)) of
        true ->
            case ((case erlang:is_map(Lst) of true -> maps:get(I_22, Lst, nil); _ -> lists:nth(I_22 + 1, Lst) end) == Ch_4) of
...
encrypt(C, Plaintext) ->
    try
        Encoded = "",
        I_24 = 0,
        N = length(maps:get("key_list", C, nil)),
        Fun_17 = fun Fun_17_loop(C, Encoded, I_24, N, Plaintext) ->
    case (I_24 < length(Plaintext)) of
        true ->
            Ch_5 = string:substr(Plaintext, I_24 + 1, ((I_24 + 1) - I_24)),
            Position = index_of(maps:get("key_list", C, nil), Ch_5),
            New_pos = ((Position + maps:get("shift_key", C, nil)) rem N),
            Encoded_2 = (Encoded ++ (case erlang:is_map(maps:get("key_list", C, nil)) of true -> maps:get(New_pos, maps:get("key_list", C, nil), nil); _ -> lists:nth(New_pos + 1, maps:get("key_list", C, nil)) end)),
            I_25 = (I_24 + 1),
...
decrypt(C_2, Encoded_message) ->
    try
        Decoded = "",
        I_26 = 0,
        N_2 = length(maps:get("key_list", C_2, nil)),
        Fun_18 = fun Fun_18_loop(C_2, Decoded, Encoded_message, I_26, N_2) ->
    case (I_26 < length(Encoded_message)) of
        true ->
            Ch_6 = string:substr(Encoded_message, I_26 + 1, ((I_26 + 1) - I_26)),
            Position_2 = index_of(maps:get("key_list", C_2, nil), Ch_6),
            New_pos_2 = ((Position_2 - maps:get("shift_key", C_2, nil)) rem N_2),
            case (New_pos_2 < 0) of
        true -> New_pos_3 = (New_pos_2 + N_2),
            New_pos_4 = New_pos_3;
        _ -> New_pos_4 = New_pos_2
    end,
            Decoded_2 = (Decoded ++ (case erlang:is_map(maps:get("key_list", C_2, nil)) of true -> maps:get(New_pos_4, maps:get("key_list", C_2, nil), nil); _ -> lists:nth(New_pos_4 + 1, maps:get("key_list", C_2, nil)) end)),
            I_27 = (I_26 + 1),
