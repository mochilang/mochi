...
next_seed(X) ->
    try
        (((X * 1103515245) + 12345) rem 2147483648)
    catch {return, Ret} -> Ret end.

rand_range(Min, Max) ->
    try
        erlang:put('seed', next_seed(erlang:get('seed'))),
        (Min + (erlang:get('seed') rem (Max - Min)))
    catch {return, Ret} -> Ret end.
...
main(_) ->
    ...
    io:format("~ts~n", [mochi_repr((((("Public key: (" ++ mochi_str((case erlang:is_map(erlang:get('pub')) of true -> maps:get(0, erlang:get('pub'), nil); _ -> lists:nth(0 + 1, erlang:get('pub')) end))) ++ ", ") ++ mochi_str((case erlang:is_map(erlang:get('pub')) of true -> maps:get(1, erlang:get('pub'), nil); _ -> lists:nth(1 + 1, erlang:get('pub')) end))) ++ ")"))]),
    io:format("~ts~n", [mochi_repr((((("Private key: (" ++ mochi_str((case erlang:is_map(erlang:get('priv')) of true -> maps:get(0, erlang:get('priv'), nil); _ -> lists:nth(0 + 1, erlang:get('priv')) end))) ++ ", ") ++ mochi_str((case erlang:is_map(erlang:get('priv')) of true -> maps:get(1, erlang:get('priv'), nil); _ -> lists:nth(1 + 1, erlang:get('priv')) end))) ++ ")"))]),
