...
rand(Max) ->
    try
        erlang:put('seed', (((erlang:get('seed') * 1103515245) + 12345) rem 2147483647)),
...
generate_valid_block_size(Message_length) ->
    ...
    case (I =< Message_length) of
        true ->
            case ((Message_length rem I) == 0) of
        true -> Factors_2 = lists:append(Factors, [I]),
            Factors_3 = Factors_2;
        _ -> Factors_3 = Factors
    end,
            I_2 = (I + 1),
...
{Factors_3, I_2, Message_length} = Fun(Factors, I, Message_length),
        Idx = rand(length(Factors_3)),
        (case erlang:is_map(Factors_3) of true -> maps:get(Idx, Factors_3, nil); _ -> lists:nth(Idx + 1, Factors_3) end)
    catch {return, Ret} -> Ret end.
...
generate_permutation_key(Block_size) ->
    ...
    case (I_3 < Block_size) of
        true ->
            Digits_2 = lists:append(Digits, [I_3]),
            I_4 = (I_3 + 1),
...
{Block_size, Digits_2, I_4} = Fun_2(Block_size, Digits, I_3),
    ...
    case (J > 0) of
        true ->
            K = rand((J + 1)),
            Temp = (case erlang:is_map(Digits_2) of true -> maps:get(J, Digits_2, nil); _ -> lists:nth(J + 1, Digits_2) end),
            Digits_3 = lists:sublist(Digits_2, J) ++ [(case erlang:is_map(Digits_2) of true -> maps:get(K, Digits_2, nil); _ -> lists:nth(K + 1, Digits_2) end)] ++ lists:nthtail(J + 1, Digits_2),
...
encrypt(Message, Key, Block_size_2) ->
    ...
    case (J_3 < Block_size_2) of
        true ->
            Encrypted_2 = (Encrypted ++ string:substr(Block, (case erlang:is_map(Key) of true -> maps:get(J_3, Key, nil); _ -> lists:nth(J_3 + 1, Key) end) + 1, (((case erlang:is_map(Key) of true -> maps:get(J_3, Key, nil); _ -> lists:nth(J_3 + 1, Key) end) + 1) - (case erlang:is_map(Key) of true -> maps:get(J_3, Key, nil); _ -> lists:nth(J_3 + 1, Key) end)))),
            J_4 = (J_3 + 1),
...
decrypt(Encrypted_3, Key_2) ->
    ...
    case (I_9 < length(Encrypted_3)) of
        true ->
    ...
    case (J_5 < Klen) of
        true ->
            Original_2 = lists:sublist(Original, (case erlang:is_map(Key_2) of true -> maps:get(J_5, Key_2, nil); _ -> lists:nth(J_5 + 1, Key_2) end)) ++ [string:substr(Block_2, J_5 + 1, ((J_5 + 1) - J_5))] ++ lists:nthtail((case erlang:is_map(Key_2) of true -> maps:get(J_5, Key_2, nil); _ -> lists:nth(J_5 + 1, Key_2) end) + 1, Original),
            J_6 = (J_5 + 1),
...
{Block_2, Decrypted, Encrypted_3, I_9, J_6, Key_2, Klen, Original_2} = Fun_7(Block_2, Decrypted, Encrypted_3, I_9, J_5, Key_2, Klen, Original),
    ...
    case (J_7 < Klen) of
        true ->
            Decrypted_2 = (Decrypted ++ (case erlang:is_map(Original_2) of true -> maps:get(J_7, Original_2, nil); _ -> lists:nth(J_7 + 1, Original_2) end)),
            J_8 = (J_7 + 1),
...
main(_) ->
    ...
    erlang:put('block_size', generate_valid_block_size(length("HELLO WORLD"))),
