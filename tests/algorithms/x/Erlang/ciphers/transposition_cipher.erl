...
join_strings(Xs) ->
    ...
    case (I < length(Xs)) of
        true ->
            Res_2 = (Res ++ (case erlang:is_map(Xs) of true -> maps:get(I, Xs, nil); _ -> lists:nth(I + 1, Xs) end)),
            I_2 = (I + 1),
...
encrypt_message(Key, Message) ->
    ...
    case (Pointer < length(Message)) of
        true ->
...
decrypt_message(Key_2, Message_2) ->
    try
        Num_cols = (((length(Message_2) + Key_2) - 1) div Key_2),
        Num_rows = Key_2,
        Num_shaded_boxes = ((Num_cols * Num_rows) - length(Message_2)),
        Plain_text = [],
        I_3 = 0,
        Fun_4 = fun Fun_4_loop(I_3, Key_2, Message_2, Num_cols, Num_rows, Num_shaded_boxes, Plain_text) ->
    case (I_3 < Num_cols) of
        true ->
            Plain_text_2 = lists:append(Plain_text, [""]),
            I_4 = (I_3 + 1),
...
{I_4, Key_2, Message_2, Num_cols, Num_rows, Num_shaded_boxes, Plain_text_2} = Fun_4(I_3, Key_2, Message_2, Num_cols, Num_rows, Num_shaded_boxes, Plain_text),
    ...
    case (Index < length(Message_2)) of
        true ->
            Plain_text_3 = lists:sublist(Plain_text_2, Col_3) ++ [((case erlang:is_map(Plain_text_2) of true -> maps:get(Col_3, Plain_text_2, nil); _ -> lists:nth(Col_3 + 1, Plain_text_2) end) ++ string:substr(Message_2, Index + 1, ((Index + 1) - Index)))] ++ lists:nthtail(Col_3 + 1, Plain_text_2),
            Col_4 = (Col_3 + 1),
