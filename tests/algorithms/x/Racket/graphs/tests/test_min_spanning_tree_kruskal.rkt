...
(define e 0)
(let/ec _break (let loop ()
  (if (< e (cond [(not sorted) 0] [(string? sorted) (string-length sorted)] [(hash? sorted) (hash-count sorted)] [else (length sorted)])) (let/ec _cont
    (if (equal? (cond [(not mst) 0] [(string? mst) (string-length mst)] [(hash? mst) (hash-count mst)] [else (length mst)]) (- n 1)) (let ()
(_break)
) (void))
    (define edge (if sorted (if (hash? sorted) (hash-ref sorted e #f) (list-ref sorted (int e))) #f))
    (set! e (let ([__l e] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (define u (if edge (if (hash? edge) (hash-ref edge 0 #f) (list-ref edge (int 0))) #f))
    (define v (if edge (if (hash? edge) (hash-ref edge 1 #f) (list-ref edge (int 1))) #f))
    (define w (if edge (if (hash? edge) (hash-ref edge 2 #f) (list-ref edge (int 2))) #f))
    (define ru (find parent u))
    (define rv (find parent v))
    (if (not (equal? ru rv)) (let ()
(set! parent (list-set parent (int ru) rv))
(set! mst (append mst (list (list u v w))))
) (void))
    (loop)) (void))))
(_return mst)
))
...
(let/ec _break (let loop ()
  (if (< i (cond [(not a) 0] [(string? a) (string-length a)] [(hash? a) (hash-count a)] [else (length a)])) (let/ec _cont
    (define e1 (if a (if (hash? a) (hash-ref a i #f) (list-ref a (int i))) #f))
    (define e2 (if b (if (hash? b) (hash-ref b i #f) (list-ref b (int i))) #f))
    (if (or (or (not (equal? (if e1 (if (hash? e1) (hash-ref e1 0 #f) (list-ref e1 (int 0))) #f) (if e2 (if (hash? e2) (hash-ref e2 0 #f) (list-ref e2 (int 0))) #f))) (not (equal? (if e1 (if (hash? e1) (hash-ref e1 1 #f) (list-ref e1 (int 1))) #f) (if e2 (if (hash? e2) (hash-ref e2 1 #f) (list-ref e2 (int 1))) #f)))) (not (equal? (if e1 (if (hash? e1) (hash-ref e1 2 #f) (list-ref e1 (int 2))) #f) (if e2 (if (hash? e2) (hash-ref e2 2 #f) (list-ref e2 (int 2))) #f)))) (let ()
(_return #f)
) (void))
    (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
...
(displayln (format "~a" sorted_result))
