;; Generated by Mochi 0.10.60 on 2025-08-07 16:40 +0700
#lang racket/base
...
(define entry (let ([h (make-hash)]) (hash-set! h "edge" e)(hash-set! h "count" (format "~a" cnt))(hash-set! h "bit" bit) h))
...
(let/ec _break (let loop ()
  ...
  (if (< j (cond [(not table) 0] [(string? table) (string-length table)] [(hash? table) (hash-count table)] [else (length table)])) (let/ec _cont
    (if (> (toi (if (if table (if (hash? table) (hash-ref table j #f) (list-ref table (int j))) #f) (hash-ref (if table (if (hash? table) (hash-ref table j #f) (list-ref table (int j))) #f) "count" "") #f)) (toi (if (if table (if (hash? table) (hash-ref table max_i #f) (list-ref table (int max_i))) #f) (hash-ref (if table (if (hash? table) (hash-ref table max_i #f) (list-ref table (int max_i))) #f) "count" "") #f))) (let ()
(set! max_i j)
) (void))
...
(when (not nodes) (set! nodes (make-hash)))
(hash-set! nodes code (append (if nodes (hash-ref nodes code (list)) #f) (list edge)))
) (let ()
(when (not nodes) (set! nodes (make-hash)))
(hash-set! nodes code (list edge))
...
(when (not clusters) (set! clusters (make-hash)))
(hash-set! clusters wt (append (if clusters (hash-ref clusters wt (list)) #f) (list code)))
) (let ()
(when (not clusters) (set! clusters (make-hash)))
(hash-set! clusters wt (list code))
...
(when (not graph) (set! graph (make-hash)))
(hash-set! graph i_code (append (if graph (hash-ref graph i_code (list)) #f) (list j_code)))
) (let ()
(when (not graph) (set! graph (make-hash)))
(hash-set! graph i_code (list j_code))
...
(let/ec _break (let loop ()
  (if (< i (cond [(not top_codes) 0] [(string? top_codes) (string-length top_codes)] [(hash? top_codes) (hash-count top_codes)] [else (length top_codes)])) (let/ec _cont
    ...
    (when (not graph) (set! graph (make-hash)))
(hash-set! graph "Header" (append (if graph (hash-ref graph "Header" (list)) #f) (list code)))
    (when (not graph) (set! graph (make-hash)))
(hash-set! graph code (list "Header"))
    (set! keys (append keys (list code)))
    (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
(define c 1)
