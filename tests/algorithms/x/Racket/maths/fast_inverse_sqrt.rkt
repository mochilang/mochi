...
(define exp (log2_floor num))
(define pow (pow2_float exp))
(define normalized (/ num pow))
(define frac (- normalized 1.0))
(define mantissa (int (* frac (pow2_float 23))))
(define exp_bits (let ([__l exp] [__r 127]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
...
(_return (* (* sign mantissa) (pow2_float exp)))
))
)
(define (absf x)
  (let/ec _return (begin
...
(define (is_close a b rel_tol)
  (let/ec _return (begin
(_return (<= (absf (- a b)) (* rel_tol (absf b))))
))
)
...
(define (test_fast_inverse_sqrt)
  (let/ec _return (begin
(if (> (absf (- (fast_inverse_sqrt 10.0) 0.3156857923527257)) 0.0001) (let ()
(panic "fast_inverse_sqrt(10) failed")
) (void))
(if (> (absf (- (fast_inverse_sqrt 4.0) 0.49915357479239103)) 0.0001) (let ()
(panic "fast_inverse_sqrt(4) failed")
) (void))
(if (> (absf (- (fast_inverse_sqrt 4.1) 0.4932849504615651)) 0.0001) (let ()
(panic "fast_inverse_sqrt(4.1) failed")
) (void))
(define i 50)
(let/ec _break (let loop ()
  (if (< i 60) (let/ec _cont
...
(let/ec _break (let loop ()
  (if (<= i 100) (let/ec _cont
    ...
    (displayln (string-append (string-append (format "~a" i) ": ") (format "~a" diff)))
