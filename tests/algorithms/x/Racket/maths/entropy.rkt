...
(define total1 0)
(let/ec _break (for ([ch (in-hash-keys (if counts (hash-ref counts "single" #f) #f))])
  (let/ec _cont
(set! total1 (let ([__l total1] [__r (if (if counts (hash-ref counts "single" #f) #f) (if (hash? (if counts (hash-ref counts "single" #f) #f)) (hash-ref (if counts (hash-ref counts "single" #f) #f) ch #f) (list-ref (if counts (hash-ref counts "single" #f) #f) (int ch))) #f)]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
  )))
(define h1 0.0)
(define i 0)
(let/ec _break (let loop ()
  (if (< i (cond [(not alphas) 0] [(string? alphas) (string-length alphas)] [(hash? alphas) (hash-count alphas)] [else (length alphas)])) (let/ec _cont
...
(define prob (/ (exact->inexact (if (if counts (hash-ref counts "single" #f) #f) (if (hash? (if counts (hash-ref counts "single" #f) #f)) (hash-ref (if counts (hash-ref counts "single" #f) #f) ch #f) (list-ref (if counts (hash-ref counts "single" #f) #f) (int ch))) #f)) (exact->inexact total1)))
(set! h1 (let ([__l h1] [__r (* prob (log2 prob))]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
) (void))
    (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
(define first_entropy (- h1))
(displayln (string-append (format "~a" (round_to_int first_entropy)) ".0"))
(define total2 0)
(let/ec _break (for ([seq (in-hash-keys (if counts (hash-ref counts "double" #f) #f))])
  (let/ec _cont
(set! total2 (let ([__l total2] [__r (if (if counts (hash-ref counts "double" #f) #f) (if (hash? (if counts (hash-ref counts "double" #f) #f)) (hash-ref (if counts (hash-ref counts "double" #f) #f) seq #f) (list-ref (if counts (hash-ref counts "double" #f) #f) (int seq))) #f)]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
  )))
(define h2 0.0)
(define a0 0)
(let/ec _break (let loop ()
  (if (< a0 (cond [(not alphas) 0] [(string? alphas) (string-length alphas)] [(hash? alphas) (hash-count alphas)] [else (length alphas)])) (let/ec _cont
    (define ch0 (slice alphas a0 (let ([__l a0] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))))
    (define a1 0)
    (let/ec _break (let loop ()
  (if (< a1 (cond [(not alphas) 0] [(string? alphas) (string-length alphas)] [(hash? alphas) (hash-count alphas)] [else (length alphas)])) (let/ec _cont
...
(define prob (/ (exact->inexact (if (if counts (hash-ref counts "double" #f) #f) (if (hash? (if counts (hash-ref counts "double" #f) #f)) (hash-ref (if counts (hash-ref counts "double" #f) #f) seq #f) (list-ref (if counts (hash-ref counts "double" #f) #f) (int seq))) #f)) (exact->inexact total2)))
...
(displayln (string-append (format "~a" (round_to_int second_entropy)) ".0"))
(define diff (- second_entropy first_entropy))
(displayln (string-append (format "~a" (round_to_int diff)) ".0"))
))
)
