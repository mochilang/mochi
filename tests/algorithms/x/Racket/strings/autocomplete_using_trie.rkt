...
(define words (list "depart" "detergent" "daring" "dog" "deer" "deal"))
(define (autocomplete_using_trie prefix)
...
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not words) 0] [(string? words) (string-length words)] [(hash? words) (hash-count words)] [else (length words)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      ...
      (if (string=? (slice w 0 (cond [(not prefix) 0] [(string? prefix) (string-length prefix)] [(hash? prefix) (hash-count prefix)] [else (length prefix)])) prefix) (let ()
(set! result (append (or result (list)) (list (string-append w " "))))
) (void))
...
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (autocomplete_using_trie "de")))
    (void)
  ))
