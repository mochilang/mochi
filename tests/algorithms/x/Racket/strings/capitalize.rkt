...
(define (capitalize sentence)
  (let/ec _return (begin
(if (equal? (cond [(not sentence) 0] [(string? sentence) (string-length sentence)] [(hash? sentence) (hash-count sentence)] [else (length sentence)]) 0) (let ()
(_return "")
) (void))
(define first (slice sentence 0 1))
(define idx (index_of lowercase first))
(define capital (if (let ([__l idx] [__r 0]) (cond [(and (number? __l) (number? __r)) (>= __l __r)] [else (>= (int __l) (int __r))])) (slice uppercase idx (let ([__l idx] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))) first))
(_return (let ([__l capital] [__r (slice sentence 1 (cond [(not sentence) 0] [(string? sentence) (string-length sentence)] [(hash? sentence) (hash-count sentence)] [else (length sentence)]))]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
))
)
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (capitalize "hello world"))
(displayln (capitalize "123 hello world"))
(displayln (capitalize " hello world"))
(displayln (capitalize "a"))
(displayln (capitalize ""))
    (void)
  ))
