...
(let/ec _break (let loop ()
  (if (let ([__l i] [__r (cond [(not vowels) 0] [(string? vowels) (string-length vowels)] [(hash? vowels) (hash-count vowels)] [else (length vowels)])]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (begin
    (let/ec _cont
      (if (string=? (if (hash? vowels) (hash-ref vowels i #f) (substring vowels (int i) (+ (int i) 1))) c) (let ()
(_return #t)
) (void))
...
(define (show s)
  (let/ec _return (begin
(displayln (to-string (count_vowels s)))
))
)
