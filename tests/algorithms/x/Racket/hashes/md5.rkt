...
(let/ec _break (let loop ()
  (if (> num 0) (let/ec _cont
    (set! bits (string-append (format "~a" (modulo num 2)) bits))
    (set! num (floor (/ num 2)))
    (loop)) (void))))
(let/ec _break (let loop ()
  (if (< (cond [(not bits) 0] [(string? bits) (string-length bits)] [(hash? bits) (hash-count bits)] [else (length bits)]) width) (let/ec _cont
    (set! bits (string-append "0" bits))
    (loop)) (void))))
(if (> (cond [(not bits) 0] [(string? bits) (string-length bits)] [(hash? bits) (hash-count bits)] [else (length bits)]) width) (let ()
...
(define hex (to_hex i))
(let/ec _break (let loop ()
  (if (< (cond [(not hex) 0] [(string? hex) (string-length hex)] [(hash? hex) (hash-count hex)] [else (length hex)]) 8) (let/ec _cont
    (set! hex (string-append "0" hex))
    (loop)) (void))))
(if (> (cond [(not hex) 0] [(string? hex) (string-length hex)] [(hash? hex) (hash-count hex)] [else (length hex)]) 8) (let ()
...
(let/ec _break (let loop ()
  (if (not (equal? (modulo (cond [(not bit_string) 0] [(string? bit_string) (string-length bit_string)] [(hash? bit_string) (hash-count bit_string)] [else (length bit_string)]) 512) 448)) (let/ec _cont
...
(define pos 0)
(let/ec _break (let loop ()
  (if (< pos (cond [(not bit_string) 0] [(string? bit_string) (string-length bit_string)] [(hash? bit_string) (hash-count bit_string)] [else (length bit_string)])) (let/ec _cont
    (define block (list))
    (define i 0)
    (let/ec _break (let loop ()
  (if (< i 512) (let/ec _cont
    ...
    (set! block (append block (list word)))
...
(define bi 0)
(let/ec _break (let loop ()
  (if (< bi (cond [(not blocks) 0] [(string? blocks) (string-length blocks)] [(hash? blocks) (hash-count blocks)] [else (length blocks)])) (let/ec _cont
    (define block (if blocks (if (hash? blocks) (hash-ref blocks bi #f) (list-ref blocks (int bi))) #f))
    (define a a0)
    (define b b0)
    (define c c0)
    (define d d0)
...
(set! g (modulo (* 7 i) 16))
    ...
    (set! f (sum_32 f a))
    (set! f (sum_32 f (if added_consts (if (hash? added_consts) (hash-ref added_consts i #f) (list-ref added_consts (int i))) #f)))
    (set! f (sum_32 f (if block (if (hash? block) (hash-ref block g #f) (list-ref block (int g))) #f)))
    (define rotated (left_rotate_32 f (if shift_amounts (if (hash? shift_amounts) (hash-ref shift_amounts i #f) (list-ref shift_amounts (int i))) #f)))
