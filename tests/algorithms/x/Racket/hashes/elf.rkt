...
(set! res (let ([__l res] [__r bit]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    ...
    (set! ua (int (floor (/ ua 2))))
    (set! ub (int (floor (/ ub 2))))
    (set! bit (* bit 2))
    (loop)) (void))))
(_return res)
))
...
(define count 0)
(let/ec _break (let loop ()
  (if (< count 32) (let/ec _cont
    (if (equal? (modulo ux 2) 0) (let ()
(set! res (let ([__l res] [__r bit]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
) (void))
    (set! ux (int (floor (/ ux 2))))
    (set! bit (* bit 2))
...
(define (elf_hash data)
  (let/ec _return (begin
(define hash_ 0)
(define i 0)
(let/ec _break (let loop ()
  (if (< i (cond [(not data) 0] [(string? data) (string-length data)] [(hash? data) (hash-count data)] [else (length data)])) (let/ec _cont
    (define c (ord (slice data i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))))
    (set! hash_ (let ([__l (* hash_ 16)] [__r c]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (define x (bit_and hash_ 4026531840))
    (if (not (equal? x 0)) (let ()
(set! hash_ (bit_xor hash_ (int (floor (/ x 16777216)))))
) (void))
    (set! hash_ (bit_and hash_ (bit_not32 x)))
    (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    (loop)) (void))))
(_return hash_)
))
...
(let* ([_start_mem (current-memory-use)] [_start (now)])
  (let/ec _return (begin
(displayln (format "~a" (elf_hash "lorem ipsum")))
    (void)
  ))
