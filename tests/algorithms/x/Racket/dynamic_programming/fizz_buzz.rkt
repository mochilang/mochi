...
(define (fizz_buzz number iterations)
...
(let/ec _break (let loop ()
  (if (<= n iterations) (let/ec _cont
    (if (equal? (modulo n 3) 0) (let ()
(set! out (string-append out "Fizz"))
) (void))
    (if (equal? (modulo n 5) 0) (let ()
(set! out (string-append out "Buzz"))
) (void))
    (if (and (not (equal? (modulo n 3) 0)) (not (equal? (modulo n 5) 0))) (let ()
(set! out (string-append out (format "~a" n)))
) (void))
...
(let* ([_start_mem (current-memory-use)] [_start (now)])
  (let/ec _return (begin
(displayln (fizz_buzz 1 7))
(displayln (fizz_buzz 1 15))
    (void)
  ))
