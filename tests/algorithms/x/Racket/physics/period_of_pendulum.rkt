...
(define (period_of_pendulum length_)
  (let/ec _return (begin
(if (< length_ 0.0) (let ()
(panic "The length should be non-negative")
) (void))
(_return (* (* 2.0 PI) (sqrt (/ length_ G))))
))
)
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (period_of_pendulum 1.23)))
(displayln (to-string (period_of_pendulum 2.37)))
(displayln (to-string (period_of_pendulum 5.63)))
(displayln (to-string (period_of_pendulum 0.0)))
    (void)
  ))
