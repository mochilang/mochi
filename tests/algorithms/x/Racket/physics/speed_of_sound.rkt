...
(define (speed_of_sound_in_a_fluid density bulk_modulus)
  (let/ec _return (begin
(if (<= density 0.0) (let ()
(panic "Impossible fluid density")
) (void))
(if (<= bulk_modulus 0.0) (let ()
(panic "Impossible bulk modulus")
) (void))
(_return (sqrtApprox (/ bulk_modulus density)))
))
)
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (speed_of_sound_in_a_fluid 998.0 2150000000.0)))
(displayln (to-string (speed_of_sound_in_a_fluid 13600.0 28500000000.0)))
    (void)
  ))
