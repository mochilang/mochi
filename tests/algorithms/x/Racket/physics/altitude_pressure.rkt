...
(define (pow_float base exponent)
  (let/ec _return (begin
(_return (exp (* exponent (ln base))))
))
)
(define (get_altitude_at_pressure pressure)
  (let/ec _return (begin
(if (let ([__l pressure] [__r 101325.0]) (cond [(and (number? __l) (number? __r)) (> __l __r)] [else (> (int __l) (int __r))])) (let ()
(panic "Value Higher than Pressure at Sea Level !")
) (void))
(if (let ([__l pressure] [__r 0.0]) (cond [(and (number? __l) (number? __r)) (< __l __r)] [else (< (int __l) (int __r))])) (let ()
(panic "Atmospheric Pressure can not be negative !")
) (void))
(define ratio (/ pressure 101325.0))
(_return (* 44330.0 (- 1.0 (pow_float ratio (/ 1.0 5.5255)))))
))
)
(let* ([_start_mem (current-memory-use)] [_start (current-inexact-monotonic-milliseconds)])
  (let/ec _return (begin
(displayln (to-string (get_altitude_at_pressure 100000.0)))
(displayln (to-string (get_altitude_at_pressure 101325.0)))
(displayln (to-string (get_altitude_at_pressure 80000.0)))
    (void)
  ))
