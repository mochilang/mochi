...
(define x n)
(if (equal? x 0) (let ()
(set! counts (list-set counts (int 0) (let ([__l (if counts (if (hash? counts) (hash-ref counts 0 #f) (safe-index counts (int 0))) #f)] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r)))))
) (void))
(let/ec _break (let loop ()
  (if (let ([__l x] [__r 0]) (> (int __l) (int __r))) (begin
    (let/ec _cont
      (define d (modulo x 10))
...
(let/ec _break (let loop ()
  (if #t (begin
    (let/ec _cont
      (define c (digits_count i))
      (if (and (and (and (and (equal_lists c (digits_count (* 2 i))) (equal_lists c (digits_count (* 3 i)))) (equal_lists c (digits_count (* 4 i)))) (equal_lists c (digits_count (* 5 i)))) (equal_lists c (digits_count (* 6 i)))) (let ()
(_return i)
) (void))
      (set! i (let ([__l i] [__r 1]) (if (and (string? __l) (string? __r)) (string-append __l __r) (+ __l __r))))
    )
