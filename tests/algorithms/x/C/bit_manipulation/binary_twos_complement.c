...
const char* twos_complement(long long number) {
    ...
    long long binary_number_length = (to_binary(abs_number), to_binary_len);
    long long complement_value = pow2(binary_number_length) - abs_number;
    const char *complement_binary = to_binary(complement_value);
    size_t complement_binary_len = complement_binary ? strlen(complement_binary) : 0;
    const char *padding = repeat_char("0", binary_number_length - complement_binary_len);
    size_t padding_len = padding ? strlen(padding) : 0;
    const char* twos_complement_number = str_concat(str_concat("1", str_int(padding)), str_int(complement_binary));
