...
long long N_STEPS = 1000LL;

double user_floor(double x);
double user_pow10(long long n);
double user_round(double x, long long n);
double simpson_integration(double(*f)(double), double a, double b, long long precision);
...
double simpson_integration(double(*f)(double), double a, double b, long long precision) {
    ...
    result = result * (h / 3.0);
    double r = user_round(result, precision);
    return r;
}
...
int main(void) {
    {
        ...
        puts(str_float(simpson_integration(square, 1.0, 2.0, 3LL)));
        puts(str_float(simpson_integration(square, 3.45, 3.2, 1LL)));
