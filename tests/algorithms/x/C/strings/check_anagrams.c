...
long long check_anagrams(const char* a, const char* b) {
    ...
    static const char* count_keys[16] = {};
    int count_vals[16] = {};
    size_t count_len = 0;
    MapSI count = { count_keys, count_vals, 0, 16 };
    long long i = 0LL;
    while (i < strlen(s1)) {
        const char* c1 = (const char[]){s1[i], 0};
        const char* c2 = (const char[]){s2[i], 0};
        if (contains_str(count.keys, count.len, c1)) {
            map_set_si(&count, c1, map_get_si(count.keys, count.vals, count.len, c1) + 1LL);
        } else {
            map_set_si(&count, c1, 1LL);
        }
        if (contains_str(count.keys, count.len, c2)) {
            map_set_si(&count, c2, map_get_si(count.keys, count.vals, count.len, c2) - 1LL);
        } else {
            map_set_si(&count, c2, -1LL);
        }
        i = i + 1LL;
    }
    for (int ch = 0; ch < 0; ch++) {
        if (map_get_si(count.keys, count.vals, count.len, ch) != 0LL) {
            return 0LL;
        }
    }
    return 1LL;
