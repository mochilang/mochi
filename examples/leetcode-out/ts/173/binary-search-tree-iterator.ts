...
function example(): void {
  ...
  if (!(bstNext(iterator) == 3)) {
    throw new Error("expect failed");
  }
  if (!(bstNext(iterator) == 7)) {
    throw new Error("expect failed");
  }
  if (!(bstHasNext(iterator) == true)) {
    throw new Error("expect failed");
  }
  if (!(bstNext(iterator) == 9)) {
    throw new Error("expect failed");
  }
  if (!(bstHasNext(iterator) == true)) {
    throw new Error("expect failed");
  }
  if (!(bstNext(iterator) == 15)) {
    throw new Error("expect failed");
  }
  if (!(bstHasNext(iterator) == true)) {
    throw new Error("expect failed");
  }
  if (!(bstNext(iterator) == 20)) {
    throw new Error("expect failed");
  }
  if (!(bstHasNext(iterator) == false)) {
    throw new Error("expect failed");
  }
}

function single_node(): void {
  let it: Record<string, any> = NewBSTIterator(Node(Leaf(), 5, Leaf()));
  if (!(bstNext(it) == 5)) {
    throw new Error("expect failed");
  }
  if (!(bstHasNext(it) == false)) {
    throw new Error("expect failed");
  }
}
