...
function getSkyline(buildings: Array<Array<number>>) : Array<Array<number>> {
	...
	for (const edge of sorted) {
		...
		if ((h < 0)) {
			...
			if ((idx >= 0)) {
				heights = heights.slice(0, idx).concat(heights.slice((idx + 1), heights.length))
			}
		}
		let curr: number = 0
...
function example_1(): void {
	if (!(_equal(getSkyline([[2, 9, 10], [3, 7, 15], [5, 12, 12], [15, 20, 10], [19, 24, 8]]), [[2, 10], [3, 15], [7, 12], [12, 0], [15, 10], [20, 8], [24, 0]]))) { throw new Error('expect failed') }
}

function example_2(): void {
	if (!(_equal(getSkyline([[0, 2, 3], [2, 5, 3]]), [[0, 3], [5, 0]]))) { throw new Error('expect failed') }
}

function single_building(): void {
	if (!(_equal(getSkyline([[1, 5, 4]]), [[1, 4], [5, 0]]))) { throw new Error('expect failed') }
}

function overlap(): void {
	if (!(_equal(getSkyline([[1, 3, 3], [1, 4, 2], [3, 4, 1]]), [[1, 3], [3, 2], [4, 0]]))) { throw new Error('expect failed') }
}
