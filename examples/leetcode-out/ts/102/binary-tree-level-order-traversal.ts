...
function levelOrder(root: Tree) : Array<Array<number>> {
	if ((() => {
	const _t = root;
...
function single_node(): void {
	if (!(_equal(levelOrder({__name: "Node", left: {__name: "Leaf"}, value: 1, right: {__name: "Leaf"}}), [[1]]))) { throw new Error('expect failed') }
}

function empty(): void {
	if (!(_equal(levelOrder({__name: "Leaf"}), []))) { throw new Error('expect failed') }
}
