...
function example_1(): void {
  if (!(largestBSTSubtree(example1) == 3)) {
    throw new Error("expect failed");
  }
}

function single_node(): void {
  if (!(largestBSTSubtree(Node(Leaf(), 1, Leaf())) == 1)) {
    throw new Error("expect failed");
  }
}

function already_bst(): void {
  let tree: Record<string, any> = Node(
    Node(Leaf(), 2, Leaf()),
    3,
    Node(Leaf(), 4, Node(Leaf(), 5, Leaf())),
  );
  if (!(largestBSTSubtree(tree) == 4)) {
    throw new Error("expect failed");
  }
}

function empty(): void {
  if (!(largestBSTSubtree(Leaf()) == 0)) {
    throw new Error("expect failed");
  }
}
