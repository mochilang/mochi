...
function topKFrequent(nums: Array<number>, k: number) : Array<number> {
	...
	for (const keyKey of Object.keys(counts)) {
		...
		pairs = pairs.concat([[key, counts[key]]])
	}
	...
	let _pairs = _items.map(it => { const p = it; return {item: it, key: (-p[1])}; });
	_pairs.sort((a, b) => {
	...
	while (((i < sorted.length) && (i < k))) {
		result = result.concat([(sorted[i] as any)[0]])
		i = (i + 1)
	}
	return result
}
...
function example_1(): void {
	if (!(_equal(topKFrequent([1, 1, 1, 2, 2, 3], 2), [1, 2]))) { throw new Error('expect failed') }
}
