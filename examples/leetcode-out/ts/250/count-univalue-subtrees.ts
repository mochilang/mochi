...
let example1: Record<string, any> = Node(
  Node(Node(Leaf(), 5, Leaf()), 1, Node(Leaf(), 5, Leaf())),
  5,
  Node(Leaf(), 5, Node(Leaf(), 5, Leaf())),
);
...
function example_1(): void {
  if (!(countUnivalSubtrees(example1) == 4)) {
    throw new Error("expect failed");
  }
}

function example_2(): void {
  if (!(countUnivalSubtrees(example2) == 6)) {
    throw new Error("expect failed");
  }
}

function single_node(): void {
  if (!(countUnivalSubtrees(Node(Leaf(), 1, Leaf())) == 1)) {
    throw new Error("expect failed");
  }
}

function empty(): void {
  if (!(countUnivalSubtrees(Leaf()) == 0)) {
    throw new Error("expect failed");
  }
}
