...
function newSnakeGame(width: number, height: number, food: Array<Array<number>>) : SnakeGame {
	let start: Array<number> = [0, 0]
	return {width: width, height: height, food: food, index: 0, body: [start], occupied: {[String(start[0]) + "," + String(start[1])]: true}}
}
...
function move(game: SnakeGame, dir: string) : MoveResult {
	...
	let eat: boolean = (((newIndex < game.food.length) && (game.food[newIndex][0] == r)) && (game.food[newIndex][1] == c))
	if ((!eat)) {
	...
	newOcc[key] = true
	let newGame: SnakeGame = {width: game.width, height: game.height, food: game.food, index: newIndex, body: newBody, occupied: newOcc}
	return {game: newGame, score: newIndex}
}

function example(): void {
	let g: SnakeGame = newSnakeGame(3, 2, [[1, 2], [0, 1]])
