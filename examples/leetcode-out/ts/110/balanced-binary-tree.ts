...
function balanced(tree: Array<Node>, idx: number): boolean {
  ...
  return (
    balanced(tree, node.left) && balanced(tree, node.right) && abs(lh - rh) <= 1
  );
}
...
function example_1(): void {
  let tree: Array<any> = [
    ...
    { value: 20, left: 3, right: 4 },
    { value: 15, left: -1, right: -1 },
    { value: 7, left: -1, right: -1 },
  ];
  if (!(isBalanced(tree, 0) == true)) {
