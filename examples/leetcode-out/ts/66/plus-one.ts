...
function example_1(): void {
  if (!_equal(plusOne([1, 2, 3]), [1, 2, 4])) {
    throw new Error("expect failed");
  }
}

function example_2(): void {
  if (!_equal(plusOne([4, 3, 2, 1]), [4, 3, 2, 2])) {
    throw new Error("expect failed");
  }
}

function example_3(): void {
  if (!_equal(plusOne([9]), [1, 0])) {
    throw new Error("expect failed");
  }
}

function carry_through_all_digits(): void {
  if (!_equal(plusOne([9, 9, 9]), [1, 0, 0, 0])) {
    throw new Error("expect failed");
  }
}

function zero(): void {
  if (!_equal(plusOne([0]), [1])) {
    throw new Error("expect failed");
  }
}
