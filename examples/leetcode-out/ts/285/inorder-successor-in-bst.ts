...
let example: Array<any> = [
  { val: 5, left: 1, right: 2 },
  { val: 3, left: 3, right: 4 },
  { val: 6, left: -1, right: -1 },
  { val: 2, left: 5, right: -1 },
  { val: 4, left: -1, right: -1 },
...
function successor_in_middle(): void {
  if (!(inorderSuccessor(example, 0, 1) == 4)) {
...
function left_subtree(): void {
  if (!(inorderSuccessor(example, 0, 3) == 1)) {
    throw new Error("expect failed");
  }
}
...
function single_node(): void {
  ...
  if (!(inorderSuccessor(tree, 0, 0) == -1)) {
    throw new Error("expect failed");
  }
}
