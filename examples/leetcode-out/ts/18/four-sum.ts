// Generated by Mochi TypeScript compiler

function fourSum(nums: Array<number>, target: number) : Array<Array<number>> {
	let sorted: Array<any> = (() => {
	const _src = nums;
	let _items = [];
	for (const n of _src) {
		_items.push(n);
	}
	let _pairs = _items.map(it => { const n = it; return {item: it, key: n}; });
	_pairs.sort((a, b) => {
		const ak = a.key; const bk = b.key;
		if (typeof ak === 'number' && typeof bk === 'number') return ak - bk;
		if (typeof ak === 'string' && typeof bk === 'string') return ak < bk ? -1 : (ak > bk ? 1 : 0);
		return String(ak) < String(bk) ? -1 : (String(ak) > String(bk) ? 1 : 0);
	});
	_items = _pairs.map(p => p.item);
	const _res = [];
	for (const n of _items) {
		_res.push(n)
	}
	return _res;
})()
	let n: number = sorted.length
	let result: Array<Array<number>> = []
	for (let i: number = 0; i < n; i++) {
		if (((i > 0) && _equal(sorted[i], sorted[(i - 1)]))) {
			continue
		}
		for (let j: number = (i + 1); j < n; j++) {
			if (((j > (i + 1)) && _equal(sorted[j], sorted[(j - 1)]))) {
				continue
			}
			let left: number = (j + 1)
			let right: number = (n - 1)
			while ((left < right)) {
				let sum: any = (((sorted[i] + sorted[j]) + sorted[left]) + sorted[right])
				if (_equal(sum, target)) {
					result = result.concat([[sorted[i], sorted[j], sorted[left], sorted[right]]])
					left = (left + 1)
					right = (right - 1)
					while (((left < right) && _equal(sorted[left], sorted[(left - 1)]))) {
						left = (left + 1)
					}
					while (((left < right) && _equal(sorted[right], sorted[(right + 1)]))) {
						right = (right - 1)
					}
				} else 				if ((sum < target)) {
					left = (left + 1)
				} else {
					right = (right - 1)
				}
			}
		}
	}
	return result
}

function example_1(): void {
	if (!(_equal(fourSum([1, 0, (-1), 0, (-2), 2], 0), [[(-2), (-1), 1, 2], [(-2), 0, 0, 2], [(-1), 0, 0, 1]]))) { throw new Error('expect failed') }
}

function example_2(): void {
	if (!(_equal(fourSum([2, 2, 2, 2, 2], 8), [[2, 2, 2, 2]]))) { throw new Error('expect failed') }
}

function main(): void {
	example_1()
	example_2()
}
function _equal(a: any, b: any): boolean {
  if (Array.isArray(a) && Array.isArray(b)) {
    if (a.length !== b.length) return false;
    for (let i = 0; i < a.length; i++) { if (!_equal(a[i], b[i])) return false; }
    return true;
  }
  if (a && b && typeof a === 'object' && typeof b === 'object') {
    const ak = Object.keys(a); const bk = Object.keys(b);
    if (ak.length !== bk.length) return false;
    for (const k of ak) { if (!bk.includes(k) || !_equal((a as any)[k], (b as any)[k])) return false; }
    return true;
  }
  return a === b;
}

main()

