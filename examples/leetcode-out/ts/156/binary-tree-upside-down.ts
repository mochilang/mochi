...
let example: Record<string, any> = Node(Node(Node(Leaf(), 4, Leaf()), 2, Node(Leaf(), 5, Leaf())), 1, Node(Leaf(), 3, Leaf()))
...
function flip_example(): void {
	...
	if (!(_equal(levelOrder(newRoot), [[4], [5, 2], [3, 1]]))) { throw new Error('expect failed') }
}

function single_node(): void {
	let tree: Record<string, any> = Node(Leaf(), 1, Leaf())
	if (!(_equal(levelOrder(upsideDown(tree)), [[1]]))) { throw new Error('expect failed') }
}
