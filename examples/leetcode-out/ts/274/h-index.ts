...
function example_1(): void {
	if (!((hIndex([3, 0, 6, 1, 5]) == 3))) { throw new Error('expect failed') }
}

function example_2(): void {
	if (!((hIndex([1, 3, 1]) == 1))) { throw new Error('expect failed') }
}
...
function all_high(): void {
	if (!((hIndex([10, 8, 5, 4, 3]) == 4))) { throw new Error('expect failed') }
}
