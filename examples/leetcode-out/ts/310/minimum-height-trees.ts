...
function findMinHeightTrees(n: number, edges: Array<Array<number>>) : Array<number> {
	if ((n <= 2)) {
...
function example_1(): void {
	if (!(_equal(findMinHeightTrees(4, [[1, 0], [1, 2], [1, 3]]), [1]))) { throw new Error('expect failed') }
}

function example_2(): void {
	let res: Array<number> = findMinHeightTrees(6, [[3, 0], [3, 1], [3, 2], [3, 4], [5, 4]])
...
function single_node(): void {
	if (!(_equal(findMinHeightTrees(1, []), [0]))) { throw new Error('expect failed') }
}

function line(): void {
	if (!(_equal(findMinHeightTrees(2, [[0, 1]]), [0, 1]))) { throw new Error('expect failed') }
}
