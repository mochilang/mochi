...
let example1: Record<string, any> = Node(Node(Node(Leaf(), 3, Leaf()), 2, Node(Leaf(), 4, Leaf())), 1, Node(Leaf(), 5, Node(Leaf(), 6, Leaf())))

let flattened1: Record<string, any> = Node(Leaf(), 1, Node(Leaf(), 2, Node(Leaf(), 3, Node(Leaf(), 4, Node(Leaf(), 5, Node(Leaf(), 6, Leaf()))))))
...
function example_1(): void {
	if (!((equalTrees(flatten(example1), flattened1) == true))) { throw new Error('expect failed') }
}

function single_node(): void {
	let tree: Record<string, any> = Node(Leaf(), 0, Leaf())
	if (!((equalTrees(flatten(tree), tree) == true))) { throw new Error('expect failed') }
}
