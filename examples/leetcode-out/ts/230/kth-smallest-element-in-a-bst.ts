...
let example2: Record<string, any> = Node(
  Node(Node(Leaf(), 2, Node(Leaf(), 1, Leaf())), 3, Node(Leaf(), 4, Leaf())),
  5,
  Node(Leaf(), 6, Leaf()),
);
...
function example_1(): void {
  if (!(kthSmallest(example1, 1) == 1)) {
    throw new Error("expect failed");
  }
}

function example_2(): void {
  if (!(kthSmallest(example2, 3) == 3)) {
    throw new Error("expect failed");
  }
}

function single_node(): void {
  if (!(kthSmallest(Node(Leaf(), 8, Leaf()), 1) == 8)) {
    throw new Error("expect failed");
  }
}

function k_equals_number_of_nodes(): void {
  if (!(kthSmallest(example1, 4) == 4)) {
    throw new Error("expect failed");
  }
}
