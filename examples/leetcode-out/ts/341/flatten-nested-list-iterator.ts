...
function newNestedIterator(nested: Array<Record<string, any>>) : NestedIterator {
	return {data: flatten(nested), index: 0}
}
...
function nestedNext(it: NestedIterator) : NextResult {
	...
	return {it: {data: it.data, index: (it.index + 1)}, val: value}
}

function example_1(): void {
	let nested: Array<Record<string, any>> = [ListItem([IntItem(1), IntItem(1)]), IntItem(2), ListItem([IntItem(1), IntItem(1)])]
...
function example_2(): void {
	let nested: Array<Record<string, any>> = [IntItem(1), ListItem([IntItem(4), ListItem([IntItem(6)])])]
	let it: NestedIterator = newNestedIterator(nested)
	let r1: NextResult = nestedNext(it)
	it = r1.it
	if (!((r1.val == 1))) { throw new Error('expect failed') }
...
function empty_inner_lists(): void {
	let nested: Array<Record<string, any>> = [ListItem([]), IntItem(3), ListItem([])]
	let it: NestedIterator = newNestedIterator(nested)
