...
function greaterSeq(a: Array<number>, i: number, b: Array<number>, j: number) : boolean {
	let x: number = i
	let y: number = j
...
function maxNumber(nums1: Array<number>, nums2: Array<number>, k: number) : Array<number> {
...
function example_1(): void {
	if (!(_equal(maxNumber([3, 4, 6, 5], [9, 1, 2, 5, 8, 3], 5), [9, 8, 6, 5, 3]))) { throw new Error('expect failed') }
}

function example_2(): void {
	if (!(_equal(maxNumber([6, 7], [6, 0, 4], 5), [6, 7, 6, 0, 4]))) { throw new Error('expect failed') }
}

function example_3(): void {
	if (!(_equal(maxNumber([3, 9], [8, 9], 3), [9, 8, 9]))) { throw new Error('expect failed') }
}

function all_from_one(): void {
	if (!(_equal(maxNumber([5, 9, 1], [], 2), [9, 1]))) { throw new Error('expect failed') }
}
