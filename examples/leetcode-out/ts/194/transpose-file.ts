...
function square_matrix(): void {
	if (!(_equal(transpose([[1, 2, 3], [4, 5, 6], [7, 8, 9]]), [[1, 4, 7], [2, 5, 8], [3, 6, 9]]))) { throw new Error('expect failed') }
}

function rectangular_matrix(): void {
	if (!(_equal(transpose([[1, 2], [3, 4], [5, 6]]), [[1, 3, 5], [2, 4, 6]]))) { throw new Error('expect failed') }
}
