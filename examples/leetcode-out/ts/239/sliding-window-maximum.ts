...
function example_1(): void {
  if (
    !_equal(maxSlidingWindow([1, 3, -1, -3, 5, 3, 6, 7], 3), [3, 3, 5, 5, 6, 7])
  ) {
...
function k_equals_array_length(): void {
  if (!_equal(maxSlidingWindow([2, 1], 2), [2])) {
    throw new Error("expect failed");
  }
}

function all_decreasing(): void {
  if (!_equal(maxSlidingWindow([9, 8, 7, 6, 5], 2), [9, 8, 7, 6])) {
    throw new Error("expect failed");
  }
}

function window_size_1(): void {
  if (!_equal(maxSlidingWindow([4, 2], 1), [4, 2])) {
    throw new Error("expect failed");
  }
}
