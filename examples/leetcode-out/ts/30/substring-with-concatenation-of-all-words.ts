...
function example_1(): void {
	if (!(_equal(findSubstring("barfoothefoobarman", ["foo", "bar"]), [0, 9]))) { throw new Error('expect failed') }
}

function example_2(): void {
	if (!(_equal(findSubstring("wordgoodgoodgoodbestword", ["word", "good", "best", "word"]), []))) { throw new Error('expect failed') }
}

function example_3(): void {
	if (!(_equal(findSubstring("barfoofoobarthefoobarman", ["bar", "foo", "the"]), [6, 9, 12]))) { throw new Error('expect failed') }
}
