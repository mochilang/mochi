...
{
    if (!((mochi_removeNthFromEnd([1, 2, 3, 4, 5], 2) == [1, 2, 3, 5]))) {
...
{
    if (!((mochi_removeNthFromEnd([1, 2], 1) == [1]))) {
        throw new Exception('expect failed');
    }
}
...
{
    if (!((mochi_removeNthFromEnd([7, 8, 9], 3) == [8, 9]))) {
        throw new Exception('expect failed');
    }
}
...
{
    if (!((mochi_removeNthFromEnd([7, 8, 9], 1) == [7, 8]))) {
        throw new Exception('expect failed');
    }
}
