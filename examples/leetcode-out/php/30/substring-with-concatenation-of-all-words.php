...
{
    if (!((mochi_findSubstring("barfoothefoobarman", ["foo", "bar"]) == [0, 9]))) {
...
{
    if (!((mochi_findSubstring("wordgoodgoodgoodbestword", ["word", "good", "best", "word"]) == []))) {
        throw new Exception('expect failed');
    }
}
...
{
    if (!((mochi_findSubstring("barfoofoobarthefoobarman", ["bar", "foo", "the"]) == [6, 9, 12]))) {
        throw new Exception('expect failed');
    }
}
