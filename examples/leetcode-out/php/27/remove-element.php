...
function mochi_test_example_1()
{
    ...
    if (!(((is_array($nums) ? array_slice($nums, 0, ($k) - (0)) : substr($nums, 0, ($k) - (0))) == [2, 2]))) {
        throw new Exception('expect failed');
    }
}
...
function mochi_test_example_2()
{
    ...
    if (!(((is_array($nums) ? array_slice($nums, 0, ($k) - (0)) : substr($nums, 0, ($k) - (0))) == [0, 1, 3, 0, 4]))) {
        throw new Exception('expect failed');
    }
}
...
function mochi_test_no_removal()
{
    ...
    if (!(((is_array($nums) ? array_slice($nums, 0, ($k) - (0)) : substr($nums, 0, ($k) - (0))) == [1, 2, 3]))) {
        throw new Exception('expect failed');
    }
}
