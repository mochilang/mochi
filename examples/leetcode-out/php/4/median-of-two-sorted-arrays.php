...
function mochi_findMedianSortedArrays($nums1, $nums2)
{
    ...
    while ((($i < (is_array($nums1) ? count($nums1) : strlen($nums1))) || ($j < (is_array($nums2) ? count($nums2) : strlen($nums2))))) {
        ...
        } elseif (($i >= (is_array($nums1) ? count($nums1) : strlen($nums1)))) {
...
{
    if (!((mochi_findMedianSortedArrays([1, 3], [2]) == 2.0))) {
...
{
    if (!((mochi_findMedianSortedArrays([1, 2], [3, 4]) == 2.5))) {
        throw new Exception('expect failed');
    }
}
...
{
    if (!((mochi_findMedianSortedArrays([], [1]) == 1.0))) {
        throw new Exception('expect failed');
    }
}
...
{
    if (!((mochi_findMedianSortedArrays([2], []) == 2.0))) {
        throw new Exception('expect failed');
    }
}
