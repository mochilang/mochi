...
func cancellationRates(trips []Trip, users []User) []Result {
	...
	for _, d := range days {
		...
		if (total > 0) {
			...
			results = append(append([]Result{}, results...), []Result{Result{Day: d, CancellationRate: round2(rate)}}...)
		}
	}
	return results
}
