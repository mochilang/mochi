...
type Twitter struct {
	Tweets map[int][]Tweet `json:"tweets"`
	Follows map[int]map[int]bool `json:"follows"`
	Time int `json:"time"`
}

func newTwitter() Twitter {
	return Twitter{Tweets: _cast[map[int][]Tweet](map[any]any{}), Follows: _cast[map[int]map[int]bool](map[any]any{}), Time: 0}
}

func postTweet(t Twitter, userId int, tweetId int) Twitter {
	var tweets map[int][]Tweet = t.Tweets
	var list []Tweet = []Tweet{}
	_tmp0 := userId
	_tmp1 := tweets
	_, _tmp2 := _tmp1[_tmp0]
	if _tmp2 {
	...
	list = []Tweet{Tweet{Id: tweetId, Time: time}}
	tweets[userId] = list
...
func getNewsFeed(t Twitter, userId int) []int {
	...
	_, _tmp5 := _tmp4[_tmp3]
	if _tmp5 {
		all = append(append([]Tweet{}, all...), t.Tweets[userId]...)
	}
	_tmp6 := userId
	_tmp7 := t.Follows
	_, _tmp8 := _tmp7[_tmp6]
	if _tmp8 {
		for f := range t.Follows[userId] {
			...
			if _tmp11 {
				all = append(append([]Tweet{}, all...), t.Tweets[f]...)
			}
		}
	}
	var sorted []Tweet = func() []Tweet {
	items := []Tweet{}
	for _, tw := range all {
		items = append(items, tw)
	}
	type pair struct { item Tweet; key any }
	...
	for ((i < len(sorted)) && (i < 10)) {
		var t Tweet = sorted[i]
		_ = t
		feed = append(append([]int{}, feed...), []int{t.Id}...)
		i = (i + 1)
	}
	return feed
}
