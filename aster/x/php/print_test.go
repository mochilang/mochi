...
func TestPrint_Golden(t *testing.T) {
	...
	var selected []string
	for _, f := range files {
		if filepath.Base(f) == "two-sum.php" {
	...
	for _, src := range files {
		...
		t.Run(name, func(t *testing.T) {
			...
			jsonPath := filepath.Join(outDir, name+".php.json")
			if shouldUpdate() {
			...
			out, err := php.Print(prog)
			if err != nil {
				t.Fatalf("print: %v", err)
			}
			outPath := filepath.Join(outDir, name+".php")
			if shouldUpdate() {
				if err := os.WriteFile(outPath, []byte(out), 0644); err != nil {
					t.Fatalf("write out: %v", err)
				}
			}
			cmd := exec.Command("php", outPath)
			got, err := cmd.CombinedOutput()
			if err != nil {
				t.Fatalf("run printed: %v\n%s", err, got)
			}
			want, err := exec.Command("php", src).CombinedOutput()
			if err != nil {
