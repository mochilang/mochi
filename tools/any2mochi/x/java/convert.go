...
// convertExpr performs very basic cleanup of Java expressions so that they
// resemble Mochi syntax. It does not attempt full parsing.
func convertExpr(expr string, structs map[string][]string) string {
		...
		if strings.HasPrefix(body, "{") && strings.HasSuffix(body, "}") {
			...
			for _, p := range parts {
				...
				if strings.HasPrefix(p, "return ") {
					stmts = append(stmts, "return "+convertExpr(strings.TrimPrefix(p, "return "), structs))
				} else if isVarAssign(p + ";") {
					n, e := parseVarAssign(p + ";")
					stmts = append(stmts, "var "+n+" = "+convertExpr(e, structs))
				} else {
					stmts = append(stmts, convertExpr(p, structs))
				}
			}
			body = strings.Join(stmts, "; ")
		} else {
			body = convertExpr(body, structs)
		}
	...
	if strings.HasPrefix(expr, "new ") && strings.Contains(expr, "{") && strings.HasSuffix(expr, "}") {
		...
		for i, it := range items {
			items[i] = convertExpr(it, structs)
		}
		...
		if open != -1 {
			...
			if fields, ok := structs[name]; ok {
				...
				if len(args) == len(fields) {
					for i, a := range args {
						args[i] = fields[i] + ": " + convertExpr(a, structs)
					}
	...
	if strings.HasSuffix(expr, ".toCharArray()") {
		return convertExpr(strings.TrimSuffix(expr, ".toCharArray()"), structs)
	}

	if idx := strings.Index(expr, ".get("); idx != -1 && strings.HasSuffix(expr, ")") {
		name := expr[:idx]
		inner := strings.TrimSuffix(expr[idx+5:], ")")
		return name + "[" + convertExpr(inner, structs) + "]"
	}
	...
	if strings.HasSuffix(expr, ".length") {
		return "len(" + convertExpr(strings.TrimSuffix(expr, ".length"), structs) + ")"
	}
	...
	if strings.HasPrefix(expr, "_indexString(") && strings.HasSuffix(expr, ")") {
		...
		if len(parts) == 2 {
			return convertExpr(parts[0], structs) + "[" + convertExpr(parts[1], structs) + "]"
		}
	}
	...
	if strings.HasPrefix(expr, "_sliceString(") && strings.HasSuffix(expr, ")") {
		...
		parts := splitArgs(inner)
		if len(parts) == 3 {
			return convertExpr(parts[0], structs) + "[" + convertExpr(parts[1], structs) + ": " + convertExpr(parts[2], structs) + "]"
		}
	}
	...
	if strings.HasPrefix(expr, "_in(") && strings.HasSuffix(expr, ")") {
		...
		parts := splitArgs(inner)
		if len(parts) == 2 {
			return convertExpr(parts[0], structs) + " in " + convertExpr(parts[1], structs)
		}
	}
	...
	for _, b := range reBuiltin {
		if strings.HasPrefix(expr, b.old) && strings.HasSuffix(expr, ")") {
			...
			return b.new + convertExpr(inner, structs) + ")"
		}
	}
...
func mapLiteral(args string, structs map[string][]string) string {
	...
	for i := 0; i < len(parts); i += 2 {
		...
		b.WriteString(convertExpr(parts[i], structs))
		b.WriteString(": ")
		b.WriteString(convertExpr(parts[i+1], structs))
	}
	b.WriteByte('}')
	return b.String()
}
