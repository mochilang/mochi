...
// Parse converts Ruby source code into a Node tree using Ripper.sexp, which
// provides a higher level AST than the default SexpBuilder.
func Parse(src string) (*Node, error) {
	if _, err := exec.LookPath("ruby"); err != nil {
		return nil, fmt.Errorf("ruby not installed: %w", err)
	}
	cmd := exec.Command("ruby", "-e", `require 'json';require 'ripper';src=STDIN.read;ast=Ripper.sexp(src);puts JSON.generate(ast)`)
...
// its AST using the Ruby ripper parser.
