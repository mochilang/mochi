...
func main() {
	...
	if *show {
		for name, fn := range functions {
			...
			for _, ins := range fn.code {
				fmt.Printf("%s r=%d a=%d b=%d s=%q\n", ins.op, ins.r, ins.a, ins.b, ins.s)
			}
		}
		return
	}
	...
	run = func(fname string) string {
		...
		for ip < len(fn.code) {
			...
			case "PRINT":
				if ins.a < len(regs) {
					fmt.Println(regs[ins.a])
				} else {
					fmt.Println()
				}
			case "CALL":
				regs[ins.r] = run(ins.s)
			case "RET":
				if ins.a < len(regs) {
					return regs[ins.a]
				}
