...
func Generate(c Case) string {
	...
	if len(sel.SelectExprs) == 1 && len(sel.OrderBy) <= 1 {
		...
		sb.WriteString("\n  select " + expr + "\n")
		if c.RowSort {
			sb.WriteString("let pairs = from v in result\n  select [if v == null { \"~\" } else { str(v) }, v]\n")
			sb.WriteString("result = from p in pairs\n  order by p[0]\n  select p[1]\n")
		}
	...
	if len(sel.SelectExprs) > 1 {
		...
		sb.WriteString("  }\n}\n")
		if c.RowSort {
			sb.WriteString("let pairs = from v in flatResult\n  select [if v == null { \"~\" } else { str(v) }, v]\n")
			sb.WriteString("flatResult = from p in pairs\n  order by p[0]\n  select p[1]\n")
		}
