...
func compileAndRunErlang(t *testing.T, srcPath, wantPath, outDir, name string) {
	...
	if err := cmd.Run(); err != nil {
		writeErlangError(outDir, name, fmt.Errorf("run error: %v\n%s", err, buf.Bytes()))
		return
	}
	...
	if !bytes.Equal(got, want) {
		writeErlangError(outDir, name, fmt.Errorf("output mismatch\n-- got --\n%s\n-- want --\n%s", got, want))
		return
	}
...
func writeErlangError(dir, name string, err error) {
