...
// ConvertSource parses PHP source code and emits minimal Mochi stubs.
func ConvertSource(src string) (string, error) {
	prog, err := Parse(src)
	if err != nil {
		return "", formatParseError("", src, err)
	}
...
// Convert parses PHP source and returns a Mochi AST node.
...
func ConvertFile(path string) (string, error) {
	...
	out, err := ConvertSource(string(data))
	if err != nil {
