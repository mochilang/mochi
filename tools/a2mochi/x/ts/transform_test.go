...
func TestTransformGolden(t *testing.T) {
	...
	rootDir := repoRoot(t)
	pattern := filepath.Join(rootDir, "archived/tests/transpiler/x/ts", "*.ts")
...
func updateReadme() {
	...
	srcDir := filepath.Join(root, "archived", "tests", "archived", "transpiler", "x", "ts")
	...
	buf.WriteString("Each `.ts` source in `archived/tests/transpiler/x/ts` has a matching `.mochi` and `.ast`\n")
