...
func TestTransform_Golden(t *testing.T) {
	...
	root := repoRoot(t)
	pattern := filepath.Join(root, "archived/tests/transpiler/x/dart", "*.dart")
...
func updateReadme() {
	...
	buf.WriteString("Each `.dart` source in `archived/tests/transpiler/x/dart` has a matching `.mochi` and `.ast`\n")
