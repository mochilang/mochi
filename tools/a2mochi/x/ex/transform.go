...
func parseIfExpr(s string) (cond, thenExpr, elseExpr string, ok bool) {
	s = strings.TrimSpace(s)
	if !strings.HasPrefix(s, "if ") {
		return
	}
	re := regexp.MustCompile(`^if (.*?),\s*do:\s*(.*?)(?:,\s*else:\s*(.*))?$`)
...
func translateExpr(expr string) string {
	...
	if strings.HasPrefix(expr, "if rem(Enum.sum(") {
	...
	if cond, thenExpr, elseExpr, ok := parseIfExpr(expr); ok {
		out := "if " + translateExpr(cond) + " then " + translateExpr(thenExpr)
		if elseExpr != "" {
			out += " else " + translateExpr(elseExpr)
		}
		return out
	}
	switch {
