...
var mochiLexer = lexer.MustSimple([]lexer.SimpleRule{
	...
	{Name: "Keyword", Pattern: `\b(test|expect|agent|intent|on|stream|emit|type|fun|extern|import|return|break|continue|let|var|if|else|then|for|while|in|generate|match|when|fetch|load|save|package|export|fact|rule|all|none|try|catch|not|set|omap|async|await)\b`},
